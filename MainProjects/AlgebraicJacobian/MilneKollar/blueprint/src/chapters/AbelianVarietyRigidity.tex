...
\begin{proof}
  ...
  By \cref{lem:rigidity_eqOn_dense_open}, the collapse hypothesis gives a nonempty
...
\begin{proof}
  ...
  \cref{lem:snd_left_isClosedMap}, the set \(G\) is closed.  The collapsed fibre is
  ...
  \cref{lem:rigidity_eqOn_saturated_open_to_affine} to \(U\).
\end{proof}
...
\begin{proof}
  \leanok
  \uses{lem:rigidity_eqAt_closedPoint_of_proper_into_affine,
    lem:morphism_eq_of_eqAt_closedPoints}
  ...
  \cref{lem:rigidity_eqAt_closedPoint_of_proper_into_affine}.  The source is reduced
  and the target is separated, so
  \cref{lem:morphism_eq_of_eqAt_closedPoints} promotes these pointwise equalities to
...
\begin{proof}
  \leanok
  \uses{lem:eq_comp_of_isAffine_of_properIntegral,
  ...
  \cref{lem:isIntegral_of_retract_of_integral}; it is also proper and locally of
  finite type over \(\bar k\).  The two points of \(X_y\) represented by \(x\) and
  \((x_0,y)\) therefore have the same image in \(U_0\) by
  \cref{lem:eq_comp_of_isAffine_of_properIntegral}.  Restricting this equality along
...
\begin{proof}
  ...
  Existence is \cref{lem:hom_additivity_over_product}.  Restricting a decomposition to
...
\begin{proof}
  ...
  axes.  Apply \cref{lem:hom_additivity_over_product} to \(\varphi\).  Both canonical

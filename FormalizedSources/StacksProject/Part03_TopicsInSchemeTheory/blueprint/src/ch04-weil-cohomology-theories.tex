...
Section \ref{weil-section-correspondences} the symmetric monoidal
graded category whose objects are smooth projective schemes and
whose morphisms are correspondences. In Section \ref{weil-section-chow-motives}
...
see Section \ref{weil-section-axioms-classical}. This notion is the
...
At the end of Section \ref{weil-section-axioms-classical} we show that
...
\medskip\noindent
In Section \ref{weil-section-cycles-nonclosed} we prove a couple of lemmas
...
discussion in Section \ref{weil-section-axioms-classical}
up to choosing a basis element of $F(1)$.
\end{enumerate}
First, in Section \ref{weil-section-axioms} we analyze the first two conditions.
After developing a few more results in Section \ref{weil-section-further}
in Section \ref{weil-section-old} we add the necessary axioms to obtain
property (3).

\medskip\noindent
In the final Section \ref{weil-section-c1} we detail an alternative approach
...
$$
Given $c \in \text{Corr}^r(X, Y)$ and $\beta \in \CH_j(Y) \otimes \mathbf{Q}$
...
dimension $d + j$ on $X_d \times Y$, hence $c \cdot \text{pr}_2^*\beta$
is a cycle of dimension $j - r$ on $X_d \times Y$ whose pushforward
...
\begin{lemma}[Composition correspondences]
\label{weil-lemma-composition-correspondences}
\source{stacks:0FG0}
\href{https://stacks.math.columbia.edu/tag/0FG0}{\texttt{Stacks tag 0FG0}}
\group{weil}
...
\begin{example}[Graph correspondence]
\label{weil-example-graph-correspondence}
\source{stacks:0FG1}
\href{https://stacks.math.columbia.edu/tag/0FG1}{\texttt{Stacks tag 0FG1}}
\group{weil}
...
\begin{lemma}[Category correspondences]
\label{weil-lemma-category-correspondences}
\source{stacks:0FG2}
\href{https://stacks.math.columbia.edu/tag/0FG2}{\texttt{Stacks tag 0FG2}}
\group{weil}
...
\begin{proof}
\uses{weil-lemma-composition-correspondences}

Everything is clear from the construction and
Lemma \ref{weil-lemma-composition-correspondences}
...
\source{stacks:0FG3}
\href{https://stacks.math.columbia.edu/tag/0FG3}{\texttt{Stacks tag 0FG3}}
\group{weil}
...
\begin{proof}
\uses{weil-lemma-category-correspondences}

In the proof of Lemma \ref{weil-lemma-category-correspondences}
...
Lemma \ref{weil-lemma-category-correspondences} we see that it
suffices to show
$$
[\Gamma_{f \circ g}] =
...
\begin{remark}[Transpose]
\label{weil-remark-transpose}
\source{stacks:0FG4}
\href{https://stacks.math.columbia.edu/tag/0FG4}{\texttt{Stacks tag 0FG4}}
\group{weil}
...
\begin{lemma}[Functor and cycles]
\label{weil-lemma-functor-and-cycles}
\source{stacks:0FG5}
\href{https://stacks.math.columbia.edu/tag/0FG5}{\texttt{Stacks tag 0FG5}}
\group{weil}

\uses{weil-example-graph-correspondence, weil-remark-transpose}
Let $f : Y \to X$ be a morphism of smooth projective schemes over $k$.
Let $[\Gamma_f] \in \text{Corr}^0(X, Y)$ be as in
Example \ref{weil-example-graph-correspondence}. Then
\begin{enumerate}
...
$[\Gamma_f^t]$ of Remark \ref{weil-remark-transpose}
...
\begin{example}[Decompose P1]
\label{weil-example-decompose-P1}
\source{stacks:0FG6}
\href{https://stacks.math.columbia.edu/tag/0FG6}{\texttt{Stacks tag 0FG6}}
\group{weil}
...
\source{stacks:0FG7}
\href{https://stacks.math.columbia.edu/tag/0FG7}{\texttt{Stacks tag 0FG7}}
\group{weil}
...
\begin{lemma}[Prep dual]
\label{weil-lemma-prep-dual}
\source{stacks:0FG8}
\href{https://stacks.math.columbia.edu/tag/0FG8}{\texttt{Stacks tag 0FG8}}
\group{weil}
...
$$
\begin{array}{ccc}X \otimes Y \underset{a \otimes \text{id}}{\longrightarrow}\underset{\text{id} \otimes a^t}{\downarrow} & Y \otimes Y \underset{\eta_Y}{\longrightarrow} & Y \overset{[Y]}{\downarrow} \\ X \otimes X \overset{\eta_X}{\longrightarrow} & X \overset{[X]}{\longrightarrow} & \Spec(k)\end{array}
$$
is commutative in the category of correspondences.
\end{lemma}

\begin{proof}
\uses{weil-lemma-composition-correspondences, weil-lemma-functor-and-cycles}
...
$\CH_{-r}(W) \to \CH_{-r - s}(W')$
(Lemma \ref{weil-lemma-composition-correspondences}).
Finally, we have Lemma \ref{weil-lemma-functor-and-cycles}
...
Example \ref{weil-example-decompose-P1} is invertible in $M_k$, see
Lemma \ref{weil-lemma-characterize-motives}.
At the end of the section we will show that every motive, i.e.,
every object of $M_k$ to has a (left) dual, see Lemma \ref{weil-lemma-dual-general}.
...
\source{stacks:0FGA}
\href{https://stacks.math.columbia.edu/tag/0FGA}{\texttt{Stacks tag 0FGA}}
\group{weil}
...
\begin{proof}
\uses{weil-lemma-contravariant-functor}

Follows immediately from Lemma \ref{weil-lemma-contravariant-functor}.
\end{proof}

\begin{lemma}[Karoubian]
\label{weil-lemma-Karoubian}
\source{stacks:0FGB}
\href{https://stacks.math.columbia.edu/tag/0FGB}{\texttt{Stacks tag 0FGB}}
\group{weil}
...
On morphisms, we use
$$
\begin{array}{c}\Mor((X, p, m), (Y, q, n)) \times
\Mor((X', p', m'), (Y', q', n')) \downarrow \\ \Mor(
(X \times X', p \otimes p', m + m'),
(Y \times Y', q \otimes q', n + n'))\end{array}
$$
given by the rule $(a, a') \longmapsto a \otimes a'$ where
$\otimes$ on correspondences is as in Section \ref{weil-section-correspondences}.
...
\begin{lemma}[Motives monoidal]
\label{weil-lemma-motives-monoidal}
\source{stacks:0FGC}
\href{https://stacks.math.columbia.edu/tag/0FGC}{\texttt{Stacks tag 0FGC}}
\group{weil}
...
\begin{proof}
\uses{weil-lemma-tensor-product}

Follows readily from Lemma \ref{weil-lemma-tensor-product}. Details omitted.
\end{proof}
...
\begin{lemma}[Inverse h2]
\label{weil-lemma-inverse-h2}
\source{stacks:0FGD}
\href{https://stacks.math.columbia.edu/tag/0FGD}{\texttt{Stacks tag 0FGD}}
\group{weil}

\uses{weil-example-decompose-P1}
With notation as in Example \ref{weil-example-decompose-P1}
\begin{enumerate}
\item
...
We will use Lemma \ref{weil-lemma-contravariant-functor} without further mention.
...
\source{stacks:0FGE}
\href{https://stacks.math.columbia.edu/tag/0FGE}{\texttt{Stacks tag 0FGE}}
\group{weil}

\uses{weil-example-decompose-P1, weil-lemma-inverse-h2}
Let $X = \mathbf{P}^1_k$ and $c_2$ be as in Example \ref{weil-example-decompose-P1}.
...
may be used to denote it. By Lemma \ref{weil-lemma-inverse-h2} the Lefschetz motive
...
\begin{lemma}[Additive]
\label{weil-lemma-additive}
\source{stacks:0FGF}
\href{https://stacks.math.columbia.edu/tag/0FGF}{\texttt{Stacks tag 0FGF}}
\group{weil}
...
Example \ref{weil-example-decompose-P1}. Then we consider
\begin{align*}
(Z, q, n)
& =
...
\end{align*}
where we have used Lemma \ref{weil-lemma-inverse-h2}.
This reduces us to the case discussed in the first paragraph.
\end{proof}

\begin{lemma}[Decompose P1]
\label{weil-lemma-decompose-P1}
\source{stacks:0FGG}
\href{https://stacks.math.columbia.edu/tag/0FGG}{\texttt{Stacks tag 0FGG}}
\group{weil}
...
This follows from Example \ref{weil-example-decompose-P1} and
Lemma \ref{weil-lemma-inverse-h2}.
\end{proof}

\begin{lemma}[Characterize motives]
\label{weil-lemma-characterize-motives}
\source{stacks:0FGH}
\href{https://stacks.math.columbia.edu/tag/0FGH}{\texttt{Stacks tag 0FGH}}
\group{weil}

\uses{weil-example-decompose-P1}
Let $X$, $c_2$ be as in Example \ref{weil-example-decompose-P1}.
...
(Z \times X^{m - n}, q \otimes c_2 \otimes \ldots \otimes c_2, m)
$$
(see proof of Lemma \ref{weil-lemma-additive}). Composing $a$ with this isomorphism
gives
$$
\sigma \circ a \in
\Hom((Y, p, m),
...
\source{stacks:0FGI}
\href{https://stacks.math.columbia.edu/tag/0FGI}{\texttt{Stacks tag 0FGI}}
\group{weil}
...
We will use Lemma \ref{weil-lemma-composition-correspondences}
...
\begin{lemma}[Dual general]
\label{weil-lemma-dual-general}
\source{stacks:0FGJ}
\href{https://stacks.math.columbia.edu/tag/0FGJ}{\texttt{Stacks tag 0FGJ}}
\group{weil}
...
This follows from Lemma \ref{weil-lemma-dual}.
\end{proof}
...
\begin{definition}[Chow group motives]
\label{weil-definition-chow-group-motives}
\source{stacks:0FGL}
\href{https://stacks.math.columbia.edu/tag/0FGL}{\texttt{Stacks tag 0FGL}}
\group{weil}
...
(Section \ref{weil-section-correspondences}) is a family of maps
$$
c_* :
...
morphisms of motives by Lemma \ref{weil-lemma-composition-correspondences}.
This functoriality of Chow groups can also be deduced from the following
lemma.

\begin{lemma}[Chow groups representable]
\label{weil-lemma-chow-groups-representable}
\source{stacks:0FGM}
\href{https://stacks.math.columbia.edu/tag/0FGM}{\texttt{Stacks tag 0FGM}}
\group{weil}
...
Lemma \ref{weil-lemma-chow-groups-representable}, the Yoneda lemma, and
the duality in Lemma \ref{weil-lemma-dual} to obtain the following.

\begin{lemma}[Manin]

\label{weil-lemma-manin}
\source{stacks:0FGN}
\href{https://stacks.math.columbia.edu/tag/0FGN}{\texttt{Stacks tag 0FGN}}
\group{weil}
...
\begin{proof}
\uses{weil-lemma-chow-groups-representable, weil-lemma-dual-general}
...
Lemma \ref{weil-lemma-chow-groups-representable}
we see that
$$
...
(Lemma \ref{weil-lemma-dual-general}) we conclude that
$$
...
$$
\begin{array}{cc}P = \mathbf{P}(\mathcal{E}) =
\underline{\text{Proj}}_X(\text{Sym}^*(\mathcal{E}))
\overset{p}{\longrightarrow} & X\end{array}
$$
...
$$
See Example \ref{weil-example-graph-correspondence}.
...
\source{stacks:0FGQ}
\href{https://stacks.math.columbia.edu/tag/0FGQ}{\texttt{Stacks tag 0FGQ}}
\group{weil}
...
\begin{proof}
\uses{weil-lemma-manin, weil-lemma-functor-and-cycles}

By Lemma \ref{weil-lemma-manin} it suffices to show that
...
of cycles by $p$ according to Lemma \ref{weil-lemma-functor-and-cycles} and
...
(Lemma \ref{weil-lemma-projective-space-bundle-formula})
that also the composition the other way around is
invertible, but it seems a bit harder to prove this directly.

\begin{lemma}[Diagonal projective bundle]
\label{weil-lemma-diagonal-projective-bundle}
\source{stacks:0FGR}
\href{https://stacks.math.columbia.edu/tag/0FGR}{\texttt{Stacks tag 0FGR}}
\group{weil}

\uses{weil-lemma-projective-space-bundle-formula}
Let $p : P \to X$ be as in Lemma \ref{weil-lemma-projective-space-bundle-formula}.
...
\begin{lemma}[Pushforward classical]
\label{weil-lemma-pushforward-classical}
\source{stacks:0FGT}
\href{https://stacks.math.columbia.edu/tag/0FGT}{\texttt{Stacks tag 0FGT}}
\group{weil}
...
\begin{remark}[Replace cup product classical]
\label{weil-remark-replace-cup-product-classical}
\source{stacks:0FGU}
\href{https://stacks.math.columbia.edu/tag/0FGU}{\texttt{Stacks tag 0FGU}}
\group{weil}
...
\begin{definition}[Weil cohomology theory classical]
\label{weil-definition-weil-cohomology-theory-classical}
\source{stacks:0FGV}
\href{https://stacks.math.columbia.edu/tag/0FGV}{\texttt{Stacks tag 0FGV}}
\group{weil}
...
\begin{lemma}[Degrees cycles classical]
\label{weil-lemma-degrees-cycles-classical}
\source{stacks:0FGW}
\href{https://stacks.math.columbia.edu/tag/0FGW}{\texttt{Stacks tag 0FGW}}
\group{weil}

\uses{weil-definition-weil-cohomology-theory-classical}
Let $H^*$ be a classical Weil cohomology theory
(Definition \ref{weil-definition-weil-cohomology-theory-classical}).
Let $X$ be a smooth projective variety of dimension $d$. The diagram
$$
\begin{array}{cc}\CH^d(X) \underset{\gamma}{\longrightarrow}\downarrow & H^{2d}(X) \overset{\int_X}{\downarrow} \\ \CH_0(X) \overset{\deg}{\longrightarrow} & F\end{array}
$$
...
\begin{lemma}[Trace product classical]
\label{weil-lemma-trace-product-classical}
\source{stacks:0FGX}
\href{https://stacks.math.columbia.edu/tag/0FGX}{\texttt{Stacks tag 0FGX}}
\group{weil}
...
\begin{lemma}[Pr2star classical]
\label{weil-lemma-pr2star-classical}
\source{stacks:0FGY}
\href{https://stacks.math.columbia.edu/tag/0FGY}{\texttt{Stacks tag 0FGY}}
\group{weil}
...
\begin{proof}
\uses{weil-lemma-trace-product-classical}

This is equivalent to the result of Lemma \ref{weil-lemma-trace-product-classical}.
\end{proof}

\begin{lemma}[Class diagonal classical]
\label{weil-lemma-class-diagonal-classical}
\source{stacks:0FGZ}
\href{https://stacks.math.columbia.edu/tag/0FGZ}{\texttt{Stacks tag 0FGZ}}
\group{weil}
...
\begin{proof}
\uses{weil-remark-replace-cup-product-classical, weil-lemma-degrees-cycles-classical, weil-lemma-trace-product-classical}

Recall that $\Delta^* : H^*(X \times X) \to H^*(X)$ is equal to the
cup product map $H^*(X) \otimes_F H^*(X) \to H^*(X)$, see
Remark \ref{weil-remark-replace-cup-product-classical}. On the other hand we have
...
of $\gamma([x])$ by Lemma \ref{weil-lemma-degrees-cycles-classical}.
Hence
$$
...
$$
by Lemma \ref{weil-lemma-trace-product-classical}; note that we made
...
\begin{lemma}[Square diagonal classical]
\label{weil-lemma-square-diagonal-classical}
\source{stacks:0FH0}
\href{https://stacks.math.columbia.edu/tag/0FH0}{\texttt{Stacks tag 0FH0}}
\group{weil}
...
\begin{proof}
\uses{weil-lemma-degrees-cycles-classical, weil-lemma-class-diagonal-classical}
...
\medskip\noindent
Equality on the left. By Lemma \ref{weil-lemma-degrees-cycles-classical} we have
\begin{align*}
...
as in Lemma \ref{weil-lemma-class-diagonal-classical}.
...
\begin{lemma}[From functor to weil classical]
\label{weil-lemma-from-functor-to-weil-classical}
\source{stacks:0FH1}
\href{https://stacks.math.columbia.edu/tag/0FH1}{\texttt{Stacks tag 0FH1}}
\group{weil}
...
\begin{proof}
\uses{weil-lemma-composition-correspondences, weil-lemma-dual, weil-lemma-functor-and-cycles, weil-lemma-prep-dual, weil-remark-replace-cup-product-classical}
...
Lemma \ref{weil-lemma-composition-correspondences} we have
$$
\CH^r(X) \otimes \mathbf{Q} = \text{Corr}^r(\Spec(k), X) =
\Hom(\mathbf{1}(-r), h(X))
$$
...
In the proof of Lemma \ref{weil-lemma-dual} we have seen that
...
Lemma \ref{weil-lemma-functor-and-cycles}. Hence
...
be as in Lemma \ref{weil-lemma-prep-dual}.
...
Lemmas \ref{weil-lemma-functor-and-cycles} and
...
\mathbf{1}(r - e)
$$
This follows immediately from Lemma \ref{weil-lemma-prep-dual}.
Thus we have axiom (C)(b).

\medskip\noindent
To prove axiom (C)(c) we use the discussion in
Remark \ref{weil-remark-replace-cup-product-classical}.
...
\begin{lemma}[From weil to functor classical]
\label{weil-lemma-from-weil-to-functor-classical}
\source{stacks:0FH2}
\href{https://stacks.math.columbia.edu/tag/0FH2}{\texttt{Stacks tag 0FH2}}
\group{weil}
...
\begin{proof}
\uses{weil-lemma-characterize-motives, weil-lemma-pr2star-classical, weil-lemma-pushforward-classical, weil-lemma-category-correspondences, weil-lemma-class-diagonal-classical, weil-lemma-square-diagonal-classical, weil-example-decompose-P1, weil-lemma-inverse-h2}

By Lemma \ref{weil-lemma-characterize-motives} it suffices to construct a functor
...
along projections given in Lemma \ref{weil-lemma-pr2star-classical}.
The third equality holds by Lemma \ref{weil-lemma-pushforward-classical}
...
Lemma \ref{weil-lemma-category-correspondences} and its proof),
then we have to show that $G([\Delta]) = \text{id}$.
This follows from the determination of
$\gamma([\Delta])$ in Lemma \ref{weil-lemma-class-diagonal-classical}
and Lemma \ref{weil-lemma-pr2star-classical}.
...
By Lemma \ref{weil-lemma-square-diagonal-classical} and axiom (A)(c)
...
Example \ref{weil-example-decompose-P1}. We have $c_0 = a \circ b$ where
...
Lemma \ref{weil-lemma-inverse-h2}. Since $F = G(\Spec(k))$, it follows from
...
\begin{proposition}[Weil cohomology theory classical]
\label{weil-proposition-weil-cohomology-theory-classical}
\source{stacks:0FH3}
\href{https://stacks.math.columbia.edu/tag/0FH3}{\texttt{Stacks tag 0FH3}}
\group{weil}
...
\begin{proof}
\uses{weil-lemma-from-functor-to-weil-classical, weil-lemma-from-weil-to-functor-classical, weil-example-decompose-P1, weil-lemma-inverse-h2}
...
Lemma \ref{weil-lemma-from-functor-to-weil-classical} and its proof.
...
Lemma \ref{weil-lemma-from-weil-to-functor-classical}.
Let $X = \mathbf{P}^1, c_0, c_2$ be as in Example \ref{weil-example-decompose-P1}.
We have constructed an isomorphism $1(-1) \to (X, c_2, 0)$ of motives in
Lemma \ref{weil-lemma-inverse-h2}. In the proof of
Lemma \ref{weil-lemma-from-weil-to-functor-classical} we have seen that
...
\begin{lemma}[Generated by separable]
\label{weil-lemma-generated-by-separable}
\source{stacks:0FH5}
\href{https://stacks.math.columbia.edu/tag/0FH5}{\texttt{Stacks tag 0FH5}}
\group{weil}
...
\source{stacks:0FH6}
\href{https://stacks.math.columbia.edu/tag/0FH6}{\texttt{Stacks tag 0FH6}}
\group{weil}
...
\begin{lemma}[Divide difference points]
\label{weil-lemma-divide-difference-points}
\source{stacks:0FH7}
\href{https://stacks.math.columbia.edu/tag/0FH7}{\texttt{Stacks tag 0FH7}}
\group{weil}
...
\begin{lemma}[Kernel to closure]
\label{weil-lemma-kernel-to-closure}
\source{stacks:0FH8}
\href{https://stacks.math.columbia.edu/tag/0FH8}{\texttt{Stacks tag 0FH8}}
\group{weil}
...
constructed in Lemma \ref{weil-lemma-chow-limit}
is torsion.
\end{lemma}
...
\source{stacks:0FH9}
\href{https://stacks.math.columbia.edu/tag/0FH9}{\texttt{Stacks tag 0FH9}}
\group{weil}
...
\begin{proof}
\uses{weil-lemma-kernel-to-closure, weil-lemma-divide-difference-points}
...
is torsion by Lemma \ref{weil-lemma-kernel-to-closure}.
...
$x$ and $x'$. See proof of Lemma \ref{weil-lemma-divide-difference-points}.
...
This section is the analogue of Section \ref{weil-section-axioms-classical}
...
$G(\mathbf{1}(1))$ sits in degree $-2$. In Section \ref{weil-section-old}
...
\source{stacks:0FHB}
\href{https://stacks.math.columbia.edu/tag/0FHB}{\texttt{Stacks tag 0FHB}}
\group{weil}
...
\source{stacks:0FHC}
\href{https://stacks.math.columbia.edu/tag/0FHC}{\texttt{Stacks tag 0FHC}}
\group{weil}
...
\begin{remark}[Replace cup product]
\label{weil-remark-replace-cup-product}
\source{stacks:0FHD}
\href{https://stacks.math.columbia.edu/tag/0FHD}{\texttt{Stacks tag 0FHD}}
\group{weil}
...
\source{stacks:0FHE}
\href{https://stacks.math.columbia.edu/tag/0FHE}{\texttt{Stacks tag 0FHE}}
\group{weil}
...
\source{stacks:0FHF}
\href{https://stacks.math.columbia.edu/tag/0FHF}{\texttt{Stacks tag 0FHF}}
\group{weil}
...
and Lemma \ref{weil-lemma-base}. Let $X' \subset X$ be an irreducible component.
...
Using axiom (C)(b) twice and Lemma \ref{weil-lemma-base}
we conclude that
$$
...
\begin{lemma}[Push unit]
\label{weil-lemma-push-unit}
\source{stacks:0FHG}
\href{https://stacks.math.columbia.edu/tag/0FHG}{\texttt{Stacks tag 0FHG}}
\group{weil}
...
\begin{proof}
\uses{weil-lemma-unit}

This is true because $1 = \gamma([X])$ in $H^0(X)$ by Lemma \ref{weil-lemma-unit}
and then we can apply axiom (C)(b).
\end{proof}

\begin{lemma}[Class diagonal]
\label{weil-lemma-class-diagonal}
\source{stacks:0FHH}
\href{https://stacks.math.columbia.edu/tag/0FHH}{\texttt{Stacks tag 0FHH}}
\group{weil}
...
\begin{proof}
\uses{weil-remark-replace-cup-product, weil-lemma-push-unit, weil-lemma-pushforward}

Recall that $\Delta^* : H^*(X \times X) \to H^*(X)$ is equal to the
cup product map $H^*(X) \otimes_F H^*(X) \to H^*(X)$, see
Remark \ref{weil-remark-replace-cup-product}. On the other
hand, recall that $\gamma([\Delta]) = \Delta_*1$ (Lemma \ref{weil-lemma-push-unit})
and hence
$$
\int_{X \times X} \gamma([\Delta]) \cup a \otimes b =
\int_{X \times X} \Delta_*1 \cup a \otimes b =
\int_X a \cup b
$$
by Lemma \ref{weil-lemma-pushforward}.
On the other hand, we have
$$
\int_{X \times X} (\sum e_{i, j} \otimes e'_{2d -i , j}) \cup a \otimes b =
\sum (\int_X a \cup e_{i, j})(\int_X e'_{2d - i, j} \cup b)
...
\begin{lemma}[Cohomology P1]
\label{weil-lemma-cohomology-P1}
\source{stacks:0FHI}
\href{https://stacks.math.columbia.edu/tag/0FHI}{\texttt{Stacks tag 0FHI}}
\group{weil}
...
\begin{proof}
\uses{weil-lemma-class-diagonal, weil-lemma-unit}
...
However, by Lemma \ref{weil-lemma-class-diagonal}
...
By Lemma \ref{weil-lemma-unit} we have $H^0(\mathbf{P}^1_k) \not = 0$.
By Poincar\'e duality, more precisely axiom (A)(b),
we have $\beta_0 = \beta_2$. Therefore the lemma holds.
\end{proof}

\begin{lemma}[Weil additive]
\label{weil-lemma-weil-additive}
\source{stacks:0FHJ}
\href{https://stacks.math.columbia.edu/tag/0FHJ}{\texttt{Stacks tag 0FHJ}}
\group{weil}
...
\begin{proof}
\uses{weil-lemma-unit, weil-lemma-push-unit, weil-lemma-pushforward}

If $X$ or $Y$ is empty, then this is true because
$H^*(\emptyset) = 0$ by Lemma \ref{weil-lemma-unit}.
...
Lemmas \ref{weil-lemma-unit} and \ref{weil-lemma-push-unit} and axiom (C)(c).
Then we see that
$$
a = a \cup 1 = a \cup i_*1 + a \cup j_*1 =
i_*(i^*a) + j_*(j^*a)
$$
by the projection formula (Lemma \ref{weil-lemma-pushforward}) and hence the map
is injective.
...
and nonvanishing of cohomology (Lemma \ref{weil-lemma-unit})
...
\begin{lemma}[From functor to weil]
\label{weil-lemma-from-functor-to-weil}
\source{stacks:0FHK}
\href{https://stacks.math.columbia.edu/tag/0FHK}{\texttt{Stacks tag 0FHK}}
\group{weil}
...
\begin{proof}
\uses{weil-lemma-from-functor-to-weil-classical, weil-lemma-composition-correspondences, weil-lemma-dual, weil-lemma-functor-and-cycles, weil-lemma-prep-dual, weil-remark-replace-cup-product-classical}

This proof is the same as the proof of
Lemma \ref{weil-lemma-from-functor-to-weil-classical};
...
nonempty and equidimensional of dimension $d$. By Lemma \ref{weil-lemma-dual}
we know that $h(X)(d)$ is a left dual to $h(X)$. Hence
...
\begin{lemma}[From weil to functor]
\label{weil-lemma-from-weil-to-functor}
\source{stacks:0FHL}
\href{https://stacks.math.columbia.edu/tag/0FHL}{\texttt{Stacks tag 0FHL}}
\group{weil}
...
\begin{proof}
\uses{weil-lemma-from-weil-to-functor-classical, weil-lemma-characterize-motives, weil-lemma-weil-additive, weil-lemma-unit, weil-lemma-pr2star, weil-lemma-pushforward, weil-lemma-category-correspondences, weil-lemma-class-diagonal, weil-lemma-base, weil-lemma-cohomology-P1, weil-example-decompose-P1, weil-lemma-inverse-h2}
...
of dimension $d$. By Lemma \ref{weil-lemma-weil-additive} we have correspondingly
$$
...
if $X = \emptyset$ (Lemma \ref{weil-lemma-unit}). Thus maps
...
\text{pr}^{123}_{23, *} \circ \text{pr}^{123, *}_{12}$
as follows immediately from the description of pushforward
along projections given in Lemma \ref{weil-lemma-pr2star}.
The third equality holds by Lemma \ref{weil-lemma-pushforward}
and the fact that $H^*$ is a functor.
The fourth equalith holds by axiom (C)(a) and the fact that
the Gysin map agrees with flat pullback for flat morphisms
(Chow Homology, Lemma \ref{chow-lemma-lci-gysin-flat}).
The fifth equality uses axiom (C)(c) as well as
Lemma \ref{weil-lemma-pushforward} to see that
$\text{pr}^{23}_{3, *} \circ \text{pr}^{123}_{23, *} =
\text{pr}^{13}_{3, *} \circ \text{pr}^{123}_{13, *}$.
The sixth equality uses the projection formula from
Lemma \ref{weil-lemma-pushforward} as well as
axiom (C)(b) to see that $
\text{pr}^{123}_{13, *}
\gamma(\text{pr}^{123, *}_{23}c' \cdot \text{pr}^{123, *}_{12}c) =
\gamma(\text{pr}^{123}_{13, *}(
...
of correspondences (Lemma \ref{weil-lemma-category-correspondences}),
then we have to show that $G([\Delta]) = \text{id}$.
This follows from the determination
of $\gamma([\Delta])$ in Lemma \ref{weil-lemma-class-diagonal}
and Lemma \ref{weil-lemma-pr2star}.
This finishes the construction of $G$ as a functor on
smooth projective schemes over $k$ and correspondences of degree $0$.

\medskip\noindent
By Lemma \ref{weil-lemma-base} we have that
$G(\Spec(k)) = H^*(\Spec(k))$ is canonically isomorphic to $F$
...
is an invertible graded $F$-vector space (in particular we don't know yet
that $G$ extends to $M_k$). By Lemma \ref{weil-lemma-cohomology-P1}
...
\begin{proof}
\uses{weil-lemma-from-functor-to-weil, weil-lemma-from-weil-to-functor}

Given $G$ as in (2) by setting $H^*(X) = G(h(X))$ we obtain data
(D0), (D1), (D2), and (D3) satisfying (A), (B), and (C),
see Lemma \ref{weil-lemma-from-functor-to-weil} and its proof.

\medskip\noindent
Conversely, given data (D0), (D1), (D2), and (D3)
satisfying (A), (B), and (C) we get a functor $G$ as in (2)
by the construction of the proof of Lemma \ref{weil-lemma-from-weil-to-functor}.
...
\begin{lemma}[Trace disjoint union]
\label{weil-lemma-trace-disjoint-union}
\source{stacks:0FHP}
\href{https://stacks.math.columbia.edu/tag/0FHP}{\texttt{Stacks tag 0FHP}}
\group{weil}
...
\begin{proof}
\uses{weil-lemma-weil-additive}

Denote $i : X \to X \amalg Y$ and $j : Y \to X \amalg Y$ be the coprojections.
By Lemma \ref{weil-lemma-weil-additive} the map
...
the proof of Lemma \ref{weil-lemma-weil-additive}.
\end{proof}

\begin{lemma}[Dim 0]
\label{weil-lemma-dim-0}
\source{stacks:0FHQ}
\href{https://stacks.math.columbia.edu/tag/0FHQ}{\texttt{Stacks tag 0FHQ}}
\group{weil}
...
\begin{proof}
\uses{weil-lemma-weil-additive, weil-lemma-unit, weil-lemma-trace-disjoint-union, weil-lemma-pr2star}
...
$$
Setting $Y = \Spec(k'')$ axioms (B)(a) and Lemma \ref{weil-lemma-weil-additive} give
$$
H^*(X) \otimes_F H^*(Y) =
\prod\nolimits_{\sigma \in \Sigma} H^*(Y)
$$
as graded commutative $F$-algebras. By Lemma \ref{weil-lemma-unit} the
...
Lemma \ref{weil-lemma-trace-disjoint-union}. Thus
$$
\int_X a = \text{pr}_{1, *}(a \otimes 1) = \sum \Spec(\sigma)^*a
$$
in $H^0(Y)$; the first equality by Lemma \ref{weil-lemma-pr2star}
...
By Lemma \ref{weil-lemma-unit} we have $\gamma([X]) = 1$.
Finally, we have $\int_X \gamma([X]) = \deg(X \to \Spec(k))$
because $\gamma([X]) = 1$ and the trace of $1$ is equal to $[k' : k]$
\end{proof}

\begin{lemma}[Degrees cycles]
\label{weil-lemma-degrees-cycles}
\source{stacks:0FHR}
\href{https://stacks.math.columbia.edu/tag/0FHR}{\texttt{Stacks tag 0FHR}}
\group{weil}
...
$$
\begin{array}{cc}\CH^d(X) \underset{\gamma}{\longrightarrow}\downarrow & H^{2d}(X)(d) \overset{\int_X}{\downarrow} \\ \CH_0(X) \overset{\deg}{\longrightarrow} & F\end{array}
$$
commutes where $\deg : \CH_0(X) \to \mathbf{Z}$ is the degree of
zero cycles discussed in Chow Homology, Section
\ref{chow-section-degree-zero-cycles}.
...
\begin{proof}
\uses{weil-lemma-dim-0, weil-lemma-generated-by-separable}
...
Lemma \ref{weil-lemma-dim-0}. This proves the lemma
because $\CH_0(X)$ is generated by the classes of points $x$ as above
by Lemma \ref{weil-lemma-generated-by-separable}.
\end{proof}

\begin{lemma}[Square diagonal]
\label{weil-lemma-square-diagonal}
\source{stacks:0FHS}
\href{https://stacks.math.columbia.edu/tag/0FHS}{\texttt{Stacks tag 0FHS}}
\group{weil}
...
\begin{proof}
\uses{weil-lemma-degrees-cycles, weil-lemma-push-unit, weil-lemma-pushforward, weil-lemma-class-diagonal, weil-remark-replace-cup-product}
...
\medskip\noindent
Equality on the left. By Lemma \ref{weil-lemma-degrees-cycles} we have
\begin{align*}
\deg([\Delta] \cdot [\Delta])
& =
\int_{X \times X} \gamma([\Delta]) \cup \gamma([\Delta]) \\
...
\int_X \Delta^*\gamma([\Delta])
\end{align*}
We have used Lemmas \ref{weil-lemma-push-unit} and
\ref{weil-lemma-pushforward}.
Write $\gamma([\Delta]) = \sum  e_{i, j} \otimes e'_{2d - i , j}$
as in Lemma \ref{weil-lemma-class-diagonal}.
Recalling that $\Delta^*$ is given by cup product
(Remark \ref{weil-remark-replace-cup-product}) we obtain
$$
\int_X \sum\nolimits_{i, j} e_{i, j} \cup e'_{2d - i, j} =
\sum\nolimits_{i, j} \int_X e_{i, j} \cup e'_{2d - i, j} =
\sum\nolimits_{i, j} (-1)^i = \sum (-1)^i\beta_i
...
\begin{lemma}[Algebra relations]
\label{weil-lemma-algebra-relations}
\source{stacks:0FHT}
\href{https://stacks.math.columbia.edu/tag/0FHT}{\texttt{Stacks tag 0FHT}}
\group{weil}
...
\begin{lemma}[Relations classes points]
\label{weil-lemma-relations-classes-points}
\source{stacks:0FHU}
\href{https://stacks.math.columbia.edu/tag/0FHU}{\texttt{Stacks tag 0FHU}}
\group{weil}
...
\begin{proof}
\uses{weil-lemma-generated-by-separable, weil-lemma-dim-0, weil-lemma-algebra-relations}
...
Lemma \ref{weil-lemma-generated-by-separable}.
Then
$$
...
(Lemma \ref{weil-lemma-dim-0}) which we will denote $\text{Tr}_x$.
...
By Lemma \ref{weil-lemma-algebra-relations} we conclude that $i^* = (i')^*$
which contradicts the assumption that $\gamma([x]) \not = \gamma([x'])$.
\end{proof}

\begin{lemma}[Classes points]
\label{weil-lemma-classes-points}
\source{stacks:0FHV}
\href{https://stacks.math.columbia.edu/tag/0FHV}{\texttt{Stacks tag 0FHV}}
\group{weil}
...
\begin{proof}
\uses{weil-lemma-generated-by-separable, weil-lemma-divide-difference-points, weil-lemma-relations-classes-points}

By Lemma \ref{weil-lemma-generated-by-separable} it suffices to show: given
...
Lemma \ref{weil-lemma-divide-difference-points} there
...
By Lemma \ref{weil-lemma-relations-classes-points} we have
$\gamma([y]) = \gamma([y'])$ in $H^{2d}(Y)(d)$.
By compatibility with pushforward in axiom (C)(b)
we conclude the same for $x$ and $x'$.
\end{proof}

\begin{lemma}[Injective]
\label{weil-lemma-injective}
\source{stacks:0FHW}
\href{https://stacks.math.columbia.edu/tag/0FHW}{\texttt{Stacks tag 0FHW}}
\group{weil}
...
\begin{proof}
\uses{weil-lemma-unit, weil-lemma-pushforward}
...
Lemma \ref{weil-lemma-unit}. Hence by the projection formula
(Lemma \ref{weil-lemma-pushforward})
...
\source{stacks:0FHX}
\href{https://stacks.math.columbia.edu/tag/0FHX}{\texttt{Stacks tag 0FHX}}
\group{weil}
...
\begin{proof}
\uses{weil-lemma-dim-0, weil-lemma-weil-additive}

We will use the results of Lemma \ref{weil-lemma-dim-0} without further mention.
Write
$$
...
deduce from axiom (B)(a) and Lemma \ref{weil-lemma-weil-additive}
that we have
$$
...
classical Weil cohomology theory (Section \ref{weil-section-axioms-classical})
when the ground field $k$ is not algebraically closed.
In Section \ref{weil-section-axioms} we listed axioms which guarantee
...
\begin{example}[Weird weil]
\label{weil-example-weird-weil}
\source{stacks:0FHZ}
\href{https://stacks.math.columbia.edu/tag/0FHZ}{\texttt{Stacks tag 0FHZ}}
\group{weil}

\uses{weil-proposition-weil-cohomology-theory-classical, weil-lemma-from-functor-to-weil-classical}
...
By Proposition \ref{weil-proposition-weil-cohomology-theory-classical}
...
Hence by Lemma \ref{weil-lemma-from-functor-to-weil-classical}
...
\begin{lemma}[H 0 separable]
\label{weil-lemma-H-0-separable}
\source{stacks:0FI0}
\href{https://stacks.math.columbia.edu/tag/0FI0}{\texttt{Stacks tag 0FI0}}
\group{weil}
...
\begin{proof}
\uses{weil-lemma-weil-additive, weil-lemma-trace-disjoint-union, weil-lemma-generated-by-separable, weil-lemma-injective, weil-lemma-otimes, weil-lemma-classes-points, weil-lemma-dim-0}
...
(Lemmas \ref{weil-lemma-weil-additive} and \ref{weil-lemma-trace-disjoint-union})
...
\medskip\noindent
By Lemma \ref{weil-lemma-generated-by-separable} we see that the closed
...
points, so $r$ is increased in this step). By Lemma \ref{weil-lemma-classes-points}
$\gamma(x_1) = \gamma(x_2) = \ldots = \gamma(x_r)$.
By axiom (A)(b) all the maps $H^0(X) \to H^0(x_i)$
are the same. This means (2) holds.

\medskip\noindent
Finally, Lemma \ref{weil-lemma-dim-0} implies
$H^0(X)$ is a separable $F$-algebra if (1) holds.
\end{proof}

\begin{lemma}[Negative cohomology]
\label{weil-lemma-negative-cohomology}
\source{stacks:0FI1}
\href{https://stacks.math.columbia.edu/tag/0FI1}{\texttt{Stacks tag 0FI1}}
\group{weil}

\uses{weil-lemma-H-0-separable}
Assume given (D0), (D1), (D2), and (D3) satisfying (A), (B), and (C).
...
of Lemma \ref{weil-lemma-H-0-separable} fail for $X$.
\end{lemma}

\begin{proof}
\uses{weil-lemma-weil-additive}

By Lemma \ref{weil-lemma-weil-additive} we may assume $Y$ is irreducible
...
\source{stacks:0FI2}
\href{https://stacks.math.columbia.edu/tag/0FI2}{\texttt{Stacks tag 0FI2}}
\group{weil}
...
of Section \ref{weil-section-axioms}
and in addition such that the equivalent conditions (1) and (2) of
Lemma \ref{weil-lemma-H-0-separable} hold for every smooth projective $X$ over $k$.
\end{definition}

By Lemma \ref{weil-lemma-negative-cohomology} this means also that there are no
...
\begin{remark}[Betti numbers in some sense]
\label{weil-remark-betti-numbers-in-some-sense}
\source{stacks:0FI3}
\href{https://stacks.math.columbia.edu/tag/0FI3}{\texttt{Stacks tag 0FI3}}
\group{weil}

\uses{weil-definition-weil-cohomology-theory}
Let $H^*$ be a Weil cohomology theory
(Definition \ref{weil-definition-weil-cohomology-theory}).
...
$$
Now, our final assumption in Definition \ref{weil-definition-weil-cohomology-theory}
...
\begin{proposition}[Weil cohomology theory again]
\label{weil-proposition-weil-cohomology-theory-again}
\source{stacks:0GIJ}
\href{https://stacks.math.columbia.edu/tag/0GIJ}{\texttt{Stacks tag 0GIJ}}
\group{weil}
...
\begin{proof}
\uses{weil-proposition-weil-cohomology-theory, weil-definition-weil-cohomology-theory}

Immediate consequence of Proposition \ref{weil-proposition-weil-cohomology-theory}
and Definition \ref{weil-definition-weil-cohomology-theory}. Of course we could
...
\begin{proposition}[Chern class]
\label{weil-proposition-chern-class}
\source{stacks:0FI5}
\href{https://stacks.math.columbia.edu/tag/0FI5}{\texttt{Stacks tag 0FI5}}
\group{weil}
...
\source{stacks:0FI7}
\href{https://stacks.math.columbia.edu/tag/0FI7}{\texttt{Stacks tag 0FI7}}
\group{weil}
...
\source{stacks:0FI8}
\href{https://stacks.math.columbia.edu/tag/0FI8}{\texttt{Stacks tag 0FI8}}
\group{weil}
...
\begin{proposition}[Chern character]
\label{weil-proposition-chern-character}
\source{stacks:0FI9}
\href{https://stacks.math.columbia.edu/tag/0FI9}{\texttt{Stacks tag 0FI9}}
\group{weil}
...
\begin{proof}
\uses{weil-proposition-chern-class, weil-lemma-splitting-principle}
...
degrees, by Lemma \ref{weil-lemma-splitting-principle} we may
...
\source{stacks:0FIA}
\href{https://stacks.math.columbia.edu/tag/0FIA}{\texttt{Stacks tag 0FIA}}
\group{weil}

\uses{weil-proposition-chern-class, weil-proposition-chern-character}
...
Propositions \ref{weil-proposition-chern-class} and
...
\begin{proof}
\uses{weil-lemma-splitting-principle}
...
Lemma \ref{weil-lemma-splitting-principle}. Then
...
\begin{lemma}[Lambda operations]
\label{weil-lemma-lambda-operations}
\source{stacks:0FIC}
\href{https://stacks.math.columbia.edu/tag/0FIC}{\texttt{Stacks tag 0FIC}}
\group{weil}
...
(D0) and (D1) as discussed in Section \ref{weil-section-axioms}.
...
which is equidimensional of dimension $d$. By Lemma \ref{weil-lemma-cycle-classes}
...
\medskip\noindent
Ad (A9). We will see in Remark \ref{weil-remark-trace} that if we have
axioms (A1) -- (A7) then the map $\lambda$ of axiom (A6) is unique.

\begin{lemma}[Chern classes]
\label{weil-lemma-chern-classes}
\source{stacks:0FIE}
\href{https://stacks.math.columbia.edu/tag/0FIE}{\texttt{Stacks tag 0FIE}}
\group{weil}
...
\begin{proof}
\uses{weil-proposition-chern-character}

Immediate from Proposition \ref{weil-proposition-chern-character}
applied to the category of smooth projective schemes over $k$,
the functor $A : X \mapsto \bigoplus_{i \geq 0} H^{2i}(X)(i)$,
and the map $c_1^H$.
\end{proof}

\begin{lemma}[Cycle classes]
\label{weil-lemma-cycle-classes}
\source{stacks:0FIF}
\href{https://stacks.math.columbia.edu/tag/0FIF}{\texttt{Stacks tag 0FIF}}
\group{weil}
...
\begin{proof}
\uses{weil-lemma-chern-classes, weil-lemma-adams-and-chern}
...
where $ch^H$ is as in Lemma \ref{weil-lemma-chern-classes} and
...
Lemma \ref{weil-lemma-chern-classes} and
...
by Lemma \ref{weil-lemma-adams-and-chern} as desired.
\end{proof}

\begin{lemma}[Divide pullback good blowing up]
\label{weil-lemma-divide-pullback-good-blowing-up}
\source{stacks:0FIH}
\href{https://stacks.math.columbia.edu/tag/0FIH}{\texttt{Stacks tag 0FIH}}
\group{weil}
...
\begin{lemma}[A5 A6 imply]
\label{weil-lemma-A5-A6-imply}
\source{stacks:0FII}
\href{https://stacks.math.columbia.edu/tag/0FII}{\texttt{Stacks tag 0FII}}
\group{weil}
...
\begin{proof}
\uses{weil-lemma-divide-pullback-good-blowing-up}
...
$$
By Lemma \ref{weil-lemma-divide-pullback-good-blowing-up} we have
$$
b^*\gamma([Z]) = \gamma(b^*[Z]) =
\gamma([E] \cdot \theta) =
...
\begin{lemma}[Poincare duality]
\label{weil-lemma-poincare-duality}
\source{stacks:0FIJ}
\href{https://stacks.math.columbia.edu/tag/0FIJ}{\texttt{Stacks tag 0FIJ}}
\group{weil}

Assume given data (D0), (D1), and (D2') satisfying axioms (A1) -- (A7).
Then axiom (A) of Section \ref{weil-section-axioms} holds with
$\int_X = \lambda$ as in axiom (A6).
\end{lemma}

\begin{proof}
\uses{weil-lemma-A5-A6-imply}
...
Lemma \ref{weil-lemma-A5-A6-imply} implies
...
\source{stacks:0FIK}
\href{https://stacks.math.columbia.edu/tag/0FIK}{\texttt{Stacks tag 0FIK}}
\group{weil}

\uses{weil-lemma-poincare-duality}
Assume given data (D0), (D1), and (D2') satisfying axioms (A1) -- (A7).
...
the proofs of Lemma \ref{weil-lemma-poincare-duality} and
...
\begin{lemma}[Trace product]
\label{weil-lemma-trace-product}
\source{stacks:0FIL}
\href{https://stacks.math.columbia.edu/tag/0FIL}{\texttt{Stacks tag 0FIL}}
\group{weil}

Assume given data (D0), (D1), and (D2') satisfying axioms (A1) -- (A7).
Then axiom (B) of Section \ref{weil-section-axioms} holds.
\end{lemma}
...
\begin{lemma}[Trace base]
\label{weil-lemma-trace-base}
\source{stacks:0FIM}
\href{https://stacks.math.columbia.edu/tag/0FIM}{\texttt{Stacks tag 0FIM}}
\group{weil}

Assume given data (D0), (D1), and (D2') satisfying axioms (A1) -- (A7).
Then axiom (C)(d) of Section \ref{weil-section-axioms} holds.
\end{lemma}

\begin{proof}
\uses{weil-lemma-trace-product}
...
in Lemma \ref{weil-lemma-trace-product}.
\end{proof}
...
of Section \ref{weil-section-axioms}, see
Lemmas \ref{weil-lemma-poincare-duality}, \ref{weil-lemma-trace-product}, and
\ref{weil-lemma-trace-base}.
Moreover, we have the pushforwards $f_* : H^*(X) \to H^*(Y)$
as constructed in Section \ref{weil-section-axioms}. The only axiom of
Section \ref{weil-section-axioms}
which isn't clear yet is axiom (C)(b).

\begin{lemma}[Ok for projective bundle]
\label{weil-lemma-ok-for-projective-bundle}
\source{stacks:0FIN}
\href{https://stacks.math.columbia.edu/tag/0FIN}{\texttt{Stacks tag 0FIN}}
\group{weil}
...
\begin{proof}
\uses{weil-lemma-pushforward, weil-lemma-diagonal-projective-bundle}
...
and by the projection formula (Lemma \ref{weil-lemma-pushforward})
we have
$$
...
Lemma \ref{weil-lemma-diagonal-projective-bundle}.
\end{proof}

\begin{lemma}[Integrate 1]
\label{weil-lemma-integrate-1}
\source{stacks:0FVR}
\href{https://stacks.math.columbia.edu/tag/0FVR}{\texttt{Stacks tag 0FVR}}
\group{weil}
...
(Remark \ref{weil-remark-trace}) the proof is complete.
\end{proof}

\begin{lemma}[Enough]
\label{weil-lemma-enough}
\source{stacks:0FIP}
\href{https://stacks.math.columbia.edu/tag/0FIP}{\texttt{Stacks tag 0FIP}}
\group{weil}
...
\begin{proof}
\uses{weil-lemma-cycle-classes, weil-lemma-poincare-duality, weil-remark-trace, weil-lemma-trace-product, weil-lemma-trace-base, weil-lemma-pushforward, weil-lemma-ok-for-projective-bundle, weil-lemma-pr2star, weil-lemma-integrate-1}

We will use without further mention that we've constructed our
cycle class map $\gamma$ in Lemma \ref{weil-lemma-cycle-classes}
compatible with intersection products and pullbacks and that
we've already shown axioms
(A), (B), (C)(a), (C)(c), and (C)(d) of Section \ref{weil-section-axioms}, see
Lemma \ref{weil-lemma-poincare-duality},
Remark \ref{weil-remark-trace}, and
Lemmas \ref{weil-lemma-trace-product} and \ref{weil-lemma-trace-base}.
In particular, we may use (for example) Lemma \ref{weil-lemma-pushforward}
...
By Lemma \ref{weil-lemma-ok-for-projective-bundle}
...
Lemma \ref{weil-lemma-ok-for-projective-bundle}
...
Lemma \ref{weil-lemma-pr2star}. Thus to finish the proof in the
finite field case, it suffices to prove that
$\int_{\Spec(k')}(1) = [k' : k]$ which we do in
Lemma \ref{weil-lemma-integrate-1}.
\end{proof}
...
\begin{lemma}[Grassmanian]
\label{weil-lemma-grassmanian}
\source{stacks:0FIQ}
\href{https://stacks.math.columbia.edu/tag/0FIQ}{\texttt{Stacks tag 0FIQ}}
\group{weil}
...
\begin{proof}
\uses{weil-lemma-ok-for-projective-bundle}

If $l = 1$ or $l = n - 1$ then $p$ is a projective bundle and
the result follows from Lemma \ref{weil-lemma-ok-for-projective-bundle}.
...
\begin{lemma}[Enough better]
\label{weil-lemma-enough-better}
\source{stacks:0FIR}
\href{https://stacks.math.columbia.edu/tag/0FIR}{\texttt{Stacks tag 0FIR}}
\group{weil}
...
\begin{proof}
\uses{weil-lemma-enough, weil-lemma-grassmanian}

By Lemma \ref{weil-lemma-enough} it suffices to show that $i_*(1) = \gamma([Z])$
...
of cycles by Lemma \ref{weil-lemma-grassmanian}. The conormal sheaf $\mathcal{C}$
...
\begin{lemma}[Injective H0]
\label{weil-lemma-injective-H0}
\source{stacks:0FVS}
\href{https://stacks.math.columbia.edu/tag/0FVS}{\texttt{Stacks tag 0FVS}}
\group{weil}
...
\begin{lemma}[Pushforward blowup]
\label{weil-lemma-pushforward-blowup}
\source{stacks:0FIS}
\href{https://stacks.math.columbia.edu/tag/0FIS}{\texttt{Stacks tag 0FIS}}
\group{weil}
...
$$
\begin{array}{cc}x' \times X' \longrightarrow\downarrow & X' \times X' \downarrow \\ x \times X \longrightarrow & X \times X\end{array}
$$
...
By Lemma \ref{weil-lemma-injective-H0} this common value maps injectively
...
\source{stacks:0FIT}
\href{https://stacks.math.columbia.edu/tag/0FIT}{\texttt{Stacks tag 0FIT}}
\group{weil}
...
\begin{proof}
\uses{weil-lemma-enough-better, weil-lemma-divide-pullback-good-blowing-up, weil-lemma-ok-for-projective-bundle, weil-lemma-pushforward-blowup}

Let $i : Z \to X$ be as in Lemma \ref{weil-lemma-enough-better}. Consider
the diagram
$$
\begin{array}{cc}E \underset{j}{\longrightarrow}\underset{\pi}{\downarrow} & X' \overset{b}{\downarrow} \\ Z \overset{i}{\longrightarrow} & X\end{array}
$$
Let $\theta \in \CH^{r - 1}(X')$ be as in
Lemma \ref{weil-lemma-divide-pullback-good-blowing-up}.
Then $\pi_*j^!\theta = [Z]$ in $\CH_*(Z)$ implies that
$\pi_*\gamma(j^!\theta) = 1$ by Lemma \ref{weil-lemma-ok-for-projective-bundle}
...
$$
Since $b_*(1) = 1$ by Lemma \ref{weil-lemma-pushforward-blowup} the
proof is complete.
\end{proof}

\begin{proposition}[Get weil]
\label{weil-proposition-get-weil}
\source{stacks:0FIU}
\href{https://stacks.math.columbia.edu/tag/0FIU}{\texttt{Stacks tag 0FIU}}
\group{weil}
...
\begin{proof}
\uses{weil-lemma-poincare-duality, weil-lemma-trace-product, weil-lemma-trace-base, weil-lemma-done, weil-definition-weil-cohomology-theory}

We have axioms (A), (B) and (C)(a), (C)(c), and (C)(d) of
Section \ref{weil-section-axioms} by
Lemmas \ref{weil-lemma-poincare-duality}, \ref{weil-lemma-trace-product}, and
\ref{weil-lemma-trace-base}.
We have axiom (C)(b) by
Lemma \ref{weil-lemma-done}.
...
\begin{lemma}[Check over extension]
\label{weil-lemma-check-over-extension}
\source{stacks:0FVT}
\href{https://stacks.math.columbia.edu/tag/0FVT}{\texttt{Stacks tag 0FVT}}
\group{weil}
...
$$
\begin{array}{cc}\Pic(X) \underset{c_1^H}{\longrightarrow}\downarrow & H^2(X)(1) \downarrow \\ \Pic(X_{k'}) \overset{c_1^{H'}}{\longrightarrow} & (H')^2(X_{k'})(1)\end{array}
$$
commute.
\end{enumerate}
...
\begin{proof}
\uses{weil-lemma-cycle-classes, weil-remark-trace}
...
$$
\begin{array}{cc}\CH^*(X) \underset{\gamma}{\longrightarrow}\underset{g^*}{\downarrow} & \bigoplus H^{2i}(X)(i) \downarrow \\ \CH^*(X_{k'}) \overset{\gamma'}{\longrightarrow} & \bigoplus (H')^{2i}(X_{k'})(i)\end{array}
$$
commutes. Observe that we have $\gamma$ as we know axioms
(A1) -- (A4) already; see Lemma \ref{weil-lemma-cycle-classes}.
...
see Lemma \ref{weil-lemma-cycle-classes}. We may decompose
$$
...
By Remark \ref{weil-remark-trace}

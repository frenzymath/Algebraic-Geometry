...
\begin{equation} \label{dmap}\dcref{4.1}
\source{refined-brill-noether-theory-hurwitz-spaces}
...
\begin{equation} \label{sd}\dcref{4.2}
\source{refined-brill-noether-theory-hurwitz-spaces}
...
\end{Lemma}
\begin{proof}
For each open subset $U \subset \pp^1$, restriction gives a commutative
square relating $\tilde{\mu}$ on global sections to its restriction on $U$.
...
There is a commutative diagram whose top row is the map $\mu$ on $\P_0$
and whose bottom row is the direct sum $\bigoplus_i\tilde{\mu}_i$ on the
components; the inclusion $\iota$ identifies compatible tuples with sections
on the central fibre. We use this diagram only to prove injectivity.
...
\begin{equation} \label{twoeq}\dcref{4.3}
\source{refined-brill-noether-theory-hurwitz-spaces}

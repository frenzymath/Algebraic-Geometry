...
(`HartshorneLib.Chapter2AffineVanishingQcoh`). The packaging is the class
...
(`HartshorneLib.Chapter2AffineCech`) with structure-sheaf sections replaced by
...
the affine vanishing engine, with its composition and naturality
...
restriction operator used file-locally in the affine vanishing engine;
...
`HartshorneLib.Chapter2AffineCech`. This instance recovers the landed

...
properness package for `Proj.toSpecZero`, but no relative projective space over
an arbitrary base scheme. Following the `AffineSpace` pattern, this file
defines relative projective space by base change from the integral model.
-/
...
`ℤ[Xᵢ : i ∈ n]` of the integral model of projective space. -/
local notation3 "𝒫[" n "]" => homogeneousSubmodule n (ULift ℤ)

/-- The total-degree grading is a graded ring structure. -/
...
/-- The structural morphism of relative projective space is proper. -/

...
/- These pointwise kernel facts belong below the Chapter 4 skyscraper development.
   The suffix keeps this foundational copy independent of that later module. -/
theorem exists_app_eq_of_cokernelπ_app_eq_zero_qcoh
    {F G : Sheaf J (ModuleCat.{u} R)} (ι : F ⟶ G) [Mono ι] (U : Cᵒᵖ)
    (c : G.obj.obj U) (hc : ((cokernel.π ι).hom.app U).hom c = 0) :
...
theorem app_injective_of_mono_qcoh {F G : Sheaf J (ModuleCat.{u} R)}
    (ι : F ⟶ G) [Mono ι] (U : Cᵒᵖ) : Function.Injective (ι.hom.app U).hom := by
  let Fsec : Sheaf J (ModuleCat.{u} R) ⥤ ModuleCat.{u} R :=
    sheafToPresheaf J (ModuleCat.{u} R) ⋙ (evaluation Cᵒᵖ (ModuleCat.{u} R)).obj U
...
theorem ker_cokernelπ_app_eq_range {F G : Sheaf J (ModuleCat.{u} R)}
  ...
  constructor
  · intro hc
    exact exists_app_eq_of_cokernelπ_app_eq_zero_qcoh ι U c hc
  · rintro ⟨a, rfl⟩
...
theorem cokernel_app_surjective_of_qcoh (hU : IsAffineOpen U)
  ...
  choose a ha using fun i j ↦ Sheaf.exists_app_eq_of_cokernelπ_app_eq_zero_qcoh ι
  ...
  have hcoc : ∀ i j l,
    ...
    intro i j l
    apply Sheaf.app_injective_of_mono_qcoh ι
...
theorem Scheme.subsingleton_hModule_one_of_qcoh {k : Type u} [CommRing k] {X : Scheme.{u}}
    ...
    Subsingleton (Sheaf.HModule (Opens.grothendieckTopology (X : TopCat)) k F 1) :=
...
`HartshorneLib.Chapter2QcohSections`. -/
...
example [IsAffine X] :
    Subsingleton
      (Sheaf.HModule (Opens.grothendieckTopology (X : TopCat)) k (X.moduleKSheaf k) 1) :=

...
by Theorem \ref{geometric and schematic fiber}, which  settles \ref{non-rational J}.
From Proposition \ref{canonical type rational points} we get \ref{I0* J}.
Suppose next  that $J_a$   is semistable. Then $Y_a$ must be simple, by  Lemma \ref{multiple for p=2}, hence
$J_a\simeq Y_a$, again from  Proposition \ref{same symbols}. Assertion \ref{semistable J}
...
can be semistable or supersingular, which gives \ref{rational J}.

It remains to verify the last assertion \ref{mw J}. According to the definition of Mordell--Weil groups we have
$$
...
In turn, the induced vertical map  between cokernels is bijective, thus \ref{mw J} holds.
\qed
...
and the sections. The former is obviously defined over $k$, and the latter  by condition \ref{mw J}.
According to  condition \ref{non-rational J}, each $\Theta\subset\bJ$ projects to  a fiber $J_a$ over some rational point  $a\in\PP^1$.
Thus it suffices to prove the following:

\begin{proposition}
\source{no-enriques-surface-over-the-integers}
\notready
\label{bijection irreducible components}
...
Condition \ref{semistable J} ensures that it also holds for $\I_n$ with $n\geq 2$.
...
If the Kodaira symbol is $\I_m^*$, we must have $m\geq 1$ by condition \ref{I0* J}.
...
The fourth case does not occur thanks to condition \ref{rational J}.

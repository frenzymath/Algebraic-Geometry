...
The Auslander--Buchsbaum formula relates the projective dimension of a finite module over a
Noetherian local ring to its depth.  For a regular local ring, a regular system of parameters also
shows directly that the ring is Cohen--Macaulay.  On a regular surface this gives depth two at
every closed point and hence the familiar Hartogs extension theorem for regular functions across
such a point.

This local-algebra consequence must be distinguished from extension of rational maps to a
complete target: depth two alone does not imply that a rational map extends.  In particular, the
Albanese rational-map extension used later follows the separate codimension-one and group-scheme
indeterminacy argument; it does not depend on the Cohen--Macaulay corollary proved here.

This chapter packages:
\begin{enumerate}
  \item the regular-sequence definition of depth and its \(\text{Ext}\) reformulation
    (\cref{def:depth} and \cref{lem:depth_via_ext}),
  \item the projective-dimension invariant of a finitely-generated module
    (\cref{def:projective_dimension}),
  \item the depth-on-a-short-exact-sequence lemma
    (\cref{lem:depth_short_exact_sequence}),
...
\end{enumerate}
A short closing application (\cref{sec:ab_application_to_a4a}) records the Hartogs consequence
for regular functions and its precise separation from the Albanese extension argument.

\section{Depth of a module over a local ring}
\label{sec:ab_depth}

\begin{definition}[Depth of a module]\leanok
  \label{def:depth}
  \dcref{custom}
...
\begin{lemma}[Depth via \(\text{Ext}\)]\leanok
  \label{lem:depth_via_ext}
  \dcref{custom}
  \lean{RingTheory.Module.depth_eq_smallest_ext_index}
  \uses{def:depth, lem:ext_smul_eq_zero_of_mem_annihilator}
...
\begin{definition}[Projective dimension]\leanok
  ...
  Auslander--Buchsbaum formula in \cref{thm:auslander_buchsbaum} concerns only nonzero \(M\).
...
\begin{lemma}[Depth on a short exact sequence]\leanok
  \label{lem:depth_short_exact_sequence}
  \dcref{custom}
  \lean{RingTheory.Module.depth_of_short_exact}
  \uses{lem:natCast_add_one_le_of_le_sub_one}
...
\leanok
  ...
  By \cref{lem:depth_via_ext}, \(\text{depth}(N')\) (resp.\ \(\text{depth}(N)\),
  ...
  the proof of \cref{lem:depth_via_ext}.
\end{proof}

\section{The Auslander--Buchsbaum formula}
\label{sec:ab_main}

\begin{theorem}[Auslander--Buchsbaum formula]\leanok
...
\leanok
  ...
  \]
  and applying \cref{lem:depth_short_exact_sequence} iteratively,
  ...
  \cref{lem:depth_short_exact_sequence}, so a common non-zero-divisor \(x \in
...
\begin{lemma}[Auslander--Buchsbaum: inductive step \(\text{pd} = k+1\)]\leanok
  \label{lem:auslander_buchsbaum_formula_succ_pd}
  \dcref{custom}
  \lean{RingTheory.auslander_buchsbaum_formula_succ_pd}
  \uses{lem:depth_drops_by_one, lem:enat_ab_inductive_combine,
    lem:projectiveDimension_ker_eq_of_surjection}
...
\end{lemma}
\begin{proof}
  \uses{lem:depth_drops_by_one, lem:depth_ses_ineqs_of_surjection_finite_localRing, lem:exists_ne_zero_ext_of_depth_eq, lem:ext_comp_mk_zero_ofHom_eq_zero_of_entries_mem_annihilator, lem:enat_ab_inductive_combine, lem:projectiveDimension_ker_eq_of_surjection}
  \leanok
  ...
  \cref{lem:projectiveDimension_ker_eq_of_surjection}.
...
\begin{lemma}
  ...
  \dcref{custom}
  \uses{def:depth, lem:depth_via_ext}
...
\begin{proof}
  ...
  (\cref{lem:depth_via_ext}) on both sides; the short exact sequence
...
\begin{lemma}[Per-syzygy minimal surjection from \(R^n\) for finite modules over a local ring]\leanok
...
\begin{lemma}[Bridge: \(\texttt{projectiveDimension}\,M = n \Rightarrow \texttt{HasProjectiveDimensionLT}\,M\,(n+1)\)]\leanok
...
\end{lemma}
\begin{lemma}[Syzygy descent of \(\texttt{HasProjectiveDimensionLT}\)]\leanok
  \label{lem:hasProjectiveDimensionLT_ker_of_surjection}
  \dcref{custom}
  \lean{RingTheory.Module.hasProjectiveDimensionLT_ker_of_surjection}
  \uses{lem:hasProjectiveDimensionLT_succ_of_projectiveDimension_eq}
...
\end{lemma}
\begin{lemma}[Ascent companion: \(\ker f\)-pd-control \(\Rightarrow M\)-pd-control]\leanok
  ...
  \uses{lem:hasProjectiveDimensionLT_ker_of_surjection}
...
[Depth lower bound on the kernel of a finite-free surjection]
  ...
  \dcref{custom}
  \uses{lem:depth_short_exact_sequence, lem:depth_pi_const_eq_depth_of_nonempty}
...
\cref{lem:auslander_buchsbaum_formula_succ_pd}. The base case is completed
...
\label{subsec:ab_ext_depth_substrate}

\begin{lemma}[Annihilator kills \(\text{Ext}\)]\leanok
...
\begin{lemma}[\(\text{Ext}\)-vanishing below the depth]\leanok
...
\leanok
  ...
  characterization in \cref{lem:depth_via_ext} would give
  \(\operatorname{depth}(M)\leq i\), contrary to the hypothesis.
\end{proof}

\begin{lemma}[\(\mathbb N_\infty\) successor bridge]\leanok
...
\begin{lemma}[Depth is a linear-equivalence invariant]\leanok
...
\label{subsec:ab_depth_pi_substrate}

\begin{lemma}[Ideal action on a constant product]\leanok
...
\begin{lemma}[Triviality of the ideal action on a constant product]\leanok
  \label{lem:ideal_smul_top_pi_const_eq_top_iff}
  \dcref{custom}
  \lean{RingTheory.Module.ideal_smul_top_pi_const_eq_top_iff}
  \uses{lem:ideal_smul_top_pi_const}
...
\leanok
  Apply \cref{lem:ideal_smul_top_pi_const}. If \(IM=M\), the product
...
\begin{definition}[Quotient of a constant product by a scalar]\leanok
...
\leanok
  ...
  \cref{def:quotSMulTopPiConstLinearEquiv} identifies the quotient product
  with the product of the quotients; apply the inductive hypothesis.
\end{proof}

\begin{lemma}[Depth of a constant product]\leanok
  \label{lem:depth_pi_const_eq_depth_of_nonempty}
  \dcref{custom}
  \lean{RingTheory.Module.depth_pi_const_eq_depth_of_nonempty}
  \uses{def:depth, lem:ideal_smul_top_pi_const_eq_top_iff, lem:isRegular_pi_const_iff_of_nonempty}
...
\leanok
  ...
  by \cref{lem:ideal_smul_top_pi_const_eq_top_iff}. The regular sequences,
  with their lengths, agree by
  \cref{lem:isRegular_pi_const_iff_of_nonempty}. Hence their suprema agree.
\end{proof}

\subsection{Surjection and projective-dimension substrate for the inductive step}
\label{subsec:ab_surjection_substrate}
...
\begin{lemma}[Kernel-SES depth inequalities for a finite free surjection]\leanok
...
\leanok
  By \cref{lem:depth_via_ext}, \(D\) is the least index at which
...
\begin{lemma}[Exact projective dimension of a syzygy]\leanok
  \label{lem:projectiveDimension_ker_eq_of_surjection}
  \dcref{custom}
  \lean{RingTheory.projectiveDimension_ker_eq_of_surjection}
  \uses{def:projective_dimension, lem:hasProjectiveDimensionLT_succ_of_projectiveDimension_eq, lem:hasProjectiveDimensionLT_ker_of_surjection, lem:hasProjectiveDimensionLT_succ_of_hasProjectiveDimensionLT_ker}
...
\begin{lemma}[\(\mathbb N_\infty\) combine for the inductive step]\leanok
...
\label{subsec:ab_matrix_collapse_substrate}

\begin{definition}[Elementary linear map]\leanok
  \label{def:elemMap}
  \dcref{custom}
...
\begin{lemma}[Evaluation of the elementary map]\leanok
...
\begin{lemma}[Matrix-collapse on \(\text{Ext}\)]\leanok
  ...
  \uses{lem:linearMap_finFunR_matrix_decomp, lem:ext_smul_eq_zero_of_mem_annihilator}
...
\leanok
  ...
  annihilates \(N\), so \cref{lem:ext_smul_eq_zero_of_mem_annihilator}
...
\begin{lemma}[Regular sequences are bounded by the Krull dimension]\leanok
...
\begin{lemma}[Cotangent image of \(x \in \mathfrak m \setminus \mathfrak m^2\)]\leanok
...
\begin{lemma}[Cotangent dimension drops by one modulo \(x\)]\leanok
...
\begin{lemma}[Nakayama witness for a positive cotangent dimension]\leanok
...
\begin{lemma}[Zero cotangent dimension forces a field]\leanok
...
\begin{lemma}[Regularity descends to \(R/(x)\)]\leanok
...
\leanok
  By \cref{lem:finrank_cotangentSpace_quot_span_singleton_succ},
...
\begin{lemma}[Members of a minimal prime are zero-divisors]\leanok
...
\begin{lemma}[\((x)\) is not a minimal prime in the inductive step]\leanok
...
\begin{lemma}[Regular local rings are domains]\leanok
...
\leanok
  ...
  \cref{lem:isDomain_of_isLocalRing_of_spanFinrank_maximalIdeal_eq_zero}.
...
\begin{lemma}[A regular element outside \(\mathfrak m^2\)]\leanok
...
\leanok
  ...
  \cref{lem:exists_notMemSq_of_spanFinrank_pos}. It is nonzero, and
  \cref{lem:isDomain_of_regularLocal} makes multiplication by \(x\)
  injective. Thus \(x\) is regular.
\end{proof}

\begin{lemma}[Regular element with regular quotient]\leanok
...
\leanok
  ...
  \cref{lem:exists_isSMulRegular_notMemSq_of_regularLocal_succ}.
  It is regular, and its quotient is regular of dimension \(k\) by
  \cref{lem:regularLocal_quotient_isRegularLocal_of_notMemSq}.
\end{proof}

\begin{lemma}[Inductive step for the regular sequence]\leanok
...
\begin{lemma}[Regular local rings have a full regular sequence]\leanok
...
\leanok
  ...
  \cref{lem:regularLocal_inductive_step} extends the regular sequence from
...
\begin{definition}[Cohen--Macaulay local ring]\leanok
...
\begin{corollary}[Regular local rings are Cohen--Macaulay]\leanok
...
Thus \cref{cor:regular_cohen_macaulay} supplies the depth-two input needed to extend regular
...
\section{Algebraic scope}
\label{sec:ab_lean_encoding}

The Auslander--Buchsbaum formula and its Cohen--Macaulay corollary depend
only on the homological algebra of finite modules over Noetherian local
rings. They are independent of the geometric constructions used later for
Albanese varieties and may therefore be applied wherever the corresponding
local rings are regular.

\section{Further consequences}
\label{sec:ab_out_of_scope}

The local result above also leads to the usual global Cohen--Macaulay
criterion and to local-cohomology vanishing below the depth. Those
consequences, as well as refinements involving dualizing complexes and
Gorenstein rings, are not needed for the Auslander--Buchsbaum formula or
its application to regular surfaces.

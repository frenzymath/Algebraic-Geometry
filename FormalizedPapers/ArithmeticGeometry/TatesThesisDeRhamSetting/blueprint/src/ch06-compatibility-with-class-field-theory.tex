...
\]
 ...
 We remark that this object is concentrated in degree $-\dim X$.
...
\end{equation}
 ...
 between left modules for $\on{Diff}(X)$ and the category $D(X)$,
...
\]
 ...
 uniquely characterized by sending $D_{\fL_n^+\bA^1}$ to
...
\]
 ...
 factors through the Contou-Carr\`ere duality pairing:
%
\[
...
\]
 ...
 Here we normalize the sign in the Contou-Carr\`ere pairing
...
\]
 ...
 where the bottom arrow is induced by the pairing of
...
\]
%
 that are easily seen to be isomorphisms for $\sF$ compact.
...
\]
 ...
 Note that $A \in A\mod^{G,w}$, so we obtain a natural object
...
\]
 ...
 (Here the superscript \emph{op} indicates the reversed multiplication.)
...
\]
 ...
 of $\Rep(G)$-module categories; by de-equivariantization
...
\]
 ...
 such that $i$ is a $G$-equivariant morphism of Lie algebras such that
$[i(\xi),-]:A \to A$
...
\]
 ...
 of Harish-Chandra bimodules acts canonically on $\sC^{G,w}$.
...
\]
 ...
 is equivalent to giving $\sF \in \sC^{G,w}$ and
...
\]
 ...
 coincides with the obstruction to strong equivariance on the object $\sF$.
...
\]
 ...
 that is an isomorphism whenever $\sF$ is compact. (This is the
...
\]
 ...
 compatible with the Harish-Chandra data is equivalent to
a map:
%
\[
...
\]
 ...
 is the obstruction to strong equivariance. Indeed, this follows
...
\]
 ...
 Under this dictionary, the forgetful functor
...
\]
 ...
 (We remind that the displayed isomorphism is a formal consequence
...
\]
 ...
 encoding the infinitesimal action of $\fL_n^+\bG_m$ on
...
\]
 ...
 We denote this morphism by $\beta$.
...
\]
 ...
 A straightforward calculation shows that the infinitesimal action
map:
%
\[
\Lie(\fL_n^+\bG_m) \simeq k[[t]]/t^n \to \Gamma(\fL_n^+\bA^1,T_{\fL_n^+\bA^1})
...
\]
 ...
 which was used in the definition of $\alpha$ above,
is the map:
%
\[
\begin{gathered}
...
\]
 ...
 Second, we form:\footnote{Here we abuse notation: in \S \ref{ss:act-inv}, we used the
...
\]
 ...
 and apply the construction of \S \ref{ss:gpd}.
...
\]
 ...
 denote the resulting section. So our objective, in effect, is to calculate
$\sigma$.
...
\end{equation}
 ...
 Moreover, this is an isomorphism of commutative algebras. It is
...
\]
 ...
 Similarly, if we use $\AJ^{-1}$ instead, we obtain the
function:
%
\[
...
\]
 ...
 where for a finite-dimensional vector
...
\]
 ...
 and applying \eqref{eq:gr-cc}, we obtain a function on
%
\[
...
\]
 ...
 Indeed, this encodes the fact that the residue pairing is
$k[[t]]$-equivariant.
...
\]
 ...
 Now the result follows from Lemma \ref{l:res-cotr}.
...
\]
 ...
 By \eqref{eq:res-res-calc}, we see the resulting function is:
%
\[
...
\]
 ...
 which Lemma \ref{l:res-res} implies is:
%
\[
(g_1,g_2,\eta) \mapsto \Res(\frac{f g_2}{g_1}\eta),
\]

 which for $g_1 = g_2 = g$ is:
%
\[
(g,\eta) \mapsto \Res(f \eta).
\]

 Again, the induced endomorphism clearly coincides with that

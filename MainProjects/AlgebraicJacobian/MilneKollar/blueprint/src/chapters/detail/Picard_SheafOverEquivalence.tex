\section{The sheaf-of-modules over-equivalence (shared slice root)}
\label{mk-chap:Picard_SheafOverEquivalence}

\section{The equivalence}
\label{mk-sec:soe_equivalence}

\begin{definition}[Sheaf-of-modules over-equivalence]\label{mk-def:sheafofmodules_over_equivalence}
  \dcref{custom}

  \uses{mk-def:phi_over, mk-def:psi_over, mk-def:over_equiv_functor_is_continuous,
    mk-def:over_equiv_inverse_is_continuous}
...
\uses{mk-def:over_equiv_functor_is_continuous, mk-def:over_equiv_inverse_is_continuous,
        mk-def:phi_over, mk-def:psi_over}
...
\label{mk-sec:soe_equivalence_helpers}

\begin{definition}[Continuity of the inverse leg of the over-equivalence]\label{mk-def:over_equiv_inverse_is_continuous}
  \dcref{custom}
...
\begin{proof}
  \(\widetilde{U}\), where density of the inverse leg and
  \(\mathtt{IsDenseSubsite} \Rightarrow \mathtt{IsContinuous}\) give continuity.
\end{proof}

\begin{definition}[Continuity of the functor leg of the over-equivalence]\label{mk-def:over_equiv_functor_is_continuous}
  \dcref{custom}

  \uses{mk-def:over_equiv_inverse_is_continuous}
...
\begin{proof}
  \uses{mk-def:over_equiv_inverse_is_continuous}
 Converting to the bare-subspace form, the density of the
  inverse leg (\ref{mk-def:over_equiv_inverse_is_continuous}) propagates, for an
  equivalence of sites, to density --- hence continuity --- of the functor leg.
\end{proof}

\begin{lemma}[Reindexed open recovers its left component]\label{mk-lem:image_overequiv_functor_obj}
  \dcref{custom}
...
\begin{lemma}[Inverse-reindexed open is its open-immersion image]\label{mk-lem:left_overequiv_inverse_obj}
  \dcref{custom}
  ...
  \[
    \bigl(e.\mathtt{inverse}.\mathrm{obj}\,W\bigr).\mathrm{left}
      \;=\; \iota_{\ast}\,W .
  \]
  This is the symmetric counterpart of \ref{mk-lem:image_overequiv_functor_obj}.
\end{lemma}
...
\begin{definition}[The structure-sheaf ring morphism \(\varphi\)]\label{mk-def:phi_over}
  \dcref{custom}

  \uses{mk-lem:image_overequiv_functor_obj}
  ...
  equality \ref{mk-lem:image_overequiv_functor_obj} (the two presheaves agreeing
...
\begin{proof}
  \uses{mk-lem:image_overequiv_functor_obj}
 It is a sub-step of
  \ref{mk-def:sheafofmodules_over_equivalence}.
\end{proof}

\begin{definition}[The inverse structure-sheaf ring morphism \(\psi\)]\label{mk-def:psi_over}
  \dcref{custom}

  \uses{mk-lem:left_overequiv_inverse_obj}
  The inverse ring morphism \(\psi\), symmetric to \ref{mk-def:phi_over}: at an open
  \(W \subseteq \widetilde{U}\) it is \(X.\mathtt{ringCatSheaf}\) applied to the open
  equality \ref{mk-lem:left_overequiv_inverse_obj}, providing the section-level inverse
...
\label{mk-sec:soe_consumers}

\subsection{Substrate helpers for the restriction comparison}
\label{mk-sec:soe_consumers_helpers}

\begin{definition}[The restriction ring morphism along the open-immersion functor]\label{mk-def:psi_restrict}
  \dcref{custom}
  ...
  \ref{mk-lem:restrict_functor_eq_pushforward_psi_restrict} holds definitionally.
\end{definition}
...
\begin{lemma}[Restriction functor as a pushforward]\label{mk-lem:restrict_functor_eq_pushforward_psi_restrict}
  \dcref{custom}

  \uses{mk-def:psi_restrict}
  The restriction functor along the open immersion is, on the nose, a sheaf-of-modules
  pushforward along the morphism \ref{mk-def:psi_restrict}:
  \[
...
\begin{definition}[Index reconciliation natural isomorphism]\label{mk-def:over_forget_nat_iso}
  \dcref{custom}
  ...
  \ref{mk-lem:image_overequiv_functor_obj}.
\end{definition}

\begin{proof}
  \uses{mk-lem:image_overequiv_functor_obj}
  \ref{mk-lem:image_overequiv_functor_obj}, and naturality is automatic since the opens
  form a thin category.
\end{proof}

\begin{lemma}[The over-equivalence carries restriction to over]\label{mk-lem:sheafofmodules_restrict_over_iso}
  \dcref{custom}

  \uses{mk-def:sheafofmodules_over_equivalence, mk-def:phi_over, mk-def:psi_restrict,
    mk-def:over_forget_nat_iso}
  Let \(M \in \Scheme.\mathtt{Modules}\,X\). Under the over-equivalence of
  \ref{mk-def:sheafofmodules_over_equivalence} there is a canonical isomorphism
  \[
...
\begin{proof}
  \uses{mk-def:sheafofmodules_over_equivalence, mk-def:phi_over, mk-def:psi_restrict,
        mk-lem:restrict_functor_eq_pushforward_psi_restrict, mk-def:over_forget_nat_iso,
        mk-def:over_equiv_functor_is_continuous}
...
\begin{lemma}[The over-equivalence carries unit to unit]\label{mk-lem:sheafofmodules_unit_over_iso}
  \dcref{custom}

  \uses{mk-def:sheafofmodules_over_equivalence, mk-def:phi_over}
  Under the over-equivalence of \ref{mk-def:sheafofmodules_over_equivalence} there is
...
\begin{proof}
  \uses{mk-def:sheafofmodules_over_equivalence}
...
\label{mk-sec:soe_chart}

\begin{lemma}[Over--restrict trivialisation bridge]\label{mk-lem:chart_over_iso}
  \dcref{custom}

  \uses{mk-lem:sheafofmodules_restrict_over_iso, mk-lem:sheafofmodules_unit_over_iso}
...
\begin{proof}
  \uses{mk-lem:sheafofmodules_restrict_over_iso, mk-lem:sheafofmodules_unit_over_iso,
        mk-def:sheafofmodules_over_equivalence}
  ...
  \[
    M.\mathtt{over}\,U
      ...
      \mathtt{unit}\,\bigl(X.\mathtt{ringCatSheaf}.\mathtt{over}\,U\bigr).
  \]
  The first arrow is the inverse of \ref{mk-lem:sheafofmodules_restrict_over_iso}; the
  ...
  arrow is \ref{mk-lem:sheafofmodules_unit_over_iso}. Each factor is an isomorphism,
  so the composite is.

  This lemma addresses the site mismatch in the chart-presentation of
  \ref{mk-lem:lbc_chart_presentation}: the given trivialisation \(e\) lives in
  ...
  \ref{mk-chap:Picard_LineBundleCoherence}. The over-equivalence of
  \ref{mk-def:sheafofmodules_over_equivalence} likewise underlies the internal-Hom
  theory: the commutativity of the internal Hom with the slice-site change of base
  (\ref{mk-lem:dual_restrict_iso}) passes through the same equivalence.
\end{proof}

\begin{definition}[Engine bridge: line-bundle over--restrict trivialisation]\label{mk-def:linebundle_chart_over_iso}
  \dcref{custom}

  \uses{mk-lem:chart_over_iso}
  ...
  precisely the general construction \ref{mk-lem:chart_over_iso}.
\end{definition}

\begin{proof}
  \uses{mk-lem:chart_over_iso}
  \(\Scheme.\mathtt{Modules.chartOverIso}\) of \ref{mk-lem:chart_over_iso} applied to
  the same data \(M\), \(U\), \(e\).
\end{proof}

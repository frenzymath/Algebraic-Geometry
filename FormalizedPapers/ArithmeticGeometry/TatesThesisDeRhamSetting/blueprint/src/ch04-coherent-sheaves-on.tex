...
\]
 ...
 and such that $f_*^{\IndCoh}(\Coh(S)) \subset \QCoh(T)^+$.
...
\]
 ...
 in which the left arrows are $t$-exact equivalences and the
...
\]
 ...
 so this map must be null-homotopic as well.
...
\]
 ...
 By descending induction, we see that there must exist
...
\]
 ...
 with subquotients lying in $H^0(A)/I\mod^{\heart} \subset A\mod^{\heart}$.
...
\]
 ...
 commutes because we assumed $i_*^{\IndCoh}$ $t$-exact;
...
\]
 ...
 for each $r$. It follows that $\sF$ is $*$-extended from:
%
\[
...
\]
 ...
 (Here we use $\iota$ to denote the map for both $\sZ^{\leq n}$
and $\sZ^{\leq n-1}$.)
...
\]
 ...
 obtained by passing to weak $\bG_m$-invariants for
...
\]
 ...
 is $t$-exact. Similarly, there is a unique $t$-structure on
...
\]
 ...
 compatibly with the canonical $\Gr_{\bG_m} =
...
\]
 ...
 where the left hand side is given its convolution monoidal
structure.
...
\]
 ...
 clearly preserves $\Coh(\sZ^{\leq n})$. Therefore, we obtain
a canonical action:
%
\[
...
\]
 ...
 Thus, we obtain a natural action of $\IndCoh(\Gr_{\bG_m}^{\leq n})$
...
\]
 ...
 denote the (conservative) forgetful functor.
...
\]
 ...
 and a $t$-structure on $\IndCoh^*(\sY^{\leq n})$.
...
\]
 ...
 Here the natural functor is fully faithful because
...
\]
 ...
 are good (where $\Coker$ indicates the \emph{homotopy cokernel},
i.e., the cone).

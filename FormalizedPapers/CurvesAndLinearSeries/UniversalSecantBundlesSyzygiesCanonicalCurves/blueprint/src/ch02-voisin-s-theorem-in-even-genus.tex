...
\end{align*}}}
One may now try to work with the sequence $0 \to \mathcal{S}_t  \to \pi_t^*M_L \to \Gamma_t \to 0$. However, each section $t \in \mathrm{H}^0(E)$ comes on an equal footing. It is therefore more natural to work with all of the sections \emph{simultaneously}.
...
${\Gamma:=\text{Ker}\left({q'}^*\mathcal{W} \twoheadrightarrow {p'}^*L_{|_D} \right).}$
...
\end{align*}
and $0 \to q^*\mathcal{O}_{\PP}(-2) \to \mathrm{H}^0(E) \otimes q^*\mathcal{O}_{\PP}(-1) \to \mathcal{G} \to 0.$
...
together with $\mathrm{H}^i(\bigwedge^{k+1-i} M_L \boxtimes \mathcal{O}_{\PP}(-i))=\mathrm{H}^{i+1}(\bigwedge^{k+1-i}M_L \boxtimes \mathcal{O}_{\PP}(-i-1))=0$ for $1 \leq i \leq k$.

...
\section{Codimension-1 indeterminacy extension (A.4.a)}
\label{mk-chap:Albanese_CodimOneExtension}
...
\ref{mk-cor:regular_cohen_macaulay} from
\ref{mk-chap:Albanese_AuslanderBuchsbaum}, and the two codimension-one outputs
feed the extension theorem in \ref{mk-chap:Albanese_Thm32RationalMapExtension}.

\section{The indeterminacy locus of a rational map}
\label{mk-sec:indeterm_locus}

\begin{definition}[Indeterminacy locus of a rational map]\label{mk-def:indeterminacy_locus}
  \dcref{I.3,custom}
...
[Indeterminacy locus is closed]
  \label{mk-lem:isClosed_indeterminacy_locus}
  \dcref{custom}
  \uses{mk-def:indeterminacy_locus}
  For any rational map \(f \colon X \dashrightarrow Y\) of schemes, the
  indeterminacy locus \(Z(f) = X \smallsetminus \mathrm{Dom}(f)\)
  (\ref{mk-def:indeterminacy_locus}) is a closed subset of \(X\): it is the
...
\begin{definition}[Codimension-1-indeterminacy-free rational maps]\label{mk-def:codim_one_indeterminacy}
  \dcref{3.1,custom}

  \uses{mk-def:indeterminacy_locus, mk-def:prime_divisor}
  ...
  \paragraph{Why the abstraction.} Milne's Theorem~3.1 (\ref{mk-thm:codim_one_extension}
  below) and Lemma~3.3 (\ref{mk-lem:milne_codim1_indeterminacy} below) both produce
  ...
  \ref{mk-chap:Albanese_Thm32RationalMapExtension} read off the combination
  (\(Z(f)\) has codim \(\geq 2\) from \ref{mk-thm:codim_one_extension} \emph{and} \(Z(f)\) is
  empty or pure codim 1 from \ref{mk-lem:milne_codim1_indeterminacy} \(\Rightarrow\) \(Z(f)\)
  is empty) as a single line.
\end{definition}

\section{Smoothness yields a DVR at every codim-1 point}
\label{mk-sec:smooth_dvr}
...
\ref{mk-lem:smooth_codim_one_dvr} below reads cleanly as ``regular local of dimension
\(1\) is a DVR'', with the regularity prerequisite supplied by the sub-lemma.

\subsection{K\"ahler differentials under localisation}
\label{mk-subsec:kaehler_localisation_substrate}
...
\ref{mk-subsec:stage6_subgap_decomposition}.

\begin{lemma}[Smooth implies flatness on stalks]\label{mk-lem:stalkMap_flat_of_smooth}
  \dcref{custom}
...
\begin{lemma}[A smooth morphism is locally standard smooth]\label{mk-lem:exists_standardSmooth_at_of_smooth}
  \dcref{custom}
...
\begin{lemma}[A standard-smooth presentation of a smooth stalk]\label{mk-lem:exists_algebra_standardSmooth_stalk_localization}
  \dcref{custom}

  \uses{mk-lem:exists_standardSmooth_at_of_smooth, mk-lem:isLocalization_atPrime_stalk_of_affineOpen}
...
\begin{proof}
  ...
  \ref{mk-lem:exists_standardSmooth_at_of_smooth}.  Their map on section rings is
  standard smooth, and
  \ref{mk-lem:isLocalization_atPrime_stalk_of_affineOpen} identifies the stalk at
  \(z\) with the localisation of the target section ring at the prime of \(z\).
\end{proof}

\begin{lemma}[K\"ahler differentials of a standard-smooth algebra are free]\label{mk-lem:module_free_kaehler_standardSmooth}
  \dcref{custom}
...
\begin{lemma}[K\"ahler-differential freeness is preserved by localisation]\label{mk-lem:module_free_kaehler_localization}
  \dcref{custom}

  \uses{mk-lem:module_free_kaehler_standardSmooth}
...
\begin{lemma}[The rank of localised K\"ahler differentials equals the relative dimension]\label{mk-lem:rank_kaehler_localization_eq_relative_dim}
  \dcref{custom}

  \uses{mk-lem:module_free_kaehler_localization}
  With notation as in \ref{mk-lem:module_free_kaehler_localization},
...
\begin{proof}
  ...
  \ref{mk-lem:module_free_kaehler_localization}, localising this basis gives
...
\label{mk-subsec:stage6_subgap_decomposition}
...
\begin{lemma}
  ...
  \label{mk-lem:smooth_algebra_krull_dim_formula}
  \dcref{00OE,custom}

  \uses{mk-lem:rank_kaehler_localization_eq_relative_dim,
        mk-lem:ringKrullDim_localization_atMaximal_mvPolynomial,
        mk-lem:ringKrullDim_quotient_localization_mvPolynomial_regular,
        mk-lem:matsumura_isRegular_of_linearIndependent_cotangent,
        mk-lem:exists_submersivePresentation_of_standardSmoothReldim,
        mk-lem:submersive_relation_cotangent_linearIndependent_localized}
...
\begin{proof}
  ...
  \ref{mk-lem:rank_kaehler_localization_eq_relative_dim} (the free
...
\begin{lemma}
  ...
  \label{mk-lem:cotangent_kahler_over_field}
  \dcref{02JK,custom}

  \uses{mk-lem:smooth_algebra_krull_dim_formula}
...
\ref{mk-lem:cotangent_kahler_over_field}.

\begin{lemma}[Residue-field tensor dimension from free rank]\label{mk-lem:finrank_residue_kaehler_of_free_rank}
  \dcref{custom}
...
\begin{lemma}[Residue-tensor dimension for a standard-smooth algebra]
  \label{mk-lem:finrank_residue_kaehler_of_standardSmooth_reldim}
  \dcref{custom}

  \uses{mk-lem:finrank_residue_kaehler_of_free_rank}
  ...
  packaging of \ref{mk-lem:finrank_residue_kaehler_of_free_rank}, discharging the
...
\begin{proof}
  ...
  \ref{mk-lem:finrank_residue_kaehler_of_free_rank}.
\end{proof}

\begin{lemma}[Cotangent isomorphism in maximal-ideal form]
  \label{mk-lem:cotangent_iso_maximalIdeal_residue_kaehler}
  \dcref{custom}

  \uses{mk-lem:cotangent_kahler_over_field}
  Under the closed-point hypotheses of \ref{mk-lem:cotangent_kahler_over_field}
  ...
  kernel-side cotangent isomorphism of \ref{mk-lem:cotangent_kahler_over_field} with the
  canonical maximal-ideal cotangent space as its domain.
\end{lemma}

\begin{proof}
  Transport the isomorphism of
  \ref{mk-lem:cotangent_kahler_over_field} along the canonical equality
  \(\ker(S_{\mathfrak q} \to \kappa) = \mathfrak m\).
\end{proof}

\begin{lemma}[Cotangent-space dimension with formally smooth residue field]\label{mk-lem:finrank_cotangentSpace_of_formallySmooth_residue}
  \dcref{custom}

  \uses{mk-lem:cotangent_iso_maximalIdeal_residue_kaehler, mk-lem:finrank_residue_kaehler_of_free_rank}
  With the closed-point hypotheses of
  \ref{mk-lem:cotangent_iso_maximalIdeal_residue_kaehler} and
...
\begin{proof}
  ...
  \ref{mk-lem:cotangent_iso_maximalIdeal_residue_kaehler} with the residue-tensor
  dimension formula \ref{mk-lem:finrank_residue_kaehler_of_free_rank}.
\end{proof}

\begin{lemma}[Cotangent-space dimension at a $\bar k$-rational point]\label{mk-lem:finrank_cotangentSpace_of_bijective_residue}
  \dcref{custom}

  \uses{mk-lem:finrank_cotangentSpace_of_formallySmooth_residue}
  Bundled closed-point form of
  \ref{mk-lem:finrank_cotangentSpace_of_formallySmooth_residue}: if \(S_{\mathfrak q}\)
...
\begin{proof}
  ...
  \ref{mk-lem:finrank_cotangentSpace_of_formallySmooth_residue}.
\end{proof}

\begin{lemma}
  [A standard-smooth stalk is regular local]
  \label{mk-lem:stage6_regular_stalk_assembly}
  \dcref{custom}

  \uses{mk-lem:rank_kaehler_localization_eq_relative_dim,
        mk-lem:smooth_algebra_krull_dim_formula,
        mk-lem:cotangent_kahler_over_field}
...
\begin{proof}
  ...
  \begin{enumerate}
    \item By \ref{mk-lem:rank_kaehler_localization_eq_relative_dim},
      the localised K\"ahler module \(\Omega_{S_{\mathfrak q} / \bar k}\)
      is free of rank \(n\) over \(S_{\mathfrak q}\).
    \item By \ref{mk-lem:smooth_algebra_krull_dim_formula}, the Krull dimension of
    ...
    \item By \ref{mk-lem:cotangent_kahler_over_field}, the cotangent exact
...
[Polynomial-ring maximal ideal height lower bound]
  \label{mk-lem:mvPolynomial_maximalIdeal_height_ge_card}
  \dcref{custom}
...
\begin{lemma}[Polynomial-ring ideal height upper bound]
  \label{mk-lem:mvPolynomial_maximalIdeal_height_le_natCard}
  \dcref{custom}
...
[Maximal-ideal height in a polynomial ring indexed by $\mathrm{Fin}\,n$]
  \label{mk-lem:mvPolynomial_maximalIdeal_height_eq_card}
  \dcref{custom}
  \uses{mk-lem:mvPolynomial_maximalIdeal_height_ge_card, mk-lem:mvPolynomial_maximalIdeal_height_le_natCard}
...
\begin{proof}
  ...
  \ref{mk-lem:mvPolynomial_maximalIdeal_height_ge_card} and the upper bound
  \ref{mk-lem:mvPolynomial_maximalIdeal_height_le_natCard} by antisymmetry.
\end{proof}

\begin{lemma}[Maximal-ideal height in a finitely generated polynomial ring]
  \label{mk-lem:mvPolynomial_maximalIdeal_height_eq_natCard}
  \dcref{custom}

  \uses{mk-lem:mvPolynomial_maximalIdeal_height_eq_card}
...
\begin{proof}
  ...
  \ref{mk-lem:mvPolynomial_maximalIdeal_height_eq_card} along the renaming isomorphism
  that it induces. Ring isomorphisms preserve heights of ideals.
\end{proof}

\begin{lemma}[Krull dimension of a localised polynomial ring]
  \label{mk-lem:ringKrullDim_localization_atMaximal_mvPolynomial}
  \dcref{custom}

  \uses{mk-lem:ringKrullDim_localization_eq_height_atPrime, mk-lem:mvPolynomial_maximalIdeal_height_eq_natCard}
...
\begin{proof}
  By
  \ref{mk-lem:ringKrullDim_localization_eq_height_atPrime} (Krull dimension equals
  height), the dimension is the height of \(\mathfrak m\), which is
  \(\#\iota\) by \ref{mk-lem:mvPolynomial_maximalIdeal_height_eq_natCard}.
\end{proof}

\begin{lemma}
[Dimension drop by a regular sequence]
  \label{mk-lem:ringKrullDim_quotient_add_eq_regular_sequence}
  \dcref{custom}
...
\begin{lemma}[Dimension of a localised polynomial quotient]
  \label{mk-lem:ringKrullDim_quotient_localization_mvPolynomial_regular}
  \dcref{custom}

  \uses{mk-lem:ringKrullDim_localization_atMaximal_mvPolynomial, mk-lem:ringKrullDim_quotient_add_eq_regular_sequence}
...
\begin{proof}
  ...
  \ref{mk-lem:ringKrullDim_localization_atMaximal_mvPolynomial}; now apply
  \ref{mk-lem:ringKrullDim_quotient_add_eq_regular_sequence}.
\end{proof}
...
[00SW --- relations are cotangent-independent]
  \label{mk-lem:submersive_relation_cotangent_linearIndependent}
  \dcref{custom}
...
[00SW --- cotangent independence transported to a localisation]
  \label{mk-lem:submersive_relation_cotangent_linearIndependent_localized}
  \dcref{custom}
  \uses{mk-lem:submersive_relation_cotangent_linearIndependent}
...
\begin{proof}
  Transport
  \ref{mk-lem:submersive_relation_cotangent_linearIndependent} through the localisation
...
\begin{lemma}[Existence of a relative dimension for a standard-smooth algebra]\label{mk-lem:exists_standardSmoothOfRelativeDimension_of_standardSmooth}
  \dcref{custom}
...
\begin{lemma}[Submersive presentation of a relative-dimension-$n$ algebra]\label{mk-lem:exists_submersivePresentation_of_standardSmoothReldim}
  \dcref{custom}

  \uses{mk-lem:exists_standardSmoothOfRelativeDimension_of_standardSmooth}
...
\begin{lemma}[An affine-open stalk is a localisation]\label{mk-lem:isLocalization_atPrime_stalk_of_affineOpen}
  \dcref{custom}
...
\begin{lemma}[A nonempty open in a one-point space is the whole space]\label{mk-lem:open_eq_top_of_subsingleton}
  \dcref{custom}
...
\begin{lemma}[Sections of $\Spec \bar k$ over a nonempty open]\label{mk-lem:gammaSpecField_ringEquiv}
  \dcref{custom}

  \uses{mk-lem:open_eq_top_of_subsingleton}
...
\begin{proof}
  ...
  \ref{mk-lem:open_eq_top_of_subsingleton} forces \(U = \top\) and the sections collapse
  to the global sections \(\Gamma(\Spec \bar k, \top) \simeq \bar k\).
\end{proof}

\paragraph{Matsumura regular-sequence bridge (Stacks 00NQ).}

\begin{lemma}[Module quotient equals ring quotient]\label{mk-lem:quotSMulTop_quotientRing_linearEquiv}
  \dcref{custom}
...
\begin{lemma}[Ring-quotient cons rule for regular sequences]\label{mk-lem:isRegular_cons_of_quotient_ring}
  \dcref{custom}

  \uses{mk-lem:quotSMulTop_quotientRing_linearEquiv}
...
\begin{proof}
  ...
  \ref{mk-lem:quotSMulTop_quotientRing_linearEquiv} to identify this module
  quotient with \(A/(r)\).
\end{proof}

\begin{lemma}[Cotangent independence descends to a quotient]\label{mk-lem:matsumura_descent_cotangent}
  \dcref{custom}
...
\begin{lemma}[Matsumura: cotangent-independent elements form a regular sequence]\label{mk-lem:matsumura_isRegular_of_linearIndependent_cotangent}
  \dcref{custom}

  \uses{mk-lem:matsumura_descent_cotangent, mk-lem:isRegular_cons_of_quotient_ring}
...
\begin{proof}
  ...
  quotient \(A/(f_1)\) by \ref{mk-lem:matsumura_descent_cotangent}, and the
  ring-quotient cons rule \ref{mk-lem:isRegular_cons_of_quotient_ring} reassembles the
  ...
  \ref{mk-lem:submersive_relation_cotangent_linearIndependent_localized}.
\end{proof}

\begin{lemma}[Smooth over \(\bar k\) \(\Rightarrow\) regular local stalk (Stacks 00TT)]\label{mk-lem:smooth_to_regular_local_ring}
  \dcref{00TT,custom}

  \uses{mk-lem:module_free_kaehler_localization, mk-lem:cotangent_kahler_over_field,
        mk-lem:smooth_algebra_krull_dim_formula, mk-lem:stage6_regular_stalk_assembly,
        mk-lem:stalkMap_flat_of_smooth, mk-lem:exists_algebra_standardSmooth_stalk_localization,
        mk-lem:finrank_cotangentSpace_of_bijective_residue, mk-lem:gammaSpecField_ringEquiv,
        mk-lem:exists_standardSmoothOfRelativeDimension_of_standardSmooth}
...
\begin{lemma}[Smooth at a codim-1 point $\Rightarrow$ principal nonzero maximal ideal]\label{mk-lem:smooth_codim_one_maximalIdeal_principal}
  \dcref{custom}

  \uses{mk-lem:smooth_to_regular_local_ring, mk-thm:ringKrullDim_stalk_eq_coheight}
...
\begin{proof}
  ...
  \ref{mk-lem:smooth_to_regular_local_ring}; the coheight bridge
  \ref{mk-thm:ringKrullDim_stalk_eq_coheight} gives Krull dimension \(1\); the
  ...
  \ref{mk-lem:smooth_codim_one_dvr}.
\end{proof}

\begin{lemma}[Smooth \(\Rightarrow\) DVR at every codim-\(1\) point]\label{mk-lem:smooth_codim_one_dvr}
  \dcref{II.6,custom}

  \uses{mk-lem:smooth_to_regular_local_ring, mk-thm:ringKrullDim_stalk_eq_coheight,
        mk-lem:smooth_codim_one_maximalIdeal_principal}
...
\begin{proof}
  ...
  By \ref{mk-lem:smooth_to_regular_local_ring} (Stacks tag \texttt{00TT}, the
...
\begin{lemma}[Image of the closed point of \(\Spec K(X)\)]\label{mk-lem:fromFunctionField_base_eq_genericPoint}
  \dcref{custom}
...
\begin{proof}Choose a representative \((U, \varphi_U)\) of \(f\); its underlying morphism
...
\begin{definition}\label{mk-def:functionFieldPullback}
  \dcref{custom}

  \uses{mk-lem:fromFunctionField_base_eq_genericPoint}
  ...
  \ref{mk-lem:fromFunctionField_base_eq_genericPoint} with the stalk
  \(\mathcal O_{Y,\eta_Y} = K(Y)\).
\end{definition}

The difference map of Milne's Lemma~3.3 (\ref{mk-sec:milne_lem33} below) is built by
...
\begin{definition}\label{mk-def:rationalMap_precomp}
  \dcref{custom}
...
\begin{lemma}\label{mk-lem:rationalMap_precomp_compHom}
  \dcref{custom}

  \uses{mk-def:rationalMap_precomp}
  ...
  \(f \circ p\).
\end{lemma}
\begin{proof}Both sides act on a chosen partial-map representative \((V, \varphi_V)\) of \(f\) by
...
\begin{lemma}\label{mk-lem:rationalMap_precomp_hom}
  \dcref{custom}
  ...
  \]
\end{lemma}
\begin{proof}Both sides are everywhere-defined; on the whole of \(W\) the left-hand side is
...
\begin{definition}\label{mk-def:rationalMap_prod}
  \dcref{custom}
...
\begin{lemma}\label{mk-lem:rationalMap_prod_proj}
  \dcref{custom}

  \uses{mk-def:rationalMap_prod}
  ...
  \(S\)).
\end{lemma}
\begin{proof}Right-composition with a morphism is functorial on generic-point morphisms:
...
\begin{definition}\label{mk-def:grpObj_diff}
  \dcref{custom}
...
\begin{lemma}\label{mk-lem:grpObjDiffLeft_comp_hom}
  \dcref{custom}

  \uses{mk-def:grpObj_diff}
...
\end{lemma}
\begin{proof}This is the triangle identity \(\varphi_{\mathrm{left}} \circ g_{\mathrm{hom}} =
...
\begin{definition}\label{mk-def:rationalMap_difference}
  \dcref{custom}

  \uses{mk-def:rationalMap_precomp, mk-def:rationalMap_prod, mk-def:grpObj_diff}
  ...
  (\ref{mk-lem:rationalMap_precomp_compHom}, \ref{mk-lem:rationalMap_precomp_hom}) with common
...
\begin{lemma}\label{mk-lem:rationalMap_difference_over}
  \dcref{custom}

  \uses{mk-def:rationalMap_difference, mk-lem:rationalMap_prod_proj, mk-lem:grpObjDiffLeft_comp_hom}
  ...
  \(X \times_{\bar k} X\).
\end{lemma}
\begin{proof}By associativity of right-composition, \(\Phi \circ (G \to \operatorname{Spec}\bar k) =
  ...
  by \ref{mk-lem:grpObjDiffLeft_comp_hom} the inner composite is
  \(\mathrm{pr}_1 \circ (G \to \operatorname{Spec}\bar k)\), and the over-\(S\) identity of the
  pairing (\ref{mk-lem:rationalMap_prod_proj}) evaluates the result to the structure morphism of
  \(X \times_{\bar k} X\).
\end{proof}
...
\begin{lemma}\label{mk-lem:compHom_domain_monotone}
  \dcref{custom}
...
\end{lemma}
\begin{proof}A partial-map representative \((V, F_V)\) of \(F\) yields the representative
  \((V, F_V \circ g)\) of \(F \circ g\) with the same domain \(V\).
\end{proof}

\begin{lemma}\label{mk-lem:difference_map_domain_lower_bound}
  \dcref{3.3,custom}

  \uses{mk-def:rationalMap_difference, mk-def:rationalMap_precomp, mk-def:rationalMap_prod,
        mk-lem:compHom_domain_monotone}
  ...
  defined at \(x\).
\end{lemma}
\begin{proof}Right-composition with \(\operatorname{diff}_G\) only enlarges the domain of
  definition (\ref{mk-lem:compHom_domain_monotone}), so it suffices to show the pairing
  ...
  the pairing (\ref{mk-def:rationalMap_prod}) factor-by-factor. Hence it represents the
...
\begin{lemma}\label{mk-lem:rationalMap_precomp_domain_bound}
  \dcref{custom}
...
\end{lemma}
\begin{proof}If \(p(w) \in \mathrm{Dom}(f)\), choose a partial-map representative \((V, f_V)\) of
  \(f\) with \(p(w) \in V\). Then \((p^{-1}V, f_V \circ (p \mid_V))\) is a representative
  of \(f \circ p\) defined at \(w\), so \(w \in \mathrm{Dom}(f \circ p)\).
\end{proof}

\begin{lemma}\label{mk-lem:rationalMap_prod_domain_bound}
  \dcref{custom}
...
\end{lemma}
\begin{proof}The maximal representatives of \(a\) and \(b\) are defined on \(\mathrm{Dom}(a)\),
...
\begin{lemma}\label{mk-lem:grpObj_reconstruction}
  \dcref{custom}
...
\end{lemma}
\begin{proof}Precomposition by \(\langle a, b\rangle\) is a group homomorphism on \(T\)-points, so
...
\begin{lemma}\label{mk-lem:scheme_reconstruction}
  \dcref{custom}

  \uses{mk-lem:grpObj_reconstruction, mk-def:grpObj_diff}
  ...
  \]
\end{lemma}
\begin{proof}The forgetful functor \((-)^{\mathrm{left}} \colon \mathbf{Sch}/\bar k \to \mathbf{Sch}\)
  ...
  \((-)^{\mathrm{left}}\) to \ref{mk-lem:grpObj_reconstruction} yields the claim.
\end{proof}

\begin{lemma}\label{mk-lem:rationalMap_difference_reconstruct}
  \dcref{custom}

  \uses{mk-def:rationalMap_difference, mk-def:rationalMap_prod, mk-lem:scheme_reconstruction}
  ...
  \]
\end{lemma}
\begin{proof}Both sides are \(\bar k\)-rational maps out of the integral scheme \(X \times_{\bar k}
  ...
  \ref{mk-lem:scheme_reconstruction}.
\end{proof}

\begin{lemma}\label{mk-lem:difference_map_domain_reconstruction}
  \dcref{3.3,custom}

  \uses{mk-lem:rationalMap_difference_reconstruct, mk-lem:rationalMap_precomp_domain_bound,
        mk-lem:rationalMap_prod_domain_bound, mk-lem:compHom_domain_monotone}
...
\end{lemma}
\begin{proof}Rewriting \(f \circ \mathrm{pr}_1\) by
  \ref{mk-lem:rationalMap_difference_reconstruct}, it suffices to bound
  \(\mathrm{Dom}\bigl(m^{\mathrm{left}} \circ (\Phi \times (f \circ \mathrm{pr}_2))\bigr)\).
  Right-composition only enlarges the domain (\ref{mk-lem:compHom_domain_monotone}), and the
  pairing is defined on \(\mathrm{Dom}(\Phi) \cap \mathrm{Dom}(f \circ \mathrm{pr}_2)\)
  (\ref{mk-lem:rationalMap_prod_domain_bound}), which contains \(\mathrm{Dom}(\Phi) \cap
  \mathrm{pr}_2^{-1}(\mathrm{Dom}(f))\) by \ref{mk-lem:rationalMap_precomp_domain_bound}.
\end{proof}
...
\begin{theorem}[Milne Theorem~3.1: codim-\(\geq 2\) extension to a complete target]\label{mk-thm:codim_one_extension}
  \dcref{3.1,custom}

  \uses{mk-def:indeterminacy_locus, mk-def:codim_one_indeterminacy,
        mk-lem:smooth_codim_one_dvr}
  ...
  indeterminacy locus \(Z(f)\) (\ref{mk-def:indeterminacy_locus}) has codimension
  \(\geq 2\) in \(X\); equivalently, \(f\) is codim-\(1\)-indeterminacy-free
  (\ref{mk-def:codim_one_indeterminacy}).
\end{theorem}
\begin{proof}Fix an indeterminacy point \(z \in Z(f)\); we must show its coheight is \(\geq 2\).
  ...
  \ref{mk-lem:smooth_codim_one_dvr} the local ring \(\mathcal O_{X, z}\) is a discrete
...
\begin{remark}
  \label{mk-rmk:codim1_role_of_ab}
  \dcref{II.4.7,custom}
  \textbf{Milne 3.1 is valuative-only; Auslander--Buchsbaum is not used here.} The proof
  above uses only normality (via the codim-\(1\) DVR of \ref{mk-lem:smooth_codim_one_dvr})
  ...
  (\ref{mk-thm:codim_one_extension} together with Milne~3.3 gives
  \ref{mk-lem:av_indeterminacyLocus_eq_empty}, \(Z(f) = \emptyset\), and then
  \ref{mk-thm:rational_map_to_av_extends} extends across the empty locus). The
  ...
  Where \ref{mk-cor:regular_cohen_macaulay} and the Auslander--Buchsbaum machinery of
  \ref{mk-chap:Albanese_AuslanderBuchsbaum} \emph{do} enter is the Milne~3.3 analysis of a
...
Combined with \ref{mk-thm:codim_one_extension}'s ``codimension \(\geq 2\)'' conclusion,
...
\begin{lemma}[Slice existence for the fibre of the self-product, and its difference-map corollary]\label{mk-lem:milne33_slice_existence}
  \dcref{3.3,custom}

  \uses{mk-lem:difference_map_domain_lower_bound, mk-lem:difference_map_domain_reconstruction}
  ...
  (\ref{mk-def:rationalMap_difference}) is defined at a point \(p_0\) lying over
...
\end{lemma}
\begin{proof}The fibre \(\mathrm{pr}_1^{-1}\{x\}\) is the base change
  ...
  \ref{mk-lem:difference_map_domain_reconstruction} places \(p\) in the domain of
  \(f \circ \mathrm{pr}_1\).
\end{proof}

\begin{lemma}[Krull's Hauptidealsatz, transported to a height-one prime below a given prime]\label{mk-lem:krull_hauptidealsatz_transport}
  \dcref{3.3,custom}
...
\end{lemma}
\begin{proof}The principal ideal \((t)\) is contained in \(\mathfrak p\), so it has a
...
\begin{lemma}[Regularity propagates to a generisation]\label{mk-lem:regular_stalk_propagates_to_generisation}
  \dcref{custom}
...
\end{lemma}
\begin{proof}Immediate from functoriality of stalks along the specialisation map: for
...
\begin{lemma}[Zero loci of regular functions are pure codimension one: the diagonal transport]\label{mk-lem:milne33_diagonal_vanishing_purity}
  \dcref{3.3,custom}

  \uses{mk-lem:regular_stalk_propagates_to_generisation}
...
\end{lemma}
\begin{proof}\uses{mk-lem:regular_stalk_propagates_to_generisation}
  ...
  stalks, an instance of \ref{mk-lem:krull_hauptidealsatz_transport} applied to
  a local equation of the locus at \(P\) --- produces a coheight-\(1\)
  generisation \(z \leadsto P\) at which \(t^{-1}\) is still not regular. By
  \ref{mk-lem:regular_stalk_propagates_to_generisation}, \(t\) itself remains
...
\begin{lemma}[Milne Lemma~3.3: pure-codim-\(1\) structure of the indeterminacy locus into a group variety]\label{mk-lem:milne_codim1_indeterminacy}
  \dcref{3.3,custom}

  \uses{mk-def:indeterminacy_locus, mk-def:codim1_cycles, mk-def:order_at_point,
        mk-def:principal_divisor}
...
\end{lemma}
\begin{proof}\uses{mk-def:rationalMap_precomp, mk-lem:rationalMap_precomp_compHom,
        mk-lem:rationalMap_precomp_hom, mk-def:rationalMap_prod, mk-lem:rationalMap_prod_proj,
        mk-def:grpObj_diff, mk-lem:grpObjDiffLeft_comp_hom, mk-def:rationalMap_difference,
        mk-lem:rationalMap_difference_over, mk-lem:compHom_domain_monotone,
        mk-lem:difference_map_domain_lower_bound, mk-lem:rationalMap_precomp_domain_bound,
        mk-lem:rationalMap_prod_domain_bound, mk-lem:grpObj_reconstruction,
        mk-lem:scheme_reconstruction, mk-lem:rationalMap_difference_reconstruct,
        mk-lem:difference_map_domain_reconstruction, mk-lem:milne33_slice_existence,
        mk-lem:milne33_diagonal_vanishing_purity}
  ...
  (\ref{mk-def:rationalMap_precomp}, whose over-\(\bar k\) part is
  \ref{mk-lem:rationalMap_precomp_compHom}, using \ref{mk-lem:rationalMap_precomp_hom} to
  identify \((s_X \circ \mathrm{pr}_i)\) with the structure morphism of \(X \times X\));
  pairing them into \(X \times X \dashrightarrow
  G \times G\) (\ref{mk-def:rationalMap_prod}, whose projection and over-\(\bar k\) properties
  are \ref{mk-lem:rationalMap_prod_proj}) and composing on the right with the difference
  morphism \(\operatorname{diff}_G^{\,\mathrm{left}} = m \circ (\mathrm{id} \times \mathrm{inv})\)
  (\ref{mk-def:grpObj_diff}) gives the named difference map \(\Phi\)
  (\ref{mk-def:rationalMap_difference}); it is again over \(\bar k\)
  (\ref{mk-lem:rationalMap_difference_over}, using \ref{mk-lem:grpObjDiffLeft_comp_hom}). Its
  domain is at least \(U \times U \subseteq X \times X\)
  (\ref{mk-lem:difference_map_domain_lower_bound}), which is dense because \(U\) is
  dense in \(X\).
  ...
  \mathrm{Dom}(\Phi)\) (\ref{mk-lem:difference_map_domain_lower_bound}), so \(\Phi\) is
  ...
  \ref{mk-lem:rationalMap_difference_reconstruct}, and its domain consequence
  \(\mathrm{Dom}(\Phi) \cap \mathrm{pr}_2^{-1}(\mathrm{Dom}(\varphi)) \subseteq
  \mathrm{Dom}(\varphi \circ \mathrm{pr}_1)\) is
  \ref{mk-lem:difference_map_domain_reconstruction}. A suitable \(u\) exists by openness of
  \(\mathrm{Dom}(\Phi)\) and irreducibility of the fibre \(X_{\kappa(x)}\), which follows
  from geometric irreducibility of \(X\); this is
  \ref{mk-lem:milne33_slice_existence}. Finally, the smooth-descent reflection
  ...
  apparatus of \ref{mk-def:codim1_cycles} and the order map of
  \ref{mk-def:order_at_point} apply on \(X \times X\). For any nonzero rational function
  ...
  \ref{mk-def:principal_divisor}: \(\mathrm{div}(f)_0\) collects the prime divisors with
  ...
  point of \(\Delta_X\) is recorded as \ref{mk-lem:milne33_diagonal_vanishing_purity},
  built from the ring-theoretic core \ref{mk-lem:krull_hauptidealsatz_transport}).
...
\begin{remark}
  \label{mk-rmk:milne_lem33_charfree}
  \dcref{3.3,custom}
  ...
  \ref{mk-chap:RiemannRoch_WeilDivisor} in the project's notation); (iii) the
  ...
  restriction, so \ref{mk-lem:milne_codim1_indeterminacy} holds over every
...
\label{mk-sec:weil_obstruction}

The proof of \ref{mk-thm:codim_one_extension}'s Step~1 (codim-\(1\) ruling-out) goes
through the valuative criterion. In the project's Weil-divisor language
(\ref{mk-chap:RiemannRoch_WeilDivisor}) the same obstruction has a cleaner phrasing:
...
recorded by \ref{mk-def:order_at_point} and \ref{mk-def:principal_divisor}.

\begin{theorem}

  [Weil-divisor criterion for codimension-one extension]
  \label{mk-thm:weil_divisor_obstruction}
  \dcref{II.6,custom}

  \uses{mk-def:codim_one_indeterminacy, mk-def:order_at_point, mk-lem:smooth_codim_one_dvr,
        mk-lem:mem_domain_partial_map_reshuffle}
  ...
  (\ref{mk-def:codim_one_indeterminacy}) if and only if, for every prime divisor
...
\begin{proof}
  ...
  By \ref{mk-lem:smooth_codim_one_dvr}, the local ring \(\mathcal O_{X, \eta_W}\) is a
  DVR with fraction field \(K(X)\) and uniformiser determined by \(W\); by
  \ref{mk-def:order_at_point} its valuation is exactly \(\ord_W\). The local ring at
...
\begin{remark}
  \label{mk-rmk:weil_obstruction_application}
  \dcref{custom}
  \textbf{Role of the valuation form.} In
  \ref{mk-chap:Albanese_Thm32RationalMapExtension}, the abstract codimension statement
  \ref{mk-thm:codim_one_extension} suffices and \ref{mk-thm:weil_divisor_obstruction}
  ...
  for the symmetric-product arguments of \ref{mk-chap:Albanese_AlbaneseUP}.
\end{remark}
...
\begin{lemma}
  ...
  \label{mk-lem:mem_domain_partial_map_reshuffle}
  \dcref{custom}
...
\begin{remark}
  \label{mk-rmk:mem_domain_partial_map_reshuffle_scope}
  \dcref{custom}
  \textbf{Relationship to \ref{mk-thm:weil_divisor_obstruction}.}
  \ref{mk-lem:mem_domain_partial_map_reshuffle} captures the definitional half of
  ...
  \ref{mk-thm:weil_divisor_obstruction} is the identification of the local ring with
...
\begin{enumerate}
  \item Smoothness makes every codimension-one local ring a discrete
    valuation ring (\ref{mk-lem:smooth_codim_one_dvr}).
  \item Properness and the valuative criterion then rule out
    codimension-one indeterminacy (\ref{mk-thm:codim_one_extension}).

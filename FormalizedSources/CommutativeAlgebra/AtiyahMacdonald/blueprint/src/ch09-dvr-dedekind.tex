...
\label{am-prop-9-1}
\dcref{9.1}
\source{atiyah-macdonald-commutative-algebra:page-0102}\uses{}
...
\source{atiyah-macdonald-commutative-algebra:page-0103}\uses{}
...
\dcref{9.2}
\source{atiyah-macdonald-commutative-algebra:page-0103}\uses{}
For a Noetherian local domain $(A,\mathfrak{m})$ of dimension one, the following are
equivalent: $A$ is a DVR; $A$ is integrally closed; $\mathfrak{m}$ is principal;
$\dim_{A/\mathfrak{m}}\mathfrak{m}/\mathfrak{m}^2=1$; every nonzero ideal is a power of $\mathfrak{m}$; and there is
$x\in A$ with every nonzero ideal $(x^n)$.
\end{proposition}
\begin{proof}
DVRs are integrally closed by Proposition~\ref{am-prop-5-18}.  If $A$ is
integrally closed, choose $a\in\mathfrak{m}$ of minimal valuation and $b\in\mathfrak{m}^{n-1}$
outside $(a)$; the element $a/b$ forces $\mathfrak{m}=(a/b)$.  A principal maximal ideal
gives one-dimensional $\mathfrak{m}/\mathfrak{m}^2$ by Nakayama, and conversely this dimension
and Proposition~\ref{am-prop-2-8} make $\mathfrak{m}$ principal.  Applying the principal
ideal argument to $A/\mathfrak{m}^n$ shows every ideal is a power of $\mathfrak{m}$.  A generator
of $\mathfrak{m}$ then gives all powers, and assigning to each nonzero element the
...
\label{am-thm-9-3}
\dcref{9.3}
\source{atiyah-macdonald-commutative-algebra:page-0104}\uses{}
...
localization $A_\mathfrak{p}$ at a nonzero prime is a DVR.  A ring with these properties
...
\label{am-thm-9-5}
\dcref{9.5}
\source{atiyah-macdonald-commutative-algebra:page-0105}\uses{}
...
\label{am-prop-9-6}
\dcref{9.6}
\source{atiyah-macdonald-commutative-algebra:page-0106}\uses{}
For a fractional ideal $M$, invertibility is equivalent to finite generation
and invertibility of every $M_\mathfrak{p}$, and equivalently finite generation and
invertibility of every $M_\mathfrak{m}$.
\end{proposition}
\begin{proof}
If $MN=A$, localize and use ideal quotients (Corollary~\ref{am-cor-3-15}) to
obtain $M_\mathfrak{p}(A_\mathfrak{p}:M_\mathfrak{p})=A_\mathfrak{p}$.  Conversely let
$I=M(A:M)\subseteq A$.  Local invertibility gives $I_\mathfrak{m}=A_\mathfrak{m}$ for all maximal
$\mathfrak{m}$, hence $I=A$ by local detection (Proposition~\ref{am-prop-3-9}); so
$M$ is invertible.
...
invertible integral ideals are finite; choose a maximal non-power ideal $\mathfrak{a}$.
Then $\mathfrak{m}^{-1}\mathfrak{a}$ is a strictly larger proper ideal unless $\mathfrak{a}=0$,
contradicting maximality and Nakayama.  Thus all ideals are powers of $\mathfrak{m}$ and
...
nonzero prime $\mathfrak{p}$, every integral ideal of $A_\mathfrak{p}$ is the localization of an
invertible ideal and is invertible; Proposition~\ref{am-prop-9-7} makes
$A_\mathfrak{p}$ a DVR, hence Theorem~\ref{am-thm-9-3} applies.
\uses{am-prop-9-6,am-prop-9-7,am-thm-9-3}
\end{proof}
\begin{corollary}[Ideal group and class group]
\label{am-cor-9-9}
\dcref{9.9}
\source{atiyah-macdonald-commutative-algebra:page-0107}\uses{}

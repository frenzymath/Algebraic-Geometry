...
Each subbundle $E_i$ also induces a Hecke transform using
$V_i:=E_i|_c\cap V$ and $W^i:=E_i|_c/V_i$. If
$f_{V_i}:E_i\to W^i_c$ is the induced map, let $E_i^\prime$ be its
kernel. These kernels fit into compatible short exact sequences
\begin{equation}
\label{commutative}
0\to E_i^\prime\to E_i\to W^i_c\to 0,\qquad
0\to E_{i+1}^\prime\to E_{i+1}\to W^{i+1}_c\to 0,
\end{equation}
\[
0\to E_{i+1}^\prime/E_i^\prime\to E_{i+1}/E_i
\to (W^{i+1}/W^i)_c\to 0.
\]
\source{very-stable-higgs-bundles-equivariant-multiplicity-mirror-symmetry}
The inclusions and quotient maps are the unique ones making these

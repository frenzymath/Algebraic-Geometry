...
\begin{proof}
  \leanokEvery element of \(\mathfrak m\) acts by zero on the \(R\)-module
...
\begin{proof}
  \leanokWrite \(\sigma : \kappa \to k\) for the inverse of the structure
...
\begin{proof}
  \leanokWrite \(f(r) = a(r) + b(r)\varepsilon\). Splitting
...
\begin{proof}
  \leanokCompose the bijections of
...
\begin{proof}
  \leanokBoth \(\mathfrak m/\mathfrak m^2\) and \(\kappa\) are modules over
...
\begin{proof}
  \leanokCompose \ref{mk-cor:tangent_dual_number_cotangent_dual} with the
...
\begin{proof}
  \leanokThe dual numbers \(k_\varepsilon\) form a local ring whose maximal
...
\begin{proof}
  \leanokThis is the fiber at \(x\) of the standard description of morphisms
...
\begin{proof}
  \leanokInstantiate \ref{mk-lem:tangent_spec_local_ring_at} at
...
\begin{proof}
  \leanokBy naturality of the canonical map out of the spectrum of a stalk,
...
\begin{proof}
  \leanok\uses{mk-lem:tangent_from_spec_stalk_over, mk-lem:tangent_spec_local_ring_at}
...
\begin{proof}
  \leanokUnder the bijection of \ref{mk-cor:tangent_spec_dual_number_at},
...
\begin{proof}
  \leanokCompose the bijection of \ref{mk-cor:tangent_over_dual_number_alghom}
...
\begin{proof}
  \leanok\uses{mk-lem:tangent_over_triangle_iff}
...
\begin{proof}
  \leanok\uses{mk-lem:tangent_section_retraction}
...
\begin{proof}
  \leanok\uses{mk-thm:tangent_over_dual_number_cotangent_dual}
...
\begin{proof}
  \leanokThe unit \(\eta\) is a morphism of schemes over
...
\begin{proof}
  \leanok\uses{mk-cor:tangent_section_cotangent_dual,
...
\begin{proof}
  \leanok\uses{mk-thm:identity_component_is_subgroup_homomorphism,
...
\begin{proof}
  \leanok\uses{mk-thm:tangent_identity_cotangent_dual, mk-thm:pic0_group_structure}
...
\begin{proof}
  \leanokThe projection is additive and unital, and multiplicative because
...
\begin{proof}
  \leanok\((1 + b\varepsilon)(1 - b\varepsilon) = 1 - b^2\varepsilon^2 = 1\)
...
\begin{proof}
  \leanokThe \(\varepsilon\)-component of \(1 + b\varepsilon\) is \(b\).
\end{proof}
...
\begin{proof}
  \leanokThe first component of \(a + 0\cdot\varepsilon\) is \(a\).
\end{proof}
...
\begin{proof}
  \leanokApply the ring homomorphism of
...
\begin{proof}
  \leanok\uses{mk-lem:dual_number_trunc_exp_injective}
...
\begin{proof}
  \leanok\uses{mk-lem:dual_number_fst_unit_inv, mk-lem:dual_number_units_fst_split,
        mk-lem:dual_number_trunc_exp_exact}
...
\begin{proof}
  \leanokImmediate from the componentwise formula.
\end{proof}
...
\begin{proof}
  \leanokBoth composites send \(a + b\varepsilon\) to \(f(a)\), on first
  components.
\end{proof}
...
\begin{proof}
  \leanokThe map \(R \to M\), \(r \mapsto rm\), is surjective precisely because \(m\)
...
\begin{proof}
  \leanokInduction on \(n\). Substituting \(M = N + IM\) into \(I^{n}M\) gives
...
\begin{proof}
  \leanokThe hypothesis says \(Rm + IM = M\). By
...
\begin{proof}
  \leanokThe kernel consists of the elements with vanishing constant term. For two
...
\begin{proof}
  \leanokThe augmentation ideal is square-zero by
...
\begin{proof}
  \leanok\uses{mk-thm:identity_component_finite_type_geom_irreducible,
        mk-thm:identity_component_base_change_commutes,
        mk-thm:fga_pic_representability}

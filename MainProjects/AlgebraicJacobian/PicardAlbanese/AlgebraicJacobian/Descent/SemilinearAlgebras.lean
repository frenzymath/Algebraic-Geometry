...
-/
import AlgebraicJacobian.Descent.SemilinearModules

/-!
# Galois descent for semilinear algebras

Ported from the sibling Algebraic Jacobian project
(`AlgebraicJacobian.Picard.GaloisDescent.SemilinearAlgebras`).

This file upgrades the module-level Speiser descent of
`AlgebraicJacobian.Descent.SemilinearModules` to **algebras**: it is the
...
* `SemilinearAction.invariantAlgHomEquiv` — **the affine Hom property**:
  ...
  `AlgebraicJacobian.Descent.FiniteGaloisQuotient`).

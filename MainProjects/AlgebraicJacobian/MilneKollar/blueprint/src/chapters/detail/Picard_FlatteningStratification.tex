\section{Flattening Stratification of a Coherent Sheaf}
\label{mk-chap:Picard_FlatteningStratification}

\section{Coherent flatness over a base}
\label{mk-sec:coh_flat}

\begin{definition}[Coherent sheaf flat over the base]\label{mk-def:coherent_sheaf_flat}
...
\begin{remark}
\label{mk-rem:coh_flat_pullback}
  \dcref{custom}
...
\label{mk-sec:generic_freeness_devissage}
...
\begin{definition}[Generically free module]\label{mk-def:generically_free}
...
\begin{lemma}[Transport of generic freeness along further localisation]\label{mk-lem:generically_free_transport}
...
\begin{lemma}[Generic freeness is invariant under isomorphism]\label{mk-lem:generically_free_of_equiv}
  \lean{AlgebraicGeometry.GenericFreeness.GenericallyFree.of_linearEquiv}
  \leanok
  \dcref{custom}

  \uses{mk-def:generically_free}
...
\begin{lemma}[Splicing: generic freeness is stable under extensions]\label{mk-lem:generically_free_splice}
...
\begin{proof}
  \leanok
\uses{mk-lem:generically_free_transport}
  ...
  \ref{mk-lem:generically_free_transport}, \((M_1)_f\) and \((M_3)_f\) are free
  \(A_f\)-modules.  A short exact sequence with free (hence projective)
  quotient splits, so \((M_2)_f \cong (M_1)_f \oplus (M_3)_f\) is free.
\end{proof}

\begin{lemma}[Generic freeness of the zero module]\label{mk-lem:generically_free_subsingleton}
...
\begin{lemma}[Torsion base case]\label{mk-lem:generically_free_torsion}
...
\begin{lemma}[Generic freeness: finite modules]\label{mk-lem:generically_free_finite}
...
\begin{lemma}[Generic flatness: finite modules]\label{mk-lem:generically_flat_finite}
...
\begin{proof}
  \leanok
\uses{mk-lem:generically_free_finite}
  Immediate from \ref{mk-lem:generically_free_finite}: free modules are flat.
\end{proof}

\begin{lemma}[Generic freeness: the module-finite case]\label{mk-lem:generically_free_module_finite}
  ...
  (This is the case of \ref{mk-thm:generic_flatness_algebraic} where the
...
\uses{mk-lem:generically_free_finite}
  ...
  \(A\)-module, so \ref{mk-lem:generically_free_finite} applies.
\end{proof}

\begin{lemma}[Generic-rank comparison for a finite module over a domain]\label{mk-lem:generic_rank_comparison}
...
\begin{lemma}[Normalisation core: generic freeness of quotient domains]\label{mk-lem:generically_free_domain_core}
...
\begin{proof}
  \leanok
\uses{mk-lem:generically_free_torsion, mk-lem:generically_free_transport,
    mk-lem:generically_free_splice, mk-lem:generic_rank_comparison}
  ...
  \ref{mk-lem:generically_free_torsion} concludes.  So assume \(A \subseteq C\).
  ...
  \ref{mk-lem:generic_rank_comparison}.  A
  ...
  ring and filtering as in \ref{mk-thm:generic_flatness_algebraic}'s dévissage,
  the inductive hypothesis (over the noetherian domain \(A_g\)) together with
  \ref{mk-lem:generically_free_splice} yields \(h \ne 0\) in \(A_g\), which we
  ...
  (\ref{mk-lem:generically_free_transport} normalising the witnesses).
\end{proof}

\begin{theorem}[Generic flatness, algebraic form]\label{mk-thm:generic_flatness_algebraic}
...
\begin{proof}
  \leanok
\uses{mk-lem:generically_free_subsingleton, mk-lem:generically_free_of_equiv,
    mk-lem:generically_free_splice, mk-lem:generically_free_domain_core}
  ...
  module (\ref{mk-lem:generically_free_subsingleton}), holds for each quotient
  domain \(B/\mathfrak{p}\) (\ref{mk-lem:generically_free_domain_core},
  transported along the isomorphism \(M_i/M_{i-1} \cong B/\mathfrak{p}_i\) by
  \ref{mk-lem:generically_free_of_equiv}), and is stable under extensions
  (the splicing lemma, \ref{mk-lem:generically_free_splice}, applied to
...
\begin{lemma}[Mixed-base stability of flatness under module localization]
  \label{mk-lem:flat_localizedModule_algebra}
...
\begin{lemma}[Base-side restriction compatibility of \(\mathrm{appLE}\)]
  \label{mk-lem:appLE_base_res_apply}
...
\begin{lemma}[Section flatness transports along equal opens]
  \label{mk-lem:flat_sections_congr}
...
\begin{lemma}[Chart-basic core of generic flatness]
  \label{mk-lem:flat_section_chartBasic}
...
\begin{proof}
  \leanok
  \uses{mk-lem:flat_localizedModule_algebra, mk-lem:appLE_res_apply,
    mk-lem:flat_sections_congr, mk-lem:qcoh_section_localization_basicOpen}
  ...
  (\ref{mk-lem:qcoh_section_localization_basicOpen}, applied to the affine
  \(W_j\)), the restriction
  ...
  section localization (\ref{mk-lem:qcoh_section_localization_basicOpen} on the
...
\begin{proof}
  \leanok
  \uses{mk-lem:flat_section_chartBasic, mk-lem:appLE_base_res_apply,
    mk-lem:qcoh_section_localization_basicOpen, mk-lem:generically_free_transport}
  ...
  (\ref{mk-lem:qcoh_section_localization_basicOpen}), flatness of
  ...
  \(f a'\) (\ref{mk-lem:generically_free_transport}), and
  \(D(c) = D(b) \subseteq p^{-1}(D(a)) = p^{-1}(D(a'))
  \subseteq p^{-1}(D(f a'))\) since \(D(a) \subseteq U \subseteq D(f)\).
  The chart-basic core (\ref{mk-lem:flat_section_chartBasic}, at the element
  ...
  \ref{mk-lem:appLE_base_res_apply}), obtaining flatness of
  \(\Gamma(\mathcal F, D(b))\) over \(\Gamma(S, U)\).
\end{proof}

\begin{theorem}[Generic flatness, geometric form]
  \label{mk-thm:generic_flatness}
  \lean{AlgebraicGeometry.genericFlatness}
  \leanok
  \dcref{custom}

  \uses{mk-thm:generic_flatness_algebraic}
...
\begin{proof}
  \leanok
  \uses{mk-thm:generic_flatness_algebraic, mk-lem:flat_section_pair,
    mk-lem:affine_finite_sections_nhds}
  ...
  (\ref{mk-lem:affine_finite_sections_nhds}); extract a finite subcover by
  ...
  generic freeness (\ref{mk-thm:generic_flatness_algebraic}) produces
  ...
  basic-open reduction, \ref{mk-lem:flat_section_pair}.
\end{proof}

\section{Existence of the flattening stratification}
\label{mk-sec:flatstrat_exists}

\begin{theorem}

[Existence of the flattening stratification]
  \label{mk-thm:flattening_stratification_exists}
  \dcref{052H,custom}
  \uses{mk-def:coherent_sheaf_flat, mk-lem:flat_locus_open, mk-lem:nonflat_locus_proper,
    mk-lem:noetherian_induction_strata, mk-thm:generic_flatness}
...
\begin{proof}
  ...
  By Lemma~\ref{mk-lem:nonflat_locus_proper}, Noetherian induction on \(S\) (whose
  inductive step invokes generic flatness, Theorem~\ref{mk-thm:generic_flatness},
  ...
  the sense of Definition~\ref{mk-def:coherent_sheaf_flat}.  On a flat family the
  ...
  \(N\) fixed, Lemma~\ref{mk-lem:noetherian_induction_strata} applies the \(n = 0\)
  stratification mechanism (Lemma~\ref{mk-lem:flat_locus_open}) successively to the
  ...
  (Definition~\ref{mk-def:coherent_sheaf_flat}) because the graded module
  ...
  local-freeness universal property of Lemma~\ref{mk-lem:flat_locus_open}.  This
  ...
  (Lemma~\ref{mk-lem:flat_locus_open}): the closure of \(|S_f|\) in \(|S|\) is
...
[Noetherian induction reduction to the flat case]
  \label{mk-lem:nonflat_locus_proper}
  \dcref{custom}
  \uses{mk-thm:generic_flatness}
...
\begin{proof}
  ...
  subscheme of \(S\)).  By \ref{mk-thm:generic_flatness} applied to
...
[Assembly of the strata by Noetherian induction]
  \label{mk-lem:noetherian_induction_strata}
  \dcref{custom}
  \uses{mk-lem:flat_locus_open, mk-lem:nonflat_locus_proper, mk-thm:flat_base_change_cohomology}
  ...
  \ref{mk-thm:flattening_stratification_exists}.  There exists an integer
  ...
  (\ref{mk-lem:flat_locus_open}) on the coherent sheaves \(E_i := \pi_* \F(N+i)\)
  ...
  \ref{mk-thm:flattening_stratification_exists} are obtained by further
...
\begin{proof}
  By \ref{mk-lem:nonflat_locus_proper}, the family of Hilbert polynomials
  ...
  Apply \ref{mk-lem:flat_locus_open} sequentially to the coherent sheaves
...
\begin{lemma}[Flat-locus stratification, special case \(n = 0\)]\label{mk-lem:flat_locus_stratification_lean}
  ...
  \ref{mk-def:coherent_sheaf_flat}.  The index \(e\) is merely an enumeration;
  unlike the canonical strata of \ref{mk-lem:flat_locus_open}, it is not
  required to equal \(\dim_{\kappa(s)} \F|_s\).
\end{lemma}

\begin{proof}
  \leanok
  \uses{mk-thm:flattening_stratification_exists}
  Apply the existence theorem \ref{mk-thm:flattening_stratification_exists} to
  ...
  \ref{mk-def:coherent_sheaf_flat} is insensitive to composing the base leg
...
\label{mk-subsec:flat_transfer_strata}
...
\begin{lemma}[Flatness under pushout base change]
  ...
  \label{mk-lem:flat_pushout_base_change}
...
\label{mk-lem:flat_section_pullback_piece}
  \lean{AlgebraicGeometry.flat_section_pullback_piece}
  \leanok
  \dcref{custom}

  \uses{mk-def:coherent_sheaf_flat, mk-lem:flat_pushout_base_change,
    mk-def:pullback_app_isoTensor_sigma}
  Let
  \[
    \begin{array}{ccc}
      Y & \xrightarrow{\;g\;} & X \\
...
\begin{proof}
  ...
  (\ref{mk-def:pullback_app_isoTensor_sigma}, Stacks 01HQ/01I8) identifies
  \(\Gamma(g^* \F, U_Y) \cong
  ...
  is now \ref{mk-lem:flat_pushout_base_change} applied to the section-ring
...
\label{mk-lem:flat_affine_local}
  \lean{AlgebraicGeometry.flat_section_of_affine_cover}
  \leanok
  \dcref{custom}

  \uses{mk-def:coherent_sheaf_flat, mk-lem:appLE_base_res_apply}
...
\begin{proof}
  ...
  \ref{mk-lem:flat_section_pair}, with the freeness input replaced by chart
  ...
  \(b\) (\ref{mk-lem:qcoh_section_localization_basicOpen}), so flatness of
  ...
  of the induced module structures is \ref{mk-lem:appLE_base_res_apply}).
\end{proof}

\begin{lemma}[Reduced subscheme of an irreducible closed subset is integral]

\label{mk-lem:vanishing_ideal_subscheme_integral}
...
\label{mk-lem:flat_stratum_component}
  \lean{AlgebraicGeometry.flat_stratum_of_irreducible}
  \leanok
  \dcref{custom}

  \uses{mk-def:coherent_sheaf_flat, mk-thm:generic_flatness,
    mk-lem:vanishing_ideal_subscheme_integral, mk-lem:flat_section_pullback_piece,
    mk-lem:flat_affine_local}
  ...
  \ref{mk-def:coherent_sheaf_flat}.
\end{lemma}

\begin{proof}
  \leanok
  \(T\) is integral (\ref{mk-lem:vanishing_ideal_subscheme_integral}) and
  ...
  \(\Gamma(\F, W_X)\), hence finite (\ref{mk-lem:flat_section_pullback_piece}),
  and this finiteness localizes to a basic open inside any given
  neighbourhood.  Generic flatness (\ref{mk-thm:generic_flatness}, in its
  ...
  applying \ref{mk-lem:flat_section_pullback_piece} to the cartesian
  ...
  (\ref{mk-lem:flat_affine_local}) all affine pairs of the stratum family
...
\begin{lemma}[Noetherian-induction reduction to the flat case]\label{mk-lem:flat_locus_reduction_lean}
  ...
  \ref{mk-def:coherent_sheaf_flat}.  The strata are obtained by successively
  removing non-empty flat open patches supplied by generic flatness
  (\ref{mk-thm:generic_flatness}).
...
\begin{proof}
  \leanok
  \uses{mk-lem:nonflat_locus_proper, mk-thm:generic_flatness,
    mk-lem:flat_stratum_component, mk-lem:vanishing_ideal_subscheme_integral}
  ...
  \ref{mk-lem:flat_stratum_component} applied to the reduced subscheme
...
\begin{lemma}[Assembly with injectively labelled strata]\label{mk-lem:flat_locus_assembly_lean}
  ...
  the sense of \ref{mk-def:coherent_sheaf_flat}.
\end{lemma}

\begin{proof}
  \leanok
  \uses{mk-lem:flat_locus_reduction_lean}
  Since the statement requires of \(P\) only injectivity, it follows from the
  reduction \ref{mk-lem:flat_locus_reduction_lean}: take its stratification and
  ...
  \ref{mk-lem:noetherian_induction_strata}.
\end{proof}

\section{The entry ideal of a matrix presentation}
\label{mk-sec:entry_ideal}

The rank strata of \ref{mk-lem:flat_locus_open} carry a canonical --- in
...
\begin{definition}[Matrix presentation]
  \label{mk-def:matrix_presentation}
...
\begin{lemma}[Finite presentation]
  \label{mk-lem:matrix_presentation_fp}
  \lean{Module.MatrixPresentation.finitePresentation}
  \leanok
  \dcref{custom}

  \uses{mk-def:matrix_presentation}
...
\begin{definition}[Base change of a presentation]
  \label{mk-def:matrix_presentation_base_change}
...
\begin{lemma}[Entry ideal under base change]
  \label{mk-lem:entry_ideal_base_change}
  \lean{Module.MatrixPresentation.entryIdeal_baseChange}
  \leanok
  \dcref{custom}

  \uses{mk-def:entry_ideal, mk-def:matrix_presentation_base_change}
...
\begin{lemma}[Vanishing relations give an isomorphism]
  \label{mk-lem:presentation_iso_of_vanish}
...
\begin{lemma}[Vanishing relations give a free module]
  \label{mk-lem:presentation_free_of_vanish}
  \lean{Module.MatrixPresentation.free_of_relMatrix_eq_zero}
  \leanok
  \dcref{custom}

  \uses{mk-lem:presentation_iso_of_vanish}
...
\begin{proof}
  ...
  \ref{mk-lem:presentation_iso_of_vanish}.
\end{proof}

\begin{lemma}[Vanishing relations give constant rank]
  \label{mk-lem:presentation_rank_of_vanish}
...
\begin{proof}
  \leanok
  By \ref{mk-lem:presentation_iso_of_vanish}, \(M \cong R^{\oplus e}\); a ring
...
\begin{theorem}[Flat everywhere-rank-\(e\) modules have vanishing relation
  matrix]
  \label{mk-thm:entry_ideal_flat_vanish}
...
\begin{proof}
  \leanok
  \uses{mk-lem:matrix_presentation_fp}
  By \ref{mk-lem:matrix_presentation_fp}, \(M\) is finitely presented, hence
...
\begin{proof}
  \leanok
  \uses{mk-def:matrix_presentation_base_change, mk-lem:entry_ideal_vanish,
    mk-lem:presentation_free_of_vanish, mk-lem:presentation_rank_of_vanish,
    mk-thm:entry_ideal_flat_vanish}
  Base change both presentations to \(A := R / I(\psi)\).  The entries of
  \(\psi \otimes A\) vanish by construction
  (\ref{mk-lem:entry_ideal_vanish}), so by
  \ref{mk-lem:presentation_free_of_vanish} and \ref{mk-lem:presentation_rank_of_vanish}
  the module \(A \otimes_R M\) is free (hence flat) of rank \(e\) at every
  prime of \(A\).  Applying \ref{mk-thm:entry_ideal_flat_vanish} to the
...
\begin{theorem}[Presentation-independence of the entry ideal]
  \label{mk-thm:entry_ideal_presentation_independent}
  ...
  subschemes of \ref{mk-lem:flat_locus_open} glue.)
\end{theorem}

\begin{proof}
  \leanok
  \uses{mk-lem:entry_ideal_le}
  Apply \ref{mk-lem:entry_ideal_le} in both directions.
\end{proof}

\begin{definition}[Fiber rank]
  \label{mk-def:fiber_rank}
...
\begin{theorem}[Point-set of the entry-ideal locus]
  \label{mk-thm:entry_ideal_prime_iff}
  \lean{Module.MatrixPresentation.entryIdeal_le_prime_iff}
  \leanok
  \source{nitsure-hilbert-quot}

  \uses{mk-def:entry_ideal, mk-def:fiber_rank}
  \dcref{custom}
...
\begin{proof}
  \leanok
  \uses{mk-def:matrix_presentation_base_change, mk-lem:entry_ideal_vanish,
    mk-lem:presentation_iso_of_vanish, mk-thm:entry_ideal_flat_vanish}
  ...
  (\ref{mk-lem:entry_ideal_vanish}), so the fiber is isomorphic to
  \(\kappa(\mathfrak p)^{\oplus e}\)
  (\ref{mk-lem:presentation_iso_of_vanish}) and has dimension \(e\).
...
\begin{proof}
  ...
  (\ref{mk-lem:matrix_presentation_fp} for \(M\), transported along the base
...
\label{mk-sec:flatstrat_rank_stratum}

The entry-ideal commutative algebra of \ref{mk-sec:entry_ideal} is local to an
...
\(n = 0\) construction (\ref{mk-lem:flat_locus_open}).  Throughout \(X\) is a
...
\begin{theorem}[Base-change invariance of the fibre rank]
  \label{mk-thm:fiber_rank_base_change}
  \lean{Ideal.fiberRank_baseChange}
  \leanok
  \dcref{custom}

  \uses{mk-def:fiber_rank}
...
\begin{corollary}[Fibre rank of a localized module]
  \label{mk-cor:fiber_rank_localized}
  \lean{Ideal.fiberRank_of_isLocalizedModule}
  \leanok
  \dcref{custom}

  \uses{mk-thm:fiber_rank_base_change, mk-lem:fiber_rank_congr}
...
\begin{proof}
  ...
  \ref{mk-lem:fiber_rank_congr} the fibre rank of \(N\) at \(\mathfrak q\)
  equals that of \(R' \otimes_R M\), which by \ref{mk-thm:fiber_rank_base_change}
...
\begin{definition}[Presentation chart]
  \label{mk-def:presentation_chart}
...
\begin{definition}[Restriction of a presentation to a basic open]
  \label{mk-def:presentation_basic_open}
  \lean{AlgebraicGeometry.Scheme.Modules.MatrixPresentationBasicOpen}
  \leanok
  \dcref{custom}

  \uses{mk-def:matrix_presentation_base_change, mk-def:presentation_chart}
  ...
  (\ref{mk-def:matrix_presentation_base_change}), transported along this
...
\begin{lemma}[Entry ideal of a restricted presentation]
  \label{mk-lem:entry_ideal_basic_open}
  \lean{AlgebraicGeometry.Scheme.Modules.entryIdeal_matrixPresentationBasicOpen}
  \leanok
  \dcref{custom}

  \uses{mk-def:presentation_basic_open, mk-lem:entry_ideal_base_change}
  With the notation of \ref{mk-def:presentation_basic_open}, the entry ideal of
...
\begin{proof}
  ...
  the image ideal by \ref{mk-lem:entry_ideal_base_change}.
\end{proof}

\begin{lemma}[Presentation charts pass to basic opens]
  \label{mk-lem:presentation_chart_basic_open}
  \lean{AlgebraicGeometry.Scheme.Modules.IsPresentationChart.basicOpen}
  \leanok
  \dcref{custom}

  \uses{mk-def:presentation_chart, mk-def:presentation_basic_open}
...
\begin{proof}
  ...
  \ref{mk-def:presentation_basic_open}.
\end{proof}

\begin{definition}[Chart fibre rank]
  \label{mk-def:chart_fiber_rank}
...
\begin{lemma}[Chart fibre rank under localization]
  \label{mk-lem:chart_fiber_rank_basic_open}
  \lean{AlgebraicGeometry.Scheme.Modules.chartFiberRank_basicOpen}
  \leanok
  \dcref{custom}

  \uses{mk-def:chart_fiber_rank, mk-cor:fiber_rank_localized}
...
\begin{proof}
  ...
  Apply \ref{mk-cor:fiber_rank_localized}.
\end{proof}

\begin{theorem}[Chart-independence of the fibre rank]
  \label{mk-thm:chart_fiber_rank_eq}
  \lean{AlgebraicGeometry.Scheme.Modules.chartFiberRank_eq}
  \leanok
  \dcref{custom}

  \uses{mk-def:chart_fiber_rank, mk-lem:chart_fiber_rank_basic_open}
...
\begin{proof}
  ...
  \ref{mk-lem:chart_fiber_rank_basic_open} the chart fibre rank computed on
  \(V\) and the one computed on \(W\) both equal the chart fibre rank computed
  on \(D\), hence agree.
\end{proof}

\begin{definition}[Point rank]
  \label{mk-def:point_rank}
  \lean{AlgebraicGeometry.Scheme.Modules.pointRank}
  \leanok
  \dcref{custom}

  \uses{mk-def:chart_fiber_rank, mk-thm:chart_fiber_rank_eq}
  ...
  \ref{mk-thm:chart_fiber_rank_eq}.
\end{definition}

\begin{lemma}[Point rank on a chart]
  \label{mk-lem:point_rank_eq_chart_fiber_rank}
  \lean{AlgebraicGeometry.Scheme.Modules.pointRank_eq_chartFiberRank}
  \leanok
  \dcref{custom}

  \uses{mk-def:point_rank, mk-thm:chart_fiber_rank_eq}
...
\begin{proof}
  \leanok
  Both are chart fibre ranks at \(x\); apply \ref{mk-thm:chart_fiber_rank_eq}.
\end{proof}

\subsection{The strata ideal}

\begin{lemma}[Locality of entry-ideal membership]
  \label{mk-lem:mem_entry_ideal_locally}
  \lean{AlgebraicGeometry.Scheme.Modules.mem_entryIdeal_of_locally}
  \leanok
  \dcref{custom}

  \uses{mk-def:entry_ideal, mk-lem:entry_ideal_basic_open}
  Let \(P = (\psi, \pi)\) be a matrix presentation of
  \(\Gamma(\mathcal G, V)\) by \(e\) generators over an affine open \(V\), and
...
\begin{proof}
  ...
  \ref{mk-lem:entry_ideal_basic_open}, membership of \(r|_{D(g)}\) in
...
\begin{lemma}[Chart-to-chart transfer of the entry-ideal condition]
  \label{mk-lem:entry_ideal_chart_transfer}
  \lean{AlgebraicGeometry.Scheme.Modules.exists_basicOpen_mem_entryIdeal_of_chart}
  \leanok
  \dcref{custom}

  \uses{mk-thm:entry_ideal_presentation_independent, mk-lem:entry_ideal_basic_open}
...
\begin{proof}
  ...
  \ref{mk-thm:entry_ideal_presentation_independent} their entry ideals coincide.
  By \ref{mk-lem:entry_ideal_basic_open} the restriction of \(r|_W\) to \(D\)
  lies in \(I(\psi_W|_{D(f)}) = I(\psi_V|_{D(g)})\).
\end{proof}

\begin{definition}[Strata ideal]
  \label{mk-def:strata_ideal}
  \lean{AlgebraicGeometry.Scheme.Modules.strataIdeal}
  \leanok
  \source{nitsure-hilbert-quot}

  \uses{mk-def:entry_ideal, mk-def:presentation_chart}
  \dcref{custom}
...
\begin{theorem}[Strata ideal on a chart is the entry ideal]
  \label{mk-thm:strata_ideal_eq_entry_ideal}
  \lean{AlgebraicGeometry.Scheme.Modules.strataIdeal_eq_entryIdeal}
  \leanok
  \dcref{custom}

  \uses{mk-def:strata_ideal, mk-lem:mem_entry_ideal_locally, mk-lem:entry_ideal_chart_transfer}
...
\begin{proof}
  ...
  \(W \subseteq V\).  By \ref{mk-lem:mem_entry_ideal_locally} it suffices to
  show that each point of \(W\) has a basic open on which \(r|_W\) lands in the
  entry ideal of the restricted \(Q\); this is
  \ref{mk-lem:entry_ideal_chart_transfer} applied to the two charts \(W\) and
...
\begin{lemma}[Strata membership from a covering family of charts]
  \label{mk-lem:mem_strata_ideal_of_charts}
...
\begin{proof}
  ...
  \ref{mk-lem:mem_entry_ideal_locally} it suffices to check local membership at
  each \(x \in W\); choosing an index \(i\) with \(x \in V_i\),
  \ref{mk-lem:entry_ideal_chart_transfer} applied to \(W\) and \(V_i\) (using
...
\begin{definition}[Charts-cover hypothesis]
  \label{mk-def:charts_cover}
  \lean{AlgebraicGeometry.Scheme.Modules.ChartsCover}
  \leanok
  \dcref{custom}

  \uses{mk-def:presentation_chart}
...
\begin{lemma}[Finite chart covers]
  \label{mk-lem:charts_cover_finite}
  \lean{AlgebraicGeometry.Scheme.Modules.ChartsCover.exists_finite_charts}
  \leanok
  \dcref{custom}

  \uses{mk-def:charts_cover, mk-lem:presentation_chart_basic_open}
...
\begin{proof}
  ...
  (\ref{mk-lem:presentation_chart_basic_open}).  These charts cover \(U\), and
  \(U\) is quasi-compact, being affine, so finitely many of them suffice.
\end{proof}

\begin{theorem}[The strata ideal is quasi-coherent]
  \label{mk-thm:map_strata_ideal_basic_open}
  \lean{AlgebraicGeometry.Scheme.Modules.map_strataIdeal_basicOpen}
  \leanok
  \source{nitsure-hilbert-quot}

  \uses{mk-def:strata_ideal, mk-lem:charts_cover_finite, mk-lem:mem_strata_ideal_of_charts,
    mk-lem:entry_ideal_basic_open, mk-thm:strata_ideal_eq_entry_ideal}
  \dcref{custom}
...
\begin{proof}
  ...
  \ref{mk-lem:charts_cover_finite} choose finitely many presentation charts
  ...
  (\ref{mk-lem:entry_ideal_basic_open}); clearing the denominator yields an
  ...
  \(f^{m} y \in I_e(U)\) by \ref{mk-lem:mem_strata_ideal_of_charts}.  Therefore
  \(r' = (f^{m} y) / f^{m + n}\) lies in the image of \(I_e(U)\) under
  restriction.  (\ref{mk-thm:strata_ideal_eq_entry_ideal} identifies the strata
...
\begin{theorem}[Support of the rank-\(e\) stratum]
  \label{mk-thm:mem_range_stratum}
  \lean{AlgebraicGeometry.Scheme.Modules.mem_range_stratumι_iff}
  \leanok
  \source{nitsure-hilbert-quot}

  \uses{mk-def:stratum_immersion, mk-thm:strata_ideal_eq_entry_ideal, mk-thm:entry_ideal_prime_iff,
    mk-def:point_rank, mk-lem:point_rank_eq_chart_fiber_rank}
  \dcref{custom}
...
\begin{proof}
  ...
  \ref{mk-thm:strata_ideal_eq_entry_ideal}.  A point of an affine open lies in
  ...
  \ref{mk-thm:entry_ideal_prime_iff} this holds iff the fibre rank of
  \(\Gamma(\mathcal G, V)\) at \(\mathfrak p_x\) --- the chart fibre rank at
  \(x\), equal to the point rank by
  \ref{mk-lem:point_rank_eq_chart_fiber_rank} --- equals \(e\).
\end{proof}

\section{Representability of the rank strata}
\label{mk-sec:flatstrat_representability}

The rank-\(e\) stratum of \ref{mk-sec:flatstrat_rank_stratum} is a closed
...
\begin{lemma}[Finitely presented modules admit matrix presentations]
  \label{mk-lem:exists_matrix_presentation}
...
\begin{lemma}[Pullback of a presentation to section modules]
  \label{mk-lem:presentation_pullback_sections}
...
\begin{proof}
  \leanok
  \uses{mk-def:pullback_app_isoTensor_sigma, mk-def:matrix_presentation_base_change}
  ...
  as \(\Gamma(Y, W)\)-modules (\ref{mk-def:pullback_app_isoTensor_sigma}).
  Base change of \(P\) along \(g^\sharp\)
  (\ref{mk-def:matrix_presentation_base_change}) is a matrix presentation of
...
\begin{lemma}[Flat sections from flatness over the identity]
  \label{mk-lem:flat_sections_of_sheaf_flat}
...
\begin{lemma}[Flatness over the identity from an affine cover]
  \label{mk-lem:sheaf_flat_of_flat_charts}
...
\begin{proof}
  \leanok
  \uses{mk-lem:flat_affine_local}
  Apply the affine-locality of flatness over the base
  (\ref{mk-lem:flat_affine_local}) to the identity morphism of \(T\) with the
...
\begin{lemma}[Flatness over the base is stable under pullback]
  \label{mk-lem:sheaf_flat_pullback}
  \lean{AlgebraicGeometry.Scheme.Modules.coherentSheafFlat_id_pullback}
  \leanok
  \dcref{custom}

  \uses{mk-def:coherent_sheaf_flat, mk-lem:pullback_isQuasicoherent_hom}
...
\begin{proof}
  \leanok
  \uses{mk-lem:flat_sections_of_sheaf_flat, mk-def:pullback_app_isoTensor_sigma,
    mk-lem:sheaf_flat_of_flat_charts}
  ...
  (\ref{mk-def:pullback_app_isoTensor_sigma}) is a base change of the flat
  module \(\Gamma(\mathcal G, V)\) (\ref{mk-lem:flat_sections_of_sheaf_flat}),
  hence flat over \(\Gamma(T', W)\).  The pullback \(g^*\mathcal G\) is
  quasi-coherent, so \ref{mk-lem:sheaf_flat_of_flat_charts} upgrades the cover
...
\begin{proof}
  \leanok
  \uses{mk-def:fiber_rank, mk-def:chart_fiber_rank,
    mk-lem:point_rank_eq_chart_fiber_rank}
  ...
  rank of \(M\) at \(\mathfrak p\) (\ref{mk-def:fiber_rank}).  The prime of
  \(R\) attached to the point \(x\) is \(\mathfrak p\) itself, so this fiber
  rank is the chart fibre rank of \(\mathcal G\) at \(x\) computed on \(W\)
  (\ref{mk-def:chart_fiber_rank}), which equals the point rank by
  \ref{mk-lem:point_rank_eq_chart_fiber_rank}.
\end{proof}

\begin{theorem}[Point rank of a pullback]
  \label{mk-thm:point_rank_pullback}
  \lean{AlgebraicGeometry.Scheme.Modules.pointRank_pullback}
  \leanok
  \dcref{custom}

  \uses{mk-def:point_rank, mk-lem:pullback_isQuasicoherent_hom}
...
\begin{proof}
  \leanok
  \uses{mk-def:pullback_app_isoTensor_sigma, mk-thm:fiber_rank_base_change,
    mk-lem:fiber_rank_congr, mk-lem:point_rank_eq_chart_fiber_rank}
  Choose an affine open \(V \ni g(y)\) and an affine open \(W \ni y\) with
  \(W \subseteq g^{-1}(V)\).  By the base-change formula for sections
  (\ref{mk-def:pullback_app_isoTensor_sigma}),
  \(\Gamma(g^*\mathcal G, W) \cong
  ...
  \(g(y)\).  By \ref{mk-lem:fiber_rank_congr} and base-change invariance of
  the fiber rank (\ref{mk-thm:fiber_rank_base_change}), the fiber rank of
...
\begin{theorem}[The strata ideal dies under a flat constant-rank pullback]
  \label{mk-thm:strata_ideal_le_ker}
  \lean{AlgebraicGeometry.Scheme.Modules.strataData_le_ker}
  \leanok
  \source{nitsure-hilbert-quot}

  \uses{mk-def:strata_data, mk-def:charts_cover, mk-def:coherent_sheaf_flat,
    mk-def:point_rank, mk-lem:pullback_isQuasicoherent_hom}
  \dcref{custom}
...
\begin{proof}
  \leanok
  \uses{mk-lem:charts_cover_finite, mk-lem:presentation_pullback_sections,
    mk-lem:flat_sections_of_sheaf_flat, mk-thm:stalk_rank_eq_point_rank,
    mk-thm:entry_ideal_flat_vanish, mk-lem:entry_ideal_vanish, mk-def:strata_ideal,
    mk-lem:appLE_base_res_apply}
  ...
  (\ref{mk-lem:charts_cover_finite}).  Given \(t\), pick \(i\) with
  \(q(t) \in V_i\), and an affine open \(W \ni t\) with
  \(W \subseteq q^{-1}(V_i)\).

  By \ref{mk-lem:presentation_pullback_sections} the module
  ...
  \ref{mk-lem:flat_sections_of_sheaf_flat}.  At every prime of
  \(\Gamma(T, W)\) its localization is free of rank equal to the point rank
  of \(q^*\mathcal G\) at the corresponding point
  (\ref{mk-thm:stalk_rank_eq_point_rank}), which is \(e\) by hypothesis.  By
  \ref{mk-thm:entry_ideal_flat_vanish} the relation matrix vanishes:
  \(q^\sharp(\psi_i) = 0\).  Hence the entry ideal \(I(\psi_i)\) is
  contained in the kernel of
  \(q^\sharp : \Gamma(X, V_i) \to \Gamma(T, W)\)
  (\ref{mk-lem:entry_ideal_vanish}).  The strata-ideal condition
  (\ref{mk-def:strata_ideal}) gives \(r|_{V_i} \in I(\psi_i)\), and the maps
  induced by \(q\) on sections commute with restriction
  (\ref{mk-lem:appLE_base_res_apply}), so
  \(q^\sharp(r)|_W = q^\sharp(r|_{V_i}) = 0\), as required.
\end{proof}

\begin{theorem}[Unique factorization through the rank-\(e\) stratum]
  \label{mk-thm:stratum_lift_unique}
  \lean{AlgebraicGeometry.Scheme.Modules.existsUnique_stratumLift}
  \leanok
  \source{nitsure-hilbert-quot}

  \uses{mk-def:stratum, mk-def:stratum_immersion, mk-def:charts_cover,
    mk-def:coherent_sheaf_flat, mk-def:point_rank, mk-lem:pullback_isQuasicoherent_hom}
  \dcref{custom}
  Let \(\mathcal G\) be a quasi-coherent \(\OO_X\)-module admitting
  \(e\)-presentation charts, and let \(q : T \to X\) be a morphism such that
  \(q^*\mathcal G\) is flat over \(T\) and has point rank \(e\) at every
...
\begin{proof}
  \leanok
  \uses{mk-thm:strata_ideal_le_ker, mk-def:strata_data}
  The stratum \(Z_e\) is the closed subscheme cut out by the strata
  ideal-sheaf datum (\ref{mk-def:strata_data}), and the kernel of its
  ...
  monomorphism.  By \ref{mk-thm:strata_ideal_le_ker} the strata ideal sheaf is
  contained in the kernel of \(q\).
\end{proof}

\begin{theorem}[The rank-\(e\) stratum flattens \(\mathcal G\)]
  \label{mk-thm:stratum_flattens}
  \lean{AlgebraicGeometry.Scheme.Modules.coherentSheafFlat_stratum}
  \leanok
  \source{nitsure-hilbert-quot}

  \uses{mk-def:stratum, mk-def:stratum_immersion, mk-def:charts_cover,
    mk-def:coherent_sheaf_flat, mk-lem:pullback_isQuasicoherent_hom}
  \dcref{custom}
  Let \(\mathcal G\) be a quasi-coherent \(\OO_X\)-module admitting
...
\begin{proof}
  \leanok
  \uses{mk-lem:presentation_pullback_sections, mk-thm:strata_ideal_eq_entry_ideal,
    mk-def:entry_ideal, mk-lem:presentation_free_of_vanish,
    mk-lem:sheaf_flat_of_flat_charts, mk-def:strata_data}
  ...
  \ref{mk-lem:presentation_pullback_sections} the section module
  \(\Gamma(\iota^*\mathcal G, \iota^{-1}(V))\) has a matrix presentation
  with relation matrix \(\iota^\sharp(\psi)\).  Every entry of \(\psi\) lies
  in the entry ideal \(I(\psi)\) (\ref{mk-def:entry_ideal}), which equals the
  strata ideal \(I_e(V)\) (\ref{mk-thm:strata_ideal_eq_entry_ideal}); and the
  ...
  (\ref{mk-lem:presentation_free_of_vanish}), in particular flat, over
  \(\Gamma(Z_e, \iota^{-1}(V))\).  Conclude by
  \ref{mk-lem:sheaf_flat_of_flat_charts}.
\end{proof}

\subsection{The chart locus}

\begin{theorem}[Chart supply on a locally noetherian scheme]
  \label{mk-thm:presentation_chart_exists}
  \lean{AlgebraicGeometry.Scheme.Modules.exists_presentationChart_mem}
  \leanok
  \source{nitsure-hilbert-quot}

  \uses{mk-def:presentation_chart, mk-def:point_rank}
  \dcref{custom}
...
\begin{proof}
  \leanok
  \uses{mk-lem:affine_finite_sections_nhds, mk-thm:nakayama_prolongation,
    mk-lem:point_rank_eq_chart_fiber_rank}
  Choose an affine open \(V \ni s\) on which \(M := \Gamma(\F, V)\) is a
  finite module over \(R := \Gamma(S, V)\)
  (\ref{mk-lem:affine_finite_sections_nhds}).  Since \(S\) is locally
  ...
  \ref{mk-lem:point_rank_eq_chart_fiber_rank}, \(e\) is the point rank of
  \(\F\) at \(s\).  The Nakayama prolongation
  (\ref{mk-thm:nakayama_prolongation}) supplies \(g \notin \mathfrak p\) such
...
\begin{lemma}[The chart locus contains the rank-\(e\) locus]
  \label{mk-lem:chart_locus_contains_rank_locus}
  \lean{AlgebraicGeometry.Scheme.Modules.mem_chartLocus_of_pointRank_eq}
  \leanok
  \dcref{custom}

  \uses{mk-def:chart_locus, mk-def:point_rank}
...
\begin{proof}
  \leanok
  \uses{mk-thm:presentation_chart_exists}
  \ref{mk-thm:presentation_chart_exists} puts such a point inside an
  \(e\)-presentation chart.
\end{proof}

\begin{lemma}[Rank bound on the chart locus]
  \label{mk-lem:chart_locus_rank_bound}
...
\begin{proof}
  \leanok
  \uses{mk-def:presentation_chart, mk-def:fiber_rank,
    mk-lem:point_rank_eq_chart_fiber_rank}
  ...
  (\ref{mk-def:fiber_rank}) --- the point rank of \(\F\) at \(s\), by
  \ref{mk-lem:point_rank_eq_chart_fiber_rank} --- is at most \(e\).
\end{proof}

\begin{lemma}[The restricted module is chart-covered]
  \label{mk-lem:charts_cover_chart_locus}
  \lean{AlgebraicGeometry.Scheme.Modules.chartsCover_chartLocus}
  \leanok
  \dcref{custom}

  \uses{mk-def:chart_locus, mk-def:charts_cover, mk-lem:pullback_isQuasicoherent_hom}
...
\begin{proof}
  \leanok
  \uses{mk-lem:presentation_pullback_sections, mk-def:presentation_chart}
  ...
  \ref{mk-lem:presentation_pullback_sections} the section module of the
...
\begin{proof}
  \leanok
  \uses{mk-thm:mem_range_stratum, mk-thm:point_rank_pullback,
    mk-lem:chart_locus_contains_rank_locus}
  Write \(\F' := \F|_{W_e}\) for the restriction to the chart locus; by
  \ref{mk-thm:point_rank_pullback} (applied to the open immersion) the point
  ...
  \(\F'\) there is \(e\) (\ref{mk-thm:mem_range_stratum}); hence the point
  rank of \(\F\) at \(s\) is \(e\).  Conversely, if the point rank of
  \(\F\) at \(s\) is \(e\), then \(s \in W_e\)
  (\ref{mk-lem:chart_locus_contains_rank_locus}); the point rank of \(\F'\)
  at \(s\) is \(e\), so \(s\) lies in the support of the stratum
  (\ref{mk-thm:mem_range_stratum}), i.e.\ in the image of \(\iota_e\).
\end{proof}

\begin{theorem}[The rank-\(e\) stratum flattens \(\F\)]
  \label{mk-thm:rank_stratum_flattens}
  \lean{AlgebraicGeometry.Scheme.Modules.coherentSheafFlat_rankStratum}
  \leanok
  \source{nitsure-hilbert-quot}

  \uses{mk-def:rank_stratum, mk-def:rank_stratum_immersion,
    mk-def:coherent_sheaf_flat}
  \dcref{custom}
...
\begin{proof}
  \leanok
  \uses{mk-thm:stratum_flattens, mk-lem:charts_cover_chart_locus}
  ...
  (\ref{mk-lem:charts_cover_chart_locus}), so \(\iota^*(\F|_{W_e})\) is flat
  over \(S_e\) by \ref{mk-thm:stratum_flattens}; flatness is invariant under
...
\begin{lemma}[The point rank is bounded on a noetherian scheme]
  \label{mk-lem:point_rank_bounded}
...
\begin{proof}
  \leanok
  \uses{mk-thm:presentation_chart_exists, mk-def:chart_locus,
    mk-lem:chart_locus_rank_bound}
  For each \(s \in S\) choose an \(e(s)\)-presentation chart \(V_s \ni s\)
  with \(e(s)\) the point rank at \(s\)
  (\ref{mk-thm:presentation_chart_exists}).  The noetherian space \(S\) is
  ...
  most \(e(s_i) \le N\) (\ref{mk-lem:chart_locus_rank_bound}).
\end{proof}

\begin{theorem}[Openness of the rank fibers of a flat pullback]
  \label{mk-thm:rank_fibers_open}
  \lean{AlgebraicGeometry.Scheme.Modules.isOpen_pointRank_pullback_eq}
  \leanok
  \dcref{custom}

  \uses{mk-def:point_rank, mk-def:coherent_sheaf_flat,
...
\begin{proof}
  \leanok
  \uses{mk-lem:affine_finite_sections_nhds, mk-lem:exists_matrix_presentation,
    mk-lem:presentation_pullback_sections, mk-lem:matrix_presentation_fp,
    mk-lem:flat_sections_of_sheaf_flat, mk-thm:stalk_rank_eq_point_rank}
  ...
  (\ref{mk-lem:affine_finite_sections_nhds}), hence finitely presented, and
  an affine open \(W \ni t_0\) with \(W \subseteq \varphi^{-1}(V)\).  Set
  \(M := \Gamma(\varphi^*\F, W)\) and \(R := \Gamma(T, W)\).  Then \(M\) is
  flat over \(R\) (\ref{mk-lem:flat_sections_of_sheaf_flat}) and finitely
  presented: a matrix presentation of \(\Gamma(\F, V)\)
  (\ref{mk-lem:exists_matrix_presentation}) pulls back to a matrix
  presentation of \(M\) (\ref{mk-lem:presentation_pullback_sections}), and a
  module with a matrix presentation is finitely presented
  (\ref{mk-lem:matrix_presentation_fp}).
  ...
  rank of \(\varphi^*\F\) (\ref{mk-thm:stalk_rank_eq_point_rank}).  Hence the
...
\begin{theorem}[Unique factorization through the rank-\(e\) stratum of the
  base]
  \label{mk-thm:factor_rank_stratum_unique}
  \lean{AlgebraicGeometry.Scheme.Modules.existsUnique_factor_rankStratum}
  \leanok
  \source{nitsure-hilbert-quot}

  \uses{mk-def:rank_stratum, mk-def:rank_stratum_immersion,
    mk-def:coherent_sheaf_flat, mk-def:point_rank}
  \dcref{custom}
  Let \(S\) be locally noetherian, \(\F\) a finitely presented
...
\begin{proof}
  \leanok
  \uses{mk-lem:chart_locus_contains_rank_locus, mk-thm:point_rank_pullback,
    mk-lem:charts_cover_chart_locus, mk-thm:stratum_lift_unique}
  Every \(\varphi(t)\) has point rank \(e\), hence lies in the chart locus
  \(W_e\) (\ref{mk-lem:chart_locus_contains_rank_locus}), so \(\varphi\)
  ...
  (\ref{mk-lem:charts_cover_chart_locus}).  By functoriality of pullback,
  \(q^*\F' \cong \varphi^*\F\) is flat over \(T\), and by
  \ref{mk-thm:point_rank_pullback} (applied twice, to \(q\) and to the open
  immersion) its point rank at \(t\) equals the point rank of \(\F\) at
  \(\varphi(t)\), namely \(e\).  Now \ref{mk-thm:stratum_lift_unique} gives a
  ...
  \ref{mk-thm:stratum_lift_unique}, \(l' = l\).
\end{proof}

\section{Universal property of the strata}
\label{mk-sec:flatstrat_universal}

\begin{theorem}[Universal property of the flat-locus stratification (case
  \(n = 0\))]
  \label{mk-thm:flattening_stratification_universal}
  ...
  \ref{mk-def:coherent_sheaf_flat}), and such that, writing
  ...
  flatness under base change (\ref{mk-lem:sheaf_flat_pullback}).  For a relative
  projective family, the analogous universal property additionally uses the
  cohomology-and-base-change layer of
  \ref{mk-lem:noetherian_induction_strata}.  Reduced strata alone do not give
...
\begin{proof}
  \leanok
  \uses{mk-lem:point_rank_bounded, mk-def:rank_stratum, mk-def:rank_stratum_immersion,
    mk-thm:mem_range_rank_stratum, mk-thm:rank_stratum_flattens,
    mk-lem:sheaf_flat_pullback, mk-thm:rank_fibers_open, mk-thm:point_rank_pullback,
    mk-thm:factor_rank_stratum_unique, mk-def:point_rank}
  \textbf{The strata.}  By \ref{mk-lem:point_rank_bounded} there is an \(N\)
  bounding the point rank of \(\F\) on \(S\).  Take
  \(I := \{0, 1, \dots, N\}\) and, for \(e \in I\), the rank-\(e\) stratum
  \(\iota_e : S_e \to S\)
  (\ref{mk-def:rank_stratum} and \ref{mk-def:rank_stratum_immersion}); each \(\iota_e\) is
  an immersion.  By \ref{mk-thm:mem_range_rank_stratum} the image of
  ...
  \ref{mk-thm:rank_stratum_flattens}.
  ...
  \ref{mk-thm:point_rank_pullback} the point rank of \(\varphi^*\F\) at \(t\)
  is the point rank of \(\F\) at \(\varphi(t)\), hence at most \(N\); so
  the \(T_e\) cover \(T\), they are pairwise disjoint, and each is open by
  \ref{mk-thm:rank_fibers_open}.  Thus \(T\) is the disjoint union of the
  ...
  \(T_e\), flat by \ref{mk-lem:sheaf_flat_pullback} --- and, by
  \ref{mk-thm:point_rank_pullback} again, the point rank of \(\F\) at the
  image of any \(t \in T_e\) is \(e\).  By
  \ref{mk-thm:factor_rank_stratum_unique} there is a unique
  ...
  (\ref{mk-thm:mem_range_rank_stratum}); it is also \(e\), since
  \(t \in T_e\) (\ref{mk-thm:point_rank_pullback}).  Hence \(e' = e\): the
...
[Flattening stratification for a coherent sheaf on a relative curve]
  \label{mk-cor:flattening_stratification_curve}
  \dcref{custom}
  \uses{mk-lem:smooth_proper_curve_projective, mk-thm:flattening_stratification_exists}
  ...
  \ref{mk-thm:flattening_stratification_exists} applies: there is a finite
...
\begin{proof}
  By \ref{mk-lem:smooth_proper_curve_projective}, \(C \times_k T \to T\) is
  ...
  \ref{mk-thm:flattening_stratification_exists} to \(\widetilde{\F}\) on
  \(\PP^n_T\) to obtain \(\{T_f\}\) with the stated universal property.
\end{proof}

\section{Mathematical ingredients}
\label{mk-sec:flatstrat_mathlib_status}
...
in the Noetherian induction of \ref{mk-lem:flat_locus_reduction_lean}.  Third,
entry ideals of finite presentations define the rank strata and their
universal property, as developed in \ref{mk-sec:entry_ideal}--
\ref{mk-sec:flatstrat_representability}.  Finally, for a projective family,
...
\ref{mk-thm:flattening_stratification_exists}.

...
structure morphism (\ref{def:overAlgebraMap}); all dimensions over \(K\) below are taken
...
Section~\ref{sec:localized_colength}.
...
\begin{proof}
  ...
  \ref{lem:primeIdealOf_ne_bot}, the open immersion \(\Spec B \to X\) matches generic
...
\begin{theorem}[Dedekind sections on every affine chart]
  \label{thm:isDedekindDomain_section}
  \dcref{custom}
...
\begin{proof}
  \leanok
  \uses{thm:overAlgebraMap_finiteType, thm:closedpoint_dvr_stalk,
    lem:fromSpec_base_mem, lem:primeIdealOf_fromSpec_base,
    lem:fromSpec_base_ne_genericPoint}
  \(B\) is a domain: \(V\) is a nonempty open of the integral scheme \(X\). \(B\) is
  Noetherian: the algebra map \(K \to B\) is of finite type
  (\ref{thm:overAlgebraMap_finiteType}), and a finite-type algebra over a field is
  ...
  \(x := x_{\mathfrak q}\) lies in \(V\) (\ref{lem:fromSpec_base_mem}) and is not the
  generic point (\ref{lem:fromSpec_base_ne_genericPoint}), so the stalk
  \(\struct{X,x}\) is a discrete valuation ring (\ref{thm:closedpoint_dvr_stalk}).
  ...
  (\ref{lem:primeIdealOf_fromSpec_base}); hence \(B_{\mathfrak q} \cong
...
\begin{proof}
  \leanok
  \uses{thm:closedpoint_isClosed, thm:overAlgebraMap_finiteType}
  The singleton \(\{x\}\) is closed in \(X\) (\ref{thm:closedpoint_isClosed}), so
  ...
  (\ref{thm:overAlgebraMap_finiteType} followed by a surjection), and a field which is
...
\begin{proof}
  \leanok
  \uses{thm:closedpoint_isClosed, def:residueOverAlgebraMap}
  As in \ref{thm:moduleFinite_quotient_primeIdealOf}, \(\mathfrak p_x\) is maximal
  (\ref{thm:closedpoint_isClosed}). The stalk \(\struct{X,x}\) is the localization of
  ...
  the residue map (\ref{def:residueOverAlgebraMap}), and the germ of the algebra map
...
\begin{proof}
  \leanok
  \uses{lem:primeIdealOf_ne_bot, thm:closedpoint_dvr_stalk, def:scheme_ord,
    lem:germ_generic_eq_algebraMap_germ, lem:adic_valuation_count,
    thm:count_factors_span_localization}
  The prime \(\mathfrak p_x\) is nonzero (\ref{lem:primeIdealOf_ne_bot}), and the
  stalk \(\struct{X,x}\) is the localization of \(B\) at \(\mathfrak p_x\) --- a
  discrete valuation ring (\ref{thm:closedpoint_dvr_stalk}) with maximal ideal
  ...
  (\ref{lem:germ_generic_eq_algebraMap_germ}), \(g\) --- the germ of \(f\) at
  \(\eta\) --- is the image of the germ of \(f\) at \(x\) under
  \(\struct{X,x} \to K(X)\).

  The order valuation \(\mathrm{ord}_x\) is the \(\mathfrak m_x\)-adic valuation of
  the discrete valuation ring \(\struct{X,x}\) (\ref{def:scheme_ord}), so its value on
  ...
  ideal generated by that germ (\ref{lem:adic_valuation_count}, applied in the
  Dedekind domain \(\struct{X,x}\)). Finally, multiplicities are preserved by
  localization at the prime (\ref{thm:count_factors_span_localization}, with
...
\begin{proof}
  \leanok
  \uses{thm:isDedekindDomain_section, lem:prime_factor_span, lem:fromSpec_base_mem,
    lem:primeIdealOf_fromSpec_base, lem:fromSpec_base_ne_genericPoint,
    thm:moduleFinite_quotient_primeIdealOf, thm:moduleFinite_quotient_span}
  \(B\) is a Dedekind domain (\ref{thm:isDedekindDomain_section}). Each prime factor
  \(P\) of \((f)\) is a nonzero prime (\ref{lem:prime_factor_span}); its point
  \(x_P\) lies in \(V\) (\ref{lem:fromSpec_base_mem}), is not the generic point
  (\ref{lem:fromSpec_base_ne_genericPoint}), and satisfies
  \(\mathfrak p_{x_P} = P\) (\ref{lem:primeIdealOf_fromSpec_base}); so the residue
  \(B/P\) is a finite \(K\)-module by
  \ref{thm:moduleFinite_quotient_primeIdealOf}. The finiteness of the Dedekind
  colength module (\ref{thm:moduleFinite_quotient_span}) concludes.
\end{proof}
...
\begin{theorem}[The chart colength dictionary]
  ...
  \uses{def:scheme_ordZ, def:residueDeg}
  \leanok
  Let \(X\) be an integral scheme, smooth of relative dimension one and locally of
...
\begin{proof}
  \leanok
  \uses{thm:isDedekindDomain_section, thm:finrank_quotient_span_eq_sum_count,
    thm:toAdd_ordZ_eq_count_factors, thm:finrank_quotient_primeIdealOf,
    thm:moduleFinite_quotient_primeIdealOf, lem:prime_factor_span,
    lem:primeIdealOf_ne_bot, lem:fromSpec_base_mem, lem:primeIdealOf_fromSpec_base,
    lem:fromSpec_base_ne_genericPoint, lem:ord_eq_one_of_mem_basicOpen,
    lem:mem_pow_iff_le_count}
  \(B\) is a Dedekind domain (\ref{thm:isDedekindDomain_section}), and \(f \neq 0\):
  its germ at \(\eta\) is the value of the unit \(g\). As in
  \ref{thm:moduleFinite_quotient_span_section}, every prime factor of \((f)\) has
  finite residue over \(K\), so the localized Dedekind colength formula
  (\ref{thm:finrank_quotient_span_eq_sum_count}) gives
  \[
    \dim_K\bigl(B/(f)\bigr) \;=\;
  ...
  (\ref{lem:ord_eq_one_of_mem_basicOpen}) and the term vanishes. Otherwise
  \(f \in \mathfrak p_x\); then \(\mathfrak p_x\) is a prime factor of \((f)\): it is
  nonzero (\ref{lem:primeIdealOf_ne_bot}), and membership \(f \in \mathfrak p_x =
  \mathfrak p_x^1\) forces multiplicity \(\ge 1\)
  (\ref{lem:mem_pow_iff_le_count}).
  ...
  (\ref{lem:primeIdealOf_fromSpec_base}). It is surjective onto the prime factors:
  a prime factor \(P\) of \((f)\) contains \(f\) (\ref{lem:prime_factor_span}), its
  point \(x_P\) lies in \(V\) (\ref{lem:fromSpec_base_mem}), is non-generic
  (\ref{lem:fromSpec_base_ne_genericPoint}) and satisfies \(\mathfrak p_{x_P} = P\)
  (\ref{lem:primeIdealOf_fromSpec_base}); and \(x_P \in S\), for otherwise the
  ...
  (\ref{thm:toAdd_ordZ_eq_count_factors}) and the residue leg gives
  \(\dim_K(B/\mathfrak p_x) = \deg_K(x)\)
  (\ref{thm:finrank_quotient_primeIdealOf}). Hence the sum over the prime factors of
...
(\ref{thm:curveDivisor_picClass_eq_iff}). This section records the degree homomorphism
...
\begin{lemma}[Linearly equivalent divisors have equal degree]
  ...
  \uses{def:curveDivisor_deg, thm:curveDivisor_picClass_eq_iff, thm:deg_divOf,
    thm:curveDivisor_deg_hom}
  \leanok
...
\begin{proof}
  \leanok
  Equal classes differ by a principal divisor (\ref{thm:curveDivisor_picClass_eq_iff}): there is
  \(g \in K(X)^\times\) with \(D - D' = \mathrm{div}\, g\). Principal divisors have degree zero
  (Theorem~\ref{thm:deg_divOf}), so \(\deg_K(D - D') = 0\); and \(\deg_K(D - D') = \deg_K D -
  \deg_K D'\) by the degree homomorphism (Theorem~\ref{thm:curveDivisor_deg_hom}). Hence
  \(\deg_K D = \deg_K D'\).
\end{proof}
...
\begin{theorem}[The Euler characteristic of a class is well-defined]
  ...
  \uses{def:divisorSheaf, def:h0_h1_chi, thm:curveDivisor_picClass_eq_iff,
    thm:mulEquivDivisorSheaf, thm:chi_congr}
  \leanok
  If two Weil divisors have the same \v Cech Picard class, \(\struct X(D) = \struct X(D')\), then
...
\begin{proof}
  \leanok
  Equal classes differ by a principal divisor (\ref{thm:curveDivisor_picClass_eq_iff}): \(D -
  ...
  (Theorem~\ref{thm:mulEquivDivisorSheaf}), and isomorphic sheaves have equal ledger invariants
  (Theorem~\ref{thm:chi_congr}), so \(\chi(\mathcal O(D)) = \chi(\mathcal O(D'))\). The argument
...
\begin{definition}[The degree homomorphism on the Picard group]
  ...
  \uses{lem:deg_eq_deg_of_picClass_eq, def:curveDivisor_picClass, def:curveDivisor_deg}
  \leanok
...
\begin{proof}
  ...
  (\ref{def:curveDivisor_picClass}); set \(\deg_{\Pic} L = \deg_K D\). This is independent of the
  chosen realization: if \(\struct X(D) = \struct X(D')\) then \(\deg_K D = \deg_K D'\)
  (Lemma~\ref{lem:deg_eq_deg_of_picClass_eq}). Applying this to a divisor's own class gives the
...
\begin{theorem}[Additivity of the class degree]
  ...
  \uses{def:classDeg, def:curveDivisor_picClass, thm:curveDivisor_picClass_hom,
    thm:curveDivisor_deg_hom}
  \leanok
...
\begin{proof}
  ...
  (\ref{def:curveDivisor_picClass}). Their product is \(L \cdot L' = \struct X(D + D')\) by
  additivity of the class map (Theorem~\ref{thm:curveDivisor_picClass_hom}), so by the
  normalization (\ref{def:classDeg}) and additivity of the divisor degree
  (Theorem~\ref{thm:curveDivisor_deg_hom}),
  \[
...
\begin{theorem}[The \(\chi\)-ledger in class form]
  ...
  \uses{def:classDeg, thm:chi_divisorSheaf}
  \leanok
  Under the standing finiteness hypotheses, for every Weil divisor \(D\),
  \[
...
\begin{proof}
  \leanok
  By the normalization \(\deg_{\Pic}\struct X(D) = \deg_K D\) (\ref{def:classDeg}), the claim is the
...
\begin{theorem}[The point-product class map has the same degree]
  ...
  \uses{def:classDeg, def:divisorClass, thm:divisorClass_eq_picClass}
  \leanok
...
\begin{proof}
  ...
  \(\mathrm{divisorClass}(D) = \struct X(D)\) (Theorem~\ref{thm:divisorClass_eq_picClass}), so the
  degree of \(\mathrm{divisorClass}(D)\) is the degree of \(\struct X(D)\), which is \(\deg_K D\) by
  the normalization (\ref{def:classDeg}).
\end{proof}
...
\begin{theorem}[Riemann--Roch-lite in class form on the challenge curve]
  ...
  \uses{def:classDeg, thm:chi_divisorSheaf_curve}
  \leanok
  For every Weil divisor \(D\) on the challenge curve \(C\),
  \[
...
\begin{proof}
  ...
  (Theorem~\ref{thm:chi_divisorSheaf_curve}). Substituting the normalization \(\deg_{\Pic}\struct
  X(D) = \deg_k D\) (\ref{def:classDeg}) gives the claim.
\end{proof}

\section{Cohomology of a Picard class is well-defined}
\label{sec:classCohomology}
...
is a \v Cech Picard class invariant (\ref{thm:chi_divisorSheaf_eq_of_picClass_eq}); the same
...
[\struct X(D')]\), then the extraction lemma \ref{thm:curveDivisor_picClass_eq_iff} writes \(D -
D' = \operatorname{div} g\) for a rational function \(g\), and multiplication by \(g\)
(\ref{thm:mulEquivDivisorSheaf}) realises an isomorphism of sheaves of \(K\)-modules \(\struct
...
\begin{proof}
  ...
  \ref{thm:chi_congr}) gives the two equalities.
\end{proof}
...
\begin{theorem}[Vanishing of \(H^1\) is a Picard-class invariant]
  ...
  \uses{def:divisorSheaf, def:HModule, thm:curveDivisor_picClass_eq_iff,
    thm:mulEquivDivisorSheaf, def:HModule_mapEquiv}
  \leanok
...
\begin{proof}
  ...
  \(H^1(\struct X(D)) \simeq H^1(\struct X(D'))\) (\ref{def:HModule_mapEquiv}), and being a
...
\begin{proof}
  ...
  \(K\)-linear equivalence of cohomology modules (\ref{def:HModule_mapEquiv}); finite
...
\begin{proof}
  ...
  as for the relative curve of Definition~\ref{def:relCurve}). Smoothness of relative dimension
...
\begin{theorem}[The base-changed curve is integral]
  ...
  \dcref{custom}
  \uses{thm:baseChangeCurve_bundle, prop:curve_integral}
...
\begin{proof}
  ...
  \(K\) by Theorem~\ref{thm:baseChangeCurve_bundle}. A geometrically reduced and geometrically
  irreducible scheme over a field is integral (Proposition~\ref{prop:curve_integral}). This is
...
\begin{theorem}[Finiteness of the cohomology of the base-changed curve]
  ...
  \dcref{custom}
  \uses{thm:baseChangeCurve_bundle, thm:h0_moduleKSheaf_curve, thm:moduleFinite_hModule_one}
...
\begin{proof}
  \leanok
  By Theorem~\ref{thm:baseChangeCurve_bundle}, \(C_K\) is a smooth curve over \(K\) --- proper,
  ...
  (Theorem~\ref{thm:h0_moduleKSheaf_curve}) and \(H^1\) is finite-dimensional
  (Theorem~\ref{thm:moduleFinite_hModule_one}).
\end{proof}

\begin{remark}[The degree homomorphism over every extension field]
  \label{rem:classDeg_baseChange}
  \dcref{custom}
  \uses{thm:baseChangeCurve_bundle, thm:baseChangeCurve_integral,
    thm:baseChangeCurve_cohomFinite, def:classDeg}
  \leanok
...
\label{sec:degree_base_field_invariance}
...
\(\pi : C_{K_2} \to C_{K_1}\)
(Chapter~\ref{chap:Cohomology}, Section~\ref{sec:base_field_transition}). Both ends carry
the full degree apparatus (\ref{rem:classDeg_baseChange}). This section proves that the
...
\begin{lemma}[The degree of a power of a class]
  ...
  \uses{def:classDeg, thm:classDeg_mul}
  \leanok
...
\begin{proof}
  ...
  (\ref{def:classDeg}). By additivity (\ref{thm:classDeg_mul}),
...
\begin{theorem}[The fiber-degree identity]
  ...
  \uses{def:scheme_ordZ, def:residueDeg, lem:baseFieldTransition_genericPoint,
    def:functionFieldMap}
  \leanok
...
\begin{proof}
  \leanok
  \uses{thm:finrank_quotient_span_section, def:transitionSectionsBaseChange,
    lem:transitionSectionsBaseChange_tmul, lem:baseFieldTransition_flat_affine_surjective,
    lem:functionFieldMap_germ, thm:baseChangeCurve_integral, thm:baseChangeCurve_bundle}
  Write \(B_1 = \Gamma(C_{K_1}, V)\) and
  \(B_2 = \Gamma(C_{K_2}, \pi^{-1}V)\). The chart \(\pi^{-1}V\) is affine (the
  transition is an affine morphism,
  \ref{lem:baseFieldTransition_flat_affine_surjective}) and contains the generic point
  of \(C_{K_2}\) (the transition matches generic points,
  \ref{lem:baseFieldTransition_genericPoint}); both curves carry the standing bundle
  hypotheses over their fields (\ref{thm:baseChangeCurve_bundle},
  \ref{thm:baseChangeCurve_integral}). By germ naturality
  (\ref{lem:functionFieldMap_germ}), the germ of \(\pi^{\sharp}t\) at the generic point
  of \(C_{K_2}\) is the value of the unit \(\pi^{\sharp}g\).

  \emph{The two colength dictionaries.} The chart colength dictionary
  (\ref{thm:finrank_quotient_span_section}) applies at both ends. Over \(K_2\), with
  ...
  The sections base change (\ref{def:transitionSectionsBaseChange}) is a ring
  isomorphism \(B_1 \otimes_{K_1} K_2 \cong B_2\) carrying \(t \otimes 1\) to
  \(\pi^{\sharp}t\) (\ref{lem:transitionSectionsBaseChange_tmul}) and commuting with
...
\begin{lemma}[Pulled local equations are regular on integral schemes]
  ...
  \uses{def:localEquations_pullback, def:localEquations, lem:functionFieldMap_germ}
  \leanok
...
\begin{proof}
  ...
  germ naturality (\ref{lem:functionFieldMap_germ}) that germ is the image under the
  \emph{injective} function-field map of the germ of \(e\) at \(\eta_X\), which is a
  nonzerodivisor of the field \(K(X)\) by regularity of \(E\)
  (\ref{def:localEquations}), i.e.\ nonzero. Contradiction.
\end{proof}
...
\begin{definition}[The pulled tracked point system]
  ...
  \uses{def:pointEquations, lem:pointUniformizerData, def:localEquations_pullback,
    lem:pullbackEqn_regular, lem:baseFieldTransition_genericPoint, def:functionFieldMap,
    lem:uniformizer, thm:baseChangeCurve_integral}
  \leanok
...
\begin{lemma}[The trivialising elements of the pulled point presentation]
  ...
  \uses{def:pointTransitionEquations, lem:localEquations_presentation,
    def:pointEquations, lem:functionFieldMap_germ}
  \leanok
...
\begin{proof}
  ...
  point system at \(\pi(z)\) (\ref{def:pointTransitionEquations}). If
  \(\pi(z) \neq x'\), that equation is the constant \(1\)
  (\ref{def:pointEquations}), and \(1\) pulls back to \(1\), with germ \(1\). If
  \(\pi(z) = x'\), the equation is the tracked section \(s\), whose germ at
  \(\eta_{C_{K_1}}\) is the chosen uniformizer \(t\) (\ref{def:pointEquations});
  by germ naturality (\ref{lem:functionFieldMap_germ}) the germ at
...
\begin{theorem}[E-iv: the degree is invariant under the base-field transition]
  ...
  \uses{def:classDeg, thm:baseChangeCurve_bundle, thm:baseChangeCurve_integral,
    thm:baseChangeCurve_cohomFinite, def:overSpecMap}
  \leanok
...
\begin{proof}
  \leanok
  \uses{def:curveDivisor_picClass, thm:curveDivisor_picClass_hom, lem:classDeg_zpow,
    thm:curveDivisor_picClass_single, thm:localEquations_picClass_pullback,
    def:pointTransitionEquations, lem:pointTransitionEquations_elem,
    lem:localEquations_presentation, def:presentationDivisor, def:curveDivisor_single,
    lem:curveDivisor_single_arith, thm:sum_ordZ_residueDeg_baseFieldTransition,
    lem:uniformizer, lem:pointUniformizerData, thm:ordZ_germGenericUnits,
    lem:baseFieldTransition_genericPoint, def:scheme_ordZ, def:residueDeg}
  \emph{Reduction to a one-point divisor.} The class map
  \(\Div(C_{K_1}) \to \Pic(C_{K_1})\) is surjective (\ref{def:curveDivisor_picClass}),
  ...
  (\ref{thm:curveDivisor_picClass_hom}) and \(\deg_{\Pic}\) is additive with
  \(\deg_{\Pic}(L^n) = n\deg_{\Pic}(L)\) (\ref{lem:classDeg_zpow}). It therefore
  ...
  (\ref{thm:curveDivisor_picClass_single}). Pulling back a local-equation system
  computes the pullback of its class (\ref{thm:localEquations_picClass_pullback},
  with regularity discharged as in \ref{def:pointTransitionEquations}), so
  ...
  (\ref{lem:localEquations_presentation}, \ref{def:curveDivisor_picClass}). Writing
  \(D''\) for the presentation divisor of the pulled system
  (\ref{def:presentationDivisor}), the normalization of \ref{def:classDeg} turns both
  ...
  \]
  (\ref{lem:curveDivisor_single_arith}).
  ...
  (\ref{def:presentationDivisor}). Off the fibre of \(x'\) the element is \(1\)
  (\ref{lem:pointTransitionEquations_elem}), of order zero; so \(D''\) is supported in
  ...
  (\ref{lem:pointTransitionEquations_elem}).
  ...
  every point of \(V\) other than \(x'\) (\ref{lem:pointUniformizerData}). The
  hypotheses of the fiber-degree identity
  (\ref{thm:sum_ordZ_residueDeg_baseFieldTransition}) hold with this chart, the unit
  ...
  contrapositive of \ref{thm:ordZ_germGenericUnits}, through the discrete valuation of
  ...
  \]
  the uniformizer having order one (\ref{lem:uniformizer}). Chaining the displays
...
\begin{proof}
  ...
  degree \(\deg_{\Pic}(1) = 0\) (\ref{def:classDeg}).
\end{proof}
...
\begin{definition}[The degree on the relative Picard group over a field]
  ...
  \uses{lem:classDeg_picFromBase, def:classDeg}
  \leanok
...
\begin{theorem}[E-iv descends to the relative Picard group]
  ...
  \uses{def:relPicDeg, def:relPicAlgMap, thm:classDeg_cechPicMap_baseFieldTransition}
  \leanok
...
\begin{proof}
  ...
  transition (\ref{def:relPicAlgMap}), so \(\varphi^{*}[L] = [\pi^{*}L]\). By the
  anchor of \ref{def:relPicDeg} and the invariance
  (\ref{thm:classDeg_cechPicMap_baseFieldTransition}),
  \[
...
\label{sec:degree_iso_invariance}
...
Section~\ref{sec:degree_base_field_invariance} with the transition replaced by the
...
\begin{proof}
  ...
  (\ref{def:overAlgebraMap}). Pullback along \(f\) commutes with restriction
...
\begin{lemma}[Residue degrees transport along an isomorphism]
  ...
  \uses{def:residueDeg, def:residueOverAlgebraMap}
  \leanok
...
\begin{proof}
  \leanok
  \uses{lem:overAlgebraMap_app, def:overAlgebraMap}
  ...
  (\ref{def:residueOverAlgebraMap}): the \(K\)-structure of a residue field is the
  composite of the algebra map on global sections, the germ, and the residue map. By
  naturality of the algebra map on sections (\ref{lem:overAlgebraMap_app}, at the full
  ...
  \(\kappa(z)\) agree (\ref{def:residueDeg}).
\end{proof}
...
\begin{definition}[The pulled tracked point system along a generic-to-generic morphism]
  ...
  \uses{def:pointEquations, lem:pointUniformizerData, def:localEquations_pullback,
    lem:pullbackEqn_regular, def:functionFieldMap, lem:uniformizer}
  \leanok
...
\begin{lemma}[The trivialising elements of the generic pulled point presentation]
  ...
  \uses{def:pointPullbackEquations, lem:localEquations_presentation,
    def:pointEquations, lem:functionFieldMap_germ}
  \leanok
...
\begin{proof}
  ...
  \(f(z)\) (\ref{def:pointPullbackEquations}). If \(f(z) \neq x'\), that equation is
  the constant \(1\) (\ref{def:pointEquations}), and \(1\) pulls back to \(1\), with
  germ \(1\). If \(f(z) = x'\), the equation is the tracked section \(s\), whose germ
  at \(\eta_X\) is the chosen uniformizer \(t\) (\ref{def:pointEquations}); by germ
  naturality (\ref{lem:functionFieldMap_germ}) the germ at \(\eta_W\) of
...
\begin{lemma}[The pulled uniformizer has order one at the preimage point]
  ...
  \uses{def:pointPullbackEquations, def:scheme_ordZ, lem:uniformizer,
    lem:pointUniformizerData}
  \leanok
...
\begin{proof}
  \leanok
  \uses{lem:functionFieldMap_germ, lem:germ_generic_eq_algebraMap_germ,
    thm:closedpoint_dvr_stalk, thm:closedpoint_dedekind_stalk, def:scheme_ord,
    lem:adic_valuation_count, lem:mem_pow_iff_le_count}
  Let \(s\) be the tracked section of the uniformizer data at \(x'\), on its tracked
  neighbourhood \(V \ni x'\), with \(\mathrm{germ}_{\eta_X}(s) = t\)
  (\ref{lem:pointUniformizerData}); write \(c \in \struct{X,x'}\) for the germ of
  ...
  valuation ring and a Dedekind domain (\ref{def:scheme_ord},
  \ref{thm:closedpoint_dvr_stalk}, \ref{thm:closedpoint_dedekind_stalk}); its value on
  ...
  (\ref{lem:mem_pow_iff_le_count}, applied in each stalk with the maximal ideal as the
  prime), so the two multiplicities agree.

  Chaining, \(\mathrm{ord}_z(f^{\sharp}t) = \mathrm{ord}_{x'}(t) = 1\), the
  uniformizer having order one (\ref{lem:uniformizer}).
\end{proof}
...
\begin{proof}
  \leanok
  \uses{lem:genericPoint_eq_of_surjective, def:curveDivisor_picClass,
    thm:curveDivisor_picClass_hom, thm:classDeg_mul, lem:classDeg_zpow,
    thm:curveDivisor_picClass_single, thm:localEquations_picClass_pullback,
    def:pointPullbackEquations, lem:pointPullbackEquations_elem,
    lem:ordZ_pointPullbackUnit, lem:residueDeg_eq_of_isIso,
    lem:localEquations_presentation, def:presentationDivisor, def:curveDivisor_single,
    lem:curveDivisor_single_arith, def:scheme_ordZ, def:residueDeg}
  This is the ladder of the base-field invariance
  (\ref{thm:classDeg_cechPicMap_baseFieldTransition}) with the transition replaced by
  ...
  \(f(\eta_W) = \eta_X\) (\ref{lem:genericPoint_eq_of_surjective}).

  \emph{Reduction to a one-point divisor.} The class map
  \(\Div(X) \to \Pic(X)\) is surjective (\ref{def:curveDivisor_picClass}), so
  \(L = \struct{X}(D)\) for a Weil divisor \(D\), a finite \(\Z\)-combination of
  one-point divisors. Both sides of the claimed identity are additive in \(L\): the
  class map and the pullback \(f^{*}\) are homomorphisms
  (\ref{thm:curveDivisor_picClass_hom}) and \(\deg_{\Pic}\) is additive with
  \(\deg_{\Pic}(L^n) = n \deg_{\Pic}(L)\) (\ref{thm:classDeg_mul},
  \ref{lem:classDeg_zpow}). It therefore suffices to prove the identity for
  \(L = \struct{X}(1 \cdot x')\), \(x'\) a non-generic point.

  \emph{The pulled point class as a divisor class.} The one-point class is the class
  of the tracked point equations at \(x'\)
  (\ref{thm:curveDivisor_picClass_single}). Pulling back a local-equation system
  computes the pullback of its class (\ref{thm:localEquations_picClass_pullback}, with
  regularity discharged as in \ref{def:pointPullbackEquations}), so \(f^{*}L\) is the
  ...
  \(f_z\) (\ref{def:presentationDivisor}). Since \(f\) is injective on points, the
  ...
  (\ref{lem:pointPullbackEquations_elem}), of order zero; at \(z_0\) the element is
  the pulled uniformizer \(f^{\sharp}(t)\) (\ref{lem:pointPullbackEquations_elem}), of
  order \(1\) (\ref{lem:ordZ_pointPullbackUnit}). Hence \(D'' = 1 \cdot z_0\)
  (\ref{def:curveDivisor_single}), of degree
  \(\deg_{K}(D'') = [\kappa(z_0) : K]\) (\ref{lem:curveDivisor_single_arith}).

  \emph{Closing with the residue transport.} By
  \ref{lem:residueDeg_eq_of_isIso},
...
\begin{theorem}[The degree is invariant under isomorphisms of curve bundles]
  ...
  \uses{def:classDeg, thm:smooth_geometrically_reduced, prop:curve_integral,
    thm:h0_moduleKSheaf_curve, thm:moduleFinite_hModule_one}
  \leanok
...
\begin{proof}
  \leanok
  \uses{thm:classDeg_cechPicMap_of_isIso, thm:smooth_geometrically_reduced,
    prop:curve_integral, thm:h0_moduleKSheaf_curve, thm:moduleFinite_hModule_one}
  The hypotheses of the scheme form are discharged on both bundles by the curve pack:
  smoothness gives geometric reducedness (\ref{thm:smooth_geometrically_reduced}),
  which together with geometric irreducibility gives integrality
  (\ref{prop:curve_integral}); properness includes quasi-compactness; and \(H^0\) and
  \(H^1\) of the structure sheaf are finite-dimensional over \(K\)
  (\ref{thm:h0_moduleKSheaf_curve}, \ref{thm:moduleFinite_hModule_one}). The
  ...
  \ref{thm:classDeg_cechPicMap_of_isIso} to it and to \(L\).
\end{proof}

\section{The degree seam of the same-carrier comparison}
\label{sec:degree_crossBase_seam}

The base-field shuffle of the relative Picard group at an affine test
(Section~\ref{sec:relPicCrossBase}) reads one class on two presentations of one fiber:
...
identified by the same-carrier comparison (\ref{def:crossBaseAffineIso}), an
...
isomorphisms of Section~\ref{sec:degree_iso_invariance} fired at the comparison.
...
\(C_L := \sigma^{*}C\) the base-changed curve over \(L\) (\ref{def:baseChange}). We
...
test of \(K\) over \(L\), as in Section~\ref{sec:relPicCrossBase}.
...
\begin{theorem}[Degree invariance along the same-carrier comparison]
  ...
  \uses{def:classDeg, thm:classDeg_cechPicMap_of_isIso, def:crossBaseAffineIso,
    lem:crossBaseAffineIso_snd, thm:baseChangeCurve_bundle,
    thm:baseChangeCurve_integral, thm:baseChangeCurve_cohomFinite, def:baseChange}
  \leanok
...
\begin{proof}
  ...
  (\ref{thm:baseChangeCurve_bundle}, \ref{thm:baseChangeCurve_integral},
  \ref{thm:baseChangeCurve_cohomFinite}, at the extension \(K/k\)). For the
  ...
  (\ref{def:baseChange}, \ref{thm:baseChangeCurve_bundle} at the extension
  ...
  projections (\ref{lem:crossBaseAffineIso_snd}), which are the structure morphisms
  of the two fibers over \(\Spec K\); so \(c\) is an isomorphism of curve bundles
  over \(\Spec K\). Apply the scheme-form invariance
  (\ref{thm:classDeg_cechPicMap_of_isIso}) to \(c\) and \(\Lambda\).
\end{proof}
...
\begin{theorem}[The relative degree is invariant under the base-field shuffle]
  ...
  \uses{def:relPicDeg, def:relPicCrossBase,
    thm:classDeg_cechPicMap_crossBaseAffineIso}
  \leanok
...
\begin{proof}
  ...
  \(\mathrm{sh}_K(x) = \pi(c^{*}\Lambda)\) (\ref{def:relPicCrossBase}). The anchor of
  Definition~\ref{def:relPicDeg}, on each side, reads the relative degree off the
  representative:
  \(\deg_{\mathrm{rel}}\bigl(\mathrm{sh}_K(x)\bigr) = \deg_{\Pic}(c^{*}\Lambda)\) and
  \(\deg_{\mathrm{rel}}(x) = \deg_{\Pic}(\Lambda)\). These agree by
  \ref{thm:classDeg_cechPicMap_crossBaseAffineIso}.
\end{proof}

\section{Multiplicativity of the degree under a finite morphism}
\label{sec:degree_pullback}
...
Section~\ref{sec:degree_base_field_invariance}, with two new algebraic inputs on a
...
\begin{proof}
  \leanok
  \uses{lem:functionFieldMap_germ}
  ...
  (\ref{lem:functionFieldMap_germ}): the germ at \(\eta_X\) of a pulled-back section is
...
\begin{lemma}[Module finiteness of the chart extension]
  ...
  \uses{def:pullbackSectionsAlgebra}
  \leanok
...
\begin{proof}
  ...
  applying germ naturality (\ref{lem:functionFieldMap_germ}), the function-field map
  \(K(Y) \to K(X)\) takes the germs of \(a\) and \(b\) at \(\eta_Y\) to equal values;
  the function-field map is injective (\ref{lem:functionFieldMap_germ}), so the germs
...
\begin{proof}
  \leanok
  \uses{lem:moduleFinite_pullbackSections, lem:faithfulSMul_pullbackSections}
  ...
  (\ref{lem:moduleFinite_pullbackSections}) and \(B \to A\) is injective
  (\ref{lem:faithfulSMul_pullbackSections}). The germ maps at the generic points
  ...
  square of \ref{def:pullbackSectionsAlgebra} commutes, so the extension
...
\begin{theorem}[The fiber-degree identity for a finite morphism]
  ...
  \uses{def:scheme_ordZ, def:residueDeg, def:functionFieldMap,
    def:pullbackSectionsAlgebra}
  \leanok
  \dcref{custom}
...
\begin{proof}
  \leanok
  \uses{thm:isDedekindDomain_section, lem:moduleFinite_pullbackSections,
    lem:faithfulSMul_pullbackSections,
    thm:finrank_functionField_eq_finrank_pullbackSections,
    thm:toAdd_ordZ_eq_count_factors, thm:finrank_quotient_primeIdealOf,
    lem:prime_factor_span, lem:primeIdealOf_ne_bot, lem:primeIdealOf_fromSpec_base,
    lem:fromSpec_base_mem, lem:fromSpec_base_ne_genericPoint, lem:appLE_overAlgebraMap,
    thm:residueDeg_pos, lem:functionFieldMap_germ}
  ...
  are Dedekind domains (\ref{thm:isDedekindDomain_section}); \(A\) is a finite
  \(B\)-module (\ref{lem:moduleFinite_pullbackSections}) along the injective chart
  extension (\ref{lem:faithfulSMul_pullbackSections}), hence torsion-free --- \(A\) is
  ...
  (\ref{lem:appLE_overAlgebraMap}). By germ naturality
  (\ref{lem:functionFieldMap_germ}), the germ of \(f^{\sharp}t\) at \(\eta_X\) is the
  ...
  (\ref{lem:fromSpec_base_mem}, \ref{lem:primeIdealOf_fromSpec_base}).

  \emph{The tracked section cuts out the tracked prime.} The point \(y\) corresponds
  to a nonzero prime \(\mathfrak p\) of \(B\) (\ref{lem:primeIdealOf_ne_bot}), maximal
  ...
  \((t)\) contains \(t\) (\ref{lem:prime_factor_span}), so at the point of \(V\)
  ...
  (\ref{thm:toAdd_ordZ_eq_count_factors}), so \((t) = \mathfrak p\). Extending along
  ...
  (\ref{thm:toAdd_ordZ_eq_count_factors}: the order is the multiplicity of the prime of
  ...
  \(\mathfrak q = \mathfrak q_z\) of \(A\) at \(z\) (\ref{lem:primeIdealOf_ne_bot})
  ...
  (\ref{lem:primeIdealOf_fromSpec_base}); and every prime \(\mathfrak q\) over
  \(\mathfrak p\) arises --- it is nonzero (its contraction is \(\mathfrak p \neq 0\)),
  so it corresponds to a non-generic point \(z\) of \(f^{-1}V\)
  (\ref{lem:fromSpec_base_mem}, \ref{lem:fromSpec_base_ne_genericPoint}) at which
  ...
  (\ref{thm:residueDeg_pos}). Members of \(S\) with zero term contribute nothing.
  Hence
  \[
  ...
  (\ref{thm:finrank_functionField_eq_finrank_pullbackSections}),
...
\begin{theorem}[Degree multiplicativity under pullback: scheme form]
  ...
  \uses{def:classDeg, def:pullbackSectionsAlgebra}
  \leanok
  \dcref{custom}
...
\begin{proof}
  \leanok
  \uses{def:curveDivisor_picClass, thm:curveDivisor_picClass_hom, thm:classDeg_mul,
    lem:classDeg_zpow, thm:curveDivisor_picClass_single,
    thm:localEquations_picClass_pullback, def:pointPullbackEquations,
    lem:pointPullbackEquations_elem, lem:localEquations_presentation,
    def:presentationDivisor, def:curveDivisor_single, lem:curveDivisor_single_arith,
    thm:sum_ordZ_residueDeg_of_isFinite, lem:uniformizer, lem:pointUniformizerData,
    thm:ordZ_germGenericUnits, def:scheme_ordZ, def:residueDeg}
  Write \(n = [K(X) : K(Y)]\).

  \emph{Reduction to a one-point divisor.} The class map
  \(\Div(Y) \to \Pic(Y)\) is surjective (\ref{def:curveDivisor_picClass}), so
  \(L = \struct{Y}(D)\) for a Weil divisor \(D\), a finite \(\Z\)-combination of
  one-point divisors. Both sides of the claimed identity are additive in \(L\): the
  class map and the pullback \(f^{*}\) are homomorphisms
  (\ref{thm:curveDivisor_picClass_hom}) and \(\deg_{\Pic}\) is additive with
  \(\deg_{\Pic}(L^m) = m \deg_{\Pic}(L)\) (\ref{thm:classDeg_mul},
  \ref{lem:classDeg_zpow}). It therefore suffices to prove the identity for
  \(L = \struct{Y}(1 \cdot x')\), \(x'\) a non-generic point of \(Y\).

  \emph{The pulled point class as a divisor class.} The one-point class is the class
  of the tracked point equations at \(x'\) (\ref{thm:curveDivisor_picClass_single}).
  Pulling back a local-equation system computes the pullback of its class
  (\ref{thm:localEquations_picClass_pullback}, with regularity discharged as in
  \ref{def:pointPullbackEquations}), so \(f^{*}L\) is the Picard class of the pulled
  tracked point system along \(f\) (\ref{def:pointPullbackEquations}, for the
  ...
  \(f_z\) (\ref{def:presentationDivisor}). Off the fiber of \(x'\) the element is
  \(1\) (\ref{lem:pointPullbackEquations_elem}), of order zero; so \(D''\) is
  supported in the finite set \(S := \mathrm{supp}(D'') \subseteq f^{-1}(x')\), and on
  the fiber the element is the pulled uniformizer
  (\ref{lem:pointPullbackEquations_elem}), so
  \[
  ...
  (\ref{lem:pointUniformizerData}), and \(\mathrm{ord}_{x'}(t) = 1\)
  (\ref{lem:uniformizer}). The hypotheses of the fiber-degree identity
  (\ref{thm:sum_ordZ_residueDeg_of_isFinite}) hold with this chart, the unit
  ...
  nontrivial order there --- the contrapositive of \ref{thm:ordZ_germGenericUnits},
...
\begin{theorem}[Degree multiplicativity under pullback of curve bundles]
  \label{thm:classDeg_cechPicMap_of_isFinite}
...
\begin{proof}
  \leanok
  \uses{thm:classDeg_cechPicMap_eq_finrank_mul, thm:smooth_geometrically_reduced,
    prop:curve_integral, thm:h0_moduleKSheaf_curve, thm:moduleFinite_hModule_one,
  ...
  matches the generic points (\ref{lem:genericPoint_eq_of_surjective}). Apply
  \ref{thm:classDeg_cechPicMap_eq_finrank_mul}.
\end{proof}
...
\begin{proof}
  \leanok
  \uses{cor:cechPicMap_eq_one_of_range_eq_singleton, thm:overHom_isProper,
    thm:smooth_geometrically_reduced, prop:curve_integral, thm:h0_moduleKSheaf_curve,
    thm:moduleFinite_hModule_one}
  The bundle \(D\) is integral (\ref{thm:smooth_geometrically_reduced},
  \ref{prop:curve_integral}), in particular reduced, and its \(H^0\) and \(H^1\) are
  finite-dimensional over \(K\) (\ref{thm:h0_moduleKSheaf_curve},
  \ref{thm:moduleFinite_hModule_one}), so the degree homomorphism of
  \ref{def:classDeg} is available on \(D\). The underlying morphism of \(h\) is proper
  (\ref{thm:overHom_isProper}: the structure morphism of \(D\) is proper and that of
  \(E\) is separated, properness including separatedness), in particular
  quasi-compact. By \ref{cor:cechPicMap_eq_one_of_range_eq_singleton},
  \(h^{*}L = 1\); and the degree of the trivial class is zero, \(\deg_{\Pic}\) being a
  homomorphism (\ref{def:classDeg}).
\end{proof}
...
\begin{definition}[The pinned two-cover and its coordinate]
  ...
  \uses{def:P1_chartOpen, def:P1_chartCoord, lem:preimage_chart_sup, lem:P1_chartOpen_inf}
  \leanok
...
\begin{proof}
  \leanok
  Since \(U_0 \cup U_1 = \PP^1\), the preimages cover \(Y\) (\ref{lem:preimage_chart_sup}), so a
  ...
  (\ref{lem:P1_chartOpen_inf}); in other words the basic open of \(t_{01}\) in \(U_0\) is
...
\begin{definition}[The fibre divisor of \(n\cdot(\text{fibre})\)]
  ...
  \uses{def:fiberCover, def:localEquations}
  \leanok
...
\begin{proof}
  \leanok
  \uses{lem:germ_generic_eq_algebraMap_germ}
  ...
  (\ref{def:fiberCover}), in particular nonzero. For any \(y \in V_0\), the germ of \(t_0\) at
  \(\eta\) is the image of the germ at \(y\) under the localisation map \(\struct{Y,y} \to K(Y)\)
  (\ref{lem:germ_generic_eq_algebraMap_germ}); were the germ at \(y\) zero, its image --- the
...
\begin{definition}[The fibre twist and its power law]
  ...
  \uses{def:fiberDivisor, def:localEquations_picClass}
  \leanok
...
\begin{proof}
  ...
  fibre divisor (\ref{def:localEquations_picClass}). Write \(g^{(n)}_{xy}\) for its transition unit
  between the members at points \(x, y\), the unique unit with \(f^{(n)}_x|_{\cap} = g^{(n)}_{xy}
  \cdot f^{(n)}_y|_{\cap}\) on the overlap (\ref{lem:localEquations_ratioUnit}), where \(f^{(n)}\)
...
\begin{theorem}[The degree of the fibre twist scales linearly]
  ...
  \uses{def:fiberTwist, def:classDeg, thm:classDeg_mul}
  \leanok
...
\begin{proof}
  \leanok
  By the power law \(\Theta_n = \Theta_1^{\,n}\) (\ref{def:fiberTwist}) and additivity of the class
  degree (\ref{thm:classDeg_mul}), we argue by induction on \(n\). The base case is
...
\begin{definition}[The fibre unit]
  ...
  \uses{def:fiberCover, def:fiberDivisor, def:P1_chartCoord, lem:P1_chartOpen_inf}
  \leanok
...
\begin{proof}
  ...
  overlap \(U_0 \cap U_1 = D_+(X_0 X_1)\) (\ref{def:P1_chartCoord}, \ref{lem:P1_chartOpen_inf}).
  ...
  (\ref{def:fiberDivisor}), so taking germs at \(\eta\) --- a ring homomorphism, compatible with
...
\begin{theorem}[The order table of the fibre unit]
  ...
  \uses{def:fiberCoordUnit, def:fiberCover, def:divOf, def:scheme_ord, def:scheme_ordZ,
    lem:germ_generic_eq_algebraMap_germ, lem:ord_algebraMap_stalk_le_one}
  \leanok
...
\begin{proof}
  ...
  (\ref{lem:germ_generic_eq_algebraMap_germ}). The image in \(K(Y)\) of a stalk element has no pole
  at \(x\) (\ref{lem:ord_algebraMap_stalk_le_one}), so \(\mathrm{ord}_x(u) \ge 0\).
  ...
  \(K(Y)^{\times} \to \Z\) (\ref{def:scheme_ordZ}), \(\mathrm{ord}_x(u) = -\mathrm{ord}_x(u^{-1}) \le
  0\).

  For \(x \in V_0\) with \(x \notin V_1\), the germ of \(t_0\) at \(x\) is not a unit, since \(x\)
  lies outside \(D(t_0) = V_0 \cap V_1\) (\ref{def:fiberCover}); it is nonzero, its image in \(K(Y)\)
  being the unit \(u\). The stalk \(\struct{Y,x}\) is a discrete valuation ring (\ref{def:scheme_ord}),
...
\begin{definition}[The fibre Weil divisor]
  ...
  \uses{thm:fiberCoordUnit_order, def:divOf, def:curveDivisor, def:curveDivisor_deg,
    thm:residueDeg_pos}
  \leanok
...
\begin{proof}
  ...
  \(\mathrm{ord}_x(u) \le 0\) (\ref{thm:fiberCoordUnit_order}), so \(F_x = \max(\mathrm{ord}_x(u), 0)
  = 0\); on \(V_0\) one has \(\mathrm{ord}_x(u) \ge 0\), so \(F_x = \mathrm{ord}_x(u)\). The degree
  \(\deg_K F = \sum_x F_x\,[\kappa(x):K]\) (\ref{def:curveDivisor_deg}) is a finite sum of
...
\begin{theorem}[The twist leaves the lattice fixed off \(V_0\)]
  ...
  \uses{def:fiberWeilDivisor, def:divisorSections}
  \leanok
...
\begin{proof}
  \leanok
  The fibre divisor \(F\) vanishes at every point of \(V_1\) (\ref{def:fiberWeilDivisor}), so at each
  ...
  of \(U\) is bounded by \(A\) (\ref{def:divisorSections}); since the bounds imposed by \(D + nF\) and
...
\begin{theorem}[The twist grows the lattice over \(V_0\)]
  ...
  \dcref{custom}
  \lean{AlgebraicGeometry.divisorSections_add_nsmul_fiberWeilDivisor_chart₀}
  \uses{def:fiberWeilDivisor, def:fiberCoordUnit, def:divisorSections, thm:mem_boundedSections_mul_iff,
    def:divOf}
  \leanok
...
\begin{proof}
  \leanok
  On \(V_0\) the fibre divisor agrees with \(\mathrm{div}(u)\) (\ref{def:fiberWeilDivisor}), so at
  ...
  \ref{def:divOf}), this reads \((D + nF)_x = D_x - \mathrm{ord}_x(u^{-n}) = \bigl(D -
  \mathrm{div}(u^{-n})\bigr)_x\). Hence \(\struct Y(D + nF)(V_0) = \struct Y\bigl(D -
  \mathrm{div}(u^{-n})\bigr)(V_0)\). By the bound-shift (\ref{thm:mem_boundedSections_mul_iff}) with
...
\begin{definition}[The denominator lattice and its exhaustion]
  ...
  \uses{thm:divisorSections_twist_fixed, thm:divisorSections_twist_chart0, def:fiberWeilDivisor,
    thm:fiberCoordUnit_order, def:divisorSections, def:divOf}
  \leanok
...
\begin{proof}
  ...
  restrictive (\ref{def:divisorSections}). Applying this to \(V_0 \cap V_1 \subseteq V_0\) and \(V_0
  \cap V_1 \subseteq V_1\), and using \(\struct Y(D + nF)(V_0 \cap V_1) = N\)
  (\ref{thm:divisorSections_twist_fixed}), both \(\struct Y(D + nF)(V_0)\) and \(\struct Y(D +
  nF)(V_1)\) are contained in \(N\); hence so is their join \(A_n\).

  \emph{The chain is increasing.} \(F\) is effective (\ref{def:fiberWeilDivisor}), so \(D + nF \le D +
  (n+1)F\); the section lattice of a larger divisor contains that of a smaller one over any fixed open
  (\ref{def:divisorSections}), over both \(V_0\) and \(V_1\), whence \(A_n \le A_{n+1}\).
  ...
  \(\mathrm{div}(s)\) are supported on a finite set of closed points (\ref{def:curveDivisor},
  \ref{def:divOf}). Consider a closed point \(x \in V_0\). If also \(x \in V_1\), then \(F_x = 0\)
  (\ref{def:fiberWeilDivisor}) and membership \(s \in N\) gives \(-\mathrm{ord}_x(s) \le D_x = (D +
  nF)_x\). If \(x \notin V_1\), then \(F_x \ge 1\) (\ref{thm:fiberCoordUnit_order}, via
  \ref{def:fiberWeilDivisor}); choosing \(n\) at least the maximum, over the finitely many exceptional
...
(Theorem~\ref{thm:twoCoverH1LinearEquiv}) runs on any sheaf of \(K\)-modules whose degree-one
...
(Definition~\ref{def:qcohOn}). This section supplies that packaging on every open, so the twisted
...
\begin{theorem}[The divisor sheaves are quasi-coherently packaged]
  ...
  \uses{def:qcohOn, def:divisorSheaf, def:divisorSections, lem:ord_algebraMap_stalk_le_one,
    lem:ord_eq_one_of_mem_basicOpen, def:divOf, def:curveDivisor}
  \leanok
...
\begin{proof}
  ...
  every closed point \(x \in U\) --- its order is \(\le 1\), no pole (Lemma~\ref{lem:ord_algebraMap_stalk_le_one})
  --- so multiplying a section of \(\struct X(D)\) by it can only improve the pole bounds: the
  product again lies in \(\struct X(D)(W)\) (Definition~\ref{def:divisorSections}). The module laws
  ...
  \(0\) (Lemma~\ref{lem:ord_eq_one_of_mem_basicOpen}), while off \(D(g)\), at a zero of \(g\), it
  has order \(\ge 1\); both \(D\) and \(\mathrm{div}(c)\) are finitely supported
  (Definitions~\ref{def:curveDivisor}, \ref{def:divOf}). Choosing \(N\) at least the finite
...
\begin{theorem}[Twisted affine vanishing for divisor sheaves]
  ...
  \uses{thm:divisorSheaf_qcohOn, thm:hModule1_vanishing_qcoh}
  \leanok
...
\begin{proof}
  \leanok
  Apply the twisted affine vanishing \ref{thm:hModule1_vanishing_qcoh} to the quasi-coherence
  packaging of \(\struct X(D)\) on \(U\) supplied by Theorem~\ref{thm:divisorSheaf_qcohOn}.
\end{proof}
...
and its fibre divisor \(F = \mathrm{fiberWeilDivisor}(\pi)\) (Definition~\ref{def:fiberWeilDivisor}),
...
\begin{definition}[The fibre lattice inside the overlap, and the twisted \(H^1\)]
  ...
  \uses{def:fiberLattice, thm:twoCoverH1LinearEquiv, thm:subsingleton_hModule_divisorSheaf_one,
    thm:divisorSections_twist_fixed, def:divisorSheaf, def:TwoCover_diff}
  \leanok
...
\begin{proof}
  ...
  (Definition~\ref{def:fiberLattice}) read through the inclusion of \(N\), the comap of a submodule
  along the subtype inclusion. For the isomorphism, the two-cover computation
  (Theorem~\ref{thm:twoCoverH1LinearEquiv}) applies to \(\struct Y(D + nF)\) on the cover \(V_0,
  V_1\): its two affine vanishing hypotheses \(H'^1(V_i, \struct Y(D + nF)) = 0\) are supplied by
  Theorem~\ref{thm:subsingleton_hModule_divisorSheaf_one}, giving
  \[
    H^1\bigl(\struct Y(D + nF)\bigr) \cong \struct Y(D + nF)(V_0 \cap V_1) \big/
      \mathrm{range}\,(\text{restriction-difference}) .
  \]
  The restriction-difference map (Definition~\ref{def:TwoCover_diff}) reads on underlying rational
  ...
  (Theorem~\ref{thm:divisorSections_twist_fixed}). Substituting identifies the cokernel with \(N /
  \overline A_n\).
\end{proof}
...
\begin{theorem}[Fibrewise large-twist vanishing]
  ...
  \uses{def:fiberLatticeH1Equiv, thm:eventually_eq_top_of_monotone_of_iSup_eq_top, def:fiberLattice,
    thm:moduleFinite_divisorSheaf, def:fiberWeilDivisor}
  \leanok
...
\begin{proof}
  ...
  (Definition~\ref{def:fiberLatticeH1Equiv}) it suffices to show \(\overline A_n = N\) for all large
  \(n\). The chain \(\overline A_n\) is increasing and exhausts \(N\) (the exhaustion of
  Definition~\ref{def:fiberLattice}, by pole clearing at the finite fibre over \([1 : 0]\)). Its
  ...
  (Theorem~\ref{thm:moduleFinite_divisorSheaf}). A finite-dimensional \(K\)-module is Noetherian, so
  the stabilisation Theorem~\ref{thm:eventually_eq_top_of_monotone_of_iSup_eq_top} gives \(n_0\) with
...
\begin{theorem}[Fibrewise large-twist vanishing along a finite map]
  ...
  \uses{thm:subsingleton_hModule_divisorSheaf_one_of_isDominant_toP1,
    thm:moduleFinite_hModule_one_of_isFinite_toP1}
  \leanok
...
\begin{proof}
  ...
  (Theorem~\ref{thm:moduleFinite_hModule_one_of_isFinite_toP1}), discharging the remaining
  finiteness hypothesis of Theorem~\ref{thm:subsingleton_hModule_divisorSheaf_one_of_isDominant_toP1}.
\end{proof}

\section{Effective witnesses and peeling}
\label{sec:effective_peeling}
...
\begin{lemma}[Class arithmetic]
  ...
  \uses{def:curveDivisor_picClass, thm:curveDivisor_picClass_hom, def:classDeg,
    thm:classDeg_mul, lem:classDeg_zpow}
  \leanok
...
\begin{proof}
  ...
  (Theorem~\ref{thm:curveDivisor_picClass_hom}) and of the degree
  (Theorem~\ref{thm:classDeg_mul}); the bound comparison unfolds the definition of the
...
\begin{lemma}[A point of \(\PP^1\) off the second chart]
  ...
  \uses{def:P1_chartOpen, def:P1_chartIota, lem:P1_chartIota_range, def:P1_chartCoord,
    def:P1_awayAlgEquiv}
  \leanok
...
\begin{proof}
  \leanok
  On the chart \(D_+(X_0) = \Spec k[X_1/X_0]\) (Definitions~\ref{def:P1_chartOpen},
  \ref{def:P1_awayAlgEquiv}), the preimage of \(D_+(X_1)\) is the basic open of the chart
  coordinate \(X_1/X_0\) (Definition~\ref{def:P1_chartCoord}). The chart coordinate is not
  ...
  (Definition~\ref{def:P1_chartIota}, with range \(D_+(X_0)\) by
  Lemma~\ref{lem:P1_chartIota_range}) lies in \(D_+(X_0)\) but not in the basic open of the
  coordinate, hence not in \(D_+(X_1)\).
\end{proof}

\begin{lemma}[A finite dominant morphism is surjective]
  \label{lem:surjective_of_isFinite_of_isDominant}
  \dcref{custom}
...
\begin{theorem}[Strict positivity of the fiber divisor's degree]
  ...
  \uses{lem:P1_point_off_chart1, lem:surjective_of_isFinite_of_isDominant,
    def:fiberWeilDivisor, def:fiberCover}
  \leanok
...
\begin{proof}
  ...
  (Lemma~\ref{lem:P1_point_off_chart1}). By surjectivity of the finite dominant \(\pi\)
  (Lemma~\ref{lem:surjective_of_isFinite_of_isDominant}) there is \(x_0 \in Y\) over \(z\);
  ...
  Definition~\ref{def:fiberWeilDivisor}).
\end{proof}

\begin{lemma}[An effective witness of a class of large degree]
  \label{lem:exists_effective_of_picClass}
  \dcref{custom}
  \lean{AlgebraicGeometry.exists_effective_of_picClass}
  \uses{thm:riemann_inequality, def:h0_h1_chi, def:divisorSheaf, def:HModule_linearEquiv0,
    def:divisorSections, def:scheme_ord, def:divOf, thm:curveDivisor_picClass_hom,
    def:curveDivisor}
  \leanok
...
\begin{proof}
  \leanok
  By the Riemann inequality (Theorem~\ref{thm:riemann_inequality}),
  ...
  (Definition~\ref{def:HModule_linearEquiv0}) and the section description of the divisor
  sheaf (Definitions~\ref{def:divisorSheaf}, \ref{def:divisorSections}), a nonzero rational
  function \(f \in K(Y)^{\times}\) with \(\mathrm{ord}_x(f) \ge -\mathrm{coeff}_x(W)\) at
  every point (Definition~\ref{def:scheme_ord}). Set \(E := W + \mathrm{div}(f)\)
  (Definition~\ref{def:divOf}): the order bound says exactly \(E \ge 0\) coefficientwise,
  and principal divisors have trivial class
  (Theorem~\ref{thm:curveDivisor_picClass_hom}), so \(\struct Y(E) = \struct Y(W)\).
\end{proof}
...
\begin{lemma}[Peeling one point]
  ...
  \uses{def:devissageSES, thm:devissageSES_shortExact, thm:skyModule_h1_vanishing,
    thm:HModule_surjective_map_f, lem:devissageDivisor_sub, def:curveDivisor_single,
    def:jumpModule}
  \leanok
...
\begin{proof}
  ...
  (Lemma~\ref{lem:devissageDivisor_sub}). The d\'evissage short exact sequence
  (Definition~\ref{def:devissageSES}, Theorem~\ref{thm:devissageSES_shortExact})
  \(0 \to \struct Y(B) \to \struct Y(D) \to \mathrm{sky}_x \to 0\) has skyscraper cokernel,
  whose \(H^1\) vanishes (Theorem~\ref{thm:skyModule_h1_vanishing}); the long exact
  sequence therefore gives a surjection \(H^1(\struct Y(B)) \twoheadrightarrow
  H^1(\struct Y(D))\) (Theorem~\ref{thm:HModule_surjective_map_f}), and a quotient of a
...
\begin{lemma}[Effective peeling]
  ...
  \uses{lem:peel_single, def:curveDivisor_single, def:curveDivisor}
  \leanok
...
\begin{proof}
  ...
  \(b \, p\) (Lemma~\ref{lem:peel_single}) gives \(H^1(\struct Y(A + E' + b\,p)) =
  H^1(\struct Y(A + E)) = 0\).
\end{proof}
...
\begin{theorem}[The class form of the fibrewise large-twist vanishing]
  ...
  \uses{thm:subsingleton_hModule_divisorSheaf_one_of_isFinite_toP1,
    thm:zero_lt_deg_fiberWeilDivisor, lem:exists_effective_of_picClass, lem:peel_effective,
    thm:subsingleton_hModule_one_class, lem:picClass_nsmul, def:classDeg,
    def:curveDivisor_picClass, thm:classDeg_mul, lem:classDeg_zpow, def:fiberWeilDivisor}
  \leanok
...
\begin{proof}
  \leanok
  Realise \(l\) by a divisor \(D_0\) (Definition~\ref{def:curveDivisor_picClass}). Run the
  fibrewise vanishing at \(D_0\)
  (Theorem~\ref{thm:subsingleton_hModule_divisorSheaf_one_of_isFinite_toP1}): there is
  \(m_0\) with \(H^1(\struct Y(D_0 + m\,F)) = 0\) for all \(m \ge m_0\), \(F\) the fiber
  divisor; fix \(m = m_0\). Let \(d' := \deg F > 0\)
  (Theorem~\ref{thm:zero_lt_deg_fiberWeilDivisor}) and \(\varphi := \struct Y(F)\). Take
  ...
  (Lemma~\ref{lem:picClass_nsmul}, Theorem~\ref{thm:classDeg_mul},
  Lemma~\ref{lem:classDeg_zpow}, using \(\deg\theta \ge 1\)). Realise \(c\) by a divisor
  \(W\); then \(1 \le \deg W + \chi(\struct Y)\), so \(c\) has an effective witness \(E\)
  (Lemma~\ref{lem:exists_effective_of_picClass}). The divisor \(D_0 + m_0 F + E\) has class
  ...
  (Lemma~\ref{lem:peel_effective}). Finally, witness-independence
  (Theorem~\ref{thm:subsingleton_hModule_one_class}) transports the vanishing from this one
...
\begin{theorem}[The rank anchor]
  ...
  \uses{thm:chi_eq_h0, thm:chi_divisorSheaf, def:h0_h1_chi}
  \leanok
...
\begin{proof}
  ...
  (Theorem~\ref{thm:chi_eq_h0}), and the \(\chi\)-ledger reads
  \(\chi(\struct Y(D)) = \deg D + \chi(\struct Y)\)
  (Theorem~\ref{thm:chi_divisorSheaf}).
\end{proof}

\section{The uniform vanishing bound}
\label{sec:uniformVanishing}
...
\begin{proof}
  ...
  \(0 < \deg F\) (Theorem~\ref{thm:zero_lt_deg_fiberWeilDivisor}).

  Let \(D\) have \(\deg D \ge b\). The residual class
  \(c := \struct Y(D) \cdot \struct Y(F)^{-n_1}\) has degree
  \(\deg D - n_1\deg F \ge 1 - \chi(\struct Y)\) (Lemma~\ref{lem:picClass_nsmul},
  Theorem~\ref{thm:classDeg_mul}, Lemma~\ref{lem:classDeg_zpow}); realise it by a divisor
  \(W\) (Definition~\ref{def:curveDivisor_picClass}) and take an effective witness
  \(E \ge 0\) of its class (Lemma~\ref{lem:exists_effective_of_picClass}). Then
  \(n_1 F + E\) is in the class of \(D\). Vanishing holds at \(n_1 F\) by the base choice;
  effective peeling (Lemma~\ref{lem:peel_effective}) transports it to \(n_1 F + E\); and
  witness-independence (Theorem~\ref{thm:subsingleton_hModule_one_class}) upgrades it to
...
\begin{theorem}[The presentation divisor of the unit fiber divisor]
  ...
  \uses{def:fiberDivisor, def:fiberCoordUnit, thm:fiberCoordUnit_order,
    def:fiberWeilDivisor, def:presentationDivisor, lem:localEquations_presentation,
    def:curveDivisor, def:fiberCover}
  \leanok
...
\begin{proof}
  ...
  (Lemma~\ref{lem:localEquations_presentation}) of the pinned two-cover system \((t_0\)
  on \(V_0\), \(1\) on \(V_1)\) (Definition~\ref{def:fiberDivisor}) has trivialising
  element the fiber coordinate unit \(u = t_0\) at points of \(V_0\)
  (Definition~\ref{def:fiberCoordUnit}) and the constant \(1\) at points off \(V_0\)
  (which lie in \(V_1\), the two charts covering \(Y\)). At \(x \in V_0\) the coefficient
  of the presentation divisor (Definition~\ref{def:presentationDivisor}) is
  \(\mathrm{ord}_x(u)\), which the fiber order table identifies with the coefficient of
  \(F = (\mathrm{div}\,u)^{+}\) at \(x\) (Theorem~\ref{thm:fiberCoordUnit_order},
  Definition~\ref{def:fiberWeilDivisor}); at \(x \notin V_0\) both coefficients are zero —
...
\begin{theorem}[\(\Theta\)-positivity of the unit fiber twist]
  ...
  \uses{thm:presentationDivisor_fiberDivisor_one, lem:localEquations_presentation,
    def:curveDivisor_picClass, def:classDeg, def:fiberTwist,
    thm:zero_lt_deg_fiberWeilDivisor, thm:picClass_eq_of_presentationDivisor_eq}
  \leanok
...
\begin{proof}
  ...
  (Definition~\ref{def:fiberTwist}); the class of a local-equation system is the class of
  the presentation divisor of its meromorphic presentation
  (Lemma~\ref{lem:localEquations_presentation},
  Theorem~\ref{thm:picClass_eq_of_presentationDivisor_eq} via the anchored class map of
  Definition~\ref{def:curveDivisor_picClass}), which is \(F\)
  (Theorem~\ref{thm:presentationDivisor_fiberDivisor_one}). Hence
  \(\Theta_1 = \struct Y(F)\), and the degree of an anchored class reads the divisor degree
  (Definition~\ref{def:classDeg}). Positivity is
  Theorem~\ref{thm:zero_lt_deg_fiberWeilDivisor}.
\end{proof}
...
\begin{theorem}[The positivity constant transports across base-field transitions]
  ...
  \uses{thm:classDeg_cechPicMap_baseFieldTransition}
  \leanok
...
\begin{proof}
  ...
  (Theorem~\ref{thm:classDeg_cechPicMap_baseFieldTransition}); the threshold transfers
  verbatim.
\end{proof}

\section{Evaluation at the graph section}
\label{sec:graphSectionEval}
...
\begin{definition}[The evaluation at the graph section]
  ...
  \uses{def:sectionOfPoint, def:graphPoint}
  \leanok
...
\begin{theorem}[The evaluation splits the \(K\)-structure]
  ...
  \uses{def:graphSectionEval, def:overSpecGammaTop, def:sectionOfPoint}
  \leanok
...
\begin{proof}
  ...
  \(g_t \circ \mathrm{snd} = \id\) (Definition~\ref{def:sectionOfPoint}), so the composite
...
\begin{theorem}[The point--evaluation dictionary]
  ...
  \uses{def:graphSectionEval, thm:graphSectionEval_algebraMap, def:graphPoint}
  \leanok
...
\begin{proof}
  ...
  a unit (the target is a field, Theorem~\ref{thm:graphSectionEval_algebraMap}), so its
...
\begin{theorem}[The quotient by the kernel is a copy of \(K\)]
  ...
  \uses{def:graphSectionEval, thm:graphSectionEval_algebraMap}
  \leanok
...
\begin{proof}
  ...
  (Theorem~\ref{thm:graphSectionEval_algebraMap}), so the quotient by its kernel is
  ...
  bijective (Theorem~\ref{thm:graphSectionEval_algebraMap} again).
\end{proof}

\section{The graph chart}
\label{sec:graphChart}
...
\begin{definition}[The graph chart and the graph equation]
  ...
  \uses{def:diagonalChartData, def:productChart, def:productChartSections,
    thm:isLocalization_away_basicOpen_productChartSections, def:pointGen_pointEv}
  \leanok
...
\begin{lemma}[The tensor-level evaluation and the graph point]
  ...
  \uses{def:graphChart, thm:appLE_productChartSections, def:graphSectionEval,
    def:graphPoint, def:diagonalChartData, def:pointGen_pointEv}
  \leanok
...
\begin{proof}
  ...
  \(p_0 \in U\) (Definition~\ref{def:graphPoint}), and the second-factor condition is
  vacuous at \(\top\). \emph{Evaluation}: on a pure tensor \(a \otimes b\), the lift-app
  rule for the section \(g_t = \langle t, \id\rangle\)
  (Theorem~\ref{thm:appLE_productChartSections}) computes
  ...
  (Definition~\ref{def:diagonalChartData}), so
...
\begin{theorem}[The graph point lies in the graph chart]
  ...
  \uses{lem:graphTensorEval, thm:ker_graphSectionEval_eq_primeIdealOf, def:graphChart,
    thm:graphSectionEval_algebraMap}
  \leanok
...
\begin{proof}
  ...
  (Theorem~\ref{thm:ker_graphSectionEval_eq_primeIdealOf}) reads: the section lies in
  \(\ker\varepsilon_t\). But its evaluation is
  \(\mathrm{ev}(1 - \tilde e_F) = 1 - 0 = 1 \neq 0\) in the field \(F\)
  (Lemma~\ref{lem:graphTensorEval}, Theorem~\ref{thm:graphSectionEval_algebraMap}) — a
  contradiction.
\end{proof}
...
\begin{theorem}[The kernel presentation of the graph ideal]
  ...
  \uses{def:graphChart, lem:graphTensorEval, thm:graphPoint_mem_graphChart,
    thm:ker_pointEv_map_localization_eq, def:diagonalChartData,
    thm:isLocalization_away_basicOpen_productChartSections, def:graphSectionEval}
  \leanok
...
\begin{proof}
  ...
  (Theorem~\ref{thm:isLocalization_away_basicOpen_productChartSections}), and on the
  localisation the evaluation \(\varepsilon_t\) restricts, along the structure map, to the
  tensor-level evaluation \(\mathrm{ev}\) (Lemma~\ref{lem:graphTensorEval}, compatible with
  ...
  (Definition~\ref{def:diagonalChartData}), so the localised point-fiber keystone
  (Theorem~\ref{thm:ker_pointEv_map_localization_eq}) computes the extension of
  \(\ker(\mathrm{ev})\) to the chart as the principal ideal on the image of the point
  generator — which is \(\gamma\) (Definition~\ref{def:graphChart}).
...
\begin{theorem}[The colength of the graph equation is one]
  ...
  \uses{thm:ker_graphSectionEval_eq_span_graphChartEqn,
    thm:finrank_quotient_ker_graphSectionEval}
  \leanok
  \[
...
\begin{proof}
  ...
  (Theorem~\ref{thm:ker_graphSectionEval_eq_span_graphChartEqn}), and the quotient by the
  kernel is a copy of \(K\)
  (Theorem~\ref{thm:finrank_quotient_ker_graphSectionEval}).
\end{proof}

\section{The degree-one certificate of the graph class}
\label{sec:graphDegree}
...
\begin{lemma}[The graph equation is nonzero]
  ...
  \uses{thm:diagonal_regularity_engine, def:graphChart, thm:graphSectionEval_algebraMap,
    thm:graphPoint_mem_graphChart, def:diagonalChartData,
    thm:isLocalization_away_basicOpen_productChartSections}
  \leanok
...
\begin{proof}
  \leanok
  The regularity engine (Theorem~\ref{thm:diagonal_regularity_engine}) makes
  ...
  (Lemma~\ref{lem:graphTensorEval}, Theorem~\ref{thm:graphSectionEval_algebraMap},
  Theorem~\ref{thm:graphPoint_mem_graphChart}), where \(1 \neq 0\).
\end{proof}

\begin{theorem}[The graph point is closed]
  \label{thm:graphPoint_ne_genericPoint}
  \dcref{custom}
  \lean{AlgebraicGeometry.graphPoint_ne_genericPoint}
  \uses{lem:graphChartEqn_ne_zero, thm:ker_graphSectionEval_eq_span_graphChartEqn,
    thm:ker_graphSectionEval_eq_primeIdealOf, lem:fromSpec_base_ne_genericPoint,
    lem:primeIdealOf_fromSpec_base}
  \leanok
...
\begin{proof}
  ...
  (Theorems~\ref{thm:ker_graphSectionEval_eq_primeIdealOf},
  \ref{thm:ker_graphSectionEval_eq_span_graphChartEqn}), a \emph{nonzero} ideal since
  \(\gamma \neq 0\) (Lemma~\ref{lem:graphChartEqn_ne_zero}). Points of nonzero primes of an
  affine open are not the generic point
  (Lemmas~\ref{lem:fromSpec_base_ne_genericPoint},
  \ref{lem:primeIdealOf_fromSpec_base}).
\end{proof}
...
\begin{lemma}[The graph equation is a unit away from the graph point]
  ...
  \uses{thm:ker_graphSectionEval_eq_span_graphChartEqn,
    thm:finrank_quotient_ker_graphSectionEval, thm:ker_graphSectionEval_eq_primeIdealOf,
    lem:primeIdealOf_fromSpec_base}
  \leanok
...
\begin{proof}
  ...
  (Theorems~\ref{thm:ker_graphSectionEval_eq_span_graphChartEqn},
  \ref{thm:ker_graphSectionEval_eq_primeIdealOf}); but \(\mathfrak p_{x_t}\) is maximal
  (Theorem~\ref{thm:finrank_quotient_ker_graphSectionEval}), forcing
  \(\mathfrak p_z = \mathfrak p_{x_t}\), hence \(z = x_t\) (points of an affine open are
  recovered from their primes, Lemma~\ref{lem:primeIdealOf_fromSpec_base}) — contradiction.
\end{proof}
...
\begin{lemma}[The trivialising element away from the graph point is \(1\)]
  ...
  \uses{def:graphLocalEquations, def:graphPoint, def:diagonalCover,
    def:localEquations_pullback, lem:localEquations_presentation}
  \leanok
...
\begin{proof}
  ...
  (Definition~\ref{def:graphLocalEquations}). At \(x \neq x_t\), the image \(q(x)\) misses
  the diagonal (Definition~\ref{def:graphPoint}), so the diagonal equation at \(q(x)\) is
  the constant \(1\) (Definition~\ref{def:diagonalCover}), whose pullback
  (Definition~\ref{def:localEquations_pullback}) is \(q^{\sharp}(1) = 1\); the presentation
  element (Lemma~\ref{lem:localEquations_presentation}) is its germ, i.e.\ \(1\).
\end{proof}
...
\begin{theorem}[The overlap identity at the graph point]
  ...
  \uses{def:graphLocalEquations, def:graphChart, thm:appLE_productChartSections,
    def:diagonalChartData, def:diagonalCover, def:diagonalChart,
    def:localEquations_pullback, lem:localEquations_presentation, def:diagonalSetup,
    def:graphPoint, lem:pointedCover_genericPoint_mem, thm:graphPoint_mem_graphChart,
    def:productChartSections}
  \leanok
...
\begin{proof}
  ...
  diagonal equation \(q^{\sharp}(e_{p_0})\) (Definitions~\ref{def:localEquations_pullback},
  \ref{def:diagonalCover}), where the diagonal equation \(e_{p_0}\) on the diagonal chart
  is the identified diagonal generator \(u \otimes 1 - 1 \otimes u\)
  (Definitions~\ref{def:diagonalChart}, \ref{def:diagonalSetup}). The lift-app rule for
  \(q = \langle\mathrm{fst}, \mathrm{snd}\circ t\rangle\)
  (Theorem~\ref{thm:appLE_productChartSections}) computes its restriction to \(W\) as
  ...
  (Definition~\ref{def:graphChart}); by the product-chart section calculus
  (Definition~\ref{def:productChartSections}), its restriction to \(W\) is
  ...
  Lemma~\ref{lem:pointedCover_genericPoint_mem} for the member,
  Theorem~\ref{thm:graphPoint_mem_graphChart} for the chart side of the overlap).
\end{proof}

\begin{theorem}[The presentation divisor of the graph is a one-point divisor]
  \label{thm:presentationDivisor_graphLocalEquations}
  \dcref{custom}
  \lean{AlgebraicGeometry.presentationDivisor_graphLocalEquations}
  \uses{lem:presentationElem_graphLocalEquations_of_ne, thm:graphPoint_ne_genericPoint,
    def:presentationDivisor, def:curveDivisor_single, def:scheme_ordZ,
    lem:localEquations_presentation, def:graphLocalEquations}
  \leanok
  ...
  \]
  a single-point divisor (Definition~\ref{def:curveDivisor_single}, legal since
  \(x_t \neq \eta\) by Theorem~\ref{thm:graphPoint_ne_genericPoint}).
\end{theorem}
\begin{proof}
  \leanok
  Compare coefficients pointwise (Definition~\ref{def:presentationDivisor}). At \(x_t\)
  both sides read \(\mathrm{ord}_{x_t}(\mathrm{elem}_{x_t})\)
  (Definition~\ref{def:scheme_ordZ}). At \(x \neq x_t\) the trivialising element is \(1\)
  (Lemma~\ref{lem:presentationElem_graphLocalEquations_of_ne}), of order zero, and the
...
\begin{theorem}[The graph class of a field point has degree one]
  ...
  \uses{thm:presentationDivisor_graphLocalEquations,
    thm:presentationElem_graphLocalEquations_val,
    thm:finrank_quotient_span_graphChartEqn, lem:isUnit_germ_graphChartEqn_of_ne,
    thm:finrank_quotient_span_section, def:classDeg, def:curveDivisor_picClass,
    lem:curveDivisor_single_arith, lem:localEquations_presentation,
    thm:graphPoint_ne_genericPoint, def:graphLocalEquations,
    thm:picClass_eq_of_presentationDivisor_eq, thm:graphPoint_mem_graphChart}
  \leanok
  \dcref{custom}
...
\begin{proof}
  ...
  (Lemma~\ref{lem:localEquations_presentation},
  Theorem~\ref{thm:picClass_eq_of_presentationDivisor_eq},
  Definition~\ref{def:curveDivisor_picClass}), which is the one-point divisor
  \(m \cdot x_t\) with \(m := \mathrm{ord}_{x_t}(\mathrm{elem}_{x_t})\)
  (Theorem~\ref{thm:presentationDivisor_graphLocalEquations}). The anchored degree of a
  one-point divisor is \(\deg(m \cdot x_t) = m \cdot [\kappa(x_t) : K]\)
  (Definition~\ref{def:classDeg}, Lemma~\ref{lem:curveDivisor_single_arith}).

  This product collapses in one shot through the chart colength dictionary
  (Theorem~\ref{thm:finrank_quotient_span_section}) on the graph chart, applied to the
  rational function \(\mathrm{elem}_{x_t}\) with section representative \(\gamma\)
  (the overlap identity, Theorem~\ref{thm:presentationElem_graphLocalEquations_val}) and
  the singleton point set \(\{x_t\}\): the hypotheses are that \(x_t\) lies in the chart
  (Theorem~\ref{thm:graphPoint_mem_graphChart}) and that \(\gamma\) is a unit at every
  other closed point of the chart (Lemma~\ref{lem:isUnit_germ_graphChartEqn_of_ne}); the
  ...
  (Theorem~\ref{thm:finrank_quotient_span_graphChartEqn}).
\end{proof}

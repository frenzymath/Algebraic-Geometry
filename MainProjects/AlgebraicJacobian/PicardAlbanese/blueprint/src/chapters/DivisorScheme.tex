...
\begin{definition}[Translated rank-one charts in degree zero]
  ...
  \uses{def:picRankOneOpen, def:rankOneAbelIso, def:pic0Functor}
  \leanok
...
\begin{proof}
  ...
  Theorem~\ref{thm:rankOneAbel_isOpenImmersion}.  Stability under composition gives the
  result.
\end{proof}
...
\begin{theorem}[Quasi-compactness of the separably closed representer]
  ...
  \uses{def:pic0_sepClosed_representableBy, def:genusAbelMap}
  \leanok
...
\begin{definition}[The separably closed Picard representation datum]
  ...
  \uses{def:pic0_sepClosed_representableBy, thm:pic0SepClosed_lft}
  \leanok
...
\begin{theorem}[The separably closed Jacobian datum]
  ...
  \uses{def:pic0SepClosedDescentInput, thm:pic0SepClosed_qc,
    def:jacobian_data}
  \leanok
...
\begin{proof}
  ...
  Definition~\ref{def:jacobian_data} using Theorem~\ref{thm:pic0SepClosed_qc}.
\end{proof}

\begin{theorem}[Finite-stage descent of the universal Picard equivalence]
  \label{thm:pic0DescentCompatibility}
  \dcref{custom}
  \uses{def:pic0SepClosedDescentInput, thm:pic0SepClosed_lfp}
  \notready
...
\begin{theorem}[A represented finite Galois stage]
  ...
  \dcref{custom}
  \uses{thm:pic0DescentCompatibility, def:pic0FiniteStageBaseChangeIso}
  \notready
...
\begin{proof}
  ...
  Theorem~\ref{thm:pic0DescentCompatibility} to a finite Galois extension and transport the
...
\begin{theorem}[Conditional orbit-affineness from projectivity]
  \label{thm:pic0FiniteStage_orbitsInAffine_of_projective}
  \dcref{custom}
  \lean{AlgebraicGeometry.pic0FiniteStageOrbitsInAffineOpen_of_isProjective}
  \uses{def:pic0FiniteGaloisAction}
  \leanok
...
\begin{theorem}[Orbit-affineness of the actual finite-stage representative]
  ...
  \uses{thm:pic0FiniteGaloisStage,
    thm:pic0FiniteStage_orbitsInAffine_of_projective}
  \notready
...
\begin{proof}
  ...
  Theorem~\ref{thm:pic0FiniteStage_orbitsInAffine_of_projective}.  What remains is a genuine
...
\begin{proof}
  \leanok
  Lemma~\ref{lem:finiteGalois_stableAffineCover} supplies stable affine charts.  Taking
...
\begin{theorem}[Effective finite-Galois descent for Picard-zero classes]
  \label{thm:pic0_finiteGalois_effectiveDescent}
  \dcref{custom}
...
\begin{theorem}[Invariant classes are equivariant maps]
  ...
  \uses{def:pic0FiniteGaloisAction,
    thm:pic0_finiteGalois_effectiveDescent}
  \leanok
...
\begin{theorem}[Conditional finite-Galois descent of Picard-zero representability]
  ...
  \uses{thm:finiteGalois_schemeDescent,
    thm:pic0_finiteGalois_effectiveDescent,
    thm:pic0_invariants_equivariant}
  \leanok
...
\begin{proof}
  ...
  Theorem~\ref{thm:finiteGalois_schemeDescent}.  Theorem~\ref{thm:pic0_invariants_equivariant}
  identifies those maps with invariant Picard-zero classes, and
  Theorem~\ref{thm:pic0_finiteGalois_effectiveDescent} identifies the latter with
...
\begin{definition}[The arbitrary-field degree-zero Picard representer]
  ...
  \dcref{custom}
  \uses{thm:pic0FiniteGaloisStage,
    thm:pic0FiniteStage_orbitsInAffine,
...
\end{definition}
\begin{proof}
  Apply Theorem~\ref{thm:pic0FiniteGaloisStage} for the finite-level represented pair and
  Theorem~\ref{thm:pic0FiniteStage_orbitsInAffine} for its orbit-affine certificate.  The
  conditional descent theorem
  \ref{thm:representableBy_of_finiteGalois_baseChange} then returns the quotient carrier and
...
\begin{definition}[Conditional Picard datum from finite-Galois descent]
  ...
  \uses{def:picRepDatum, thm:representableBy_of_finiteGalois_baseChange}
  \leanok
...
\begin{definition}[Conditional Jacobian datum from finite-Galois descent]
  ...
  \uses{def:picRepDatum_finiteGaloisDescent, def:jacobian_data}
  \leanok
...
\begin{definition}[The Jacobian datum from the chosen representation]
  ...
  \uses{def:pic0_representableBy, def:jacobian_data,
    def:jacobianData_finiteGaloisDescent}
  \notready
...
\end{definition}
\begin{proof}
  Once Definition~\ref{def:pic0_representableBy} supplies its unconditional represented pair,
  carry local finite type and quasi-compactness through the finite Galois quotient and apply
  Definition~\ref{def:jacobianData_finiteGaloisDescent}.  No second carrier, representation,

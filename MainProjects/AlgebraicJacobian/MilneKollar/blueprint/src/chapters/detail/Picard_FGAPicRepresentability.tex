...
\begin{proof}The first transformation sends the class represented by
...
\begin{proof}\uses{mk-thm:rigid_pushforward_locally_free}
...
\begin{proof}\uses{mk-thm:rigid_pushforward_locally_free, mk-lem:gamma_base_change}
...
\begin{proof}Each standard chart \(V_i \subseteq \mathbb{P}^1_k\) is affine with section
...
\begin{proof}The same statement for semilinear \(\Gamma\)-modules is the classical
...
\begin{proof}\uses{mk-thm:speiser_descent}
...
\begin{proof}Let \(x \in X\) and let \(U\) be an affine open containing the orbit of \(x\).
...
\begin{proof}This is a definition.
\end{proof}
...
\begin{proof}\uses{mk-def:rel_pic_sharp}
...
\begin{proof}\uses{mk-def:div_functor}
...
\begin{proof}\uses{mk-def:div_functor_carrier}
...
\begin{proof}\uses{mk-def:pic_et_sheaf}
...
\begin{proof}\uses{mk-thm:fga_pic_representability_etale}
...
\begin{proof}\uses{mk-def:has_pic_scheme}

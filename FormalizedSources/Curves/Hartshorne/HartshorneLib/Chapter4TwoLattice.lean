...
`H¹(C, 𝒪_C)` of a curve `C` equipped with a finite morphism `π : C ⟶ ℙ¹`.
...
* `LaurentPolynomial.moduleFinite_quotient_sup_of_exists_smul_mem`: the symmetric
  localization form, given `Module.Finite k[T;T⁻¹] N`.
* `LaurentPolynomial.moduleFinite_quotient_sup_of_exists_pow_smul_mem`: the same, with the
  power hypotheses phrased via `T 1 ^ m` and `T (-1) ^ m`.
-/
...
theorem moduleFinite_quotient_sup_of_span_eq_top {Q₀ Q₁ : Submodule k N} {s : Finset N}
  ...
  set F : Finset N := (Finset.Ioo (-(M : ℤ)) 0 ×ˢ s).image
      (fun p => (T p.1 : k[T;T⁻¹]) • p.2) with hF
  have window : ∀ x ∈ s, ∀ j : ℤ,
      (T j : k[T;T⁻¹]) • x ∈ Q₀ ⊔ Q₁ ⊔ span k (F : Set N) := by
    intro x hx j
  ...
  have hsmul : ∀ x ∈ s, ∀ f : k[T;T⁻¹],
      f • x ∈ Q₀ ⊔ Q₁ ⊔ span k (F : Set N) := by
    intro x hx f
...
theorem moduleFinite_quotient_sup_of_exists_smul_mem [Module.Finite k[T;T⁻¹] N]
  ...
  · rw [eq_top_iff, ← ht]
    ...
    have hcancel : (T (-(μ n : ℤ)) : k[T;T⁻¹]) •
        ((T (μ n : ℤ) : k[T;T⁻¹]) • n) = n := by

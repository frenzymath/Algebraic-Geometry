...
/-!
# Tangent-space substrate: dual-number points of a scheme at a point (W5-T1, scheme layer)

Scheme-level substrate for the Kleiman §5 tangent-space theorem (Wave-5 brick
T1, roadmap `AJCR.w5-av.t1`), bridging Mathlib's
...
(`Tangent/TangentDualNumbers.lean`), this identifies the dual-number points of
...
morphism of a `k`-scheme) is threaded through — done in
`Tangent/TangentStalkAlgebra.lean`.

## Provenance (W5-T1 port, cross-project)

Ported from `Algebraic-Jacobian-Challenge`,
`AlgebraicJacobian/Picard/TangentSpaceSchemePoints.lean` (sorry-free, stated
in mathlib generality); see the provenance note of
`Tangent/TangentDualNumbers.lean` and inbox `I-0495` overlap #2. The import
list is trimmed to what the declarations actually use (the source carried four
unused imports, an artefact of its own edit history).
-/

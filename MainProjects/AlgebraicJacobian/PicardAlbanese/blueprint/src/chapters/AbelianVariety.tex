...
\begin{proof}
  \leanok
  \uses{lem:slice_factorisation}
  By \ref{lem:slice_factorisation},
...
\begin{proof}
  \leanok
  \uses{lem:constant_into_affine}
  By \ref{lem:constant_into_affine} there is \(z : \Spec R \to W\), a priori not over
  ...
  \ref{lem:constant_into_affine}), so \(w \circ z = \mathrm{id}_{\Spec R}\).
\end{proof}

\section{Points and products}
\label{sec:points_products}
...
\begin{proof}
  \leanok
  \uses{lem:constant_into_affine_over, lem:point_ext, lem:slice_constancy_transfer,
    cor:sections_iso}
  ...
  point over \(K\), unique by \ref{lem:point_ext}, and conversely the underlying point of
  ...
  (\ref{cor:sections_iso}); by the relative constancy lemma
  \ref{lem:constant_into_affine_over} there is a point \(c\) of \(Z\) over \(K\) with
  \[
  ...
  \(\mathrm{pr}_2 \circ \hat p = y_0\) by \ref{lem:point_ext}; thus the second component of
  \(\hat p\) is constant with value \(y_0\), and the transfer lemma
  \ref{lem:slice_constancy_transfer}, applied with the contraction hypothesis
  ...
  \(\mathrm{pr}_2 \circ \hat p = \hat y\) by \ref{lem:point_ext}, and the transfer lemma
  \ref{lem:slice_constancy_transfer} gives
  \[
    f \circ \hat p \;=\; c .
  \]
...
\begin{proof}
  \leanok
  \uses{thm:rigidity, lem:product_integral}
  ...
  \emph{Rigidity applies.} In Theorem~\ref{thm:rigidity} take \(X = Y = A\), \(Z = B\),
  ...
  \ref{lem:product_integral}; and \(B\) is separated and locally of finite type. The
...
automatically commutes with the inversions, so Corollary~\ref{cor:pointed_hom} says: a

...
induced by $\mathcal{L}_{A} = \mathcal{L}(-\sum_{i=1}^n a_i p_i)$ and $\ca L_{A} \otimes \pi^* \ca B$ respectively.
 ...
 $\bullet$ For the term $\exp\left(-\frac12 \eta \right)$,  we use \cref{Lem:vertterm} to obtain
\begin{align*}
...
invariant.
 ...
 $\bullet$ For the exponential term, we must show that the divisor
...
(i) There are two pairs of universal curves with line bundle $(\frak C,\frak L)$ and $(\frak C', \frak L')$

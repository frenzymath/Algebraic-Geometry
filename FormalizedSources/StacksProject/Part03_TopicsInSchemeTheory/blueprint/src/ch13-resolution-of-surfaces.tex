...
(Theorem \ref{resolve-theorem-resolve}) tells us that a Noetherian
...
is reduced) and to use Lemma \ref{resolve-lemma-formally-unramified}.
However, the way our proof works, it turns out it is easier to
use Lemma \ref{resolve-lemma-normalization-completion}
to lift finiteness of the normalization over the
completion to finiteness of the normalization over $A$.

\medskip\noindent
Ad V: This is Proposition \ref{resolve-proposition-Grauert-Riemenschneider}
...
of $X/A$ (Remark \ref{resolve-remark-dualizing-setup}).
In fact, by duality the result is equivalent to a statement
(Lemma \ref{resolve-lemma-R1-injective})
...
sheaves (Lemma \ref{resolve-lemma-nontrivial-normal-bundle}).
...
Definition \ref{resolve-definition-reduce-to-rational}.
...
Lemma \ref{resolve-lemma-blow-up-normal-rational}.

\medskip\noindent
Ad D: The resolution of rational double points proceeds more or
less by hand, see
Section \ref{resolve-section-rational-double-points}.
...
on the material on formal arcs from Section \ref{resolve-section-arcs}
...
(Lemma \ref{resolve-lemma-existence-implies-existence-by-normalized-blowing-ups}).
A sequence of normalized blowups can be lifted from the completion
by Lemma \ref{resolve-lemma-normalized-blowup-completion}.
...
$A_0$ regular, see Lemma \ref{resolve-lemma-resolve-complete}.
...
See Remark \ref{resolve-remark-compare-Garel} for a discussion.
...
\begin{lemma}[Trace well defined]
\label{resolve-lemma-trace-well-defined}
\source{stacks:0ADZ}
\href{https://stacks.math.columbia.edu/tag/0ADZ}{\texttt{Stacks tag 0ADZ}}
\group{resolve}
...
commutative diagram
$$
\begin{array}{cc}R_{univ}[x]/(x^p - a) \longrightarrow\underset{\varphi_{univ}}{\downarrow} & R[x]/(x^p - a) \overset{\varphi}{\downarrow} \\ R_{univ}[y]/(y^p - b) \longrightarrow & R[y]/(y^p - b)\end{array}
$$
...
\begin{lemma}[Trace higher]
\label{resolve-lemma-trace-higher}
\source{stacks:0AX5}
\href{https://stacks.math.columbia.edu/tag/0AX5}{\texttt{Stacks tag 0AX5}}
\group{resolve}
...
\begin{proof}
\uses{resolve-lemma-trace-well-defined}

For $t = 0$ we use the composition
$$
\Omega_{S/\Lambda} \to \Omega_{S/R} \to \Omega_R \to \Omega_{R/\Lambda}
$$
where the second map is Lemma \ref{resolve-lemma-trace-well-defined}.
...
of Lemma \ref{resolve-lemma-trace-well-defined} is contained in $R\text{d}a$ we
...
\begin{remark}[Compare Garel]
\label{resolve-remark-compare-Garel}
\source{stacks:0FLF}
\href{https://stacks.math.columbia.edu/tag/0FLF}{\texttt{Stacks tag 0FLF}}
\group{resolve}

\uses{resolve-lemma-trace-higher}
Let $\mathbf{F}_p \subset \Lambda \subset R \subset S$ and $\text{Tr}$
be as in Lemma \ref{resolve-lemma-trace-higher}. By
...
\begin{lemma}[Trace extends]
\label{resolve-lemma-trace-extends}
\source{stacks:0AX6}
\href{https://stacks.math.columbia.edu/tag/0AX6}{\texttt{Stacks tag 0AX6}}
\group{resolve}
...
Lemma \ref{resolve-lemma-trace-higher}.
\end{lemma}
...
\source{stacks:0AGQ}
\href{https://stacks.math.columbia.edu/tag/0AGQ}{\texttt{Stacks tag 0AGQ}}
\group{resolve}
...
\begin{lemma}[Blowup regular]
\label{resolve-lemma-blowup-regular}
\source{stacks:0AGR}
\href{https://stacks.math.columbia.edu/tag/0AGR}{\texttt{Stacks tag 0AGR}}
\group{resolve}
...
\begin{lemma}[Blowup pic]
\label{resolve-lemma-blowup-pic}
\source{stacks:0C5G}
\href{https://stacks.math.columbia.edu/tag/0C5G}{\texttt{Stacks tag 0C5G}}
\group{resolve}
...
\begin{proof}
\uses{resolve-lemma-blowup, resolve-lemma-blowup-regular}
...
By Lemma \ref{resolve-lemma-blowup} the invertible $\mathcal{O}_X$-module
...
(and the fact that $X$ is normal by Lemma \ref{resolve-lemma-blowup-regular})
we conclude that $\mathcal{L} = \mathcal{O}_X(nE)$.
\end{proof}

\begin{lemma}[Cohomology of blowup]
\label{resolve-lemma-cohomology-of-blowup}
\source{stacks:0AGS}
\href{https://stacks.math.columbia.edu/tag/0AGS}{\texttt{Stacks tag 0AGS}}
\group{resolve}
...
By Lemma \ref{resolve-lemma-blowup} we have $E = \mathbf{P}^1_\kappa$ and
...
diagram
$$
\begin{array}{ccccc}0 \longrightarrow & \mathfrak m^{\max(0, n)} \longrightarrow\downarrow & \mathfrak m^{\max(0, n - 1)} \longrightarrow\downarrow & \mathfrak m^{\max(0, n)}/\mathfrak m^{\max(0, n - 1)} \longrightarrow\downarrow & 0 \\ 0 \longrightarrow & \Gamma(X, \mathcal{O}_X(n)) \longrightarrow & \Gamma(X, \mathcal{O}_X(n - 1)) \longrightarrow & \Gamma(E, \mathcal{O}_E(n - 1)) \longrightarrow & 0\end{array}
$$
with exact rows. In fact, the rows are exact also for $n < 0$
because in this case the groups on the right are zero.
In the proof of Lemma \ref{resolve-lemma-blowup}
...
\begin{lemma}[Blowup improve]
\label{resolve-lemma-blowup-improve}
\source{stacks:0AGT}
\href{https://stacks.math.columbia.edu/tag/0AGT}{\texttt{Stacks tag 0AGT}}
\group{resolve}
...
\begin{proof}
\uses{resolve-lemma-cohomology-of-blowup}
...
$H^1(X, I\mathcal{O}_X) = 0$ by Lemma \ref{resolve-lemma-cohomology-of-blowup}.
...
$$
\begin{array}{ccccc} &  & A/I \downarrow &  &  \\ 0 \longrightarrow & \Gamma(X, \mathcal{O}_X(-dE)/I\mathcal{O}_X) \longrightarrow & \Gamma(X, \mathcal{O}_X/I\mathcal{O}_X) \longrightarrow & \Gamma(X, \mathcal{O}_X/\mathcal{O}_X(-dE)) \longrightarrow & 0\end{array}
$$
...
\begin{lemma}[Differentials of blowup]
\label{resolve-lemma-differentials-of-blowup}
\source{stacks:0B4L}
\href{https://stacks.math.columbia.edu/tag/0B4L}{\texttt{Stacks tag 0B4L}}
\group{resolve}
...
$r : X \to \mathbf{P}^1$ be as in Lemma \ref{resolve-lemma-blowup}.
...
Lemma \ref{resolve-lemma-blowup} on an affine open of $X$ the morphism $f$
...
\begin{lemma}[Make ideal principal]
\label{resolve-lemma-make-ideal-principal}
\source{stacks:0AHH}
\href{https://stacks.math.columbia.edu/tag/0AHH}{\texttt{Stacks tag 0AHH}}
\group{resolve}
...
\begin{proof}
\uses{resolve-lemma-blowup-improve}
...
$$
\begin{array}{cc}\text{Proj}(\bigoplus\nolimits_{d \geq 0} \mathfrak m_{x_i}^d) \longrightarrow\downarrow & X' \downarrow \\ \Spec(\mathcal{O}_{X, x_i}) \longrightarrow & X\end{array}
$$
...
Lemma \ref{resolve-lemma-blowup-improve} for the ideal
...
$\text{Proj}(\bigoplus\nolimits_{d \geq 0} \mathfrak m_{x_i}^d)$.
The final assertion of Lemma \ref{resolve-lemma-blowup-improve}
...
\begin{lemma}[Dominate by blowing up in points]
\label{resolve-lemma-dominate-by-blowing-up-in-points}
\source{stacks:0AHI}
\href{https://stacks.math.columbia.edu/tag/0AHI}{\texttt{Stacks tag 0AHI}}
\group{resolve}
...
\begin{proof}
\uses{resolve-lemma-make-ideal-principal}
...
By Lemma \ref{resolve-lemma-make-ideal-principal}
...
\begin{lemma}[Extend rational map blowing up]
\label{resolve-lemma-extend-rational-map-blowing-up}
\source{stacks:0C5H}
\href{https://stacks.math.columbia.edu/tag/0C5H}{\texttt{Stacks tag 0C5H}}
\group{resolve}
...
\begin{proof}
\uses{resolve-lemma-dominate-by-blowing-up-in-points}
...
Apply Lemma \ref{resolve-lemma-dominate-by-blowing-up-in-points}
...
\begin{definition}[Normalized blowup]
\label{resolve-definition-normalized-blowup}
\source{stacks:0BBS}
\href{https://stacks.math.columbia.edu/tag/0BBS}{\texttt{Stacks tag 0BBS}}
\group{resolve}
...
\begin{lemma}[Nagata normalized blowup]
\label{resolve-lemma-Nagata-normalized-blowup}
\source{stacks:0BFT}
\href{https://stacks.math.columbia.edu/tag/0BFT}{\texttt{Stacks tag 0BFT}}
\group{resolve}

\uses{resolve-definition-normalized-blowup}
In Definition \ref{resolve-definition-normalized-blowup} if $X$ is Nagata,
...
\begin{lemma}[Dominate by normalized blowing up]
\label{resolve-lemma-dominate-by-normalized-blowing-up}
\source{stacks:0BBT}
\href{https://stacks.math.columbia.edu/tag/0BBT}{\texttt{Stacks tag 0BBT}}
\group{resolve}
...
$$
\begin{array}{ccccc}X_n \longrightarrow\downarrow & X_{n - 1} \longrightarrow & \ldots \longrightarrow & X_1 \longrightarrow & X_0 \downarrow \\ Y \longrightarrow &  &  &  & X\end{array}
$$
...
$$
\begin{array}{cc}Y' \underset{f'}{\longrightarrow}\downarrow & X' \downarrow \\ Y \overset{f}{\longrightarrow} & X\end{array}
$$
...
\begin{lemma}[Extend rational map normalized blowing up]
\label{resolve-lemma-extend-rational-map-normalized-blowing-up}
\source{stacks:0C5I}
\href{https://stacks.math.columbia.edu/tag/0C5I}{\texttt{Stacks tag 0C5I}}
\group{resolve}
...
\begin{proof}
\uses{resolve-lemma-dominate-by-normalized-blowing-up}

Applying
Divisors, Lemma \ref{divisors-lemma-extend-rational-map-after-modification}
we find a proper morphism $p : X' \to X$ which is an isomorphism
over $U$ and a morphism $f' : X' \to Y$ agreeing with $f$ over $U$.
Apply Lemma \ref{resolve-lemma-dominate-by-normalized-blowing-up}
to the morphism $p : X' \to X$. The composition $X_n \to X' \to Y$ is
the desired morphism.
\end{proof}
...
\source{stacks:0BFV}
\href{https://stacks.math.columbia.edu/tag/0BFV}{\texttt{Stacks tag 0BFV}}
\group{resolve}
...
\begin{lemma}[Equivalence properties]
\label{resolve-lemma-equivalence-properties}
\source{stacks:0BFW}
\href{https://stacks.math.columbia.edu/tag/0BFW}{\texttt{Stacks tag 0BFW}}
\group{resolve}
...
\begin{lemma}[Equivalence fibre]
\label{resolve-lemma-equivalence-fibre}
\source{stacks:0BFX}
\href{https://stacks.math.columbia.edu/tag/0BFX}{\texttt{Stacks tag 0BFX}}
\group{resolve}
...
\begin{lemma}[Equivalence sequence blowups]
\label{resolve-lemma-equivalence-sequence-blowups}
\source{stacks:0BFY}
\href{https://stacks.math.columbia.edu/tag/0BFY}{\texttt{Stacks tag 0BFY}}
\group{resolve}
...
\begin{proof}
\uses{resolve-lemma-equivalence-fibre}
...
(Lemma \ref{resolve-lemma-equivalence-fibre})
...
Lemma \ref{resolve-lemma-equivalence-sequence-blowups}
for normalized blowups.

\begin{lemma}[Equivalence sequence normalized blowups]
\label{resolve-lemma-equivalence-sequence-normalized-blowups}
\source{stacks:0BFZ}
\href{https://stacks.math.columbia.edu/tag/0BFZ}{\texttt{Stacks tag 0BFZ}}
\group{resolve}
...
\begin{proof}
\uses{resolve-lemma-equivalence-sequence-blowups}
...
\begin{definition}[Situation: Vanishing]
\label{resolve-situation-vanishing}
\source{stacks:0AX8}
\href{https://stacks.math.columbia.edu/tag/0AX8}{\texttt{Stacks tag 0AX8}}
\group{resolve}
...
\begin{lemma}[Dominate by scheme modification]
\label{resolve-lemma-dominate-by-scheme-modification}
\source{stacks:0B4M}
\href{https://stacks.math.columbia.edu/tag/0B4M}{\texttt{Stacks tag 0B4M}}
\group{resolve}

\uses{resolve-situation-vanishing}
In Situation \ref{resolve-situation-vanishing} there exists a $U$-admissible
...
\begin{lemma}[Nice meromorphic function]
\label{resolve-lemma-nice-meromorphic-function}
\source{stacks:0AX9}
\href{https://stacks.math.columbia.edu/tag/0AX9}{\texttt{Stacks tag 0AX9}}
\group{resolve}

\uses{resolve-situation-vanishing}
In Situation \ref{resolve-situation-vanishing} there exists a nonzero
...
We will use the observations made following Situation \ref{resolve-situation-vanishing}
...
\begin{lemma}[Nontrivial normal bundle]
\label{resolve-lemma-nontrivial-normal-bundle}
\source{stacks:0AXA}
\href{https://stacks.math.columbia.edu/tag/0AXA}{\texttt{Stacks tag 0AXA}}
\group{resolve}

\uses{resolve-situation-vanishing}
In Situation \ref{resolve-situation-vanishing} assume $X$ is normal.
...
\begin{proof}
\uses{resolve-situation-vanishing, resolve-lemma-nice-meromorphic-function}

We will use the observations made following Situation \ref{resolve-situation-vanishing}
without further mention. Let $f$ be a function as in
Lemma \ref{resolve-lemma-nice-meromorphic-function}.
...
\begin{lemma}[H1 injective]
\label{resolve-lemma-H1-injective}
\source{stacks:0AXB}
\href{https://stacks.math.columbia.edu/tag/0AXB}{\texttt{Stacks tag 0AXB}}
\group{resolve}
...
\begin{proof}
\uses{resolve-lemma-nontrivial-normal-bundle}
...
Picture
$$
\begin{array}{ccc} & X' \underset{g}{\searrow}\longrightarrow & P \underset{\pi}{\downarrow} \\ f^{-1}(U) \underset{\sigma'}{\nearrow}\longrightarrow &  & X \underset{\sigma}{\uparrow}\end{array}
$$
...
Lemma \ref{resolve-lemma-nontrivial-normal-bundle}
and the proof is complete.
\end{proof}

\begin{lemma}[R1 injective]
\label{resolve-lemma-R1-injective}
\source{stacks:0AXC}
\href{https://stacks.math.columbia.edu/tag/0AXC}{\texttt{Stacks tag 0AXC}}
\group{resolve}

\uses{resolve-situation-vanishing}
In Situation \ref{resolve-situation-vanishing} assume $X$ is normal and $A$ Nagata.
Then
$$
...
\begin{proof}
\uses{resolve-lemma-H1-injective}
...
$$
By Lemma \ref{resolve-lemma-H1-injective} the pullback of this extension
...
\begin{remark}[Dualizing setup]
\label{resolve-remark-dualizing-setup}
\source{stacks:0B4R}
\href{https://stacks.math.columbia.edu/tag/0B4R}{\texttt{Stacks tag 0B4R}}
\group{resolve}
...
consequence of Lemma \ref{resolve-lemma-R1-injective} and the general theory of
duality.
...
\source{stacks:0AXD}
\href{https://stacks.math.columbia.edu/tag/0AXD}{\texttt{Stacks tag 0AXD}}
\group{resolve}

\uses{resolve-situation-vanishing, resolve-remark-dualizing-setup}
In Situation \ref{resolve-situation-vanishing} assume
\begin{enumerate}
...
(Remark \ref{resolve-remark-dualizing-setup}). Then $R^1f_*\omega_X = 0$.
\end{proposition}

\begin{proof}
\uses{resolve-lemma-R1-injective}
...
$$
which is impossible by Lemma \ref{resolve-lemma-R1-injective}.
...
\source{stacks:0AXF}
\href{https://stacks.math.columbia.edu/tag/0AXF}{\texttt{Stacks tag 0AXF}}
\group{resolve}
...
$$
\begin{array}{ccc}X' \underset{f'}{\searrow}\underset{g}{\longrightarrow} &  & X \overset{f}{\swarrow} \\  & \Spec(A) & \end{array}
$$
where $f$ and $f'$ are modifications as in Situation \ref{resolve-situation-vanishing}
...
\begin{lemma}[Bound a torsion]
\label{resolve-lemma-bound-a-torsion}
\source{stacks:0AXJ}
\href{https://stacks.math.columbia.edu/tag/0AXJ}{\texttt{Stacks tag 0AXJ}}
\group{resolve}
...
$H^1(f^{-1}(U), \mathcal{O}_X)$. By Lemma \ref{resolve-lemma-H1-injective}
...
\begin{definition}[Reduce to rational]
\label{resolve-definition-reduce-to-rational}
\source{stacks:0B4N}
\href{https://stacks.math.columbia.edu/tag/0B4N}{\texttt{Stacks tag 0B4N}}
\group{resolve}
...
Lemma \ref{resolve-lemma-reduce-to-rational}. The following lemma
...
\begin{lemma}[Rational propagates]
\label{resolve-lemma-rational-propagates}
\source{stacks:0BG0}
\href{https://stacks.math.columbia.edu/tag/0BG0}{\texttt{Stacks tag 0BG0}}
\group{resolve}
...
\begin{proof}
\uses{resolve-lemma-exact-sequence}
...
By Lemma \ref{resolve-lemma-exact-sequence} we have $R^1g_*\mathcal{O}_{X'} = 0$.
Clearly this means that $H^1(Y, \mathcal{O}_Y) = 0$ as desired.
\end{proof}

\begin{lemma}[Reduce to rational]
\label{resolve-lemma-reduce-to-rational}
\source{stacks:0B4P}
\href{https://stacks.math.columbia.edu/tag/0B4P}{\texttt{Stacks tag 0B4P}}
\group{resolve}
...
\begin{proof}
\uses{resolve-lemma-exact-sequence, resolve-lemma-dominate-by-normalized-blowing-up, resolve-lemma-rational-propagates}

We choose a modification $X \to \Spec(A)$ with $X$ normal
which maximizes the length of $H^1(X, \mathcal{O}_X)$.
By Lemma \ref{resolve-lemma-exact-sequence}
...
dominates $X$, see Lemma \ref{resolve-lemma-dominate-by-normalized-blowing-up}.
For a closed point $x' \in X_n$ mapping to $x \in X$ we can apply
Lemma \ref{resolve-lemma-rational-propagates} to the ring map
$\mathcal{O}_{X, x} \to \mathcal{O}_{X_n, x'}$
to see that $\mathcal{O}_{X_n, x'}$ defines a rational singularity.
\end{proof}

\begin{lemma}[Go up separable]
\label{resolve-lemma-go-up-separable}
\source{stacks:0AXL}
\href{https://stacks.math.columbia.edu/tag/0AXL}{\texttt{Stacks tag 0AXL}}
\group{resolve}
...
\begin{proof}
\uses{resolve-lemma-bound-a-torsion}
...
$$
\begin{array}{ccc}Y' \downarrow\longrightarrow & Y \longrightarrow & \Spec(B) \downarrow \\ X' \longrightarrow &  & \Spec(A)\end{array}
$$
...
$$
\begin{array}{cc}Y \underset{\pi}{\downarrow}\underset{g}{\longrightarrow} & \Spec(B) \downarrow \\ X \overset{f}{\longrightarrow} & \Spec(A)\end{array}
$$
...
This is the content of Lemma \ref{resolve-lemma-bound-a-torsion} and
(\ref{equation-a-torsion}).
\end{proof}

\begin{lemma}[Regular rational]
\label{resolve-lemma-regular-rational}
\source{stacks:0B4Q}
\href{https://stacks.math.columbia.edu/tag/0B4Q}{\texttt{Stacks tag 0B4Q}}
\group{resolve}
...
\begin{proof}
\uses{resolve-lemma-dominate-by-blowing-up-in-points, resolve-lemma-blowup-regular, resolve-lemma-exact-sequence, resolve-lemma-cohomology-of-blowup}
...
\begin{proof}
\uses{resolve-proposition-Grauert-Riemenschneider, resolve-lemma-bound-a-torsion}
...
boundedness follows from Lemma \ref{resolve-lemma-bound-a-torsion} and
(\ref{equation-a-torsion}).
\end{proof}

\begin{lemma}[Compare differentials dualizing]
\label{resolve-lemma-compare-differentials-dualizing}
\source{stacks:0B4T}
\href{https://stacks.math.columbia.edu/tag/0B4T}{\texttt{Stacks tag 0B4T}}
\group{resolve}
...
\begin{proof}
\uses{resolve-remark-dualizing-setup, resolve-lemma-blowup-regular, resolve-lemma-differentials-of-blowup, resolve-lemma-cohomology-of-blowup}
...
Remark \ref{resolve-remark-dualizing-setup} and
Lemma \ref{resolve-lemma-blowup-regular}.
...
Lemma \ref{resolve-lemma-differentials-of-blowup}. It follows that
either
$$
...
\begin{lemma}[Go up degree p]
\label{resolve-lemma-go-up-degree-p}
\source{stacks:0B4U}
\href{https://stacks.math.columbia.edu/tag/0B4U}{\texttt{Stacks tag 0B4U}}
\group{resolve}
...
\begin{proof}
\uses{resolve-lemma-trace-extends, resolve-lemma-trace-higher, resolve-lemma-go-up-separable, resolve-lemma-dominate-by-blowing-up-in-points, resolve-lemma-compare-differentials-dualizing, resolve-lemma-bound-dualizing-implies-bound}
...
$$
of Lemma \ref{resolve-lemma-trace-extends}. By duality this determines a map
$$
...
$$
by the explicit description of the trace map in Lemma \ref{resolve-lemma-trace-higher}.

\medskip\noindent
Let $Y \to \Spec(B)$ be a normal modification. Exactly as in the proof of
Lemma \ref{resolve-lemma-go-up-separable} we can reduce to the case where $Y$
is finite over a modification $X$ of $\Spec(A)$. By
Lemma \ref{resolve-lemma-dominate-by-blowing-up-in-points} we may
even assume $X \to \Spec(A)$ is the result of a sequence of
blowing ups in closed points. Picture:
$$
\begin{array}{cc}Y \underset{\pi}{\downarrow}\underset{g}{\longrightarrow} & \Spec(B) \downarrow \\ X \overset{f}{\longrightarrow} & \Spec(A)\end{array}
$$
We may apply Lemma \ref{resolve-lemma-trace-extends} to $\pi$ and
...
Lemma \ref{resolve-lemma-compare-differentials-dualizing}
...
Lemma \ref{resolve-lemma-bound-dualizing-implies-bound}.
\end{proof}
...
\begin{definition}[Situation: Rational]
\label{resolve-situation-rational}
\source{stacks:0B4W}
\href{https://stacks.math.columbia.edu/tag/0B4W}{\texttt{Stacks tag 0B4W}}
\group{resolve}
...
\source{stacks:0B4X}
\href{https://stacks.math.columbia.edu/tag/0B4X}{\texttt{Stacks tag 0B4X}}
\group{resolve}

\uses{resolve-situation-rational}
In Situation \ref{resolve-situation-rational}.
...
\begin{lemma}[Sections powers I rational]
\label{resolve-lemma-sections-powers-I-rational}
\source{stacks:0B4Y}
\href{https://stacks.math.columbia.edu/tag/0B4Y}{\texttt{Stacks tag 0B4Y}}
\group{resolve}

\uses{resolve-situation-rational}
In Situation \ref{resolve-situation-rational} assume
...
\begin{proof}
\uses{resolve-situation-vanishing, resolve-lemma-globally-generated}

We have $H^0(X, \mathcal{O}_X) = A$, see discussion following
Situation \ref{resolve-situation-vanishing}. Then
...
Lemma \ref{resolve-lemma-globally-generated} shows that $H^1(X, \mathcal{I}^n) = 0$.
...
Lemma \ref{resolve-lemma-globally-generated}
...
\begin{lemma}[Blow up normal rational]
\label{resolve-lemma-blow-up-normal-rational}
\source{stacks:0B4Z}
\href{https://stacks.math.columbia.edu/tag/0B4Z}{\texttt{Stacks tag 0B4Z}}
\group{resolve}
...
\begin{proof}
\uses{resolve-lemma-sections-powers-I-rational}
...
$$
the last equality by Lemma \ref{resolve-lemma-sections-powers-I-rational}.
...
\begin{lemma}[Cohomology blow up rational]
\label{resolve-lemma-cohomology-blow-up-rational}
\source{stacks:0B63}
\href{https://stacks.math.columbia.edu/tag/0B63}{\texttt{Stacks tag 0B63}}
\group{resolve}
...
\begin{proof}
\uses{resolve-lemma-blow-up-normal-rational, resolve-lemma-sections-powers-I-rational}
...
By Lemma \ref{resolve-lemma-blow-up-normal-rational} the scheme $X$ is normal.
Then $E$ is Cohen-Macaulay by
Divisors, Lemma \ref{divisors-lemma-normal-effective-Cartier-divisor-S1}.
Lemma \ref{resolve-lemma-sections-powers-I-rational} applies and we obtain
...
\begin{lemma}[Dualizing rational]
\label{resolve-lemma-dualizing-rational}
\source{stacks:0BBU}
\href{https://stacks.math.columbia.edu/tag/0BBU}{\texttt{Stacks tag 0BBU}}
\group{resolve}

\uses{resolve-situation-rational}
In Situation \ref{resolve-situation-rational} assume $A$ has a
...
\begin{proof}
\uses{resolve-proposition-Grauert-Riemenschneider, resolve-lemma-bound-dualizing-implies-bound}

By Grauert-Riemenschneider
(Proposition \ref{resolve-proposition-Grauert-Riemenschneider}) we see that
...
$$
(for example see proof of Lemma \ref{resolve-lemma-bound-dualizing-implies-bound})
and since $A$ defines a rational singularity we obtain
$f_*\omega_X = \omega_A$.
\end{proof}

\begin{lemma}[Dualizing blow up rational]
\label{resolve-lemma-dualizing-blow-up-rational}
\source{stacks:0B64}
\href{https://stacks.math.columbia.edu/tag/0B64}{\texttt{Stacks tag 0B64}}
\group{resolve}

\uses{resolve-situation-rational, resolve-lemma-dualizing-rational}
In Situation \ref{resolve-situation-rational} assume $A$ has a
...
Lemma \ref{resolve-lemma-dualizing-rational} is surjective.
\end{enumerate}
\end{lemma}

\begin{proof}
\uses{resolve-lemma-cohomology-blow-up-rational, resolve-lemma-dualizing-rational}

We will use the results of Lemma \ref{resolve-lemma-cohomology-blow-up-rational}
...
in Lemma \ref{resolve-lemma-dualizing-rational} to define the map.
\end{proof}

\begin{lemma}[Rational to gorenstein]
\label{resolve-lemma-rational-to-gorenstein}
\source{stacks:0BBV}
\href{https://stacks.math.columbia.edu/tag/0BBV}{\texttt{Stacks tag 0BBV}}
\group{resolve}
...
\begin{proof}
\uses{resolve-lemma-dominate-by-normalized-blowing-up, resolve-lemma-blow-up-normal-rational, resolve-lemma-blowup-regular, resolve-lemma-dualizing-rational, resolve-lemma-dualizing-blow-up-rational}
...
By Lemma \ref{resolve-lemma-dominate-by-normalized-blowing-up}
we can choose a sequence of normalized blowups
$$
X_n \to X_{n - 1} \to \ldots \to X_1 \to \Spec(A)
$$
such that $X_n$ dominates $Y$. By Lemma \ref{resolve-lemma-blow-up-normal-rational}
...
Lemma \ref{resolve-lemma-dualizing-rational}. For each $i$ the morphism
$X_{i + 1} \to X_i$ is either a blowup of $x_i$ or an isomorphism
at $x_i$. Since $x_i$ is always a singular point, it follows from
Lemma \ref{resolve-lemma-dualizing-blow-up-rational}
...
$$
\begin{array}{cc}T \underset{a_1}{\longrightarrow}\underset{a}{\searrow} & X_1 \overset{b}{\downarrow} \\  & X\end{array}
$$
...
$$
\begin{array}{ccccc}T \underset{a}{\downarrow}\underset{a_1}{\searrow}\underset{a_2}{\searrow}\overset{a_3}{\searrow} &  &  &  &  \\ (X, p) & (X_1, p_1) \longleftarrow & (X_2, p_2) \longleftarrow & (X_3, p_3) \longleftarrow & \ldots \longleftarrow\end{array}
$$
This kind of sequence of blowups can be characterized as follows.

\begin{lemma}[Sequence blowups]
\label{resolve-lemma-sequence-blowups}
\source{stacks:0BG2}
\href{https://stacks.math.columbia.edu/tag/0BG2}{\texttt{Stacks tag 0BG2}}
\group{resolve}
...
\begin{lemma}[Sequence blowups along arc becomes nonsingular]
\label{resolve-lemma-sequence-blowups-along-arc-becomes-nonsingular}
\source{stacks:0BG3}
\href{https://stacks.math.columbia.edu/tag/0BG3}{\texttt{Stacks tag 0BG3}}
\group{resolve}
...
\begin{lemma}[Iso completions]
\label{resolve-lemma-iso-completions}
\source{stacks:0BG5}
\href{https://stacks.math.columbia.edu/tag/0BG5}{\texttt{Stacks tag 0BG5}}
\group{resolve}
...
\begin{lemma}[Port regularity to completion]
\label{resolve-lemma-port-regularity-to-completion}
\source{stacks:0BG6}
\href{https://stacks.math.columbia.edu/tag/0BG6}{\texttt{Stacks tag 0BG6}}
\group{resolve}
...
\begin{proof}
\uses{resolve-lemma-iso-completions}
...
\begin{lemma}[Descend admissible blowup]
\label{resolve-lemma-descend-admissible-blowup}
\source{stacks:0AFK}
\href{https://stacks.math.columbia.edu/tag/0AFK}{\texttt{Stacks tag 0AFK}}
\group{resolve}
...
\begin{lemma}[Blowup still good]
\label{resolve-lemma-blowup-still-good}
\source{stacks:0BG7}
\href{https://stacks.math.columbia.edu/tag/0BG7}{\texttt{Stacks tag 0BG7}}
\group{resolve}
...
\begin{proof}
\uses{resolve-lemma-dominate-by-scheme-modification, resolve-lemma-descend-admissible-blowup, resolve-lemma-exact-sequence, resolve-lemma-blow-up-normal-rational, resolve-lemma-iso-completions}
...
By Lemma \ref{resolve-lemma-dominate-by-scheme-modification}
there exists a $U^\wedge$-admissible blowup $Y' \to \Spec(A^\wedge)$
dominating $Y$. By Lemma \ref{resolve-lemma-descend-admissible-blowup}
...
Lemma \ref{resolve-lemma-exact-sequence}.
...
By Lemma \ref{resolve-lemma-blow-up-normal-rational} we see that both $Y$
...
an isomorphism on completions (Lemma \ref{resolve-lemma-iso-completions})
we conclude.
\end{proof}
...
\source{stacks:0BG8}
\href{https://stacks.math.columbia.edu/tag/0BG8}{\texttt{Stacks tag 0BG8}}
\group{resolve}
...
\begin{lemma}[Normalization completion]
\label{resolve-lemma-normalization-completion}
\source{stacks:0BG9}
\href{https://stacks.math.columbia.edu/tag/0BG9}{\texttt{Stacks tag 0BG9}}
\group{resolve}
...
\begin{lemma}[Normalized blowup completion]
\label{resolve-lemma-normalized-blowup-completion}
\source{stacks:0BGA}
\href{https://stacks.math.columbia.edu/tag/0BGA}{\texttt{Stacks tag 0BGA}}
\group{resolve}
...
\begin{proof}
\uses{resolve-lemma-normalization-completion}
...
By Lemma \ref{resolve-lemma-normalization-completion}
...
In Section \ref{resolve-section-rational-singularities}
...
\begin{definition}[Situation: Rational double point]
\label{resolve-situation-rational-double-point}
\source{stacks:0BGC}
\href{https://stacks.math.columbia.edu/tag/0BGC}{\texttt{Stacks tag 0BGC}}
\group{resolve}
...
\begin{lemma}[Issquare]
\label{resolve-lemma-issquare}
\source{stacks:0BGD}
\href{https://stacks.math.columbia.edu/tag/0BGD}{\texttt{Stacks tag 0BGD}}
\group{resolve}
...
Situation \ref{resolve-situation-rational-double-point}.
Let $X \to \Spec(A)$ be the blowing up of $\mathfrak m$ in $\Spec(A)$.
By Lemma \ref{resolve-lemma-blow-up-normal-rational} we see that $X$ is normal.
All singularities of $X$ are rational singularities
by Lemma \ref{resolve-lemma-rational-propagates}.
Since $\omega_A = A$ we see from Lemma \ref{resolve-lemma-dualizing-blow-up-rational}
that $\omega_X \cong \mathcal{O}_X$ (see discussion in
Remark \ref{resolve-remark-dualizing-setup} for conventions).
Thus all singularities of $X$ are Gorenstein.
Moreover, the local rings of $X$ at closed point have
normal completions by Lemma \ref{resolve-lemma-blowup-still-good}.
...
$\omega_E = \mathcal{O}_E(-1)$ by Lemma \ref{resolve-lemma-dualizing-blow-up-rational}.
By Lemma \ref{resolve-lemma-cohomology-blow-up-rational} we have
...
$$
by Lemma \ref{resolve-lemma-cohomology-blow-up-rational} and Riemann-Roch
on $E$.
...
The claim now follows from Lemma \ref{resolve-lemma-issquare}.
...
Lemma \ref{resolve-lemma-sequence-blowups}
...
Lemma \ref{resolve-lemma-sequence-blowups-along-arc-becomes-nonsingular}
...
local rings (Lemma \ref{resolve-lemma-iso-completions}) we conclude
...
Lemmas \ref{resolve-lemma-sequence-blowups} and
\ref{resolve-lemma-sequence-blowups-along-arc-becomes-nonsingular}
and the same reasoning as before.

\begin{lemma}[Resolve rational double points]
\label{resolve-lemma-resolve-rational-double-points}
\source{stacks:0BGE}
\href{https://stacks.math.columbia.edu/tag/0BGE}{\texttt{Stacks tag 0BGE}}
\group{resolve}
...
\begin{lemma}[Regular alteration implies]
\label{resolve-lemma-regular-alteration-implies}
\source{stacks:0BGG}
\href{https://stacks.math.columbia.edu/tag/0BGG}{\texttt{Stacks tag 0BGG}}
\group{resolve}
...
\begin{lemma}[Algebra helper]
\label{resolve-lemma-algebra-helper}
\source{stacks:0BGH}
\href{https://stacks.math.columbia.edu/tag/0BGH}{\texttt{Stacks tag 0BGH}}
\group{resolve}
...
$$
\begin{array}{cc}B \longrightarrow\downarrow & C \downarrow \\ B^\wedge \longrightarrow & C^\wedge\end{array}
$$
...
\begin{lemma}[Regular alteration implies local]
\label{resolve-lemma-regular-alteration-implies-local}
\source{stacks:0BGI}
\href{https://stacks.math.columbia.edu/tag/0BGI}{\texttt{Stacks tag 0BGI}}
\group{resolve}
...
\begin{proof}
\uses{resolve-lemma-regular-alteration-implies, resolve-lemma-algebra-helper, resolve-lemma-port-regularity-to-completion}

Parts (1) and (2) follow from Lemma \ref{resolve-lemma-regular-alteration-implies}.
...
(Properties, Lemma \ref{properties-lemma-normal-integral-sections}).
Thus $B \subset C$ and we conclude that $B$ is finite over $A$
as $A$ is Noetherian. By Lemma \ref{resolve-lemma-algebra-helper}
...
Lemma \ref{resolve-lemma-port-regularity-to-completion}
...
is regular by Lemma \ref{resolve-lemma-port-regularity-to-completion}
...
\source{stacks:0BGK}
\href{https://stacks.math.columbia.edu/tag/0BGK}{\texttt{Stacks tag 0BGK}}
\group{resolve}
...
\begin{definition}[Resolution surface]
\label{resolve-definition-resolution-surface}
\source{stacks:0BGL}
\href{https://stacks.math.columbia.edu/tag/0BGL}{\texttt{Stacks tag 0BGL}}
\group{resolve}
...
\begin{lemma}[Existence implies existence by normalized blowing ups]
\label{resolve-lemma-existence-implies-existence-by-normalized-blowing-ups}
\source{stacks:0BGM}
\href{https://stacks.math.columbia.edu/tag/0BGM}{\texttt{Stacks tag 0BGM}}
\group{resolve}
...
\begin{proof}
\uses{resolve-lemma-regular-alteration-implies-local, resolve-lemma-port-regularity-to-completion, resolve-lemma-normalized-blowup-completion, resolve-lemma-dominate-by-normalized-blowing-up, resolve-lemma-dominate-by-blowing-up-in-points, resolve-lemma-regular-rational, resolve-lemma-rational-propagates, resolve-lemma-rational-to-gorenstein, resolve-lemma-equivalence-sequence-blowups, resolve-lemma-resolve-rational-double-points}

By Lemma \ref{resolve-lemma-regular-alteration-implies-local}
the completion $A^\wedge$ of $A$ is normal.
By Lemma \ref{resolve-lemma-port-regularity-to-completion} we see
...
$$
dominating $X$ (Lemma \ref{resolve-lemma-dominate-by-normalized-blowing-up}).
The morphism $Y_n \to X$ is an isomorphism away from
finitely many points of $X$.
Hence we can apply Lemma \ref{resolve-lemma-dominate-by-blowing-up-in-points}
...
$$
\begin{array}{cccc} & Y_n \searrow\longrightarrow &  & \Spec(A) \\ X_m \longrightarrow\nearrow &  & X \nearrow & \end{array}
$$
...
(Lemma \ref{resolve-lemma-regular-rational}).
Apply Lemma \ref{resolve-lemma-rational-propagates} to
$\mathcal{O}_{X, x_a} \to \mathcal{O}_{Y_n, y_a}$
to see that $\mathcal{O}_{Y_n, y_a}$ defines a
rational singularity. By Lemma \ref{resolve-lemma-rational-to-gorenstein}
...
$$
\begin{array}{cccccc} & Y_{n + n'} \longrightarrow &  & Y_n \searrow\longrightarrow &  & \Spec(A) \\ X_{m + m'} \nearrow\longrightarrow &  & X_m \longrightarrow\nearrow &  & X \nearrow & \end{array}
$$
...
Lemma \ref{resolve-lemma-resolve-rational-double-points}.
Arguing exactly as above we conclude that the lemma is true.
\end{proof}

\begin{lemma}[Resolve complete]
\label{resolve-lemma-resolve-complete}
\source{stacks:0BGN}
\href{https://stacks.math.columbia.edu/tag/0BGN}{\texttt{Stacks tag 0BGN}}
\group{resolve}
...
$$
\begin{array}{cc}X \longrightarrow\downarrow & \Spec(A) \downarrow \\ Y \longrightarrow & \Spec(B)\end{array}
$$
...
Lemma \ref{resolve-lemma-port-regularity-to-completion}.
By Lemma \ref{resolve-lemma-equivalence-sequence-normalized-blowups}
...
Then either Lemma \ref{resolve-lemma-go-up-separable} or \ref{resolve-lemma-go-up-degree-p}
implies that reduction to rational singularities is possible.
By Lemma \ref{resolve-lemma-reduce-to-rational} we conclude that there exists a
...
By Lemma \ref{resolve-lemma-rational-to-gorenstein}
there exists a finite sequence of blowups in singular closed points
$$
...
\source{stacks:0BGP}
\href{https://stacks.math.columbia.edu/tag/0BGP}{\texttt{Stacks tag 0BGP}}
\group{resolve}
...
\begin{proof}
\uses{resolve-lemma-regular-alteration-implies, resolve-lemma-regular-alteration-implies-local, resolve-lemma-resolve-complete, resolve-lemma-existence-implies-existence-by-normalized-blowing-ups, resolve-lemma-normalized-blowup-completion, resolve-lemma-port-regularity-to-completion, resolve-lemma-equivalence-sequence-normalized-blowups}

The implications (3) $\Rightarrow$ (2) $\Rightarrow$ (1) are immediate.

\medskip\noindent
Let $X \to Y$ be an alteration with $X$ regular. Then $Y^\nu \to Y$
is finite by Lemma \ref{resolve-lemma-regular-alteration-implies}.
...
Lemmas \ref{resolve-lemma-resolve-complete} and
...
Lemma \ref{resolve-lemma-normalized-blowup-completion}
there is a corresponding sequence of normalized blowing ups
$$
...
\begin{lemma}[Resolve curve]
\label{resolve-lemma-resolve-curve}
\source{stacks:0BI4}
\href{https://stacks.math.columbia.edu/tag/0BI4}{\texttt{Stacks tag 0BI4}}
\group{resolve}
...
\begin{proof}
\uses{resolve-lemma-regular-alteration-implies}

The implications (3) $\Rightarrow$ (2) $\Rightarrow$ (1) are immediate.
The implication (1) $\Rightarrow$ (4) follows from
Lemma \ref{resolve-lemma-regular-alteration-implies}.
...
\begin{lemma}[Blowup curve]
\label{resolve-lemma-blowup-curve}
\source{stacks:0BI5}
\href{https://stacks.math.columbia.edu/tag/0BI5}{\texttt{Stacks tag 0BI5}}
\group{resolve}

\uses{resolve-lemma-resolve-curve}
...
Lemma \ref{resolve-lemma-resolve-curve}. Then there exists a finite sequence
$$
...
blowups given to us by Lemma \ref{resolve-lemma-resolve-curve}. Let
...
\begin{lemma}[Blowup nonsingular curves meeting at point]
\label{resolve-lemma-blowup-nonsingular-curves-meeting-at-point}
\source{stacks:0BI7}
\href{https://stacks.math.columbia.edu/tag/0BI7}{\texttt{Stacks tag 0BI7}}
\group{resolve}
...
\begin{lemma}[Blowup curves]
\label{resolve-lemma-blowup-curves}
\source{stacks:0BI8}
\href{https://stacks.math.columbia.edu/tag/0BI8}{\texttt{Stacks tag 0BI8}}
\group{resolve}
...
equivalent conditions of Lemma \ref{resolve-lemma-resolve-curve}. Then there
...
\begin{proof}
\uses{resolve-lemma-blowup-curve, resolve-lemma-blowup-nonsingular-curves-meeting-at-point}

It follows from Lemma \ref{resolve-lemma-blowup-curve} that we may assume $Y_i$
...
it (Lemma \ref{resolve-lemma-blowup-nonsingular-curves-meeting-at-point})
...
\begin{lemma}[Turn into effective Cartier]
\label{resolve-lemma-turn-into-effective-Cartier}
\source{stacks:0BIB}
\href{https://stacks.math.columbia.edu/tag/0BIB}{\texttt{Stacks tag 0BIB}}
\group{resolve}
...
Lemma \ref{resolve-lemma-resolve-curve}. Then there exists a sequence
$$
X_n \to \ldots \to X_1 \to X
$$
...
\begin{proof}
\uses{resolve-lemma-blowup, resolve-lemma-turn-into-effective-Cartier, resolve-lemma-blowup-curves, resolve-lemma-blowup-nonsingular-curves-meeting-at-point}
...
(Lemma \ref{resolve-lemma-blowup}) and the strict transform $Y'$ of each irreducible
...
Lemmas \ref{resolve-lemma-turn-into-effective-Cartier} and \ref{resolve-lemma-blowup-curves}
...
kind. This follows from Lemma \ref{resolve-lemma-blowup}.
...
\begin{lemma}[Factor through contraction]
\label{resolve-lemma-factor-through-contraction}
\source{stacks:0C5J}
\href{https://stacks.math.columbia.edu/tag/0C5J}{\texttt{Stacks tag 0C5J}}
\group{resolve}
...
\begin{lemma}[Contraction unique]
\label{resolve-lemma-contraction-unique}
\source{stacks:0C5K}
\href{https://stacks.math.columbia.edu/tag/0C5K}{\texttt{Stacks tag 0C5K}}
\group{resolve}
...
\begin{proof}
\uses{resolve-lemma-factor-through-contraction}

This is immediate from the universal property of
Lemma \ref{resolve-lemma-factor-through-contraction}.
\end{proof}

\begin{lemma}[Exceptional first kind local]
\label{resolve-lemma-exceptional-first-kind-local}
\source{stacks:0C2K}
\href{https://stacks.math.columbia.edu/tag/0C2K}{\texttt{Stacks tag 0C2K}}
\group{resolve}
...
\source{stacks:0C2L}
\href{https://stacks.math.columbia.edu/tag/0C2L}{\texttt{Stacks tag 0C2L}}
\group{resolve}
...
\begin{proof}
\uses{resolve-lemma-exceptional-first-kind-local, resolve-lemma-blowup}
...
Let $E_n = nE$ as in Lemma \ref{resolve-lemma-exceptional-first-kind-local}.
Observe that
$$
...
Lemma \ref{resolve-lemma-exceptional-first-kind-local}.
...
First, we note that Lemma \ref{resolve-lemma-exceptional-first-kind-local}
...
curve of the first kind by Lemma \ref{resolve-lemma-blowup}.
...
\begin{lemma}[Pic blowup]
\label{resolve-lemma-pic-blowup}
\source{stacks:0C5L}
\href{https://stacks.math.columbia.edu/tag/0C5L}{\texttt{Stacks tag 0C5L}}
\group{resolve}
...
\begin{proof}
\uses{resolve-lemma-cohomology-of-blowup, resolve-lemma-blowup-pic}
...
is an isomorphism. This follows from Lemma \ref{resolve-lemma-cohomology-of-blowup}
part (4).
...
the Picard group in Lemma \ref{resolve-lemma-blowup-pic}. Thus
the claim is proved.
...
\begin{remark}[Pic blowup]
\label{resolve-remark-pic-blowup}
\source{stacks:0C5M}
\href{https://stacks.math.columbia.edu/tag/0C5M}{\texttt{Stacks tag 0C5M}}
\group{resolve}

\uses{resolve-lemma-pic-blowup}
Let $b : X \to X'$ be the contraction of an
exceptional curve of the first kind $E \subset X$.
From Lemma \ref{resolve-lemma-pic-blowup} we obtain an identification
$$
...
Lemma \ref{resolve-lemma-pic-blowup} shows that $\mathcal{L}' = b_*\mathcal{L}(nE)$.
Of course the assignment $\mathcal{L} \mapsto \mathcal{L}'$ is
a group homomorphism.
\end{remark}

\begin{lemma}[Lift sections and h1]
\label{resolve-lemma-lift-sections-and-h1}
\source{stacks:0C2J}
\href{https://stacks.math.columbia.edu/tag/0C2J}{\texttt{Stacks tag 0C2J}}
\group{resolve}
...
\begin{lemma}[Contract ample]
\label{resolve-lemma-contract-ample}
\source{stacks:0C2M}
\href{https://stacks.math.columbia.edu/tag/0C2M}{\texttt{Stacks tag 0C2M}}
\group{resolve}

\uses{resolve-remark-pic-blowup}
...
is ample with $\mathcal{L}'$ as in Remark \ref{resolve-remark-pic-blowup}.
\end{enumerate}
\end{lemma}

\begin{proof}
\uses{resolve-lemma-lift-sections-and-h1, resolve-lemma-contraction}

Let $n$ be the degree of $\mathcal{L}|_E$ as in
Lemma \ref{resolve-lemma-lift-sections-and-h1}.
...
(by Lemma \ref{resolve-lemma-lift-sections-and-h1} and the fact that $n > 0$)
...
\medskip\noindent
Below we will check the conditions of Lemma \ref{resolve-lemma-contraction}.
...
$$
of $\psi|_U$ where $\text{pr}$ is the projection using the first
...
\begin{lemma}[Contract when quasi projective]
\label{resolve-lemma-contract-when-quasi-projective}
\source{stacks:0C2N}
\href{https://stacks.math.columbia.edu/tag/0C2N}{\texttt{Stacks tag 0C2N}}
\group{resolve}
...
\begin{proof}
\uses{resolve-lemma-contract-ample}

Both cases follow from Lemma \ref{resolve-lemma-contract-ample}
...
By Lemma \ref{resolve-lemma-contract-ample} there exists a contraction
...
\begin{lemma}[Regular dim 2 quasi projective]
\label{resolve-lemma-regular-dim-2-quasi-projective}
\source{stacks:0C5N}
\href{https://stacks.math.columbia.edu/tag/0C5N}{\texttt{Stacks tag 0C5N}}
\group{resolve}
...
\begin{proof}
\uses{resolve-lemma-extend-rational-map-blowing-up, resolve-lemma-contract-when-quasi-projective, resolve-lemma-contraction-unique}
...
By Lemma \ref{resolve-lemma-extend-rational-map-blowing-up}
...
By Lemma \ref{resolve-lemma-contract-when-quasi-projective}
(and uniqueness of contractions Lemma \ref{resolve-lemma-contraction-unique})
we conclude  that $X_{n - 1}, \ldots, X_0 = X$ are quasi-projective over $S$
as desired.
\end{proof}

\begin{lemma}[Regular dim 2 projective]
\label{resolve-lemma-regular-dim-2-projective}
\source{stacks:0C5P}
\href{https://stacks.math.columbia.edu/tag/0C5P}{\texttt{Stacks tag 0C5P}}
\group{resolve}
...
\begin{proof}
\uses{resolve-lemma-regular-dim-2-quasi-projective}

This follows from
Lemma \ref{resolve-lemma-regular-dim-2-quasi-projective} and
Morphisms, Lemma \ref{morphisms-lemma-projective-is-quasi-projective-proper}.
\end{proof}
...
\begin{lemma}[Proper birational regular surfaces]
\label{resolve-lemma-proper-birational-regular-surfaces}
\source{stacks:0C5R}
\href{https://stacks.math.columbia.edu/tag/0C5R}{\texttt{Stacks tag 0C5R}}
\group{resolve}
...
\begin{proof}
\uses{resolve-lemma-extend-rational-map-blowing-up, resolve-lemma-factor-through-contraction}
...
desired factorization by Lemma \ref{resolve-lemma-factor-through-contraction}.
We will prove that
...
Picture
$$
\begin{array}{ccc}E \longrightarrow\underset{g}{\downarrow} & X' \underset{f'}{\downarrow}\longrightarrow & X \overset{f}{\downarrow} \\ E' \longrightarrow & Y' \longrightarrow & Y\end{array}
$$
...
\begin{lemma}[Birational regular surfaces]
\label{resolve-lemma-birational-regular-surfaces}
\source{stacks:0C5S}
\href{https://stacks.math.columbia.edu/tag/0C5S}{\texttt{Stacks tag 0C5S}}
\group{resolve}
...
\begin{proof}
\uses{resolve-lemma-extend-rational-map-blowing-up, resolve-lemma-proper-birational-regular-surfaces}
...
of contractions by Lemma \ref{resolve-lemma-proper-birational-regular-surfaces}.

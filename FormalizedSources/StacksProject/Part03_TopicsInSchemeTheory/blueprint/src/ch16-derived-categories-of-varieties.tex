...
\begin{lemma}[Serre functor exists]
\label{equiv-lemma-Serre-functor-exists}
\source{stacks:0FY4}
\href{https://stacks.math.columbia.edu/tag/0FY4}{\texttt{Stacks tag 0FY4}}
\group{equiv}
...
\begin{definition}[Serre functor]
\label{equiv-definition-Serre-functor}
\source{stacks:0FY5}
\href{https://stacks.math.columbia.edu/tag/0FY5}{\texttt{Stacks tag 0FY5}}
\group{equiv}

\uses{equiv-lemma-Serre-functor-exists}
Let $k$ be a field. Let $\mathcal{T}$ be a $k$-linear
triangulated category such that $\dim_k \Hom_\mathcal{T}(X, Y) < \infty$
for all $X, Y \in \Ob(\mathcal{T})$. We say {\it a Serre functor
exists} if the equivalent conditions of Lemma \ref{equiv-lemma-Serre-functor-exists}
...
\begin{remark}[Matress]
\label{equiv-remark-matress}
\source{stacks:0H8B}
\href{https://stacks.math.columbia.edu/tag/0H8B}{\texttt{Stacks tag 0H8B}}
\group{equiv}

Let $X^0 \to X^1 \to X^2 \to X^0[1]$ and $Y^0 \to Y^1 \to Y^2 \to Y^0[1]$
be distinguished triangles in a triangulated category. For
$p \in \mathbf{Z}$ write $p = 3n + i$ with $i \in \{0, 1, 2\}$ and set
we set $X^p = X^i[n]$. Simlarly for $Y^q$. Consider the double complex
with terms
$$
K^{p, q} = \Hom(X^{-p}, Y^q)
$$
The differential $d_1 : K^{p, q} \to K^{p + 1, q}$ is given by the map
$X^{-p - 1} \to X^{-p}$ (equal to the corresponding map
in the first distinguished triangle up the a shift)
and the differential $d_2 : K^{p, q} \to K^{p, q + 1}$ likewise
by the map $Y^q \to Y^{q + 1}$. From
Derived Categories, Lemma \ref{derived-lemma-representable-homological}
we see that the rows and columns of this double complex are exact.
Furthermore, we see that $K^{p, q} = K^{p + 3, q - 3}$ and these
equalities are compatible with the differentials. Finally, axiom TR3
implies one additional property: given
$\alpha \in K^{p, q}$ and $\beta \in K^{p - 1, q + 1}$ such
that $d_2 \alpha = d_1 \beta$, there exists a $\gamma \in K^{p - 2, q + 2}$
such that
$d_1 \gamma = d_2 \beta $ in $K^{p - 1, q + 2}$ and
$d_2 \gamma = d_1 \alpha $ in $K^{p - 2, q + 3} = K^{p + 1, q}$.
(Hint: for $p = q = 0$ this is exactly the statement of TR3
and for other indices prove it by shifting.)
A double complex with these properties is called a {\it matress}
(see \cite{Bondal-Kapranov}).
\end{remark}

\begin{remark}[Dual matress]
\label{equiv-remark-dual-matress}
\source{stacks:0H8C}
\href{https://stacks.math.columbia.edu/tag/0H8C}{\texttt{Stacks tag 0H8C}}
\group{equiv}

\uses{equiv-remark-matress}
Let $k$ be a field and let $K^{\bullet, \bullet}$ be a double complex
of finite dimensional $k$-vector spaces which is also a matress as in
Remark \ref{equiv-remark-matress}. We claim that the dual double complex
$L^{p, q} = \Hom_k(K^{-q, -p}, k)$ is also a matress. The exactness of
rows and columns and the 3-periodicity are immediate. To see the
additional condition holds, consider the linear map
$$
\partial :
...
$$
sending $(\alpha, \beta, \gamma)$ to
$(d_2 \alpha - d_1 \beta, d_2 \gamma - d_1 \alpha, d_1 \gamma - d_2 \beta)$
with identifications as in Remark \ref{equiv-remark-matress}.
The condition of being a matress is that
$$
\Im(\partial) \cap
...
$$
Denoting ${}^\wedge$ the dual of a vector space or map, taking duals
we get
$$
\Ker(\partial^\wedge) +
...
$$
where the $\text{pr}$ term indicates projection. This implies
that
$\Im(\partial^\wedge) \cap (L^{-p, -q} \oplus L^{-p + 1, -q - 1} \oplus 0)$
is equal to
$\Im(\partial^\wedge|_{L^{-p, -q - 1} \oplus 0 \oplus 0})$ and this
translates into the matress condition for $L^{\bullet, \bullet}$.
Some details omitted.
\end{remark}

\begin{lemma}[Serre functor]
\label{equiv-lemma-Serre-functor}
\source{stacks:0FY6}
\href{https://stacks.math.columbia.edu/tag/0FY6}{\texttt{Stacks tag 0FY6}}
\group{equiv}

\uses{equiv-definition-Serre-functor}
In the situation of Definition \ref{equiv-definition-Serre-functor}.
...
\begin{proof}
\uses{equiv-remark-matress, equiv-remark-dual-matress}
...
$$
\begin{array}{ccccc}S(C)[-1] \underset{S(h[-1])}{\longrightarrow} & S(A) \underset{S(f)}{\longrightarrow} & S(B) \underset{S(g)}{\longrightarrow} & S(C) \underset{S(h)}{\longrightarrow} & S(A)[1] \\ S(C)[-1] \overset{S(h[-1])}{\longrightarrow}\uparrow & S(A) \overset{i}{\longrightarrow}\uparrow & E \overset{p}{\longrightarrow}\overset{\varphi}{\uparrow} & S(C) \overset{S(h)}{\longrightarrow}\uparrow & S(A)[1] \uparrow\end{array}
$$
...
$$
\begin{array}{ccccc}\Hom(X, S(C)[-1]) \longrightarrow & \Hom(X, S(A)) \longrightarrow & \Hom(X, S(B)) \longrightarrow & \Hom(X, S(C)) \longrightarrow & \Hom(X, S(A)[1]) \\ \Hom(X, S(C)[-1]) \longrightarrow\uparrow & \Hom(X, S(A)) \longrightarrow\uparrow & \Hom(X, E) \longrightarrow\overset{\varphi}{\uparrow} & \Hom(X, S(C)) \longrightarrow\uparrow & \Hom(X, S(A)[1]) \uparrow\end{array}
$$
...
\medskip\noindent
To find $\varphi$, let us form the matress of Remark \ref{equiv-remark-matress}
from the distinguished triangles
$A \to B \to C \to A[1]$ and $S(A) \to E \to S(C) \to S(A)[1]$.
By Remark \ref{equiv-remark-dual-matress} the $k$-linear dual
...
\begin{lemma}[Serre functor Gorenstein proper]
\label{equiv-lemma-Serre-functor-Gorenstein-proper}
\source{stacks:0FY8}
\href{https://stacks.math.columbia.edu/tag/0FY8}{\texttt{Stacks tag 0FY8}}
\group{equiv}
...
\begin{lemma}[Perfect for R]
\label{equiv-lemma-perfect-for-R}
\source{stacks:0FYA}
\href{https://stacks.math.columbia.edu/tag/0FYA}{\texttt{Stacks tag 0FYA}}
\group{equiv}
...
\begin{lemma}[Coherent on projective space]
\label{equiv-lemma-coherent-on-projective-space}
\source{stacks:0FYB}
\href{https://stacks.math.columbia.edu/tag/0FYB}{\texttt{Stacks tag 0FYB}}
\group{equiv}
...
\begin{proof}
\uses{equiv-lemma-perfect-for-R}
...
\medskip\noindent
Assume (4). Let $R$ be as in Lemma \ref{equiv-lemma-perfect-for-R}.
...
Lemma \ref{equiv-lemma-perfect-for-R} and hence is a compact object of $D(R)$.
...
\source{stacks:0FYC}
\href{https://stacks.math.columbia.edu/tag/0FYC}{\texttt{Stacks tag 0FYC}}
\group{equiv}
...
\begin{lemma}[Characterize dbcoh projective]
\label{equiv-lemma-characterize-dbcoh-projective}
\source{stacks:0FYD}
\href{https://stacks.math.columbia.edu/tag/0FYD}{\texttt{Stacks tag 0FYD}}
\group{equiv}
...
\begin{proof}
\uses{equiv-lemma-finiteness, equiv-lemma-coherent-on-projective-space}

The implication (1) $\Rightarrow$ (2) follows from
Lemma \ref{equiv-lemma-finiteness}.
...
We conclude by Lemma \ref{equiv-lemma-coherent-on-projective-space}.
\end{proof}
...
\begin{lemma}[Maps from compact filtered]
\label{equiv-lemma-maps-from-compact-filtered}
\source{stacks:0FYF}
\href{https://stacks.math.columbia.edu/tag/0FYF}{\texttt{Stacks tag 0FYF}}
\group{equiv}
...
$$
\begin{array}{ccc}E \underset{a - b}{\longrightarrow}\downarrow & E' \downarrow\longrightarrow & E'' \downarrow \\ 0 \longrightarrow & X \longrightarrow & X\end{array}
$$
and we find that the resulting arrow $(E' \to X) \to (E'' \to X)$
equalizes $a$ and $b$.
\end{proof}

\begin{lemma}[Van den bergh]
\label{equiv-lemma-van-den-bergh}
\source{stacks:0FYG}
\href{https://stacks.math.columbia.edu/tag/0FYG}{\texttt{Stacks tag 0FYG}}
\group{equiv}
...
\begin{proof}
\uses{equiv-lemma-maps-from-compact-filtered}
...
$$
The colimit is filtered by Lemma \ref{equiv-lemma-maps-from-compact-filtered}.
...
\source{stacks:0FYH}
\href{https://stacks.math.columbia.edu/tag/0FYH}{\texttt{Stacks tag 0FYH}}
\group{equiv}
...
\begin{proof}
\uses{equiv-lemma-van-den-bergh, equiv-lemma-characterize-dbcoh-projective}
...
By Lemma \ref{equiv-lemma-van-den-bergh} we may assume
$F(E) = \Hom_X(E, K)$ for some $K \in \Ob(D_\QCoh(\mathcal{O}_X))$.
Then it follows that $K$ is in $D^b_{\textit{Coh}}(\mathcal{O}_X)$
by Lemma \ref{equiv-lemma-characterize-dbcoh-projective}.
\end{proof}

\begin{lemma}[Homological representable]
\label{equiv-lemma-homological-representable}
\source{stacks:0H4A}
\href{https://stacks.math.columbia.edu/tag/0H4A}{\texttt{Stacks tag 0H4A}}
\group{equiv}
...
\begin{proof}
\uses{equiv-theorem-bondal-van-den-bergh}
...
Hence we may apply Theorem \ref{equiv-theorem-bondal-van-den-bergh}
...
\begin{lemma}[Always right adjoints]
\label{equiv-lemma-always-right-adjoints}
\source{stacks:0FYN}
\href{https://stacks.math.columbia.edu/tag/0FYN}{\texttt{Stacks tag 0FYN}}
\group{equiv}
...
\begin{proof}
\uses{equiv-theorem-bondal-van-den-bergh, equiv-lemma-finiteness, equiv-lemma-homological-representable}
...
Hence by Theorem \ref{equiv-theorem-bondal-van-den-bergh}
it suffices to show that
$$
\sum\nolimits_{i \in \mathbf{Z}} \dim_k \Ext^i_Y(F(K), M) < \infty
$$
This follows from Lemma \ref{equiv-lemma-finiteness}.

\medskip\noindent
For the existence of the left adjoint we argue in the same
manner using Lemma \ref{equiv-lemma-homological-representable}
in stead of Theorem \ref{equiv-theorem-bondal-van-den-bergh}.
\end{proof}
...
\begin{definition}[Fourier mukai functor]
\label{equiv-definition-fourier-mukai-functor}
\source{stacks:0FYQ}
\href{https://stacks.math.columbia.edu/tag/0FYQ}{\texttt{Stacks tag 0FYQ}}
\group{equiv}
...
\begin{lemma}[Fourier Mukai QCoh]
\label{equiv-lemma-fourier-Mukai-QCoh}
\source{stacks:0FYR}
\href{https://stacks.math.columbia.edu/tag/0FYR}{\texttt{Stacks tag 0FYR}}
\group{equiv}
...
\begin{lemma}[Compose fourier mukai]
\label{equiv-lemma-compose-fourier-mukai}
\source{stacks:0FYS}
\href{https://stacks.math.columbia.edu/tag/0FYS}{\texttt{Stacks tag 0FYS}}
\group{equiv}
...
\begin{proof}
\uses{equiv-lemma-fourier-Mukai-QCoh}

The statement makes sense by Lemma \ref{equiv-lemma-fourier-Mukai-QCoh}.
We are going to use
Derived Categories of Schemes, Lemmas
\ref{perfect-lemma-quasi-coherence-pullback},
...
$$
\begin{array}{cc}X \times_S Y \times_S Z \downarrow\longrightarrow & Y \times_S Z \overset{p^{YZ}_Y}{\downarrow} \\ X \times_S Y \overset{p^{XY}_Y}{\longrightarrow} & Y\end{array}
$$
...
\begin{lemma}[Fourier mukai]
\label{equiv-lemma-fourier-mukai}
\source{stacks:0FYT}
\href{https://stacks.math.columbia.edu/tag/0FYT}{\texttt{Stacks tag 0FYT}}
\group{equiv}
...
\begin{lemma}[Fourier mukai Coh]
\label{equiv-lemma-fourier-mukai-Coh}
\source{stacks:0FYU}
\href{https://stacks.math.columbia.edu/tag/0FYU}{\texttt{Stacks tag 0FYU}}
\group{equiv}
...
\begin{example}[Diagonal fourier mukai]
\label{equiv-example-diagonal-fourier-mukai}
\source{stacks:0FYV}
\href{https://stacks.math.columbia.edu/tag/0FYV}{\texttt{Stacks tag 0FYV}}
\group{equiv}
...
\begin{lemma}[Fourier mukai right adjoint]
\label{equiv-lemma-fourier-mukai-right-adjoint}
\source{stacks:0FYW}
\href{https://stacks.math.columbia.edu/tag/0FYW}{\texttt{Stacks tag 0FYW}}
\group{equiv}
...
By Lemma \ref{equiv-lemma-fourier-Mukai-QCoh} we obtain a functor $\Phi$
...
\begin{lemma}[Fourier mukai left adjoint]
\label{equiv-lemma-fourier-mukai-left-adjoint}
\source{stacks:0FYX}
\href{https://stacks.math.columbia.edu/tag/0FYX}{\texttt{Stacks tag 0FYX}}
\group{equiv}
...
\begin{proof}
\uses{equiv-lemma-fourier-Mukai-QCoh, equiv-lemma-fourier-mukai-right-adjoint}
...
then these are as in Lemma \ref{equiv-lemma-fourier-mukai-right-adjoint}
and we get the result. It is clear that $K'$ is perfect as a
tensor product of perfect objects so that the discussion in
Lemma \ref{equiv-lemma-fourier-mukai-right-adjoint} applies to it.
To show that the procedure of
Lemma \ref{equiv-lemma-fourier-mukai-right-adjoint} applied to $K'$
...
Lemma \ref{equiv-lemma-fourier-mukai-right-adjoint} produces a map
$$
\Hom_Y(N, \Phi(M)) \longrightarrow \Hom_X(\Phi'(N), M)
$$
functorial in $M$ in $D_\QCoh(\mathcal{O}_X)$ and $N$ in
...
\begin{lemma}[Fourier mukai flat proper over noetherian]
\label{equiv-lemma-fourier-mukai-flat-proper-over-noetherian}
\source{stacks:0FYY}
\href{https://stacks.math.columbia.edu/tag/0FYY}{\texttt{Stacks tag 0FYY}}
\group{equiv}

\uses{equiv-lemma-compose-fourier-mukai}
...
computed as in Lemma \ref{equiv-lemma-compose-fourier-mukai},
...
\begin{proof}
\uses{equiv-lemma-fourier-mukai, equiv-lemma-compose-fourier-mukai, equiv-lemma-fourier-mukai-right-adjoint, equiv-lemma-fourier-mukai-left-adjoint}

Part (1) is immediate from Lemma \ref{equiv-lemma-fourier-mukai} part (4).

\medskip\noindent
Part (2) follows from Lemma \ref{equiv-lemma-compose-fourier-mukai} and the
fact that
$K'' = R\text{pr}_{13, *}(
L\text{pr}_{12}^*K
\otimes_{\mathcal{O}_{X \times_S Y \times_S Z}}^\mathbf{L}
...
sheaves follows from Lemma \ref{equiv-lemma-fourier-mukai-right-adjoint} with
...
Lemma \ref{equiv-lemma-fourier-mukai-left-adjoint} instead of
...
\begin{lemma}[On product]
\label{equiv-lemma-on-product}
\source{stacks:0FZ0}
\href{https://stacks.math.columbia.edu/tag/0FZ0}{\texttt{Stacks tag 0FZ0}}
\group{equiv}
...
\begin{lemma}[On product general]
\label{equiv-lemma-on-product-general}
\source{stacks:0FZ1}
\href{https://stacks.math.columbia.edu/tag/0FZ1}{\texttt{Stacks tag 0FZ1}}
\group{equiv}
...
\begin{proof}
\uses{equiv-lemma-on-product}

Follows from Lemma \ref{equiv-lemma-on-product} by a limit argument.
...
Then we apply Lemma \ref{equiv-lemma-on-product}
to $\mathcal{F}_{ij}$ and we pullback.
\end{proof}

\begin{lemma}[Diagonal resolution]
\label{equiv-lemma-diagonal-resolution}
\source{stacks:0FZ2}
\href{https://stacks.math.columbia.edu/tag/0FZ2}{\texttt{Stacks tag 0FZ2}}
\group{equiv}
...
Thus the lemma follows immediately from Lemma \ref{equiv-lemma-on-product}.
\end{proof}

\begin{lemma}[Ext 0 regular]
\label{equiv-lemma-Ext-0-regular}
\source{stacks:0FZ3}
\href{https://stacks.math.columbia.edu/tag/0FZ3}{\texttt{Stacks tag 0FZ3}}
\group{equiv}
...
\begin{lemma}[Split complex regular]
\label{equiv-lemma-split-complex-regular}
\source{stacks:0FZ4}
\href{https://stacks.math.columbia.edu/tag/0FZ4}{\texttt{Stacks tag 0FZ4}}
\group{equiv}
...
\begin{proof}
\uses{equiv-lemma-Ext-0-regular}
...
This follows from Lemma \ref{equiv-lemma-Ext-0-regular}.
\end{proof}

\begin{lemma}[Diagonal trick]
\label{equiv-lemma-diagonal-trick}
\source{stacks:0FZ5}
\href{https://stacks.math.columbia.edu/tag/0FZ5}{\texttt{Stacks tag 0FZ5}}
\group{equiv}
...
\begin{proof}
\uses{equiv-lemma-diagonal-resolution, equiv-lemma-split-complex-regular}
...
Hence we may choose a resolution as in Lemma \ref{equiv-lemma-diagonal-resolution}.
...
Hence by Lemma \ref{equiv-lemma-split-complex-regular} we see that
...
\begin{lemma}[Smooth proper strong generator]
\label{equiv-lemma-smooth-proper-strong-generator}
\source{stacks:0FZ6}
\href{https://stacks.math.columbia.edu/tag/0FZ6}{\texttt{Stacks tag 0FZ6}}
\group{equiv}
...
\begin{proof}
\uses{equiv-lemma-diagonal-trick, equiv-example-diagonal-fourier-mukai}

Using Lemma \ref{equiv-lemma-diagonal-trick} choose finite locally free
...
Example \ref{equiv-example-diagonal-fourier-mukai}.
...
\begin{lemma}[Diagonal trick proper]
\label{equiv-lemma-diagonal-trick-proper}
\source{stacks:0FZ7}
\href{https://stacks.math.columbia.edu/tag/0FZ7}{\texttt{Stacks tag 0FZ7}}
\group{equiv}
...
\begin{proof}
\uses{equiv-lemma-smooth-proper-strong-generator}

In the proof of Lemma \ref{equiv-lemma-smooth-proper-strong-generator}
...
\source{stacks:0FZ8}
\href{https://stacks.math.columbia.edu/tag/0FZ8}{\texttt{Stacks tag 0FZ8}}
\group{equiv}
...
\begin{proof}
\uses{equiv-lemma-diagonal-trick-proper}

Combine Lemma \ref{equiv-lemma-diagonal-trick-proper} with
...
\source{stacks:0FZ9}
\href{https://stacks.math.columbia.edu/tag/0FZ9}{\texttt{Stacks tag 0FZ9}}
\group{equiv}
...
\begin{proof}
\uses{equiv-lemma-split-complex-regular}
...
Lemma \ref{equiv-lemma-split-complex-regular}. The final statement
is clear from this.
\end{proof}
...
\source{stacks:0FZT}
\href{https://stacks.math.columbia.edu/tag/0FZT}{\texttt{Stacks tag 0FZT}}
\group{equiv}
...
\begin{lemma}[Sibling fully faithful]
\label{equiv-lemma-sibling-fully-faithful}
\source{stacks:0FZU}
\href{https://stacks.math.columbia.edu/tag/0FZU}{\texttt{Stacks tag 0FZU}}
\group{equiv}
...
\begin{lemma}[Sibling faithful]
\label{equiv-lemma-sibling-faithful}
\source{stacks:0FZV}
\href{https://stacks.math.columbia.edu/tag/0FZV}{\texttt{Stacks tag 0FZV}}
\group{equiv}

\uses{equiv-definition-siblings}
Let $F$ and $F'$ be siblings as in Definition \ref{equiv-definition-siblings}.
Then
\begin{enumerate}
...
$X \in \Ob(\mathcal{A})$ given to us by Definition \ref{equiv-definition-siblings}.
...
$$
\begin{array}{cc}E \downarrow\longrightarrow & X \downarrow \\ F(E) \longrightarrow & F(X)\end{array}
$$
...
$$
\begin{array}{cc}F(X) \overset{\gamma}{\downarrow}\underset{F(\delta)}{\longrightarrow} & F(Y[1]) \underset{\epsilon}{\downarrow} \\ F(X) \overset{\theta}{\longrightarrow} & F(Y[1])\end{array}
$$
...
help with the proof of Proposition \ref{equiv-proposition-siblings-isomorphic}.

\begin{lemma}[Good map]
\label{equiv-lemma-good-map}
\source{stacks:0GWF}
\href{https://stacks.math.columbia.edu/tag/0GWF}{\texttt{Stacks tag 0GWF}}
\group{equiv}
...
\begin{lemma}[Good map zero]
\label{equiv-lemma-good-map-zero}
\source{stacks:0GWG}
\href{https://stacks.math.columbia.edu/tag/0GWG}{\texttt{Stacks tag 0GWG}}
\group{equiv}
...
\begin{proposition}[Siblings isomorphic]
\label{equiv-proposition-siblings-isomorphic}
\source{stacks:0FZW}
\href{https://stacks.math.columbia.edu/tag/0FZW}{\texttt{Stacks tag 0FZW}}
\group{equiv}
...
\begin{proof}
\uses{equiv-definition-siblings, equiv-lemma-good-map, equiv-lemma-good-map-zero}

By part (2) of Definition \ref{equiv-definition-siblings} the image of the functor
...
\vcenter{
\begin{array}{cc}X \underset{c_X}{\downarrow}\underset{f}{\longrightarrow} & X' \overset{c_{X'}}{\downarrow} \\ F(X) \overset{F(f)}{\longrightarrow} & F(X')\end{array}
}
\end{equation}
is commutative.
...
diagram
$$
\begin{array}{cccc}E \underset{c_E}{\downarrow}\longrightarrow & A \underset{c_A}{\downarrow}\underset{f}{\longrightarrow} & A'[1] \overset{c_{A'}[1]}{\downarrow}\underset{\gamma}{\downarrow}\overset{\epsilon}{\swarrow}\longrightarrow & E[1] \overset{c_E[1]}{\downarrow} \\ F(E) \longrightarrow & F(A) \overset{F(f)}{\longrightarrow} & F(A')[1] \longrightarrow & F(E)[1]\end{array}
$$
...
$H^b(X)$ is nonzero. Choose $N[-b] \to X$ as in Lemma \ref{equiv-lemma-good-map}.
...
$$
\begin{array}{cccc}N[-b] \longrightarrow\underset{c_N[-b]}{\downarrow} & X \longrightarrow\underset{c_{N[-b] \to X}}{\downarrow} & Y \longrightarrow\overset{c_Y}{\downarrow} & N[-b + 1] \overset{c_N[-b + 1]}{\downarrow} \\ F(N)[-b] \longrightarrow & F(X) \longrightarrow & F(Y) \longrightarrow & F(N)[-b + 1]\end{array}
$$
...
\medskip\noindent
Let $N'[-b] \to X$ be another map as in Lemma \ref{equiv-lemma-good-map}
and let us prove that $c_{N[-b] \to X} = c_{N'[-b] \to X}$.
Observe that the map $(N \oplus N')[-b] \to X$ also satisfies the
conditions of Lemma \ref{equiv-lemma-good-map}.
...
where $N[-b] \to X$ is as in Lemma \ref{equiv-lemma-good-map}. To finish
...
\medskip\noindent
Case I: $b' < b$. In this case, by Lemma \ref{equiv-lemma-good-map-zero}
we may choose $N[-b] \to X$ as in Lemma \ref{equiv-lemma-good-map}
such that the composition $N[-b] \to X \to X'$ is zero.
...
as in Lemma \ref{equiv-lemma-good-map}.
...
\begin{lemma}[Get fully faithful]
\label{equiv-lemma-get-fully-faithful}
\source{stacks:0G24}
\href{https://stacks.math.columbia.edu/tag/0G24}{\texttt{Stacks tag 0G24}}
\group{equiv}
...
\begin{lemma}[Duality at point]
\label{equiv-lemma-duality-at-point}
\source{stacks:0G02}
\href{https://stacks.math.columbia.edu/tag/0G02}{\texttt{Stacks tag 0G02}}
\group{equiv}
...
\begin{lemma}[Orthogonal point sheaf]
\label{equiv-lemma-orthogonal-point-sheaf}
\source{stacks:0G03}
\href{https://stacks.math.columbia.edu/tag/0G03}{\texttt{Stacks tag 0G03}}
\group{equiv}
...
\begin{proof}
\uses{equiv-lemma-duality-at-point}
...
Part (1) follows from part (5), Lemma \ref{equiv-lemma-duality-at-point}, and the
...
\begin{lemma}[Hom into point sheaf]
\label{equiv-lemma-hom-into-point-sheaf}
\source{stacks:0GWZ}
\href{https://stacks.math.columbia.edu/tag/0GWZ}{\texttt{Stacks tag 0GWZ}}
\group{equiv}
...
\begin{lemma}[Get fully faithful geometric]
\label{equiv-lemma-get-fully-faithful-geometric}
\source{stacks:0G25}
\href{https://stacks.math.columbia.edu/tag/0G25}{\texttt{Stacks tag 0G25}}
\group{equiv}
...
\begin{proof}
\uses{equiv-lemma-always-right-adjoints, equiv-lemma-get-fully-faithful, equiv-lemma-orthogonal-point-sheaf}

By Lemma \ref{equiv-lemma-always-right-adjoints} the functor $F$
has both a left and a right adjoint. Thus we may apply the criterion
of Lemma \ref{equiv-lemma-get-fully-faithful}
because assumptions (2) and (3) of that lemma
follow from Lemma \ref{equiv-lemma-orthogonal-point-sheaf}.
\end{proof}

\begin{lemma}[Noah pre]
\label{equiv-lemma-noah-pre}
\source{stacks:0G26}
\href{https://stacks.math.columbia.edu/tag/0G26}{\texttt{Stacks tag 0G26}}
\group{equiv}
...
\begin{proof}
\uses{equiv-lemma-get-fully-faithful-geometric}

By Lemma \ref{equiv-lemma-get-fully-faithful-geometric} it suffices to show
that the maps
$$
F : \Ext^i_X(\mathcal{O}_x, \mathcal{O}_{x'})
\longrightarrow
...
\begin{definition}[Siblings geometric]
\label{equiv-definition-siblings-geometric}
\source{stacks:0FZZ}
\href{https://stacks.math.columbia.edu/tag/0FZZ}{\texttt{Stacks tag 0FZZ}}
\group{equiv}
...
are siblings in the sense of Definition \ref{equiv-definition-siblings}
...
\begin{lemma}[Exact functor preserving Coh]
\label{equiv-lemma-exact-functor-preserving-Coh}
\source{stacks:0G00}
\href{https://stacks.math.columbia.edu/tag/0G00}{\texttt{Stacks tag 0G00}}
\group{equiv}
...
\begin{proof}
\uses{equiv-lemma-fourier-mukai-Coh, equiv-lemma-sibling-fully-faithful}
...
Lemma \ref{equiv-lemma-fourier-mukai-Coh}.
Observe that $F$ and $F'$ satisfy the first and second
condition of Lemma \ref{equiv-lemma-sibling-fully-faithful} and hence are siblings.
\end{proof}

\begin{remark}[Difficult]
\label{equiv-remark-difficult}
\source{stacks:0G01}
\href{https://stacks.math.columbia.edu/tag/0G01}{\texttt{Stacks tag 0G01}}
\group{equiv}

\uses{equiv-proposition-siblings-isomorphic, equiv-theorem-fully-faithful, equiv-lemma-exact-functor-preserving-Coh}
...
Proposition \ref{equiv-proposition-siblings-isomorphic} via the argument
given in the proof of Theorem \ref{equiv-theorem-fully-faithful}.
However, in general we do not know whether siblings are isomorphic.
Even in the situation of Lemma \ref{equiv-lemma-exact-functor-preserving-Coh}
...
\begin{lemma}[Two functors pre]
\label{equiv-lemma-two-functors-pre}
\source{stacks:0GX0}
\href{https://stacks.math.columbia.edu/tag/0GX0}{\texttt{Stacks tag 0GX0}}
\group{equiv}
...
\begin{proof}
\uses{equiv-lemma-always-right-adjoints, equiv-lemma-orthogonal-point-sheaf}

Recall that $F$ and $G$ have both adjoints, see
Lemma \ref{equiv-lemma-always-right-adjoints}. In particular
...
Lemma \ref{equiv-lemma-orthogonal-point-sheaf} and we win.
\end{proof}

\begin{lemma}[Noah]
\label{equiv-lemma-noah}
\source{stacks:0G27}
\href{https://stacks.math.columbia.edu/tag/0G27}{\texttt{Stacks tag 0G27}}
\group{equiv}
...
\begin{proof}
\uses{equiv-lemma-noah-pre, equiv-lemma-two-functors-pre, equiv-lemma-hom-into-point-sheaf, equiv-lemma-sibling-fully-faithful}

By Lemma \ref{equiv-lemma-noah-pre} the functor $F$ is fully faithful.
By Lemma \ref{equiv-lemma-two-functors-pre} the essential image of
...
Thus by Lemma \ref{equiv-lemma-hom-into-point-sheaf} we see that $F(M)$
...
Using Lemma \ref{equiv-lemma-sibling-fully-faithful}
...
\begin{lemma}[Two functors]
\label{equiv-lemma-two-functors}
\source{stacks:0G06}
\href{https://stacks.math.columbia.edu/tag/0G06}{\texttt{Stacks tag 0G06}}
\group{equiv}
...
\begin{proof}
\uses{equiv-lemma-two-functors-pre, equiv-lemma-noah, equiv-lemma-sibling-faithful, equiv-lemma-compose-fourier-mukai}

The essential image of $G$ is contained in the essential
image of $F$ by Lemma \ref{equiv-lemma-two-functors-pre}.
Consider the functor $H = F^{-1} \circ G$
which makes sense as $F$ is fully faithful.
By Lemma \ref{equiv-lemma-noah} we obtain an automorphism $f : X \to X$
...
$H$ is an auto-equivalence by Lemma \ref{equiv-lemma-sibling-faithful}
...
Lemma \ref{equiv-lemma-compose-fourier-mukai}
we conclude.
\end{proof}
...
\begin{definition}[Situation: Fully faithful]
\label{equiv-situation-fully-faithful}
\source{stacks:0G08}
\href{https://stacks.math.columbia.edu/tag/0G08}{\texttt{Stacks tag 0G08}}
\group{equiv}
...
$$
where each $\mathcal{E}_i$ and $\mathcal{G}_i$ is a finite locally
free $\mathcal{O}_X$-module, see Lemma \ref{equiv-lemma-diagonal-resolution}.
Using the complex
\begin{equation}
...
By Lemma \ref{equiv-lemma-boundedness} we find there exists an integer $m \geq 0$
...
Lemma \ref{equiv-lemma-bounded-fibres} after possibly replacing $m$ by
...
\source{stacks:0G0B}
\href{https://stacks.math.columbia.edu/tag/0G0B}{\texttt{Stacks tag 0G0B}}
\group{equiv}
...
\begin{proof}
\uses{equiv-lemma-two-functors}

Apply Lemma \ref{equiv-lemma-two-functors} to $F$ and the functor
$G$ constructed above.
\end{proof}
...
\source{stacks:0G0C}
\href{https://stacks.math.columbia.edu/tag/0G0C}{\texttt{Stacks tag 0G0C}}
\group{equiv}
...
\begin{proof}
\uses{equiv-lemma-fully-faithful, equiv-proposition-siblings-isomorphic}

Let $F'$ be the Fourier-Mukai functor which is a sibling of $F$
as in Lemma \ref{equiv-lemma-fully-faithful}.
By Proposition \ref{equiv-proposition-siblings-isomorphic} we have $F \cong F'$
...
\source{stacks:0G0D}
\href{https://stacks.math.columbia.edu/tag/0G0D}{\texttt{Stacks tag 0G0D}}
\group{equiv}
...
\begin{proof}
\uses{equiv-lemma-fully-faithful, equiv-lemma-sibling-faithful}

The functor $F'$ of Lemma \ref{equiv-lemma-fully-faithful}
is an equivalence by Lemma \ref{equiv-lemma-sibling-faithful}.
\end{proof}

\begin{lemma}[Uniqueness]
\label{equiv-lemma-uniqueness}
\source{stacks:0G0E}
\href{https://stacks.math.columbia.edu/tag/0G0E}{\texttt{Stacks tag 0G0E}}
\group{equiv}
...
by Lemma \ref{equiv-lemma-on-product}.
...
\begin{lemma}[Base change is functor]
\label{equiv-lemma-base-change-is-functor}
\source{stacks:0G0G}
\href{https://stacks.math.columbia.edu/tag/0G0G}{\texttt{Stacks tag 0G0G}}
\group{equiv}
...
The diagram
$$
\begin{array}{cc}X' \times_{S'} Y' \times_{S'} Z' \longrightarrow\underset{\text{pr}'_{13}}{\downarrow} & X \times_S Y \times_S Z \overset{\text{pr}_{13}}{\downarrow} \\ X' \times_{S'} Z' \longrightarrow & X \times_S Z\end{array}
$$
...
\begin{definition}[Relative equivalence kernel]
\label{equiv-definition-relative-equivalence-kernel}
\source{stacks:0G0I}
\href{https://stacks.math.columbia.edu/tag/0G0I}{\texttt{Stacks tag 0G0I}}
\group{equiv}
...
\begin{lemma}[Equivalences rek]
\label{equiv-lemma-equivalences-rek}
\source{stacks:0G0J}
\href{https://stacks.math.columbia.edu/tag/0G0J}{\texttt{Stacks tag 0G0J}}
\group{equiv}

\uses{equiv-definition-relative-equivalence-kernel, equiv-lemma-fourier-Mukai-QCoh, equiv-lemma-fourier-mukai}
With notation as in Definition \ref{equiv-definition-relative-equivalence-kernel}
...
$\Phi_K : D_\QCoh(\mathcal{O}_X) \to D_\QCoh(\mathcal{O}_Y)$
(Lemma \ref{equiv-lemma-fourier-Mukai-QCoh})
and $\Phi_K : D_{perf}(\mathcal{O}_X) \to D_{perf}(\mathcal{O}_Y)$
(Lemma \ref{equiv-lemma-fourier-mukai})
are equivalences.
\end{lemma}

\begin{proof}
\uses{equiv-lemma-compose-fourier-mukai, equiv-example-diagonal-fourier-mukai}

Immediate from Lemma \ref{equiv-lemma-compose-fourier-mukai} and
Example \ref{equiv-example-diagonal-fourier-mukai}.
\end{proof}

\begin{lemma}[Base change rek]
\label{equiv-lemma-base-change-rek}
\source{stacks:0G0K}
\href{https://stacks.math.columbia.edu/tag/0G0K}{\texttt{Stacks tag 0G0K}}
\group{equiv}

\uses{equiv-definition-relative-equivalence-kernel}
With notation as in Definition \ref{equiv-definition-relative-equivalence-kernel}
let $K$ be the Fourier-Mukai kernel of a relative equivalence from $X$
...
exist in Definition \ref{equiv-definition-relative-equivalence-kernel}.
...
condition. Since
$$
\begin{array}{cc}X_1 \times_{S_1} Y_1 \times_{S_1} X_1 \longrightarrow\underset{\text{pr}_{13}}{\downarrow} & X \times_S Y \times_S X \overset{\text{pr}_{13}}{\downarrow} \\ X_1 \times_{S_1} X_1 \longrightarrow & X \times_S X\end{array}
$$
...
\begin{lemma}[Descend rek]
\label{equiv-lemma-descend-rek}
\source{stacks:0G0L}
\href{https://stacks.math.columbia.edu/tag/0G0L}{\texttt{Stacks tag 0G0L}}
\group{equiv}
...
\begin{proof}
\uses{equiv-definition-relative-equivalence-kernel, equiv-lemma-base-change-rek}
...
$$
See proof of Lemma \ref{equiv-lemma-base-change-rek}.
...
$$
See proof of Lemma \ref{equiv-lemma-base-change-rek}. Thus by
...
The title of this section refers to Lemma \ref{equiv-lemma-no-deformations}

\begin{lemma}[Deform koszul]
\label{equiv-lemma-deform-koszul}
\source{stacks:0G0N}
\href{https://stacks.math.columbia.edu/tag/0G0N}{\texttt{Stacks tag 0G0N}}
\group{equiv}
...
$$
\begin{array}{ccc}K_\bullet \searrow\underset{\varphi_\bullet}{\dashrightarrow} &  & K_\bullet(f_1, \ldots, f_r) \swarrow \\  & A/J & \end{array}
$$
...
$$
\begin{array}{ccc}K_\bullet(\overline{f}_1, \ldots, \overline{f}_r)
\searrow\underset{\overline{\varphi}_\bullet}{\longrightarrow} &  & K_\bullet(\overline{f}_1, \ldots, \overline{f}_r) \swarrow \\  & (A/\mathfrak m A)/(\overline{f}_1, \ldots, \overline{f}_r) & \end{array}
$$
...
\begin{lemma}[Limit arguments]
\label{equiv-lemma-limit-arguments}
\source{stacks:0G0P}
\href{https://stacks.math.columbia.edu/tag/0G0P}{\texttt{Stacks tag 0G0P}}
\group{equiv}
...
\begin{lemma}[Isomorphism in neighbourhood]
\label{equiv-lemma-isomorphism-in-neighbourhood}
\source{stacks:0G0Q}
\href{https://stacks.math.columbia.edu/tag/0G0Q}{\texttt{Stacks tag 0G0Q}}
\group{equiv}
...
\begin{lemma}[No deformations]
\label{equiv-lemma-no-deformations}
\source{stacks:0G0R}
\href{https://stacks.math.columbia.edu/tag/0G0R}{\texttt{Stacks tag 0G0R}}
\group{equiv}
...
\begin{proof}
\uses{equiv-lemma-orthogonal-point-sheaf, equiv-lemma-deform-koszul, equiv-lemma-limit-arguments, equiv-lemma-isomorphism-in-neighbourhood}
...
$$
Hence by Lemma \ref{equiv-lemma-orthogonal-point-sheaf}
...
$\mathcal{O}_{Y_s \times Y_s, z}$.
By Lemma \ref{equiv-lemma-deform-koszul} applied to
...
By some limit arguments (Lemma \ref{equiv-lemma-limit-arguments})
...
Lemma \ref{equiv-lemma-isomorphism-in-neighbourhood}.
This finishes the proof.
\end{proof}

\begin{lemma}[No deformations better]
\label{equiv-lemma-no-deformations-better}
\source{stacks:0G0S}
\href{https://stacks.math.columbia.edu/tag/0G0S}{\texttt{Stacks tag 0G0S}}
\group{equiv}
...
\begin{proof}
\uses{equiv-lemma-base-change-rek, equiv-definition-relative-equivalence-kernel, equiv-lemma-base-change-is-functor, equiv-lemma-no-deformations}
...
Lemma \ref{equiv-lemma-base-change-rek}. Let $K'_0$ in
...
Definition \ref{equiv-definition-relative-equivalence-kernel}.
Hence by
Lemma \ref{equiv-lemma-base-change-rek}
...
(Lemma \ref{equiv-lemma-base-change-is-functor}).
...
$$
Thus by Lemma \ref{equiv-lemma-no-deformations} we conclude that $Y \to S$
...
\begin{lemma}[Countable finite type]
\label{equiv-lemma-countable-finite-type}
\source{stacks:0G0U}
\href{https://stacks.math.columbia.edu/tag/0G0U}{\texttt{Stacks tag 0G0U}}
\group{equiv}
...
\begin{lemma}[Countable abelian]
\label{equiv-lemma-countable-abelian}
\source{stacks:0G0V}
\href{https://stacks.math.columbia.edu/tag/0G0V}{\texttt{Stacks tag 0G0V}}
\group{equiv}
...
\begin{lemma}[Countable perfect]
\label{equiv-lemma-countable-perfect}
\source{stacks:0G0W}
\href{https://stacks.math.columbia.edu/tag/0G0W}{\texttt{Stacks tag 0G0W}}
\group{equiv}
...
\begin{proof}
\uses{equiv-lemma-countable-abelian}
...
Hence by Lemma \ref{equiv-lemma-countable-abelian}
it suffices to prove that $\textit{Coh}(\mathcal{O}_X)$ is
countable. This we omit.
\end{proof}

\begin{lemma}[Countable isos]
\label{equiv-lemma-countable-isos}
\source{stacks:0G0X}
\href{https://stacks.math.columbia.edu/tag/0G0X}{\texttt{Stacks tag 0G0X}}
\group{equiv}
...
\begin{proof}
\uses{equiv-lemma-countable-finite-type}
...
\medskip\noindent
By Lemma \ref{equiv-lemma-countable-finite-type} there is a countable set $I$ and
...
$$
\begin{array}{cc}\Spec(K) \underset{s}{\downarrow}\longrightarrow & \Spec(A) \overset{s'}{\downarrow} \\ S \longrightarrow & S_0\end{array}
$$
...
\begin{lemma}[Countable equivs]
\label{equiv-lemma-countable-equivs}
\source{stacks:0G0Y}
\href{https://stacks.math.columbia.edu/tag/0G0Y}{\texttt{Stacks tag 0G0Y}}
\group{equiv}
...
\begin{proof}
\uses{equiv-lemma-countable-finite-type, equiv-lemma-countable-perfect, equiv-lemma-base-change-rek, equiv-proposition-equivalence, equiv-lemma-fourier-mukai-flat-proper-over-noetherian, equiv-lemma-uniqueness, equiv-lemma-descend-rek}

Choose a countable subfield $k \subset K$ for example the prime field.
By Lemmas \ref{equiv-lemma-countable-finite-type} and \ref{equiv-lemma-countable-perfect}
...
see Lemma \ref{equiv-lemma-base-change-rek}. Let us prove property (4).

\medskip\noindent
Consider smooth proper schemes $X$ and $Y$ over $K$
such that there is a $K$-linear exact equivalence
$F : D_{perf}(\mathcal{O}_X) \to D_{perf}(\mathcal{O}_Y)$.
By Proposition \ref{equiv-proposition-equivalence}
...
By Lemma \ref{equiv-lemma-fourier-mukai-flat-proper-over-noetherian}
...
by Lemma \ref{equiv-lemma-uniqueness}. Hence we see that $M$ is the
...
By Lemma \ref{equiv-lemma-descend-rek}
...
\begin{definition}[Derived equivalent]
\label{equiv-definition-derived-equivalent}
\source{stacks:0G10}
\href{https://stacks.math.columbia.edu/tag/0G10}{\texttt{Stacks tag 0G10}}
\group{equiv}
...
\begin{theorem}[Countable]
\label{equiv-theorem-countable}
\source{stacks:0G11}
\href{https://stacks.math.columbia.edu/tag/0G11}{\texttt{Stacks tag 0G11}}
\group{equiv}
...
\begin{proof}
\uses{equiv-lemma-countable-equivs, equiv-lemma-countable-isos, equiv-lemma-base-change-rek, equiv-lemma-no-deformations-better}

Choose a countable set $I$ and for $i \in I$ systems
$(S_i/K, X_i \to S_i, Y_i \to S_i, M_i)$ satisfying properties
(1), (2), (3), and (4) of Lemma \ref{equiv-lemma-countable-equivs}.
...
the set of isomorphism classes of fibres $Y_s$ for $s \in S(K)$
such that $X_s \cong \mathbf{X}$ is countable.
By Lemma \ref{equiv-lemma-countable-isos} applied
...
the pullback of $M$. By Lemma \ref{equiv-lemma-base-change-rek}
...
Lemma \ref{equiv-lemma-no-deformations-better}

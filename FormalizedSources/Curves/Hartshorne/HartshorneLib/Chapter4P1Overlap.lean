...
/-- Every Laurent polynomial splits into a polynomial in `T` and one in `T⁻¹`. -/
theorem exists_toLaurent_add_aeval {R : Type*} [CommSemiring R] (f : LaurentPolynomial R) :
    ∃ p q : Polynomial R, f = p.toLaurent + Polynomial.aeval (T (-1) : LaurentPolynomial R) q := by

...
should read
Section \ref{categories-section-formal-cat-cat}
...
\source{stacks:0014}
\href{https://stacks.math.columbia.edu/tag/0014}{\texttt{Stacks tag 0014}}
\group{categories}
...
\begin{remark}[Big categories]
\label{categories-remark-big-categories}
\source{stacks:0015}
\href{https://stacks.math.columbia.edu/tag/0015}{\texttt{Stacks tag 0015}}
\group{categories}
...
\begin{remark}[Unique identity]
\label{categories-remark-unique-identity}
\source{stacks:0016}
\href{https://stacks.math.columbia.edu/tag/0016}{\texttt{Stacks tag 0016}}
\group{categories}
...
\source{stacks:0017}
\href{https://stacks.math.columbia.edu/tag/0017}{\texttt{Stacks tag 0017}}
\group{categories}
...
\source{stacks:0018}
\href{https://stacks.math.columbia.edu/tag/0018}{\texttt{Stacks tag 0018}}
\group{categories}

A {\it groupoid} is a category where every morphism is an isomorphism.
\end{definition}

\begin{example}[Group groupoid]
\label{categories-example-group-groupoid}
\source{stacks:0019}
\href{https://stacks.math.columbia.edu/tag/0019}{\texttt{Stacks tag 0019}}
\group{categories}
...
\begin{example}[Set groupoid]
\label{categories-example-set-groupoid}
\source{stacks:001A}
\href{https://stacks.math.columbia.edu/tag/001A}{\texttt{Stacks tag 001A}}
\group{categories}
...
\source{stacks:001B}
\href{https://stacks.math.columbia.edu/tag/001B}{\texttt{Stacks tag 001B}}
\group{categories}
...
\source{stacks:001C}
\href{https://stacks.math.columbia.edu/tag/001C}{\texttt{Stacks tag 001C}}
\group{categories}
...
\source{stacks:001D}
\href{https://stacks.math.columbia.edu/tag/001D}{\texttt{Stacks tag 001D}}
\group{categories}
...
\begin{remark}[Functor into sets]
\label{categories-remark-functor-into-sets}
\source{stacks:001E}
\href{https://stacks.math.columbia.edu/tag/001E}{\texttt{Stacks tag 001E}}
\group{categories}
...
\begin{example}[Group homomorphism functor]
\label{categories-example-group-homomorphism-functor}
\source{stacks:001F}
\href{https://stacks.math.columbia.edu/tag/001F}{\texttt{Stacks tag 001F}}
\group{categories}

\uses{categories-example-group-groupoid}
A homomorphism $p : G\to H$ of groups gives rise to a functor
between the associated groupoids in Example \ref{categories-example-group-groupoid}. It is
faithful (resp.\ fully faithful) if and only if $p$ is injective (resp.\ an
isomorphism).
\end{example}

\begin{example}[Category over X]
\label{categories-example-category-over-X}
\source{stacks:001G}
\href{https://stacks.math.columbia.edu/tag/001G}{\texttt{Stacks tag 001G}}
\group{categories}
...
\begin{example}[Category under X]
\label{categories-example-category-under-X}
\source{stacks:001H}
\href{https://stacks.math.columbia.edu/tag/001H}{\texttt{Stacks tag 001H}}
\group{categories}
...
\begin{definition}[Transformation functors]
\label{categories-definition-transformation-functors}
\source{stacks:001I}
\href{https://stacks.math.columbia.edu/tag/001I}{\texttt{Stacks tag 001I}}
\group{categories}
...
$$
\begin{array}{cc}F(x) \overset{t_x}{\longrightarrow}\underset{F(\phi)}{\downarrow} & G(x) \overset{G(\phi)}{\downarrow} \\ F(y) \overset{t_y}{\longrightarrow} & G(y)\end{array}
$$
\end{enumerate}
\end{definition}
...
$$
\begin{array}{cc}\mathcal{A}
\xrightarrow{\substack{F\\G\\t}} & \mathcal{B}\end{array}
$$
...
\begin{remark}[Functors sets sets]
\label{categories-remark-functors-sets-sets}
\source{stacks:02C2}
\href{https://stacks.math.columbia.edu/tag/02C2}{\texttt{Stacks tag 02C2}}
\group{categories}
...
\begin{definition}[Equivalence categories]
\label{categories-definition-equivalence-categories}
\source{stacks:001J}
\href{https://stacks.math.columbia.edu/tag/001J}{\texttt{Stacks tag 001J}}
\group{categories}
...
\begin{lemma}[Construct quasi inverse]
\label{categories-lemma-construct-quasi-inverse}
\source{stacks:05SG}
\href{https://stacks.math.columbia.edu/tag/05SG}{\texttt{Stacks tag 05SG}}
\group{categories}
...
\begin{proof}
To construct $j : \mathcal{B} \to \mathcal{A}$, there are two steps. Firstly,
we define the map $j : \Ob(\mathcal{B}) \to \Ob(\mathcal{A})$ that associates
$j(X)$ to $X \in \mathcal{B}$. Secondly, if $X,Y \in \Ob(\mathcal{B})$ and
$\phi : X \to Y$, we consider $\phi' := i_Y \circ\phi\circ i_X^{-1}$. There is
a unique $\varphi$ verifying $F(\varphi) = \phi'$, using that $F$ is fully
faithful. We define $j(\phi) = \varphi$. We omit the verification that $j$
is a functor. By construction the diagram
$$
\begin{array}{cc}X \underset{i_X}{\longrightarrow}\underset{\phi}{\downarrow} & F(j(X)) \overset{F\circ j(\phi)}{\downarrow} \\ Y \overset{i_Y}{\longrightarrow} & F(j(Y))\end{array}
$$
commutes. Hence, as each $i_X$ is an isomorphism, $\{i_X\}_X$ is an
isomorphism of functors $\text{id}_\mathcal{B} \to F\circ j$. To conclude, we
have to also prove that $j \circ F$ is isomorphic to $\text{id}_\mathcal{A}$.
However, since $F$ is fully faithful, in order to do this it suffices
to prove this after post-composing with $F$, i.e., it suffices to
show that $F \circ j \circ F$ is isomorphic to $F \circ \text{id}_\mathcal{A}$
(small detail omitted). Since $F \circ j \cong \text{id}_\mathcal{B}$
this is clear.
\end{proof}

\begin{lemma}[Equivalence categories]
\label{categories-lemma-equivalence-categories}
\source{stacks:02C3}
\href{https://stacks.math.columbia.edu/tag/02C3}{\texttt{Stacks tag 02C3}}
\group{categories}
...
\begin{proof}
\uses{categories-lemma-construct-quasi-inverse}
...
\source{stacks:001K}
\href{https://stacks.math.columbia.edu/tag/001K}{\texttt{Stacks tag 001K}}
\group{categories}
...
\source{stacks:001M}
\href{https://stacks.math.columbia.edu/tag/001M}{\texttt{Stacks tag 001M}}
\group{categories}
...
\source{stacks:001N}
\href{https://stacks.math.columbia.edu/tag/001N}{\texttt{Stacks tag 001N}}
\group{categories}
...
\source{stacks:02X6}
\href{https://stacks.math.columbia.edu/tag/02X6}{\texttt{Stacks tag 02X6}}
\group{categories}
...
\source{stacks:001O}
\href{https://stacks.math.columbia.edu/tag/001O}{\texttt{Stacks tag 001O}}
\group{categories}
...
Remark \ref{categories-remark-big-categories}. On the other hand,
...
\source{stacks:001P}
\href{https://stacks.math.columbia.edu/tag/001P}{\texttt{Stacks tag 001P}}
\group{categories}
...
\source{stacks:001Q}
\href{https://stacks.math.columbia.edu/tag/001Q}{\texttt{Stacks tag 001Q}}
\group{categories}
...
\source{stacks:001S}
\href{https://stacks.math.columbia.edu/tag/001S}{\texttt{Stacks tag 001S}}
\group{categories}
...
the diagram
$$
\begin{array}{cccc}w \overset{\beta}{\searrow}\underset{\gamma}{\searrow}\underset{\alpha}{\searrow} &  &  &  \\  &  & x \times y \underset{p}{\downarrow}\underset{q}{\longrightarrow} & y \\  &  & x & \end{array}
$$
commute.
\end{definition}
...
\begin{definition}[Has products of pairs]
\label{categories-definition-has-products-of-pairs}
\source{stacks:001T}
\href{https://stacks.math.columbia.edu/tag/001T}{\texttt{Stacks tag 001T}}
\group{categories}
...
\source{stacks:04AO}
\href{https://stacks.math.columbia.edu/tag/04AO}{\texttt{Stacks tag 04AO}}
\group{categories}
...
the diagram
$$
\begin{array}{cccc} & y \overset{j}{\downarrow}\overset{\beta}{\searrow} &  &  \\ x \overset{i}{\longrightarrow}\underset{\alpha}{\searrow} & x \amalg y \overset{\gamma}{\searrow} &  &  \\  &  &  & w\end{array}
$$
commute.
\end{definition}
...
\begin{definition}[Has coproducts of pairs]
\label{categories-definition-has-coproducts-of-pairs}
\source{stacks:04AP}
\href{https://stacks.math.columbia.edu/tag/04AP}{\texttt{Stacks tag 04AP}}
\group{categories}
...
\begin{definition}[Fibre products]
\label{categories-definition-fibre-products}
\source{stacks:001V}
\href{https://stacks.math.columbia.edu/tag/001V}{\texttt{Stacks tag 001V}}
\group{categories}
...
$$
\begin{array}{cc}x \times_y z \underset{q}{\longrightarrow}\underset{p}{\downarrow} & z \overset{g}{\downarrow} \\ x \overset{f}{\longrightarrow} & y\end{array}
$$
...
the diagram
$$
\begin{array}{cccc}w \overset{\beta}{\searrow}\underset{\gamma}{\searrow}\underset{\alpha}{\searrow} &  &  &  \\  &  & x \times_y z \overset{p}{\downarrow}\underset{q}{\longrightarrow} & z \overset{g}{\downarrow} \\  &  & x \overset{f}{\longrightarrow} & y\end{array}
$$
commute.
\end{definition}
...
\source{stacks:08N0}
\href{https://stacks.math.columbia.edu/tag/08N0}{\texttt{Stacks tag 08N0}}
\group{categories}

We say a commutative diagram
$$
\begin{array}{cc}w \longrightarrow\downarrow & z \downarrow \\ x \longrightarrow & y\end{array}
$$
...
\begin{definition}[Has fibre products]
\label{categories-definition-has-fibre-products}
\source{stacks:001W}
\href{https://stacks.math.columbia.edu/tag/001W}{\texttt{Stacks tag 001W}}
\group{categories}
...
\source{stacks:001X}
\href{https://stacks.math.columbia.edu/tag/001X}{\texttt{Stacks tag 001X}}
\group{categories}
...
\begin{lemma}[Composition representable]
\label{categories-lemma-composition-representable}
\source{stacks:001Y}
\href{https://stacks.math.columbia.edu/tag/001Y}{\texttt{Stacks tag 001Y}}
\group{categories}
...
\begin{proof}
\uses{categories-definition-fibre-products}

Let $t \in \Ob(\mathcal C)$ and $ \varphi \in \Mor_{\mathcal C}(t,z) $.
As $g$ and $f$ are representable, we obtain commutative diagrams
$$
\begin{array}{cc}y \times_z t \underset{q}{\longrightarrow}\underset{p}{\downarrow} & t \overset{\varphi}{\downarrow} \\ y \overset{g}{\longrightarrow} & z\end{array}
\quad\quad
\begin{array}{cc}x \times_y (y\! \times_z\! t) \underset{q'}{\longrightarrow}\underset{p'}{\downarrow} & y \times_z t \overset{p}{\downarrow} \\ x \overset{f}{\longrightarrow} & y\end{array}
$$
with the universal property of Definition \ref{categories-definition-fibre-products}.
...
$$
\begin{array}{cccc}w \overset{\beta}{\searrow}\underset{\delta}{\searrow}\underset{f\circ\alpha}{\searrow} &  &  &  \\  &  & y \times_z t \underset{p}{\downarrow}\underset{q}{\longrightarrow} & t \overset{\varphi}{\downarrow} \\  &  & y \overset{g}{\longrightarrow} & z\end{array}
$$
and
$$
\begin{array}{cccc}w \overset{\delta}{\searrow}\underset{\gamma}{\searrow}\underset{\alpha}{\searrow} &  &  &  \\  &  & x \times_y (y\!\times_z\! t) \underset{p'}{\downarrow}\underset{q'}{\longrightarrow} & y \times_z t \overset{p}{\downarrow} \\  &  & x \overset{f}{\longrightarrow} & y\end{array}
$$
commute. Then, $\gamma$ makes the diagram
$$
\begin{array}{cccc}w \overset{\beta}{\searrow}\underset{\gamma}{\searrow}\underset{\alpha}{\searrow} &  &  &  \\  &  & x \times_z t \underset{p'}{\downarrow}\underset{q\circ q'}{\longrightarrow} & t \overset{\varphi}{\downarrow} \\  &  & x \overset{g\circ f}{\longrightarrow} & z\end{array}
$$
...
\begin{lemma}[Base change representable]
\label{categories-lemma-base-change-representable}
\source{stacks:001Z}
\href{https://stacks.math.columbia.edu/tag/001Z}{\texttt{Stacks tag 001Z}}
\group{categories}
...
\begin{lemma}[Fibre product presheaves]
\label{categories-lemma-fibre-product-presheaves}
\source{stacks:0022}
\href{https://stacks.math.columbia.edu/tag/0022}{\texttt{Stacks tag 0022}}
\group{categories}
...
Lemma \ref{categories-lemma-yoneda} this gives a transformation
...
fibre product, if it exists, see Section \ref{categories-section-fibre-products}.
The analogy with Definition \ref{categories-definition-representable-morphism}
prompts us to define a notion
of representable transformations.

\begin{definition}[Representable map presheaves]
\label{categories-definition-representable-map-presheaves}
\source{stacks:0023}
\href{https://stacks.math.columbia.edu/tag/0023}{\texttt{Stacks tag 0023}}
\group{categories}
...
$h_U \times_{\xi, G, a} F$ is representable.
\end{definition}

\begin{lemma}[Representable over representable]
\label{categories-lemma-representable-over-representable}
\source{stacks:03KC}
\href{https://stacks.math.columbia.edu/tag/03KC}{\texttt{Stacks tag 03KC}}
\group{categories}
...
\begin{lemma}[Representable diagonal]
\label{categories-lemma-representable-diagonal}
\source{stacks:0024}
\href{https://stacks.math.columbia.edu/tag/0024}{\texttt{Stacks tag 0024}}
\group{categories}
...
\begin{proof}
\uses{categories-lemma-fibre-product-presheaves}
...
$$
\begin{array}{cc}h_W \downarrow\longrightarrow & h_U \times h_V \overset{\xi \times \xi'}{\downarrow} \\ F \longrightarrow & F \times F\end{array}
$$
is cartesian. Using the explicit description of fibre products
in Lemma \ref{categories-lemma-fibre-product-presheaves} the reader sees that this
...
\source{stacks:0026}
\href{https://stacks.math.columbia.edu/tag/0026}{\texttt{Stacks tag 0026}}
\group{categories}
...
$$
\begin{array}{cc}y \underset{g}{\longrightarrow}\underset{f}{\downarrow} & z \overset{q}{\downarrow} \\ x \overset{p}{\longrightarrow} & x\amalg_y z\end{array}
$$
...
the diagram
$$
\begin{array}{cccc}y \underset{g}{\longrightarrow}\underset{f}{\downarrow} & z \overset{q}{\downarrow}\overset{\beta}{\searrow} &  &  \\ x \overset{p}{\longrightarrow}\overset{\alpha}{\searrow} & x \amalg_y z \overset{\gamma}{\searrow} &  &  \\  &  &  & w\end{array}
$$
commute.
\end{definition}
...
free from the discussion in Section \ref{categories-section-fibre-products}.

\begin{definition}[Cocartesian]
\label{categories-definition-cocartesian}
\source{stacks:08N1}
\href{https://stacks.math.columbia.edu/tag/08N1}{\texttt{Stacks tag 08N1}}
\group{categories}

We say a commutative diagram
$$
\begin{array}{cc}y \longrightarrow\downarrow & z \downarrow \\ x \longrightarrow & w\end{array}
$$
...
\begin{definition}[Equalizers]
\label{categories-definition-equalizers}
\source{stacks:0028}
\href{https://stacks.math.columbia.edu/tag/0028}{\texttt{Stacks tag 0028}}
\group{categories}
...
\begin{definition}[Coequalizers]
\label{categories-definition-coequalizers}
\source{stacks:002A}
\href{https://stacks.math.columbia.edu/tag/002A}{\texttt{Stacks tag 002A}}
\group{categories}
...
\begin{definition}[Initial final]
\label{categories-definition-initial-final}
\source{stacks:002C}
\href{https://stacks.math.columbia.edu/tag/002C}{\texttt{Stacks tag 002C}}
\group{categories}
...
\source{stacks:003B}
\href{https://stacks.math.columbia.edu/tag/003B}{\texttt{Stacks tag 003B}}
\group{categories}
...
\begin{example}[Mono epi sets]
\label{categories-example-mono-epi-sets}
\source{stacks:003C}
\href{https://stacks.math.columbia.edu/tag/003C}{\texttt{Stacks tag 003C}}
\group{categories}
...
\begin{lemma}[Characterize mono epi]
\label{categories-lemma-characterize-mono-epi}
\source{stacks:08LR}
\href{https://stacks.math.columbia.edu/tag/08LR}{\texttt{Stacks tag 08LR}}
\group{categories}
...
\begin{proof}
Suppose that $f$ is a monomorphism. Let $W$ be an object of $\mathcal C$
and $\alpha, \beta \in \Mor_\mathcal C(W,X)$ such that $f\circ \alpha = f\circ
\beta$. Therefore $\alpha = \beta$ as $f$ is monic. In addition, we have the
commutative diagram
$$
\begin{array}{cc}X \overset{\text{id}_X}{\longrightarrow}\underset{\text{id}_X}{\downarrow} & X \overset{f}{\downarrow} \\ X \overset{f}{\longrightarrow} & Y\end{array}
$$
which verify the universal property with $\gamma := \alpha = \beta$.
Thus $X$ is indeed the fibre product $X\times_Y X$.
...
\begin{array}{cc}X \overset{\text{id}_X}{\longrightarrow}\underset{\text{id}_X}{\downarrow} & X \overset{f}{\downarrow} \\ X \overset{f}{\longrightarrow} & Y\end{array}
$$
commutes and if $W \in \Ob(\mathcal C)$ and $\alpha, \beta : W \to X$
such that $f \circ \alpha = f \circ \beta$, we have a unique
$\gamma$ verifying
$$
...
\source{stacks:002E}
\href{https://stacks.math.columbia.edu/tag/002E}{\texttt{Stacks tag 002E}}
\group{categories}
...
\source{stacks:002F}
\href{https://stacks.math.columbia.edu/tag/002F}{\texttt{Stacks tag 002F}}
\group{categories}
...
\begin{remark}[Diagram small]
\label{categories-remark-diagram-small}
\source{stacks:002G}
\href{https://stacks.math.columbia.edu/tag/002G}{\texttt{Stacks tag 002G}}
\group{categories}

\uses{categories-remark-big-categories}
...
\begin{remark}[Limit colim]
\label{categories-remark-limit-colim}
\source{stacks:002H}
\href{https://stacks.math.columbia.edu/tag/002H}{\texttt{Stacks tag 002H}}
\group{categories}
...
limits and colimits of sets in Section \ref{categories-section-limit-sets}
...
\begin{remark}[Cones and cocones]
\label{categories-remark-cones-and-cocones}
\source{stacks:0G2U}
\href{https://stacks.math.columbia.edu/tag/0G2U}{\texttt{Stacks tag 0G2U}}
\group{categories}
...
$$
\begin{array}{ccc} & W \underset{q_i}{\swarrow}\overset{q_{i'}}{\searrow} &  \\ M_i \overset{M(\phi)}{\longrightarrow} &  & M_{i'}\end{array}
$$
...
$$
\begin{array}{ccc}M_i \overset{M(\phi)}{\longrightarrow}\underset{t_i}{\searrow} &  & M_{i'} \overset{t_{i'}}{\swarrow} \\  & W & \end{array}
$$
...
\source{stacks:002I}
\href{https://stacks.math.columbia.edu/tag/002I}{\texttt{Stacks tag 002I}}
\group{categories}
...
\source{stacks:002J}
\href{https://stacks.math.columbia.edu/tag/002J}{\texttt{Stacks tag 002J}}
\group{categories}
...
\begin{lemma}[Functorial colimit]
\label{categories-lemma-functorial-colimit}
\source{stacks:002K}
\href{https://stacks.math.columbia.edu/tag/002K}{\texttt{Stacks tag 002K}}
\group{categories}
...
$$
\begin{array}{cc}M_i \underset{t_i}{\downarrow}\longrightarrow & \colim_\mathcal{I} M \overset{\theta}{\downarrow} \\ N_{H(i)} \longrightarrow & \colim_\mathcal{J} N\end{array}
$$
commute.
\end{lemma}
...
\begin{lemma}[Functorial limit]
\label{categories-lemma-functorial-limit}
\source{stacks:002L}
\href{https://stacks.math.columbia.edu/tag/002L}{\texttt{Stacks tag 002L}}
\group{categories}
...
such that all the diagrams
$$
\begin{array}{cc}\lim_\mathcal{J} N \overset{\theta}{\downarrow}\longrightarrow & N_{H(i)} \underset{t_i}{\downarrow} \\ \lim_\mathcal{I} M \longrightarrow & M_i\end{array}
$$
commute.
\end{lemma}
...
\begin{lemma}[Colimits commute]
\label{categories-lemma-colimits-commute}
\source{stacks:002M}
\href{https://stacks.math.columbia.edu/tag/002M}{\texttt{Stacks tag 002M}}
\group{categories}

Let $\mathcal{I}$, $\mathcal{J}$ be index categories. Let
...
\begin{lemma}[Limits products equalizers]
\label{categories-lemma-limits-products-equalizers}
\source{stacks:002N}
\href{https://stacks.math.columbia.edu/tag/002N}{\texttt{Stacks tag 002N}}
\group{categories}
...
$$
\begin{array}{cc}\prod_{i \in I} M_i
\overset{\phi}{\longrightarrow}\underset{\psi}{\longrightarrow} & \prod_{a \in A} M_{t(a)}\end{array}
$$
...
\begin{lemma}[If all coproducts and coequalizers exist, all colimits exist.]
\label{categories-lemma-colimits-coproducts-coequalizers}
\source{stacks:002P}
\href{https://stacks.math.columbia.edu/tag/002P}{\texttt{Stacks tag 002P}}
\group{categories}
...
$$
\begin{array}{cc}\coprod_{a \in A} M_{s(a)}
\overset{\phi}{\longrightarrow}\underset{\psi}{\longrightarrow} & \coprod_{i \in I} M_i\end{array}
$$
exists, where the morphisms are determined by their components
...
$$
\begin{array}{ccccccc} & i_1 \swarrow\searrow &  & i_3 \swarrow &  & i_{2n-1} \searrow &  \\ i = i_0 &  & i_2 &  & \ldots &  & i_{2n} = i'\end{array}
$$
...
describe. We will explain this in Section \ref{categories-section-directed-colimits}.
...
\begin{definition}[Category connected]
\label{categories-definition-category-connected}
\source{stacks:002S}
\href{https://stacks.math.columbia.edu/tag/002S}{\texttt{Stacks tag 002S}}
\group{categories}
...
\begin{lemma}[Connected limit over X]
\label{categories-lemma-connected-limit-over-X}
\source{stacks:002T}
\href{https://stacks.math.columbia.edu/tag/002T}{\texttt{Stacks tag 002T}}
\group{categories}
...
\begin{lemma}[Connected colimit under X]
\label{categories-lemma-connected-colimit-under-X}
\source{stacks:04AR}
\href{https://stacks.math.columbia.edu/tag/04AR}{\texttt{Stacks tag 04AR}}
\group{categories}
...
\begin{proof}
\uses{categories-lemma-connected-limit-over-X}

Omitted. Hint: This lemma is dual to Lemma \ref{categories-lemma-connected-limit-over-X}.
\end{proof}
...
\source{stacks:04E6}
\href{https://stacks.math.columbia.edu/tag/04E6}{\texttt{Stacks tag 04E6}}
\group{categories}
...
$$
\begin{array}{ccc} & y \swarrow\downarrow\searrow &  \\ H(x_{2k}) & H(x_{2k + 1}) \longleftarrow\longrightarrow & H(x_{2k + 2})\end{array}
$$
...
consist of exactly one equivalence class.
\end{definition}

\begin{lemma}[Cofinal]
\label{categories-lemma-cofinal}
\source{stacks:04E7}
\href{https://stacks.math.columbia.edu/tag/04E7}{\texttt{Stacks tag 04E7}}
\group{categories}
...
\source{stacks:09WP}
\href{https://stacks.math.columbia.edu/tag/09WP}{\texttt{Stacks tag 09WP}}
\group{categories}
...
such that the diagrams
$$
\begin{array}{ccc}H(x_{2k}) \searrow & H(x_{2k + 1}) \longleftarrow\longrightarrow\downarrow & H(x_{2k + 2}) \swarrow \\  & y & \end{array}
$$
commute for $k = 0, \ldots, n - 1$.
\end{enumerate}
\end{definition}
...
\source{stacks:002R}
\href{https://stacks.math.columbia.edu/tag/002R}{\texttt{Stacks tag 002R}}
\group{categories}
...
\begin{lemma}[Colimit constant connected fibers]
\label{categories-lemma-colimit-constant-connected-fibers}
\source{stacks:05US}
\href{https://stacks.math.columbia.edu/tag/05US}{\texttt{Stacks tag 05US}}
\group{categories}

\uses{categories-definition-fibre-category}
Let $F : \mathcal{I} \to \mathcal{I}'$ be a functor.
Assume
\begin{enumerate}
\item the fibre categories (see
Definition \ref{categories-definition-fibre-category})
...
\begin{proof}
\uses{categories-lemma-cofinal}

One can prove this by showing that $\mathcal{I}$ is cofinal in
$\mathcal{I}'$ and applying Lemma \ref{categories-lemma-cofinal}.
...
\begin{lemma}[Product with connected]
\label{categories-lemma-product-with-connected}
\source{stacks:0A2B}
\href{https://stacks.math.columbia.edu/tag/0A2B}{\texttt{Stacks tag 0A2B}}
\group{categories}
...
\begin{proof}
\uses{categories-lemma-colimit-constant-connected-fibers}

This is a special case of Lemma \ref{categories-lemma-colimit-constant-connected-fibers}.
\end{proof}
...
connected, see
Definition \ref{categories-definition-category-connected}.
It turns out that there are ``enough'' finite index categories.

\begin{lemma}[Finite diagram category]
\label{categories-lemma-finite-diagram-category}
\source{stacks:05XU}
\href{https://stacks.math.columbia.edu/tag/05XU}{\texttt{Stacks tag 05XU}}
\group{categories}
...
\begin{proof}
\uses{categories-definition-limit}
...
$q_i : W \to M(x_i)$ as in
Definition \ref{categories-definition-limit}.
...
\begin{lemma}[Fibre products equalizers exist]
\label{categories-lemma-fibre-products-equalizers-exist}
\source{stacks:04AT}
\href{https://stacks.math.columbia.edu/tag/04AT}{\texttt{Stacks tag 04AT}}
\group{categories}
...
\begin{proof}
\uses{categories-lemma-finite-diagram-category}
...
Lemma \ref{categories-lemma-finite-diagram-category}.
...
\begin{lemma}[Almost finite limits exist]
\label{categories-lemma-almost-finite-limits-exist}
\source{stacks:04AU}
\href{https://stacks.math.columbia.edu/tag/04AU}{\texttt{Stacks tag 04AU}}
\group{categories}
...
\begin{proof}
\uses{categories-lemma-limits-products-equalizers}
...
Lemma \ref{categories-lemma-limits-products-equalizers}
...
\begin{lemma}[Finite limits exist]
\label{categories-lemma-finite-limits-exist}
\source{stacks:002O}
\href{https://stacks.math.columbia.edu/tag/002O}{\texttt{Stacks tag 002O}}
\group{categories}
...
\begin{lemma}[Push outs coequalizers exist]
\label{categories-lemma-push-outs-coequalizers-exist}
\source{stacks:04AV}
\href{https://stacks.math.columbia.edu/tag/04AV}{\texttt{Stacks tag 04AV}}
\group{categories}
...
\begin{proof}
\uses{categories-lemma-fibre-products-equalizers-exist}

Omitted. Hint: This is dual to
Lemma \ref{categories-lemma-fibre-products-equalizers-exist}.
\end{proof}

\begin{lemma}[Almost finite colimits exist]
\label{categories-lemma-almost-finite-colimits-exist}
\source{stacks:04AW}
\href{https://stacks.math.columbia.edu/tag/04AW}{\texttt{Stacks tag 04AW}}
\group{categories}
...
\begin{proof}
\uses{categories-lemma-almost-finite-limits-exist}

Omitted. Hint: This is the dual of
Lemma \ref{categories-lemma-almost-finite-limits-exist}.
\end{proof}

\begin{lemma}[Colimits exist]
\label{categories-lemma-colimits-exist}
\source{stacks:002Q}
\href{https://stacks.math.columbia.edu/tag/002Q}{\texttt{Stacks tag 002Q}}
\group{categories}
...
\begin{proof}
\uses{categories-lemma-finite-limits-exist}

Omitted. Hint: This is dual to Lemma \ref{categories-lemma-finite-limits-exist}.
\end{proof}
...
\source{stacks:002V}
\href{https://stacks.math.columbia.edu/tag/002V}{\texttt{Stacks tag 002V}}
\group{categories}
...
\begin{lemma}[Directed commutes]
\label{categories-lemma-directed-commutes}
\source{stacks:002W}
\href{https://stacks.math.columbia.edu/tag/002W}{\texttt{Stacks tag 002W}}
\group{categories}
...
\begin{lemma}[Cofinal in filtered]
\label{categories-lemma-cofinal-in-filtered}
\source{stacks:0BUC}
\href{https://stacks.math.columbia.edu/tag/0BUC}{\texttt{Stacks tag 0BUC}}
\group{categories}
...
\begin{lemma}[Preserve products]
\label{categories-lemma-preserve-products}
\source{stacks:09WQ}
\href{https://stacks.math.columbia.edu/tag/09WQ}{\texttt{Stacks tag 09WQ}}
\group{categories}
...
\begin{proof}
\uses{categories-lemma-directed-commutes}
...
condition Lemma \ref{categories-lemma-directed-commutes}
even if you only care about finite products.
\end{proof}

\begin{lemma}[Colimits abelian as sets]
\label{categories-lemma-colimits-abelian-as-sets}
\source{stacks:09WR}
\href{https://stacks.math.columbia.edu/tag/09WR}{\texttt{Stacks tag 09WR}}
\group{categories}
...
\begin{lemma}[Split into connected]
\label{categories-lemma-split-into-connected}
\source{stacks:09WS}
\href{https://stacks.math.columbia.edu/tag/09WS}{\texttt{Stacks tag 09WS}}
\group{categories}

Let $\mathcal{I}$ be an index category, i.e., a category. Assume
that for every solid diagram
$$
\begin{array}{cc}x \downarrow\longrightarrow & y \downarrow \\ z \dashrightarrow & w\end{array}
$$
...
\begin{lemma}[Preserve injective maps]
\label{categories-lemma-preserve-injective-maps}
\source{stacks:09WT}
\href{https://stacks.math.columbia.edu/tag/09WT}{\texttt{Stacks tag 09WT}}
\group{categories}
...
\begin{proof}
\uses{categories-lemma-split-into-connected}
...
Lemma \ref{categories-lemma-split-into-connected}. Since
...
Section \ref{categories-section-limit-sets} without further mention.
...
$$
\begin{array}{ccc} & i_1 \swarrow\searrow &  \\ i_0 \searrow &  & i_2 \swarrow \\  & w & \end{array}
$$
...
$$
\begin{array}{ccccccc} & i_3 \swarrow\searrow &  & i_5 \swarrow &  & i_{2n-1} \searrow &  \\ w &  & i_4 &  & \ldots &  & i_{2n} = i'\end{array}
$$
...
\begin{lemma}[Split into directed]
\label{categories-lemma-split-into-directed}
\source{stacks:002X}
\href{https://stacks.math.columbia.edu/tag/002X}{\texttt{Stacks tag 002X}}
\group{categories}
...
\begin{lemma}[Almost directed commutes equalizers]
\label{categories-lemma-almost-directed-commutes-equalizers}
\source{stacks:002Y}
\href{https://stacks.math.columbia.edu/tag/002Y}{\texttt{Stacks tag 002Y}}
\group{categories}

\uses{categories-lemma-split-into-directed}
Let $\mathcal{I}$ be an index category satisfying the hypotheses of
Lemma \ref{categories-lemma-split-into-directed} above. Then colimits over $\mathcal{I}$
commute with fibre products and equalizers in sets (and more generally
with finite connected limits).
\end{lemma}

\begin{proof}
\uses{categories-lemma-split-into-directed, categories-lemma-directed-commutes}

By
Lemma \ref{categories-lemma-split-into-directed}
we may write $\mathcal{I} = \coprod \mathcal{I}_j$ with each $\mathcal{I}_j$
filtered. By
Lemma \ref{categories-lemma-directed-commutes}
...
\begin{definition}[Codirected]
\label{categories-definition-codirected}
\source{stacks:04AZ}
\href{https://stacks.math.columbia.edu/tag/04AZ}{\texttt{Stacks tag 04AZ}}
\group{categories}
...
nonempty sets has nonempty limit (Lemma \ref{categories-lemma-nonempty-limit}).
...
\source{stacks:00D3}
\href{https://stacks.math.columbia.edu/tag/00D3}{\texttt{Stacks tag 00D3}}
\group{categories}
...
\begin{definition}[System over poset]
\label{categories-definition-system-over-poset}
\source{stacks:0030}
\href{https://stacks.math.columbia.edu/tag/0030}{\texttt{Stacks tag 0030}}
\group{categories}
...
Given an inverse system $(M_i, f_{ii'})$ over $I$ the limit
...
\begin{remark}[Preorder versus partial order]
\label{categories-remark-preorder-versus-partial-order}
\source{stacks:0CN1}
\href{https://stacks.math.columbia.edu/tag/0CN1}{\texttt{Stacks tag 0CN1}}
\group{categories}
...
The upshot of Remark \ref{categories-remark-preorder-versus-partial-order}
...
\source{stacks:0031}
\href{https://stacks.math.columbia.edu/tag/0031}{\texttt{Stacks tag 0031}}
\group{categories}

\uses{categories-definition-directed-set}
...
(Definition \ref{categories-definition-directed-set}): $I$ is nonempty and
...
\begin{lemma}[Directed category system]
\label{categories-lemma-directed-category-system}
\source{stacks:0032}
\href{https://stacks.math.columbia.edu/tag/0032}{\texttt{Stacks tag 0032}}
\group{categories}

\uses{categories-lemma-functorial-colimit, categories-lemma-functorial-limit}
...
\colim_\mathcal{I} M
$$
of Lemma \ref{categories-lemma-functorial-colimit} is an isomorphism.
\item For every category $\mathcal{C}$ and every diagram
...
\lim_{i \in I} M(x_i)
$$
of Lemma \ref{categories-lemma-functorial-limit} is an isomorphism.
\end{enumerate}
\end{lemma}

\begin{proof}
\uses{categories-definition-system-over-poset, categories-lemma-cofinal, categories-definition-cofinal}

As explained in the text following
Definition \ref{categories-definition-system-over-poset}, we may view
...
Lemma \ref{categories-lemma-cofinal}, the colimit of a diagram
...
unique final object. Since, by Lemma \ref{categories-lemma-cofinal},
...
Definition \ref{categories-definition-cofinal} is satisfied. Given
...
$$
\begin{array}{ccc} & M_0(\mathcal{F}_{12}) &  \\ M_0(\mathcal{F}_{1}) \nearrow &  & M_0(\mathcal{F}_{2}) \nwarrow \\  & i \nwarrow\nearrow & \end{array}
$$
...
\begin{remark}[Trick needed]
\label{categories-remark-trick-needed}
\source{stacks:09P8}
\href{https://stacks.math.columbia.edu/tag/09P8}{\texttt{Stacks tag 09P8}}
\group{categories}

\uses{categories-lemma-directed-category-system}
...
Lemma \ref{categories-lemma-directed-category-system} is essential.
\end{remark}

\begin{lemma}[Nonempty limit]
\label{categories-lemma-nonempty-limit}
\source{stacks:086J}
\href{https://stacks.math.columbia.edu/tag/086J}{\texttt{Stacks tag 086J}}
\group{categories}
...
Lemma \ref{categories-lemma-directed-category-system}.
...
\begin{definition}[Essentially constant diagram]
\label{categories-definition-essentially-constant-diagram}
\source{stacks:05PU}
\href{https://stacks.math.columbia.edu/tag/05PU}{\texttt{Stacks tag 05PU}}
\group{categories}

\uses{categories-remark-cones-and-cocones, categories-lemma-essentially-constant-is-limit-colimit}
...
Remark \ref{categories-remark-cones-and-cocones}. We say $M$ is
...
Remark \ref{categories-remark-cones-and-cocones}. We say
...
\end{enumerate}
\end{enumerate}
Please keep in mind Lemma \ref{categories-lemma-essentially-constant-is-limit-colimit}
...
Remarks \ref{categories-remark-ind-category} and \ref{categories-remark-pro-category}.
...
\begin{definition}[Essentially constant system]
\label{categories-definition-essentially-constant-system}
\source{stacks:05PV}
\href{https://stacks.math.columbia.edu/tag/05PV}{\texttt{Stacks tag 05PV}}
\group{categories}
...
\begin{lemma}[Essentially constant is limit colimit]
\label{categories-lemma-essentially-constant-is-limit-colimit}
\source{stacks:0G2V}
\href{https://stacks.math.columbia.edu/tag/0G2V}{\texttt{Stacks tag 0G2V}}
\group{categories}
...
\begin{remark}[Ind category]
\label{categories-remark-ind-category}
\source{stacks:05PW}
\href{https://stacks.math.columbia.edu/tag/05PW}{\texttt{Stacks tag 05PW}}
\group{categories}
...
\begin{remark}[Pro category]
\label{categories-remark-pro-category}
\source{stacks:05PX}
\href{https://stacks.math.columbia.edu/tag/05PX}{\texttt{Stacks tag 05PX}}
\group{categories}
...
\begin{example}[Pro morphism inverse systems]
\label{categories-example-pro-morphism-inverse-systems}
\source{stacks:0G2W}
\href{https://stacks.math.columbia.edu/tag/0G2W}{\texttt{Stacks tag 0G2W}}
\group{categories}
...
$$
\begin{array}{ccc}X_{m(n, n')} \longrightarrow\downarrow &  & X_{m(n)} \overset{a_n}{\downarrow} \\ X_{m(n')} \overset{a_{n'}}{\longrightarrow} & Y_{n'} \longrightarrow & Y_n\end{array}
$$
...
diagram
$$
\begin{array}{cccc}\ldots \longrightarrow & X_{m(3)} \overset{a_3}{\downarrow}\longrightarrow & X_{m(2)} \overset{a_2}{\downarrow}\longrightarrow & X_{m(1)} \overset{a_1}{\downarrow} \\ \ldots \longrightarrow & Y_3 \longrightarrow & Y_2 \longrightarrow & Y_1\end{array}
$$
...
\begin{remark}[Pro category copresheaves]
\label{categories-remark-pro-category-copresheaves}
\source{stacks:0G2X}
\href{https://stacks.math.columbia.edu/tag/0G2X}{\texttt{Stacks tag 0G2X}}
\group{categories}

\uses{categories-remark-pro-category}
...
Remark \ref{categories-remark-pro-category}. We omit the construction of a
transformation $B \to A$ given a morphism of pro-objects from $F$ to $G$.
\end{remark}

\begin{lemma}[Image essentially constant]
\label{categories-lemma-image-essentially-constant}
\source{stacks:05SH}
\href{https://stacks.math.columbia.edu/tag/05SH}{\texttt{Stacks tag 05SH}}
\group{categories}
...
\begin{lemma}[Characterize essentially constant ind]
\label{categories-lemma-characterize-essentially-constant-ind}
\source{stacks:05PY}
\href{https://stacks.math.columbia.edu/tag/05PY}{\texttt{Stacks tag 05PY}}
\group{categories}
...
\begin{proof}
It is clear that (3) implies (2). Assume (2).
Then $\text{id}_X \in \Mor_\mathcal{C}(X, X)$
...
\medskip\noindent
Assume (4). Let $k$ be an object of $\mathcal{I}$. Setting $W = M_k$
we deduce there exists a unique morphism $M_k \to X$ such that
there exists a $j$ and morphisms $k \to j$ and
$i \to j$ in $\mathcal{I}$ such that $M_k \to X \to M_i \to M_j$
is equal to $M_k \to M_j$. The uniqueness guarantees that we obtain a
cocone $(X, \{M_k \to X\})$. In this way we see that (4) implies (2);
...
\begin{lemma}[Characterize essentially constant pro]
\label{categories-lemma-characterize-essentially-constant-pro}
\source{stacks:05PZ}
\href{https://stacks.math.columbia.edu/tag/05PZ}{\texttt{Stacks tag 05PZ}}
\group{categories}
...
\begin{proof}
\uses{categories-lemma-characterize-essentially-constant-ind}

This lemma is dual to Lemma \ref{categories-lemma-characterize-essentially-constant-ind}.
\end{proof}

\begin{lemma}[Cofinal essentially constant]
\label{categories-lemma-cofinal-essentially-constant}
\source{stacks:0A1S}
\href{https://stacks.math.columbia.edu/tag/0A1S}{\texttt{Stacks tag 0A1S}}
\group{categories}
...
\begin{proof}
\uses{categories-lemma-characterize-essentially-constant-ind, categories-lemma-cofinal}

This follows formally from
Lemmas \ref{categories-lemma-characterize-essentially-constant-ind} and
\ref{categories-lemma-cofinal}.
\end{proof}

\begin{lemma}[Essentially constant over product]
\label{categories-lemma-essentially-constant-over-product}
\source{stacks:0A2C}
\href{https://stacks.math.columbia.edu/tag/0A2C}{\texttt{Stacks tag 0A2C}}
\group{categories}
...
\begin{proof}
\uses{categories-lemma-cofinal-essentially-constant}

We omit the verification that $\mathcal{I} \times \mathcal{J}$ is
filtered. The equivalence follows from
Lemma \ref{categories-lemma-cofinal-essentially-constant}
because $p$ is cofinal (verification omitted).
\end{proof}

\begin{lemma}[Initial essentially constant]
\label{categories-lemma-initial-essentially-constant}
\source{stacks:0A1T}
\href{https://stacks.math.columbia.edu/tag/0A1T}{\texttt{Stacks tag 0A1T}}
\group{categories}
...
\begin{proof}
\uses{categories-lemma-characterize-essentially-constant-pro, categories-lemma-initial, categories-lemma-cofinal}

This follows formally from
Lemmas \ref{categories-lemma-characterize-essentially-constant-pro},
\ref{categories-lemma-initial}, \ref{categories-lemma-cofinal}, and
...
\source{stacks:0034}
\href{https://stacks.math.columbia.edu/tag/0034}{\texttt{Stacks tag 0034}}
\group{categories}
...
\begin{lemma}[Characterize left exact]
\label{categories-lemma-characterize-left-exact}
\source{stacks:0035}
\href{https://stacks.math.columbia.edu/tag/0035}{\texttt{Stacks tag 0035}}
\group{categories}

\uses{categories-lemma-finite-limits-exist}
Let $F : \mathcal{A} \to \mathcal{B}$ be a functor.
Suppose all finite limits exist in $\mathcal{A}$,
see Lemma \ref{categories-lemma-finite-limits-exist}.
The following are equivalent:
\begin{enumerate}
...
Lemma \ref{categories-lemma-limits-products-equalizers} shows that (2) implies (1).
...
\begin{lemma}[Characterize right exact]
\label{categories-lemma-characterize-right-exact}
\source{stacks:0GMN}
\href{https://stacks.math.columbia.edu/tag/0GMN}{\texttt{Stacks tag 0GMN}}
\group{categories}

\uses{categories-lemma-colimits-exist}
Let $F : \mathcal{A} \to \mathcal{B}$ be a functor.
Suppose all finite colimits exist in $\mathcal{A}$,
see Lemma \ref{categories-lemma-colimits-exist}.
The following are equivalent:
\begin{enumerate}
...
\begin{proof}
\uses{categories-lemma-characterize-left-exact}

Dual to Lemma \ref{categories-lemma-characterize-left-exact}.
\end{proof}
...
\source{stacks:0037}
\href{https://stacks.math.columbia.edu/tag/0037}{\texttt{Stacks tag 0037}}
\group{categories}
...
\begin{lemma}[Adjoint exists]
\label{categories-lemma-adjoint-exists}
\source{stacks:0A8B}
\href{https://stacks.math.columbia.edu/tag/0A8B}{\texttt{Stacks tag 0A8B}}
\group{categories}
...
\begin{proof}
\uses{categories-lemma-yoneda}

For each $y$ choose an object $v(y)$ and an isomorphism
$\Mor_\mathcal{C}(-, v(y)) \to \Mor_\mathcal{D}(u(-), y)$
of functors. By Yoneda's lemma (Lemma \ref{categories-lemma-yoneda})
...
\begin{lemma}[Left adjoint composed fully faithful]
\label{categories-lemma-left-adjoint-composed-fully-faithful}
\source{stacks:0FWV}
\href{https://stacks.math.columbia.edu/tag/0FWV}{\texttt{Stacks tag 0FWV}}
\group{categories}

\uses{categories-definition-adjoint}
\begin{reference}
Bhargav Bhatt, private communication.
\end{reference}
Let $u$ be a left adjoint to $v$ as in Definition \ref{categories-definition-adjoint}.
\begin{enumerate}
...
\begin{lemma}[Adjoint fully faithful]
\label{categories-lemma-adjoint-fully-faithful}
\source{stacks:07RB}
\href{https://stacks.math.columbia.edu/tag/07RB}{\texttt{Stacks tag 07RB}}
\group{categories}
...
\begin{proof}
\uses{categories-lemma-left-adjoint-composed-fully-faithful}
...
Lemma \ref{categories-lemma-left-adjoint-composed-fully-faithful} we see
...
\begin{lemma}[Adjoint exact]
\label{categories-lemma-adjoint-exact}
\source{stacks:0038}
\href{https://stacks.math.columbia.edu/tag/0038}{\texttt{Stacks tag 0038}}
\group{categories}
...
\begin{proof}
\uses{categories-remark-limit-colim}
...
object, see Remark \ref{categories-remark-limit-colim}. So
$$
\begin{matrix}
...
\begin{lemma}[Exact adjoint]
\label{categories-lemma-exact-adjoint}
\source{stacks:0039}
\href{https://stacks.math.columbia.edu/tag/0039}{\texttt{Stacks tag 0039}}
\group{categories}

\uses{categories-definition-adjoint}
Let $u$ be a left adjoint of $v$ as in Definition \ref{categories-definition-adjoint}.
\begin{enumerate}
...
\begin{proof}
\uses{categories-lemma-adjoint-exact}

Obvious from the definitions and Lemma \ref{categories-lemma-adjoint-exact}.
\end{proof}

\begin{lemma}[Unit counit relations]
\label{categories-lemma-unit-counit-relations}
\source{stacks:0GLL}
\href{https://stacks.math.columbia.edu/tag/0GLL}{\texttt{Stacks tag 0GLL}}
\group{categories}
...
\begin{lemma}[Transformation between functors and adjoints]
\label{categories-lemma-transformation-between-functors-and-adjoints}
\source{stacks:0B65}
\href{https://stacks.math.columbia.edu/tag/0B65}{\texttt{Stacks tag 0B65}}
\group{categories}
...
$$
\begin{array}{cc}u_2 \circ v_1 \underset{\beta}{\longrightarrow}\underset{\beta^\vee}{\downarrow} & u_1 \circ v_1 \downarrow \\ u_2 \circ v_2 \longrightarrow & \text{id}\end{array}
$$
...
\begin{lemma}[Compose counits]
\label{categories-lemma-compose-counits}
\source{stacks:0DV0}
\href{https://stacks.math.columbia.edu/tag/0DV0}{\texttt{Stacks tag 0DV0}}
\group{categories}
...
Definition \ref{categories-definition-adjoint}.
...
in Remark \ref{categories-remark-how-to-use-it}.

\begin{lemma}[A version of brown]
\label{categories-lemma-a-version-of-brown}
\source{stacks:0AHN}
\href{https://stacks.math.columbia.edu/tag/0AHN}{\texttt{Stacks tag 0AHN}}
\group{categories}

\uses{categories-remark-big-categories}
Let $\mathcal{C}$ be a big\footnote{See Remark \ref{categories-remark-big-categories}.}
...
$$
see Section \ref{categories-section-opposite}. Let $y$ be an arbitrary object of
...
\source{stacks:0AHP}
\href{https://stacks.math.columbia.edu/tag/0AHP}{\texttt{Stacks tag 0AHP}}
\group{categories}
...
\source{stacks:0AHQ}
\href{https://stacks.math.columbia.edu/tag/0AHQ}{\texttt{Stacks tag 0AHQ}}
\group{categories}
...
\begin{proof}
\uses{categories-lemma-a-version-of-brown, categories-lemma-yoneda}

The assumptions imply that for every object $y$ of $\mathcal{D}$
the functor $x \mapsto \Mor_\mathcal{D}(y, G(x))$ satisfies the
assumptions of Lemma \ref{categories-lemma-a-version-of-brown}.
Thus it is representable by an object, let's call it $F(y)$.
An application of Yoneda's lemma (Lemma \ref{categories-lemma-yoneda})
...
\source{stacks:0FWX}
\href{https://stacks.math.columbia.edu/tag/0FWX}{\texttt{Stacks tag 0FWX}}
\group{categories}
...
\begin{lemma}[Extend functor by colim]
\label{categories-lemma-extend-functor-by-colim}
\source{stacks:0FWY}
\href{https://stacks.math.columbia.edu/tag/0FWY}{\texttt{Stacks tag 0FWY}}
\group{categories}
...
$$
\begin{array}{cc}X_i \longrightarrow\downarrow & X \overset{\alpha}{\downarrow} \\ Y_j \longrightarrow & Y\end{array}
$$
...
$$
\begin{array}{ccc}X_i \longrightarrow\searrow &  & X_\lambda \swarrow \\  & X & \end{array}
$$
...
\source{stacks:04VC}
\href{https://stacks.math.columbia.edu/tag/04VC}{\texttt{Stacks tag 04VC}}
\group{categories}
...
$$
\begin{array}{cc}X \underset{t}{\downarrow}\underset{g}{\longrightarrow} & Y \overset{s}{\downarrow} \\ Z \overset{f}{\dashrightarrow} & W\end{array}
$$
...
$$
\begin{array}{cc}X \underset{t}{\downarrow}\underset{g}{\dashrightarrow} & Y \overset{s}{\downarrow} \\ Z \overset{f}{\longrightarrow} & W\end{array}
$$
...
Lemma \ref{categories-lemma-left-localization}):
\begin{enumerate}
...
$$
\begin{array}{ccc} & Y_1 \overset{u}{\downarrow} &  \\ X \overset{f_1}{\nearrow}\overset{f_3}{\longrightarrow}\underset{f_2}{\searrow} & Y_3 & Y \underset{s_1}{\nwarrow}\underset{s_3}{\longleftarrow}\overset{s_2}{\swarrow} \\  & Y_2 \underset{v}{\uparrow} & \end{array}
$$
...
$$
\begin{array}{cc}Y \underset{s}{\downarrow}\underset{g}{\longrightarrow} & Z' \overset{u}{\downarrow} \\ Y' \overset{h}{\longrightarrow} & Z''\end{array}
$$
...
\begin{lemma}[Left localization]
\label{categories-lemma-left-localization}
\source{stacks:04VD}
\href{https://stacks.math.columbia.edu/tag/04VD}{\texttt{Stacks tag 04VD}}
\group{categories}
...
$$
\begin{array}{ccc}X \downarrow\underset{f_1}{\longrightarrow} & Y_1 \overset{a}{\downarrow} & Y \overset{s_1}{\longleftarrow}\downarrow \\ X \overset{f_2}{\longrightarrow} & Y_2 & Y \underset{s_2}{\longleftarrow}\end{array}
$$
...
$$
\begin{array}{ccc} & Y_1 &  \\ X \overset{f_1}{\nearrow}\overset{f_3}{\longrightarrow}\underset{f_2}{\searrow} & Y_3 \underset{a_{31}}{\uparrow}\overset{a_{32}}{\downarrow} & Y \underset{s_1}{\nwarrow}\underset{s_3}{\longleftarrow}\overset{s_2}{\swarrow} \\  & Y_2 & \end{array}
$$
with $s_i \in S$.
First we apply LMS2 to get a commutative diagram
$$
\begin{array}{cc}Y \underset{s_1}{\downarrow}\underset{s_2}{\longrightarrow} & Y_2 \overset{a_{24}}{\downarrow} \\ Y_1 \overset{a_{14}}{\dashrightarrow} & Y_4\end{array}
$$
...
$$
\begin{array}{ccc}X \downarrow\underset{f_1}{\longrightarrow} & Y_1 \overset{a_{14}}{\downarrow} & Y \overset{s_1}{\longleftarrow}\downarrow \\ X \overset{f_4}{\longrightarrow} & Y_4 & Y \underset{s_4}{\longleftarrow}\end{array}
$$
...
$$
\begin{array}{cc}Y \underset{s}{\downarrow}\underset{g}{\longrightarrow} & Z' \overset{u_2}{\downarrow} \\ Y' \overset{h_2}{\longrightarrow} & Z_2\end{array}
$$
...
$$
\begin{array}{cc}Z' \underset{u_2}{\downarrow}\underset{u_3}{\longrightarrow} & Z_3 \overset{u_{34}}{\downarrow} \\ Z_2 \overset{h_{24}}{\longrightarrow} & Z_4\end{array}
$$
...
commutative diagram
$$
\begin{array}{ccc}X \downarrow\underset{h_2\circ f}{\longrightarrow} & Z_2 \overset{h_{24}}{\downarrow} & Z \overset{u_2 \circ t}{\longleftarrow}\downarrow \\ X \downarrow\overset{h_4\circ f}{\longrightarrow} & Z_4 & Z \downarrow\underset{u_4 \circ t}{\longleftarrow} \\ X \overset{h_3 \circ f}{\longrightarrow} & Z_3 \overset{u_{34}}{\uparrow} & Z \underset{u_3 \circ t}{\longleftarrow}\end{array}
$$
...
$$
\vcenter{
\begin{array}{cc}Y \underset{s_2}{\downarrow}\overset{g}{\longrightarrow} & Z' \overset{u}{\downarrow} \\ Y_2 \overset{h}{\longrightarrow} & Z''\end{array}
}
\quad
\Rightarrow
\quad
\vcenter{
\begin{array}{cc}Y \underset{s_1}{\downarrow}\overset{g}{\longrightarrow} & Z' \overset{u}{\downarrow} \\ Y_1 \overset{h \circ a}{\longrightarrow} & Z''\end{array}
}
$$
(with $u \in S$),
...
$$
\begin{array}{cccc} &  &  & Z \downarrow \\  &  & Y \downarrow\longrightarrow & Z' \downarrow \\  & X \downarrow\longrightarrow & Y' \downarrow\dashrightarrow & Z'' \downarrow \\ W \longrightarrow & X' \dashrightarrow & Y'' \dashrightarrow & Z'''\end{array}
$$
...
\begin{remark}[Motivation localization]
\label{categories-remark-motivation-localization}
\source{stacks:0BM1}
\href{https://stacks.math.columbia.edu/tag/0BM1}{\texttt{Stacks tag 0BM1}}
\group{categories}
...
\begin{definition}[Left localization as fraction]
\label{categories-definition-left-localization-as-fraction}
\source{stacks:0BM2}
\href{https://stacks.math.columbia.edu/tag/0BM2}{\texttt{Stacks tag 0BM2}}
\group{categories}
...
\begin{lemma}[Morphisms left localization]
\label{categories-lemma-morphisms-left-localization}
\source{stacks:04VE}
\href{https://stacks.math.columbia.edu/tag/04VE}{\texttt{Stacks tag 04VE}}
\group{categories}
...
\begin{proof}
\uses{categories-definition-multiplicative-system}
...
$$
\begin{array}{cc}Y \underset{s_1}{\downarrow}\underset{s_2}{\longrightarrow} & Y_2 \overset{t_2}{\downarrow} \\ Y_1 \overset{a_1}{\longrightarrow} & Y'\end{array}
$$
...
\begin{lemma}[Equality morphisms left localization]
\label{categories-lemma-equality-morphisms-left-localization}
\source{stacks:04VF}
\href{https://stacks.math.columbia.edu/tag/04VF}{\texttt{Stacks tag 04VF}}
\group{categories}
...
\begin{proof}
\uses{categories-lemma-left-localization, categories-definition-left-localization-as-fraction}

We are going to use that $S^{-1}\mathcal{C}$ is a category
(Lemma \ref{categories-lemma-left-localization}) and we will
use the notation of Definition \ref{categories-definition-left-localization-as-fraction}
...
$$
\begin{array}{ccc} & Y' \overset{u}{\downarrow} &  \\ X \overset{f}{\nearrow}\overset{h}{\longrightarrow}\underset{g}{\searrow} & Z & Y' \underset{\text{id}_{Y'}}{\nwarrow}\underset{t}{\longleftarrow}\overset{\text{id}_{Y'}}{\swarrow} \\  & Y' \underset{v}{\uparrow} & \end{array}
$$
...
Definition \ref{categories-definition-left-localization-as-fraction} that
...
\begin{remark}[Left localization morphisms colimit]
\label{categories-remark-left-localization-morphisms-colimit}
\source{stacks:05Q0}
\href{https://stacks.math.columbia.edu/tag/05Q0}{\texttt{Stacks tag 05Q0}}
\group{categories}

\uses{categories-definition-directed, categories-lemma-morphisms-left-localization, categories-lemma-equality-morphisms-left-localization}
...
$$
\begin{array}{ccc} & Y \underset{s}{\swarrow}\overset{t}{\searrow} &  \\ Y' \overset{a}{\longrightarrow} &  & Y''\end{array}
$$
where $a : Y' \to Y''$ is arbitrary. We claim that the category
$Y/S$ is filtered (see
Definition \ref{categories-definition-directed}).
...
$$
\begin{array}{cc}Y \underset{s_1}{\downarrow}\underset{s_2}{\longrightarrow} & Y_2 \overset{t}{\downarrow} \\ Y_1 \overset{a}{\longrightarrow} & Y_3\end{array}
$$
...
\begin{lemma}[Properties left localization]
\label{categories-lemma-properties-left-localization}
\source{stacks:04VG}
\href{https://stacks.math.columbia.edu/tag/04VG}{\texttt{Stacks tag 04VG}}
\group{categories}
...
\begin{lemma}[Left localization limits]
\label{categories-lemma-left-localization-limits}
\source{stacks:05Q2}
\href{https://stacks.math.columbia.edu/tag/05Q2}{\texttt{Stacks tag 05Q2}}
\group{categories}
...
Lemma \ref{categories-lemma-directed-commutes} we have
\begin{align*}
...
\begin{lemma}[Left localization diagram]
\label{categories-lemma-left-localization-diagram}
\source{stacks:05Q3}
\href{https://stacks.math.columbia.edu/tag/05Q3}{\texttt{Stacks tag 05Q3}}
\group{categories}
...
$\mathcal{C}$ and if
$$
\begin{array}{cc}Q(X) \underset{Q(f)}{\downarrow}\underset{a}{\longrightarrow} & Q(X') \overset{Q(f')}{\downarrow} \\ Q(Y) \overset{b}{\longrightarrow} & Q(Y')\end{array}
$$
is a commutative diagram in $S^{-1}\mathcal{C}$, then there exist
a morphism $f'' : X'' \to Y''$ in $\mathcal{C}$ and a commutative
diagram
$$
\begin{array}{ccc}X \underset{f}{\downarrow}\underset{g}{\longrightarrow} & X'' \overset{f''}{\downarrow} & X' \overset{f'}{\downarrow}\overset{s}{\longleftarrow} \\ Y \overset{h}{\longrightarrow} & Y'' & Y' \underset{t}{\longleftarrow}\end{array}
$$
in $\mathcal{C}$ with $s, t \in S$ and $a = s^{-1}g$, $b = t^{-1}h$.
\end{lemma}

\begin{proof}
\uses{categories-remark-left-localization-morphisms-colimit, categories-lemma-equality-morphisms-left-localization}
...
$f'' : X'' \to Y''$ such that
$$
\begin{array}{ccc}X \underset{f}{\downarrow}\underset{g}{\longrightarrow} & X'' \overset{f''}{\downarrow} & X' \overset{f'}{\downarrow}\overset{s}{\longleftarrow} \\ Y & Y'' & Y' \underset{t}{\longleftarrow}\end{array}
$$
...
Remark \ref{categories-remark-left-localization-morphisms-colimit}
...
Hence using
Lemma \ref{categories-lemma-equality-morphisms-left-localization}
...
system. We define a new category $S^{-1}\mathcal{C}$ as follows
(we verify this works in the proof of
Lemma \ref{categories-lemma-right-localization}):
\begin{enumerate}
\item We set $\Ob(S^{-1}\mathcal{C}) = \Ob(\mathcal{C})$.
\item Morphisms $X \to Y$ of $S^{-1}\mathcal{C}$ are given by pairs
...
fitting into the commutative diagram
$$
\begin{array}{ccc} & X_1 \underset{s_1}{\swarrow}\overset{f_1}{\searrow} &  \\ X & X_3 \underset{s_3}{\longleftarrow}\underset{u}{\uparrow}\overset{v}{\downarrow}\overset{f_3}{\longrightarrow} & Y \\  & X_2 \overset{s_2}{\nwarrow}\underset{f_2}{\nearrow} & \end{array}
$$
\item The composition of the equivalence classes of the pairs
...
where $h$ and $u \in S$ are chosen to fit into a commutative diagram
$$
\begin{array}{cc}X'' \underset{u}{\downarrow}\overset{h}{\longrightarrow} & Y' \overset{t}{\downarrow} \\ X' \overset{f}{\longrightarrow} & Y\end{array}
$$
which exists by assumption.
\item The identity morphism $X \to X$ in $S^{-1} \mathcal{C}$ is the
equivalence class of the pair $(\text{id} : X \to X,
...
\begin{lemma}[Right localization]
\label{categories-lemma-right-localization}
\source{stacks:04VH}
\href{https://stacks.math.columbia.edu/tag/04VH}{\texttt{Stacks tag 04VH}}
\group{categories}
...
\begin{proof}
\uses{categories-lemma-left-localization}
...
\begin{definition}[Right localization as fraction]
\label{categories-definition-right-localization-as-fraction}
\source{stacks:0BM3}
\href{https://stacks.math.columbia.edu/tag/0BM3}{\texttt{Stacks tag 0BM3}}
\group{categories}
...
\ref{categories-definition-left-localization-as-fraction} hold.
We can ``write any finite collection of morphisms with the same source
as fractions with common denominator''.

\begin{lemma}[Morphisms right localization]
\label{categories-lemma-morphisms-right-localization}
\source{stacks:04VI}
\href{https://stacks.math.columbia.edu/tag/04VI}{\texttt{Stacks tag 04VI}}
\group{categories}
...
\begin{proof}
\uses{categories-lemma-morphisms-left-localization}

This lemma is the dual of
Lemma \ref{categories-lemma-morphisms-left-localization}
...
\begin{lemma}[Equality morphisms right localization]
\label{categories-lemma-equality-morphisms-right-localization}
\source{stacks:04VJ}
\href{https://stacks.math.columbia.edu/tag/04VJ}{\texttt{Stacks tag 04VJ}}
\group{categories}
...
\begin{proof}
\uses{categories-lemma-equality-morphisms-left-localization}

This is dual to
Lemma \ref{categories-lemma-equality-morphisms-left-localization}.
\end{proof}

\begin{remark}[Right localization morphisms colimit]
\label{categories-remark-right-localization-morphisms-colimit}
\source{stacks:05Q4}
\href{https://stacks.math.columbia.edu/tag/05Q4}{\texttt{Stacks tag 05Q4}}
\group{categories}

\uses{categories-definition-codirected, categories-remark-left-localization-morphisms-colimit, categories-lemma-morphisms-right-localization, categories-lemma-equality-morphisms-right-localization}
...
commutative diagrams
$$
\begin{array}{ccc}X' \underset{s}{\searrow}\underset{a}{\longrightarrow} &  & X'' \overset{t}{\swarrow} \\  & X & \end{array}
$$
where $a : X' \to X''$ is arbitrary. The category
$S/X$ is cofiltered (see
Definition \ref{categories-definition-codirected}).
(This is dual to the corresponding statement in
Remark \ref{categories-remark-left-localization-morphisms-colimit}.)
Now the combined results of
Lemmas \ref{categories-lemma-morphisms-right-localization} and
...
\begin{lemma}[Properties right localization]
\label{categories-lemma-properties-right-localization}
\source{stacks:04VK}
\href{https://stacks.math.columbia.edu/tag/04VK}{\texttt{Stacks tag 04VK}}
\group{categories}
...
\begin{proof}
\uses{categories-lemma-properties-left-localization}

This lemma is the dual of
Lemma \ref{categories-lemma-properties-left-localization}
and follows formally from that lemma by replacing all
categories in sight by their opposites.
\end{proof}

\begin{lemma}[Right localization limits]
\label{categories-lemma-right-localization-limits}
\source{stacks:05Q6}
\href{https://stacks.math.columbia.edu/tag/05Q6}{\texttt{Stacks tag 05Q6}}
\group{categories}
...
\begin{proof}
\uses{categories-lemma-left-localization-limits}

This is dual to Lemma \ref{categories-lemma-left-localization-limits}.
\end{proof}

\begin{lemma}[Right localization diagram]
\label{categories-lemma-right-localization-diagram}
\source{stacks:05Q7}
\href{https://stacks.math.columbia.edu/tag/05Q7}{\texttt{Stacks tag 05Q7}}
\group{categories}
...
diagram
$$
\begin{array}{ccc}X \underset{f}{\downarrow} & X'' \overset{s}{\longleftarrow}\overset{f''}{\downarrow}\underset{g}{\longrightarrow} & X' \overset{f'}{\downarrow} \\ Y & Y'' \underset{t}{\longleftarrow}\overset{h}{\longrightarrow} & Y'\end{array}
$$
in $\mathcal{C}$ with $s, t \in S$ and $a = gs^{-1}$, $b = ht^{-1}$.
\end{lemma}

\begin{proof}
\uses{categories-lemma-left-localization-diagram}

This lemma is dual to
Lemma \ref{categories-lemma-left-localization-diagram}.
\end{proof}
...
\begin{lemma}[Multiplicative system]
\label{categories-lemma-multiplicative-system}
\source{stacks:04VL}
\href{https://stacks.math.columbia.edu/tag/04VL}{\texttt{Stacks tag 04VL}}
\group{categories}
...
\begin{proof}
\uses{categories-lemma-properties-left-localization, categories-lemma-properties-right-localization}

Denote $\mathcal{C}_{left}$, $\mathcal{C}_{right}$ the two categories
of fractions. By the universal properties of
Lemmas \ref{categories-lemma-properties-left-localization} and
...
\begin{definition}[Saturated multiplicative system]
\label{categories-definition-saturated-multiplicative-system}
\source{stacks:05Q8}
\href{https://stacks.math.columbia.edu/tag/05Q8}{\texttt{Stacks tag 05Q8}}
\group{categories}
...
\begin{lemma}[What gets inverted]
\label{categories-lemma-what-gets-inverted}
\source{stacks:05Q9}
\href{https://stacks.math.columbia.edu/tag/05Q9}{\texttt{Stacks tag 05Q9}}
\group{categories}
...
\begin{proof}
\uses{categories-lemma-multiplicative-system}
...
Lemma \ref{categories-lemma-multiplicative-system}.)
The relation $\text{id}_X = s^{-1}gf$ in $S^{-1}\mathcal{C}$ means
there exists a commutative diagram
$$
\begin{array}{ccc} & X' \overset{u}{\downarrow} &  \\ X \overset{gf}{\nearrow}\overset{f'}{\longrightarrow}\underset{\text{id}_X}{\searrow} & X'' & X \underset{s}{\nwarrow}\underset{s'}{\longleftarrow}\overset{\text{id}_X}{\swarrow} \\  & X \underset{v}{\uparrow} & \end{array}
$$
...
$$
\begin{array}{cc}X \underset{t}{\downarrow}\underset{g}{\longrightarrow} & Y \overset{s}{\downarrow} \\ Z \underset{a}{\downarrow} &  \\ Z' \overset{f'}{\longrightarrow} & W\end{array}
$$
...
\rruppertwocell^{F''}{t'}
\underset{(}{\longrightarrow}.3){F'}
\rrlowertwocell_F{t} &  & \mathcal{B}\end{array}
\text{ composes to }
\begin{array}{ccc}\mathcal{A}
\rrtwocell^{F''}_F{\ \ t \circ t'} &  & \mathcal{B}\end{array}
$$
...
Of course we also have that ${}_{\text{id}_\mathcal{B}}t = t$.
...
diagram is commutative
$$
\begin{array}{cc}G \circ F \overset{{}_Gt}{\longrightarrow}\underset{s_F}{\downarrow} & G \circ F' \overset{s_{F'}}{\downarrow} \\ G' \circ F \underset{{}_{G'}t}{\longrightarrow} & G' \circ F'\end{array}
$$
in other words ${}_{G'}t \circ s_F = s_{F'}\circ {}_Gt$.
To prove this we just consider what happens on
any object $x \in \Ob(\mathcal{A})$:
$$
\begin{array}{cc}G(F(x)) \overset{G(t_x)}{\longrightarrow}\underset{s_{F(x)}}{\downarrow} & G(F'(x)) \overset{s_{F'(x)}}{\downarrow} \\ G'(F(x)) \underset{G'(t_x)}{\longrightarrow} & G'(F'(x))\end{array}
$$
...
\source{stacks:003E}
\href{https://stacks.math.columbia.edu/tag/003E}{\texttt{Stacks tag 003E}}
\group{categories}

Given a diagram as in the left hand side of:
$$
\begin{array}{ccc}\mathcal{A}
\xrightarrow{\substack{F\\F'\\t}} & \mathcal{B}
\xrightarrow{\substack{G\\G'\\s}} & \mathcal{C}\end{array}
\text{ gives }
\begin{array}{ccc}\mathcal{A}
\rrtwocell^{G \circ F} _{G' \circ F'}{\ \ s \star t} &  & \mathcal{C}\end{array}
$$
...
\begin{lemma}[Properties 2 cat cats]
\label{categories-lemma-properties-2-cat-cats}
\source{stacks:003F}
\href{https://stacks.math.columbia.edu/tag/003F}{\texttt{Stacks tag 003F}}
\group{categories}
...
$$
\begin{array}{ccccc}\mathcal{A}
\rruppertwocell^F{t}
\underset{(}{\longrightarrow}.3){F'}
\rrlowertwocell_{F''}{t'} &  & \mathcal{B}
\rruppertwocell^G{s}
\underset{(}{\longrightarrow}.3){G'}
\rrlowertwocell_{G''}{s'} &  & \mathcal{C}\end{array}
$$
...
see Definition \ref{categories-definition-product-category}.
...
Section \ref{categories-section-formal-cat-cat} and
Example \ref{categories-example-2-1-category-of-categories}.
...
\source{stacks:003H}
\href{https://stacks.math.columbia.edu/tag/003H}{\texttt{Stacks tag 003H}}
\group{categories}
...
\source{stacks:02X7}
\href{https://stacks.math.columbia.edu/tag/02X7}{\texttt{Stacks tag 02X7}}
\group{categories}
...
\begin{remark}[Big 2 categories]
\label{categories-remark-big-2-categories}
\source{stacks:003J}
\href{https://stacks.math.columbia.edu/tag/003J}{\texttt{Stacks tag 003J}}
\group{categories}

\uses{categories-definition-2-1-category}
Big $2$-categories.
...
\item The $2$-category of categories fibred in groupoids over a fixed category;
this is a $(2, 1)$-category.
\item The $2$-category of stacks over a fixed site.
\item The $(2, 1)$-category of stacks over a fixed site.
\item The $2$-category of stacks in groupoids over a fixed site;
this is a $(2, 1)$-category.
\item The $2$-category of stacks in setoids over a fixed site;
this is a $(2, 1)$-category.
\item The $2$-category of algebraic stacks over a fixed scheme;
this is a $(2, 1)$-category.
\end{enumerate}
See Definition \ref{categories-definition-2-1-category}.
...
\ref{categories-section-definition-categories} extends to the more general setting of
$2$-categories as follows.

\begin{definition}[Equivalence]
\label{categories-definition-equivalence}
\source{stacks:003L}
\href{https://stacks.math.columbia.edu/tag/003L}{\texttt{Stacks tag 003L}}
\group{categories}
...
\begin{definition}[Functor into 2 category]
\label{categories-definition-functor-into-2-category}
\source{stacks:003N}
\href{https://stacks.math.columbia.edu/tag/003N}{\texttt{Stacks tag 003N}}
\group{categories}
...
$$
\begin{array}{ccccc}\varphi(x)
\rrtwocell^{\varphi(f)}_{\varphi(f)}{\ \ \ \ \text{id}_{\varphi(f)}} &  & \varphi(y)
\rrtwocell^{\text{id}_{\varphi(y)}}_{\varphi(\text{id}_y)}{\alpha_y} &  & \varphi(y)\end{array}
=
\begin{array}{ccc}\varphi(x)
\rrtwocell^{\varphi(f)}_{\varphi(\text{id}_y) \circ \varphi(f)}{\ \ \ \ \alpha_{\text{id}_y, f}} &  & \varphi(y)\end{array}
$$
\item for any morphism $f : x \to y$ in $\mathcal{A}$ we have
...
$$
\begin{array}{ccc}\varphi(h \circ g \circ f)
\underset{\alpha_{h, g \circ f}}{\downarrow}\underset{\alpha_{h \circ g, f}}{\longrightarrow} &  & \varphi(h \circ g) \circ \varphi(f)
\overset{\alpha_{h, g} \star \text{id}_{\varphi(f)}}{\downarrow} \\ \varphi(h) \circ \varphi(g \circ f)
\overset{\text{id}_{\varphi(h)} \star \alpha_{g, f}}{\longrightarrow} &  & \varphi(h) \circ \varphi(g) \circ \varphi(f)\end{array}
$$
\end{enumerate}
\end{enumerate}
\end{definition}
...
\source{stacks:003I}
\href{https://stacks.math.columbia.edu/tag/003I}{\texttt{Stacks tag 003I}}
\group{categories}
...
\begin{example}[2 1 category of categories]
\label{categories-example-2-1-category-of-categories}
\source{stacks:003K}
\href{https://stacks.math.columbia.edu/tag/003K}{\texttt{Stacks tag 003K}}
\group{categories}

\uses{categories-remark-big-2-categories}
The $2$-category $\textit{Cat}$, see Remark \ref{categories-remark-big-2-categories},
...
\begin{remark}[Other 2 categories]
\label{categories-remark-other-2-categories}
\source{stacks:003M}
\href{https://stacks.math.columbia.edu/tag/003M}{\texttt{Stacks tag 003M}}
\group{categories}

\uses{categories-example-2-1-category-of-categories}
...
$$
\begin{array}{cc}w \longrightarrow\downarrow & y \downarrow \\ x \longrightarrow & z\end{array}
$$
...
$$
\begin{array}{ccc} & w \underset{a}{\longrightarrow}\underset{b}{\downarrow} & x \overset{f}{\downarrow} \\  & y \overset{g}{\longrightarrow} & z.\end{array}
$$
...
such that
$$
\begin{array}{ccc}f \circ a'
\underset{\text{id}_f \star \alpha}{\longrightarrow}\underset{\phi'}{\downarrow} &  & f \circ a \circ k
\overset{\phi \star \text{id}_k}{\downarrow} \\ g \circ b'
\overset{\text{id}_g \star \beta}{\longrightarrow} &  & g \circ b \circ k\end{array}
$$
is commutative,
...
$$
\begin{array}{ccccc}a'
\underset{\alpha_2}{\searrow}\underset{\alpha_1}{\longrightarrow} & a \circ k_1
\overset{\text{id}_a \star \delta}{\downarrow} &  & b \circ k_1
\underset{\text{id}_b \star \delta}{\downarrow} & b'
\overset{\beta_1}{\longleftarrow}\overset{\beta_2}{\swarrow} \\  & a \circ k_2 &  & b \circ k_2 & \end{array}
$$
commute,
...
$$
\begin{array}{ccccc}(w'', a'', b'', \phi'')
\rrtwocell^{(k'_1, \alpha'_1, \beta'_1)}_{(k'_2, \alpha'_2, \beta'_2)}{\delta'} &  & (w', a', b', \phi')
\rrtwocell^{(k_1, \alpha_1, \beta_1)}_{(k_2, \alpha_2, \beta_2)}{\delta} &  & (w, a, b, \phi)\end{array}
$$
is given by the diagram
$$
\begin{array}{cc}(w'', a'', b'', \phi'')
\xrightarrow{\substack{(k_1 \circ k'_1, (\alpha_1 \star \text{id}_{k'_1}) \circ \alpha'_1, (\beta_1 \star \text{id}_{k'_1}) \circ \beta'_1)\\(k_2 \circ k'_2, (\alpha_2 \star \text{id}_{k'_2}) \circ \alpha'_2, (\beta_2 \star \text{id}_{k'_2}) \circ \beta'_2)\\\ \ \ \delta \star \delta'}} & (w, a, b, \phi)\end{array}
$$
\end{enumerate}
...
\begin{definition}[Final object 2 category]
\label{categories-definition-final-object-2-category}
\source{stacks:003P}
\href{https://stacks.math.columbia.edu/tag/003P}{\texttt{Stacks tag 003P}}
\group{categories}
...
\begin{definition}[2 fibre products]
\label{categories-definition-2-fibre-products}
\source{stacks:003Q}
\href{https://stacks.math.columbia.edu/tag/003Q}{\texttt{Stacks tag 003Q}}
\group{categories}
...
$$
\begin{array}{ccc} & x \times_z y \overset{p}{\longrightarrow}\underset{q}{\downarrow} & x \overset{f}{\downarrow} \\  & y \overset{g}{\longrightarrow} & z\end{array}
$$
...
making the diagram
$$
\begin{array}{cccc}w\overset{a}{\searrow}\underset{\gamma}{\searrow}\underset{b}{\searrow} &  &  &  \\  &  & x \times_z y \underset{p}{\longrightarrow}\underset{q}{\downarrow} & x \overset{f}{\downarrow} \\  &  & y \overset{g}{\longrightarrow} & z\end{array}
$$
2-commute such that for suitable choices of
$a \to p \circ \gamma$ and $b \to q \circ \gamma$
the diagram
$$
\begin{array}{cc}f \circ a \longrightarrow\underset{\phi}{\downarrow} & f \circ p \circ \gamma
\overset{\psi \star \text{id}_\gamma}{\downarrow} \\ g\circ b
\longrightarrow & g \circ q \circ \gamma\end{array}
$$
...
\begin{example}[2 fibre product categories]
\label{categories-example-2-fibre-product-categories}
\source{stacks:003R}
\href{https://stacks.math.columbia.edu/tag/003R}{\texttt{Stacks tag 003R}}
\group{categories}
...
$$
\begin{array}{cc}F(A) \overset{f}{\longrightarrow}\overset{F(a)}{\downarrow} & G(B) \overset{G(b)}{\downarrow} \\ F(A') \overset{f'}{\longrightarrow} & G(B')\end{array}
$$
is commutative.
\end{enumerate}
...
\begin{lemma}[2 fibre product categories]
\label{categories-lemma-2-fibre-product-categories}
\source{stacks:02X9}
\href{https://stacks.math.columbia.edu/tag/02X9}{\texttt{Stacks tag 02X9}}
\group{categories}

\uses{categories-example-2-fibre-product-categories}
In the $(2, 1)$-category of categories $2$-fibre products exist and
are given by the construction of
Example \ref{categories-example-2-fibre-product-categories}.
\end{lemma}
...
\begin{remark}[Other description 2 fibre product]
\label{categories-remark-other-description-2-fibre-product}
\source{stacks:06RL}
\href{https://stacks.math.columbia.edu/tag/06RL}{\texttt{Stacks tag 06RL}}
\group{categories}
...
\begin{lemma}[Functoriality 2 fibre product]
\label{categories-lemma-functoriality-2-fibre-product}
\source{stacks:02XA}
\href{https://stacks.math.columbia.edu/tag/02XA}{\texttt{Stacks tag 02XA}}
\group{categories}

Let
$$
\begin{array}{ccc} & \mathcal{Y} \underset{I}{\downarrow}\overset{K}{\searrow} &  \\ \mathcal{X} \overset{H}{\longrightarrow}\overset{L}{\searrow} & \mathcal{Z} \overset{M}{\searrow} & \mathcal{B} \overset{G}{\downarrow} \\  & \mathcal{A} \overset{F}{\longrightarrow} & \mathcal{C}\end{array}
$$
...
\begin{lemma}[Equivalence 2 fibre product]
\label{categories-lemma-equivalence-2-fibre-product}
\source{stacks:02XB}
\href{https://stacks.math.columbia.edu/tag/02XB}{\texttt{Stacks tag 02XB}}
\group{categories}

\uses{categories-lemma-functoriality-2-fibre-product}
Assumptions as in Lemma \ref{categories-lemma-functoriality-2-fibre-product}.
\begin{enumerate}
...
$$
\begin{array}{c}\Mor_\mathcal{X}(X, X')
\times_{\Mor_\mathcal{Z}(Z, Z')}
\Mor_\mathcal{Y}(Y, Y')
\downarrow \\ \Mor_\mathcal{A}(L(X), L(X'))
\times_{\Mor_\mathcal{C}(M(Z), M(Z'))}
\Mor_\mathcal{B}(K(Y), K(Y'))\end{array}
$$
...
\begin{lemma}[Associativity 2 fibre product]
\label{categories-lemma-associativity-2-fibre-product}
\source{stacks:02XC}
\href{https://stacks.math.columbia.edu/tag/02XC}{\texttt{Stacks tag 02XC}}
\group{categories}

Let
$$
\begin{array}{ccccc}\mathcal{A} \searrow &  & \mathcal{C} \swarrow\searrow &  & \mathcal{E} \swarrow \\  & \mathcal{B} &  & \mathcal{D} & \end{array}
$$
...
\begin{lemma}[Triple 2 fibre product pr02]
\label{categories-lemma-triple-2-fibre-product-pr02}
\source{stacks:04S7}
\href{https://stacks.math.columbia.edu/tag/04S7}{\texttt{Stacks tag 04S7}}
\group{categories}

Let
$$
\begin{array}{ccccc}\mathcal{A} \searrow &  & \mathcal{C} \swarrow\searrow &  & \mathcal{E} \swarrow \\  & \mathcal{B} \underset{F}{\searrow} &  & \mathcal{D} \overset{G}{\swarrow} &  \\  &  & \mathcal{F} &  & \end{array}
$$
...
\begin{lemma}[2 fibre product erase factor]
\label{categories-lemma-2-fibre-product-erase-factor}
\source{stacks:02XD}
\href{https://stacks.math.columbia.edu/tag/02XD}{\texttt{Stacks tag 02XD}}
\group{categories}
...
\begin{lemma}[Diagonal 1]
\label{categories-lemma-diagonal-1}
\source{stacks:02XE}
\href{https://stacks.math.columbia.edu/tag/02XE}{\texttt{Stacks tag 02XE}}
\group{categories}

Let
$$
\begin{array}{cc}\mathcal{C}_3 \longrightarrow\downarrow & \mathcal{S} \overset{\Delta}{\downarrow} \\ \mathcal{C}_1 \times \mathcal{C}_2 \overset{G_1 \times G_2}{\longrightarrow} & \mathcal{S} \times \mathcal{S}\end{array}
$$
...
\mathcal{S}$ constructed in Example \ref{categories-example-2-fibre-product-categories}.
...
\begin{lemma}[Diagonal 2]
\label{categories-lemma-diagonal-2}
\source{stacks:02XF}
\href{https://stacks.math.columbia.edu/tag/02XF}{\texttt{Stacks tag 02XF}}
\group{categories}

Let
$$
\begin{array}{cc}\mathcal{C}' \longrightarrow\downarrow & \mathcal{S} \overset{\Delta}{\downarrow} \\ \mathcal{C} \overset{(G_1, G_2)}{\longrightarrow} & \mathcal{S} \times \mathcal{S}\end{array}
$$
be a $2$-fibre product of categories.
Then there is a canonical isomorphism
$$
...
\source{stacks:04Z1}
\href{https://stacks.math.columbia.edu/tag/04Z1}{\texttt{Stacks tag 04Z1}}
\group{categories}
...
Then the diagram
$$
\begin{array}{cc}\mathcal{A} \times_\mathcal{C} \mathcal{B} \downarrow\longrightarrow & \mathcal{A} \times_\mathcal{D} \mathcal{B} \downarrow \\ \mathcal{C} \overset{\Delta_{\mathcal{C}/\mathcal{D}}}{\longrightarrow}\longrightarrow & \mathcal{C} \times_\mathcal{D} \mathcal{C}\end{array}
$$
...
\begin{lemma}[Base change diagonal]
\label{categories-lemma-base-change-diagonal}
\source{stacks:04YR}
\href{https://stacks.math.columbia.edu/tag/04YR}{\texttt{Stacks tag 04YR}}
\group{categories}

Let
$$
\begin{array}{cc}\mathcal{U} \downarrow\longrightarrow & \mathcal{V} \downarrow \\ \mathcal{X} \longrightarrow & \mathcal{Y}\end{array}
$$
be a $2$-fibre product of categories. Then the diagram
$$
\begin{array}{cc}\mathcal{U} \downarrow\longrightarrow & \mathcal{U} \times_\mathcal{V} \mathcal{U} \downarrow \\ \mathcal{X} \longrightarrow & \mathcal{X} \times_\mathcal{Y} \mathcal{X}\end{array}
$$
is $2$-cartesian.
\end{lemma}

\begin{proof}
\uses{categories-lemma-associativity-2-fibre-product, categories-lemma-2-fibre-product-erase-factor}
...
\end{align*}
see
Lemmas \ref{categories-lemma-associativity-2-fibre-product} and
...
\begin{definition}[Categories over C]
\label{categories-definition-categories-over-C}
\source{stacks:003Y}
\href{https://stacks.math.columbia.edu/tag/003Y}{\texttt{Stacks tag 003Y}}
\group{categories}
...
exactly as is done for $\textit{Cat}$ in Section \ref{categories-section-formal-cat-cat}.
...
\begin{definition}[Fibre category]
\label{categories-definition-fibre-category}
\source{stacks:02XH}
\href{https://stacks.math.columbia.edu/tag/02XH}{\texttt{Stacks tag 02XH}}
\group{categories}
...
\begin{lemma}[2 product categories over C]
\label{categories-lemma-2-product-categories-over-C}
\source{stacks:0040}
\href{https://stacks.math.columbia.edu/tag/0040}{\texttt{Stacks tag 0040}}
\group{categories}
...
\item the diagram
$$
\begin{array}{cc}F(x) \overset{f}{\longrightarrow}\overset{F(a)}{\downarrow} & G(y) \overset{G(b)}{\downarrow} \\ F(x') \overset{f'}{\longrightarrow} & G(y')\end{array}
$$
is commutative.
\end{enumerate}
\end{enumerate}
...
\begin{proof}
\uses{categories-lemma-2-fibre-product-categories}
...
Details omitted; compare with Lemma \ref{categories-lemma-2-fibre-product-categories}.
\end{proof}

\begin{example}[Different 2 fibre products]
\label{categories-example-different-2-fibre-products}
\source{stacks:0H2D}
\href{https://stacks.math.columbia.edu/tag/0H2D}{\texttt{Stacks tag 0H2D}}
\group{categories}

\uses{categories-lemma-2-product-categories-over-C, categories-lemma-2-fibre-product-categories, categories-example-2-fibre-product-categories}
The constructions of $2$-fibre products of categories over categories given in
Lemma \ref{categories-lemma-2-product-categories-over-C} and of categories in
Lemma \ref{categories-lemma-2-fibre-product-categories}
(as in Example \ref{categories-example-2-fibre-product-categories})
produce non-equivalent outputs in general.
Namely, let $\mathcal{S}$ be the groupoid category with one object and two
arrows, and let $\mathcal{X}$ be the discrete category with one object.
Taking the $2$-fibre product $\mathcal{X} \times_\mathcal{S} \mathcal{X}$
as categories yields the discrete category with two objects. However, if
we view all of these as categories over $\mathcal{S}$, the
$2$-fiber product $\mathcal{X} \times_\mathcal{S} \mathcal{X}$
as categories over $\mathcal{S}$ is the discrete category with one object.
The difference is that (in the notation of
Lemma \ref{categories-lemma-2-product-categories-over-C}), we were allowed to choose
any comparison isomorphism $f$ in the first situation, but could only
choose the identity arrow in the second situation.
\end{example}

\begin{lemma}[Fibre 2 fibre product categories over C]
\label{categories-lemma-fibre-2-fibre-product-categories-over-C}
\source{stacks:02XI}
\href{https://stacks.math.columbia.edu/tag/02XI}{\texttt{Stacks tag 02XI}}
\group{categories}
...
$$
\begin{array}{ccc}z \overset{p}{\downarrow}\longrightarrow & ? \longrightarrow\overset{p}{\downarrow} & x \overset{p}{\downarrow} \\ p(z) \longrightarrow & V \overset{f}{\longrightarrow} & U\end{array}
$$
If such a morphism $V \times_U x \to x$ exists then it is called
a strongly cartesian morphism.

\begin{definition}[Cartesian over C]
\label{categories-definition-cartesian-over-C}
\source{stacks:02XK}
\href{https://stacks.math.columbia.edu/tag/02XK}{\texttt{Stacks tag 02XK}}
\group{categories}
...
Note that by the Yoneda Lemma \ref{categories-lemma-yoneda}, given
...
\begin{lemma}[Composition cartesian]
\label{categories-lemma-composition-cartesian}
\source{stacks:02XL}
\href{https://stacks.math.columbia.edu/tag/02XL}{\texttt{Stacks tag 02XL}}
\group{categories}
...
\begin{proof}
\uses{categories-definition-cartesian-over-C}
...
Definition \ref{categories-definition-cartesian-over-C}
...
\begin{lemma}[Cartesian over cartesian]
\label{categories-lemma-cartesian-over-cartesian}
\source{stacks:09WU}
\href{https://stacks.math.columbia.edu/tag/09WU}{\texttt{Stacks tag 09WU}}
\group{categories}
...
\begin{lemma}[Strongly cartesian fibre product]
\label{categories-lemma-strongly-cartesian-fibre-product}
\source{stacks:06N4}
\href{https://stacks.math.columbia.edu/tag/06N4}{\texttt{Stacks tag 06N4}}
\group{categories}
...
\source{stacks:02XM}
\href{https://stacks.math.columbia.edu/tag/02XM}{\texttt{Stacks tag 02XM}}
\group{categories}
...
such that
$$
\begin{array}{cc}f^\ast x \underset{f^\ast \phi}{\longrightarrow}\downarrow & f^\ast x' \downarrow \\ x \overset{\phi}{\longrightarrow} & x'\end{array}
$$
...
\begin{definition}[Pullback functor fibred category]
\label{categories-definition-pullback-functor-fibred-category}
\source{stacks:02XN}
\href{https://stacks.math.columbia.edu/tag/02XN}{\texttt{Stacks tag 02XN}}
\group{categories}

\uses{categories-definition-split-fibred-category}
Assume $p : \mathcal{S} \to \mathcal{C}$ is a fibred category.
\begin{enumerate}
...
also with Definition \ref{categories-definition-split-fibred-category}.}
...
\source{stacks:02XO}
\href{https://stacks.math.columbia.edu/tag/02XO}{\texttt{Stacks tag 02XO}}
\group{categories}

\uses{categories-definition-functor-into-2-category}
Assume $p : \mathcal{S} \to \mathcal{C}$ is a fibred category.
...
diagram commutes
$$
\begin{array}{cc}g^\ast f^\ast y \longrightarrow & f^\ast y \downarrow \\ (f \circ g)^\ast y \longrightarrow\overset{(\alpha_{g, f})_y}{\uparrow} & y\end{array}
$$
...
\begin{proof}
\uses{categories-definition-cartesian-over-C, categories-lemma-composition-cartesian}
...
following Definition \ref{categories-definition-cartesian-over-C} and
Lemma \ref{categories-lemma-composition-cartesian}). In the same way,
...
\begin{lemma}[Fibred equivalent]
\label{categories-lemma-fibred-equivalent}
\source{stacks:042G}
\href{https://stacks.math.columbia.edu/tag/042G}{\texttt{Stacks tag 042G}}
\group{categories}
...
The conclusion from Lemma \ref{categories-lemma-fibred-equivalent} is that
...
\begin{definition}[Fibred categories over C]
\label{categories-definition-fibred-categories-over-C}
\source{stacks:02XP}
\href{https://stacks.math.columbia.edu/tag/02XP}{\texttt{Stacks tag 02XP}}
\group{categories}

\uses{categories-definition-categories-over-C}
Let $\mathcal{C}$ be a category.
The {\it $2$-category of fibred categories over $\mathcal{C}$}
is the sub $2$-category of the $2$-category of categories
over $\mathcal{C}$ (see Definition \ref{categories-definition-categories-over-C})
defined as follows:
\begin{enumerate}
...
\begin{lemma}[2 product fibred categories over C]
\label{categories-lemma-2-product-fibred-categories-over-C}
\source{stacks:02XQ}
\href{https://stacks.math.columbia.edu/tag/02XQ}{\texttt{Stacks tag 02XQ}}
\group{categories}

\uses{categories-lemma-2-product-categories-over-C}
Let $\mathcal{C}$ be a category.
...
described in Lemma \ref{categories-lemma-2-product-categories-over-C} is fibred.
...
\source{stacks:02XR}
\href{https://stacks.math.columbia.edu/tag/02XR}{\texttt{Stacks tag 02XR}}
\group{categories}
...
\begin{proof}
\uses{categories-definition-fibred-category}
...
Definition \ref{categories-definition-fibred-category} are satisfied and we win.
\end{proof}

\begin{lemma}[Fibred over fibred]
\label{categories-lemma-fibred-over-fibred}
\source{stacks:09WV}
\href{https://stacks.math.columbia.edu/tag/09WV}{\texttt{Stacks tag 09WV}}
\group{categories}
...
\begin{proof}
\uses{categories-lemma-cartesian-over-cartesian}

This follows from the definitions and
Lemma \ref{categories-lemma-cartesian-over-cartesian}.
\end{proof}

\begin{lemma}[Fibred category representable goes up]
\label{categories-lemma-fibred-category-representable-goes-up}
\source{stacks:06N5}
\href{https://stacks.math.columbia.edu/tag/06N5}{\texttt{Stacks tag 06N5}}
\group{categories}
...
\begin{proof}
\uses{categories-lemma-strongly-cartesian-fibre-product}
...
Lemma \ref{categories-lemma-strongly-cartesian-fibre-product}.
\end{proof}

\begin{lemma}[Ameliorate morphism fibred categories]
\label{categories-lemma-ameliorate-morphism-fibred-categories}
\source{stacks:08NF}
\href{https://stacks.math.columbia.edu/tag/08NF}{\texttt{Stacks tag 08NF}}
\group{categories}
...
$$
\begin{array}{ccc}\mathcal{X} \overset{u}{\longrightarrow} & \mathcal{X}' \overset{v}{\longrightarrow}\overset{w}{\longleftarrow} & \mathcal{Y}\end{array}
$$
...
$$
\begin{array}{ccc}y' \underset{f'}{\downarrow}\longrightarrow & h^*y \longrightarrow\underset{h^*f}{\downarrow} & y \underset{f}{\downarrow} \\ F(x') \longrightarrow & F(h^*x) \longrightarrow & F(x)\end{array}
$$
...
\begin{lemma}[Inertia fibred category]
\label{categories-lemma-inertia-fibred-category}
\source{stacks:034H}
\href{https://stacks.math.columbia.edu/tag/034H}{\texttt{Stacks tag 034H}}
\group{categories}
...
$\phi : x \to y$ such that
$$
\begin{array}{cc}x\underset{\phi}{\longrightarrow}\underset{\alpha}{\downarrow} & y\overset{\beta}{\downarrow} \\ x\overset{\phi}{\longrightarrow} & y \\  & \end{array}
$$
commutes, and
...
\begin{proof}
\uses{categories-lemma-2-product-fibred-categories-over-C, categories-lemma-fibred-equivalent}

Note that (2) follows from (1) by
Lemmas \ref{categories-lemma-2-product-fibred-categories-over-C} and
\ref{categories-lemma-fibred-equivalent}. Thus it suffices to prove (1).
We will use without further mention the construction of the $2$-fibre product
from
Lemma \ref{categories-lemma-2-product-fibred-categories-over-C}.
...
\begin{definition}[Inertia fibred category]
\label{categories-definition-inertia-fibred-category}
\source{stacks:034I}
\href{https://stacks.math.columbia.edu/tag/034I}{\texttt{Stacks tag 034I}}
\group{categories}

\uses{categories-lemma-inertia-fibred-category}
Let $\mathcal{C}$ be a category.
\begin{enumerate}
...
Lemma \ref{categories-lemma-inertia-fibred-category}.
...
square
$$
\begin{array}{cc}\mathcal{S}_1 \underset{F_1}{\downarrow}\underset{G}{\longrightarrow} & \mathcal{S}_2 \overset{F_2}{\downarrow} \\ \mathcal{S}'_1 \overset{G'}{\longrightarrow} & \mathcal{S}'_2\end{array}
$$
...
\begin{lemma}[Relative inertia as fibre product]
\label{categories-lemma-relative-inertia-as-fibre-product}
\source{stacks:04Z6}
\href{https://stacks.math.columbia.edu/tag/04Z6}{\texttt{Stacks tag 04Z6}}
\group{categories}

Let $F : \mathcal{S} \to \mathcal{S}'$ be a $1$-morphism of categories
fibred over a category $\mathcal{C}$. Then the diagram
$$
\begin{array}{ccc}\mathcal{I}_{\mathcal{S}/\mathcal{S}'}
\underset{F \circ (\ref{equation-inertia-structure-map})}{\downarrow}\underset{(\ref{equation-comparison})}{\longrightarrow} &  & \mathcal{I}_\mathcal{S} \overset{(\ref{equation-functorial})}{\downarrow} \\ \mathcal{S}' \overset{e}{\longrightarrow} &  & \mathcal{I}_{\mathcal{S}'}\end{array}
$$
...
\begin{definition}[Fibred groupoids]
\label{categories-definition-fibred-groupoids}
\source{stacks:003T}
\href{https://stacks.math.columbia.edu/tag/003T}{\texttt{Stacks tag 003T}}
\group{categories}
...
$$
\begin{array}{cccc}y \longrightarrow & x & p(y) \longrightarrow & p(x) \\ z \uparrow\nearrow &  & p(z) \uparrow\nearrow &  \\  &  &  & \end{array}
$$
...
\vcenter{
\begin{array}{ccc}z' \downarrow\overset{\gamma}{\searrow} &  &  \\ z \uparrow\overset{\psi}{\longrightarrow}\overset{p}{\downarrow} & y \overset{\phi}{\longrightarrow}\overset{p}{\downarrow} & x \overset{p}{\downarrow} \\ W \overset{g}{\longrightarrow} & V \overset{f}{\longrightarrow} & U \\  &  & \end{array}
}
\end{equation}
...
\begin{lemma}[Fibred groupoids]
\label{categories-lemma-fibred-groupoids}
\source{stacks:003V}
\href{https://stacks.math.columbia.edu/tag/003V}{\texttt{Stacks tag 003V}}
\group{categories}

\uses{categories-lemma-fibred}
Let $p : \mathcal{S} \to \mathcal{C}$ be a functor.
The following are equivalent
\begin{enumerate}
...
constructed in Lemma \ref{categories-lemma-fibred}
defines a pseudo functor from $\mathcal{C}^{opp}$ in to
the $(2, 1)$-category of groupoids.
\end{lemma}

\begin{proof}
\uses{categories-definition-fibred-groupoids, categories-lemma-fibred}
...
$$
\begin{array}{cccc}y \overset{f}{\longrightarrow} & x & U \overset{\text{id}_U}{\longrightarrow} & U \\ x \uparrow\underset{\text{id}_x}{\nearrow} &  & U \uparrow\underset{\text{id}_U}{\nearrow} &  \\  &  &  & \end{array}
$$
...
so $h = f$. Condition (2) of Definition \ref{categories-definition-fibred-groupoids} says
exactly that every morphism of $\mathcal{S}$ is strongly cartesian. Hence
condition (1) of Definition \ref{categories-definition-fibred-groupoids} implies that
$\mathcal{S}$ is a fibred category over $\mathcal{C}$.
...
The final statement follows directly from Lemma \ref{categories-lemma-fibred}.
\end{proof}

\begin{lemma}[Fibred gives fibred groupoids]
\label{categories-lemma-fibred-gives-fibred-groupoids}
\source{stacks:03WQ}
\href{https://stacks.math.columbia.edu/tag/03WQ}{\texttt{Stacks tag 03WQ}}
\group{categories}
...
\begin{proof}
\uses{categories-lemma-composition-cartesian, categories-definition-fibred-groupoids, categories-definition-cartesian-over-C}

Note that the construction makes sense since by
Lemma \ref{categories-lemma-composition-cartesian}
...
Lemma \ref{categories-lemma-composition-cartesian} again).
\end{proof}

\begin{example}[Group homomorphism fibreedingroupoids]
\label{categories-example-group-homomorphism-fibreedingroupoids}
\source{stacks:003U}
\href{https://stacks.math.columbia.edu/tag/003U}{\texttt{Stacks tag 003U}}
\group{categories}

\uses{categories-example-group-homomorphism-functor, categories-example-group-groupoid}
A homomorphism of groups $p : G \to H$ gives rise to a functor
$p : \mathcal{S}\to\mathcal{C}$ as in Example
\ref{categories-example-group-homomorphism-functor}. This functor
...
kernel of $p$ as in Example \ref{categories-example-group-groupoid}.
\end{example}
...
\ref{categories-example-group-homomorphism-fibreedingroupoids} when $G \to H$ is not
surjective. Here is another example.

\begin{example}[Not fibred in groupoids but fibre cats are]
\label{categories-example-not-fibred-in-groupoids-but-fibre-cats-are}
\source{stacks:02C4}
\href{https://stacks.math.columbia.edu/tag/02C4}{\texttt{Stacks tag 02C4}}
\group{categories}
...
two lifts.
$$
\begin{array}{cccccc}B' \overset{g'}{\longrightarrow} & T' &  & B \overset{g}{\longrightarrow} & T &  \\ A' \overset{??}{\uparrow}\underset{h'}{\nearrow} &  & \overset{above}{\uparrow} & A \overset{f}{\uparrow}\underset{gf = h}{\nearrow} &  &  \\  &  &  &  &  & \end{array}
$$
\end{example}
...
\begin{definition}[Categories fibred in groupoids over C]
\label{categories-definition-categories-fibred-in-groupoids-over-C}
\source{stacks:02XS}
\href{https://stacks.math.columbia.edu/tag/02XS}{\texttt{Stacks tag 02XS}}
\group{categories}

\uses{categories-definition-fibred-categories-over-C}
Let $\mathcal{C}$ be a category.
...
over $\mathcal{C}$ (see Definition \ref{categories-definition-fibred-categories-over-C})
defined as follows:
\begin{enumerate}
...
\begin{lemma}[2 product fibred categories]
\label{categories-lemma-2-product-fibred-categories}
\source{stacks:0041}
\href{https://stacks.math.columbia.edu/tag/0041}{\texttt{Stacks tag 0041}}
\group{categories}
...
\begin{proof}
\uses{categories-lemma-2-product-fibred-categories-over-C, categories-lemma-2-product-categories-over-C, categories-lemma-fibred-groupoids, categories-lemma-fibre-2-fibre-product-categories-over-C, categories-lemma-2-fibre-product-categories}

By Lemma \ref{categories-lemma-2-product-fibred-categories-over-C}
the fibre product as described in
Lemma \ref{categories-lemma-2-product-categories-over-C} is a fibred category.
Hence it suffices to prove that the fibre categories are
groupoids, see Lemma \ref{categories-lemma-fibred-groupoids}.
By Lemma \ref{categories-lemma-fibre-2-fibre-product-categories-over-C}
it is enough to show that the $2$-fibre product of groupoids
is a groupoid, which is clear (from the construction in
Lemma \ref{categories-lemma-2-fibre-product-categories} for example).
\end{proof}

\begin{remark}[2 product fibred categories same]
\label{categories-remark-2-product-fibred-categories-same}
\source{stacks:0H2E}
\href{https://stacks.math.columbia.edu/tag/0H2E}{\texttt{Stacks tag 0H2E}}
\group{categories}

\uses{categories-lemma-2-product-fibred-categories, categories-example-2-fibre-product-categories, categories-lemma-2-product-categories-over-C}
Let $\mathcal{C}$ be a category. Let $f : \mathcal{X} \to \mathcal{S}$
and $g : \mathcal{Y} \to \mathcal{S}$ be $1$-morphisms of
categories fibred in groupoids over $\mathcal{C}$.
Let $p : \mathcal{S} \to \mathcal{C}$ be the given functor.
We claim the $2$-fibre product of Lemma \ref{categories-lemma-2-product-fibred-categories}
is canonically equivalent (as a category) to the one in
Example \ref{categories-example-2-fibre-product-categories}.
Objects of the former are quadruples $(U, x, y, \alpha)$
where $p(\alpha) = \text{id}_U$
(see Lemma \ref{categories-lemma-2-product-categories-over-C})
and objects of the latter are triples $(x, y, \alpha)$ (see
Example \ref{categories-example-2-fibre-product-categories}).
The equivalence between the two categories is given by
the rules $(U, x, y, \alpha) \mapsto (x, y, \alpha)$
and $(x, y, \alpha) \mapsto (p(f(x)), x, y', \alpha')$
where $\alpha' = g(\gamma)^{-1} \circ \alpha$ and $\gamma : y' \to y$
is a lift of the arrow $p(\alpha) : p(f(x)) \to p(g(y))$.
Details omitted.
\end{remark}

\begin{lemma}[Equivalence fibred categories]
\label{categories-lemma-equivalence-fibred-categories}
\source{stacks:003Z}
\href{https://stacks.math.columbia.edu/tag/003Z}{\texttt{Stacks tag 003Z}}
\group{categories}
...
\begin{proof}
\uses{categories-lemma-equivalence-categories}

Let $x, y$ be objects of $\mathcal{S}$ lying over the same object $U$.
Consider the commutative diagram
$$
\begin{array}{ccc}\Mor_\mathcal{S}(x, y) \underset{p}{\searrow}\underset{G}{\longrightarrow} &  & \Mor_{\mathcal{S}'}(G(x), G(y)) \overset{p'}{\swarrow} \\  & \Mor_\mathcal{C}(U, U) & \end{array}
$$
...
Lemma \ref{categories-lemma-equivalence-categories}) to produce an inverse
...
\begin{lemma}[Fully faithful diagonal equivalence]
\label{categories-lemma-fully-faithful-diagonal-equivalence}
\source{stacks:04Z7}
\href{https://stacks.math.columbia.edu/tag/04Z7}{\texttt{Stacks tag 04Z7}}
\group{categories}
...
\begin{proof}
\uses{categories-lemma-equivalence-fibred-categories}

By
Lemma \ref{categories-lemma-equivalence-fibred-categories}
...
\begin{lemma}[Morphisms equivalent fibred groupoids]
\label{categories-lemma-morphisms-equivalent-fibred-groupoids}
\source{stacks:03YT}
\href{https://stacks.math.columbia.edu/tag/03YT}{\texttt{Stacks tag 03YT}}
\group{categories}
...
\begin{lemma}[Inertia fibred groupoids]
\label{categories-lemma-inertia-fibred-groupoids}
\source{stacks:042I}
\href{https://stacks.math.columbia.edu/tag/042I}{\texttt{Stacks tag 042I}}
\group{categories}
...
\begin{proof}
\uses{categories-lemma-inertia-fibred-category, categories-lemma-2-product-fibred-categories}
...
Lemma \ref{categories-lemma-2-product-fibred-categories}.
\end{proof}

\begin{lemma}[Cute groupoids]
\label{categories-lemma-cute-groupoids}
\source{stacks:02XT}
\href{https://stacks.math.columbia.edu/tag/02XT}{\texttt{Stacks tag 02XT}}
\group{categories}
...
\begin{proof}
\uses{categories-lemma-cute, categories-lemma-fibred-groupoids}

We have already seen in Lemma \ref{categories-lemma-cute} that $p'$ is a fibred
category. Hence it suffices to prove the fibre categories are groupoids,
see Lemma \ref{categories-lemma-fibred-groupoids}.
...
\begin{lemma}[Fibred in groupoids over fibred in groupoids]
\label{categories-lemma-fibred-in-groupoids-over-fibred-in-groupoids}
\source{stacks:09WW}
\href{https://stacks.math.columbia.edu/tag/09WW}{\texttt{Stacks tag 09WW}}
\group{categories}
...
\begin{proof}
\uses{categories-lemma-fibred-groupoids, categories-lemma-fibred-over-fibred, categories-lemma-composition-cartesian}

One can prove this directly from the definition. However, we will argue
using the criterion of Lemma \ref{categories-lemma-fibred-groupoids}.
By Lemma \ref{categories-lemma-fibred-over-fibred} we see that $\mathcal{A}$
...
Lemma \ref{categories-lemma-composition-cartesian} and the fact that every
morphism of $\mathcal{A}$ is strongly $\mathcal{B}$-cartesian
(see Lemma \ref{categories-lemma-fibred-groupoids}).
\end{proof}

\begin{lemma}[Fibred groupoids fibre product goes up]
\label{categories-lemma-fibred-groupoids-fibre-product-goes-up}
\source{stacks:06N6}
\href{https://stacks.math.columbia.edu/tag/06N6}{\texttt{Stacks tag 06N6}}
\group{categories}
...
\begin{proof}
\uses{categories-lemma-fibred-category-representable-goes-up}

Follows from
Lemma \ref{categories-lemma-fibred-category-representable-goes-up}.
\end{proof}

\begin{lemma}[Ameliorate morphism categories fibred groupoids]
\label{categories-lemma-ameliorate-morphism-categories-fibred-groupoids}
\source{stacks:06N7}
\href{https://stacks.math.columbia.edu/tag/06N7}{\texttt{Stacks tag 06N7}}
\group{categories}
...
\begin{proof}
\uses{categories-lemma-2-product-categories-over-C, categories-lemma-2-product-fibred-categories, categories-definition-fibred-groupoids}
...
\begin{lemma}[Amelioration unique]
\label{categories-lemma-amelioration-unique}
\source{stacks:06N8}
\href{https://stacks.math.columbia.edu/tag/06N8}{\texttt{Stacks tag 06N8}}
\group{categories}
...
$$
\begin{array}{ccc}\mathcal{X}' \underset{f}{\searrow} & \mathcal{X} \overset{a}{\longleftarrow}\overset{F}{\downarrow}\underset{b}{\longrightarrow} & \mathcal{X}'' \overset{g}{\swarrow} \\  & \mathcal{Y} & \end{array}
$$
...
\begin{proof}
\uses{categories-lemma-ameliorate-morphism-categories-fibred-groupoids, categories-lemma-equivalence-2-fibre-product, categories-lemma-equivalence-fibred-categories}
...
Lemma \ref{categories-lemma-ameliorate-morphism-categories-fibred-groupoids}.
...
$$
\begin{array}{cc}\mathcal{X} \underset{F}{\downarrow} & \mathcal{X}'' \overset{b^{-1}}{\longleftarrow}\overset{g}{\downarrow} \\ \mathcal{Y} & \mathcal{Y} \longleftarrow\end{array}
$$
...
Lemma \ref{categories-lemma-equivalence-2-fibre-product}).
...
\source{stacks:02XV}
\href{https://stacks.math.columbia.edu/tag/02XV}{\texttt{Stacks tag 02XV}}
\group{categories}
...
determines $(h, \nu)$ on the right:
$$
\begin{array}{cccc}V \overset{f}{\longrightarrow} & U & (V, f^*x) \overset{(f, \text{id}_{f^*x})}{\longrightarrow} & (U, x) \\ W \overset{h}{\uparrow}\underset{g}{\nearrow} &  & (W, z) \overset{(h, \nu)}{\uparrow}\underset{(g, \psi)}{\nearrow} & \end{array}
$$
...
\begin{definition}[Split fibred category]
\label{categories-definition-split-fibred-category}
\source{stacks:02XW}
\href{https://stacks.math.columbia.edu/tag/02XW}{\texttt{Stacks tag 02XW}}
\group{categories}

\uses{categories-example-functor-categories}
Let $\mathcal{C}$ be a category.
Suppose that $F : \mathcal{C}^{opp} \to \textit{Cat}$ is a functor
to the $2$-category of categories.
We will write $p_F : \mathcal{S}_F \to \mathcal{C}$ for the
fibred category constructed in
Example \ref{categories-example-functor-categories}.
...
\source{stacks:02XX}
\href{https://stacks.math.columbia.edu/tag/02XX}{\texttt{Stacks tag 02XX}}
\group{categories}

\uses{categories-definition-pullback-functor-fibred-category}
Let $\mathcal{C}$ be a category.
Let $\mathcal{S}$ be a fibred category over $\mathcal{C}$.
Then $\mathcal{S}$ is split if and only if for some choice
of pullbacks (see Definition \ref{categories-definition-pullback-functor-fibred-category})
...
\begin{lemma}[Fibred strict]
\label{categories-lemma-fibred-strict}
\source{stacks:004A}
\href{https://stacks.math.columbia.edu/tag/004A}{\texttt{Stacks tag 004A}}
\group{categories}
...
\begin{proof}
\uses{categories-definition-pullback-functor-fibred-category, categories-lemma-fibred}

Let us make a choice of pullbacks (see
Definition \ref{categories-definition-pullback-functor-fibred-category}).
By Lemma \ref{categories-lemma-fibred} we get pullback functors $f^*$ for
...
\ref{categories-lemma-fibred} to think of $g^\ast\varphi$ as a morphism from
...
\begin{example}[Functor groupoids]
\label{categories-example-functor-groupoids}
\source{stacks:0049}
\href{https://stacks.math.columbia.edu/tag/0049}{\texttt{Stacks tag 0049}}
\group{categories}

\uses{categories-example-functor-categories, categories-definition-functor-into-2-category, categories-lemma-fibred-groupoids, categories-definition-fibred-groupoids}
...
$$
\begin{array}{cccc}V \overset{f}{\longrightarrow} & U & (V, y) \overset{(f, \phi)}{\longrightarrow} & (U, x) \\ W \overset{h}{\uparrow}\underset{g}{\nearrow} &  & (W, z) \overset{(h, \nu)}{\uparrow}\underset{(g, \psi)}{\nearrow} &  \\  &  &  & \end{array}
$$
...
\begin{definition}[Split category fibred in groupoids]
\label{categories-definition-split-category-fibred-in-groupoids}
\source{stacks:04TL}
\href{https://stacks.math.columbia.edu/tag/04TL}{\texttt{Stacks tag 04TL}}
\group{categories}

\uses{categories-example-functor-groupoids}
Let $\mathcal{C}$ be a category.
Suppose that $F : \mathcal{C}^{opp} \to \textit{Groupoids}$ is a functor
to the $2$-category of groupoids.
We will write $p_F : \mathcal{S}_F \to \mathcal{C}$ for the
category fibred in groupoids constructed in
Example \ref{categories-example-functor-groupoids}.
A {\it split category fibred in groupoids} is a
category fibred in groupoids isomorphic (!)
over $\mathcal{C}$ to one of these categories {\it $\mathcal{S}_F$}.
\end{definition}

\begin{lemma}[Fibred groupoids strict]
\label{categories-lemma-fibred-groupoids-strict}
\source{stacks:02XY}
\href{https://stacks.math.columbia.edu/tag/02XY}{\texttt{Stacks tag 02XY}}
\group{categories}
...
\begin{proof}
\uses{categories-definition-pullback-functor-fibred-category, categories-lemma-fibred, categories-lemma-fibred-groupoids}

Make a choice of pullbacks (see
Definition \ref{categories-definition-pullback-functor-fibred-category}).
By Lemmas \ref{categories-lemma-fibred} and \ref{categories-lemma-fibred-groupoids}
...
(g^\ast\varphi, \text{id}_{V'})$ where we use the unique identifications
$g^\ast f_i^\ast x_i = (f_i \circ g)^\ast x_i$ from Lemma
\ref{categories-lemma-fibred-groupoids} to think of $g^\ast\varphi$ as a morphism from
$(f_1 \circ g)^\ast x_1$ to $(f_2 \circ g)^\ast x_2$. Clearly, these pullback
functors $g^\ast$ have the property that
$g_1^\ast \circ g_2^\ast = (g_2\circ g_1)^\ast$, in other words $\mathcal{S}'$
is split as desired.
...
\source{stacks:02Y0}
\href{https://stacks.math.columbia.edu/tag/02Y0}{\texttt{Stacks tag 02Y0}}
\group{categories}
...
\begin{definition}[Category fibred sets]
\label{categories-definition-category-fibred-sets}
\source{stacks:0043}
\href{https://stacks.math.columbia.edu/tag/0043}{\texttt{Stacks tag 0043}}
\group{categories}
...
and presheaves (see Definition \ref{categories-definition-presheaf}).
To do this it makes sense to first make the following definition.

\begin{definition}[Categories fibred in sets over C]
\label{categories-definition-categories-fibred-in-sets-over-C}
\source{stacks:04S8}
\href{https://stacks.math.columbia.edu/tag/04S8}{\texttt{Stacks tag 04S8}}
\group{categories}

\uses{categories-definition-categories-fibred-in-groupoids-over-C}
Let $\mathcal{C}$ be a category.
...
\begin{lemma}[2 product categories fibred sets]
\label{categories-lemma-2-product-categories-fibred-sets}
\source{stacks:0047}
\href{https://stacks.math.columbia.edu/tag/0047}{\texttt{Stacks tag 0047}}
\group{categories}
...
\begin{example}[Presheaf]
\label{categories-example-presheaf}
\source{stacks:04TM}
\href{https://stacks.math.columbia.edu/tag/04TM}{\texttt{Stacks tag 04TM}}
\group{categories}

\uses{categories-example-functor-categories, categories-example-functor-groupoids}
This example is the analogue of
Examples \ref{categories-example-functor-categories} and
...
\begin{lemma}[Categories fibred in sets are precisely presheaves.]
\label{categories-lemma-2-category-fibred-sets}
\source{stacks:02Y2}
\href{https://stacks.math.columbia.edu/tag/02Y2}{\texttt{Stacks tag 02Y2}}
\group{categories}


\uses{categories-example-presheaf}
Let $\mathcal{C}$ be a category.
...
\ref{categories-lemma-fibred-groupoids}. It follows that we may think of the
...
\begin{example}[Fibred category from functor of points]
\label{categories-example-fibred-category-from-functor-of-points}
\source{stacks:0044}
\href{https://stacks.math.columbia.edu/tag/0044}{\texttt{Stacks tag 0044}}
\group{categories}

\uses{categories-example-hom-functor, categories-example-category-over-X, categories-lemma-2-category-fibred-sets, categories-example-presheaf}
Let $\mathcal{C}$ be a category. Let $X \in \Ob(\mathcal{C})$.
Consider the representable presheaf $h_X = \Mor_\mathcal{C}(-, X)$
(see Example \ref{categories-example-hom-functor}).
On the other hand, consider the category $p : \mathcal{C}/X \to \mathcal{C}$
from Example \ref{categories-example-category-over-X}.
...
Lemma \ref{categories-lemma-2-category-fibred-sets}
we have
$$
...
\source{stacks:02XZ}
\href{https://stacks.math.columbia.edu/tag/02XZ}{\texttt{Stacks tag 02XZ}}
\group{categories}
...
\begin{definition}[Category fibred setoids]
\label{categories-definition-category-fibred-setoids}
\source{stacks:04SA}
\href{https://stacks.math.columbia.edu/tag/04SA}{\texttt{Stacks tag 04SA}}
\group{categories}
...
\begin{definition}[Categories fibred in setoids over C]
\label{categories-definition-categories-fibred-in-setoids-over-C}
\source{stacks:02Y1}
\href{https://stacks.math.columbia.edu/tag/02Y1}{\texttt{Stacks tag 02Y1}}
\group{categories}
...
\begin{lemma}[2 product categories fibred setoids]
\label{categories-lemma-2-product-categories-fibred-setoids}
\source{stacks:04SB}
\href{https://stacks.math.columbia.edu/tag/04SB}{\texttt{Stacks tag 04SB}}
\group{categories}
...
has 2-fibre products. More precisely, the 2-fibre product described in
Lemma \ref{categories-lemma-2-product-categories-over-C} returns a category fibred in
...
\begin{lemma}[Setoid fibres]
\label{categories-lemma-setoid-fibres}
\source{stacks:0045}
\href{https://stacks.math.columbia.edu/tag/0045}{\texttt{Stacks tag 0045}}
\group{categories}
...
by Lemma \ref{categories-lemma-2-category-fibred-sets}.

\begin{lemma}[2 category fibred setoids]
\label{categories-lemma-2-category-fibred-setoids}
\source{stacks:04SC}
\href{https://stacks.math.columbia.edu/tag/04SC}{\texttt{Stacks tag 04SC}}
\group{categories}

\uses{categories-lemma-setoid-fibres, categories-definition-functor-into-2-category}
Let $\mathcal{C}$ be a category. The construction of
Lemma \ref{categories-lemma-setoid-fibres}
...
\begin{proof}
\uses{categories-lemma-2-category-fibred-sets, categories-lemma-setoid-fibres}

By
Lemma \ref{categories-lemma-2-category-fibred-sets}
the target of $F$ is a category hence the assertion makes sense.
The construction of
Lemma \ref{categories-lemma-setoid-fibres} part (2)
...
\begin{lemma}[Characterize fibred setoids inertia]
\label{categories-lemma-characterize-fibred-setoids-inertia}
\source{stacks:042J}
\href{https://stacks.math.columbia.edu/tag/042J}{\texttt{Stacks tag 042J}}
\group{categories}
...
\begin{lemma}[Category fibred setoids presheaves products]
\label{categories-lemma-category-fibred-setoids-presheaves-products}
\source{stacks:04SD}
\href{https://stacks.math.columbia.edu/tag/04SD}{\texttt{Stacks tag 04SD}}
\group{categories}

\uses{categories-lemma-2-category-fibred-setoids}
Let $\mathcal{C}$ be a category.
The construction of
Lemma \ref{categories-lemma-2-category-fibred-setoids}
...
\begin{definition}[Representable fibred category]
\label{categories-definition-representable-fibred-category}
\source{stacks:0046}
\href{https://stacks.math.columbia.edu/tag/0046}{\texttt{Stacks tag 0046}}
\group{categories}
...
\begin{lemma}[Characterize representable fibred category]
\label{categories-lemma-characterize-representable-fibred-category}
\source{stacks:02Y3}
\href{https://stacks.math.columbia.edu/tag/02Y3}{\texttt{Stacks tag 02Y3}}
\group{categories}
...
\begin{proof}
\uses{categories-lemma-setoid-fibres, categories-lemma-2-category-fibred-sets, categories-lemma-yoneda, categories-example-fibred-category-from-functor-of-points}
...
Hence it is an isomorphism, see Lemma \ref{categories-lemma-2-category-fibred-sets}.
Hence by the Yoneda Lemma \ref{categories-lemma-yoneda} (via
Example \ref{categories-example-fibred-category-from-functor-of-points} for example)
it is given by an isomorphism $X' \to X$.
\end{proof}

\begin{lemma}[Morphisms representable fibred categories]
\label{categories-lemma-morphisms-representable-fibred-categories}
\source{stacks:04SF}
\href{https://stacks.math.columbia.edu/tag/04SF}{\texttt{Stacks tag 04SF}}
\group{categories}
...
\begin{proof}
\uses{categories-example-fibred-category-from-functor-of-points, categories-lemma-2-category-fibred-setoids, categories-lemma-yoneda}
...
$$
By the Yoneda
Lemma \ref{categories-lemma-yoneda}
...
\source{stacks:0GWI}
\href{https://stacks.math.columbia.edu/tag/0GWI}{\texttt{Stacks tag 0GWI}}
\group{categories}
...
(see Definition \ref{categories-definition-pullback-functor-fibred-category}).
We define a functor
$$
\mathcal{S}_U
\longrightarrow
...
$$
where $\alpha_{g, f}$ is as in Lemma \ref{categories-lemma-fibred}.
\end{enumerate}
...
$$
denotes the category of $1$-morphisms in this $2$-category
(see Definition \ref{categories-definition-categories-over-C}).
...
\source{stacks:004B}
\href{https://stacks.math.columbia.edu/tag/004B}{\texttt{Stacks tag 004B}}
\group{categories}
...
\begin{proof}
\uses{categories-definition-pullback-functor-fibred-category, categories-lemma-fibred-groupoids}
...
(f \circ g)^*x \xrightarrow{(\alpha_{g, f})_x} g^*f^*x \rightarrow f^*x
$$
where $\alpha_{g, f}$ is as in Lemma \ref{categories-lemma-fibred-groupoids}.
\end{enumerate}
We omit the verification that this is an inverse to the functor
of the lemma.
\end{proof}

\begin{remark}[Alternative fibred groupoids strict]
\label{categories-remark-alternative-fibred-groupoids-strict}
\source{stacks:076J}
\href{https://stacks.math.columbia.edu/tag/076J}{\texttt{Stacks tag 076J}}
\group{categories}

\uses{categories-lemma-fibred-groupoids-strict, categories-example-functor-groupoids, categories-lemma-yoneda-2category, categories-lemma-equivalence-fibred-categories}
We can use the $2$-Yoneda lemma to give an alternative proof of
Lemma \ref{categories-lemma-fibred-groupoids-strict}.
Let $p : \mathcal{S} \to \mathcal{C}$ be a category fibred in groupoids.
...
$\mathcal{S}'$ by Lemma \ref{categories-lemma-equivalence-fibred-categories}.
\end{remark}
...
the $2$-fibre product
$$
\begin{array}{cc}(\mathcal{C}/U) \times_\mathcal{Y} \mathcal{X} \longrightarrow\downarrow & \mathcal{X} \overset{F}{\downarrow} \\ \mathcal{C}/U \overset{G}{\longrightarrow} & \mathcal{Y}\end{array}
$$
...
see Lemma \ref{categories-lemma-yoneda-2category} and its proof.
...
\begin{lemma}[Identify fibre product]
\label{categories-lemma-identify-fibre-product}
\source{stacks:02Y5}
\href{https://stacks.math.columbia.edu/tag/02Y5}{\texttt{Stacks tag 02Y5}}
\group{categories}
...
\begin{lemma}[Prepare representable map stack in groupoids]
\label{categories-lemma-prepare-representable-map-stack-in-groupoids}
\source{stacks:02Y6}
\href{https://stacks.math.columbia.edu/tag/02Y6}{\texttt{Stacks tag 02Y6}}
\group{categories}
...
\begin{proof}
\uses{categories-lemma-2-product-fibred-categories, categories-lemma-cute-groupoids}

We have already seen in Lemma \ref{categories-lemma-2-product-fibred-categories}
that the composition
$$
(\mathcal{C}/U) \times_\mathcal{Y} \mathcal{X}
\longrightarrow
...
Lemma \ref{categories-lemma-cute-groupoids}.
\end{proof}

\begin{definition}[Representable map categories fibred in groupoids]
\label{categories-definition-representable-map-categories-fibred-in-groupoids}
\source{stacks:02Y7}
\href{https://stacks.math.columbia.edu/tag/02Y7}{\texttt{Stacks tag 02Y7}}
\group{categories}
...
\begin{lemma}[Spell out representable map stack in groupoids]
\label{categories-lemma-spell-out-representable-map-stack-in-groupoids}
\source{stacks:02Y8}
\href{https://stacks.math.columbia.edu/tag/02Y8}{\texttt{Stacks tag 02Y8}}
\group{categories}
...
\begin{proof}
\uses{categories-lemma-identify-fibre-product, categories-lemma-characterize-representable-fibred-category}

Clear from the description of fibre categories in
Lemma \ref{categories-lemma-identify-fibre-product} and the characterization
of representable fibred categories in
Lemma \ref{categories-lemma-characterize-representable-fibred-category}.
\end{proof}

\begin{lemma}[Criterion representable map stack in groupoids]
\label{categories-lemma-criterion-representable-map-stack-in-groupoids}
\source{stacks:02Y9}
\href{https://stacks.math.columbia.edu/tag/02Y9}{\texttt{Stacks tag 02Y9}}
\group{categories}
...
Definition \ref{categories-definition-final-object-2-category}). And it has
...
\begin{lemma}[Representable diagonal groupoids]
\label{categories-lemma-representable-diagonal-groupoids}
\source{stacks:02YA}
\href{https://stacks.math.columbia.edu/tag/02YA}{\texttt{Stacks tag 02YA}}
\group{categories}
...
\begin{proof}
\uses{categories-lemma-diagonal-1, categories-remark-2-product-fibred-categories-same, categories-lemma-diagonal-2}
...
$$
\begin{array}{cc}(\mathcal{C}/U \times V)
\times_{(\mathcal{S} \times \mathcal{S})}
\mathcal{S}
\longrightarrow\downarrow & \mathcal{S} \downarrow \\ \mathcal{C}/U \times V \overset{(G, G')}{\longrightarrow} & \mathcal{S} \times \mathcal{S}\end{array}
$$
is representable by assumption.
This means there exists $W \to U \times V$ in $\mathcal{C}$,
such that
$$
\begin{array}{cc}\mathcal{C}/W \downarrow\longrightarrow & \mathcal{S} \downarrow \\ \mathcal{C}/U \times \mathcal{C}/V \longrightarrow & \mathcal{S} \times \mathcal{S}\end{array}
$$
is cartesian. This implies that
$\mathcal{C}/W \cong \mathcal{C}/U \times_\mathcal{S} \mathcal{C}/V$
(see Lemma \ref{categories-lemma-diagonal-1} and
Remark \ref{categories-remark-2-product-fibred-categories-same})
as desired.
...
\mathcal{C}/V \times_{(\mathcal{S} \times \mathcal{S})} \mathcal{S}
\end{eqnarray*}
(for the last isomorphism see Lemma \ref{categories-lemma-diagonal-2} and
Remark \ref{categories-remark-2-product-fibred-categories-same})
...
\vcenter{
\begin{array}{cc}X \otimes (Y \otimes ( Z \otimes W)) \longrightarrow\downarrow & X \otimes ((Y \otimes Z) \otimes W) \downarrow \\ (X \otimes Y) \otimes (Z \otimes W) \downarrow & (X \otimes (Y \otimes Z)) \otimes W \swarrow \\ ((X \otimes Y) \otimes Z) \otimes W & \end{array}
}
\end{equation}
is commutative where every arrow is determined by a suitable application
of $\phi$ and functoriality of $\otimes$.

\medskip\noindent
In \cite[Theorem 3.1]{associativity} it is explained how a triple
$(\mathcal{C}, \otimes, \phi)$ as a above is coherent. A reformulation
is that such a triple gives rise to a system of well defined functors
$$
...
X_1 \otimes \ldots \otimes X_n \otimes Y_1 \otimes \ldots \otimes Y_m
$$
such that all possible diagrams formed from these isomorphisms
commute (the pentagram diagram above is an example). For $n = 5$
...
\end{matrix}
$$
are all functorially isomorphic to $X_1 \otimes \ldots \otimes X_5$
and that these isomorphisms are all compatible with all possible
isomorphisms one gets between these functors using $\phi$ in
3 consecutive spots (when possible). We will use this without
further mention in the following and we will no longer write
parentheses when writing iterations of $\otimes$.

\medskip\noindent
A {\it unit} for a triple $(\mathcal{C}, \otimes, \phi)$ as above
is an object $\mathbf{1}$ of $\mathcal{C}$ together with functorial
isomorphisms
$$
...
\vcenter{
\begin{array}{ccc}X \otimes (\mathbf{1} \otimes Y) \underset{\phi}{\longrightarrow}\underset{\text{id} \otimes l}{\searrow} &  & (X \otimes \mathbf{1}) \otimes Y \overset{r \otimes \text{id}}{\swarrow} \\  & X \otimes Y & \end{array}
}
\end{equation}
is commutative. We will often think of units as pairs $(\mathbf{1}, 1)$
as in the following lemma.

\begin{lemma}[Monoidal unit]
\label{categories-lemma-monoidal-unit}
\source{stacks:0HAW}
\href{https://stacks.math.columbia.edu/tag/0HAW}{\texttt{Stacks tag 0HAW}}
\group{categories}
...
\begin{lemma}[Monoidal unit unique]
\label{categories-lemma-monoidal-unit-unique}
\source{stacks:0HAX}
\href{https://stacks.math.columbia.edu/tag/0HAX}{\texttt{Stacks tag 0HAX}}
\group{categories}

\uses{categories-lemma-monoidal-unit}
Let $(\mathcal{C}, \otimes, \phi)$ be as above.
Let $(\mathbf{1}, 1)$ be a unit (see Lemma \ref{categories-lemma-monoidal-unit}).
Then
\begin{enumerate}
...
Part (1) was shown in the proof of Lemma \ref{categories-lemma-monoidal-unit}.
For $a \in \Gamma$ we have
\begin{align*}
...
\end{align*}
Thus $1 \circ (a \otimes \text{id}_\mathbf{1}) = a \circ 1$
and this implies one half of (3). The other half follows in
the same way. To see (2) we observe that for $a, b \in \Gamma$
we have
$$
a \circ b =
...
$$
and $b \circ a$ evaluates to the same expression. Thus (2) holds.
To see (4) suppose that $(\mathbf{1}', 1')$ is a second unit.
Using $r$ and $l'$ there are isomorphisms
$\mathbf{1}' \otimes \mathbf{1} \to \mathbf{1}'$ and
$\mathbf{1}' \otimes \mathbf{1} \to \mathbf{1}$. Thus
there exists an isomorphism $t : \mathbf{1}' \to \mathbf{1}$.
Then the diagram
$$
\begin{array}{cc}\mathbf{1}' \otimes \mathbf{1}' \underset{t \otimes t}{\longrightarrow}\underset{1'}{\downarrow} & \mathbf{1} \otimes \mathbf{1} \overset{1}{\downarrow} \\ \mathbf{1}' \overset{t}{\longrightarrow} & \mathbf{1}\end{array}
$$
commutes up to an automorphism $a$ of $\mathbf{1}$. After replacing
$t$ by $a \circ t$ the diagram will commute (hint: use (3) to see
that $1 \circ (a \otimes a) = a^2 \circ 1$).
\end{proof}

\begin{definition}[Monoidal category]
\label{categories-definition-monoidal-category}
\source{stacks:0FFK}
\href{https://stacks.math.columbia.edu/tag/0FFK}{\texttt{Stacks tag 0FFK}}
\group{categories}
...
and we denote $1 : \mathbf{1} \otimes \mathbf{1} \to \mathbf{1}$
a chosen isomorphism; as the pair $(\mathbf{1}, 1)$ is determined
up to unique isomorphism (Lemma \ref{categories-lemma-monoidal-unit-unique})
there is no harm in choosing one.

\begin{lemma}[Monoidal coherence]
\label{categories-lemma-monoidal-coherence}
\source{stacks:0HAY}
\href{https://stacks.math.columbia.edu/tag/0HAY}{\texttt{Stacks tag 0HAY}}
\group{categories}

\uses{categories-lemma-monoidal-unit}
In a monoidal category $\mathcal{C}, \otimes, \phi, \mathbf{1}, 1$
and with notation as in the proof of Lemma \ref{categories-lemma-monoidal-unit}
we have
\begin{enumerate}
...
\cite[Theorem 5.2]{associativity}.
\end{lemma}

\begin{proof}
\uses{categories-lemma-monoidal-unit}

We have seen (1) in the proof of Lemma \ref{categories-lemma-monoidal-unit}.
We have seen in the proof of Lemma \ref{categories-lemma-monoidal-unit}
that $l_X$ and $l_{X \otimes Y}$ are
the unique morphisms such that
$\text{id}_\mathbf{1} \otimes l_{X \otimes Y} =
1 \otimes \text{id}_{X \otimes Y}$ and
$\text{id}_\mathbf{1} \otimes l_X = 1 \otimes \text{id}_X$.
Part (2) follows immediately. Part (3) is proved in a similar manner.
Jointly with the commutativity of (\ref{equation-pentagon})
and (\ref{equation-triangle}) this means the final statement of
the lemma holds.
\end{proof}

In \cite[Theorem 5.2]{associativity} it is explained how a quintuple
$(\mathcal{C}, \otimes, \phi, \mathbf{1}, 1)$ as in the lemma above
is coherent. A reformulation is that in a monoidal category (in our sense)
for every $n \geq i \geq 0$ we have functorial isomorphisms
$$
...
$$
such that all possible diagrams formed from these isomorphisms
commute. So for example starting with two insertions of $\mathbf{1}$
and using the isomorphisms in different order would result in the same
morphism. Besides the convention on removing parentheses when writing
iterates of $\otimes$, from now on we identify $X \otimes \mathbf{1}$ and
$\mathbf{1} \otimes X$ with $X$ without further mention.
Moreover, we will say ``let $\mathcal{C}$ be a monoidal category''
with $\otimes, \phi, \mathbf{1}$ understood.

\begin{definition}[Functor monoidal categories]
\label{categories-definition-functor-monoidal-categories}
\source{stacks:0FFL}
\href{https://stacks.math.columbia.edu/tag/0FFL}{\texttt{Stacks tag 0FFL}}
\group{categories}
...
$$
\begin{array}{ccc}F(X) \otimes (F(Y) \otimes F(Z)) \longrightarrow\downarrow & F(X) \otimes F(Y \otimes Z) \longrightarrow & F(X \otimes (Y \otimes Z)) \downarrow \\ (F(X) \otimes F(Y)) \otimes F(Z) \longrightarrow & F(X \otimes Y) \otimes F(Z) \longrightarrow & F((X \otimes Y) \otimes Z)\end{array}
$$
...
\source{stacks:0FFM}
\href{https://stacks.math.columbia.edu/tag/0FFM}{\texttt{Stacks tag 0FFM}}
\group{categories}
...
\source{stacks:0FFN}
\href{https://stacks.math.columbia.edu/tag/0FFN}{\texttt{Stacks tag 0FFN}}
\group{categories}

\uses{categories-lemma-invertible}
...
Lemma \ref{categories-lemma-invertible} hold.
\end{definition}
...
\source{stacks:0FFP}
\href{https://stacks.math.columbia.edu/tag/0FFP}{\texttt{Stacks tag 0FFP}}
\group{categories}
...
$$
\vcenter{
\begin{array}{cc}X \underset{1}{\searrow}\underset{\eta \otimes 1}{\longrightarrow} & X \otimes Y \otimes X \overset{1 \otimes \epsilon}{\downarrow} \\  & X\end{array}
}
\quad\text{and}\quad
\vcenter{
\begin{array}{cc}Y \underset{1}{\searrow}\underset{1 \otimes \eta}{\longrightarrow} & Y \otimes X \otimes Y \overset{\epsilon \otimes 1}{\downarrow} \\  & Y\end{array}
}
$$
...
\begin{lemma}[Left dual]
\label{categories-lemma-left-dual}
\source{stacks:0FFQ}
\href{https://stacks.math.columbia.edu/tag/0FFQ}{\texttt{Stacks tag 0FFQ}}
\group{categories}
...
\Mor(Z' \otimes X, Z)
$$
where we use $\epsilon$ in the second arrow.
To show these arrows are mutually inverse, consider a map
$a : Z' \to Z \otimes Y$. We have to show that
$$
...
Z \otimes Y
$$
is equal to $a$. The composition of the first two arrows equals
$(\text{id}_{Z \otimes Y} \otimes \eta) \circ a :
Z' \to Z \otimes Y \to Z \otimes Y \otimes X \otimes Y$.
Then the composition of $\text{id}_{Z \otimes Y} \otimes \eta$
and $\text{id}_Z \otimes \epsilon \otimes \text{id}_Y$ equals
the identity by definition of the dual. Similarly for the other
composition. We omit the proof of the second equality.
\end{proof}

\begin{remark}[Left dual adjoint]
\label{categories-remark-left-dual-adjoint}
\source{stacks:0FFR}
\href{https://stacks.math.columbia.edu/tag/0FFR}{\texttt{Stacks tag 0FFR}}
\group{categories}

\uses{categories-lemma-left-dual}
Lemma \ref{categories-lemma-left-dual} says in particular that $Z \mapsto Z \otimes Y$
...
$$
\begin{array}{cc}\Mor(Z' \otimes X, Z) \longrightarrow\underset{\text{id}_W \otimes -}{\downarrow} & \Mor(Z', Z \otimes Y) \overset{\text{id}_W \otimes -}{\downarrow} \\ \Mor(W \otimes Z' \otimes X, W \otimes Z) \longrightarrow & \Mor(W \otimes Z', W \otimes Z \otimes Y)\end{array}
$$
commutes.
...
\begin{lemma}[Tensor dual]
\label{categories-lemma-tensor-dual}
\source{stacks:0FFS}
\href{https://stacks.math.columbia.edu/tag/0FFS}{\texttt{Stacks tag 0FFS}}
\group{categories}
...
\begin{proof}
\uses{categories-remark-left-dual-adjoint}

Follows from uniqueness of adjoints and Remark \ref{categories-remark-left-dual-adjoint}.
\end{proof}
...
\vcenter{
\begin{array}{ccc}X \otimes (Y \otimes Z) \underset{\phi}{\longrightarrow}\overset{\psi}{\downarrow} & (X \otimes Y) \otimes Z \underset{\psi}{\longrightarrow} & Z \otimes (X \otimes Y) \overset{\phi}{\downarrow} \\ X \otimes (Z \otimes Y) \overset{\phi}{\longrightarrow} & (X \otimes Z) \otimes Y \overset{\psi}{\longrightarrow} & (Z \otimes X) \otimes Y\end{array}
}
\end{equation}
commutes.
...
\source{stacks:0FFW}
\href{https://stacks.math.columbia.edu/tag/0FFW}{\texttt{Stacks tag 0FFW}}
\group{categories}
...
is a monoidal category and we may use the language and notation
discussed above.

\begin{lemma}[Symmetric monoidal coherence]
\label{categories-lemma-symmetric-monoidal-coherence}
\source{stacks:0HB0}
\href{https://stacks.math.columbia.edu/tag/0HB0}{\texttt{Stacks tag 0HB0}}
\group{categories}
...
\cite[Theorem 5.1]{associativity}.
\end{lemma}

\begin{proof}
\uses{categories-lemma-monoidal-unit-unique, categories-lemma-monoidal-coherence}

We may write $\psi = a \otimes \text{id}_\mathbf{1}$ for a unique
isomorphism $a : \mathbf{1} \to \mathbf{1}$.
Lemma \ref{categories-lemma-monoidal-unit-unique} implies that
$a \otimes \text{id}_\mathbf{1} = \text{id}_\mathbf{1} \otimes a$.
Functoriality of $\psi$ says that the diagram
$$
\begin{array}{cc}(\mathbf{1} \otimes \mathbf{1}) \otimes \mathbf{1}
\underset{\psi}{\longrightarrow}\underset{1 \otimes \text{id}_\mathbf{1}}{\downarrow} & \mathbf{1} \otimes (\mathbf{1} \otimes \mathbf{1})
\overset{\text{id}_\mathbf{1} \otimes 1}{\downarrow} \\ \mathbf{1} \otimes \mathbf{1}  \overset{\psi}{\longrightarrow} & \mathbf{1} \otimes \mathbf{1}\end{array}
$$
commutes. Thus the top arrow is equal to
$a \otimes \text{id}_\mathbf{1} \otimes \text{id}_\mathbf{1}$.
Thus (\ref{equation-hexagon}) for $X = Y = Z = \mathbf{1}$ says that
$a \otimes \text{id}_\mathbf{1} \otimes \text{id}_\mathbf{1}$
is equal to its own square. Hence $a = \text{id}_\mathbf{1}$.
This proves (1).

\medskip\noindent
Part (2) states that $\psi : X \otimes \mathbf{1} \to \mathbf{1} \otimes X$
is the identity, if we identify the source and the target with $X$
in our monoidal category. This follows from the commutativity of
(\ref{equation-hexagon}) for $\mathbf{1}, X, \mathbf{1}$, namely
$$
\begin{array}{ccc}\mathbf{1} \otimes (X \otimes \mathbf{1}) \underset{\phi}{\longrightarrow}\overset{\psi}{\downarrow} & (\mathbf{1} \otimes X) \otimes \mathbf{1} \underset{\psi}{\longrightarrow} & \mathbf{1} \otimes (\mathbf{1} \otimes X) \overset{\phi}{\downarrow} \\ \mathbf{1} \otimes (\mathbf{1} \otimes X) \overset{\phi}{\longrightarrow} & (\mathbf{1} \otimes \mathbf{1}) \otimes X \overset{\psi}{\longrightarrow} & (\mathbf{1} \otimes \mathbf{1}) \otimes X\end{array}
$$
commutes and we know all but one of the morphisms $\psi$
in this diagram are equal to the identity.

\medskip\noindent
In addition to (1) and (2) the commutativity of the diagrams
(\ref{equation-pentagon}), (\ref{equation-triangle}), (\ref{equation-hexagon})
and the results of Lemma \ref{categories-lemma-monoidal-coherence}
imply the final statement of the lemma.
\end{proof}

In \cite[Theorem 5.1]{associativity} it is explained how a sextuple
$(\mathcal{C}, \otimes, \phi, \psi, \mathbf{1}, 1)$ as in the lemma above
is coherent. A reformulation is that in a symmetric monoidal category
(in our sense) for every $n \geq 1$ and permutation $\sigma$
of $\{1, \ldots, n\}$ we have functorial isomorphisms
$$
X_1 \otimes \ldots \otimes X_n
\to
...
$$
such that all possible diagrams formed from these isomorphisms
commute and these isomorphisms are compatible with the structure
of a monoidal category (e.g., with the isomorphisms when we insert
a $\mathbf{1}$ in a slot).

\begin{lemma}[Dual symmetric]
\label{categories-lemma-dual-symmetric}
\source{stacks:0FN8}
\href{https://stacks.math.columbia.edu/tag/0FN8}{\texttt{Stacks tag 0FN8}}
\group{categories}

\uses{categories-definition-dual}
...
$\epsilon : Y \otimes X \to \mathbf{1}$
be a left dual of $X$ as in Definition \ref{categories-definition-dual}.
...
\begin{definition}[Functor symmetric monoidal categories]
\label{categories-definition-functor-symmetric-monoidal-categories}
\source{stacks:0FFY}
\href{https://stacks.math.columbia.edu/tag/0FFY}{\texttt{Stacks tag 0FFY}}
\group{categories}
...
$$
\begin{array}{cc}F(X) \otimes F(Y) \longrightarrow\downarrow & F(X \otimes Y) \downarrow \\ F(Y) \otimes F(X) \longrightarrow & F(Y \otimes X)\end{array}
$$
commutes.
\end{definition}

\begin{remark}[Internal hom monoidal]
\label{categories-remark-internal-hom-monoidal}
\source{stacks:0GWJ}
\href{https://stacks.math.columbia.edu/tag/0GWJ}{\texttt{Stacks tag 0GWJ}}
\group{categories}
...
\begin{definition}[Dotted arrows]
\label{categories-definition-dotted-arrows}
\source{stacks:0H18}
\href{https://stacks.math.columbia.edu/tag/0H18}{\texttt{Stacks tag 0H18}}
\group{categories}
...
\vcenter{
\begin{array}{cc}S \underset{x}{\longrightarrow}\underset{j}{\downarrow} & X \overset{f}{\downarrow} \\ T \overset{y}{\longrightarrow}\nearrow & Y\end{array}
}
\end{equation}
...
In the situation of Definition \ref{categories-definition-dotted-arrows}, there
...
\source{stacks:0H1A}
\href{https://stacks.math.columbia.edu/tag/0H1A}{\texttt{Stacks tag 0H1A}}
\group{categories}

Let $\mathcal{C}$ be a $(2,1)$-category. Assume given a $2$-commutative diagram
$$
\begin{array}{ccc}S \underset{x'}{\longrightarrow}\underset{j}{\downarrow} & X' \overset{p}{\downarrow}\underset{q}{\longrightarrow} & X \overset{f}{\downarrow} \\ T \overset{y'}{\longrightarrow} & Y' \overset{g}{\longrightarrow} & Y\end{array}
$$
...
as in Section \ref{categories-section-2-fibre-products}. Details omitted.
\end{proof}

\begin{lemma}[Cat dotted arrows composition]
\label{categories-lemma-cat-dotted-arrows-composition}
\source{stacks:0H1B}
\href{https://stacks.math.columbia.edu/tag/0H1B}{\texttt{Stacks tag 0H1B}}
\group{categories}

Let $\mathcal{C}$ be a $(2,1)$-category. Assume given a solid $2$-commutative
diagram
$$
\begin{array}{cc}S \underset{x}{\longrightarrow}\underset{j}{\downarrow} & X \overset{f}{\downarrow} \\  & Y \overset{g}{\downarrow} \\ T \overset{z}{\longrightarrow}\nearrow & Z\end{array}
$$
in $\mathcal{C}$.
...
$$
\begin{array}{cc}S \underset{f \circ x}{\longrightarrow}\underset{j}{\downarrow} & Y \overset{g}{\downarrow} \\ T \overset{z}{\longrightarrow}\nearrow & Z\end{array}
$$

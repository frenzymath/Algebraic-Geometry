...
/-!
# Tangent-space substrate: derivations and the cotangent dual (W5-T1, algebra layer)

Commutative-algebra substrate for the Kleiman §5 tangent-space theorem
(Wave-5 brick T1, roadmap `AJCR.w5-av.t1`): the identification of the Zariski
tangent space of a `k`-scheme at a `k`-rational point with the dual of the
cotangent space `m/m²`, expressed at the level of the local ring at the point.
Feeds the scalar identity `dim_k T₀(d.J) = genus C` (T5) through
`Tangent/TangentCotangentCount.lean`.
...
Ported from the sibling project `Algebraic-Jacobian-Challenge`,
`AlgebraicJacobian/Picard/TangentSpaceDualNumbers.lean`, which is sorry-free
and stated in mathlib generality (no AJC-specific dependency); both projects
pin toolchain `v4.31.0` and mathlib `fabf563a7c95`, so the port is
transcription plus kernel re-verification, per the cross-project overlap
audit of inbox `I-0495` (overlap #2). Nothing here is re-derived: the
alternative was two independent derivations of the same dual-number
dictionary, which is exactly the waste the workspace exists to prevent.

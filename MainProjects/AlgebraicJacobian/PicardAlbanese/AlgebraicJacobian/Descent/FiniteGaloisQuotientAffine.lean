...
-/
import AlgebraicJacobian.Descent.FiniteGaloisQuotient

/-!
# The affine finite Galois quotient theorem

Ported from the sibling Algebraic Jacobian project
(`AlgebraicJacobian.Picard.FiniteGaloisQuotientAffine`).

Let `L/K` be a finite Galois extension with group `Γ = Gal(L/K)` and let `A` be
an `L`-algebra with a semilinear `Γ`-action (`specSemilinearGalAction`, the
affine model of `AlgebraicJacobian.Descent.FiniteGaloisQuotient`).  This file
proves that `Spec (A^Γ) ⟶ Spec K` **is a finite Galois quotient** of `Spec A` —
the full `IsGaloisQuotient` predicate (`isGaloisQuotient_spec`) — by Speiser
descent only, never Noether invariant finiteness.

## Structure of the proof

1. **The base-change isomorphism** `Spec A^Γ ×_{Spec K} Spec L ≅ Spec A`
   (`affineQuotientPullbackIso`): mathlib's `pullbackSpecIso K (A^Γ) L`
   composed with the tensor-factor swap `Spec (A^Γ ⊗[K] L) ≅ Spec (L ⊗[K] A^Γ)`
   (the engine convention keeps the Galois-acted factor `L` on the **left**)
   and with `Spec` of the Speiser descent isomorphism
   `descentAlgEquiv : L ⊗[K] A^Γ ≃ₐ[L] A`.  It is over `Spec L`
   (`affineQuotientPullbackIso_hom_specMap`) and `Γ`-equivariant
   (`isEquivariant_affineQuotientPullbackIso_hom`).

2. **The `T`-points property for affine `T`**
   (`existsUnique_pullbackBaseChange_of_affine`): the scheme-level transport of
   the algebraic Hom property `affineGaloisQuotientHomEquiv`, along the
   identification `T ×_{Spec K} Spec L ≅ Spec (L ⊗[K] B)` (`pullbackSpecLIso`)
   for `T = Spec B`.

3. **The `T`-points property for all schemes `T`**: uniqueness globalizes first
   from an affine open cover (`affineQuotient_hom_unique`); existence then
   glues the local affine solutions via `Scheme.Cover.glueMorphisms`, with
   agreement on the (possibly non-affine) overlaps supplied by the globalized
   uniqueness.

The file also provides the base-change composition calculus
(`pullbackBaseChange_comp`, `pullbackBaseChange_congr`) consumed by the
quotient uniqueness and gluing layers.

Sources: Kleiman, *The Picard scheme* §4; Milne, *Jacobian Varieties* §4; the
affine heart is Speiser's theorem (`SemilinearAlgebras.descentAlgEquiv`).
-/

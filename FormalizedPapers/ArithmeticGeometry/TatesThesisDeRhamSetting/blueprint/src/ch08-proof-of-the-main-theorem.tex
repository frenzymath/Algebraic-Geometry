...
\]
 ...
 where the structural functors are the standard
...
\source{tates-thesis-de-rham-setting}
\vcenter{\text{[Diagram omitted; see source PDF.]}}
\end{equation}

 of $D^*(\fL^+\bG_m)$-module categories for each $n$.
...
\]
 ...
 Here $\pi^*$ is the quasi-coherent pullback functor. We refer
...
\end{equation}
 ...
 (We have omitted terms $\lambda_{-,*}^{\IndCoh}$ to simplify
...
\]
 ...
 compatible with the actions of $A^{op} \in \Rep(G)$ on both sides.
...
\]
 ...
 is an epimorphism, we see that the first point above
...
\]
 ...
 Moreover, it is easy to see that $\Delta_{n+1}(-\cdot \partial_{\alpha_n})$
...
\]
 ...
 i.e., the left arrow in \eqref{eq:fund-ses-2}. From
...
\]
 ...
 and so an action of $D^*(\fL^{pos} \bG_m)$ on
$\IndCoh^*(\sY)$.
...
\]
 ...
 I.e., in the notation of \cite{dmod}, we have $T = \mu_{t,*,!-dR}$.
...
\]
 ...
 (Here we consider $\omega_{\fL^+\bA^1}$ as a $\fL^+\bG_m$-equivariant
object.)
...
\]
 ...
 coincides with \eqref{eq:can-unit-spec}.
...
\]
 ...
 of (symmetric) monoidal DG categories. Therefore, we find
...
\]
 ...
 compatibly in $n$ (using the $p_{-}^!$ functors and
...
\]
 ...
 satisfying some compatibilities (that do not involve higher
homotopy theory).
...
\]
 ...
 in the abelian category $D(\fL_{n+1}^+\bA^1)^{\fL_{n+1}\bG_m,w,\heart}[n+1]$
...
\]
 ...
 The latter is canonically isomorphism to
...
\]
 ...
 (which e.g. follows by replacing functors with kernels
...
\]
 ...
 of $D^*(\fL\bG_m)$-module categories such that the induced morphism:
%
\[
D^!(\fL^+\bA^1) \subset D^!(\fL\bA^1) \to \IndCoh^*(\sY)
\]

 of $D^*(\fL^{pos}\bG_m)$-module categories coincides with the functor
...
\]
 ...
 that we observe is an equivalence. Indeed, this follows
...
\]
 ...
 Using $t^{-n}$ to identify the $n$th term here with
...
\]
 ...
 of $D^*(\fL\bG_m)$-module categories restricting to

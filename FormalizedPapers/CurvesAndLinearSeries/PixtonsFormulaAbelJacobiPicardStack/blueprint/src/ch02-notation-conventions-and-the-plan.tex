...
\begin{remark}[Key notation]
\label{default}
\source{pixtons-formula-abel-jacobi-picard-stack}
We write $\frak M_{g,n}$ and $\Mbar_{g,n}$ for the prestable and stable
marked-curve stacks, with universal curve $\frak C$.  The tuple
$A=(a_1,\ldots,a_n)$ has total degree $d$.  The absolute and relative Picard
stacks are $\Picabs_{g,n}$ and $\Picrel_{g,n}$; $\CHop$ denotes operational
Chow groups.  Their tautological classes are $\xi_i$, $\eta_{a,b}$, and
$\psi_i$, while $\mathsf P_{g,A,d}^{c}$ and $\DRop_{g,A}$ denote Pixton's
cycle and the operational double-ramification cycle.
\end{remark}

...
\begin{proof}
  \leanok
  \uses{def:quotAlgEquivPiFactors}
  The Chinese remainder decomposition \ref{def:quotAlgEquivPiFactors} identifies
...
\label{sec:localized_colength}

The ord-weighted formula \ref{thm:finrank_quotient_span_eq_sum_ord} assumes each prime
...
formula \ref{thm:finrank_quotient_span_pow} applies. The bookkeeping needed for the
...
\begin{proof}
  \leanok
  \uses{thm:finrank_quotSucc_jump}
  ...
  \ref{thm:finrank_quotSucc_jump}, hence finite-dimensional. Thus \(B/I^{n+1}\) is an
...
\begin{proof}
  ...
  \ref{lem:adic_valuation_count}, \(v_P(f) = \mathrm{ord}_P(f)\), giving the claim.
\end{proof}
...
\begin{proof}
  ...
  \[
    ...
    f \in P^n .
  \]
  By \ref{lem:mem_pow_iff_le_count}, applied in \(S\) (a Dedekind domain, with
...
\begin{proof}
  \leanok
  \uses{thm:moduleFinite_quotient_pow_of_isPrincipal}
  ...
  By \ref{def:quotPowLinearEquiv} the residue \(S/\mathfrak m \cong B/P\) is
  finite-dimensional, so the d\'evissage
  \ref{thm:moduleFinite_quotient_pow_of_isPrincipal}, applied in \(S\) to the nonzero
  principal ideal \(\mathfrak m\), makes every \(S/\mathfrak m^n\) finite-dimensional;
  transporting back along \ref{def:quotPowLinearEquiv} finishes.
\end{proof}

\begin{theorem}[The per-prime colength collapse]
  \label{thm:finrank_quotient_pow_atPrime}
  \dcref{custom}
...
\begin{proof}
  \leanok
  \uses{thm:moduleFinite_quotient_pow_of_isPrincipal, thm:finrank_quotient_span_pow}
  As in \ref{thm:moduleFinite_quotient_pow_atPrime}, \(S\) is a discrete valuation
  ring with nonzero principal maximal ideal \(\mathfrak m\), the quotients
  \(S/\mathfrak m^n\) are finite-dimensional
  (\ref{thm:moduleFinite_quotient_pow_of_isPrincipal}), and the prime-power formula
  \ref{thm:finrank_quotient_span_pow} in \(S\) gives
  ...
  (\ref{def:quotPowLinearEquiv}) transport this to the claim.
\end{proof}
...
\begin{proof}
  \leanok
  \uses{lem:prime_factor_span, thm:moduleFinite_quotient_pow_atPrime}
  The ideal \((f)\) is nonzero, so the Chinese remainder decomposition
  \ref{def:quotAlgEquivPiFactors} identifies \(B/(f)\) with the finite product
  \(\prod_{P \mid (f)} B/P^{\,\mathrm{ord}_P(f)}\). Each factor is
  finite-dimensional: \(P\) is a nonzero prime (\ref{lem:prime_factor_span}), hence
  maximal, with finite-dimensional residue by hypothesis, so
  \ref{thm:moduleFinite_quotient_pow_atPrime} applies with \(S = B_P\) (a domain,
...
\begin{theorem}[Dedekind colength formula, localized form]
  ...
  \uses{thm:finrank_quotient_eq_sum_factors_pow}
  \leanok
  Let \(B\) be a Dedekind domain over \(k\) and \(f \neq 0\) an element such that the
  residue \(B/P\) is finite-dimensional over \(k\) for every prime factor \(P\) of
  \((f)\). Then
...
\begin{proof}
  \leanok
  \uses{lem:prime_factor_span, thm:moduleFinite_quotient_pow_atPrime,
    thm:finrank_quotient_pow_atPrime}
  Each prime factor \(P\) of \((f)\) is nonzero (\ref{lem:prime_factor_span}), hence
  maximal, and \ref{thm:moduleFinite_quotient_pow_atPrime} with \(S = B_P\) makes each
  prime-power quotient \(B/P^{\,\mathrm{ord}_P(f)}\) finite-dimensional. The
  multi-prime formula \ref{thm:finrank_quotient_eq_sum_factors_pow} then gives
  \(\dim_k(B/(f)) = \sum_{P \mid (f)} \dim_k\bigl(B/P^{\,\mathrm{ord}_P(f)}\bigr)\),
  and the per-prime collapse \ref{thm:finrank_quotient_pow_atPrime} (again through
...
\label{sec:aeval_transport}

Two small transport toolkits consumed by the sheaf-level assembly of the rigid engine
(Chapter~\ref{chap:Cohomology}). For an endomorphism \(e\) of an \(S\)-module \(M\),
...
(\ref{thm:twoLattice_coh1}, \ref{thm:twoLattice_coh0}).
...
\begin{theorem}[Base change of chart-lattice finiteness]
  \label{thm:moduleFinite_aeval_baseChange}
  \dcref{custom}
...
\label{sec:point_fiber_ideal}

The diagonal-ideal package of Section~\ref{sec:diagonal_ideal} computes the kernel of the
...
\begin{proof}
  ...
  generator \(u_B \otimes 1 - 1 \otimes u_B\) (Definition~\ref{def:diagonalSetup}) is \(g\).
...
\begin{theorem}[The kernel of the point base change]
  ...
  \uses{def:pointGen_pointEv, lem:diagonal_telescoping, lem:diagonal_section}
  \leanok
...
\begin{proof}
  \leanok
  Mirror of Theorem~\ref{thm:ker_baseChange}. \(\beta(g) = 0\) since in
  ...
  divisible by the difference of the points (as in Lemma~\ref{lem:diagonal_telescoping}).
  ...
  Lemma~\ref{lem:diagonal_section}). An element of \(\ker\beta\) is then killed by the
...
\begin{theorem}[The localised point-fiber ideal]
  ...
  \uses{thm:ker_pointBaseChange, thm:ker_pointEvTwo, def:pointGen_pointEv,
    thm:exists_idempotent_lift, thm:ker_map_localization_eq}
  \leanok
...
\begin{proof}
  \leanok
  Mirror of Theorem~\ref{thm:ker_map_localization_eq}.
  ...
  (Definition~\ref{def:pointGen_pointEv}), which is principal on the pushed generator
  (Theorem~\ref{thm:ker_pointEvTwo}, whose hypothesis holds with
  \(e := \beta_B(\tilde e)\), the pushes intertwining by
  Definition~\ref{def:pointGen_pointEv}); lift the factorisation through the surjection
  \(\beta\) and correct by \(\ker\beta = (g)\) (Theorem~\ref{thm:ker_pointBaseChange}).
  ...
  \(\varepsilon(g) = 0\) (Definition~\ref{def:pointGen_pointEv}).
\end{proof}

\section{Depth and the Auslander--Buchsbaum formula}
\label{sec:auslander_buchsbaum}
...
\begin{lemma}[The covariant long exact sequence of \(\Ext\)]
  ...
  \uses{lem:ext_contravariant_les}
  \mathlibok
  \textit{Provided by Mathlib.}
...
\begin{theorem}[Depth via \(\Ext\)]
  ...
  \uses{def:depth, lem:ext_annihilator_smul, lem:ext_covariant_les}
  \leanok
  \dcref{custom}
...
\begin{proof}
  ...
  supremum of Definition~\ref{def:depth}; moreover, for the same reason, any
  ...
  by \ref{lem:ext_annihilator_smul}.
  ...
  \(\Ext^0(\kappa, M)\) (\ref{lem:ext_covariant_les}) and coincides with the action of
  \(x\), which is zero by (b); hence \(\Ext^0(\kappa, M) = 0\). If \(i = j + 1\) with
  \(j < n\): in the covariant long exact sequence (\ref{lem:ext_covariant_les})
  \[
  ...
  \]
  of the covariant long exact sequence (\ref{lem:ext_covariant_les}) has both outer
...
\begin{theorem}[Depth in short exact sequences]
  ...
  \uses{thm:depth_ext, lem:ext_covariant_les}
  \leanok
  \dcref{custom}
...
\begin{proof}
  \leanok
  Write \(\kappa = R/\mathfrak m\). In each case it suffices, by \ref{thm:depth_ext}, to
  ...
  (\ref{lem:ext_covariant_les}) applied to the given short exact sequence, using
  \ref{thm:depth_ext} to convert the depth bounds on the other two modules into
  \(\Ext\)-vanishing.
  ...
  monomorphism \(N' \to N\) is injective (\ref{lem:ext_covariant_les}) and lands in
...
\begin{proof}
  ...
  Definition~\ref{def:depth}. In the supremum branch, a sequence \(x_1, \dots, x_n\) is
...
\begin{lemma}[Depth of a finite power]
  ...
  \uses{def:depth, lem:ideal_smul_top_pi, lem:depth_linearEquiv}
  \leanok
  Let \(R\) be a commutative ring, \(I \subseteq R\) an ideal, \(M\) an \(R\)-module and
...
\begin{proof}
  \leanok
  The side condition agrees on both sides: by \ref{lem:ideal_smul_top_pi},
  ...
  the two suprema of Definition~\ref{def:depth} coincide. Induct on the sequence. For
  ...
  \(M\) (\(\iota\) nonempty for the converse); and by \ref{lem:ideal_smul_top_pi} at the
  ...
  \ref{lem:depth_linearEquiv}) and the inductive hypothesis applied to \(M/xM\), the
  tails match as well.
\end{proof}
...
\begin{proof}
  \leanok
  Immediate from Definition~\ref{def:projectiveDimension}: the projective dimension is
...
\begin{lemma}[Syzygy descent]
  ...
  \uses{lem:ext_covariant_les, lem:ext_contravariant_les}
  \leanok
...
\begin{proof}
  ...
  (\ref{lem:ext_contravariant_les}) contains the exact segment
  \[
...
\begin{proof}
  ...
  (\ref{lem:ext_contravariant_les}) for \(0 \to K \to R^n \to M \to 0\) contains
  \[
...
\begin{lemma}[The two depth inequalities of the kernel sequence]
  ...
  \uses{thm:depth_ses, lem:depth_pi_const}
  \leanok
...
\begin{proof}
  ...
  and (3) of \ref{thm:depth_ses} apply to \(0 \to K \to R^n \to M \to 0\); substitute
  \(\mathrm{depth}(R^n) = \mathrm{depth}(R)\) from \ref{lem:depth_pi_const}
  (\(n \geq 1\)).
\end{proof}
...
\begin{proof}
  \leanok
  By \ref{thm:depth_ext}, \(\Ext^i(\kappa, M) = 0\) for all \(i < D\), while
...
\begin{theorem}[Auslander--Buchsbaum, positive projective dimension]
  ...
  \uses{lem:minimal_surjection, lem:pd_lt_of_pd_eq, lem:syzygy_descent,
    lem:syzygy_ascent, lem:depth_ses_kernel, lem:ext_nonzero_at_depth,
    lem:ext_matrix_collapse, lem:ext_covariant_les, thm:depth_ext, thm:depth_ses,
    lem:depth_pi_const, lem:ideal_smul_top_pi, lem:depth_linearEquiv}
  \leanok
  \dcref{custom}
...
\begin{proof}
  ...
  (\ref{lem:minimal_surjection}); \(n \geq 1\) since \(M \neq 0\). By
  \ref{lem:pd_lt_of_pd_eq} and \ref{lem:syzygy_descent} (at \(k = 0\)), \(K\) has
  ...
  \(\alpha \in \Ext^D(\kappa, R^{k'})\) (\ref{lem:ext_nonzero_at_depth}, using
  \(\mathrm{depth}(R^{k'}) = D\)). By \ref{lem:ext_matrix_collapse},
  \(A_*\alpha = 0\). If \(D = 0\), postcomposition with the monomorphism \(A\) is
  injective on \(\Ext^0\) (\ref{lem:ext_covariant_les}), contradicting
  \(\alpha \neq 0\); so \(D = D' + 1\). The covariant long exact sequence
  (\ref{lem:ext_covariant_les}) contains
  \[
  ...
  \(\mathrm{depth}(M) \leq D'\) by \ref{thm:depth_ext}, i.e.\
  ...
  \(f : R^n \twoheadrightarrow M\) (\ref{lem:minimal_surjection}), \(n \geq 1\), and set
  \(K = \ker f\). Then \(\mathrm{pd}_R(K) = k + 1\) exactly: \(\leq\) by
  \ref{lem:pd_lt_of_pd_eq} and \ref{lem:syzygy_descent}; and if \(K\) had projective
  dimension \(< k + 1\), then \ref{lem:syzygy_ascent} would give
  ...
  \((k + 1) + \mathrm{depth}(K) = \mathrm{depth}(R)\), and \ref{lem:depth_ses_kernel}
...
\begin{theorem}[The Auslander--Buchsbaum formula]
  ...
  \uses{thm:auslander_buchsbaum_succ, lem:depth_pi_const, lem:depth_linearEquiv,
    def:projectiveDimension}
  \leanok
  \dcref{custom}
  Let \((R, \mathfrak m)\) be a Noetherian local ring and \(M\) a nonzero finite
...
\begin{proof}
  \leanok
  If \(n = 0\), then \(M\) is projective (Definition~\ref{def:projectiveDimension}),
  finite over the local Noetherian ring \(R\), hence free: \(M \cong R^{k}\) with
  \(k = \mathrm{rank}\, M \geq 1\) (as \(M \neq 0\)). By \ref{lem:depth_linearEquiv} and
  \ref{lem:depth_pi_const}, \(\mathrm{depth}(M) = \mathrm{depth}(R^k) =
  \mathrm{depth}(R)\), which is the formula with \(n = 0\). If \(n = k + 1\), this is
  \ref{thm:auslander_buchsbaum_succ}.
\end{proof}

\section{Cohen--Macaulay and regular local rings}
\label{sec:regular_cm}
...
\begin{lemma}[The regular quotient step]
  ...
  \uses{thm:cotangent_dim_drop, lem:notMemSq_witness}
  \leanok
...
\begin{proof}
  ...
  local Noetherian ring. By \ref{thm:cotangent_dim_drop},
...
\begin{theorem}[Regular local rings are domains]
  ...
  \uses{lem:notMemSq_witness, lem:regular_quotient}
  \leanok
  \dcref{custom}
...
\begin{proof}
  ...
  (\ref{lem:notMemSq_witness}). By \ref{lem:regular_quotient}, \(R/(x)\) is regular
  ...
  \ref{lem:regular_quotient} and the inductive hypothesis, so \((x')\) is prime; a
...
\begin{theorem}[A regular system of parameters is a regular sequence]
  ...
  \uses{lem:notMemSq_witness, thm:regular_isDomain, lem:regular_quotient}
  \leanok
...
\begin{proof}
  ...
  pick \(x \in \mathfrak m \setminus \mathfrak m^2\) (\ref{lem:notMemSq_witness}); then
  \(x \neq 0\) (zero lies in \(\mathfrak m^2\)) and \(R\) is a domain
  (\ref{thm:regular_isDomain}), so \(x\) is a non-zero-divisor on \(R\). By
  \ref{lem:regular_quotient}, \(R/(x)\) is regular local with
...
\begin{theorem}[Regular local rings are Cohen--Macaulay]
  ...
  \uses{def:cohen_macaulay, def:depth, thm:regular_system_isRegular}
  \leanok
  \dcref{custom}
...
\begin{proof}
  \leanok
  The side condition of Definition~\ref{def:depth} is in the supremum branch:
  \(\mathfrak m \cdot R = \mathfrak m \neq R\).

  \emph{Lower bound.} By \ref{thm:regular_system_isRegular} there is an \(R\)-regular
  ...
  Cohen--Macaulay (Definition~\ref{def:cohen_macaulay}).
\end{proof}

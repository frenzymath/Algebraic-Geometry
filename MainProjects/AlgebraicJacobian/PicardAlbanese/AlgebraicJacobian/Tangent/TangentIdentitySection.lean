...
/-!
# Tangent-space substrate: rational sections and the identity section (W5-T1, section layer)

The last generic layer of the Kleiman §5 tangent-space computation before it
lands on the Jacobian: a *section* of the structure morphism of a `k`-scheme
hits a `k`-rational point, so the tangent-space description
`overDualNumberAtEquivCotangentSpaceDual` of
`Tangent/TangentStalkAlgebra.lean` applies at it unconditionally. For a
...
- `AlgebraicGeometry.GroupScheme.identityDualNumberEquivCotangentSpaceDual`
  ...
  `G = d.J`; the wiring to the Jacobian datum lives in
  `Tangent/Pic0TangentSpace.lean`, and only the `H¹(C, 𝒪_C)` identification
  (T3/T4) remains.

## Provenance (W5-T1 port, cross-project)

Ported from `Algebraic-Jacobian-Challenge`,
`AlgebraicJacobian/Picard/TangentSpaceIdentitySection.lean` (sorry-free,
mathlib generality); see `Tangent/TangentDualNumbers.lean` and inbox `I-0495`.
-/

...
-/
import AlgebraicJacobian.Descent.FiniteGaloisQuotientAffine
...
(`AlgebraicJacobian.Picard.StableAffineCover`).
...
theorem exists_stable_affineOpen_of_orbitsInAffineOpen [FiniteDimensional K L]
  ...
  classical
  obtain ⟨U, hxU⟩ := OrbitsInAffineOpen.exists_affineOpen (rho := ρ) x

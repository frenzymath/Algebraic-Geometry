...
\end{align}
is surjective on global sections and $\mathcal{O}_{\PP}(k-2)$ is globally generated, it must be surjective as a morphism of sheaves. Morphism (\ref{bundle-ind-vero}) therefore induces a Veronese morphism $$\widetilde{\psi} : \PP(\mathrm{H}^0(E)) \to \PP(\mathrm{K}_{k-1,1}(X,L))$$ of degree $k-2$.

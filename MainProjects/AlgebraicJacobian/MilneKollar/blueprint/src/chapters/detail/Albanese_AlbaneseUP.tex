...
\label{mk-chap:Albanese_AlbaneseUP}
...
\label{mk-sec:albaneseup_setup}

The construction of the identity component of the Picard scheme gives
\[
  J:=\Pic^0_{C/k}.
\]
It is a group scheme by \ref{mk-thm:pic0_group_structure}, smooth by
\ref{mk-thm:pic0_smooth}, proper by \ref{mk-thm:pic0_proper}, and geometrically
irreducible by \ref{mk-thm:pic0_geom_irred}.

\begin{definition}
  [The Abel--Jacobi morphism]
  \label{mk-def:abel_jacobi_morphism}
  \dcref{custom}

  \uses{mk-def:pic_scheme, mk-thm:pic0_group_structure}
...
\begin{lemma}
  ...
  \label{mk-lem:abel_jacobi_morphism}
  \dcref{custom}
  \uses{mk-def:abel_jacobi_morphism}
...
\label{mk-sec:albaneseup_symmetric_powers}

\begin{definition}
  [Symmetric power of a curve]
  \label{mk-def:symmetric_power_curve}
  \dcref{custom}
...
\begin{lemma}
  ...
  \label{mk-lem:symmetric_power_projection_properties}
  \dcref{custom}
  \uses{mk-def:symmetric_power_curve}
...
\begin{definition}
  ...
  \label{mk-def:symmetric_power_curve_carrier}
  \dcref{custom}
...
\begin{lemma}
  ...
  \label{mk-lem:symmetric_power_nonsingular}
  \dcref{custom}
  \uses{mk-def:symmetric_power_curve}
...
\begin{definition}
  ...
  \label{mk-def:symmetric_product_av_map}
  \dcref{custom}

  \uses{mk-def:symmetric_power_curve_carrier}
...
\begin{lemma}
  ...
  \label{mk-lem:symmetric_product_av_map}
  \dcref{custom}
  \uses{mk-def:symmetric_power_curve, mk-def:symmetric_product_av_map}
...
\begin{definition}
  ...
  \label{mk-def:symmetric_product_to_jacobian_morphism}
  \dcref{custom}

  \uses{mk-def:abel_jacobi_morphism, mk-def:symmetric_product_av_map}
  \[
...
\begin{lemma}
  ...
  \label{mk-lem:symmetric_product_to_jacobian_formula}
  \dcref{custom}
  \uses{mk-def:symmetric_product_to_jacobian_morphism,
    mk-lem:abel_jacobi_morphism, mk-lem:symmetric_product_av_map}
...
\begin{lemma}
  ...
  \label{mk-lem:symmetric_product_general_divisor}
  \dcref{custom}
  \uses{mk-lem:symmetric_product_to_jacobian_formula}
...
\begin{proof}
  \uses{mk-lem:symmetric_power_projection_properties,
    mk-thm:adelic_serreDuality, mk-thm:adelic_riemannRoch}
...
\begin{lemma}
  ...
  \label{mk-lem:jacobian_invariant_one_forms}
  \dcref{custom}
...
\begin{proof}
  \uses{mk-thm:pic0_smooth, mk-thm:pic0_proper, mk-thm:pic0_geom_irred}
...
\begin{lemma}
  ...
  \label{mk-lem:abel_jacobi_differential}
  \dcref{custom}
  \uses{mk-def:abel_jacobi_morphism}
...
\begin{proof}
  \uses{mk-thm:pic0_tangent_space_iso, mk-thm:adelic_serreDuality}
...
\begin{lemma}
  ...
  \label{mk-lem:abel_jacobi_one_forms}
  \dcref{custom}
  \uses{mk-lem:jacobian_invariant_one_forms,
    mk-lem:abel_jacobi_differential}
...
\begin{proof}
  By \ref{mk-lem:jacobian_invariant_one_forms}, a form on \(J\) is determined
  by its value at the identity.  Under
  \ref{mk-lem:abel_jacobi_differential}, pulling it back along
...
\begin{lemma}
  ...
  \label{mk-lem:jacobian_dimension}
  \dcref{custom}
...
\begin{proof}
  \uses{mk-lem:abel_jacobi_one_forms, mk-thm:pic0_smooth}
  ...
  \ref{mk-lem:abel_jacobi_one_forms}, this space is isomorphic to
  \(H^0(C,\Omega_C^1)\), whose dimension is the genus \(g\).
\end{proof}

\begin{proposition}
  [Differentials on a symmetric power]
  \label{mk-prop:symmetric_product_differentials}
  \dcref{custom}
  \uses{mk-lem:symmetric_product_to_jacobian_formula}
...
\begin{proof}
  ...
  By \ref{mk-lem:abel_jacobi_one_forms}, it sends a form to
...
\begin{lemma}
  ...
  \label{mk-lem:symmetric_product_tangent_exact}
  \dcref{custom}
  \uses{mk-lem:symmetric_product_to_jacobian_formula}
...
\begin{proof}
  \uses{mk-prop:symmetric_product_differentials,
    mk-lem:symmetric_power_nonsingular, mk-thm:adelic_serreDuality,
    mk-thm:adelic_riemannRoch}
  ...
  fibre.  By \ref{mk-prop:symmetric_product_differentials}, the annihilator
...
\begin{lemma}
  ...
  \label{mk-lem:symmetric_product_to_jacobian}
  \dcref{custom}
  \uses{mk-lem:symmetric_product_to_jacobian_formula}
...
\begin{proof}
  \uses{mk-lem:symmetric_power_projection_properties,
    mk-lem:symmetric_product_general_divisor,
    mk-lem:symmetric_product_tangent_exact, mk-lem:jacobian_dimension,
  ...
  By \ref{mk-lem:symmetric_product_general_divisor}, the map is injective on
  ...
  so \ref{mk-lem:symmetric_product_tangent_exact} makes
  ...
  equality being \ref{mk-lem:jacobian_dimension}.  Hence
  \(W^g=J\).  Surjectivity follows because \(C^{(g)}\) is proper and its
  image is exactly \(W^g\).
\end{proof}

\section{Descent and the universal property}
\label{mk-sec:albaneseup_descent}

\begin{lemma}
  [Descent through the birational symmetric power]
  \label{mk-lem:descent_through_birational_sigma}
  \dcref{custom}

  \uses{mk-def:symmetric_product_av_map,
    mk-def:symmetric_product_to_jacobian_morphism}
...
\begin{proof}
  \uses{mk-lem:symmetric_product_av_map, mk-lem:symmetric_product_to_jacobian,
    mk-lem:symmetric_power_nonsingular, mk-thm:pic0_smooth,
    mk-thm:rational_map_to_av_extends}
  ...
  variety, so \ref{mk-thm:rational_map_to_av_extends} extends this rational
  ...
  \ref{mk-lem:symmetric_power_nonsingular}, hence reduced, and \(A\) is
...
\begin{definition}
  ...
  \label{mk-def:basepoint_section_symmetric_power}
  \dcref{custom}
  \uses{mk-def:symmetric_power_curve}
...
\begin{lemma}
  ...
  \label{mk-lem:basepoint_section_to_jacobian}
  \dcref{custom}
  \uses{mk-def:basepoint_section_symmetric_power,
    mk-lem:symmetric_product_to_jacobian_formula,
    mk-def:abel_jacobi_morphism}
...
\begin{lemma}
  ...
  \label{mk-lem:basepoint_section_symmetric_map}
  \dcref{custom}
  \uses{mk-def:basepoint_section_symmetric_power,
    mk-lem:symmetric_product_av_map}
...
\begin{lemma}
  ...
  \label{mk-lem:albanese_eq_iff_symmetricPower_eq}
  \dcref{custom}
...
\begin{proof}
  \uses{mk-lem:abel_jacobi_morphism, mk-lem:symmetric_product_av_map,
    mk-lem:symmetric_product_to_jacobian_formula,
    mk-lem:av_regular_map_is_hom, mk-lem:basepoint_section_to_jacobian,
    mk-lem:basepoint_section_symmetric_map}
  Suppose first that \(\varphi=\psi\circ\iota_{P_0}\).  Evaluating at
  \(P_0\) gives \(\psi(0_J)=0_A\), so
  \ref{mk-lem:av_regular_map_is_hom} shows that \(\psi\) is a homomorphism.
  ...
  \ref{mk-lem:basepoint_section_symmetric_map} and
  \ref{mk-lem:basepoint_section_to_jacobian} identify the resulting equation
...
\begin{theorem}
  ...
  \label{mk-thm:albanese_universal_property}
  \dcref{custom}
...
\begin{proof}
  \uses{mk-lem:descent_through_birational_sigma,
    mk-lem:albanese_eq_iff_symmetricPower_eq}
  By \ref{mk-lem:descent_through_birational_sigma}, there is a unique
  \(\psi\colon J\to A\) satisfying
  \(\varphi^{(g)}=\psi\circ f^{(g)}\).  The equivalence in
  \ref{mk-lem:albanese_eq_iff_symmetricPower_eq}, applied to every candidate
...
\begin{corollary}
  ...
  \label{mk-cor:albanese_universal_property_homomorphism}
  \dcref{custom}
  \uses{mk-thm:albanese_universal_property}

  In the situation of \ref{mk-thm:albanese_universal_property}, the unique
...
\begin{proof}
  \uses{mk-lem:av_regular_map_is_hom, mk-lem:abel_jacobi_morphism}
  Since \(\iota_{P_0}(P_0)=0_J\) and \(\varphi(P_0)=0_A\), the
  factorisation gives \(\psi(0_J)=0_A\).  The pointed rigidity theorem
  \ref{mk-lem:av_regular_map_is_hom} therefore makes \(\psi\) a group

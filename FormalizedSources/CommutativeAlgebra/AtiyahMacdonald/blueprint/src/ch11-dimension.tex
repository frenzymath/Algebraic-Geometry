...
\source{atiyah-macdonald-commutative-algebra:page-0125}
...
\label{am-thm-11-1}
\dcref{11.1}
\source{atiyah-macdonald-commutative-algebra:page-0126}
...
\label{am-prop-11-3}
\dcref{11.3}
\source{atiyah-macdonald-commutative-algebra:page-0127}
...
\dcref{11.4}
\source{atiyah-macdonald-commutative-algebra:page-0127}
Let $(A,\m)$ be Noetherian local, $\q$ $\m$-primary, and $M$ finite with a
stable $\q$-filtration.  Then $M/M_n$ has finite length; for large $n$ its
length is a polynomial $g(n)$ of degree at most the least number of generators
of $\q$; and the degree and leading coefficient are independent of the stable
filtration.
\end{proposition}
\begin{proof}
The graded pieces are finite modules over the Artin ring $A/\q$, so have finite
...
\label{am-cor-11-5}
\dcref{11.5}
\source{atiyah-macdonald-commutative-algebra:page-0128}
For an $\m$-primary $\q$, $\length(A/\q^n)$ is eventually a polynomial
$\chi_\q(n)$ of degree at most the least number of generators of $\q$.
\uses{am-prop-11-4}
\end{corollary}
\begin{proof}
Apply Proposition~\ref{am-prop-11-4} to $M=A$ with its $\q$-adic filtration.
\end{proof}
\begin{proposition}[Independence of the primary ideal]
\label{am-prop-11-6}
\dcref{11.6}
\source{atiyah-macdonald-commutative-algebra:page-0128}
For $\m$-primary $\q$, $\deg\chi_\q=\deg\chi_\m$.
\end{proposition}
\begin{proof}
For some $r$, $\m\supseteq\q\supseteq\m^r$.  Thus
$\chi_\m(n)\le\chi_\q(n)\le\chi_\m(rn)$ for large $n$, forcing equal degrees.
\end{proof}
\begin{definition}[The integer $d(A)$]
\label{am-def-hilbert-dimension}
\source{atiyah-macdonald-commutative-algebra:page-0128}
The common degree of the characteristic polynomials $\chi_\q(n)$, as $\q$
ranges over the $\m$-primary ideals, is denoted by $d(A)$.
\uses{am-prop-11-6}
\end{definition}
...
$\m$-primary ideal is at most $d(A)$.
...
\label{am-prop-11-8}
\dcref{11.8}
\source{atiyah-macdonald-commutative-algebra:page-0129}
If $x$ is a non-zero-divisor on a finite module $M$, then
$\deg\chi_{\q}(M/xM)\le\deg\chi_{\q}(M)-1$.
\end{proposition}
\begin{proof}
Set $N=xM\simeq M$ and filter $N$ by $N\cap\q^nM$.  The exact sequence
$0\to N/(N\cap\q^nM)\to M/\q^nM\to(M/xM)/\q^n(M/xM)\to0$ and
...
\end{proposition}
\begin{proof}
Induct on $d(A)$.  If $d=0$, lengths $\length(A/\m^n)$ stabilize, so
Nakayama gives $\m^n=0$ and $A$ is Artin.  For $d>0$, a prime chain
$\p_0\subsetneq\cdots\subsetneq\p_r$ and $x\in\p_1\setminus\p_0$ reduce to
the domain $A/\p_0$ and its quotient by $x$.  The regular-element inequality
...
\label{am-cor-11-12}
\dcref{11.12}
\source{atiyah-macdonald-commutative-algebra:page-0130}
...
\end{corollary}
\begin{proof}
The height of $\p$ is $\dim A_\p$ and is finite by Corollary~\ref{am-cor-11-11}.
\end{proof}
\begin{proposition}[Parameters]
\label{am-prop-11-13}
\dcref{11.13}
\source{atiyah-macdonald-commutative-algebra:page-0130}
If $\dim A=d$ for a Noetherian local ring $(A,\m)$, there is an
$\m$-primary ideal generated by $d$ elements.  Thus $d\le s(A)$, where $s(A)$
is the least number of generators of an $\m$-primary ideal.
\end{proposition}
\begin{proof}
Choose $x_i\in\m$ inductively outside the finite union of minimal primes of
...
prime over the ideal is $\m$, so it is $\m$-primary.
...
prime chain; the degree of $\chi_\m(n)=\length(A/\m^n)$; and the least number
of generators of an $\m$-primary ideal.
...
\dcref{11.15}
\source{atiyah-macdonald-commutative-algebra:page-0130}
For a Noetherian local ring, $\dim A\le\dim_{A/\m}(\m/\m^2)$.
\uses{am-prop-2-8,am-prop-11-13}
\end{corollary}
\begin{proof}
A basis of $\m/\m^2$ generates $\m$ by Proposition~\ref{am-prop-2-8}; apply
...
\label{am-cor-11-17}
\dcref{11.17}
\source{atiyah-macdonald-commutative-algebra:page-0131}
...
\dcref{11.18}
\source{atiyah-macdonald-commutative-algebra:page-0131}
If $x\in\m$ is a non-zero-divisor in a Noetherian local ring, then
$\dim A/(x)=\dim A-1$.
\uses{am-cor-11-9,am-thm-11-14}
\end{corollary}
\begin{proof}
The upper bound is Corollary~\ref{am-cor-11-9}; parameters for $A/(x)$ lift
to an $\m$-primary ideal generated by one additional element in $A$, giving
...
$f$ of degree $s$ satisfies $f(x_1,\ldots,x_d)\in\q^{s+1}$ for
$\q=(x_1,\ldots,x_d)$, then every coefficient of $f$ lies in $\m$.
\uses{am-thm-11-14,am-prop-11-3}
\end{proposition}
\begin{proof}
Reduce modulo $\m$ and map the polynomial ring over $A/\q$ onto
$\operatorname{gr}_\q(A)$.  A nonzero reduced homogeneous relation would lower
...
\label{am-cor-11-21}
\dcref{11.21}
\source{atiyah-macdonald-commutative-algebra:page-0132}
If a field $k\subseteq A$ maps isomorphically onto $A/\m$, a system of
parameters is algebraically independent over $k$.
\uses{am-prop-11-20}
\end{corollary}
\begin{proof}
The lowest homogeneous part of a polynomial relation has coefficients in $k$
and Proposition~\ref{am-prop-11-20} forces them into $\m$, hence to zero.
\end{proof}
...
$\operatorname{gr}_\m(A)\simeq k[t_1,\ldots,t_d]$;
$\dim_k\m/\m^2=d$; and $\m$ is generated by $d$ elements.  Such a ring is
regular local.
\end{theorem}
\begin{proof}
The first implication reads degree one.  The second gives generators by
Proposition~\ref{am-prop-2-8}.  If $\m=(x_1,\ldots,x_d)$, the map
$k[t_1,\ldots,t_d]\to\operatorname{gr}_\m(A)$ is onto, and Proposition~\ref{am-prop-11-20}
...
\dcref{11.23}
\source{atiyah-macdonald-commutative-algebra:page-0132}
If $\bigcap_n\aideal^n=0$ and $\operatorname{gr}_\aideal(A)$ is a domain, then
$A$ is a domain.
\end{lemma}
\begin{proof}
...
\label{am-prop-11-24}
\dcref{11.24}
\source{atiyah-macdonald-commutative-algebra:page-0133}
...
local dimension $\dim A(V)_\m$ is independent of the point and equals
...
\label{am-lem-11-26}
\dcref{11.26}
\source{atiyah-macdonald-commutative-algebra:page-0134}
If $B\subseteq A$ are domains, $B$ integrally closed, $A$ integral over $B$,
and $\m\in\Max A$ contracts to $\n$, then $\n$ is maximal and
$\dim A_\m=\dim B_\n$.
\end{lemma}
\begin{proof}
...
for chains of primes after contraction, and going down (Theorem~\ref{am-thm-5-16})
 lifts every chain in $B_\n$ to one in $A_\m$; hence equality.

...
\begin{lemma}[Extend curves to spaces]
...
\begin{lemma}[Polarized curves in polarized]
\label{moduli-curves-lemma-polarized-curves-in-polarized}
\source{stacks:0DPZ}
\href{https://stacks.math.columbia.edu/tag/0DPZ}{\texttt{Stacks tag 0DPZ}}
\group{moduli-curves}
...
\begin{lemma}[Polarized curves over curves]
...
\begin{lemma}[Etale locally scheme]
...
Lemma \ref{moduli-curves-lemma-polarized-curves-over-curves}.
...
Remark \ref{moduli-curves-remark-boundedness-aut-does-not-work-surfaces}.

\begin{lemma}[Curves diagonal separated fp]
...
is surjective and smooth by Lemma \ref{moduli-curves-lemma-polarized-curves-over-curves}.
...
\begin{remark}[Boundedness aut does not work surfaces]
...
This is immediate from Lemma \ref{moduli-curves-lemma-curves-diagonal-separated-fp}.
...
of the diagram
$$
\begin{array}{cc}R \underset{s}{\longrightarrow}\underset{t}{\downarrow} & U \downarrow \\ U \longrightarrow & \Curvesstack\end{array}
$$
...
$$
\begin{array}{cc}T \underset{p}{\longrightarrow}\underset{q}{\downarrow} & U \downarrow \\ S \longrightarrow & \Curvesstack\end{array}
$$
...
By Lemma \ref{moduli-curves-lemma-curves-diagonal-separated-fp}
...
\begin{lemma}[In DM locus vector fields]
...
this uses Lemma \ref{moduli-curves-lemma-curves-diagonal-separated-fp} and
More on Groupoids in Spaces, Lemma
\ref{spaces-more-groupoids-lemma-group-space-scheme-locally-finite-type-over-k}.
...
by the method discussed in Section \ref{moduli-curves-section-open}.
\end{proof}

\begin{lemma}[CM 1 curves]
...
\begin{proof}
By Lemma \ref{moduli-curves-lemma-CM-curves} it is clear that we have
...
Lemmas \ref{moduli-curves-lemma-pre-genus-curves} and \ref{moduli-curves-lemma-CM-1-curves}.
\end{proof}
...
\begin{lemma}[Pre genus one piece per genus]
...
lemma not involving $R^1f_*$ or $H^1$, see Lemma \ref{moduli-curves-lemma-pre-genus-curves}.
The existence of the decomposition into open and closed substacks
follows immediately from the discussion in Section \ref{moduli-curves-section-open}
and Lemma \ref{moduli-curves-lemma-genus}. This proves the characterization in (1).
...
\begin{lemma}[Geometrically reduced curves]
...
\begin{lemma}[Geometrically reduced connected 1 curves]
...
\begin{proof}
By Lemmas \ref{moduli-curves-lemma-geometrically-reduced-curves},
\ref{moduli-curves-lemma-geomred-in-CM}, \ref{moduli-curves-lemma-CM-curves}, and \ref{moduli-curves-lemma-CM-1-curves}
...
\begin{lemma}[Geomredcon in h0 1]
...
\ref{varieties-lemma-proper-geometrically-reduced-global-sections}
and Lemmas \ref{moduli-curves-lemma-pre-genus-curves} and
\ref{moduli-curves-lemma-geometrically-reduced-connected-1-curves}.
The final statement follows from Lemma \ref{moduli-curves-lemma-genus}.
\end{proof}

\begin{lemma}[One piece per genus]
...
and combine Lemmas \ref{moduli-curves-lemma-geomredcon-in-h0-1} and
\ref{moduli-curves-lemma-pre-genus-one-piece-per-genus}.
Second proof:
The existence of the decomposition into open and closed substacks
follows immediately from the discussion in Section \ref{moduli-curves-section-open}
and Lemma \ref{moduli-curves-lemma-geomredcon-in-h0-1}.
...
\begin{lemma}[Gorenstein 1 curves]
\label{moduli-curves-lemma-gorenstein-1-curves}
\source{stacks:0E6M}
\href{https://stacks.math.columbia.edu/tag/0E6M}{\texttt{Stacks tag 0E6M}}
\group{moduli-curves}
...
Lemmas \ref{moduli-curves-lemma-gorenstein-curves} and \ref{moduli-curves-lemma-CM-1-curves}.
\end{proof}
...
In Section \ref{moduli-curves-section-open} we have seen
...
\begin{lemma}[Isolated sings curves]
...
of finite presentation (Lemma \ref{moduli-curves-lemma-curves-qs-lfp})
...
\begin{lemma}[Big smooth part curves]
...
(1) holds by combining Lemma \ref{moduli-curves-lemma-in-smooth-locus} with
...
Lemmas \ref{moduli-curves-lemma-lci-curves} and \ref{moduli-curves-lemma-isolated-sings-curves}.
\end{proof}
...
\begin{lemma}[Smooth curves]
\label{moduli-curves-lemma-smooth-curves}
\source{stacks:0DZZ}
\href{https://stacks.math.columbia.edu/tag/0DZZ}{\texttt{Stacks tag 0DZZ}}
\group{moduli-curves}
...
shown for $\Curvesstack^{smooth}$ and Lemma \ref{moduli-curves-lemma-CM-1-curves}.
\end{proof}

\begin{lemma}[Smooth curves smooth]
...
and Lemma \ref{moduli-curves-lemma-big-smooth-part-curves}.
\end{proof}

\begin{lemma}[Smooth curves h0]
...
Lemmas \ref{moduli-curves-lemma-pre-genus-curves} and \ref{moduli-curves-lemma-smooth-curves}.
...
\begin{definition}[Deligne mumford smooth]
...
\begin{lemma}[Smooth one piece per genus]
...
\begin{proof}
Combine Lemmas \ref{moduli-curves-lemma-smooth-curves-h0} and
\ref{moduli-curves-lemma-pre-genus-one-piece-per-genus}.
You can also use
Lemma \ref{moduli-curves-lemma-one-piece-per-genus}
instead.
\end{proof}

\begin{lemma}[Smooth curves h0 smooth]
\label{moduli-curves-lemma-smooth-curves-h0-smooth}
\source{stacks:0E84}
\href{https://stacks.math.columbia.edu/tag/0E84}{\texttt{Stacks tag 0E84}}
\group{moduli-curves}
...
Lemma \ref{moduli-curves-lemma-big-smooth-part-curves}.
\end{proof}
...
See Lemma \ref{moduli-curves-lemma-big-smooth-part-curves}.
...
\begin{lemma}[Nodal curves smooth]
...
$$
\begin{array}{cc}X_U \underset{f'}{\downarrow}\underset{g'}{\longrightarrow} & X \overset{f}{\downarrow} \\ U \overset{g}{\longrightarrow} & S\end{array}
$$
...
$X_i = X \times_S S_i$ is a scheme, see Lemma \ref{moduli-curves-lemma-etale-locally-scheme}.
...
Lemmas \ref{moduli-curves-lemma-CM-1-curves} and \ref{moduli-curves-lemma-curves-qs-lfp}),
...
\source{stacks:0E6Q}
\href{https://stacks.math.columbia.edu/tag/0E6Q}{\texttt{Stacks tag 0E6Q}}
\group{moduli-curves}
...
$$
studied in Lemma \ref{moduli-curves-lemma-CM-dualizing}
is called the {\it relative dualizing sheaf} of $f$.
\end{definition}

In the situation of Definition \ref{moduli-curves-definition-relative-dualizing-sheaf}
...
\begin{lemma}[Gorenstein dualizing]
...
Lemma \ref{moduli-curves-lemma-CM-dualizing} and deduce the result from
...
Lemmas \ref{moduli-curves-lemma-pre-genus-curves} and \ref{moduli-curves-lemma-nodal-curves}.
\end{proof}
...
Lemma \ref{moduli-curves-lemma-prestable-one-piece-per-genus}.

\begin{lemma}[Prestable one piece per genus]
...
follows from Lemmas \ref{moduli-curves-lemma-prestable-curves} and
\ref{moduli-curves-lemma-pre-genus-one-piece-per-genus}.
\end{proof}

\begin{lemma}[Prestable curves smooth]
\label{moduli-curves-lemma-prestable-curves-smooth}
\source{stacks:0E6W}
\href{https://stacks.math.columbia.edu/tag/0E6W}{\texttt{Stacks tag 0E6W}}
\group{moduli-curves}
...
Lemma \ref{moduli-curves-lemma-nodal-curves-smooth}.
\end{proof}
...
\begin{proof}
Assume (2). Using the material in Section \ref{moduli-curves-section-relative-dualizing}
...
compatibly with further base change, see Lemma \ref{moduli-curves-lemma-genus}.
...
our definitions in Section \ref{moduli-curves-section-relative-dualizing}.}.
...
Motivated by Lemma \ref{moduli-curves-lemma-semistable} we make the following definition.
...
in accordance with Definition \ref{moduli-curves-definition-semistable}.

\medskip\noindent
By the discussion in Section \ref{moduli-curves-section-open}
it suffices to look at families $f : X \to S$ of
prestable curves. By Lemma \ref{moduli-curves-lemma-semistable}
...
\begin{lemma}[Semistable one piece per genus]
\label{moduli-curves-lemma-semistable-one-piece-per-genus}
\source{stacks:0E71}
\href{https://stacks.math.columbia.edu/tag/0E71}{\texttt{Stacks tag 0E71}}
\group{moduli-curves}
...
\begin{proof}
Combine Lemmas \ref{moduli-curves-lemma-semistable-curves} and
\ref{moduli-curves-lemma-prestable-one-piece-per-genus}.
\end{proof}

\begin{lemma}[Semistable curves smooth]
...
Motivated by Lemma \ref{moduli-curves-lemma-stable} we make the following definition.

\begin{definition}[Stable]
\label{moduli-curves-definition-stable}
\source{stacks:0E75}
\href{https://stacks.math.columbia.edu/tag/0E75}{\texttt{Stacks tag 0E75}}
\group{moduli-curves}
...
Lemma \ref{moduli-curves-lemma-stable} above.
This shows that our definition agrees with most definitions one finds
in the literature.

\begin{lemma}[Stable curves]
...
By the discussion in Section \ref{moduli-curves-section-open}
it suffices to look at families $f : X \to S$ of
prestable curves. By Lemma \ref{moduli-curves-lemma-stable}
we obtain the desired openness of the locus in question.
Formation of this open commutes with arbitrary base change,
...
\begin{lemma}[Stable one piece per genus]
...
\begin{proof}
Combine Lemmas \ref{moduli-curves-lemma-stable-curves} and
\ref{moduli-curves-lemma-prestable-one-piece-per-genus}.
\end{proof}

\begin{lemma}[Stable curves smooth]
...
\begin{lemma}[Stable curves deligne mumford]
...
By Lemmas \ref{moduli-curves-lemma-stable-curves}, \ref{moduli-curves-lemma-DM-curves}, and
\ref{moduli-curves-lemma-in-DM-locus-vector-fields} it suffices to show that
$\text{Der}_k(\mathcal{O}_X, \mathcal{O}_X) = 0$.
This is proven in
...
\begin{lemma}[Smooth dense in stable]
...
Lemma \ref{moduli-curves-lemma-smooth-dense}.
\end{proof}
...
\overline{\mathcal{M}}_g
$$
See Lemma \ref{moduli-curves-lemma-stabilization-morphism}.
...
\'etale topology on $S$.
Thus by Lemma \ref{moduli-curves-lemma-etale-locally-scheme} we may assume
...
\begin{lemma}[Contract basic uniqueness]
...
see Lemma \ref{moduli-curves-lemma-curves-qs-lfp} and
Limits of Stacks, Lemma
\ref{stacks-limits-lemma-representable-by-spaces-limit-preserving}.
...
Lemma \ref{moduli-curves-lemma-curves-qs-lfp} and
Limits of Stacks, Lemma
\ref{stacks-limits-lemma-representable-by-spaces-limit-preserving}.
...
$$
\begin{array}{ccccc}\ldots \longrightarrow & X_n \longrightarrow\downarrow & X_{n - 1} \longrightarrow\downarrow & \ldots \longrightarrow & X_s \downarrow \\ \ldots \longrightarrow & Y_n \longrightarrow\downarrow & Y_{n - 1} \longrightarrow\downarrow & \ldots \longrightarrow & Y_0 \downarrow \\ \ldots \longrightarrow & \Spec(\Lambda/\mathfrak m^{n + 1}) \longrightarrow & \Spec(\Lambda/\mathfrak m^n) \longrightarrow & \ldots \longrightarrow & \Spec(\kappa)\end{array}
$$
...
\begin{lemma}[Contract prestable to stable]
...
By Lemma \ref{moduli-curves-lemma-contract-basic}
...
Lemmas \ref{moduli-curves-lemma-stable-curves} and \ref{moduli-curves-lemma-stable-one-piece-per-genus}.
After possibly shrinking $U$ once more, assertion (1) of the lemma for
$c : X_U \to Y$ follows from Lemma \ref{moduli-curves-lemma-contract}.
...
at least locally on $S$ by Lemma \ref{moduli-curves-lemma-contract-basic-uniqueness}.
...
\begin{lemma}[Stabilization morphism]
...
Lemma \ref{moduli-curves-lemma-contract-prestable-to-stable} are stable
...
by Lemma \ref{moduli-curves-lemma-contract-prestable-to-stable}. Since the generic
...
promised in Lemma \ref{moduli-curves-lemma-stable-reduction}
is unique up to unique isomorphism.

\begin{lemma}[Unique stable model]
...
(Lemma \ref{moduli-curves-lemma-contract-prestable-to-stable}). We claim
...
By either
Lemma \ref{moduli-curves-lemma-contract-basic-uniqueness} or
Lemma \ref{moduli-curves-lemma-contract-prestable-to-stable}
...
From Lemma \ref{moduli-curves-lemma-stable-reduction} and
...
\begin{proof}
Part (1) is an immediate consequence of Lemma \ref{moduli-curves-lemma-stable-reduction} and
Semistable Reduction, Theorem \ref{models-theorem-semistable-reduction}.
...
By our choice of $L$ and $A$ and Lemma \ref{moduli-curves-lemma-stable-reduction}
...
locally of finite presentation by Lemma \ref{moduli-curves-lemma-curves-qs-lfp}.
...
By Lemma \ref{moduli-curves-lemma-smooth-dense-in-stable} the open immersion
...
$$
\begin{array}{ccc}\Spec(K) \longrightarrow\downarrow & \mathcal{M}_g \longrightarrow & \overline{\mathcal{M}}_g \downarrow \\ \Spec(R) \longrightarrow\nearrow &  & \Spec(\mathbf{Z})\end{array}
$$
...
Lemma \ref{moduli-curves-lemma-unique-stable-model}.
\end{proof}

\begin{lemma}[Stable quasi compact]
...
\begin{proof}
We will use the notation from Section \ref{moduli-curves-section-polarized-curves}.
Consider the subset
$$
...
By Lemma \ref{moduli-curves-lemma-polarized-curves-in-polarized} we see that
$$
...
\begin{theorem}[Stable smooth proper]
...
Smoothness is Lemma \ref{moduli-curves-lemma-stable-curves-smooth}.
Deligne-Mumford is Lemma \ref{moduli-curves-lemma-stable-curves-deligne-mumford}.
Openness of $\mathcal{M}_g$ is Lemma \ref{moduli-curves-lemma-smooth-dense-in-stable}.
We know that $\overline{\mathcal{M}}_g \to \Spec(\mathbf{Z})$
is separated by Lemma \ref{moduli-curves-lemma-stable-separated} and we know that
$\overline{\mathcal{M}}_g$ is quasi-compact by
Lemma \ref{moduli-curves-lemma-stable-quasi-compact}.
...
$$
\begin{array}{ccc}\Spec(K) \longrightarrow\underset{j}{\downarrow} & \mathcal{M}_g \longrightarrow & \overline{\mathcal{M}}_g \downarrow \\ \Spec(A) \longrightarrow &  & \Spec(\mathbf{Z})\end{array}
$$
...
induced diagram
$$
\begin{array}{cc}\Spec(K') \longrightarrow\underset{j'}{\downarrow} & \overline{\mathcal{M}}_g \downarrow \\ \Spec(A') \longrightarrow\nearrow & \Spec(\mathbf{Z})\end{array}
$$

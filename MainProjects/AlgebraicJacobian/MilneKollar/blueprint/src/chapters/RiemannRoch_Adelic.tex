...
% archon:covers AlgebraicJacobian/RiemannRoch/Adelic/CechAcyclicInstance.lean
% archon:covers AlgebraicJacobian/RiemannRoch/Adelic/CechComparisonGate.lean
% archon:covers AlgebraicJacobian/RiemannRoch/Adelic/FinitenessP1.lean
% archon:covers AlgebraicJacobian/RiemannRoch/Adelic/FiniteMapToP1.lean
% archon:covers AlgebraicJacobian/RiemannRoch/Adelic/NonconstantToP1.lean
% archon:covers AlgebraicJacobian/RiemannRoch/Adelic/P1BaseCase.lean
% archon:covers AlgebraicJacobian/RiemannRoch/Adelic/P1ChartData.lean
% archon:covers AlgebraicJacobian/RiemannRoch/Adelic/ChiLedger.lean
% archon:covers AlgebraicJacobian/RiemannRoch/Adelic/GenusFiniteness.lean
% archon:covers AlgebraicJacobian/RiemannRoch/Adelic/GenusUnconditional.lean
% archon:covers AlgebraicJacobian/RiemannRoch/Adelic/GateInstances.lean
...
\Cref{chap:RiemannRoch_WeilDivisor} assigns to every prime divisor \(P\)
an order \(v_P\colon K^\times\to\mathbb Z\).  The Mayer--Vietoris theory
of \Cref{chap:Cohomology_MayerVietoris} computes the first cohomology of
...
\]
The bridge of \Cref{sec:RiemannRoch_Adelic_cokernel} identifies
...
of \Cref{def:divisor_degree}; over a general field it does not.  In this
...
\begin{definition}[Point valuation]
  ...
  \leanok
  \uses{def:prime_divisor, def:isRegularInCodimensionOne}
  \dcref{custom}
...
\begin{lemma}[Order is the additive normalization of the point valuation]
  ...
  \leanok
  \uses{def:adelic_pointValuation, def:order_at_point}
...
\begin{proof}
  ...
  \Cref{def:order_at_point}.
\end{proof}

\begin{definition}[Residue field and residue degree]
  \label{def:adelic_residue_degree}
  \uses{thm:adelic_chartRing_isDedekindDomain, def:adelic_pointValuation}
  \dcref{custom}
...
\begin{definition}[Residue-weighted degree]
  ...
  \uses{def:adelic_residue_degree, def:codim1_cycles}
  \dcref{custom}
...
\begin{theorem}[Weighted degree of a principal divisor]
  ...
  \uses{def:adelic_divisor_degree,
    thm:adelic_exists_finiteMorphismToP1}
  \dcref{custom}
...
\begin{definition}[The function-field twist cokernel]
  ...
  \leanok
  \uses{def:adelic_sectionOfDivisor}
...
\begin{proposition}[Comparison at the zero divisor]
  ...
  \dcref{custom}
  \uses{def:adelic_coverH1, def:adelic_function_field_H1}
...
[The Čech cosimplicial object of a cover]
  ...
  \uses{def:adelic_coverH1, def:Scheme_cechCohomology}
...
[Cofaces are restriction maps]
  ...
  \uses{def:adelic_cechCosimplicial}
...
[The degree-\(0\) and degree-\(1\) Čech differentials]
  ...
  \uses{lem:adelic_alternatingCofaceMapComplex_objD,
    lem:adelic_cech_coface_restriction}
...
\begin{proof}
  \leanok
  Immediate from \Cref{lem:adelic_alternatingCofaceMapComplex_objD} at
...
\begin{proof}
  \leanok
  \uses{lem:adelic_cech_d01_d12_formula, lem:adelic_cech_coface_restriction}
  ...
  repeats a value, so by \Cref{lem:adelic_cech_d01_d12_formula} the
  ...
  \Cref{lem:adelic_cech_coface_restriction} (parallel restrictions
...
[The structure-sheaf \(H^1\) is the cover cokernel]
  ...
  \leanok
  \uses{thm:adelic_affine_h1_vanishing,
    thm:adelic_mv_h1_cokernel_equiv}
...
\begin{proof}
  \leanok
  Apply \Cref{thm:adelic_mv_h1_cokernel_equiv} to the structure sheaf;
  its two vanishing hypotheses are \Cref{thm:adelic_affine_h1_vanishing}.
...
By \Cref{thm:adelic_HModuleOne_linearEquiv_cokernel}, it remains to prove
...
[A nonconstant map to \(\mathbb{P}^1\) is finite]
  ...
  \uses{def:adelic_nonconstant_map_gate, thm:adelic_nonconstant_map_isFinite}
...
\begin{proof}
  ...
  (\Cref{def:adelic_nonconstant_map_gate}), with \(\pi(x_1) \ne
  \pi(x_2)\) for some \(x_1, x_2 \in C\). Since \(\mathbb{P}^1_k\) is
  separated and of finite type over \(k\), \Cref{thm:adelic_nonconstant_map_isFinite}
...
[Integral-model bridge to \(\mathbb P^1_k\)]
  ...
  \uses{thm:adelic_exists_nonconstant_projint, def:adelic_nonconstant_map_gate}
...
[Dimension one for standard-smooth curves]
  ...
  \uses{lem:adelic_quasiFinite_dim_le}
...
\begin{proof}
  ...
  \Cref{lem:adelic_quasiFinite_dim_le} gives
...
[Stalks of the curve have dimension \(\le 1\)]
  ...
  \uses{lem:adelic_relDimOne_dim_le_one}
...
\begin{proof}
  ...
  \Cref{lem:adelic_relDimOne_dim_le_one} applies directly.
\end{proof}
...
[Coheight \(\le 1\) on the curve]
  ...
  \uses{lem:adelic_curve_stalk_dim_le_one, thm:ringKrullDim_stalk_eq_coheight}
  Every point \(z\) of \(C\) has topological coheight
  \(\operatorname{coheight} z \le 1\).
\end{lemma}
\begin{proof}
  \leanok
  Immediate from \Cref{lem:adelic_curve_stalk_dim_le_one} and the
  stalk--coheight bridge \Cref{thm:ringKrullDim_stalk_eq_coheight}:
  \[
...
[Non-generic points of the curve are closed]
  ...
  \uses{lem:adelic_curve_coheight_le_one}
...
[Proper closed subsets of the curve are finite]
  ...
  \uses{lem:adelic_nongeneric_closed}
...
\begin{proof}
  ...
  \ne \eta\), which is closed by \Cref{lem:adelic_nongeneric_closed}.
...
[Fibres of a nonconstant map are finite]
  ...
  \uses{lem:adelic_proper_closed_finite}
...
\begin{proof}
  ...
  by \Cref{lem:adelic_proper_closed_finite}. If \(\{y\}\) is not closed,
  ...
  (\Cref{lem:adelic_nongeneric_closed}: every non-generic point of
...
[Nonconstant morphisms from the curve are finite]
  ...
  \leanok
  \lean{AlgebraicGeometry.Adelic.isFinite_left_of_exists_ne}
  \uses{lem:adelic_nonconstant_finite_fibres}
...
\begin{proof}
  ...
  (\Cref{lem:adelic_nonconstant_finite_fibres}). A proper, locally
...
[The two-cover cokernel of the curve is finite-dimensional]
  ...
  \leanok
  \uses{thm:adelic_exists_finiteMorphismToP1, thm:adelic_chart_finite_over_P1,
    thm:adelic_cokernel_P1_zero, def:adelic_coverH1}
...
\begin{proof}
  ...
  source of the Laurent split of \Cref{thm:adelic_cokernel_P1_zero}),
...
\label{sec:RiemannRoch_Adelic_P1ChartData}
...
[Span of the projective-line chart]
  ...
  \uses{lem:adelic_p1Monomial_eq_coordPow}
...
\begin{proof}
  ...
  summand is, by \Cref{lem:adelic_p1Monomial_eq_coordPow}, the constant
...
[Laurent chart data on \(\mathbb P^1\)]
  ...
  \uses{def:projective_space, thm:adelic_span_p1CoordAway_pow_top}
...
[Finite-dimensionality of \(H^1(C,\struct C)\)]
  ...
  \uses{thm:adelic_HModuleOne_linearEquiv_cokernel,
    thm:adelic_finiteDimensional_cokernel_zero,
    thm:adelic_exists_finiteMorphismToP1,
    thm:adelic_P1_laurent_chart_data, def:genus}
...
\begin{proof}
  ...
  \Cref{thm:adelic_finiteDimensional_cokernel_zero}, and
  \Cref{thm:adelic_HModuleOne_linearEquiv_cokernel} transports this
...
[The coboundary and the linear system land in the overlap]
  ...
  \uses{def:adelic_coboundary, def:adelic_sectionOfDivisor,
    lem:adelic_sectionOfDivisor_antitone_open}
...
\begin{proof}
  ...
  open (\Cref{lem:adelic_sectionOfDivisor_antitone_open}: a larger open
...
[Exactness at the \(L\)-terms]
  ...
  \uses{lem:adelic_coboundary_le_overlap}
...
[The function-field quotient and the window map]
  ...
  \uses{def:adelic_function_field_H1, def:adelic_coboundary,
    lem:adelic_coboundary_le_overlap}
...
[The window map is injective]
  ...
  \uses{def:adelic_H1_window, thm:adelic_linearSystem_inf_overlap_eq}
...
\begin{proof}
  ...
  \Cref{thm:adelic_linearSystem_inf_overlap_eq} this forces \(f \in
  L(D)\), i.e.\ \([f] = 0\).
\end{proof}
...
\begin{proposition}[Comparison with the divisorial sheaf]
  ...
  \dcref{custom}
  \uses{def:adelic_divisorial_sheaf,
    def:adelic_function_field_H1,
    thm:adelic_mv_h1_cokernel_equiv}
...
[Twist exact sequence]
  ...
  \dcref{custom}
  \uses{def:adelic_divisorial_sheaf,
    thm:adelic_function_field_H1_compare}
...
[The single-point order-\(\ge m\) subgroup]
  ...
  \uses{def:adelic_pointValuation}
...
[The local step identity]
  ...
  \uses{def:adelic_orderGe, def:adelic_sectionOfDivisor}
...
[The local step quotient injects into the valuation quotient]
  ...
  \uses{thm:adelic_local_step_identity}
...
\begin{proof}
  \leanok
  Injectivity transports \Cref{thm:adelic_local_step_identity} (a
...
[The local step is bounded by the residue degree]
  ...
  \uses{thm:adelic_local_step_injective, thm:adelic_constant_field_order_zero,
    def:adelic_pointValuation}
  Put
  \[
    \deg_P^{\mathrm{val}}
...
\begin{proof}
  \leanok
  The injection of \Cref{thm:adelic_local_step_injective} lands in a
...
\begin{theorem}[Residue degree one over an algebraically closed base]
  ...
  \uses{def:adelic_residue_degree}
...
\begin{corollary}[The weighted degree is the geometric degree]
  ...
  \uses{def:adelic_divisor_degree, thm:adelic_residue_degree_one}
...
\begin{proof}
  \leanok
  \uses{thm:adelic_residue_degree_one}
  ...
  right. The weights agree by \Cref{thm:adelic_residue_degree_one}.
\end{proof}

\begin{lemma}[The valuation quotient is the residue field]
  \label{lem:adelic_valuation_quotient_residue_field}
  \dcref{custom}
  \uses{def:adelic_residue_degree, def:adelic_orderGe}
...
[The local step has residue degree under approximation]
  ...
  \leanok
  \uses{thm:adelic_local_step_injective,
    thm:adelic_local_step_dim_le_residue_degree}
...
\begin{proof}
  ...
  injective by \Cref{thm:adelic_local_step_injective}.  Hence it is an
...
\begin{proposition}[Sections of a finite-support quotient]
  ...
  \dcref{custom}
  \notready
  \uses{def:adelic_divisorial_sheaf, def:codim1_cycles}
...
\begin{proposition}[The one-point stalk quotient]
  ...
  \notready
  \uses{def:adelic_divisorial_sheaf,
    lem:adelic_valuation_quotient_residue_field}
  If \(D'=D+P\) and
...
[Dimension of the twist quotient]
  ...
  \dcref{custom}
  \uses{thm:adelic_finite_support_quotient_sections,
    thm:adelic_twist_stalk_quotient,
    def:adelic_divisor_degree}
...
\end{proposition}
\begin{proof}
  By \Cref{thm:adelic_finite_support_quotient_sections}, global sections
  ...
  \(\kappa(P)\) by \Cref{thm:adelic_twist_stalk_quotient}, and therefore has
  \(k\)-dimension \(\deg P\).  Additivity of dimension along the filtration
  gives
  \(\dim_k \mathcal{Q}(D, D') = \sum_P (D'(P) - D(P))\deg P = \deg_k(D' - D)\),
  the residue-weighted degree of \Cref{def:adelic_divisor_degree}.
\end{proof}
...
[The \(\chi\)-ledger dimensions]
  ...
  \uses{def:adelic_H1_window, thm:adelic_constant_field_order_zero}
...
\begin{corollary}[The base value of the divisor ledger]
  ...
  \dcref{custom}
  \uses{thm:adelic_global_sections_constants,
    def:adelic_divisorial_sheaf, def:adelic_sectionOfDivisor}
...
\begin{proof}
  ...
  \Cref{thm:adelic_global_sections_constants} identifies this space with
  \(k\).
\end{proof}
...
[Conditional overlap-ledger identity]
  ...
  \uses{lem:adelic_finrank_alternating_exact4,
    thm:adelic_windowMap_injective, def:adelic_chi_ledger}
...
\begin{proof}
  \leanok
  \Cref{lem:adelic_finrank_alternating_exact4} applied to the four-term
  sequence, whose injectivity at the left is
  \Cref{thm:adelic_windowMap_injective}.
\end{proof}
...
[One-step \(\chi\)-additivity]
  ...
  \uses{thm:adelic_ledger_alternating,
    thm:adelic_local_step_dim_eq_residue_degree}
  Under the exactness and finite-dimensionality hypotheses of
  \Cref{thm:adelic_ledger_alternating} for \(D\le D'\),
  \[
...
\begin{proof}
  ...
  \Cref{thm:adelic_ledger_alternating} and rearranging with
  \(\chi = \ell - h^1\) gives the first claim.  The one-point formula
  follows from \Cref{thm:adelic_local_step_dim_eq_residue_degree}.
\end{proof}
...
\begin{theorem}[Telescoping along an effective divisor]
  ...
  \leanok
  \uses{thm:adelic_chi_additivity}
...
[Euler characteristic of a divisor]
  ...
  \uses{thm:adelic_twist_exact_sequence,
    thm:adelic_dim_adele_quotient_eq_degree,
    thm:adelic_module_finite_genus,
    thm:adelic_function_field_H1_zero_compare,
    thm:adelic_ell_zero, def:genus}
  \dcref{custom}
...
\begin{proof}
  ...
  \Cref{thm:adelic_ell_zero}.  Moreover,
  \(i(0)=\dim_k\check H^1_{\mathrm{ff}}(0)=g\) by
  \Cref{thm:adelic_function_field_H1_zero_compare} and
  \cref{thm:adelic_HModuleOne_linearEquiv_cokernel}. Hence
  \(\chi(D) = \deg D + 1 - g\).
\end{proof}
...
\begin{proof}
  ...
  \Cref{lem:adelic_chi_le_ell}.
\end{proof}

\begin{corollary}
[Riemann inequality and finiteness of the twist cokernel]
  \label{thm:adelic_riemann_inequality}
  \uses{thm:adelic_chi_eq_chi_zero_add_degree, thm:adelic_module_finite_genus, thm:adelic_twist_exact_sequence, thm:adelic_dim_adele_quotient_eq_degree, lem:adelic_riemann_inequality_conditional}
  \dcref{custom}
...
\begin{proof}
  ...
  \Cref{lem:adelic_riemann_inequality_conditional} specialises, with
  \(\chi(0) = 1-g\)
  (\Cref{thm:adelic_chi_eq_chi_zero_add_degree}), to
...
\begin{proposition}[The principal-parts resolution]
  ...
  \dcref{custom}
  \notready
  \uses{def:adelic_divisorial_sheaf, def:adelic_orderGe}
...
\begin{proposition}[Adelic and two-chart cohomology agree]
  ...
  \dcref{custom}
  \notready
  \uses{thm:adelic_principal_parts_resolution,
    thm:adelic_function_field_H1_compare}
  For every divisor \(D\),
  \[
    H^1_{\mathbb A}(D)
...
\begin{proof}
  ...
  \Cref{thm:adelic_principal_parts_resolution} identifies its degree-one
  ...
  \Cref{thm:adelic_function_field_H1_compare}.
\end{proof}

\begin{definition}[Weil differentials]
  \label{def:adelic_weil_differentials}
  \uses{thm:adelic_adele_cover_compare}
  \dcref{custom}
...
\begin{proposition}[The Weil-differential pairing]
  ...
  \notready
  \uses{def:adelic_weil_differentials,
    thm:adelic_adele_cover_compare}
  \dcref{custom}
...
\begin{proof}
  ...
  \Cref{thm:adelic_adele_cover_compare} for the displayed pairing.
\end{proof}
...
\begin{theorem}[Eventual vanishing of the adelic index]
  ...
  \dcref{custom}
  \notready
  \uses{thm:adelic_module_finite_genus,
    thm:adelic_adele_cover_compare,
    thm:adelic_riemann_inequality,
    thm:adelic_principal_degree_zero}
...
\begin{theorem}[The divisor of a Weil differential]
  ...
  \notready
  \uses{def:adelic_weil_differentials,
    thm:adelic_residuePairing,
    thm:adelic_index_eventually_zero}
  \dcref{custom}
...
\begin{proof}
  ...
  \Cref{thm:adelic_index_eventually_zero} bounds the degrees occurring in
...
\begin{theorem}[Scaling a differential]
  ...
  \notready
  \uses{thm:adelic_weil_differential_divisor}
  \dcref{custom}
...
\begin{theorem}[One-dimensionality of Weil differentials]
  ...
  \notready
  \uses{thm:adelic_residuePairing,
    thm:adelic_weil_differential_divisor,
    thm:adelic_weil_differential_divisor_mul,
    thm:adelic_riemann_inequality,
    thm:adelic_negative_degree_vanishing,
    thm:adelic_chi_eq_chi_zero_add_degree}
  \dcref{custom}
...
[Serre duality: \(i(D) = \ell(K_C - D)\)]
  ...
  \uses{thm:adelic_residuePairing,
    thm:adelic_adele_cover_compare,
    thm:adelic_weil_differentials_finrank_one,
    thm:adelic_weil_differential_divisor_mul}
  \dcref{custom}
...
\end{theorem}
\begin{proof}
  By \Cref{thm:adelic_weil_differentials_finrank_one}, every Weil
  differential is uniquely \(x\omega\).  By
  \Cref{thm:adelic_weil_differential_divisor_mul},
  ...
  \Cref{thm:adelic_residuePairing}, and the third is
  \Cref{thm:adelic_adele_cover_compare}; taking dimensions gives the final
  equality.
\end{proof}
...
\begin{theorem}[Geometric degree of a principal divisor, from the \(\chi\)-ledger]
  ...
  \uses{cor:adelic_degK_eq_degree, thm:adelic_chi_eq_chi_zero_add_degree}
...
\begin{proof}
  \leanok
  \uses{cor:adelic_degK_eq_degree}
  ...
  \Cref{cor:adelic_degK_eq_degree}, giving the displayed equality.
\end{proof}

\begin{lemma}[Negative-degree divisors have no nonzero sections]
  \label{thm:adelic_negative_degree_vanishing}
  \uses{thm:adelic_principal_degree_zero,
    def:adelic_sectionOfDivisor}
  \dcref{custom}
...
\begin{proof}
  ...
  \Cref{thm:adelic_principal_degree_zero}; an effective divisor cannot
...
\end{corollary}
\begin{proof}
  By \Cref{thm:adelic_canonical_degree}, \(\deg(K_C-D)<0\).  Therefore
  \Cref{thm:adelic_negative_degree_vanishing} and duality give
  \(i(D)=\ell(K_C-D)=0\).  Finally
  \Cref{thm:adelic_function_field_H1_compare} supplies the sheaf-cohomology
  statement.
\end{proof}

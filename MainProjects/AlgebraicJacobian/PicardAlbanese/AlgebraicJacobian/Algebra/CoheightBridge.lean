...
of a scheme point and the **ring-theoretic Krull dimension** of its stalk.
Ported from the Albanese layer of the previous Algebraic-Jacobian tree
(identical toolchain and mathlib pin), re-kernel-verified here.
...
the project-facing consumer wrapper used by the pole-purity and codim-1
extension layers (`Albanese/PolePurity.lean`, `Albanese/CodimOneExtension.lean`).
-/

...
\]
The source display uses $v_P(A)$ in place of $v_P(D)$; the divisor parameter
$D$ is the consistent reading.
\end{definition}
...
The source says "the space if linear forms"; this is the evident "space of
linear forms" typo.
\end{proof}
...
\(\Omega_{\mathbb F}\neq\varnothing\). The source's displayed sentence writes
"$A-\mathbb F(D)+\mathbb F$"; this is the evident $A_{\mathbb F}(D)+\mathbb F$
condition and is retained in the private ledger.
\end{remark}
...
$\omega_2=(x_1x_2^{-1})\omega_1$. The source accidentally writes $\omega_2=\omega_1$
at the intermediate generation claim; the scalar conclusion is the intended one.
\end{proof}
...
the source prints the same inequality for a pole, which we preserve, while the
intended pole condition is $v_P(\omega)<0$. Regularity at $P$ means

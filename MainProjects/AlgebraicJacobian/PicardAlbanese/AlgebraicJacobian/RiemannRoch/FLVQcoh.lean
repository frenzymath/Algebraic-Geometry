...
/-!
# FLV-2(a) — quasi-coherence packaging of the divisor sheaves `𝒪(D)`

This is the first half of the Wave-4 fibrewise-large-twist-vanishing carrier brick
(`informal/w4-flv-worksheet.md` §2.3, brick FLV-2): the divisor sheaves of the curve bundle
satisfy the quasi-coherence packaging `Scheme.QcohOn` of
`AlgebraicJacobian.Cohomology.QcohSections` on **every** open `U`, so the ported twisted
affine Serre vanishing (`IsAffineOpen.subsingleton_hModule'_one_of_qcoh`,
`AlgebraicJacobian.Cohomology.AffineVanishingQcoh`) fires on them over every affine open —
the second discharge family pinned by `informal/w4-datum-design.md` §3.2.
...
* **(P2a) denominator clearing** is ord bookkeeping: `germ_η g` has order `0` on `D(g)`
  ...
  pattern as the lattice exhaustion `iSup_fiberLattice` of
  `AlgebraicJacobian.RiemannRoch.FLVLattice`.
...
* `AlgebraicGeometry.IsAffineOpen.subsingleton_hModule'_divisorSheaf_one` — the discharge:
  `Subsingleton (Sheaf.HModule' (X.divisorSheaf K D) U 1)` for affine `U`, from the ported
  headline. This is the pair of vanishing instances the two-cover H¹ carrier
  (`Scheme.twoCoverH1LinearEquiv`) consumes at the chart preimages `V₀`, `V₁` in
  `AlgebraicJacobian.RiemannRoch.FLVVanishing`.
-/

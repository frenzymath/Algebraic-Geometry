...
import AlgebraicJacobian.Descent.SemilinearAlgebras

/-!
# The finite Galois quotient engine (scheme layer)

Ported from the sibling Algebraic Jacobian project
(`AlgebraicJacobian.Picard.FiniteGaloisQuotient`).  The scheme-level action
vocabulary (`toSpecAut`, `SemilinearGalAction`, `OrbitsInAffineOpen`,
`IsStableOpen`, `HasStableAffineCover`) already lives in
`AlgebraicJacobian.Descent.SemilinearAction` with identical spellings, so this
file only supplies the engine statements built on top of it.

Let `L/K` be a finite Galois extension with group `Γ = Gal(L/K)` and let `X` be
an `L`-scheme with a semilinear `Γ`-action.  This file provides:

* `specSemilinearGalAction` — the affine model: a semilinear
  `MulSemiringAction Γ A` induces a semilinear action on `Spec A` over `Spec L`
  (validates the sign/direction conventions of `SemilinearGalAction`).
* `pullbackSemilinearGalAction` — the canonical action on a base change
  `Y ×_{Spec K} Spec L` through the second factor, used to *state* equivariance
  of descent isomorphisms and the `T`-points property.
* `pullbackBaseChange` — the base change to `Spec L` of a morphism over
  `Spec K`, with its projection calculus.
* `IsGaloisQuotient ρ g` — the full quotient predicate: `Γ`-equivariant
  base-change isomorphism over `Spec L` **and** the universal `T`-points
  property `Hom_K(T, Y) ≅ Hom_L(T_L, X)^Γ` (unique descent of equivariant
  morphisms).
* `HasGaloisQuotient` — the existence gate under the orbit-in-affine
  hypothesis.  The orbit hypothesis is essential (Hironaka trap: a free `ℤ/2`
  action on Hironaka's threefold swapping two curves that meet no common affine
  open has no scheme quotient), so quotient existence is never stated for a
  bare `X`.
* `instMulSemiringActionTensor` — the left-factor Galois action `γ ⊗ id` on
  `L ⊗[K] B`, which is semilinear.
* `specOverEquivAlgHom` — affine relative Spec adjunction, triangle form.
* `affineGaloisQuotientHomEquiv` — the affine case of the Hom property: for
  `X = Spec A` and affine `T = Spec B`,
...
Sources: Milne, *Jacobian Varieties* §4 and Kleiman, *The Picard scheme* (FGA
explained) §4 use exactly this finite Galois quotient to descend
`Pic_{C_{k'}/k'}` to `k`; the affine heart is Speiser's theorem.
-/
...
/-- Existence gate for the finite Galois quotient under the orbit-in-affine
hypothesis.  The intended discharge is: a `Γ`-stable affine cover
(`HasStableAffineCover`, from `OrbitsInAffineOpen` via the EGA II 4.5.4
pattern), `Spec (A_U^Γ)` on each stable affine piece by Speiser descent
(`SemilinearAlgebras.descentAlgEquiv`; the affine Hom property is
`affineGaloisQuotientHomEquiv` below), glued via `Scheme.GlueData`.  The orbit
hypothesis is **essential** (Hironaka trap — see the module docstring). -/

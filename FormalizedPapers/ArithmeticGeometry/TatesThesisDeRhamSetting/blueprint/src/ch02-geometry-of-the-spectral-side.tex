...
\]
 ...
 Similarly, we let $\fL^+ Z$ denote the functor:
%
\[
\Spec(A) \mapsto \Hom_{\AffSch}(\Spec(A[[t]]),Z).
\]
...
\]
 ...
 where $\Spec(k) \to \fL \bB \bG_m$ corresponds to the
...
\]
 ...
 This is the moduli space of line bundles on the
...
\]
 ...
 intertwines the gauge action of $\fL^+ \bG_m$ on
...
\]
 ...
 for $\Gr_{\bG_m}^{\circ}$ the connected component
...
\]
 ...
 where the bottom map is obtained as the product
...
\]
 ...
 where we are using $d\log:\fL \bG_m \to \fL \bA^1 dt$.
...
\]
 ...
 We emphasize that this is a \emph{derived} equalizer;
...
\]
 ...
 We will define this action more rigorously below,
...
\]
 ...
 is a map of monoids $(\fL \bA^1 \times \fL \bA^1 dt,\dot) \to
...
\]
 ...
 where the previously unconsidered map is this
...
\]
 ...
 Here the $\fL^+ \bG_m$-action is constructed as in
...
\]
 ...
 for suitable meaning of $\o{\sD}_{dR}$
(cf. \cite{}).
...
\]
 ...
 similarly to $\sZ$; again, the construction
...
\]
 ...
 is closed, where the coordinates on the latter affine
...
\]
 ...
 with vertical maps induced by the projection
...
\]
 ...
 is flat by the argument for Lemma \ref{l:pre-axes},
...
\]
 ...
 is given by:\footnote{As always, the meaning of this
...
\]
 ...
 This map records the leading (i.e.,
...
\end{equation}
 ...
 where the latter map is evaluation. Explicitly, this map
...
\end{equation}
 ...
 defined as follows. By definition, this map will
factor as:
%
\[
...
\]
 ...
 where the last map remains to be defined.
...
\]
 ...
 For $r \geq 0$, we sometimes let $\iota^r$ denote the $r$-fold
composition of $\iota$.
...
\]
 ...
 Here the horizontal arrows send
$(\sL,\nabla)$ to $(\sL,\nabla,0)$; i.e., we
...
\]
 ...
 denote its (suitably normalized) tensor powers.
...
\]
 ...
 given by pairing with the section $s|_0 \in \sL|_0$.
...
\]
 ...
 and that the resulting map defines a short exact sequence
...
\]
 ...
 of bi-graded $k[x,y]/xy$ modules along the morphism
...
\end{equation}
 ...
 in $\QCoh(\sZ^{\leq n})^{\heart}$ in which the right
...
\]
 ...
 Here $b_{-n}$ is as in \S \ref{ss:coords} below.
...
\]
 ...
 (induced by $\widetilde{\zeta}$ from \S \ref{sss:maps-notation})
is an isomorphism.
...
\]
 ...
 Here each of the structural maps in the limit is affine,
...
\]
 ...
 maps through $\sU_1$ and induces an isomorphism:
...
\]
 ...
 obtained by inverting the elements $\{b_{-1}-i\}$ for each $i \geq 0$.
...
\]
 ...
 for each $i \geq 0$, which means that $a_i = 0$ in this localization
...
\]
 ...
 is \emph{not} an isomorphism. However, it is an isomorphism
...
\]
 ...
 For an integer $m \geq 0$, let $A_{1,\leq m}$ denote the
...
\]
 ...
 where the horizontal axis is the $b_{-1}$-axis, and the fiber
...
\]
 ...
 where the reddening indicates that interpret the
...
\]
 ...
 where the bluing in the picture indicates that we interpret

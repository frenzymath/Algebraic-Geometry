...
\begin{definition}[Presheaves sets]
\label{sites-definition-presheaves-sets}
\source{stacks:00V2}
\href{https://stacks.math.columbia.edu/tag/00V2}{\texttt{Stacks tag 00V2}}
\group{sites}
...
\source{stacks:00V3}
\href{https://stacks.math.columbia.edu/tag/00V3}{\texttt{Stacks tag 00V3}}
\group{sites}
...
\begin{remark}[Big presheaves]
\label{sites-remark-big-presheaves}
\source{stacks:00V4}
\href{https://stacks.math.columbia.edu/tag/00V4}{\texttt{Stacks tag 00V4}}
\group{sites}
...
\begin{definition}[Presheaves injective surjective]
\label{sites-definition-presheaves-injective-surjective}
\source{stacks:00V6}
\href{https://stacks.math.columbia.edu/tag/00V6}{\texttt{Stacks tag 00V6}}
\group{sites}
...
\begin{lemma}[Mono epi]
\label{sites-lemma-mono-epi}
\source{stacks:00V7}
\href{https://stacks.math.columbia.edu/tag/00V7}{\texttt{Stacks tag 00V7}}
\group{sites}
...
\begin{definition}[Sub presheaf]
\label{sites-definition-sub-presheaf}
\source{stacks:00V8}
\href{https://stacks.math.columbia.edu/tag/00V8}{\texttt{Stacks tag 00V8}}
\group{sites}
...
\source{stacks:00V9}
\href{https://stacks.math.columbia.edu/tag/00V9}{\texttt{Stacks tag 00V9}}
\group{sites}
...
\source{stacks:00VA}
\href{https://stacks.math.columbia.edu/tag/00VA}{\texttt{Stacks tag 00VA}}
\group{sites}

\uses{sites-lemma-image}
Notation as in Lemma \ref{sites-lemma-image}. We
...
this functor commutes with all limits and colimits.
...
\end{matrix}
\end{equation}
We sometimes drop the superscript ${}^u$ from the notation and we simply write
$\mathcal{I}_V$.
We will use these categories to define a left adjoint to the functor $u^p$.
Before we do so we prove a few technical lemmas.

\begin{lemma}[Almost directed]
\label{sites-lemma-almost-directed}
\source{stacks:00X4}
\href{https://stacks.math.columbia.edu/tag/00X4}{\texttt{Stacks tag 00X4}}
\group{sites}
...
\begin{lemma}[Directed]
\label{sites-lemma-directed}
\source{stacks:00X3}
\href{https://stacks.math.columbia.edu/tag/00X3}{\texttt{Stacks tag 00X3}}
\group{sites}
...
\begin{proof}
\uses{sites-lemma-almost-directed}

The assumptions imply that the assumptions of
Lemma \ref{sites-lemma-almost-directed}
...
the diagram
$$
\begin{array}{cc}\mathcal{F}(U) \overset{c(\phi)}{\longrightarrow}\underset{\text{id}}{\downarrow} & u_p\mathcal{F}(V) \overset{g^*}{\downarrow} \\ \mathcal{F}(U) \overset{c(\phi \circ g)}{\longrightarrow} & u_p\mathcal{F}(V')\end{array}
$$
...
\source{stacks:00VD}
\href{https://stacks.math.columbia.edu/tag/00VD}{\texttt{Stacks tag 00VD}}
\group{sites}
...
\begin{lemma}[Adjoints u]
\label{sites-lemma-adjoints-u}
\source{stacks:00VE}
\href{https://stacks.math.columbia.edu/tag/00VE}{\texttt{Stacks tag 00VE}}
\group{sites}
...
Lemma \ref{sites-lemma-recover} says that there is a
...
\begin{remark}[Functoriality presheaves values]
\label{sites-remark-functoriality-presheaves-values}
\source{stacks:00VF}
\href{https://stacks.math.columbia.edu/tag/00VF}{\texttt{Stacks tag 00VF}}
\group{sites}
...
\begin{lemma}[Pullback representable presheaf]
\label{sites-lemma-pullback-representable-presheaf}
\source{stacks:04D2}
\href{https://stacks.math.columbia.edu/tag/04D2}{\texttt{Stacks tag 04D2}}
\group{sites}
...
\source{stacks:0396}
\href{https://stacks.math.columbia.edu/tag/0396}{\texttt{Stacks tag 0396}}
\group{sites}
...
\source{stacks:00VH}
\href{https://stacks.math.columbia.edu/tag/00VH}{\texttt{Stacks tag 00VH}}
\group{sites}
...
\begin{remark}[No big sites]
\label{sites-remark-no-big-sites}
\source{stacks:00VI}
\href{https://stacks.math.columbia.edu/tag/00VI}{\texttt{Stacks tag 00VI}}
\group{sites}

\uses{sites-definition-topology-associated-site, sites-theorem-topology-and-topos, sites-remark-shrink-coverings, sites-definition-site, sites-lemma-choice-set-coverings-immaterial}
...
(see Definition \ref{sites-definition-topology-associated-site}).
...
they are equal, see Theorem \ref{sites-theorem-topology-and-topos}.
...
Remark \ref{sites-remark-shrink-coverings} below, and end up with a
...
the last sentence of Remark \ref{sites-remark-shrink-coverings}.

\medskip\noindent
Our approach will be to work with sites as in Definition \ref{sites-definition-site}
...
Lemma \ref{sites-lemma-choice-set-coverings-immaterial} below that the resulting
...
\begin{example}[Site topological]
\label{sites-example-site-topological}
\source{stacks:00VJ}
\href{https://stacks.math.columbia.edu/tag/00VJ}{\texttt{Stacks tag 00VJ}}
\group{sites}
...
\begin{example}[Site on group]
\label{sites-example-site-on-group}
\source{stacks:00VK}
\href{https://stacks.math.columbia.edu/tag/00VK}{\texttt{Stacks tag 00VK}}
\group{sites}

\uses{sites-definition-site}
...
of Definition \ref{sites-definition-site}. The result is not a
...
\begin{example}[Indiscrete]
\label{sites-example-indiscrete}
\source{stacks:07GE}
\href{https://stacks.math.columbia.edu/tag/07GE}{\texttt{Stacks tag 07GE}}
\group{sites}
...
\begin{array}{cc}\prod\nolimits_{i\in I}
\mathcal{F}(U_i)
\overset{\text{pr}_0^*}{\longrightarrow}\underset{\text{pr}_1^*}{\longrightarrow} & \prod\nolimits_{(i_0, i_1) \in I \times I}
\mathcal{F}(U_{i_0} \times_U U_{i_1})\end{array}
$$
...
\begin{definition}[Sheaf sets]
\label{sites-definition-sheaf-sets}
\source{stacks:00VM}
\href{https://stacks.math.columbia.edu/tag/00VM}{\texttt{Stacks tag 00VM}}
\group{sites}
...
\begin{array}{ccc}\mathcal{F}(U) \longrightarrow & \prod\nolimits_{i\in I}
\mathcal{F}(U_i)
\overset{\text{pr}_0^*}{\longrightarrow}\underset{\text{pr}_1^*}{\longrightarrow} & \prod\nolimits_{(i_0, i_1) \in I \times I}
\mathcal{F}(U_{i_0} \times_U U_{i_1})\end{array}
\end{equation}
...
\begin{remark}[Sheaf condition empty covering]
\label{sites-remark-sheaf-condition-empty-covering}
\source{stacks:04B3}
\href{https://stacks.math.columbia.edu/tag/04B3}{\texttt{Stacks tag 04B3}}
\group{sites}
...
\begin{example}[Sheaves topological]
\label{sites-example-sheaves-topological}
\source{stacks:00VO}
\href{https://stacks.math.columbia.edu/tag/00VO}{\texttt{Stacks tag 00VO}}
\group{sites}

\uses{sites-example-site-topological}
Let $X$ be a topological space. Let $X_{Zar}$ be the
site constructed in Example \ref{sites-example-site-topological}.
...
\begin{example}[Topological wrong]
\label{sites-example-topological-wrong}
\source{stacks:00VP}
\href{https://stacks.math.columbia.edu/tag/00VP}{\texttt{Stacks tag 00VP}}
\group{sites}

\uses{sites-example-site-topological}
Let $X$ be a topological space. Let us consider the site $X'_{Zar}$ which is
the same as the site $X_{Zar}$ of
Example \ref{sites-example-site-topological} except that
...
\begin{definition}[Category sheaves sets]
\label{sites-definition-category-sheaves-sets}
\source{stacks:00VQ}
\href{https://stacks.math.columbia.edu/tag/00VQ}{\texttt{Stacks tag 00VQ}}
\group{sites}
...
set for covering families then Definition \ref{sites-definition-sheaf-sets}
...
$$
\begin{array}{ccc}\Mor_\mathcal{A}(X, \mathcal{F}(U)) \longrightarrow & \prod
\Mor_\mathcal{A}(X, \mathcal{F}(U_i))
\overset{\text{pr}_0^*}{\longrightarrow}\underset{\text{pr}_1^*}{\longrightarrow} & \prod
\Mor_\mathcal{A}(X, \mathcal{F}(U_{i_0} \times_U U_{i_1}))\end{array}
$$
...
\source{stacks:00VR}
\href{https://stacks.math.columbia.edu/tag/00VR}{\texttt{Stacks tag 00VR}}
\group{sites}
...
\begin{definition}[Morphism coverings]
\label{sites-definition-morphism-coverings}
\source{stacks:00VT}
\href{https://stacks.math.columbia.edu/tag/00VT}{\texttt{Stacks tag 00VT}}
\group{sites}
...
$$
\begin{array}{cc}U_i \longrightarrow\downarrow & V_{\alpha(i)} \downarrow \\ U \longrightarrow & V\end{array}
$$
is commutative.
...
\begin{definition}[Combinatorial tautological]
\label{sites-definition-combinatorial-tautological}
\source{stacks:00VU}
\href{https://stacks.math.columbia.edu/tag/00VU}{\texttt{Stacks tag 00VU}}
\group{sites}
...
$$
\begin{array}{ccccccc}U_i \searrow\longrightarrow &  & V_{\alpha(i)} \swarrow &  & V_j \searrow\longrightarrow &  & U_{\beta(j)} \swarrow \\  & U &  &  &  & U & \end{array}
$$
with isomorphisms as horizontal arrows.
\end{enumerate}
\end{definition}

\begin{lemma}[Tautological combinatorial]
\label{sites-lemma-tautological-combinatorial}
\source{stacks:00VV}
\href{https://stacks.math.columbia.edu/tag/00VV}{\texttt{Stacks tag 00VV}}
\group{sites}
...
\begin{lemma}[Tautological same sheaf]
\label{sites-lemma-tautological-same-sheaf}
\source{stacks:00VW}
\href{https://stacks.math.columbia.edu/tag/00VW}{\texttt{Stacks tag 00VW}}
\group{sites}
...
\begin{lemma}[Compare separated presheaf condition]
\label{sites-lemma-compare-separated-presheaf-condition}
\source{stacks:0G1K}
\href{https://stacks.math.columbia.edu/tag/0G1K}{\texttt{Stacks tag 0G1K}}
\group{sites}
...
\begin{lemma}[Compare sheaf condition]
\label{sites-lemma-compare-sheaf-condition}
\source{stacks:0G1L}
\href{https://stacks.math.columbia.edu/tag/0G1L}{\texttt{Stacks tag 0G1L}}
\group{sites}
...
\begin{proof}
\uses{sites-lemma-compare-separated-presheaf-condition}

By Lemma \ref{sites-lemma-compare-separated-presheaf-condition} the map
...
\begin{lemma}[Refine same topology]
\label{sites-lemma-refine-same-topology}
\source{stacks:00VX}
\href{https://stacks.math.columbia.edu/tag/00VX}{\texttt{Stacks tag 00VX}}
\group{sites}
...
\begin{proof}
\uses{sites-lemma-tautological-same-sheaf, sites-lemma-compare-sheaf-condition}

Part (1) follows directly from Lemma \ref{sites-lemma-tautological-same-sheaf}
and the definitions.
...
Lemma \ref{sites-lemma-compare-sheaf-condition}.
\end{proof}

\begin{lemma}[Choice set coverings immaterial]
\label{sites-lemma-choice-set-coverings-immaterial}
\source{stacks:00VY}
\href{https://stacks.math.columbia.edu/tag/00VY}{\texttt{Stacks tag 00VY}}
\group{sites}

\uses{sites-definition-site}
Let $\mathcal{C}$ be a category.
Let $\text{Cov}(\mathcal{C})$ be a proper class of coverings
satisfying conditions (1), (2) and (3) of Definition \ref{sites-definition-site}.
...
\begin{proof}
\uses{sites-lemma-refine-same-topology, sites-lemma-tautological-combinatorial}

This is clear from Lemmas \ref{sites-lemma-refine-same-topology}
and \ref{sites-lemma-tautological-combinatorial} above as the hypothesis
...
Example \ref{sites-example-site-on-group}. We will describe the
...
$$
\begin{array}{cc}U_i \times_U U_j \downarrow\longrightarrow & U_j \overset{s_j}{\downarrow} \\ U_i \overset{s_i}{\longrightarrow} & S\end{array}
$$
...
$$
\begin{array}{ccc}\textit{PSh}(\mathcal{T}_G) \overset{\mathcal{F} \mapsto \mathcal{F}({}_GG)}{\longrightarrow} &  & G\textit{-Sets} \underset{S \mapsto \mathcal{F}_S}{\swarrow} \\  & \Sh(\mathcal{T}_G) \nwarrow & \end{array}
$$
...
\begin{proposition}[Sheaves on group]
\label{sites-proposition-sheaves-on-group}
\source{stacks:00W0}
\href{https://stacks.math.columbia.edu/tag/00W0}{\texttt{Stacks tag 00W0}}
\group{sites}
...
\begin{proof}
\uses{sites-lemma-tautological-same-sheaf}
...
\ref{sites-lemma-tautological-same-sheaf} the sheaf $\mathcal{H}$
...
$$
\begin{array}{ccc}\mathcal{H}(U) \longrightarrow & \mathcal{H}({}_GG)
\overset{\text{pr}_0^*}{\longrightarrow}\underset{\text{pr}_1^*}{\longrightarrow} & \prod_{h \in H}
\mathcal{H}({}_GG).\end{array}
$$
...
$$
\begin{array}{cccc} & U_i \longrightarrow &  & V_{\alpha(i)} \\ U_i \times_U U_j \longrightarrow\nearrow\searrow &  & V_{\alpha(i)} \times_V V_{\alpha(j)} \nearrow\searrow &  \\  & U_j \longrightarrow &  & V_{\alpha(j)}\end{array}.
$$
...
\begin{lemma}[Limit of diagram of sheaves exists and coincides with limit as presheaves]
\label{sites-lemma-limit-sheaf}
\source{stacks:00W2}
\href{https://stacks.math.columbia.edu/tag/00W2}{\texttt{Stacks tag 00W2}}
\group{sites}
...
\begin{example}[Singleton sheaf]
\label{sites-example-singleton-sheaf}
\source{stacks:00W3}
\href{https://stacks.math.columbia.edu/tag/00W3}{\texttt{Stacks tag 00W3}}
\group{sites}
...
\begin{lemma}[Plus presheaf]
\label{sites-lemma-plus-presheaf}
\source{stacks:00W4}
\href{https://stacks.math.columbia.edu/tag/00W4}{\texttt{Stacks tag 00W4}}
\group{sites}
...
Lemma \ref{sites-lemma-independent-refinement} shows that
...
\begin{lemma}[Plus functorial]
\label{sites-lemma-plus-functorial}
\source{stacks:00W5}
\href{https://stacks.math.columbia.edu/tag/00W5}{\texttt{Stacks tag 00W5}}
\group{sites}
...
$$
\begin{array}{cc}\mathcal{F} \longrightarrow\downarrow & \mathcal{F}^{+} \downarrow \\ \mathcal{G} \longrightarrow & \mathcal{G}^{+}\end{array}
$$
\end{proof}
...
\source{stacks:00W6}
\href{https://stacks.math.columbia.edu/tag/00W6}{\texttt{Stacks tag 00W6}}
\group{sites}
...
\begin{lemma}[Independent refinement]
\label{sites-lemma-independent-refinement}
\source{stacks:00W7}
\href{https://stacks.math.columbia.edu/tag/00W7}{\texttt{Stacks tag 00W7}}
\group{sites}
...
$$
\begin{array}{ccc} & V_{\alpha(i)} \searrow &  \\ U_i \overset{f_i}{\nearrow}\underset{g_i}{\searrow} &  & V \\  & V_{\beta(i)} \nearrow & \end{array}
$$
is commutative for every $i\in I$. Hence $f$ and $g$ factor through
the fibre product
$$
\begin{array}{cc} & V_{\alpha(i)} \\ U_i \overset{\varphi}{\longrightarrow}\overset{f_i}{\nearrow}\underset{g_i}{\searrow} & V_{\alpha(i)} \times_V V_{\beta(i)} \underset{\text{pr}_1}{\uparrow}\overset{\text{pr}_2}{\downarrow} \\  & V_{\beta(i)}.\end{array}
$$
...
\begin{remark}[Both refine same H0]
\label{sites-remark-both-refine-same-H0}
\source{stacks:00W8}
\href{https://stacks.math.columbia.edu/tag/00W8}{\texttt{Stacks tag 00W8}}
\group{sites}
...
\begin{lemma}[Plus surjective]
\label{sites-lemma-plus-surjective}
\source{stacks:00W9}
\href{https://stacks.math.columbia.edu/tag/00W9}{\texttt{Stacks tag 00W9}}
\group{sites}
...
\begin{proof}
\uses{sites-lemma-independent-refinement}

Namely, let $\{U_i \to U\}$ be a covering such that $s$ arises
from the element $(s_i) \in H^0(\{U_i \to U\}, \mathcal{F})$.
According to Lemma \ref{sites-lemma-independent-refinement} we may
...
\source{stacks:00WA}
\href{https://stacks.math.columbia.edu/tag/00WA}{\texttt{Stacks tag 00WA}}
\group{sites}
...
\source{stacks:00WB}
\href{https://stacks.math.columbia.edu/tag/00WB}{\texttt{Stacks tag 00WB}}
\group{sites}
...
\begin{proof}
\uses{sites-lemma-plus-surjective, sites-lemma-common-refinement}
...
for $s$ as in Lemma \ref{sites-lemma-plus-surjective},
and a similar covering $\mathcal{U}'$ for $s'$. By Lemma
\ref{sites-lemma-common-refinement}, we can find a common refinement,
...
Lemma \ref{sites-lemma-plus-surjective}) $\{U_{ij} \to U_i\}$
...
\source{stacks:00WG}
\href{https://stacks.math.columbia.edu/tag/00WG}{\texttt{Stacks tag 00WG}}
\group{sites}
...
\begin{proposition}[Sheafification adjoint]
\label{sites-proposition-sheafification-adjoint}
\source{stacks:00WH}
\href{https://stacks.math.columbia.edu/tag/00WH}{\texttt{Stacks tag 00WH}}
\group{sites}
...
\begin{proof}
\uses{sites-lemma-plus-functorial, sites-theorem-plus, sites-lemma-plus-surjective}

By Lemma \ref{sites-lemma-plus-functorial} we get a commutative diagram
$$
\begin{array}{ccc}\mathcal{F} \longrightarrow\downarrow & \mathcal{F}^{+} \longrightarrow\downarrow & \mathcal{F}^{++} \downarrow \\ \mathcal{G} \longrightarrow & \mathcal{G}^{+} \longrightarrow & \mathcal{G}^{++}\end{array}
$$
and by Theorem \ref{sites-theorem-plus} the lower horizontal maps
are isomorphisms. The uniqueness follows from Lemma
\ref{sites-lemma-plus-surjective} which says that every section of
...
to the inclusion functor $i$.

\begin{example}[Constant sheaf]
\label{sites-example-constant-sheaf}
\source{stacks:0H7J}
\href{https://stacks.math.columbia.edu/tag/0H7J}{\texttt{Stacks tag 0H7J}}
\group{sites}

\uses{sites-definition-global-sections}
Let $\mathcal{C}$ be a site. Let $E$ be a set.
The {\it constant sheaf $\underline{E}$ with value $E$}
is the sheafification of the presheaf given by the constant
functor with value $E$. It is characterized by the rule
$\Mor_{\Sh(\mathcal{C})}(\underline{E}, \mathcal{F}) =
\Mor_{\textit{Sets}}(E, \Gamma(\mathcal{C}, \mathcal{F}))$
for every sheaf $\mathcal{F}$ on $\mathcal{C}$ where the
notation is from Definition \ref{sites-definition-global-sections}.
\end{example}

We finish this section with a couple of lemmas.

\begin{lemma}[Colimit in category of sheaves equals sheafification of colimit in category of presheaves.]
\label{sites-lemma-colimit-sheaves}
\source{stacks:00WI}
\href{https://stacks.math.columbia.edu/tag/00WI}{\texttt{Stacks tag 00WI}}
\group{sites}
...
\source{stacks:00WJ}
\href{https://stacks.math.columbia.edu/tag/00WJ}{\texttt{Stacks tag 00WJ}}
\group{sites}
...
\begin{proof}
\uses{sites-lemma-common-refinement, sites-lemma-independent-refinement, sites-lemma-limit-sheaf}

Since it is a left adjoint it is right exact, see
Categories, Lemma \ref{categories-lemma-exact-adjoint}.
On the other hand, by Lemmas \ref{sites-lemma-common-refinement}
and Lemma \ref{sites-lemma-independent-refinement} the colimits
...
Lemma \ref{sites-lemma-limit-sheaf}.
\end{proof}

\begin{lemma}[Sections sheafification]
\label{sites-lemma-sections-sheafification}
\source{stacks:00WK}
\href{https://stacks.math.columbia.edu/tag/00WK}{\texttt{Stacks tag 00WK}}
\group{sites}
...
\begin{lemma}[Isomorphism sheafifications]
\label{sites-lemma-isomorphism-sheafifications}
\source{stacks:0H6Y}
\href{https://stacks.math.columbia.edu/tag/0H6Y}{\texttt{Stacks tag 0H6Y}}
\group{sites}
...
\begin{proof}
\uses{sites-lemma-sections-sheafification}

Let $U \in \Ob(\mathcal{C})$.
Let us prove that $\mathcal{F}^\#(U) \to \mathcal{G}^\#(U)$ is
surjective. To do this we will use Lemma \ref{sites-lemma-sections-sheafification}
...
\begin{definition}[Sheaves injective surjective]
\label{sites-definition-sheaves-injective-surjective}
\source{stacks:00WM}
\href{https://stacks.math.columbia.edu/tag/00WM}{\texttt{Stacks tag 00WM}}
\group{sites}
...
\begin{lemma}[Mono epi sheaves]
\label{sites-lemma-mono-epi-sheaves}
\source{stacks:00WN}
\href{https://stacks.math.columbia.edu/tag/00WN}{\texttt{Stacks tag 00WN}}
\group{sites}
...
\begin{lemma}[Coequalizer surjection]
\label{sites-lemma-coequalizer-surjection}
\source{stacks:086K}
\href{https://stacks.math.columbia.edu/tag/086K}{\texttt{Stacks tag 086K}}
\group{sites}

Let $\mathcal{C}$ be a site. Let $\mathcal{F} \to \mathcal{G}$
be a surjection of sheaves of sets. Then the diagram
$$
\begin{array}{ccc}\mathcal{F} \times_\mathcal{G} \mathcal{F}
\longrightarrow\longrightarrow & \mathcal{F} \longrightarrow & \mathcal{G}\end{array}
$$
represents $\mathcal{G}$ as a coequalizer.
\end{lemma}

\begin{proof}
\uses{sites-lemma-mono-epi-sheaves}
...
Lemma \ref{sites-lemma-mono-epi-sheaves}
...
Definition \ref{sites-definition-canonical-topology} but we will not
need this).
...
\begin{definition}[Universal effective epimorphisms]
\label{sites-definition-universal-effective-epimorphisms}
\source{stacks:00WP}
\href{https://stacks.math.columbia.edu/tag/00WP}{\texttt{Stacks tag 00WP}}
\group{sites}
...
$$
\begin{array}{ccc}\Mor_\mathcal{C}(U, X) \longrightarrow & \prod\nolimits_{i \in I} \Mor_\mathcal{C}(U_i, X)
\longrightarrow\longrightarrow & \prod\nolimits_{(i, j) \in I^2} \Mor_\mathcal{C}(U_i \times_U U_j, X)\end{array}
$$
...
satisfies the axioms of Definition \ref{sites-definition-site}.
...
\begin{definition}[Weaker than canonical]
\label{sites-definition-weaker-than-canonical}
\source{stacks:00WQ}
\href{https://stacks.math.columbia.edu/tag/00WQ}{\texttt{Stacks tag 00WQ}}
\group{sites}
...
\begin{definition}[Representable sheaf]
\label{sites-definition-representable-sheaf}
\source{stacks:00WR}
\href{https://stacks.math.columbia.edu/tag/00WR}{\texttt{Stacks tag 00WR}}
\group{sites}
...
\begin{lemma}[Coverings become surjective after sheafification.]
\label{sites-lemma-covering-surjective-after-sheafification}
\source{stacks:00WT}
\href{https://stacks.math.columbia.edu/tag/00WT}{\texttt{Stacks tag 00WT}}
\group{sites}
...
By Lemma \ref{sites-lemma-mono-epi-sheaves} above we have to show that
$\coprod\nolimits_{i \in I} h_{U_i}^\# \to h_U^\#$
...
\begin{lemma}[Sheaf coequalizer representable]
\label{sites-lemma-sheaf-coequalizer-representable}
\source{stacks:00WS}
\href{https://stacks.math.columbia.edu/tag/00WS}{\texttt{Stacks tag 00WS}}
\group{sites}
...
$$
\begin{array}{ccc}\mathcal{F}_1 \longrightarrow\longrightarrow & \mathcal{F}_0 \longrightarrow & \mathcal{F}\end{array}
$$
...
\begin{proof}
\uses{sites-lemma-mono-epi-sheaves, sites-lemma-coequalizer-surjection}
...
This is an epimorphism according to Lemma \ref{sites-lemma-mono-epi-sheaves} and
...
as above. See Lemma \ref{sites-lemma-coequalizer-surjection}.
\end{proof}

\begin{lemma}[Colimit representable]
\label{sites-lemma-colimit-representable}
\source{stacks:0GLW}
\href{https://stacks.math.columbia.edu/tag/0GLW}{\texttt{Stacks tag 0GLW}}
\group{sites}
...
\begin{proof}
\uses{sites-proposition-sheafification-adjoint}
...
(Proposition \ref{sites-proposition-sheafification-adjoint}) we have
...
\source{stacks:00WV}
\href{https://stacks.math.columbia.edu/tag/00WV}{\texttt{Stacks tag 00WV}}
\group{sites}
...
\begin{lemma}[Pushforward sheaf]
\label{sites-lemma-pushforward-sheaf}
\source{stacks:00WW}
\href{https://stacks.math.columbia.edu/tag/00WW}{\texttt{Stacks tag 00WW}}
\group{sites}
...
\begin{lemma}[Adjoint sheaves]
\label{sites-lemma-adjoint-sheaves}
\source{stacks:00WX}
\href{https://stacks.math.columbia.edu/tag/00WX}{\texttt{Stacks tag 00WX}}
\group{sites}

\uses{sites-lemma-pushforward-sheaf}
In the situation of Lemma \ref{sites-lemma-pushforward-sheaf}.
The functor $u_s : \mathcal{G} \mapsto (u_p \mathcal{G})^\#$
is a left adjoint to $u^s$.
\end{lemma}

\begin{proof}
\uses{sites-lemma-adjoints-u, sites-proposition-sheafification-adjoint}

Follows directly from Lemma \ref{sites-lemma-adjoints-u} and
Proposition \ref{sites-proposition-sheafification-adjoint}.
\end{proof}

Here is a technical lemma.

\begin{lemma}[Technical up]
\label{sites-lemma-technical-up}
\source{stacks:00WY}
\href{https://stacks.math.columbia.edu/tag/00WY}{\texttt{Stacks tag 00WY}}
\group{sites}
...
\begin{lemma}[Pullback representable sheaf]
\label{sites-lemma-pullback-representable-sheaf}
\source{stacks:04D3}
\href{https://stacks.math.columbia.edu/tag/04D3}{\texttt{Stacks tag 04D3}}
\group{sites}
...
\begin{proof}
\uses{sites-lemma-pullback-representable-presheaf, sites-lemma-technical-up}

Follows from
Lemmas \ref{sites-lemma-pullback-representable-presheaf}
and \ref{sites-lemma-technical-up}.
\end{proof}


\begin{remark}[Quasi continuous]
\label{sites-remark-quasi-continuous}
\source{stacks:00WZ}
\href{https://stacks.math.columbia.edu/tag/00WZ}{\texttt{Stacks tag 00WZ}}
\group{sites}

\uses{sites-definition-combinatorial-tautological, sites-lemma-pushforward-sheaf, sites-lemma-adjoint-sheaves, sites-remark-both-refine-same-H0}
...
see Definition \ref{sites-definition-combinatorial-tautological}
...
$u(T \times_V V_i) \to u(T) \times_{u(V)} u(V_i)$ is an isomorphism.
\end{enumerate}
We are going to see that Lemmas \ref{sites-lemma-pushforward-sheaf}
and \ref{sites-lemma-adjoint-sheaves} hold in case
...
\ref{sites-remark-both-refine-same-H0} we see that
...
\ref{sites-lemma-pushforward-sheaf} holds. Lemma \ref{sites-lemma-adjoint-sheaves}
follows immediately.
\end{remark}

\section{Morphisms of sites}
\label{sites-section-morphism-sites}
\begin{definition}[Morphism sites]
\label{sites-definition-morphism-sites}
\source{stacks:00X1}
\href{https://stacks.math.columbia.edu/tag/00X1}{\texttt{Stacks tag 00X1}}
\group{sites}
...
\begin{example}[Continuous map]
\label{sites-example-continuous-map}
\source{stacks:00X2}
\href{https://stacks.math.columbia.edu/tag/00X2}{\texttt{Stacks tag 00X2}}
\group{sites}

\uses{sites-example-site-topological, sites-remark-quasi-continuous}
Let $f : X  \to Y$ be a continuous map of topological spaces.
Recall that we have sites $X_{Zar}$ and $Y_{Zar}$,
see Example \ref{sites-example-site-topological}. Consider the functor
...
\begin{example}[Finer topology]
\label{sites-example-finer-topology}
\source{stacks:0EWI}
\href{https://stacks.math.columbia.edu/tag/0EWI}{\texttt{Stacks tag 0EWI}}
\group{sites}

\uses{sites-lemma-limit-sheaf, sites-lemma-sheafification-exact, sites-definition-morphism-sites}
Let $\mathcal{C}$ be a category. Let
$$
...
of presheaves (Lemma \ref{sites-lemma-limit-sheaf}) and $\tau$-sheafification
commutes with finite limits (Lemma \ref{sites-lemma-sheafification-exact}).
...
conditions of Definition \ref{sites-definition-morphism-sites}.
\end{example}

\begin{lemma}[Composition of site functors respects continuity of site functors.]
\label{sites-lemma-composition-morphisms-sites}
\source{stacks:03CB}
\href{https://stacks.math.columbia.edu/tag/03CB}{\texttt{Stacks tag 03CB}}
\group{sites}
...
\begin{definition}[Composition morphisms sites]
\label{sites-definition-composition-morphisms-sites}
\source{stacks:03CC}
\href{https://stacks.math.columbia.edu/tag/03CC}{\texttt{Stacks tag 03CC}}
\group{sites}
...
Lemma \ref{sites-lemma-composition-morphisms-sites}).

\begin{lemma}[Directed morphism]
\label{sites-lemma-directed-morphism}
\source{stacks:00X5}
\href{https://stacks.math.columbia.edu/tag/00X5}{\texttt{Stacks tag 00X5}}
\group{sites}
...
\begin{proof}
\uses{sites-lemma-sheafification-exact}
...
see Section \ref{sites-section-limits-colimits-PSh}).
And since sheafification commutes with finite limits as well
(Lemma \ref{sites-lemma-sheafification-exact}) we conclude because
$u_s = \# \circ u_p$.
\end{proof}

\begin{proposition}[Get morphism]
\label{sites-proposition-get-morphism}
\source{stacks:00X6}
\href{https://stacks.math.columbia.edu/tag/00X6}{\texttt{Stacks tag 00X6}}
\group{sites}
...
\begin{proof}
\uses{sites-lemma-directed, sites-lemma-directed-morphism}

This follows from Lemmas \ref{sites-lemma-directed} and
\ref{sites-lemma-directed-morphism}.
\end{proof}

\begin{remark}[Explain left exact]
\label{sites-remark-explain-left-exact}
\source{stacks:00X7}
\href{https://stacks.math.columbia.edu/tag/00X7}{\texttt{Stacks tag 00X7}}
\group{sites}

\uses{sites-proposition-get-morphism}
The conditions of Proposition \ref{sites-proposition-get-morphism} above
...
Lemma \ref{sites-lemma-continuous-with-continuous-left-adjoint} will
provide another way to prove a continuous functor
gives rise to a morphism of sites.

\begin{remark}[Quasi continuous morphism sites]
\label{sites-remark-quasi-continuous-morphism-sites}
\source{stacks:00X8}
\href{https://stacks.math.columbia.edu/tag/00X8}{\texttt{Stacks tag 00X8}}
\group{sites}

\uses{sites-definition-morphism-sites, sites-remark-quasi-continuous, sites-proposition-get-morphism}
(Skip on first reading.)
Let $\mathcal{C}$ and $\mathcal{D}$ be sites. Analogously to
Definition \ref{sites-definition-morphism-sites} we say that
a {\it quasi-morphism of sites $f : \mathcal{D} \to \mathcal{C}$}
is given by a quasi-continuous functor $u : \mathcal{C} \to \mathcal{D}$
(see Remark \ref{sites-remark-quasi-continuous}) such that $u_s$ is exact.
The analogue of Proposition \ref{sites-proposition-get-morphism} in this
...
In Definition \ref{sites-definition-morphism-sites} the condition that $u_s$
...
\begin{lemma}[Morphism of sites covering]
\label{sites-lemma-morphism-of-sites-covering}
\source{stacks:08H2}
\href{https://stacks.math.columbia.edu/tag/08H2}{\texttt{Stacks tag 08H2}}
\group{sites}
...
\source{stacks:00XA}
\href{https://stacks.math.columbia.edu/tag/00XA}{\texttt{Stacks tag 00XA}}
\group{sites}
...
Section \ref{sites-section-2-category}).
...
\medskip\noindent
The {\it empty topos} is the topos
...
using either Lemma \ref{sites-lemma-cocontinuous-morphism-topoi}
or the following lemma.

\begin{lemma}[Morphism sites topoi]
\label{sites-lemma-morphism-sites-topoi}
\source{stacks:00XC}
\href{https://stacks.math.columbia.edu/tag/00XC}{\texttt{Stacks tag 00XC}}
\group{sites}
...
\begin{proof}
\uses{sites-definition-morphism-sites}

This is obvious from Definition \ref{sites-definition-morphism-sites}.
\end{proof}

\begin{remark}[Pt topos]
\label{sites-remark-pt-topos}
\source{stacks:00XD}
\href{https://stacks.math.columbia.edu/tag/00XD}{\texttt{Stacks tag 00XD}}
\group{sites}
...
\begin{remark}[Morphism topoi big]
\label{sites-remark-morphism-topoi-big}
\source{stacks:00XB}
\href{https://stacks.math.columbia.edu/tag/00XB}{\texttt{Stacks tag 00XB}}
\group{sites}
...
we do this in the case of points of topoi in Section \ref{sites-section-points}.
\end{remark}

\begin{remark}[Quasi continuous morphism topoi]
\label{sites-remark-quasi-continuous-morphism-topoi}
\source{stacks:00XE}
\href{https://stacks.math.columbia.edu/tag/00XE}{\texttt{Stacks tag 00XE}}
\group{sites}

\uses{sites-remark-quasi-continuous-morphism-sites, sites-lemma-morphism-sites-topoi}
(Skip on first reading.)
Let $\mathcal{C}$ and $\mathcal{D}$ be sites.
A quasi-morphism of sites $f : \mathcal{D} \to \mathcal{C}$
(see Remark \ref{sites-remark-quasi-continuous-morphism-sites})
gives rise to a morphism of topoi $f$ from
$\Sh(\mathcal{D})$ to $\Sh(\mathcal{C})$
exactly as in Lemma \ref{sites-lemma-morphism-sites-topoi}.
\end{remark}
...
Example \ref{sites-example-site-on-group} and
...
set but $G$ action defined by $g \cdot \xi = \varphi(g)\xi$ for
$g \in G$ and $\xi \in U$.
...
and
Lemma \ref{sites-lemma-morphism-sites-topoi}
...
Lemma \ref{sites-lemma-pullback-representable-sheaf}). Hence we see that
...
\source{stacks:090H}
\href{https://stacks.math.columbia.edu/tag/090H}{\texttt{Stacks tag 090H}}
\group{sites}

\uses{sites-definition-morphism-coverings}
...
(see Definition \ref{sites-definition-morphism-coverings})
...
\begin{lemma}[Conclude quasi compact]
\label{sites-lemma-conclude-quasi-compact}
\source{stacks:0D05}
\href{https://stacks.math.columbia.edu/tag/0D05}{\texttt{Stacks tag 0D05}}
\group{sites}
...
\begin{proof}
\uses{sites-example-site-topological, sites-definition-site, sites-definition-quasi-compact}
...
Example \ref{sites-example-site-topological}). However, then we change the notion
...
will be a site as in Definition \ref{sites-definition-site}.
In this site the object $X = U = [0, 1]$ is quasi-compact in the sense of
Definition \ref{sites-definition-quasi-compact} but $U$ does not satisfy (2).
...
\begin{lemma}[Quasi compact]
\label{sites-lemma-quasi-compact}
\source{stacks:0D06}
\href{https://stacks.math.columbia.edu/tag/0D06}{\texttt{Stacks tag 0D06}}
\group{sites}
...
\begin{proof}
\uses{sites-lemma-colimit-sheaves, sites-lemma-covering-surjective-after-sheafification}
...
(Lemma \ref{sites-lemma-colimit-sheaves}), for each $a$
...
By Lemma \ref{sites-lemma-covering-surjective-after-sheafification}
...
\begin{definition}[Quasi compact topos]
\label{sites-definition-quasi-compact-topos}
\source{stacks:0D07}
\href{https://stacks.math.columbia.edu/tag/0D07}{\texttt{Stacks tag 0D07}}
\group{sites}
...
By Lemma \ref{sites-lemma-quasi-compact}
if the site $\mathcal{C}$ has a final object $X$, then
$\Sh(\mathcal{C})$ is quasi-compact if and only if $X$ is quasi-compact.

\begin{lemma}[Sheaf surjections transmit quasi-compactness.]
\label{sites-lemma-image-of-quasi-compact}
\source{stacks:0GMP}
\href{https://stacks.math.columbia.edu/tag/0GMP}{\texttt{Stacks tag 0GMP}}
\group{sites}
...
\begin{lemma}[Finite copoducts of sheaves conserve quasi-compactness.]
\label{sites-lemma-coproduct-quasi-compact}
\source{stacks:0GMQ}
\href{https://stacks.math.columbia.edu/tag/0GMQ}{\texttt{Stacks tag 0GMQ}}
\group{sites}
...
\begin{lemma}[Directed colimits sections]
\label{sites-lemma-directed-colimits-sections}
\source{stacks:0738}
\href{https://stacks.math.columbia.edu/tag/0738}{\texttt{Stacks tag 0738}}
\group{sites}
...
\begin{proof}
\uses{sites-definition-separated, sites-lemma-colimit-sheaves, sites-theorem-plus, sites-lemma-conclude-quasi-compact}

Assume all the transition maps are injective. In this case the presheaf
$\mathcal{F}' : V \mapsto \colim_i \mathcal{F}_i(V)$ is
separated (see Definition \ref{sites-definition-separated}).
By Lemma \ref{sites-lemma-colimit-sheaves}
we have
$(\mathcal{F}')^\# = \colim_i \mathcal{F}_i$.
By Theorem \ref{sites-theorem-plus}
...
\medskip\noindent
Assume the hypothesis of (4). By Lemma \ref{sites-lemma-conclude-quasi-compact}
...
\begin{lemma}[Directed colimits global sections]
\label{sites-lemma-directed-colimits-global-sections}
\source{stacks:0GMR}
\href{https://stacks.math.columbia.edu/tag/0GMR}{\texttt{Stacks tag 0GMR}}
\group{sites}
...
\begin{proof}
\uses{sites-definition-separated, sites-lemma-colimit-sheaves, sites-theorem-plus, sites-lemma-image-of-quasi-compact}
...
are surjective.
By (4)(b) and Lemma \ref{sites-lemma-image-of-quasi-compact} we conclude
...
$$
\begin{array}{cccc}\mathcal{K} \times \mathcal{K} \longrightarrow\longrightarrow & \mathcal{K} \downarrow\longrightarrow & {*} \overset{s}{\downarrow} &  \\  & \mathcal{F}_i \longrightarrow & \mathcal{F} \xlongequal{} & \colim \mathcal{F}_i\end{array}
$$
...
\begin{remark}[Stronger conditions]
\label{sites-remark-stronger-conditions}
\source{stacks:0GMS}
\href{https://stacks.math.columbia.edu/tag/0GMS}{\texttt{Stacks tag 0GMS}}
\group{sites}

\uses{sites-lemma-directed-colimits-global-sections}
Let $\mathcal{C}$ be a site. There are several ways to ensure that
the hypotheses of part (4) of
Lemma \ref{sites-lemma-directed-colimits-global-sections}
...
\begin{lemma}[Colimit over ordinal sections]
\label{sites-lemma-colimit-over-ordinal-sections}
\source{stacks:0GS0}
\href{https://stacks.math.columbia.edu/tag/0GS0}{\texttt{Stacks tag 0GS0}}
\group{sites}
...
We need an analogue of Lemma \ref{sites-lemma-directed-colimits-sections}
...
\begin{definition}[Situation: Inverse limit sites]
\label{sites-situation-inverse-limit-sites}
\source{stacks:0A34}
\href{https://stacks.math.columbia.edu/tag/0A34}{\texttt{Stacks tag 0A34}}
\group{sites}
...
\begin{lemma}[Colimit sites]
\label{sites-lemma-colimit-sites}
\source{stacks:09YL}
\href{https://stacks.math.columbia.edu/tag/09YL}{\texttt{Stacks tag 09YL}}
\group{sites}

\uses{sites-situation-inverse-limit-sites}
In Situation \ref{sites-situation-inverse-limit-sites} we can construct
...
In this way we see that axiom (3) of Definition \ref{sites-definition-site} holds.

\medskip\noindent
To verify axiom (2) of Definition \ref{sites-definition-site}
...
\medskip\noindent
We omit the proof of axiom (1) of Definition \ref{sites-definition-site}.
\end{proof}

\begin{lemma}[Compute pullback to limit]
\label{sites-lemma-compute-pullback-to-limit}
\source{stacks:0A35}
\href{https://stacks.math.columbia.edu/tag/0A35}{\texttt{Stacks tag 0A35}}
\group{sites}

\uses{sites-situation-inverse-limit-sites, sites-lemma-colimit-sites}
In Situation \ref{sites-situation-inverse-limit-sites} let
$u_i : \mathcal{C}_i \to \mathcal{C}$ be as constructed in
Lemma \ref{sites-lemma-colimit-sites}. Then $u_i$ defines a morphism
...
\begin{proof}
\uses{sites-lemma-colimit-sites, sites-lemma-limit-sheaf}

It is immediate from the arguments in the proof of
Lemma \ref{sites-lemma-colimit-sites} that the functors $u_i$ are continuous.
...
(Lemma \ref{sites-lemma-limit-sheaf}), the functors $f_a^{-1}$ commutes with finite
...
\source{stacks:09YN}
\href{https://stacks.math.columbia.edu/tag/09YN}{\texttt{Stacks tag 09YN}}
\group{sites}

\uses{sites-situation-inverse-limit-sites, sites-lemma-colimit-sites}
In Situation \ref{sites-situation-inverse-limit-sites} assume given
\begin{enumerate}
...
on the site $\mathcal{C}$ of Lemma \ref{sites-lemma-colimit-sites}.
...
\begin{proof}
\uses{sites-lemma-directed-colimits-sections}
...
Lemma \ref{sites-lemma-directed-colimits-sections}.
\end{proof}

\begin{lemma}[Colimit push pull]
\label{sites-lemma-colimit-push-pull}
\source{stacks:0EXJ}
\href{https://stacks.math.columbia.edu/tag/0EXJ}{\texttt{Stacks tag 0EXJ}}
\group{sites}

\uses{sites-situation-inverse-limit-sites, sites-lemma-colimit}
In Situation \ref{sites-situation-inverse-limit-sites} assume
...
is a canonical map
satisfying a cocycle condition as in Lemma \ref{sites-lemma-colimit}.
\end{lemma}

\begin{proof}
\uses{sites-lemma-colimit}
...
$$
by Lemma \ref{sites-lemma-colimit}. Since $u_j(u_a(U_i)) = U$
...
\begin{lemma}[Recover pu]
\label{sites-lemma-recover-pu}
\source{stacks:00XG}
\href{https://stacks.math.columbia.edu/tag/00XG}{\texttt{Stacks tag 00XG}}
\group{sites}
...
\begin{lemma}[Adjoints pu]
\label{sites-lemma-adjoints-pu}
\source{stacks:00XH}
\href{https://stacks.math.columbia.edu/tag/00XH}{\texttt{Stacks tag 00XH}}
\group{sites}
...
\begin{proof}
\uses{sites-lemma-adjoints-u, sites-lemma-recover-pu, sites-remark-functoriality-presheaves-values}

This is proved in exactly the same way as the proof
of Lemma \ref{sites-lemma-adjoints-u}. We note that the map
$u^p{}_pu \mathcal{F} \to \mathcal{F}$ from
Lemma \ref{sites-lemma-recover-pu} is the map that
...
By Remark \ref{sites-remark-functoriality-presheaves-values}
...
\begin{lemma}[Adjoint functors]
\label{sites-lemma-adjoint-functors}
\source{stacks:09VQ}
\href{https://stacks.math.columbia.edu/tag/09VQ}{\texttt{Stacks tag 09VQ}}
\group{sites}
...
\begin{definition}[Cocontinuous]
\label{sites-definition-cocontinuous}
\source{stacks:00XJ}
\href{https://stacks.math.columbia.edu/tag/00XJ}{\texttt{Stacks tag 00XJ}}
\group{sites}
...
\begin{lemma}[Pu sheaf]
\label{sites-lemma-pu-sheaf}
\source{stacks:00XK}
\href{https://stacks.math.columbia.edu/tag/00XK}{\texttt{Stacks tag 00XK}}
\group{sites}
...
\begin{proof}
\uses{sites-lemma-colimit-sheaves, sites-lemma-mono-epi-sheaves}

Let $\{V_j \to V\}_{j \in J}$ be a covering of the site $\mathcal{D}$.
We have to show that
$$
\begin{array}{ccccc} & \ \phantom{}_pu\mathcal{F}(V) \longrightarrow & \prod {}_pu\mathcal{F}(V_j) \longrightarrow\longrightarrow & \prod {}_pu\mathcal{F}(V_j \times_V V_{j'}) & \end{array}
$$
...
\begin{array}{ccc}\coprod u^p h_{V_j \times_V V_{j'}} \longrightarrow\longrightarrow & \coprod u^p h_{V_j} \longrightarrow & u^p h_V\end{array}
\end{equation}
...
category of presheaves, see Lemma \ref{sites-lemma-colimit-sheaves}.)

\medskip\noindent
We first show that the second arrow of (\ref{equation-coequalizer})
becomes surjective after sheafification.
To do this we use Lemma \ref{sites-lemma-mono-epi-sheaves}. Thus it suffices to
...
$$
\begin{array}{cc}u(U) \underset{s'}{\longrightarrow}\underset{s}{\downarrow} & V_{j'} \downarrow \\ V_j \longrightarrow & V\end{array}
$$
...
\begin{lemma}[Exact cocontinuous]
\label{sites-lemma-exact-cocontinuous}
\source{stacks:00XL}
\href{https://stacks.math.columbia.edu/tag/00XL}{\texttt{Stacks tag 00XL}}
\group{sites}

\uses{sites-lemma-pu-sheaf}
Let $\mathcal{C}$ and $\mathcal{D}$ be sites.
Let $u : \mathcal{C} \to \mathcal{D}$ be cocontinuous.
The functor
$\Sh(\mathcal{D}) \to \Sh(\mathcal{C})$,
...
in Lemma \ref{sites-lemma-pu-sheaf} above. Moreover, it
is exact.
\end{lemma}

\begin{proof}
\uses{sites-lemma-sheafification-exact, sites-lemma-limit-sheaf}
...
see Lemma \ref{sites-lemma-sheafification-exact}.
Moreover, the inclusion
$i : \Sh(\mathcal{D}) \to \textit{PSh}(\mathcal{D})$
is left exact by Lemma \ref{sites-lemma-limit-sheaf}. Finally, the functor
...
\begin{lemma}[Technical pu]
\label{sites-lemma-technical-pu}
\source{stacks:00XM}
\href{https://stacks.math.columbia.edu/tag/00XM}{\texttt{Stacks tag 00XM}}
\group{sites}

\uses{sites-lemma-exact-cocontinuous}
In the situation of Lemma \ref{sites-lemma-exact-cocontinuous}.
...
\begin{remark}[Cartesian cocontinuous]
\label{sites-remark-cartesian-cocontinuous}
\source{stacks:09W7}
\href{https://stacks.math.columbia.edu/tag/09W7}{\texttt{Stacks tag 09W7}}
\group{sites}
...
Definition \ref{sites-definition-morphism-sites},
Proposition \ref{sites-proposition-get-morphism}, and
Lemma \ref{sites-lemma-morphism-sites-topoi}.

\begin{lemma}[Cocontinuous morphism topoi]
\label{sites-lemma-cocontinuous-morphism-topoi}
\source{stacks:00XO}
\href{https://stacks.math.columbia.edu/tag/00XO}{\texttt{Stacks tag 00XO}}
\group{sites}
...
This is exactly the content of Lemma \ref{sites-lemma-exact-cocontinuous}.
\end{proof}

\begin{lemma}[Composition of site functors respects cocontinuity of site functors.]
\label{sites-lemma-composition-cocontinuous}
\source{stacks:03L5}
\href{https://stacks.math.columbia.edu/tag/03L5}{\texttt{Stacks tag 03L5}}
\group{sites}
...
${}_pv \circ {}_pu = {}_p(v \circ u)$, see Lemma \ref{sites-lemma-pu-sheaf}.
...
\begin{example}[Open immersion cocontinuous]
\label{sites-example-open-immersion-cocontinuous}
\source{stacks:00XP}
\href{https://stacks.math.columbia.edu/tag/00XP}{\texttt{Stacks tag 00XP}}
\group{sites}

\uses{sites-example-site-topological, sites-example-continuous-map, sites-lemma-cocontinuous-morphism-topoi}
Let $X$ be a topological space.
Let $j : U  \to X$ be the inclusion of an open subspace.
Recall that we have sites $X_{Zar}$ and $U_{Zar}$,
see Example \ref{sites-example-site-topological}.
...
see Example \ref{sites-example-continuous-map}.
...
We conclude by Lemma \ref{sites-lemma-cocontinuous-morphism-topoi} above
...
\begin{example}[Open map cocontinuous]
\label{sites-example-open-map-cocontinuous}
\source{stacks:00XQ}
\href{https://stacks.math.columbia.edu/tag/00XQ}{\texttt{Stacks tag 00XQ}}
\group{sites}

\uses{sites-example-open-immersion-cocontinuous, sites-example-site-topological, sites-example-continuous-map, sites-lemma-cocontinuous-morphism-topoi}
This example is a slight generalization of
Example \ref{sites-example-open-immersion-cocontinuous}.
Let $f : X \to Y$ be a continuous map of topological spaces.
...
In the first Example \ref{sites-example-open-immersion-cocontinuous}
the functor $v$ is also continuous. But in the second
Example \ref{sites-example-open-map-cocontinuous} it is generally not continuous
because condition (2) of Definition \ref{sites-definition-continuous} may fail.
Hence the following lemma applies to the first example, but not to the second.

\begin{lemma}[Sheafification is redundant in topoi morphisms associated to simultaneously continuous ...]
\label{sites-lemma-when-shriek}
\source{stacks:00XR}
\href{https://stacks.math.columbia.edu/tag/00XR}{\texttt{Stacks tag 00XR}}
\group{sites}
...
\begin{proof}
\uses{sites-lemma-pushforward-sheaf, sites-lemma-adjoint-sheaves}

By Lemma \ref{sites-lemma-pushforward-sheaf} for any sheaf $\mathcal{G}$
...
adjoint to $u^s$ by Lemma \ref{sites-lemma-adjoint-sheaves}.
The statement on limits and colimits follows from the
discussion in Categories, Section \ref{categories-section-adjoint}.
\end{proof}

In the situation of Lemma \ref{sites-lemma-when-shriek} above we see that we have
...
Example \ref{sites-example-open-immersion-cocontinuous} the functor
...
\begin{lemma}[Preserve equalizers]
\label{sites-lemma-preserve-equalizers}
\source{stacks:00XS}
\href{https://stacks.math.columbia.edu/tag/00XS}{\texttt{Stacks tag 00XS}}
\group{sites}
...
\begin{proof}
\uses{sites-lemma-limit-sheaf, sites-lemma-sheafification-exact, sites-lemma-almost-directed}

Assume (a), (b), and (c).
We have $g_! = (u_p\ )^\#$. Recall (Lemma \ref{sites-lemma-limit-sheaf}) that
limits of sheaves are equal to the corresponding limits as presheaves.
And sheafification commutes with finite limits
(Lemma \ref{sites-lemma-sheafification-exact}). Thus it
...
\source{stacks:00XT}
\href{https://stacks.math.columbia.edu/tag/00XT}{\texttt{Stacks tag 00XT}}
\group{sites}
...
\begin{proof}
\uses{sites-lemma-pu-sheaf, sites-lemma-when-shriek}

Let $X$ be an object of $\mathcal{C}$.
In Lemmas \ref{sites-lemma-pu-sheaf} and \ref{sites-lemma-when-shriek} we have seen that
...
(Section \ref{sites-section-subtopoi}) and moreover, the given embedding has a
retraction.

\begin{lemma}[Bigger site]
\label{sites-lemma-bigger-site}
\source{stacks:00XU}
\href{https://stacks.math.columbia.edu/tag/00XU}{\texttt{Stacks tag 00XU}}
\group{sites}
...
The composition
$$
\begin{array}{ccc}\Sh(\mathcal{C}) \overset{g}{\longrightarrow} & \Sh(\mathcal{D}) \overset{f}{\longrightarrow} & \Sh(\mathcal{C})\end{array}
$$
...
\begin{proof}
\uses{sites-proposition-get-morphism, sites-lemma-morphism-sites-topoi, sites-lemma-when-shriek, sites-lemma-back-and-forth}

By assumption the functor $u$ satisfies the hypotheses of
Proposition \ref{sites-proposition-get-morphism}. Hence $u$ defines
a morphism of sites and hence a morphism of topoi $f$ as in
Lemma \ref{sites-lemma-morphism-sites-topoi}. The formulas
$f_* = g^{-1}$ and $f^{-1} = g_!$ are clear from the lemma cited and
Lemma \ref{sites-lemma-when-shriek}.
We have
$f_* \circ g_* = g^{-1} \circ g_* \cong \text{id}$, and
$g^{-1} \circ f^{-1} = g^{-1} \circ g_! \cong \text{id}$
by Lemma \ref{sites-lemma-back-and-forth}.
...
\begin{example}[Closed map cocontinuous false]
\label{sites-example-closed-map-cocontinuous-false}
\source{stacks:00XV}
\href{https://stacks.math.columbia.edu/tag/00XV}{\texttt{Stacks tag 00XV}}
\group{sites}
...
be functors of the underlying categories such that $v$ is right adjoint to $u$.
In this case, if $v$ is continuous, then $u$ is cocontinuous
(Lemma \ref{sites-lemma-continuous-with-continuous-left-adjoint}).
If $u$ is cocontinuous, then it is often
(but not always, see Example \ref{sites-example-not-equivalent})
the case that $v$ is continuous, and if so, then
$v$ defines a morphism of sites whose
associated morphism of topoi is the same as that defined by $u$.

\begin{lemma}[Have functor other way]
\label{sites-lemma-have-functor-other-way}
\source{stacks:00XX}
\href{https://stacks.math.columbia.edu/tag/00XX}{\texttt{Stacks tag 00XX}}
\group{sites}

\uses{sites-lemma-cocontinuous-morphism-topoi}
Let $\mathcal{C}$ and $\mathcal{D}$ be sites. Let
$u : \mathcal{C} \to \mathcal{D}$, and $v : \mathcal{D} \to \mathcal{C}$
...
Lemma \ref{sites-lemma-cocontinuous-morphism-topoi}.
Then
\begin{enumerate}
...
\begin{proof}
\uses{sites-lemma-cocontinuous-morphism-topoi, sites-lemma-pu-sheaf, sites-lemma-adjoint-functors}
...
\mathcal{F} \circ v
$$
The first equality is Lemma \ref{sites-lemma-cocontinuous-morphism-topoi}.
The second equality is Lemma \ref{sites-lemma-pu-sheaf}.
The third equality is Lemma \ref{sites-lemma-adjoint-functors}.
The final equality is the definition of $v^p$ in
Section \ref{sites-section-functoriality-PSh}. This proves (1).
For $\mathcal{G}$ as in (2) we have
$$
g^{-1}\mathcal{G} = (u^p\mathcal{G})^\# = (v_p\mathcal{G})^\#
$$
The first equality is Lemma \ref{sites-lemma-cocontinuous-morphism-topoi}.
The second equality is Lemma \ref{sites-lemma-adjoint-functors}.
\end{proof}

\begin{lemma}[Have functor other way morphism]
\label{sites-lemma-have-functor-other-way-morphism}
\source{stacks:00XY}
\href{https://stacks.math.columbia.edu/tag/00XY}{\texttt{Stacks tag 00XY}}
\group{sites}

\uses{sites-lemma-have-functor-other-way}
Notation and assumptions as in Lemma \ref{sites-lemma-have-functor-other-way}.
...
\begin{proof}
\uses{sites-lemma-have-functor-other-way, sites-definition-morphism-sites}

We will use the results of Lemma \ref{sites-lemma-have-functor-other-way}
...
Definition \ref{sites-definition-morphism-sites}). Since
...
\begin{example}[Finer topology bis]
\label{sites-example-finer-topology-bis}
\source{stacks:0EWJ}
\href{https://stacks.math.columbia.edu/tag/0EWJ}{\texttt{Stacks tag 0EWJ}}
\group{sites}

\uses{sites-example-finer-topology, sites-lemma-have-functor-other-way, sites-lemma-have-functor-other-way-morphism}
This example continues the discussion of
Example \ref{sites-example-finer-topology}
...
Lemmas \ref{sites-lemma-have-functor-other-way} and
...
\begin{lemma}[Continuous with continuous left adjoint]
\label{sites-lemma-continuous-with-continuous-left-adjoint}
\source{stacks:09VR}
\href{https://stacks.math.columbia.edu/tag/09VR}{\texttt{Stacks tag 09VR}}
\group{sites}

\uses{sites-lemma-have-functor-other-way, sites-lemma-have-functor-other-way-morphism}
Let $\mathcal{C}$ and $\mathcal{D}$ be sites and let
...
\item the results of Lemmas \ref{sites-lemma-have-functor-other-way} and
\ref{sites-lemma-have-functor-other-way-morphism} hold.
\end{enumerate}
...
Let $U$ be an object of $\mathcal{C}$ and let $\{V_j \to u(U)\}_{j \in J}$
be a covering in $\mathcal{D}$. Then $\{v(V_j) \to v(u(U))\}_{i \in I}$ is
a covering in $\mathcal{C}$. Via the adjunction map $U \to v(u(U))$ we
can base change this to a covering $\{W_j \to U\}_{j \in J}$ with
$W_j = v(V_j) \times_{v(u(U))} U$. Denoting $p_j : W_j \to v(V_j)$ the
first projection, we obtain maps
$$
u(W_j) \xrightarrow{u(p_j)} u(v(V_j)) \longrightarrow V_j
$$
where the second arrow is the adjunction map. This determines a morphism
$\{u(W_j) \to u(U)\} \to \{V_j \to u(U)\}$ of families of maps with fixed
target, showing that $u$ is indeed cocontinuous. The other statements are
immediate from Lemmas \ref{sites-lemma-have-functor-other-way} and
\ref{sites-lemma-have-functor-other-way-morphism}.
\end{proof}

\begin{example}[Not equivalent]
\label{sites-example-not-equivalent}
\source{stacks:0H6Z}
\href{https://stacks.math.columbia.edu/tag/0H6Z}{\texttt{Stacks tag 0H6Z}}
\group{sites}

\uses{sites-lemma-continuous-with-continuous-left-adjoint}
Let $\mathcal{C}$ and $\mathcal{D}$ be sites. Let
$u : \mathcal{C} \to \mathcal{D}$ and $v : \mathcal{D} \to \mathcal{C}$
be functors of the underlying categories such that $v$ is right adjoint to $u$.
Lemma \ref{sites-lemma-continuous-with-continuous-left-adjoint}
...
\source{stacks:08NH}
\href{https://stacks.math.columbia.edu/tag/08NH}{\texttt{Stacks tag 08NH}}
\group{sites}

\uses{sites-lemma-cocontinuous-morphism-topoi, sites-lemma-when-shriek}
Let $\mathcal{C}$ and $\mathcal{D}$ be sites. Let
...
Lemmas \ref{sites-lemma-cocontinuous-morphism-topoi} and
...
\begin{proof}
\uses{sites-lemma-adjoint-functors, sites-lemma-limit-sheaf, sites-lemma-adjoints-u, sites-lemma-sheafification-exact, sites-lemma-pushforward-sheaf, sites-lemma-adjoint-sheaves, sites-lemma-have-functor-other-way}

Recall that $g_!\mathcal{F}$ is the sheafification of $u_p\mathcal{F}$.
Hence (1)(a) follows from the fact that $u_p = w^p$ by
Lemma \ref{sites-lemma-adjoint-functors}.
...
it from Lemma \ref{sites-lemma-have-functor-other-way}.
\end{proof}
...
\begin{lemma}[Existence lower shriek]
\label{sites-lemma-existence-lower-shriek}
\source{stacks:09YX}
\href{https://stacks.math.columbia.edu/tag/09YX}{\texttt{Stacks tag 09YX}}
\group{sites}
...
\begin{proof}
\uses{sites-lemma-sheaf-coequalizer-representable}
...
coequalizer diagram
$$
\begin{array}{ccc}\mathcal{H}_1 \longrightarrow\longrightarrow & \mathcal{H}_0 \longrightarrow & \mathcal{H}\end{array}
$$
where $\mathcal{H}_i = \coprod h_{V_{i, j}}^\#$
is a coproduct with $V_{i, j} \in E$.
This is possible by assumption (2), see
Lemma \ref{sites-lemma-sheaf-coequalizer-representable}
(for those worried about set theoretical issues, note that
the construction given in
Lemma \ref{sites-lemma-sheaf-coequalizer-representable} is canonical).
Define $f_!(\mathcal{H})$ to be the sheaf on $\mathcal{C}$
which makes
$$
\begin{array}{ccc}f_!\mathcal{H}_1 \longrightarrow\longrightarrow & f_!\mathcal{H}_0 \longrightarrow & f_!\mathcal{H}\end{array}
$$
...
& =
\text{Equalizer}(
\begin{array}{cc}\Mor(f_!\mathcal{H}_0, \mathcal{F}) \longrightarrow\longrightarrow & \Mor(f_!\mathcal{H}_1, \mathcal{F})\end{array}
) \\
& =
\text{Equalizer}(
\begin{array}{cc}\Mor(\mathcal{H}_0, f^{-1}\mathcal{F}) \longrightarrow\longrightarrow & \Mor(\mathcal{H}_1, f^{-1}\mathcal{F})\end{array}
) \\
& =
...
$$
This is clearly cocontinuous and continuous. Hence by
Lemma \ref{sites-lemma-when-shriek} we obtain a morphism of topoi
$$
j_U : \Sh(\mathcal{C}/U) \longrightarrow \Sh(\mathcal{C})
$$
given by $j_U^{-1}$ and $j_{U*}$ and a functor $j_{U!}$
left adjoint to $j_U^{-1}$.

\begin{definition}[Localize]
\label{sites-definition-localize}
\source{stacks:00Y0}
\href{https://stacks.math.columbia.edu/tag/00Y0}{\texttt{Stacks tag 00Y0}}
\group{sites}
...
\begin{lemma}[Describe j shriek]
\label{sites-lemma-describe-j-shriek}
\source{stacks:03CD}
\href{https://stacks.math.columbia.edu/tag/03CD}{\texttt{Stacks tag 03CD}}
\group{sites}
...
\begin{proof}
\uses{sites-lemma-when-shriek, sites-lemma-technical-up}

By Lemma \ref{sites-lemma-when-shriek} we have
$j_{U!}(\mathcal{G}^\#) = ((j_U)_p\mathcal{G}^\#)^\#$.
By Lemma \ref{sites-lemma-technical-up} this is equal to $((j_U)_p\mathcal{G})^\#$.
Hence it suffices to prove that $(j_U)_p$ is given by
the formula above for any presheaf $\mathcal{G}$ on $\mathcal{C}/U$.
OK, and by the definition in Section \ref{sites-section-functoriality-PSh} we have
$$
(j_U)_p\mathcal{G}(V)
=
...
\begin{lemma}[Describe j shriek representable]
\label{sites-lemma-describe-j-shriek-representable}
\source{stacks:03HU}
\href{https://stacks.math.columbia.edu/tag/03HU}{\texttt{Stacks tag 03HU}}
\group{sites}
...
\begin{proof}
\uses{sites-lemma-describe-j-shriek}

Denote $p : X \to U$ the structure morphism of $X$.
By Lemma \ref{sites-lemma-describe-j-shriek} we see $j_{U!}(h_{X/U}^\#)$
...
\begin{lemma}[Essential image j shriek]
\label{sites-lemma-essential-image-j-shriek}
\source{stacks:00Y1}
\href{https://stacks.math.columbia.edu/tag/00Y1}{\texttt{Stacks tag 00Y1}}
\group{sites}
...
\begin{proof}
\uses{sites-lemma-plus-presheaf, sites-lemma-plus-functorial, sites-theorem-plus, sites-lemma-describe-j-shriek}
...
$$
\begin{array}{cc}\textit{PSh}(\mathcal{C})/h_U \longrightarrow\downarrow & \textit{PSh}(\mathcal{C}/U) \downarrow \\ \Sh(\mathcal{C})/h_U^\# \longrightarrow & \Sh(\mathcal{C}/U)\end{array}
$$
...
$$
\vcenter{
\begin{array}{cc}\mathcal{F}_\varphi \longrightarrow\downarrow & {*} \downarrow \\ \mathcal{F}|_{\mathcal{C}/U} \longrightarrow & h_U|_{\mathcal{C}/U}\end{array}
}
...
Lemmas \ref{sites-lemma-plus-presheaf} and \ref{sites-lemma-plus-functorial}
and Theorem \ref{sites-theorem-plus}.
...
in Lemma \ref{sites-lemma-describe-j-shriek} and $\gamma$ is the composition of
...
$$
\begin{array}{cc}\textit{PSh}(\mathcal{C}/U) \longrightarrow\downarrow & \textit{PSh}(\mathcal{C})/h_U \downarrow \\ \Sh(\mathcal{C}/U) \longrightarrow & \Sh(\mathcal{C})/h_U^\#\end{array}
$$
...
Lemma \ref{sites-lemma-essential-image-j-shriek}
...
\begin{lemma}[J shriek commutes equalizers fibre products]
\label{sites-lemma-j-shriek-commutes-equalizers-fibre-products}
\source{stacks:04BB}
\href{https://stacks.math.columbia.edu/tag/04BB}{\texttt{Stacks tag 04BB}}
\group{sites}
...
\begin{proof}
\uses{sites-lemma-essential-image-j-shriek, sites-lemma-describe-j-shriek, sites-lemma-preserve-equalizers}

Via Lemma \ref{sites-lemma-essential-image-j-shriek}
...
Lemma \ref{sites-lemma-describe-j-shriek}
using that sheafification is exact. (Also, in case $\mathcal{C}$ has
fibre products and equalizers, the result follows from
Lemma \ref{sites-lemma-preserve-equalizers}.)
\end{proof}

\begin{lemma}[J shriek reflects injections surjections]
\label{sites-lemma-j-shriek-reflects-injections-surjections}
\source{stacks:0E8E}
\href{https://stacks.math.columbia.edu/tag/0E8E}{\texttt{Stacks tag 0E8E}}
\group{sites}
...
\begin{proof}
\uses{sites-lemma-mono-epi-sheaves, sites-lemma-essential-image-j-shriek}
...
\begin{lemma}[Compute j shriek restrict]
\label{sites-lemma-compute-j-shriek-restrict}
\source{stacks:03EE}
\href{https://stacks.math.columbia.edu/tag/03EE}{\texttt{Stacks tag 03EE}}
\group{sites}
...
\source{stacks:03EH}
\href{https://stacks.math.columbia.edu/tag/03EH}{\texttt{Stacks tag 03EH}}
\group{sites}
...
$$
\begin{array}{ccc}\mathcal{C}/V \underset{j_V}{\searrow}\underset{j}{\longrightarrow} &  & \mathcal{C}/U \overset{j_U}{\swarrow} \\  & \mathcal{C} & \end{array}
$$
...
\begin{proof}
\uses{sites-lemma-composition-cocontinuous}

The commutativity of the diagram is immediate.
The agreement of $j$ with $j_{V/U}$ follows from the definitions. By
Lemma \ref{sites-lemma-composition-cocontinuous}
we see that the following diagram of morphisms of topoi
\begin{equation}
\label{equation-relocalize}
\vcenter{
\begin{array}{ccc}\Sh(\mathcal{C}/V) \underset{j_V}{\searrow}\underset{j}{\longrightarrow} &  & \Sh(\mathcal{C}/U) \overset{j_U}{\swarrow} \\  & \Sh(\mathcal{C}) & \end{array}
}
\end{equation}
...
\begin{lemma}[Relocalize explicit]
\label{sites-lemma-relocalize-explicit}
\source{stacks:04IL}
\href{https://stacks.math.columbia.edu/tag/04IL}{\texttt{Stacks tag 04IL}}
\group{sites}

\uses{sites-lemma-relocalize, sites-lemma-essential-image-j-shriek}
Notation $\mathcal{C}$, $f : V \to U$, $j_U$, $j_V$, and $j$ as in
Lemma \ref{sites-lemma-relocalize}. Via the identifications
...
by Lemma \ref{sites-lemma-relocalize}.
...
$$
\begin{array}{ccc}\mathcal{H}' = \mathcal{H} \times_{\varphi, h_U^\#, f} h_V^\#
\overset{\varphi'}{\longrightarrow} &  & h_V^\#\end{array}
$$
of $\Sh(\mathcal{C})/h_V^\#$
...
\source{stacks:0494}
\href{https://stacks.math.columbia.edu/tag/0494}{\texttt{Stacks tag 0494}}
\group{sites}

\uses{sites-example-indiscrete, sites-lemma-describe-j-shriek}
...
chaotic topology on $\mathcal{C}$ (see Example \ref{sites-example-indiscrete}).
...
\begin{remark}[Localization cartesian cocontinuous]
\label{sites-remark-localization-cartesian-cocontinuous}
\source{stacks:09W8}
\href{https://stacks.math.columbia.edu/tag/09W8}{\texttt{Stacks tag 09W8}}
\group{sites}

\uses{sites-lemma-relocalize, sites-remark-cartesian-cocontinuous}
Let $\mathcal{C}$ be a site. Let $U \to V$ be a morphism of $\mathcal{C}$.
The cocontinuous functors $\mathcal{C}/U \to \mathcal{C}$ and
$j : \mathcal{C}/U \to \mathcal{C}/V$ (Lemma \ref{sites-lemma-relocalize})
satisfy property $P$ of Remark \ref{sites-remark-cartesian-cocontinuous}.
...
\begin{lemma}[Maps of sheaves glue.]
\label{sites-lemma-glue-maps}
\source{stacks:04TQ}
\href{https://stacks.math.columbia.edu/tag/04TQ}{\texttt{Stacks tag 04TQ}}
\group{sites}
...
\begin{proof}
\uses{sites-lemma-relocalize}

The restrictions used in the lemma are those of
Lemma \ref{sites-lemma-relocalize}.
...
$$
\begin{array}{cccc}\mathcal{F}(V) \xlongequal{} & H^0(\mathcal{V}, \mathcal{F}) \downarrow\longrightarrow & \prod \mathcal{F}(V_i)
\underset{\prod \varphi_i}{\downarrow}\longrightarrow\longrightarrow & \prod \mathcal{F}(V_i \times_V V_j) \underset{\prod \varphi_{ij}}{\downarrow} \\ \mathcal{G}(V) \xlongequal{} & H^0(\mathcal{V}, \mathcal{G}) \longrightarrow & \prod \mathcal{G}(V_i)
\longrightarrow\longrightarrow & \prod \mathcal{G}(V_i \times_V V_j)\end{array}
$$
The equality signs come from the sheaf condition; see
Section \ref{sites-section-sheafification} for the notation
$H^0(\mathcal{V}, -)$.
...
\begin{lemma}[The category of sheaves on a site is cartesian closed]
\label{sites-lemma-internal-hom-sheaf}
\source{stacks:0BWQ}
\href{https://stacks.math.columbia.edu/tag/0BWQ}{\texttt{Stacks tag 0BWQ}}
\group{sites}
...
\begin{lemma}[Hom sheaf hU]
\label{sites-lemma-hom-sheaf-hU}
\source{stacks:0D7X}
\href{https://stacks.math.columbia.edu/tag/0D7X}{\texttt{Stacks tag 0D7X}}
\group{sites}
...
\begin{proof}
\uses{sites-lemma-internal-hom-sheaf, sites-lemma-compute-j-shriek-restrict}
...
Lemmas \ref{sites-lemma-internal-hom-sheaf} and
...
$$
\begin{array}{ccc}\mathcal{F}_i|_{\mathcal{C}/U_i \times_U U_j \times_U U_k}
\underset{\varphi_{ik}}{\longrightarrow}\underset{\varphi_{ij}}{\searrow} &  & \mathcal{F}_k|_{\mathcal{C}/U_i \times_U U_j \times_U U_k} \\  & \mathcal{F}_j|_{\mathcal{C}/U_i \times_U U_j \times_U U_k}
\underset{\varphi_{jk}}{\nearrow} & \end{array}
$$
...
\begin{lemma}[Glue sheaves]
\label{sites-lemma-glue-sheaves}
\source{stacks:04TR}
\href{https://stacks.math.columbia.edu/tag/04TR}{\texttt{Stacks tag 04TR}}
\group{sites}
...
$$
\begin{array}{cc}\mathcal{F}|_{\mathcal{C}/U_i \times_U U_j}
\underset{\text{id}}{\downarrow}\underset{\varphi_i}{\longrightarrow} & \mathcal{F}_i|_{\mathcal{C}/U_i \times_U U_j} \overset{\varphi_{ij}}{\downarrow} \\ \mathcal{F}|_{\mathcal{C}/U_i \times_U U_j} \overset{\varphi_j}{\longrightarrow} & \mathcal{F}_j|_{\mathcal{C}/U_i \times_U U_j}\end{array}
$$
are commutative.
\end{lemma}
...
\begin{lemma}[Mapping property glue]
\label{sites-lemma-mapping-property-glue}
\source{stacks:04TS}
\href{https://stacks.math.columbia.edu/tag/04TS}{\texttt{Stacks tag 04TS}}
\group{sites}
...
\begin{proof}
\uses{sites-lemma-glue-maps, sites-lemma-glue-sheaves}

In
Lemma \ref{sites-lemma-glue-maps}
we saw that the functor is fully faithful, and in
Lemma \ref{sites-lemma-glue-sheaves}
we proved that it is essentially surjective (by explicitly constructing
a quasi-inverse functor).
\end{proof}

Let $\mathcal{C}$ be a site. We are going to discuss
a version of glueing sheaves on the entire site $\mathcal{C}$.
For each object $U$ in $\mathcal{C}$, let $\mathcal{F}_U$
be a sheaf on $\mathcal{C}/U$. Recall that there is a functor
$j_f : \mathcal{C}/V \rightarrow \mathcal{C}/U$ associated to each
morphism $f : V \rightarrow U$ in $\mathcal{C}$, given by
$(a : W \to V) \mapsto (f \circ a : W \to U)$. For each such $f$, let
$$
c_f : j_f^{-1} \mathcal{F}_U \to \mathcal{F}_V
$$
be an isomorphism of sheaves. Assume that given any two arrows
$f : V \to U$ and $g : W \to V$ in $\mathcal{C}$, the composition
$c_g \circ j_{g}^{-1} c_f$ is equal to $c_{f \circ g}$. We will call
such a collection of data $(\mathcal{F}_{U}, c_f)$ an
{\it absolute glueing data for sheaves of sets on $\mathcal{C}$}.
A morphism of absolute glueing data
$(\mathcal{F}_{U}, c_f) \to (\mathcal{G}_{U}, c'_f)$
is given by a collection $(\varphi_U)$
of morphisms of sheaves $\varphi_U : \mathcal{F}_U \to \mathcal{G}_U$,
such that
$$
\begin{array}{cc}j_f^{-1} \mathcal{F}_U \underset{c_f}{\longrightarrow}\underset{j_f^{-1} \varphi_U}{\downarrow} & \mathcal{F}_V \overset{\varphi_V}{\downarrow} \\ j_f^{-1} \mathcal{G}_U \overset{c'_f}{\longrightarrow} & \mathcal{G}_V\end{array}
$$
commutes for every morphism $f : V \to U$ in $\mathcal{C}$.

\medskip\noindent
Associated to any sheaf $\mathcal{F}$ on $\mathcal{C}$ is its
{\it canonical absolute glueing data} $(\mathcal{F}|_{\mathcal{C}/U},c_f)$,
where the canonical isomorphisms
$c_f : j_f^{-1} \mathcal{F}|_{\mathcal{C}/U} \to
\mathcal{F}|_{\mathcal{C}/V}$
for $f : V \to U$ come from the relation
$j_V = j_U \circ j_f$ as in Lemma \ref{sites-lemma-relocalize}.
Any morphism $\varphi : \mathcal{F} \to \mathcal{G}$
of sheaves of $\mathcal{C}$ induces a morphism
$(\varphi|_{\mathcal{C}/U})$ of canonical absolute glueing data.

\begin{lemma}[Glue sheaves absolute]
\label{sites-lemma-glue-sheaves-absolute}
\source{stacks:0GWK}
\href{https://stacks.math.columbia.edu/tag/0GWK}{\texttt{Stacks tag 0GWK}}
\group{sites}
...
$$
\begin{array}{ccc}\mathcal{F}_U(U) \longrightarrow\downarrow & \mathcal{F}_U(V) \overset{c_f}{\longrightarrow} & \mathcal{F}_V(V) \downarrow \\ \mathcal{F}(U) \longrightarrow &  & \mathcal{F}(V)\end{array}
$$
...
\begin{remark}[Variant glue sheaves absolute]
\label{sites-remark-variant-glue-sheaves-absolute}
\source{stacks:0GWL}
\href{https://stacks.math.columbia.edu/tag/0GWL}{\texttt{Stacks tag 0GWL}}
\group{sites}

\uses{sites-lemma-glue-sheaves-absolute}
There is a variant of Lemma \ref{sites-lemma-glue-sheaves-absolute}
which comes up in algebraic geometry. Namely, suppose that $\mathcal{C}$
is a site with all fibre products and for each $U \in \Ob(\mathcal{C})$
we are given a full subcategory $U_\tau \subset \mathcal{C}/U$ with
the following properties
\begin{enumerate}
\item $U/U$ is in $U_\tau$,
\item for $V/U$ in $U_\tau$ and covering $\{V_j \to V\}$
of $\mathcal{C}$ we have $V_j/U$ in $U_\tau$ and
\item for a morphism $U' \to U$ of $\mathcal{C}$ and $V/U$ in $U_\tau$
the base change $V' = V \times_U U'$ is in $U'_\tau$.
\end{enumerate}
In this setting $U_\tau$ is a site for all $U$ in $\mathcal{C}$
and the base change functor $U_\tau \to U'_\tau$ defines a morphism
$f_\tau : U_\tau \to U'_\tau$ of sites for all morphisms $f : U' \to U$
of $\mathcal{C}$. The glueing statement we obtain then reads as follows:
A sheaf $\mathcal{F}$ on $\mathcal{C}$ is given by the following data:
\begin{enumerate}
\item for every $U \in \Ob(\mathcal{C})$ a sheaf
$\mathcal{F}_U$ on $U_\tau$,
\item for every $f : U' \to U$ in $\mathcal{C}$ a map
$c_f : f_\tau^{-1}\mathcal{F}_U \to \mathcal{F}_{U'}$.
\end{enumerate}
These data are subject to the following conditions:
\begin{enumerate}
\item[(a)] given $f : U' \to U$ and $g : U'' \to U'$ in
$\mathcal{C}$ the composition
$c_g \circ g_\tau^{-1}c_f$ is equal to $c_{f \circ g}$, and
\item[(b)] if $f : U' \to U$ is in $U_\tau$ then $c_f$ is an isomorphism.
\end{enumerate}
If we ever need this we will precisely state and prove this here.
(Note that this result is slightly different from the statements
above as we are not requiring all the maps $c_f$ to be isomorphisms!)
\end{remark}
...
\begin{lemma}[Describe j shriek good site]
\label{sites-lemma-describe-j-shriek-good-site}
\source{stacks:03HT}
\href{https://stacks.math.columbia.edu/tag/03HT}{\texttt{Stacks tag 03HT}}
\group{sites}

\uses{sites-definition-weaker-than-canonical, sites-lemma-describe-j-shriek}
Let $\mathcal{C}$ be a site.
Let $U \in \Ob(\mathcal{C})$.
If the topology on $\mathcal{C}$ is subcanonical, see
Definition \ref{sites-definition-weaker-than-canonical},
...
of Lemma \ref{sites-lemma-describe-j-shriek} directly.
...
\begin{lemma}[Localize given products]
\label{sites-lemma-localize-given-products}
\source{stacks:03CE}
\href{https://stacks.math.columbia.edu/tag/03CE}{\texttt{Stacks tag 03CE}}
\group{sites}
...
\begin{proof}
\uses{sites-definition-site, sites-lemma-have-functor-other-way, sites-lemma-have-functor-other-way-morphism}
...
statements of the lemma follow from Lemmas \ref{sites-lemma-have-functor-other-way}
and \ref{sites-lemma-have-functor-other-way-morphism}.
\end{proof}

\begin{lemma}[Relocalize given fibre products]
\label{sites-lemma-relocalize-given-fibre-products}
\source{stacks:09W9}
\href{https://stacks.math.columbia.edu/tag/09W9}{\texttt{Stacks tag 09W9}}
\group{sites}

\uses{sites-lemma-relocalize}
Let $\mathcal{C}$ be a site. Let $U \to V$ be a morphism of $\mathcal{C}$.
Assume $\mathcal{C}$ has fibre products. Let $j$ be as in
Lemma \ref{sites-lemma-relocalize}. Then
\begin{enumerate}
...
\begin{proof}
\uses{sites-lemma-localize-given-products, sites-lemma-relocalize}

Follows from Lemma \ref{sites-lemma-localize-given-products} since $j$ may be viewed
as a localization functor by Lemma \ref{sites-lemma-relocalize}.
\end{proof}
...
\source{stacks:00Y2}
\href{https://stacks.math.columbia.edu/tag/00Y2}{\texttt{Stacks tag 00Y2}}
\group{sites}
...
\begin{proof}
\uses{sites-lemma-back-and-forth}

This is a special case of Lemma \ref{sites-lemma-back-and-forth}
because the assumption on $U$ is equivalent to the fully faithfulness
of the localization functor $\mathcal{C}/U \to \mathcal{C}$.
\end{proof}

\begin{lemma}[Localize cartesian square]
\label{sites-lemma-localize-cartesian-square}
\source{stacks:0EYV}
\href{https://stacks.math.columbia.edu/tag/0EYV}{\texttt{Stacks tag 0EYV}}
\group{sites}

\uses{sites-lemma-relocalize}
Let $\mathcal{C}$ be a site. Let
$$
\begin{array}{cc}U' \downarrow\longrightarrow & U \downarrow \\ V' \longrightarrow & V\end{array}
$$
be a commutative diagram of $\mathcal{C}$. The
morphisms of Lemma \ref{sites-lemma-relocalize}
produce commutative diagrams
$$
\vcenter{
\begin{array}{cc}\mathcal{C}/U' \underset{j_{U'/V'}}{\downarrow}\underset{j_{U'/U}}{\longrightarrow} & \mathcal{C}/U \overset{j_{U/V}}{\downarrow} \\ \mathcal{C}/V' \overset{j_{V'/V}}{\longrightarrow} & \mathcal{C}/V\end{array}
}
\quad\text{and}\quad
\vcenter{
\begin{array}{cc}\Sh(\mathcal{C}/U') \underset{j_{U'/V'}}{\downarrow}\underset{j_{U'/U}}{\longrightarrow} & \Sh(\mathcal{C}/U) \overset{j_{U/V}}{\downarrow} \\ \Sh(\mathcal{C}/V') \overset{j_{V'/V}}{\longrightarrow} & \Sh(\mathcal{C}/V)\end{array}
}
$$
...
\begin{proof}
\uses{sites-lemma-composition-cocontinuous, sites-lemma-essential-image-j-shriek, sites-lemma-relocalize-explicit}
...
of the diagram of topoi by Lemma \ref{sites-lemma-composition-cocontinuous}.
...
Via the identifications of Lemma \ref{sites-lemma-essential-image-j-shriek}
we may think of our diagram of topoi as
$$
\begin{array}{cc}\Sh(\mathcal{C})/h_{U'}^\# \downarrow\longrightarrow & \Sh(\mathcal{C})/h_U^\# \downarrow \\ \Sh(\mathcal{C})/h_{V'}^\# \longrightarrow & \Sh(\mathcal{C})/h_V^\#\end{array}
$$
and we know how to interpret the functors $j^{-1}$ and $j_!$
by Lemma \ref{sites-lemma-relocalize-explicit}. Thus we have to show
...
\begin{lemma}[Localize morphism]
\label{sites-lemma-localize-morphism}
\source{stacks:03CF}
\href{https://stacks.math.columbia.edu/tag/03CF}{\texttt{Stacks tag 03CF}}
\group{sites}

\uses{sites-lemma-essential-image-j-shriek}
...
$$
\begin{array}{cc}\Sh(\mathcal{C}/U) \underset{j_U}{\longrightarrow}\underset{f'}{\downarrow} & \Sh(\mathcal{C}) \overset{f}{\downarrow} \\ \Sh(\mathcal{D}/V) \overset{j_V}{\longrightarrow} & \Sh(\mathcal{D}).\end{array}
$$
...
\begin{proof}
\uses{sites-lemma-technical-up, sites-lemma-pullback-representable-sheaf, sites-lemma-essential-image-j-shriek, sites-lemma-compute-j-shriek-restrict}

It is clear that $u'$ is continuous, and hence we get functors
$f'_* = (u')^s = (u')^p$ (see
Sections \ref{sites-section-functoriality-PSh}
and \ref{sites-section-continuous-functors})
...
The second equality by Lemma \ref{sites-lemma-technical-up}.
The third equality because $u \circ j_V = j_U \circ u'$.
The fourth equality by Lemma \ref{sites-lemma-technical-up} again.
...
$$
\begin{array}{ccc}\Sh(\mathcal{C}/U) \underset{j_{U!}}{\longrightarrow} & \Sh(\mathcal{C})/h_U^\# \longrightarrow & \Sh(\mathcal{C}) \\ \Sh(\mathcal{D}/V) \overset{j_{V!}}{\longrightarrow}\overset{(f')^{-1}}{\uparrow} & \Sh(\mathcal{D})/h_V^\# \longrightarrow\overset{f^{-1}}{\uparrow} & \Sh(\mathcal{D}) \overset{f^{-1}}{\uparrow}\end{array}
$$
The middle arrow makes sense as $f^{-1}h_V^\# = (h_{u(V)})^\# = h_U^\#$, see
Lemma \ref{sites-lemma-pullback-representable-sheaf}.
...
in Lemma \ref{sites-lemma-compute-j-shriek-restrict}. Namely,
...
Lemma \ref{sites-lemma-localize-morphism}
above.

\begin{lemma}[Localize morphism strong]
\label{sites-lemma-localize-morphism-strong}
\source{stacks:03EF}
\href{https://stacks.math.columbia.edu/tag/03EF}{\texttt{Stacks tag 03EF}}
\group{sites}

\uses{sites-lemma-localize-given-products, sites-lemma-localize-morphism}
...
$$
\begin{array}{cc}\mathcal{C}/U \underset{j_U}{\longrightarrow}\underset{f'}{\downarrow} & \mathcal{C} \overset{f}{\downarrow} \\ \mathcal{D}/V \overset{j_V}{\longrightarrow} & \mathcal{D}\end{array}
$$
...
as in Lemma \ref{sites-lemma-localize-given-products}.
...
\begin{proof}
\uses{sites-proposition-get-morphism, sites-definition-composition-morphisms-sites}

Note that $u$ satisfies the assumptions of
Proposition \ref{sites-proposition-get-morphism} and hence induces
...
sites (see Definition \ref{sites-definition-composition-morphisms-sites})
and because
$$
...
\begin{lemma}[Relocalize morphism]
\label{sites-lemma-relocalize-morphism}
\source{stacks:04IN}
\href{https://stacks.math.columbia.edu/tag/04IN}{\texttt{Stacks tag 04IN}}
\group{sites}

\uses{sites-lemma-localize-morphism, sites-lemma-relocalize, sites-lemma-essential-image-j-shriek}
Let $f : \mathcal{C} \to \mathcal{D}$ be a morphism of sites
corresponding to the continuous functor $u : \mathcal{D} \to \mathcal{C}$.
...
$$
\begin{array}{cc}\Sh(\mathcal{C}/U) \underset{j_U}{\longrightarrow}\underset{f_c}{\downarrow} & \Sh(\mathcal{C}) \overset{f}{\downarrow} \\ \Sh(\mathcal{D}/V) \overset{j_V}{\longrightarrow} & \Sh(\mathcal{D}).\end{array}
$$
...
$$
\begin{array}{cc}\Sh(\mathcal{C}/U') \underset{j_{U'/U}}{\longrightarrow}\underset{f_{c'}}{\downarrow} & \Sh(\mathcal{C}/U) \overset{f_c}{\downarrow} \\ \Sh(\mathcal{D}/V') \overset{j_{V'/V}}{\longrightarrow} & \Sh(\mathcal{D}/V).\end{array}
$$
commutes.
\end{lemma}

\begin{proof}
\uses{sites-lemma-localize-morphism, sites-lemma-relocalize-explicit, sites-lemma-pullback-representable-sheaf}
...
Lemma \ref{sites-lemma-relocalize-explicit}
with
Lemma \ref{sites-lemma-localize-morphism}.
...
\begin{lemma}[Localize cocontinuous]
\label{sites-lemma-localize-cocontinuous}
\source{stacks:03EG}
\href{https://stacks.math.columbia.edu/tag/03EG}{\texttt{Stacks tag 03EG}}
\group{sites}
...
$$
\begin{array}{cc}\mathcal{C}/U \underset{j_U}{\longrightarrow}\underset{u'}{\downarrow} & \mathcal{C} \overset{u}{\downarrow} \\ \mathcal{D}/V \overset{j_V}{\longrightarrow} & \mathcal{D}\end{array}
$$
...
\begin{lemma}[Localize cocontinuous downstairs]
\label{sites-lemma-localize-cocontinuous-downstairs}
\source{stacks:0D5R}
\href{https://stacks.math.columbia.edu/tag/0D5R}{\texttt{Stacks tag 0D5R}}
\group{sites}
...
$$
\vcenter{
\begin{array}{cc}\,_V^u\mathcal{I} \underset{j}{\longrightarrow}\underset{u'}{\downarrow} & \mathcal{C} \overset{u}{\downarrow} \\ \mathcal{D}/V \overset{j_V}{\longrightarrow} & \mathcal{D}\end{array}
}
\quad\text{where}\quad
\begin{matrix}
j : (U, \psi) \mapsto U \\
...
$$
\begin{array}{cc}\Sh({}^u_V\mathcal{I}) \underset{j}{\longrightarrow}\underset{f'}{\downarrow} & \Sh(\mathcal{C}) \overset{f}{\downarrow} \\ \Sh(\mathcal{D}/V) \overset{j_V}{\longrightarrow} & \Sh(\mathcal{D})\end{array}
$$
...
(by Lemma \ref{sites-lemma-when-shriek} as $j$ is continuous and
cocontinuous)
...
\source{stacks:0FN1}
\href{https://stacks.math.columbia.edu/tag/0FN1}{\texttt{Stacks tag 0FN1}}
\group{sites}
...
and functors
$$
\begin{array}{cc}\mathcal{C}' \underset{v'}{\longrightarrow}\underset{u'}{\downarrow} & \mathcal{C} \overset{u}{\downarrow} \\ \mathcal{D}' \overset{v}{\longrightarrow} & \mathcal{D}\end{array}
$$
Assume
\begin{enumerate}
\item $u$, $u'$, $v$, and $v'$ are cocontinuous giving rise to morphisms of
topoi $f$, $f'$, $g$, and $g'$ by Lemma \ref{sites-lemma-cocontinuous-morphism-topoi},
...
Recall that $g^{-1}$ is given by $v^p$ (Lemma \ref{sites-lemma-when-shriek}) and
...
\source{stacks:0FN2}
\href{https://stacks.math.columbia.edu/tag/0FN2}{\texttt{Stacks tag 0FN2}}
\group{sites}
...
$$
With notation as in Sections \ref{sites-section-morphism-sites}
and \ref{sites-section-cocontinuous-morphism-topoi} assume
\begin{enumerate}
...
second by Lemma \ref{sites-lemma-when-shriek}. We have
$$
...
Please compare with \cite[Expos\'e III, Proposition 4.9.4]{SGA4}.

\begin{lemma}[Equivalence]
\label{sites-lemma-equivalence}
\source{stacks:03A0}
\href{https://stacks.math.columbia.edu/tag/03A0}{\texttt{Stacks tag 03A0}}
\group{sites}
...
\medskip\noindent
Note that Lemma \ref{sites-lemma-when-shriek} applies and we have
...
the diagram
$$
\begin{array}{cc}U_i' \times_{U'} U_j' \underset{\text{pr}_2}{\longrightarrow}\underset{\text{pr}_1}{\downarrow} & U_j' \overset{c_j}{\downarrow} \\ U_i' \overset{c_i}{\longrightarrow} & U\end{array}
$$
...
Lemma \ref{sites-lemma-equivalence} a name.

\begin{definition}[Special cocontinuous functor]
\label{sites-definition-special-cocontinuous-functor}
\source{stacks:03CG}
\href{https://stacks.math.columbia.edu/tag/03CG}{\texttt{Stacks tag 03CG}}
\group{sites}

\uses{sites-lemma-equivalence}
Let $\mathcal{C}$, $\mathcal{D}$ be sites.
...
the assumptions and conclusions of Lemma \ref{sites-lemma-equivalence}.
\end{definition}

\begin{lemma}[Localize special cocontinuous]
\label{sites-lemma-localize-special-cocontinuous}
\source{stacks:03CH}
\href{https://stacks.math.columbia.edu/tag/03CH}{\texttt{Stacks tag 03CH}}
\group{sites}

\uses{sites-lemma-localize-cocontinuous}
Let $\mathcal{C}$, $\mathcal{D}$ be sites.
...
$$
\begin{array}{cc}\mathcal{C}/U \underset{j_U}{\longrightarrow}\downarrow & \mathcal{C} \overset{u}{\downarrow} \\ \mathcal{D}/u(U) \overset{j_{u(U)}}{\longrightarrow} & \mathcal{D}\end{array}
$$
as in Lemma \ref{sites-lemma-localize-cocontinuous}.
The left vertical arrow is a special cocontinuous functor.
Hence in the commutative diagram of topoi
$$
\begin{array}{cc}\Sh(\mathcal{C}/U) \underset{j_U}{\longrightarrow}\downarrow & \Sh(\mathcal{C}) \overset{u}{\downarrow} \\ \Sh(\mathcal{D}/u(U)) \overset{j_{u(U)}}{\longrightarrow} & \Sh(\mathcal{D})\end{array}
$$
the vertical arrows are equivalences.
\end{lemma}

\begin{proof}
\uses{sites-lemma-localize-cocontinuous, sites-lemma-equivalence}

We have seen the existence and commutativity of the diagrams in
Lemma \ref{sites-lemma-localize-cocontinuous}. We have to check
hypotheses (1) -- (5) of Lemma \ref{sites-lemma-equivalence} for the
induced functor $u : \mathcal{C}/U \to \mathcal{D}/u(U)$.
This is completely mechanical.

\medskip\noindent
Property (1). This is Lemma \ref{sites-lemma-localize-cocontinuous}.
...
\begin{lemma}[Special equivalence]
\label{sites-lemma-special-equivalence}
\source{stacks:03A1}
\href{https://stacks.math.columbia.edu/tag/03A1}{\texttt{Stacks tag 03A1}}
\group{sites}

\uses{sites-definition-weaker-than-canonical}
Let $\mathcal{C}$ be a site. Let
...
\begin{proof}
\uses{sites-definition-site, sites-lemma-coequalizer-surjection, sites-lemma-equivalence, sites-lemma-mono-epi-sheaves, sites-lemma-sheafification-exact, sites-lemma-covering-surjective-after-sheafification, sites-lemma-sheaf-coequalizer-representable, sites-lemma-when-shriek}
...
$$
\begin{array}{cc}\Mor_{\Sh(\mathcal{C})}(
\coprod_{i \in I} \mathcal{F}_i, \mathcal{G})
\longrightarrow\longrightarrow & \Mor_{\Sh(\mathcal{C})}(
\coprod_{(i_0, i_1) \in I \times I}
\mathcal{F}_{i_0} \times_\mathcal{F} \mathcal{F}_{i_1}, \mathcal{G})\end{array}
$$
is $\Mor_{\Sh(\mathcal{C})}(\mathcal{F}, \mathcal{G}).$
This is clear (for example use Lemma \ref{sites-lemma-coequalizer-surjection}).

\medskip\noindent
To prove (3) we
have to check conditions (1) -- (5) of Lemma \ref{sites-lemma-equivalence}.
...
Lemma \ref{sites-lemma-sheafification-exact}, and
Lemma \ref{sites-lemma-covering-surjective-after-sheafification}).
Properties (3), (4) of Lemma \ref{sites-lemma-equivalence}
...
Property (5) of Lemma \ref{sites-lemma-equivalence} is
Lemma \ref{sites-lemma-sheaf-coequalizer-representable}.
Denote $g : \Sh(\mathcal{C}) \to \Sh(\mathcal{C}')$ the
equivalence of topoi associated with $v$ by Lemma \ref{sites-lemma-equivalence}.
...
\begin{lemma}[Topos good site]
\label{sites-lemma-topos-good-site}
\source{stacks:03CI}
\href{https://stacks.math.columbia.edu/tag/03CI}{\texttt{Stacks tag 03CI}}
\group{sites}
...
\begin{proof}
\uses{sites-example-singleton-sheaf, sites-lemma-special-equivalence}
...
Example \ref{sites-example-singleton-sheaf}). We are going to inductively
...
$U \mapsto h_U^\#$. Apply
Lemma \ref{sites-lemma-special-equivalence}.
\end{proof}

Here is the goal of the current section.

\begin{lemma}[Morphism topoi comes from morphism sites]
\label{sites-lemma-morphism-topoi-comes-from-morphism-sites}
\source{stacks:03A2}
\href{https://stacks.math.columbia.edu/tag/03A2}{\texttt{Stacks tag 03A2}}
\group{sites}

\uses{sites-lemma-equivalence, sites-lemma-morphism-sites-topoi}
\begin{reference}
...
$$
\begin{array}{ccc}\mathcal{C} \underset{v}{\longrightarrow} & \mathcal{C}' & \mathcal{D} \overset{u}{\longleftarrow}\end{array}
$$
such that
\begin{enumerate}
...
\Sh(\mathcal{D})
$$
where the first arrow comes from $v$ via Lemma \ref{sites-lemma-equivalence}
and the second arrow from $u$ via Lemma \ref{sites-lemma-morphism-sites-topoi}.
\end{enumerate}
\end{lemma}

\begin{proof}
\uses{sites-lemma-special-equivalence}
...
\medskip\noindent
Part (1) follows from Lemma \ref{sites-lemma-special-equivalence}.
...
$u^s \circ g_* = f_*$. Let $\mathcal{F}$ be a sheaf on $\mathcal{C}$
and let $V$ be an object of $\mathcal{D}$. Then
\begin{align*}
...
f_{\ast}\mathcal{F}(V)
\end{align*}
The first equality because $u^s = u^p$.
The second equality is the Yoneda lemma.
The third equality by adjointness of $g^{-1}$ and $g_*$.
The fourth equality is by Lemma \ref{sites-lemma-special-equivalence} (4).
The fifth equality by adjointness of $f^{-1}$ and $f_*$.
The sixth equality by the Yoneda lemma.
Hence $u^s g_*\mathcal{F} = f_*\mathcal{F}$ and
this finishes the proof of the lemma.
\end{proof}

\begin{remark}[Morphism topoi comes from morphism sites]
\label{sites-remark-morphism-topoi-comes-from-morphism-sites}
\source{stacks:03CJ}
\href{https://stacks.math.columbia.edu/tag/03CJ}{\texttt{Stacks tag 03CJ}}
\group{sites}

\uses{sites-lemma-morphism-topoi-comes-from-morphism-sites, sites-proposition-get-morphism, sites-lemma-topos-good-site}
Notation and assumptions
as in Lemma \ref{sites-lemma-morphism-topoi-comes-from-morphism-sites}.
If the site $\mathcal{D}$ has a final object and fibre products
then the functor $u : \mathcal{D} \to \mathcal{C}'$ satisfies
all the assumptions of Proposition \ref{sites-proposition-get-morphism}.
...
Lemmas \ref{sites-lemma-topos-good-site}
to $\mathcal{D}$
and then
Lemma \ref{sites-lemma-morphism-topoi-comes-from-morphism-sites}
...
$$
\begin{array}{cc}\Sh(\mathcal{C}) \underset{g}{\downarrow}\underset{f}{\longrightarrow} & \Sh(\mathcal{D}) \overset{e}{\downarrow} \\ \Sh(\mathcal{C}') \overset{f'}{\longrightarrow} & \Sh(\mathcal{D}')\end{array}
$$
...
\begin{remark}[Equivalence topoi comes from morphism sites]
\label{sites-remark-equivalence-topoi-comes-from-morphism-sites}
\source{stacks:03CK}
\href{https://stacks.math.columbia.edu/tag/03CK}{\texttt{Stacks tag 03CK}}
\group{sites}

\uses{sites-lemma-morphism-topoi-comes-from-morphism-sites, sites-remark-morphism-topoi-comes-from-morphism-sites}
Notation and assumptions
as in Lemma \ref{sites-lemma-morphism-topoi-comes-from-morphism-sites}.
...
\Sh(\mathcal{D})
$$
as in Remark \ref{sites-remark-morphism-topoi-comes-from-morphism-sites}, but
...
\begin{lemma}[Localize topos]
\label{sites-lemma-localize-topos}
\source{stacks:04GZ}
\href{https://stacks.math.columbia.edu/tag/04GZ}{\texttt{Stacks tag 04GZ}}
\group{sites}
...
Section \ref{sites-section-localize}
such that
\begin{enumerate}
...
\begin{proof}
\uses{sites-lemma-topos-good-site, sites-lemma-essential-image-j-shriek, sites-lemma-compute-j-shriek-restrict}

Apply
Lemma \ref{sites-lemma-topos-good-site}.
...
Lemma \ref{sites-lemma-compute-j-shriek-restrict}.
\end{proof}

\begin{lemma}[Localize pushforward]
\label{sites-lemma-localize-pushforward}
\source{stacks:04H0}
\href{https://stacks.math.columbia.edu/tag/04H0}{\texttt{Stacks tag 04H0}}
\group{sites}

\uses{sites-lemma-localize-topos}
In the situation of Lemma \ref{sites-lemma-localize-topos}, the functor
...
\begin{proof}
\uses{sites-lemma-localize-topos, sites-lemma-internal-hom-sheaf}
...
(\mathcal{G}/\mathcal{F})).
$$
By Lemma \ref{sites-lemma-localize-topos} this set is the fiber over
...
$$
By the adjunction in Lemma \ref{sites-lemma-internal-hom-sheaf}, we have
\begin{align*}
\Mor_{\Sh(\mathcal{C})}(h_U^{\#}\times\mathcal{F}, \mathcal{G})
& =
...
\begin{lemma}[Localize topos site]
\label{sites-lemma-localize-topos-site}
\source{stacks:0791}
\href{https://stacks.math.columbia.edu/tag/0791}{\texttt{Stacks tag 0791}}
\group{sites}
...
Lemma \ref{sites-lemma-when-shriek} there exists a morphism of topoi
...
\begin{definition}[Localize topos]
\label{sites-definition-localize-topos}
\source{stacks:04IP}
\href{https://stacks.math.columbia.edu/tag/04IP}{\texttt{Stacks tag 04IP}}
\group{sites}
...
\to
\Sh(\mathcal{C})$ of
Lemma \ref{sites-lemma-localize-topos}
is called the {\it localization morphism}.
\end{enumerate}
\end{definition}
...
\source{stacks:04IQ}
\href{https://stacks.math.columbia.edu/tag/04IQ}{\texttt{Stacks tag 04IQ}}
\group{sites}

\uses{sites-lemma-localize-topos, sites-lemma-essential-image-j-shriek}
...
\begin{proof}
\uses{sites-lemma-essential-image-j-shriek, sites-lemma-localize-topos}
...
\begin{lemma}[Relocalize topos]
\label{sites-lemma-relocalize-topos}
\source{stacks:04IR}
\href{https://stacks.math.columbia.edu/tag/04IR}{\texttt{Stacks tag 04IR}}
\group{sites}
...
morphisms of topoi
$$
\begin{array}{ccc}\Sh(\mathcal{C})/\mathcal{G} \underset{j_\mathcal{G}}{\searrow}\underset{j}{\longrightarrow} &  & \Sh(\mathcal{C})/\mathcal{F} \overset{j_\mathcal{F}}{\swarrow} \\  & \Sh(\mathcal{C}) & \end{array}
$$
...
\source{stacks:04IS}
\href{https://stacks.math.columbia.edu/tag/04IS}{\texttt{Stacks tag 04IS}}
\group{sites}

\uses{sites-lemma-relocalize-topos, sites-lemma-essential-image-j-shriek}
Assume $\mathcal{C}$ and $s : \mathcal{G} \to \mathcal{F}$ are as in
Lemma \ref{sites-lemma-relocalize-topos}.
...
\begin{proof}
\uses{sites-lemma-relocalize-explicit, sites-lemma-relocalize-topos}
...
This section is the analogue of
Section \ref{sites-section-localize-morphisms}
for morphisms of topoi.

\begin{lemma}[Localize morphism topoi]
\label{sites-lemma-localize-morphism-topoi}
\source{stacks:04H1}
\href{https://stacks.math.columbia.edu/tag/04H1}{\texttt{Stacks tag 04H1}}
\group{sites}
...
$$
\begin{array}{cc}\Sh(\mathcal{C})/\mathcal{F} \underset{j_\mathcal{F}}{\longrightarrow}\underset{f'}{\downarrow} & \Sh(\mathcal{C}) \overset{f}{\downarrow} \\ \Sh(\mathcal{D})/\mathcal{G} \overset{j_\mathcal{G}}{\longrightarrow} & \Sh(\mathcal{D}).\end{array}
$$
...
\begin{proof}
\uses{sites-remark-morphism-topoi-comes-from-morphism-sites, sites-proposition-get-morphism, sites-lemma-pullback-representable-sheaf, sites-lemma-localize-morphism, sites-lemma-localize-compare}
...
Remark \ref{sites-remark-morphism-topoi-comes-from-morphism-sites}
...
Lemma \ref{sites-lemma-localize-compare}
...
\source{stacks:04IU}
\href{https://stacks.math.columbia.edu/tag/04IU}{\texttt{Stacks tag 04IU}}
\group{sites}

\uses{sites-lemma-pullback-representable-sheaf, sites-lemma-localize-morphism-topoi, sites-lemma-localize-morphism, sites-lemma-localize-compare}
...
Lemma \ref{sites-lemma-localize-morphism-topoi}
...
\begin{proof}
\uses{sites-lemma-localize-morphism-topoi}
...
\begin{lemma}[Relocalize morphism topoi]
\label{sites-lemma-relocalize-morphism-topoi}
\source{stacks:04IV}
\href{https://stacks.math.columbia.edu/tag/04IV}{\texttt{Stacks tag 04IV}}
\group{sites}

\uses{sites-lemma-localize-morphism-topoi, sites-lemma-relocalize-topos}
Let $f : \Sh(\mathcal{C}) \to \Sh(\mathcal{D})$
...
There exists a commutative diagram of topoi
$$
\begin{array}{cc}\Sh(\mathcal{C})/\mathcal{F} \underset{j_\mathcal{F}}{\longrightarrow}\underset{f_s}{\downarrow} & \Sh(\mathcal{C}) \overset{f}{\downarrow} \\ \Sh(\mathcal{D})/\mathcal{G} \overset{j_\mathcal{G}}{\longrightarrow} & \Sh(\mathcal{D}).\end{array}
$$
...
commutes, then the diagram
$$
\begin{array}{cc}\Sh(\mathcal{C})/\mathcal{F}'
\underset{j_{\mathcal{F}'/\mathcal{F}}}{\longrightarrow}\underset{f_{s'}}{\downarrow} & \Sh(\mathcal{C})/\mathcal{F} \overset{f_s}{\downarrow} \\ \Sh(\mathcal{D})/\mathcal{G}' \overset{j_{\mathcal{G}'/\mathcal{G}}}{\longrightarrow} & \Sh(\mathcal{D})/\mathcal{G}.\end{array}
$$
commutes.
\end{lemma}

\begin{proof}
\uses{sites-lemma-relocalize-topos, sites-lemma-localize-morphism-topoi}
...
\source{stacks:04IW}
\href{https://stacks.math.columbia.edu/tag/04IW}{\texttt{Stacks tag 04IW}}
\group{sites}

\uses{sites-lemma-relocalize-morphism, sites-lemma-relocalize-morphism-topoi, sites-lemma-localize-compare}
Let $f : \mathcal{C} \to \mathcal{D}$ be a morphism of sites given
by the continuous functor $u : \mathcal{D} \to \mathcal{C}$.
...
Then the diagram of morphisms of topoi of
Lemma \ref{sites-lemma-relocalize-morphism}
agrees with the diagram of morphisms of topoi of
Lemma \ref{sites-lemma-relocalize-morphism-topoi}
via the identifications $j_\mathcal{F} = j_U$
and $j_\mathcal{G} = j_V$ of
Lemma \ref{sites-lemma-localize-compare}.
\end{lemma}
...
\begin{proof}
\uses{sites-lemma-relocalize-compare, sites-lemma-localize-morphism-compare}

This follows on combining
Lemmas \ref{sites-lemma-relocalize-compare} and
\ref{sites-lemma-localize-morphism-compare}.
\end{proof}

\section{Points}
\label{sites-section-points}
\begin{definition}[Point topos]
\label{sites-definition-point-topos}
\source{stacks:00Y4}
\href{https://stacks.math.columbia.edu/tag/00Y4}{\texttt{Stacks tag 00Y4}}
\group{sites}
...
Lemma \ref{sites-lemma-site-point-morphism}.
...
\source{stacks:00Y5}
\href{https://stacks.math.columbia.edu/tag/00Y5}{\texttt{Stacks tag 00Y5}}
\group{sites}
...
\begin{lemma}[Adjoint point push stalk]
\label{sites-lemma-adjoint-point-push-stalk}
\source{stacks:00Y7}
\href{https://stacks.math.columbia.edu/tag/00Y7}{\texttt{Stacks tag 00Y7}}
\group{sites}
...
In analogy with Section \ref{sites-section-continuous-functors}
we have the following lemma.

\begin{lemma}[Point pushforward sheaf]
\label{sites-lemma-point-pushforward-sheaf}
\source{stacks:00Y8}
\href{https://stacks.math.columbia.edu/tag/00Y8}{\texttt{Stacks tag 00Y8}}
\group{sites}
...
\begin{proof}
\uses{sites-lemma-adjoint-point-push-stalk}
...
(Lemma \ref{sites-lemma-adjoint-point-push-stalk}) restricted to sheaves.
...
In particular Lemma \ref{sites-lemma-point-pushforward-sheaf}
holds when $p = u$ is a point. In this case we think of the
sheaf $u^sE$ as the ``skyscraper'' sheaf with value $E$ at $p$.

\begin{definition}[Pushforward point]
\label{sites-definition-pushforward-point}
\source{stacks:00Y9}
\href{https://stacks.math.columbia.edu/tag/00Y9}{\texttt{Stacks tag 00Y9}}
\group{sites}

\uses{sites-lemma-point-pushforward-sheaf}
Let $p$ be a point of the site $\mathcal{C}$ given by the functor $u$.
For a set $E$ we define $p_*E = u^sE$ the sheaf
described in Lemma \ref{sites-lemma-point-pushforward-sheaf} above.
...
\begin{lemma}[Point site topos]
\label{sites-lemma-point-site-topos}
\source{stacks:00YA}
\href{https://stacks.math.columbia.edu/tag/00YA}{\texttt{Stacks tag 00YA}}
\group{sites}
...
\begin{proof}
\uses{sites-definition-point, sites-definition-topos, sites-lemma-covering-surjective-after-sheafification, sites-lemma-sheafification-exact, sites-lemma-points-recover, sites-lemma-point-pushforward-sheaf, sites-lemma-sheaf-coequalizer-representable}

Proof of (1). By the above the functors $p_*$ and $p^{-1}$ are adjoint.
The functor $p^{-1}$ is required to be exact by
Definition \ref{sites-definition-point}.
Hence the conditions imposed in
Definition \ref{sites-definition-topos}
...
\begin{lemma}[Site point morphism]
\label{sites-lemma-site-point-morphism}
\source{stacks:04EL}
\href{https://stacks.math.columbia.edu/tag/04EL}{\texttt{Stacks tag 04EL}}
\group{sites}

\uses{sites-remark-pt-topos, sites-lemma-point-site-topos}
...
Remark \ref{sites-remark-pt-topos}.
Then
\begin{enumerate}
...
Lemma \ref{sites-lemma-point-site-topos}.
\end{enumerate}
\end{lemma}

\begin{proof}
\uses{sites-remark-pt-topos, sites-definition-point, sites-definition-morphism-sites}
...
\begin{lemma}[Stalk skyscraper]
\label{sites-lemma-stalk-skyscraper}
\source{stacks:05UX}
\href{https://stacks.math.columbia.edu/tag/05UX}{\texttt{Stacks tag 05UX}}
\group{sites}
...
$$
\begin{array}{ccc}p^{-1}p_*\{*\} \underset{p^{-1}p_*i_e}{\longrightarrow}\underset{\cong}{\downarrow} &  & p^{-1}p_*E \downarrow \\ \{*\} \overset{i_e}{\longrightarrow} &  & E\end{array}
$$
whence the map $E \to (p_*E)_p = p^{-1}p_*E$ as desired.
\end{proof}

\begin{lemma}[Skyscraper functor exact]
\label{sites-lemma-skyscraper-functor-exact}
\source{stacks:05UY}
\href{https://stacks.math.columbia.edu/tag/05UY}{\texttt{Stacks tag 05UY}}
\group{sites}
...
\begin{proof}
\uses{sites-lemma-point-site-topos, sites-definition-pushforward-point}
...
Definition \ref{sites-definition-pushforward-point}.
...
\begin{lemma}[Neighbourhoods cofiltered]
\label{sites-lemma-neighbourhoods-cofiltered}
\source{stacks:0F4E}
\href{https://stacks.math.columbia.edu/tag/0F4E}{\texttt{Stacks tag 0F4E}}
\group{sites}
...
According to Lemma \ref{sites-lemma-limit-sheaf} this is a sheaf and
...
Section \ref{sites-section-limits-colimits-PSh}. Thus we have to show that
$$
...
\begin{lemma}[Neighbourhoods directed]
\label{sites-lemma-neighbourhoods-directed}
\source{stacks:00YB}
\href{https://stacks.math.columbia.edu/tag/00YB}{\texttt{Stacks tag 00YB}}
\group{sites}
...
\begin{proof}
\uses{sites-lemma-directed, sites-lemma-neighbourhoods-cofiltered}

Please note the analogy with Lemma \ref{sites-lemma-directed}.
...
The ``consequently'' clause is Lemma \ref{sites-lemma-neighbourhoods-cofiltered}.
\end{proof}

\begin{proposition}[Point limits]
\label{sites-proposition-point-limits}
\source{stacks:00YC}
\href{https://stacks.math.columbia.edu/tag/00YC}{\texttt{Stacks tag 00YC}}
\group{sites}
...
\begin{proof}
\uses{sites-definition-point, sites-lemma-points-recover, sites-lemma-point-pushforward-sheaf, sites-lemma-neighbourhoods-directed}

Suppose first that $p$ is a point (Definition \ref{sites-definition-point})
given by a functor $u$. Condition (2) is satisfied directly from
the definition of a point. By Lemma \ref{sites-lemma-points-recover}
we have $(h_U)_p = u(U)$. By Lemma \ref{sites-lemma-point-pushforward-sheaf}
...
conditions of Definition \ref{sites-definition-point}. And its
stalk functor is exact, because it is a left adjoint by
Lemma \ref{sites-lemma-point-pushforward-sheaf} and it commutes
with finite limits by Lemma \ref{sites-lemma-neighbourhoods-directed}.
\end{proof}

\begin{remark}[Improve proposition points limits]
\label{sites-remark-improve-proposition-points-limits}
\source{stacks:00YD}
\href{https://stacks.math.columbia.edu/tag/00YD}{\texttt{Stacks tag 00YD}}
\group{sites}
...
\begin{example}[Point topological]
\label{sites-example-point-topological}
\source{stacks:00YE}
\href{https://stacks.math.columbia.edu/tag/00YE}{\texttt{Stacks tag 00YE}}
\group{sites}

\uses{sites-example-site-topological}
Let $X$ be a topological space. Let $X_{Zar}$ be the site of
Example \ref{sites-example-site-topological}.
...
\begin{example}[Point topology]
\label{sites-example-point-topology}
\source{stacks:04EJ}
\href{https://stacks.math.columbia.edu/tag/04EJ}{\texttt{Stacks tag 04EJ}}
\group{sites}

\uses{sites-example-site-topological, sites-lemma-point-site-topos, sites-proposition-point-limits, sites-example-point-topological}
...
Example \ref{sites-example-point-topological}.
\end{example}

\begin{example}[Point G sets]
\label{sites-example-point-G-sets}
\source{stacks:00YF}
\href{https://stacks.math.columbia.edu/tag/00YF}{\texttt{Stacks tag 00YF}}
\group{sites}

\uses{sites-example-site-on-group, sites-lemma-stalk-skyscraper}
...
Lemma \ref{sites-lemma-stalk-skyscraper}
are the obvious ones.
\end{example}

\begin{example}[Indiscrete points]
\label{sites-example-indiscrete-points}
\source{stacks:08RH}
\href{https://stacks.math.columbia.edu/tag/08RH}{\texttt{Stacks tag 08RH}}
\group{sites}

\uses{sites-example-indiscrete, sites-definition-point}
Let $\mathcal{C}$ be a category endowed with the chaotic topology
(Example \ref{sites-example-indiscrete}). For every object $U_0$ of $\mathcal{C}$
the functor $u : U \mapsto \Mor_\mathcal{C}(U_0, U)$ defines a point
$p$ of $\mathcal{C}$. Namely, conditions (1) and (2) of
Definition \ref{sites-definition-point} are immediate as the only coverings
...
\begin{lemma}[Point functor]
\label{sites-lemma-point-functor}
\source{stacks:0F4F}
\href{https://stacks.math.columbia.edu/tag/0F4F}{\texttt{Stacks tag 0F4F}}
\group{sites}
...
\begin{lemma}[A map of sites defines a map on points, and pullback respects stalks.]
\label{sites-lemma-point-morphism-sites}
\source{stacks:05V1}
\href{https://stacks.math.columbia.edu/tag/05V1}{\texttt{Stacks tag 05V1}}
\group{sites}


\uses{sites-definition-point}
...
\begin{proof}
\uses{sites-lemma-point-morphism-sites, sites-definition-point, sites-lemma-point-pushforward-sheaf, sites-lemma-point-functor}
[First proof Lemma \ref{sites-lemma-point-morphism-sites}]
Note that since $u$ is continuous and since $v$ defines a point,
it is immediate that $v \circ u$ satisfies conditions (1) and (2) of
Definition \ref{sites-definition-point}. Let us prove the displayed equality.
...
by Lemma \ref{sites-lemma-point-pushforward-sheaf}, and the third
by Lemma \ref{sites-lemma-point-functor}.
...
\begin{proof}
\uses{sites-lemma-point-morphism-sites, sites-lemma-site-point-morphism, sites-lemma-composition-morphisms-sites, sites-remark-pt-topos, sites-definition-point, sites-lemma-point-pushforward-sheaf}
[Second proof Lemma \ref{sites-lemma-point-morphism-sites}]
By
Lemma \ref{sites-lemma-site-point-morphism}
...
\begin{lemma}[Point morphism topoi]
\label{sites-lemma-point-morphism-topoi}
\source{stacks:05V2}
\href{https://stacks.math.columbia.edu/tag/05V2}{\texttt{Stacks tag 05V2}}
\group{sites}
...
\begin{lemma}[Point localize]
\label{sites-lemma-point-localize}
\source{stacks:04H2}
\href{https://stacks.math.columbia.edu/tag/04H2}{\texttt{Stacks tag 04H2}}
\group{sites}
...
$$
\begin{array}{cc} & \Sh(pt) \overset{p}{\downarrow}\underset{q}{\swarrow} \\ \Sh(\mathcal{C}/U) \overset{j_U}{\longrightarrow} & \Sh(\mathcal{C})\end{array}
$$
...
\begin{proof}
\uses{sites-lemma-site-point-morphism, sites-lemma-localize-morphism, sites-lemma-cocontinuous-morphism-topoi}
...
$$
\begin{array}{cc}\Sh(\mathcal{S}/u(U)) \underset{j_{u(U)}}{\longrightarrow}\underset{p'}{\downarrow} & \Sh(\mathcal{S}) \overset{p}{\downarrow} \\ \Sh(\mathcal{C}/U) \overset{j_U}{\longrightarrow} & \Sh(\mathcal{C}),\end{array}
$$
...
diagram of topoi
$$
\begin{array}{cccc}\Sh(\mathcal{S}) \overset{a}{\searrow}\underset{q}{\searrow}\overset{p}{\searrow} &  &  &  \\  & \Sh(\mathcal{S}/u(U)) \underset{j_{u(U)}}{\longrightarrow}\overset{p'}{\downarrow} & \Sh(\mathcal{S}) \overset{p}{\downarrow} &  \\  & \Sh(\mathcal{C}/U) \overset{j_U}{\longrightarrow} & \Sh(\mathcal{C}) & \end{array}
$$
Namely:
\begin{enumerate}
\item The morphism
...
\begin{lemma}[Points above point]
\label{sites-lemma-points-above-point}
\source{stacks:04H3}
\href{https://stacks.math.columbia.edu/tag/04H3}{\texttt{Stacks tag 04H3}}
\group{sites}

\uses{sites-lemma-point-localize}
Let $\mathcal{C}$, $p$, $u$, $U$ be as in
Lemma \ref{sites-lemma-point-localize}.
The construction of
Lemma \ref{sites-lemma-point-localize}
...
\begin{proof}
\uses{sites-lemma-point-site-topos, sites-lemma-technical-pu, sites-lemma-sheafification-exact}
...
$$
\vcenter{
\begin{array}{cc}\Mor_{\mathcal{C}/U}(W/U, V/U) \longrightarrow\downarrow & \Mor_\mathcal{C}(W, V) \downarrow \\ \Mor_{\mathcal{C}/U}(W/U, U/U) \longrightarrow & \Mor_\mathcal{C}(W, U)\end{array}
}
\quad
\vcenter{
\begin{array}{cc}h_{V/U}^\# \longrightarrow\downarrow & j_U^{-1}(h_V^\#) \downarrow \\ h_{U/U}^\# \longrightarrow & j_U^{-1}(h_U^\#)\end{array}
}
$$
...
Lemma \ref{sites-lemma-technical-pu}.)
...
$$
\begin{array}{cc}v(V/U) \longrightarrow\downarrow & u(V) \downarrow \\ v(U/U) \longrightarrow & u(U)\end{array}
$$
...
\source{stacks:04H4}
\href{https://stacks.math.columbia.edu/tag/04H4}{\texttt{Stacks tag 04H4}}
\group{sites}
...
\begin{proof}
\uses{sites-lemma-describe-j-shriek, sites-lemma-point-pushforward-sheaf}
...
\source{stacks:04H5}
\href{https://stacks.math.columbia.edu/tag/04H5}{\texttt{Stacks tag 04H5}}
\group{sites}

\uses{sites-lemma-stalk-j-shriek}
Warning: The result of
Lemma \ref{sites-lemma-stalk-j-shriek}
...
\begin{definition}[2 morphism topoi]
\label{sites-definition-2-morphism-topoi}
\source{stacks:04IA}
\href{https://stacks.math.columbia.edu/tag/04IA}{\texttt{Stacks tag 04IA}}
\group{sites}
...
$$
\begin{array}{ccc}\Sh(\mathcal{C})
\rrtwocell^f_g{t} &  & \Sh(\mathcal{D})\end{array}
$$
...
$$
\begin{array}{cc}\Mor_{\Sh(\mathcal{D})}(\mathcal{G}, f_*\mathcal{F})
\underset{t \circ -}{\downarrow}\xlongequal{} & \Mor_{\Sh(\mathcal{C})}(f^{-1}\mathcal{G}, \mathcal{F})
\overset{- \circ t}{\downarrow} \\ \Mor_{\Sh(\mathcal{D})}(\mathcal{G}, g_*\mathcal{F})
\xlongequal{} & \Mor_{\Sh(\mathcal{C})}(g^{-1}\mathcal{G}, \mathcal{F})\end{array}
$$
commutes. Because of set theoretic difficulties (see
Remark \ref{sites-remark-morphism-topoi-big})
...
$$
\begin{array}{ccc}\Sh(\mathcal{C})
\xrightarrow{\substack{f\\g\\t}} & \Sh(\mathcal{D})
\xrightarrow{\substack{f'\\g'\\s}} & \Sh(\mathcal{E})\end{array}
$$
...
$$
\begin{array}{ccc}f'_*f_*\mathcal{F}
\overset{f'_*t}{\longrightarrow} & f'_*g_*\mathcal{F}
\overset{s}{\longrightarrow} & g'_*g_*\mathcal{F}\end{array}
\quad\text{or}\quad
\begin{array}{ccc}f'_*f_*\mathcal{F}
\overset{s}{\longrightarrow} & g'_*f_*\mathcal{F}
\overset{g'_*t}{\longrightarrow} & g'_*g_*\mathcal{F}\end{array}
$$
...
\begin{lemma}[Maps u points]
\label{sites-lemma-maps-u-points}
\source{stacks:00YH}
\href{https://stacks.math.columbia.edu/tag/00YH}{\texttt{Stacks tag 00YH}}
\group{sites}

\uses{sites-lemma-points-recover}
Let $\mathcal{C}$ be a site.
...
\begin{definition}[Morphism points]
\label{sites-definition-morphism-points}
\source{stacks:00YI}
\href{https://stacks.math.columbia.edu/tag/00YI}{\texttt{Stacks tag 00YI}}
\group{sites}
...
=
\Sh(pt)
\rrtwocell^p_{p'}{f} &  & \Sh(\mathcal{C})\end{array}
$$
...
described in Example \ref{sites-example-point-G-sets}.
...
\begin{definition}[Enough points]
\label{sites-definition-enough-points}
\source{stacks:00YK}
\href{https://stacks.math.columbia.edu/tag/00YK}{\texttt{Stacks tag 00YK}}
\group{sites}
...
\begin{lemma}[Exactness stalks]
\label{sites-lemma-exactness-stalks}
\source{stacks:00YL}
\href{https://stacks.math.columbia.edu/tag/00YL}{\texttt{Stacks tag 00YL}}
\group{sites}
...
\source{stacks:00YM}
\href{https://stacks.math.columbia.edu/tag/00YM}{\texttt{Stacks tag 00YM}}
\group{sites}
...
\begin{proof}
\uses{sites-lemma-exactness-stalks, sites-lemma-points-recover, sites-lemma-point-pushforward-sheaf, sites-lemma-mono-epi-sheaves}
...
\ref{sites-lemma-exactness-stalks} there is an $i$ such that $a_{p_i}
\not = a'_{p_i}$. Recall that $(h_U)^\#_{p_i} = u_i(U)$, by
Lemmas \ref{sites-lemma-points-recover} and
...
injective and surjective, see Lemma \ref{sites-lemma-mono-epi-sheaves}.
...
Lemma \ref{sites-lemma-points-above-point}.

\begin{lemma}[Localize enough]
\label{sites-lemma-localize-enough}
\source{stacks:04H6}
\href{https://stacks.math.columbia.edu/tag/04H6}{\texttt{Stacks tag 04H6}}
\group{sites}
...
\begin{proof}
\uses{sites-lemma-essential-image-j-shriek, sites-lemma-stalk-j-shriek}
...
\begin{lemma}[Enough points local]
\label{sites-lemma-enough-points-local}
\source{stacks:06UL}
\href{https://stacks.math.columbia.edu/tag/06UL}{\texttt{Stacks tag 06UL}}
\group{sites}
...
\begin{proof}
\uses{sites-lemma-point-morphism-topoi}
...
Lemma \ref{sites-lemma-point-morphism-topoi}.
...
\begin{lemma}[Check morphism sites]
\label{sites-lemma-check-morphism-sites}
\source{stacks:0F4G}
\href{https://stacks.math.columbia.edu/tag/0F4G}{\texttt{Stacks tag 0F4G}}
\group{sites}
...
\begin{proof}
\uses{sites-lemma-point-pushforward-sheaf, sites-lemma-point-functor, sites-definition-morphism-sites}
...
that $f$ is a morphism of sites by Definition \ref{sites-definition-morphism-sites}.
\end{proof}
...
associated to $u$ as in Section \ref{sites-section-points}.
...
see Lemma \ref{sites-lemma-maps-u-points}.

\begin{lemma}[Refine]
\label{sites-lemma-refine}
\source{stacks:00YO}
\href{https://stacks.math.columbia.edu/tag/00YO}{\texttt{Stacks tag 00YO}}
\group{sites}
...
\begin{lemma}[Refine all at once]
\label{sites-lemma-refine-all-at-once}
\source{stacks:00YP}
\href{https://stacks.math.columbia.edu/tag/00YP}{\texttt{Stacks tag 00YP}}
\group{sites}
...
and this would, combined with Lemma \ref{sites-lemma-refine} above easily prove
...
as in Lemma \ref{sites-lemma-refine} with respect to the system
$e = (\{W_k \to W\}, f\in u'(W))$.
...
\begin{proposition}[Criterion points]
\label{sites-proposition-criterion-points}
\source{stacks:00YQ}
\href{https://stacks.math.columbia.edu/tag/00YQ}{\texttt{Stacks tag 00YQ}}
\group{sites}
...
\begin{proof}
\uses{sites-lemma-enough, sites-lemma-refine-all-at-once, sites-proposition-point-limits}
...
$\mathcal{F}_p$. See Lemma \ref{sites-lemma-enough}.
Consider the system $(J, \geq, V_j, g_{jj'})$
with $J = \{1\}$, $V_1 = U$, $g_{11} = \text{id}_U$.
Apply Lemma \ref{sites-lemma-refine-all-at-once}.
...
Proposition \ref{sites-proposition-point-limits} (and the discussion
at the beginning of this section which guarantees that $u$
commutes with finite limits).
\end{proof}

\begin{lemma}[Criterion points]
\label{sites-lemma-criterion-points}
\source{stacks:0DW0}
\href{https://stacks.math.columbia.edu/tag/0DW0}{\texttt{Stacks tag 0DW0}}
\group{sites}

\uses{sites-proposition-criterion-points}
...
\begin{lemma}[W contractible]
\label{sites-lemma-w-contractible}
\source{stacks:090K}
\href{https://stacks.math.columbia.edu/tag/090K}{\texttt{Stacks tag 090K}}
\group{sites}
...
\begin{proof}
\uses{sites-definition-sheaves-injective-surjective, sites-lemma-covering-surjective-after-sheafification}
...
$s|_{U_i}$, see Definition \ref{sites-definition-sheaves-injective-surjective}.
...
\begin{definition}[W contractible]
\label{sites-definition-w-contractible}
\source{stacks:090L}
\href{https://stacks.math.columbia.edu/tag/090L}{\texttt{Stacks tag 090L}}
\group{sites}

\uses{sites-lemma-w-contractible}
Let $\mathcal{C}$ be a site.
\begin{enumerate}
\item We say an object $U$ of $\mathcal{C}$ is {\it weakly contractible}
if the equivalent conditions of Lemma \ref{sites-lemma-w-contractible} hold.
...
\begin{lemma}[Exactness properties]
\label{sites-lemma-exactness-properties}
\source{stacks:04D6}
\href{https://stacks.math.columbia.edu/tag/04D6}{\texttt{Stacks tag 04D6}}
\group{sites}
...
Picture
$$
\begin{array}{ccccccc}(6) \searrow &  &  &  &  & (9) \Longrightarrow\searrow & (8) \\  & (4) \Longleftrightarrow & (11) & (2) \Longleftrightarrow & (7) \nearrow\searrow &  & (10) \\ (5) \nearrow &  &  &  &  & (3) \Longrightarrow & (1)\end{array}
$$
\end{lemma}

\begin{proof}
\uses{sites-lemma-coequalizer-surjection}
...
$$
\begin{array}{cc}\mathcal{F} \longrightarrow\longrightarrow & \mathcal{F}' \\ f^{-1}f_*\mathcal{F} \longrightarrow\longrightarrow\uparrow & f^{-1}f_*\mathcal{F}' \uparrow\end{array}
$$
...
$$
\begin{array}{cc}\mathcal{F} \longrightarrow & \mathcal{F}' \\ f^{-1}f_*\mathcal{F} \longrightarrow\uparrow & f^{-1}f_*\mathcal{F}' \uparrow\end{array}
$$
...
by Lemma \ref{sites-lemma-coequalizer-surjection}.
...
$$
\begin{array}{cc}f_*\mathcal{F} \longrightarrow & f_*f^{-1}\mathcal{G} \\ \mathcal{G}' \uparrow\longrightarrow & \mathcal{G} \uparrow\end{array}
$$
so that $\mathcal{G}' \to \mathcal{G}$ is surjective also. Consider
the commutative diagram
$$
\begin{array}{cc}\mathcal{F} \longrightarrow & f^{-1}\mathcal{G} \\ f^{-1}f_*\mathcal{F} \longrightarrow\uparrow & f^{-1}f_*f^{-1}\mathcal{G} \uparrow \\ f^{-1}\mathcal{G}' \uparrow\longrightarrow & f^{-1}\mathcal{G} \uparrow\end{array}
$$
...
$$
\begin{array}{cc}\mathcal{F} \longrightarrow & \mathcal{F}' \\ \mathcal{F}'' \uparrow\longrightarrow & f^{-1}f_*\mathcal{F}' \uparrow\end{array}
$$
...
$$
\begin{array}{ccc} & \mathcal{F} \longrightarrow & \mathcal{F}' \\ f^{-1}\mathcal{G}' \longrightarrow & \mathcal{F}'' \uparrow\longrightarrow & f^{-1}f_*\mathcal{F}' \uparrow\end{array}
$$
Pushing this down using $f_*$ we get a commutative diagram
$$
\begin{array}{ccc} & f_*\mathcal{F} \longrightarrow & f_*\mathcal{F}' \\ f_*f^{-1}\mathcal{G}' \longrightarrow & f_*\mathcal{F}'' \uparrow\longrightarrow & f_*f^{-1}f_*\mathcal{F}' \uparrow \\ \mathcal{G}' \uparrow\longrightarrow &  & f_*\mathcal{F}' \uparrow\end{array}
$$
...
\source{stacks:04D7}
\href{https://stacks.math.columbia.edu/tag/04D7}{\texttt{Stacks tag 04D7}}
\group{sites}
...
$$
\begin{array}{cc}\coprod h_{V_j}^\# \underset{\coprod s_j}{\longrightarrow}\downarrow & \mathcal{F} \overset{a}{\downarrow} \\ h_{u(U)}^\# \overset{s}{\longrightarrow} & \mathcal{G}\end{array}
$$
By assumption there exists a covering $\{U_i \to U\}$ such that we can
enlarge the commutative diagram above as follows
$$
\begin{array}{ccc} & \coprod h_{V_j}^\# \underset{\coprod s_j}{\longrightarrow}\downarrow & \mathcal{F} \overset{a}{\downarrow} \\ \coprod h_{u(U_i)}^\# \longrightarrow\nearrow & h_{u(U)}^\# \overset{s}{\longrightarrow} & \mathcal{G}\end{array}
$$
...
\begin{example}[Weaker not coequalizer]
\label{sites-example-weaker-not-coequalizer}
\source{stacks:04D8}
\href{https://stacks.math.columbia.edu/tag/04D8}{\texttt{Stacks tag 04D8}}
\group{sites}

\uses{sites-lemma-weaker, sites-example-continuous-map}
Assume $f : \mathcal{D} \to \mathcal{C}$ satisfies the assumptions of
Lemma \ref{sites-lemma-weaker}.
...
Example \ref{sites-example-continuous-map}),
...
\begin{lemma}[Cover from below]
\label{sites-lemma-cover-from-below}
\source{stacks:04D9}
\href{https://stacks.math.columbia.edu/tag/04D9}{\texttt{Stacks tag 04D9}}
\group{sites}
...
\begin{example}[Cover from below]
\label{sites-example-cover-from-below}
\source{stacks:08LS}
\href{https://stacks.math.columbia.edu/tag/08LS}{\texttt{Stacks tag 08LS}}
\group{sites}

\uses{sites-lemma-cover-from-below, sites-example-site-on-group}
We construct a morphism $f : \mathcal{D} \to \mathcal{C}$ satisfying
the assumptions of Lemma \ref{sites-lemma-cover-from-below}. Namely, let
...
surjective maps (Example \ref{sites-example-site-on-group}).
Let $u : \mathcal{T}_H \to \mathcal{T}_G$ be the functor described
in Section \ref{sites-section-G-sets-morphisms}
...
the hypothesis of Lemma \ref{sites-lemma-cover-from-below}.
...
(as it should according to Lemma \ref{sites-lemma-cover-from-below})
...
\source{stacks:04B5}
\href{https://stacks.math.columbia.edu/tag/04B5}{\texttt{Stacks tag 04B5}}
\group{sites}
...
\begin{lemma}[Characterize empty]
\label{sites-lemma-characterize-empty}
\source{stacks:04B6}
\href{https://stacks.math.columbia.edu/tag/04B6}{\texttt{Stacks tag 04B6}}
\group{sites}
...
\begin{proof}
\uses{sites-theorem-plus, sites-remark-sheaf-condition-empty-covering, sites-definition-site}
...
presheaf, see
Theorem \ref{sites-theorem-plus}.
...
Remark \ref{sites-remark-sheaf-condition-empty-covering}.
...
\begin{definition}[Almost cocontinuous]
\label{sites-definition-almost-cocontinuous}
\source{stacks:04B7}
\href{https://stacks.math.columbia.edu/tag/04B7}{\texttt{Stacks tag 04B7}}
\group{sites}
...
Example \ref{sites-example-closed-map-cocontinuous-false}
...
\begin{lemma}[Almost cocontinuous sheafification]
\label{sites-lemma-almost-cocontinuous-sheafification}
\source{stacks:04B8}
\href{https://stacks.math.columbia.edu/tag/04B8}{\texttt{Stacks tag 04B8}}
\group{sites}
...
\begin{lemma}[Continuous almost cocontinuous]
\label{sites-lemma-continuous-almost-cocontinuous}
\source{stacks:04B9}
\href{https://stacks.math.columbia.edu/tag/04B9}{\texttt{Stacks tag 04B9}}
\group{sites}
...
\begin{proof}
\uses{sites-lemma-colimit-sheaves, sites-lemma-almost-cocontinuous-sheafification}
...
Lemma \ref{sites-lemma-almost-cocontinuous-sheafification}
...
\begin{lemma}[Morphism of sites almost cocontinuous]
\label{sites-lemma-morphism-of-sites-almost-cocontinuous}
\source{stacks:04BA}
\href{https://stacks.math.columbia.edu/tag/04BA}{\texttt{Stacks tag 04BA}}
\group{sites}
...
\begin{proof}
\uses{sites-lemma-continuous-almost-cocontinuous}

This is a special case of Lemma \ref{sites-lemma-continuous-almost-cocontinuous}.
\end{proof}
...
\source{stacks:08LU}
\href{https://stacks.math.columbia.edu/tag/08LU}{\texttt{Stacks tag 08LU}}
\group{sites}
...
According to Lemma \ref{sites-lemma-exactness-properties}
...
\begin{definition}[Subtopos]
\label{sites-definition-subtopos}
\source{stacks:08LV}
\href{https://stacks.math.columbia.edu/tag/08LV}{\texttt{Stacks tag 08LV}}
\group{sites}
...
\begin{lemma}[Open subtopos]
\label{sites-lemma-open-subtopos}
\source{stacks:08LW}
\href{https://stacks.math.columbia.edu/tag/08LW}{\texttt{Stacks tag 08LW}}
\group{sites}
...
\begin{proof}
\uses{sites-lemma-localize-topos, sites-lemma-topos-good-site, sites-lemma-localize-given-products, sites-lemma-restrict-back}

We have seen in Lemma \ref{sites-lemma-localize-topos} that
$\Sh(\mathcal{C})/\mathcal{F}$ is a topos. In fact, we recall the
proof. First we apply Lemma \ref{sites-lemma-topos-good-site}
...
$$
by Lemma \ref{sites-lemma-localize-given-products}.
...
Lemma \ref{sites-lemma-restrict-back}.
\end{proof}

\begin{definition}[Open subtopos]
\label{sites-definition-open-subtopos}
\source{stacks:08LX}
\href{https://stacks.math.columbia.edu/tag/08LX}{\texttt{Stacks tag 08LX}}
\group{sites}

\uses{sites-lemma-open-subtopos}
Let $\mathcal{C}$ be a site. A strictly full subcategory
...
$\Sh(\mathcal{C})/\mathcal{F}$ described in Lemma \ref{sites-lemma-open-subtopos}.
\end{definition}
...
\begin{lemma}[Closed subtopos]
\label{sites-lemma-closed-subtopos}
\source{stacks:08LY}
\href{https://stacks.math.columbia.edu/tag/08LY}{\texttt{Stacks tag 08LY}}
\group{sites}
...
\begin{proof}
\uses{sites-lemma-topos-good-site, sites-definition-site, sites-proposition-get-morphism}

We apply Lemma \ref{sites-lemma-topos-good-site} to see that we may assume
...
satisfy the conditions (1), (2), (3) of Definition \ref{sites-definition-site}.
...
\begin{definition}[Closed subtopos]
\label{sites-definition-closed-subtopos}
\source{stacks:08LZ}
\href{https://stacks.math.columbia.edu/tag/08LZ}{\texttt{Stacks tag 08LZ}}
\group{sites}

\uses{sites-lemma-closed-subtopos}
Let $\mathcal{C}$ be a site. A strictly full subcategory
$E \subset \Sh(\mathcal{C})$ is an {\it closed subtopos}
if there exists a subsheaf $\mathcal{F}$ of the final object
of $\Sh(\mathcal{C})$ such that $E$ is the subtopos
described in Lemma \ref{sites-lemma-closed-subtopos}.
\end{definition}

All right, and now we can define what it means to have a closed immersion
and an open immersion of topoi.

\begin{definition}[Immersion topoi]
\label{sites-definition-immersion-topoi}
\source{stacks:08M0}
\href{https://stacks.math.columbia.edu/tag/08M0}{\texttt{Stacks tag 08M0}}
\group{sites}
...
\source{stacks:08M1}
\href{https://stacks.math.columbia.edu/tag/08M1}{\texttt{Stacks tag 08M1}}
\group{sites}
...
\begin{proof}
\uses{sites-lemma-closed-subtopos, sites-lemma-localize-topos, sites-lemma-exactness-properties}
...
By Lemma \ref{sites-lemma-closed-subtopos}
...
be the localization functor (Lemma \ref{sites-lemma-localize-topos}).
...
Lemma \ref{sites-lemma-exactness-properties} and hence
...
Section \ref{sites-section-sheafification}, especially the proof of
Lemma \ref{sites-lemma-sheafification-exact})
and since $s$ commutes with filtered colimits.
\item[] ($\gamma$) We get the following commutative diagram
$$
\begin{array}{cc}\Sh(\mathcal{C}, \mathcal{A}) \longrightarrow\underset{s}{\downarrow} & \textit{PSh}(\mathcal{C}, \mathcal{A}) \overset{{}^\#}{\longleftarrow}\overset{s}{\downarrow} \\ \Sh(\mathcal{C}) \longrightarrow & \textit{PSh}(\mathcal{C}) \longleftarrow\end{array}
$$
...
The proof is the same as the proof of Lemma \ref{sites-lemma-sheafification-exact}.
\end{itemize}

\begin{definition}[Pushforward algebraic structures]
\label{sites-definition-pushforward-algebraic-structures}
\source{stacks:00YS}
\href{https://stacks.math.columbia.edu/tag/00YS}{\texttt{Stacks tag 00YS}}
\group{sites}
...
the functor $u_p$ in Section \ref{sites-section-functoriality-PSh}
...
\begin{lemma}[Push pull good case]
\label{sites-lemma-push-pull-good-case}
\source{stacks:00YT}
\href{https://stacks.math.columbia.edu/tag/00YT}{\texttt{Stacks tag 00YT}}
\group{sites}

\uses{sites-proposition-get-morphism}
Suppose the functor $u : \mathcal{C} \to \mathcal{D}$ satisfies
the hypotheses of Proposition \ref{sites-proposition-get-morphism},
...
\begin{proof}
\uses{sites-proposition-get-morphism, sites-lemma-adjoints-u}

We have defined $f_* = u^p$ above.
In the course of the proof of Proposition \ref{sites-proposition-get-morphism}
...
same way as the proof of Lemma \ref{sites-lemma-adjoints-u}.
...
singleton sheaf (see Example \ref{sites-example-singleton-sheaf})
to $\mathcal{F}$, and
...
\begin{proposition}[Morphisms of topoi preserve algebraic structure.]
\label{sites-proposition-functoriality-algebraic-structures-topoi}
\source{stacks:00YV}
\href{https://stacks.math.columbia.edu/tag/00YV}{\texttt{Stacks tag 00YV}}
\group{sites}
...
\begin{remark}[No pullback presheaves]
\label{sites-remark-no-pullback-presheaves}
\source{stacks:00YU}
\href{https://stacks.math.columbia.edu/tag/00YU}{\texttt{Stacks tag 00YU}}
\group{sites}
...
\source{stacks:06UN}
\href{https://stacks.math.columbia.edu/tag/06UN}{\texttt{Stacks tag 06UN}}
\group{sites}
...
\begin{lemma}[Compute global sections]
\label{sites-lemma-compute-global-sections}
\source{stacks:0792}
\href{https://stacks.math.columbia.edu/tag/0792}{\texttt{Stacks tag 0792}}
\group{sites}

Let $\mathcal{C}$ be a site. Let $a, b : V \to U$ be objects of $\mathcal{C}$
such that
$$
\begin{array}{ccc}h_V^\# \longrightarrow\longrightarrow & h_U^\# \longrightarrow & {*}\end{array}
$$
...
morphisms of topoi
$$
\begin{array}{ccc}\Sh(\mathcal{D}) \underset{h}{\searrow}\underset{f}{\longrightarrow} &  & \Sh(\mathcal{C}) \overset{g}{\swarrow} \\  & \Sh(\mathcal{B}) & \end{array}
$$
...
$$
\begin{array}{cc}\Sh(\mathcal{D}) \underset{h}{\downarrow}\underset{f}{\longrightarrow} & \Sh(\mathcal{C}) \overset{g}{\downarrow} \\ \Sh(\mathcal{B}) \overset{e}{\longrightarrow} & \Sh(\mathcal{A})\end{array}
$$
...
\source{stacks:0F6X}
\href{https://stacks.math.columbia.edu/tag/0F6X}{\texttt{Stacks tag 0F6X}}
\group{sites}

Consider a commutative diagram
$$
\begin{array}{cc}\Sh(\mathcal{B}') \underset{k}{\longrightarrow}\underset{f'}{\downarrow} & \Sh(\mathcal{B}) \overset{f}{\downarrow} \\ \Sh(\mathcal{C}') \overset{l}{\longrightarrow}\underset{g'}{\downarrow} & \Sh(\mathcal{C}) \overset{g}{\downarrow} \\ \Sh(\mathcal{D}') \overset{m}{\longrightarrow} & \Sh(\mathcal{D})\end{array}
$$
...
\source{stacks:0F6Y}
\href{https://stacks.math.columbia.edu/tag/0F6Y}{\texttt{Stacks tag 0F6Y}}
\group{sites}

Consider a commutative diagram
$$
\begin{array}{ccc}\Sh(\mathcal{C}'') \underset{g'}{\longrightarrow}\underset{f''}{\downarrow} & \Sh(\mathcal{C}') \underset{g}{\longrightarrow}\underset{f'}{\downarrow} & \Sh(\mathcal{C}) \overset{f}{\downarrow} \\ \Sh(\mathcal{D}'') \overset{h'}{\longrightarrow} & \Sh(\mathcal{D}') \overset{h}{\longrightarrow} & \Sh(\mathcal{D})\end{array}
$$
...
$u^p$ and $u_p$ from Section \ref{sites-section-functoriality-PSh} and
$u^s$ and $u_s$ from Section \ref{sites-section-continuous-functors}
is taken from \cite[pages 14 and 42]{ArtinTopologies}. Having made these
choices, the notation for the functor
${}_pu$ in Section \ref{sites-section-more-functoriality-PSh} and
${}_su$ in Section \ref{sites-section-cocontinuous-functors}
seems reasonable.
...
a continuous functor (Definition \ref{sites-definition-continuous}).
...
(Lemma \ref{sites-lemma-morphism-sites-topoi}) is the same as that in
...
Suppose that $\mathcal{C}$ and $\mathcal{D}$ are sites and that
$u : \mathcal{C} \to \mathcal{D}$ is
a cocontinuous functor (Definition \ref{sites-definition-cocontinuous}).
The functor ${}_su$ (Lemma \ref{sites-lemma-pu-sheaf}) is denoted $u_*$ in
...
\source{stacks:00YX}
\href{https://stacks.math.columbia.edu/tag/00YX}{\texttt{Stacks tag 00YX}}
\group{sites}
...
\begin{lemma}[Sieves set]
\label{sites-lemma-sieves-set}
\source{stacks:00YZ}
\href{https://stacks.math.columbia.edu/tag/00YZ}{\texttt{Stacks tag 00YZ}}
\group{sites}
...
define $S$ as the union of the images (Definition \ref{sites-definition-image})
...
\begin{definition}[Sieve generated]
\label{sites-definition-sieve-generated}
\source{stacks:00Z1}
\href{https://stacks.math.columbia.edu/tag/00Z1}{\texttt{Stacks tag 00Z1}}
\group{sites}

\uses{sites-lemma-sieves-set}
Let $\mathcal{C}$ be a category.
Given a family of morphisms $\{f_i : U_i \to U\}_{i\in I}$
of $\mathcal{C}$ with target $U$ we say the sieve
$S$ on $U$ described in Lemma \ref{sites-lemma-sieves-set}
part (\ref{item-sieve-generated}) is the {\it sieve  on $U$
generated by the morphisms $f_i$}.
\end{definition}

\begin{definition}[Pullback sieve]
\label{sites-definition-pullback-sieve}
\source{stacks:00Z2}
\href{https://stacks.math.columbia.edu/tag/00Z2}{\texttt{Stacks tag 00Z2}}
\group{sites}
...
\begin{lemma}[Pullback sieve section]
\label{sites-lemma-pullback-sieve-section}
\source{stacks:00Z3}
\href{https://stacks.math.columbia.edu/tag/00Z3}{\texttt{Stacks tag 00Z3}}
\group{sites}
...
\source{stacks:00Z4}
\href{https://stacks.math.columbia.edu/tag/00Z4}{\texttt{Stacks tag 00Z4}}
\group{sites}
...
\begin{lemma}[Topology basic]
\label{sites-lemma-topology-basic}
\source{stacks:00Z5}
\href{https://stacks.math.columbia.edu/tag/00Z5}{\texttt{Stacks tag 00Z5}}
\group{sites}
...
\begin{proof}
\uses{sites-definition-topology, sites-lemma-pullback-sieve-section}

Proof of (1). The intersection of the empty family follows from condition
(3) of Definition \ref{sites-definition-topology}.
Let $S, S' \in J(U)$. Consider $S'' = S \cap S'$. For every
$V \to U$ in $S(V)$ we have
$$
...
\medskip\noindent
Proof of (2). Let $S \in J(U)$ and $S' \supset S$. For every
$V \to U$ in $S(V)$ we have $S' \times_U V = h_V$ by
Lemma \ref{sites-lemma-pullback-sieve-section}. Thus
...
\source{stacks:00Z6}
\href{https://stacks.math.columbia.edu/tag/00Z6}{\texttt{Stacks tag 00Z6}}
\group{sites}
...
\begin{lemma}[Play with topologies]
\label{sites-lemma-play-with-topologies}
\source{stacks:00Z7}
\href{https://stacks.math.columbia.edu/tag/00Z7}{\texttt{Stacks tag 00Z7}}
\group{sites}
...
\begin{definition}[Sheaf sets topology]
\label{sites-definition-sheaf-sets-topology}
\source{stacks:00Z8}
\href{https://stacks.math.columbia.edu/tag/00Z8}{\texttt{Stacks tag 00Z8}}
\group{sites}
...
\begin{lemma}[Topology presheaves sheaves]
\label{sites-lemma-topology-presheaves-sheaves}
\source{stacks:00Z9}
\href{https://stacks.math.columbia.edu/tag/00Z9}{\texttt{Stacks tag 00Z9}}
\group{sites}
...
\begin{proof}
\uses{sites-lemma-play-with-topologies}
...
Lemma \ref{sites-lemma-play-with-topologies}.
...
of presheaves
$$
\begin{array}{cc}S' \times_U V \longrightarrow\downarrow & h_V \downarrow \\ S' \times_U W \longrightarrow & h_W\end{array}
$$
...
\begin{definition}[Canonical topology]
\label{sites-definition-canonical-topology}
\source{stacks:00ZA}
\href{https://stacks.math.columbia.edu/tag/00ZA}{\texttt{Stacks tag 00ZA}}
\group{sites}

\uses{sites-lemma-topology-presheaves-sheaves}
Let $\mathcal{C}$ be a category.
The finest topology on $\mathcal{C}$ such that
all representable presheaves are sheaves, see
Lemma \ref{sites-lemma-topology-presheaves-sheaves},
...
Lemma \ref{sites-lemma-refine-same-topology}.
...
see Theorem \ref{sites-theorem-topology-and-topos}.

\medskip\noindent
Accepting this for the moment it makes sense to study the
topology determined by a site.

\begin{lemma}[Site gives topology]
\label{sites-lemma-site-gives-topology}
\source{stacks:00ZC}
\href{https://stacks.math.columbia.edu/tag/00ZC}{\texttt{Stacks tag 00ZC}}
\group{sites}

\uses{sites-definition-sieve-generated, sites-definition-sheaf-sets-topology, sites-definition-sheaf-sets}
...
\ref{sites-definition-sieve-generated}) is contained in $S$.
\begin{enumerate}
\item This $J$ is a topology on $\mathcal{C}$.
\item A presheaf $\mathcal{F}$ is a sheaf for this topology
(see Definition \ref{sites-definition-sheaf-sets-topology})
if and only if it is a sheaf on the site (see
Definition \ref{sites-definition-sheaf-sets}).
\end{enumerate}
\end{lemma}

\begin{proof}
\uses{sites-definition-site, sites-definition-topology}
...
(Definition \ref{sites-definition-topology}). As an example we
...
\begin{definition}[Topology associated site]
\label{sites-definition-topology-associated-site}
\source{stacks:00ZD}
\href{https://stacks.math.columbia.edu/tag/00ZD}{\texttt{Stacks tag 00ZD}}
\group{sites}

\uses{sites-lemma-site-gives-topology}
Let $\mathcal{C}$ be a site with coverings $\text{Cov}(\mathcal{C})$.
The {\it topology associated to $\mathcal{C}$} is the topology
$J$ constructed in Lemma \ref{sites-lemma-site-gives-topology} above.
\end{definition}
...
are the same, by Lemma \ref{sites-lemma-site-gives-topology}.
...
\begin{remark}[Enlarge coverings]
\label{sites-remark-enlarge-coverings}
\source{stacks:00ZE}
\href{https://stacks.math.columbia.edu/tag/00ZE}{\texttt{Stacks tag 00ZE}}
\group{sites}

\uses{sites-definition-combinatorial-tautological}
...
\begin{remark}[Shrink coverings]
\label{sites-remark-shrink-coverings}
\source{stacks:00ZF}
\href{https://stacks.math.columbia.edu/tag/00ZF}{\texttt{Stacks tag 00ZF}}
\group{sites}

\uses{sites-definition-combinatorial-tautological, sites-definition-site}
...
equivalent (see Definition \ref{sites-definition-combinatorial-tautological})
...
as defined in Definition \ref{sites-definition-site}.
...
ordered by inclusion, see Lemma \ref{sites-lemma-sieves-set}. The transition
...
sieve $S \cap S'$ is a covering sieve too, see Lemma \ref{sites-lemma-sieves-set}.

\medskip\noindent
Let $f : V \to U$ be a morphism in $\mathcal{C}$.
Let $S \in J(U)$. There is a commutative diagram
$$
\begin{array}{cc}S \times_U V \longrightarrow\downarrow & h_V \downarrow \\ S \longrightarrow & h_U\end{array}
$$
...
\begin{lemma}[L presheaf]
\label{sites-lemma-L-presheaf}
\source{stacks:00ZH}
\href{https://stacks.math.columbia.edu/tag/00ZH}{\texttt{Stacks tag 00ZH}}
\group{sites}
...
\begin{definition}[Presheaf separated topology]
\label{sites-definition-presheaf-separated-topology}
\source{stacks:00ZI}
\href{https://stacks.math.columbia.edu/tag/00ZI}{\texttt{Stacks tag 00ZI}}
\group{sites}
...
\begin{theorem}[L topology]
\label{sites-theorem-L-topology}
\source{stacks:00ZJ}
\href{https://stacks.math.columbia.edu/tag/00ZJ}{\texttt{Stacks tag 00ZJ}}
\group{sites}
...
\begin{proof}
\uses{sites-definition-sheaf-sets-topology, sites-lemma-sieves-set, sites-lemma-pullback-sieve-section}

Part (3) is trivial from the definition of $L$ and
the definition of a sheaf (Definition \ref{sites-definition-sheaf-sets-topology}).
...
$S' \cap S$ which is also a covering sieve, see Lemma \ref{sites-lemma-sieves-set}.
Suppose $V\in \Ob(\mathcal{C})$, and
$\alpha : V \to U$ in $S(V)$.
Then we have $S \times_U V = h_V$,
see Lemma \ref{sites-lemma-pullback-sieve-section}. Thus the restrictions
...
\begin{definition}[Associated sheaf topology]
\label{sites-definition-associated-sheaf-topology}
\source{stacks:00ZL}
\href{https://stacks.math.columbia.edu/tag/00ZL}{\texttt{Stacks tag 00ZL}}
\group{sites}
...
\begin{proposition}[Sheafification adjoint topology]
\label{sites-proposition-sheafification-adjoint-topology}
\source{stacks:00ZM}
\href{https://stacks.math.columbia.edu/tag/00ZM}{\texttt{Stacks tag 00ZM}}
\group{sites}
...
Same as the proof of Proposition \ref{sites-proposition-sheafification-adjoint}.
\end{proof}

\section{Topologies and sheaves}
\label{sites-section-topology-and-sheaves}
\begin{lemma}[Sieve sheafification]
\label{sites-lemma-sieve-sheafification}
\source{stacks:00ZO}
\href{https://stacks.math.columbia.edu/tag/00ZO}{\texttt{Stacks tag 00ZO}}
\group{sites}
...
\begin{proof}
\uses{sites-lemma-L-presheaf}

First we make a couple of general remarks.
We will use that $S^\# = LLS$, and $h_U^\# = LLh_U$.
In particular, by Lemma \ref{sites-lemma-L-presheaf}, we see that
...
\begin{theorem}[Topology and topos]
\label{sites-theorem-topology-and-topos}
\source{stacks:00ZP}
\href{https://stacks.math.columbia.edu/tag/00ZP}{\texttt{Stacks tag 00ZP}}
\group{sites}
...
\begin{proof}
\uses{sites-lemma-sieve-sheafification}

It is a tautology that if $J = J'$ then the notions of sheaves
are the same. Conversely, Lemma \ref{sites-lemma-sieve-sheafification}
...
\begin{lemma}[Finer topology]
\label{sites-lemma-finer-topology}
\source{stacks:00ZQ}
\href{https://stacks.math.columbia.edu/tag/00ZQ}{\texttt{Stacks tag 00ZQ}}
\group{sites}

\uses{sites-theorem-topology-and-topos}
Assumption and notation as in Theorem \ref{sites-theorem-topology-and-topos}.
...
Lemma \ref{sites-lemma-sieve-sheafification}. We get a commutative
diagram
$$
\begin{array}{ccc}S \longrightarrow\downarrow & S^\# \longrightarrow\downarrow & S^{\#, \prime} \downarrow \\ h_U \longrightarrow & h_U^\# \longrightarrow & h_U^{\#, \prime}\end{array}
$$
...
Recall from Section \ref{sites-section-points} that given a functor
...
\begin{definition}[Point topology]
\label{sites-definition-point-topology}
\source{stacks:00ZT}
\href{https://stacks.math.columbia.edu/tag/00ZT}{\texttt{Stacks tag 00ZT}}
\group{sites}

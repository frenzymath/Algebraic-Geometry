...
The first chapter of the algebraic-stacks blueprint defines representability
by testing a morphism against representable presheaves and taking pullbacks.
Mathlib's `Functor.relativelyRepresentable` is exactly this construction.  We
keep source-facing abbreviations here and expose the basic closure results.
-/
...
/-- Scheme-specialized form of `morphismScheme_representable` (Stacks tag
02ZR). -/
...
/-- Composition of representable transformations is representable (Stacks tag
02ZS). -/
...
/-- Representability is preserved by a pullback of transformations (Stacks
tag 02ZT). -/

...
\begin{lemma}[Dimension of a prime-divisor stalk]
  ...
  \leanok
  \uses{def:isRegularInCodimOne_dvrStalk}
...
\begin{proof}
  \leanok
  \uses{def:primeDivisor_ofOpen, lem:primeDivisor_stalkIso}
  Extend a prime divisor of \(U\) to \(X\).  Its stalk in \(X\) is a DVR, and
  \cref{lem:primeDivisor_stalkIso} transports this property to the
...
\begin{proof}
  ...
  non-zero-divisor branch is \cref{lem:ord_ringEquiv}; the other branch is
...
\begin{lemma}[Naturality of fraction-field order]
  ...
  \leanok
  \uses{lem:ordMonoidWithZeroHom_ringEquiv}
...
\begin{proof}
  ...
  \cref{lem:ordMonoidWithZeroHom_ringEquiv} to numerator and denominator.
\end{proof}
...
\begin{proof}
  \leanok
  Apply \cref{lem:ordFrac_ringEquiv} to the stalk isomorphism and the
...
\begin{lemma}[Order is invariant under restriction to an open]
  ...
  \leanok
  \uses{def:order_at_point, def:functionFieldIso,
...
\begin{proof}
  ...
  \uses{lem:functionFieldIso_compat,
    lem:ordFrac_stalkIso_naturality}
  The compatibility square of \cref{lem:functionFieldIso_compat} supplies
  the hypothesis of \cref{lem:ordFrac_stalkIso_naturality}.
\end{proof}
...
\begin{proof}
  \leanok
  This is the total convention in \cref{def:order_at_point}.
\end{proof}
...
\begin{proof}
  \leanok
  This is \cref{lem:order_inv} applied to the underlying field element.
\end{proof}
...
\begin{lemma}[Order of a power]
  ...
  \leanok
  \uses{lem:order_one, lem:order_mul_of_ne_zero}
...
\begin{proof}
  \leanok
  Induct on \(n\), using \cref{lem:order_one} at zero and
  \cref{lem:order_mul_of_ne_zero} at the successor step.
\end{proof}
...
\begin{proof}
  ...
  \cref{def:divisor_closed_point} gives the asserted divisor.
\end{proof}
...
\begin{proof}
  \leanok
  Select the positive branch in \cref{def:divisor_closed_point}.
\end{proof}
...
\begin{proof}
  \leanok
  Select the negative branch in \cref{def:divisor_closed_point}.
\end{proof}
...
\begin{proof}
  \leanok
  \uses{thm:divisor_degree_hom}
  This is additivity in \cref{thm:divisor_degree_hom}.
\end{proof}
...
\begin{proof}
  \leanok
  \uses{thm:divisor_degree_hom}
  Apply the homomorphism of \cref{thm:divisor_degree_hom} to \(-D\).
\end{proof}
...
\begin{proof}
  \leanok
  \uses{lem:degree_add, lem:degree_neg}
  Combine \cref{lem:degree_add} and \cref{lem:degree_neg}.
\end{proof}
...
\begin{remark}
  ...
  \dcref{custom}
  \uses{lem:rationalMap_order_finite_support}
...
\begin{definition}[Principal divisor]
  ...
  \leanok
  \uses{def:codim1_cycles, def:order_at_point,
    lem:rationalMap_order_finite_support}
...
\begin{proof}
  \leanok
  \uses{lem:principal_apply, lem:order_one}
...
\begin{proof}
  \leanok
  \uses{lem:principal_apply, lem:order_mul_of_ne_zero, lem:order_one}
  Compare coefficients at every \(Y\).  The product formula for the order
  gives
  \(\operatorname{div}(fg)(Y)=\operatorname{div}(f)(Y)
    +\operatorname{div}(g)(Y)\), and \cref{lem:order_one} treats the
  identity.
\end{proof}
...
\begin{definition}[Pullback of a divisor along a finite morphism of curves]
  ...
  \dcref{custom}
  \uses{lem:curve_closed_point_prime_divisor, def:order_at_point}
...
\begin{theorem}[Degree of a finite pullback]
  ...
  \dcref{custom}
  \uses{def:finite_curve_divisor_pullback, def:divisor_degree}
...
\begin{lemma}[A rational function gives a finite map to the projective line]
  ...
  \dcref{custom}
  \uses{def:finite_curve_divisor_pullback, def:principal_divisor}
...
\end{theorem}
\begin{proof}
  \uses{lem:rational_function_finite_map_to_p1,
    thm:finite_curve_degree_pullback}
  If \(f\) is constant, its order at every point is zero.  Otherwise
  \cref{lem:rational_function_finite_map_to_p1} gives a finite morphism
  ...
  \[
    \deg(\operatorname{div}(f))
      ...
      =\deg(\varphi_f)(1-1)=0
  \]
  by \cref{thm:finite_curve_degree_pullback}.
\end{proof}
...
\begin{lemma}[A simple zero contributes positive degree]
  ...
  \leanok
  \uses{def:principal_divisor, def:WeilDivisor_positivePart,
    def:divisor_degree}
...
\begin{proof}
  \leanok
  \uses{lem:principal_apply, lem:degree_positivePart_eq_sum_max}
...
\begin{lemma}[Linear equivalence is an equivalence relation]
  ...
  \dcref{custom}
  \uses{def:linear_equivalence}
  Linear equivalence is reflexive, symmetric, and transitive.
\end{lemma}
\begin{proof}
  \uses{thm:principal_hom, lem:principal_one, lem:order_units_inv}
...
\begin{theorem}[Degree on the divisor class group of a curve]
  ...
  \dcref{custom}
  \uses{def:divisor_class_group, def:divisor_degree}
...
\end{theorem}
\begin{proof}
  \uses{thm:principal_deg_zero}
  By \cref{thm:principal_deg_zero}, the degree homomorphism vanishes on the

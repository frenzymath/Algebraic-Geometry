\section{Tensor structure on the relative Picard sheaf}
\label{mk-chap:Picard_TensorObjSubstrate}

\section{The substrate \(\Scheme.\mathtt{Modules}.\mathtt{tensorObj}\)}
\label{mk-sec:tensorobj_substrate}
\label{mk-sec:tensorobj_api_survey}

\begin{definition}[The substrate operation $\otimes$ on $\Scheme.\mathtt{Modules}\,X$]\label{mk-def:scheme_modules_tensorobj}
...
\begin{lemma}[Functoriality of $\otimes_X$]\label{mk-lem:scheme_modules_tensorobj_functoriality}
  ...
  \ref{mk-def:scheme_modules_tensorobj} extends to a bifunctor
  \[
  ...
  recorded in \ref{mk-rem:scheme_modules_monoidal_off_path}.
\end{lemma}
...
\begin{proof}
  ...
  \ref{mk-def:scheme_modules_tensorobj} respect the canonical morphisms on
  ...
  (\ref{mk-rem:scheme_modules_monoidal_off_path}).
\end{proof}

\begin{remark}[The Picard law needs only coherence isomorphisms]
  \label{mk-rem:scheme_modules_monoidal_off_path}
  \dcref{custom}
  \uses{mk-def:scheme_modules_tensorobj, mk-lem:scheme_modules_tensorobj_functoriality, mk-lem:whisker_of_W}
  The group law is defined on isomorphism classes, so it needs only the
  tensor bifunctor and the existence of associator, unit, braiding, and inverse
  isomorphisms.  A full symmetric monoidal structure on all
  \(\Scheme.\mathtt{Modules}\,X\) is therefore unnecessary.  The associator is
  obtained by gluing the affine tensor associators through
  \ref{mk-lem:tensorobj_restrict_iso}; unit and braiding use
  \ref{mk-lem:tensorobj_unit_iso} and \ref{mk-lem:tensorobj_comm_iso}.
\end{remark}

\section{The lift through \(\texttt{LineBundle.OnProduct}\)}
\label{mk-sec:tensorobj_onproduct_lift}

The consumer in chapter \ref{mk-chap:Picard_RelPicFunctor} works not with
...
\ref{mk-chap:Picard_LineBundlePullback}. This section records the
...
\ref{mk-lem:tensorobj_assoc_iso} (through the linchpin
\ref{mk-lem:tensorobj_restrict_iso}), unit and commutativity by the cheap
\(\mathtt{mapIso}\) pattern (\ref{mk-lem:tensorobj_unit_iso} and
\ref{mk-lem:tensorobj_comm_iso}),
...
\(\texttt{LineBundle.OnProduct}\) subtype (\ref{mk-lem:tensorobj_lift_onproduct}) and
the interaction with the pullback functor \(\pi_T^*\). The open-immersion
restriction-compatibility isomorphism \ref{mk-lem:tensorobj_restrict_iso} is on the
critical path precisely because it is consumed by the direct-gluing associator
\ref{mk-lem:tensorobj_assoc_iso}; it is \emph{not} tied to the superseded
whiskering-stability lemma \ref{mk-lem:islocallyinjective_whisker_of_W}, which belongs
to the off-path route~(e).

\begin{lemma}[Sectionwise lax-monoidal structure on \(\mathtt{restrictScalars}\,\varphi\)]\label{mk-lem:restrictscalars_laxmonoidal}
  ...
  \ref{mk-lem:tensorobj_restrict_iso}, whose proof identifies restriction along an
...
\begin{lemma}[Tensor product commutes with restriction along an open immersion]\label{mk-lem:tensorobj_restrict_iso}
  \lean{AlgebraicGeometry.Scheme.Modules.tensorObj_restrict_iso}
  \leanok
  \dcref{custom}

  \uses{mk-def:scheme_modules_tensorobj, mk-lem:restrictscalars_ringiso_tensorequiv}
...
\begin{proof}
  ...
  (\ref{mk-def:scheme_modules_tensorobj}). The pullback commutes with
  ...
  \ref{mk-lem:restrictscalars_ringiso_tensorequiv}
  ...
  \ref{mk-lem:restrictscalars_ringiso_tensorequiv} yields the asserted isomorphism.

  This presheaf-level adjunction is the presheaf analogue of the sheaf-level
  \(\mathtt{SheafOfModules.pushforwardPushforwardAdj}\); its construction uses the
  natural transformations \(\mathtt{pushforwardNatTrans}\) and
  \(\mathtt{pushforwardCongr}\), together with \(\mathtt{pushforwardId}\) and
  \(\mathtt{pushforwardComp}\).
  ...
  \ref{mk-lem:tensorobj_assoc_iso} consumes the comparison
  \((M \otimes_X N)|_f \cong M|_f \otimes_U N|_f\) on its sole consumer
  \ref{mk-lem:tensorobj_preserves_locally_trivial}, where the restriction is commuted
...
\begin{lemma}[Base change along a ring isomorphism commutes with $\otimes$]\label{mk-lem:restrictscalars_ringiso_tensorequiv}
  ...
  \ref{mk-lem:tensorobj_restrict_iso}, leaving the presheaf-level pushforward
  adjunction as the sole open residual there.
\end{lemma}

\begin{lemma}[Tensor product of locally-trivial modules is locally trivial]\label{mk-lem:tensorobj_preserves_locally_trivial}
  ...
  \ref{mk-def:scheme_modules_tensorobj} is again locally trivial of rank
...
Picard group law. The associator \ref{mk-lem:tensorobj_assoc_iso} is realized
\emph{unconditionally} through the varying-ring stalk--tensor commutation
\ref{mk-lem:islocallyinjective_whiskerleft_via_stalk}
(\ref{mk-sec:tensorobj_stalk_tensor}), which closes the single left-whiskering
...
\ref{mk-lem:whisker_of_W}, it is a closed lemma, not a stub to be skipped.)
...
\emph{existence} of the coherence isomorphisms (\ref{mk-lem:tensorobj_isoclass_commgroup}),
and the associator is realized as the sheafification transport of the presheaf
associator (\ref{mk-lem:tensorobj_assoc_iso}), whose single open left-whisker obligation
is closed by the d.2 route of \ref{mk-lem:islocallyinjective_whiskerleft_via_stalk}. The
...
[Sheafification inverts a localizer morphism of presheaves of modules]
  \label{mk-lem:isiso_sheafification_map_of_W}
...
\label{mk-sec:tensorobj_route_e}
...
(\ref{mk-lem:tensorobj_isoclass_commgroup}, built by hand as the scheme-theoretic
...
transport of the presheaf associator (\ref{mk-lem:tensorobj_assoc_iso}), whose
\emph{single} open left-whisker obligation is closed by the d.2 stalk--tensor
commutation (\ref{mk-lem:islocallyinjective_whiskerleft_via_stalk} and
\ref{mk-lem:stalk_tensor_commutation}); it uses \emph{no} \emph{full}
...
section --- the \emph{pair} of whiskering-stability fields (\ref{mk-lem:whisker_of_W})
...
(\ref{mk-sec:tensorobj_api_survey}): apply
...
\ref{mk-sec:tensorobj_stalk_tensor} (\ref{mk-lem:stalk_tensor_commutation} and
\ref{mk-lem:islocallyinjective_whiskerleft_via_stalk}) and is therefore a live ingredient
of the unconditional associator route.

\begin{lemma}

[The $R.\mathtt{stalk}\,x$-linear stalk map of a morphism of presheaves of modules]
  \label{mk-lem:stalk_linear_map}
  ...
  \ref{mk-lem:stalk_tensor_commutation} and
  \ref{mk-lem:islocallyinjective_whiskerleft_via_stalk}. They build on the
...
(\ref{mk-lem:tensorobj_isoclass_commgroup}). The associator instead arises as the
sheafification transport whose single open left-whisker obligation is closed by the
live lemma \ref{mk-lem:islocallyinjective_whiskerleft_via_stalk}. The stalk ingredients
(d.1)/(d.2) themselves --- the \(R_x\)-linear stalk map \ref{mk-lem:stalk_linear_map}
and the stalk--tensor commutation \ref{mk-lem:stalk_tensor_commutation} --- are
...
[Local injectivity of a whiskered localizer morphism (stalkwise, flatness-free)]
  \label{mk-lem:islocallyinjective_whisker_of_W}
  \lean{PresheafOfModules.isLocallyInjective_whiskerLeft_of_W}
  \leanok
  \dcref{custom}

  \uses{mk-def:scheme_modules_tensorobj, mk-lem:stalk_linear_map, mk-lem:tensorobj_restrict_iso, mk-lem:tensorobj_unit_iso}
  ...
  \ref{mk-lem:islocallyinjective_whiskerleft_via_stalk}, which the associator's
  sheafification transport consumes and which is closed \emph{unconditionally} by the
  d.2 stalk--tensor commutation (\ref{mk-lem:stalk_tensor_commutation}); the d.1/d.2
...
\emph{PRIMARY route (\(F\) locally trivial --- the consumer's case; sheaf-level,
  ...
  sole consumer --- the associator \ref{mk-lem:tensorobj_assoc_iso} --- supplies.
  ...
  On such a \(V\):
  \begin{itemize}
    \item the substrate--restriction compatibility \ref{mk-lem:tensorobj_restrict_iso}
    ...
    \item the trivialisation \(F|_V \cong \mathcal{O}_V\) together with the left
      unitor \ref{mk-lem:tensorobj_unit_iso} (\(\mathcal{O}_V \otimes_V (-) \cong
  ...
  single comparison lemma \ref{mk-lem:tensorobj_restrict_iso} (a presheaf-level
  ...
  (\ref{mk-sec:tensorobj_api_survey}), ported to \(\mathtt{PresheafOfModules}\). A
  ...
  \ref{mk-lem:tensorobj_restrict_iso}.
\end{proof}
...
isomorphisms (\ref{mk-lem:tensorobj_assoc_iso}, via \ref{mk-lem:tensorobj_restrict_iso}).
...
[Flatness-free whiskering preserves the sheafification localizer]
  \label{mk-lem:whisker_of_W}
  \lean{PresheafOfModules.W_whiskerLeft_of_W}
  \leanok
  \dcref{custom}

  \uses{mk-def:scheme_modules_tensorobj, mk-lem:islocallyinjective_whisker_of_W}
  With notation as in \ref{mk-lem:islocallyinjective_whisker_of_W}: for an
  ...
  \(\mathtt{MorphismProperty.IsMonoidal}\) (\ref{mk-sec:tensorobj_api_survey}) for
...
\begin{proof}
  \leanok
  \uses{mk-lem:islocallyinjective_whisker_of_W}
  ...
  injectivity} is \ref{mk-lem:islocallyinjective_whisker_of_W}. The two together
...
braiding isomorphisms (\ref{mk-lem:tensorobj_isoclass_commgroup}, assembled by hand
rather than off a coherent monoidal category's \(\mathtt{Skeleton}\)), and the
associator is now built directly by
gluing (\ref{mk-lem:tensorobj_assoc_iso}, via \ref{mk-lem:tensorobj_restrict_iso}). This
block is retained for the historical record only and must \emph{not} be formalized.

\begin{lemma}[Associator for $\otimes_X$ (unconditional)]\label{mk-lem:tensorobj_assoc_iso}
  \lean{AlgebraicGeometry.Scheme.Modules.tensorObj_assoc_iso}
  \leanok
  \dcref{custom}

  \uses{mk-def:scheme_modules_tensorobj, mk-lem:islocallyinjective_whiskerleft_via_stalk, mk-lem:isiso_sheafification_map_of_W}
  Let \(X\) be a scheme and let
  ...
  \(\otimes\)-iso-classes (\ref{mk-lem:tensorobj_isoclass_commgroup}) consumes only
...
\begin{proof}
  \uses{mk-def:scheme_modules_tensorobj, mk-lem:islocallyinjective_whiskerleft_via_stalk}
  ...
  \ref{mk-lem:islocallyinjective_whiskerleft_via_stalk} (\ref{mk-sec:tensorobj_stalk_tensor}):
  ...
  unconditional once d.2 (\ref{mk-lem:stalk_tensor_commutation}) is
  ...
  restriction-compatibility comparison \ref{mk-lem:tensorobj_restrict_iso}.

  Concretely, the same isomorphism is described by gluing canonical local isomorphisms
  through the restriction-compatibility comparison \ref{mk-lem:tensorobj_restrict_iso}.
  Fix a
  ...
  \begin{itemize}
    \item the first arrow is two applications of the restriction-compatibility
      isomorphism \ref{mk-lem:tensorobj_restrict_iso} along the open immersion
    ...
    \item the middle arrow is the \emph{canonical presheaf-level associator}
      ...
      \ref{mk-sec:tensorobj_api_survey}(i)), read on the restricted modules;
    \item the third arrow is again two applications of
      \ref{mk-lem:tensorobj_restrict_iso}, in the opposite direction.
  \end{itemize}
  Each of the three arrows is a \emph{canonical} isomorphism: the
  restriction-compatibility comparison \ref{mk-lem:tensorobj_restrict_iso} is induced
  ...
  \ref{mk-lem:tensorobj_restrict_iso} and of the presheaf associator, so the two local
  ...
  restriction-compatibility comparison \ref{mk-lem:tensorobj_restrict_iso}; the presheaf
  associator (\ref{mk-sec:tensorobj_api_survey}(i)) and the sheaf-of-modules
  internal-hom gluing are already available.

  Note that, unlike the pointwise-existence statement
  \ref{mk-lem:tensorobj_preserves_locally_trivial} --- a \emph{proposition} proved
  ...
  are not consumed; the comparison \ref{mk-lem:tensorobj_restrict_iso} is applied to the
  ...
  (\ref{mk-lem:whisker_of_W}): only the single left-whisker obligation
  \ref{mk-lem:islocallyinjective_whiskerleft_via_stalk}, closed via the d.2 stalk--tensor
  commutation \ref{mk-lem:stalk_tensor_commutation}, is consumed by the sheafification
...
[Left and right unitors for $\otimes_X$]
  \label{mk-lem:tensorobj_unit_iso}
...
\begin{lemma}[Braiding for $\otimes_X$]\label{mk-lem:tensorobj_comm_iso}
...
\begin{lemma}[Inverse of an invertible module]\label{mk-lem:tensorobj_inverse_invertible}
  \dcref{01CR,custom}

  \uses{mk-def:scheme_modules_tensorobj, mk-lem:tensorobj_preserves_locally_trivial, mk-lem:tensorobj_restrict_iso}
  \dcref{custom}
...
\begin{proof}
  \uses{mk-lem:tensorobj_preserves_locally_trivial, mk-lem:tensorobj_restrict_iso, mk-lem:tensorobj_unit_iso, mk-lem:internal_hom_eval, mk-lem:internal_hom_isSheaf, mk-lem:dual_isLocallyTrivial, mk-lem:dual_restrict_iso, mk-lem:sheafofmodules_hom_of_local_compat, mk-lem:hom_of_local_compat_restrict, mk-lem:isiso_of_isiso_restrict, mk-lem:trivialisation_restrict_compat, mk-lem:tensorobj_unit_self_duality_collapse}
  The dual object and its evaluation are now supplied by the internal-hom
  infrastructure of \ref{mk-sec:tensorobj_dual_infra}. Take
  \(L^{-1} := \mathcal{H}om_{\mathcal{O}_X}(L, \mathcal{O}_X)\) to be the
  sheaf-level dual \ref{mk-lem:internal_hom_isSheaf} --- a genuine object of
  \(\Scheme.\mathtt{Modules}\,X\), assembled from the presheaf internal hom
  \ref{mk-def:presheaf_internal_hom} by sheafification --- and let
  \(\varepsilon_L : L \otimes_X L^{-1} \to \mathcal{O}_X\) be the evaluation
  (contraction) morphism \ref{mk-lem:internal_hom_eval}. With both in hand the
  ...
  \ref{mk-lem:dual_isLocallyTrivial}.
  ...
  open \(U\), the restriction-compatibility isomorphism \ref{mk-lem:tensorobj_restrict_iso}
  ...
  \ref{mk-lem:tensorobj_unit_iso} (\(\mathcal{O}_U \otimes_U (-) \cong (-)\)) and hence an
  ...
  \ref{mk-lem:tensorobj_restrict_iso} and of the unitor under further restriction. As
  ...
  local-triviality of \(L^{-1}\) (Step~1) is \ref{mk-lem:dual_isLocallyTrivial}, the
  dual object and its evaluation are \ref{mk-lem:internal_hom_isSheaf} and
  \ref{mk-lem:internal_hom_eval}, and the local-iso\(\Rightarrow\)global-iso passage
  (Step~3) is the already-closed \ref{mk-lem:tensorobj_restrict_iso}.

  \emph{Formal dependency.} The proof of
  \(\mathtt{exists\_tensorObj\_inverse}\) is \emph{not} a one-line consequence: it is
  the gluing argument \ref{mk-rem:dual_discharges_inverse}, which assembles the global
  ...
  \ref{mk-lem:sheafofmodules_hom_of_local_compat} together with its restriction-value
  connector \ref{mk-lem:hom_of_local_compat_restrict}; the local-iso\(\Rightarrow\)global-iso
  passage \ref{mk-lem:isiso_of_isiso_restrict}; the overlap cocycle equality, whose two
  hardest constituents are the trivialisation restriction-compatibility chain
  \ref{mk-lem:trivialisation_restrict_compat} and the unit self-duality collapse
  \ref{mk-lem:tensorobj_unit_self_duality_collapse}. The dual restriction commutation
  \ref{mk-lem:dual_restrict_iso} is one input among several. All of these inputs are
  established, including the restriction-naturality of the trivialisation chain
  (\ref{mk-lem:trivialisation_restrict_compat}), itself a chain of per-constituent naturality
  squares.
\end{proof}

\begin{lemma}[Closure of \(\otimes\) on the locally-trivial carrier \(\texttt{LineBundle.OnProduct}\)]\label{mk-lem:tensorobj_lift_onproduct}
  \lean{AlgebraicGeometry.Scheme.Modules.tensorObjOnProduct}
  \leanok
  \dcref{custom}

  \uses{mk-def:scheme_modules_tensorobj, mk-lem:tensorobj_preserves_locally_trivial}
  ...
  \]
  introduced in \ref{mk-chap:Picard_LineBundlePullback}. The bifunctor
  \(\otimes_{C \times_S T}\) of
  \ref{mk-lem:scheme_modules_tensorobj_functoriality} restricts to this subtype:
...
\begin{proof}
  \leanok
  \uses{mk-lem:tensorobj_preserves_locally_trivial}
  Closure is exactly
  \ref{mk-lem:tensorobj_preserves_locally_trivial}: the tensor product of two
  locally-trivial rank-one modules is again locally trivial of rank one
  (\(\mathtt{tensorObj\_isLocallyTrivial}\)). The construction
  ...
  \(\mathtt{IsInvertible}\) of \ref{mk-def:scheme_modules_isinvertible} is a
  ...
  \ref{mk-lem:tensorobj_isoclass_commgroup}, not this lemma.)
\end{proof}

\section{The varying-ring stalk--tensor commutation (d.2) and the unconditional
left-whiskering}
\label{mk-sec:tensorobj_stalk_tensor}


\begin{lemma}[Stalk of a tensor product (d.2): varying-ring stalk--tensor commutation]\label{mk-lem:stalk_tensor_commutation}
  \dcref{custom}

  \uses{mk-def:scheme_modules_tensorobj, mk-lem:stalk_linear_map}
...
\begin{proof}
  ...
  \ref{mk-lem:stalk_linear_map}). By the universal property of the relative tensor
  ...
  for the d.1 stalk map \(\mathtt{stalkLinearMap}\) (\ref{mk-lem:stalk_linear_map}).
  ...
  map \(R(W) \to R_x\) (the corresponding lemma \(\mathtt{germ\_smul}\)) — the same germ--scalar
...
naturality} of the d.2 commutation isomorphism \ref{mk-lem:stalk_tensor_commutation}.
We state both before the lemma that consumes them.

\begin{lemma}[Unconditional left-whiskering of the localizer, via d.2]\label{mk-lem:islocallyinjective_whiskerleft_via_stalk}
  \lean{PresheafOfModules.isLocallyInjective_whiskerLeft_of_W}
  \leanok
  \dcref{custom}

  \uses{mk-def:scheme_modules_tensorobj, mk-lem:stalk_tensor_commutation, mk-lem:stalk_linear_map}
  ...
  \ref{mk-lem:islocallyinjective_whisker_of_W} that the unconditional associator
  \ref{mk-lem:tensorobj_assoc_iso} consumes.
\end{lemma}
\begin{proof}
  \leanok
  \uses{mk-lem:stalk_tensor_commutation, mk-lem:stalk_linear_map}
  ...
  is an isomorphism at every point \(x \in X\). By \ref{mk-lem:stalk_linear_map} this
  ...
  isomorphism \(\mathtt{stalkTensorIso}\) of \ref{mk-lem:stalk_tensor_commutation}
  ...
  \ref{mk-lem:tensorobj_assoc_iso} is realized.
\end{proof}

\subsection{Stalk--tensor comparison helper declarations}
\label{mk-subsec:tensorobj_stalktensor_helpers}


\begin{definition}
[Forward stalk--tensor comparison map and its \(R_x\)-linear packaging]
  \label{mk-def:stalk_tensor_desc}
  \dcref{custom}

  \uses{mk-def:scheme_modules_tensorobj, mk-lem:stalk_linear_map, mk-def:rev_bihom}
  \textit{Source: \cite{stacks-project}, Modules on Ringed Spaces,
  Lemma~\texttt{lemma-stalk-tensor-product}.}
  The forward half of the stalk--tensor commutation \ref{mk-lem:stalk_tensor_commutation}
  ...
  \(R_x\)-balanced dual bilinear map (\ref{mk-def:rev_bihom}) is the candidate inverse
  \(\mathtt{stalkTensorRev}\). Together these are the underlying maps of the isomorphism
  \ref{mk-lem:stalk_tensor_commutation}.
\end{definition}
\begin{proof}
  \uses{mk-def:scheme_modules_tensorobj, mk-lem:stalk_linear_map, mk-def:rev_bihom}
 \(\mathtt{stalkTensorBilin}\) is bilinear and
...
[The dual (co)evaluation bihom of the reverse stalk--tensor map]
  ...
  reverse map \(\mathtt{stalkTensorRev}\) of \ref{mk-def:stalk_tensor_desc}. For a fixed
...
\begin{proof}
  \uses{mk-def:scheme_modules_tensorobj, mk-lem:stalk_linear_map}
 Additivity of each leg is checked on germ generators via
...
restriction-compatibility isomorphism \ref{mk-lem:tensorobj_restrict_iso} nor a
separate tensor-inverse lemma is needed there. Under the PRIMARY locally-trivial
route, by contrast, both \ref{mk-lem:tensorobj_restrict_iso} (via the PRIMARY proof
of \ref{mk-lem:islocallyinjective_whisker_of_W}) and the tensor-inverse lemma
\ref{mk-lem:tensorobj_inverse_invertible} (supplying inverses to
\ref{mk-lem:tensorobj_isoclass_commgroup}) are on the critical path.

\begin{definition}[\(\otimes\)-invertibility of an \(\mathcal{O}_X\)-module]\label{mk-def:scheme_modules_isinvertible}
  \dcref{01CR,custom}
  ...
  \(\otimes_X\) the substrate of \ref{mk-def:scheme_modules_tensorobj}. The
...
\begin{lemma}[The unit object is \(\otimes\)-invertible]\label{mk-lem:unit_isinvertible}
  \dcref{custom}

  \uses{mk-def:scheme_modules_isinvertible, mk-def:scheme_modules_tensorobj}
  ...
  criterion \ref{mk-def:scheme_modules_isinvertible} is met. This is the invertibility of
  the identity element of the relative Picard group law.
\end{lemma}
\begin{proof}
  \uses{mk-def:scheme_modules_isinvertible, mk-lem:tensorobj_unit_iso}
 The witness is \(N = \mathcal{O}_X\) with the isomorphism
  \(\mathcal{O}_X \otimes_X \mathcal{O}_X \cong \mathcal{O}_X\) supplied by the substrate
  unitor \ref{mk-lem:tensorobj_unit_iso}.
\end{proof}

\begin{lemma}[The commutative monoid of \(\otimes\)-iso-classes and its units]
  \label{mk-lem:tensorobj_isoclass_commgroup}
  \dcref{01CR,custom}

  \uses{mk-def:scheme_modules_tensorobj, mk-def:scheme_modules_isinvertible, mk-lem:scheme_modules_tensorobj_functoriality, mk-lem:tensorobj_lift_onproduct, mk-lem:tensorobj_inverse_invertible, mk-lem:tensorobj_assoc_iso, mk-lem:tensorobj_unit_iso, mk-lem:tensorobj_comm_iso, mk-def:tensorobjisoofiso, mk-def:tensorobj_unit_self_iso, mk-def:tensorobj_middlefour, mk-def:pic_setoid, mk-def:pic_mul, mk-def:pic_inv}
  \dcref{custom}
  \textit{Source: \cite{stacks-project}, Tag 01CR (Modules on Ringed Spaces,
  ...
  (\ref{mk-def:scheme_modules_isinvertible}) form a commutative monoid under the
  ...
  (\ref{mk-lem:scheme_modules_tensorobj_functoriality}) and again locally trivial by
  the closure lemma \ref{mk-lem:tensorobj_lift_onproduct}; the identity is
  \([\mathcal{O}_X]\); and the inverse of \([L]\) is the class of the dual
  \([L^{-1}]\) supplied by \ref{mk-lem:tensorobj_inverse_invertible} (with
  ...
  \]
  where the unit laws are the unitors \ref{mk-lem:tensorobj_unit_iso}, associativity is
  the associator \ref{mk-lem:tensorobj_assoc_iso}, and commutativity is the braiding
  \ref{mk-lem:tensorobj_comm_iso}, each consumed only as the proposition
  ...
  (the unitors \ref{mk-lem:tensorobj_unit_iso}, the braiding
  \ref{mk-lem:tensorobj_comm_iso}) or locally-trivial-scoped and assembled (the
  associator \ref{mk-lem:tensorobj_assoc_iso}). The only remaining obligations on this
  critical path are the restriction-compatibility comparison
  \ref{mk-lem:tensorobj_restrict_iso} and the
  dual/inverse \ref{mk-lem:tensorobj_inverse_invertible}. The associator
  \ref{mk-lem:tensorobj_assoc_iso} is supplied by the unconditional route of
  \ref{mk-sec:tensorobj_stalk_tensor} and transitively invokes the d.2 stalk--tensor
  commutation \ref{mk-lem:stalk_tensor_commutation}.
...
\begin{proof}
  ...
  bifunctoriality of \(\otimes_X\)
  (\ref{mk-lem:scheme_modules_tensorobj_functoriality}) --- an isomorphism in either
  argument tensors to an isomorphism --- and its value is again locally trivial by
  the closure lemma \ref{mk-lem:tensorobj_lift_onproduct}, so the operation closes on
  ...
  \]
  associativity is \ref{mk-lem:tensorobj_assoc_iso}, the unit laws are the unitors
  \ref{mk-lem:tensorobj_unit_iso}, and commutativity is the braiding
  \ref{mk-lem:tensorobj_comm_iso}. No pentagon, triangle or hexagon identity is
  ...
  \ref{mk-lem:tensorobj_inverse_invertible} satisfies \(L \otimes_X L^{-1} \cong
  \mathcal{O}_X\), so \([L^{-1}]\) is a two-sided inverse of \([L]\); equivalently,
  \([L]\) is a unit precisely when \(L\) is \(\otimes\)-invertible
  (\ref{mk-def:scheme_modules_isinvertible}), the inverse witness carried
...
\label{mk-sec:tensorobj_pullback_monoidality}

The relative Picard consumer (\ref{mk-chap:Picard_RelPicFunctor}) carries its line
bundles on the \emph{local-triviality} predicate \(\mathtt{IsLocallyTrivial}\)
(\ref{mk-def:IsLocallyTrivial}): its carrier \(\mathtt{OnProduct}\) is the
...
(\ref{mk-lem:pullback_tensor_iso_loctriv}); the invertibility corollary
...
\ref{mk-lem:tensorobj_restrict_iso}, and not a corollary of it. That lemma handles
...
(\ref{mk-lem:pullback_tensor_iso_loctriv}). The second is that pullback preserves the
structure sheaf, \(f^*\mathcal{O}_X \cong \mathcal{O}_Y\)
(\ref{mk-lem:pullback_unit_iso}, already in hand). Given a locally-trivial inverse
...
\ref{mk-lem:pullback_tensor_iso_loctriv} is the load-bearing input.
...
local triviality} (\ref{mk-lem:IsLocallyTrivial_pullback}): if \(M\) is trivialised
...
free (\ref{mk-lem:pullback_tensor_map}); the entire remaining content is the
...
\mathtt{pullbackTensorMap}\) (\ref{mk-lem:pullback_tensor_map}) to an isomorphism
through a chart-chase --- the same shape as the proven local-triviality results
\ref{mk-lem:IsLocallyTrivial_pullback} and
\ref{mk-lem:tensorobj_preserves_locally_trivial}, and \emph{not} via any concrete
...
\begin{itemize}
  \item[(D1$'$)] \emph{Naturality of \(\delta\) in \((M,N)\).} The sheaf-level
    ...
    (\ref{mk-lem:pullback_tensor_map_natural}). This naturality is what lets a chart
  ...
  \item[(D2$'$)] \emph{\(\delta\) on the unit pair \((\mathcal{O},\mathcal{O})\) is an
    ...
    (\ref{mk-lem:pullback_unit_iso}, an isomorphism for \emph{all} \(f\)); hence
    \(\delta_{(\mathcal{O},\mathcal{O})}\) is an isomorphism
    (\ref{mk-lem:pullback_tensor_iso_unit}).
  \item[(D3$'$)] \emph{Composition coherence of \(\delta\)} (the genuinely new
    ...
    \(\mathtt{pullbackComp}\,h\,f\) (\ref{mk-lem:pullback_tensor_map_basechange}).
    ...
    \ref{mk-lem:pullbackObjUnitToUnit_comp} does \emph{not} transfer; the coherence is
  ...
  \item[(D4$'$)] \emph{Chart-chase assembly.} Choose a common affine cover \(\{U_i\}\)
    ...
    \ref{mk-lem:tensorobj_preserves_locally_trivial}). On each \(f^{-1}U_i\), the
    ...
    \ref{mk-lem:pullback_tensor_map_isiso_open_immersion} (K1) --- aligns the restriction with
    ...
    \ref{mk-lem:isiso_of_isiso_restrict} promotes it to a global isomorphism. The
    structure mirrors the chase of \ref{mk-lem:IsLocallyTrivial_pullback}.
\end{itemize}
...
open-immersion comparison \ref{mk-lem:pullback_tensor_map_isiso_open_immersion}, whose
...
machinery (\ref{mk-lem:tensorobj_preserves_locally_trivial},
\ref{mk-lem:isiso_of_isiso_restrict}). No concrete inverse-image model, no
filtered-colimit/\(\otimes\) interchange, and no general comparison for arbitrary
modules is constructed.

\begin{lemma}[The presheaf pushforward is lax monoidal]\label{mk-lem:presheaf_pushforward_laxmonoidal}
...
\begin{lemma}[The presheaf pullback is oplax monoidal; the comparison map $\delta$]\label{mk-lem:presheaf_pullback_oplaxmonoidal}
  \lean{AlgebraicGeometry.Scheme.Modules.presheafPullbackOplaxMonoidal}
  \leanok
  \dcref{custom}

  \uses{mk-lem:presheaf_pushforward_laxmonoidal}
  ...
  \ref{mk-lem:presheaf_pushforward_laxmonoidal} --- is \emph{oplax monoidal}. In
  ...
  is exactly what the chart-chase of \ref{mk-lem:pullback_tensor_iso_loctriv}
...
\begin{proof}
  ...
  which is lax monoidal by \ref{mk-lem:presheaf_pushforward_laxmonoidal}. Mathlib's
...
\begin{lemma}[The sheaf-level pullback--tensor comparison map $\delta_{\mathrm{sheaf}}$]\label{mk-lem:pullback_tensor_map}
  \lean{AlgebraicGeometry.Scheme.Modules.pullbackTensorMap}
  \leanok
  \dcref{custom}

  \uses{mk-lem:presheaf_pullback_oplaxmonoidal, mk-def:scheme_modules_tensorobj, mk-def:sheafify_tensor_unit_iso, mk-def:pullback_val_iso}
...
\begin{proof}
  \leanok
  \uses{mk-lem:presheaf_pullback_oplaxmonoidal, mk-def:scheme_modules_tensorobj, mk-def:sheafify_tensor_unit_iso, mk-def:pullback_val_iso}
  The presheaf pullback is oplax monoidal (\ref{mk-lem:presheaf_pullback_oplaxmonoidal}),
  ...
  \ref{mk-lem:tensorobj_restrict_iso}: the abstract pullback of \(\mathcal{O}\)-modules
...
\begin{lemma}[Reduction of $\mathtt{pullbackTensorMap}$ iso-ness to the sheafified presheaf $\delta$]\label{mk-lem:isiso_pullbacktensormap_of_sheafifydelta}
  \lean{AlgebraicGeometry.Scheme.Modules.isIso_pullbackTensorMap_of_isIso_sheafifyDelta}
  \leanok
  \dcref{custom}

  \uses{mk-lem:pullback_tensor_map, mk-lem:isiso_sheafify_tensorhom_pullbackvaliso}
  Let \(f : Y \to X\) be a morphism of schemes and let
  ...
  of \ref{mk-lem:pullback_tensor_map} is an isomorphism.
\end{lemma}

\begin{proof}
  \leanok
  \uses{mk-lem:pullback_tensor_map, mk-lem:isiso_sheafify_tensorhom_pullbackvaliso}
  Unfolding \(\mathtt{pullbackTensorMap}\) (\ref{mk-lem:pullback_tensor_map}) exhibits it as a
  four-fold composite
  \[
...
\begin{lemma}[Composition coherence of the pushforward unit comparison]\label{mk-lem:unitToPushforwardObjUnit_comp}
  ...
  the pullback-side coherence \ref{mk-lem:pullbackObjUnitToUnit_comp}.
\end{lemma}
...
\begin{lemma}[Composition coherence of the pullback unit comparison]\label{mk-lem:pullbackObjUnitToUnit_comp}
  \lean{AlgebraicGeometry.Scheme.Modules.pullbackObjUnitToUnit_comp}
  \leanok
  \dcref{custom}

  \uses{mk-lem:unitToPushforwardObjUnit_comp}
  Let \(h : Z \to Y\) and \(f : Y \to X\) be composable morphisms of schemes. Write
  ...
  for \ref{mk-lem:pullback_unit_iso}: the left-adjoint pullback is defined abstractly, with
...
\begin{proof}
  ...
  (\ref{mk-lem:unitToPushforwardObjUnit_comp}) across the pullback--pushforward adjunction,
  ...
  \ref{mk-lem:unitToPushforwardObjUnit_comp}, namely
  \(\mathrm{upu}\,f \mathbin{;} f_*(\mathrm{upu}\,h) \mathbin{;}
  (\mathtt{pushforwardComp}\,h\,f).\mathrm{hom}\). Thus the two transposed sides coincide
  by \ref{mk-lem:unitToPushforwardObjUnit_comp}, and the original identity follows by the
  injectivity of the transpose.
\end{proof}

\begin{lemma}[Pullback preserves the structure sheaf]\label{mk-lem:pullback_unit_iso}
  \lean{AlgebraicGeometry.Scheme.Modules.pullbackUnitIso}
  \leanok
  \dcref{custom}

  \uses{mk-def:scheme_modules_tensorobj, mk-def:pullback_obj_unit_to_unit_iso}
...
\begin{lemma}[Naturality of the sheaf-level pullback--tensor comparison $\delta_{\mathrm{sheaf}}$ (D1$'$)]\label{mk-lem:pullback_tensor_map_natural}
  \lean{AlgebraicGeometry.Scheme.Modules.pullbackTensorMap_natural}
  \leanok
  \dcref{custom}

  \uses{mk-lem:pullback_tensor_map, mk-lem:presheaf_pullback_oplaxmonoidal, mk-lem:sheafify_tensor_unit_iso_natural, mk-lem:pullback_val_iso_natural}
  Let \(f : Y \to X\) be a morphism of schemes. The sheaf-level comparison map
  \(\delta_{\mathrm{sheaf}} = \mathtt{pullbackTensorMap}\) of
  \ref{mk-lem:pullback_tensor_map} is \emph{natural} in both module arguments: for
...
\begin{proof}
  \leanok
  \uses{mk-lem:pullback_tensor_map, mk-lem:presheaf_pullback_oplaxmonoidal, mk-lem:sheafify_tensor_unit_iso_natural, mk-lem:pullback_val_iso_natural}
  The map \(\delta_{\mathrm{sheaf}}\) is built (\ref{mk-lem:pullback_tensor_map}) by
  transporting the abstract oplax comparison \(\delta\) of the presheaf pullback
  (\ref{mk-lem:presheaf_pullback_oplaxmonoidal}) through the sheafification
  ...
  \(\mathtt{sheafifyTensorUnitIso}\) (Square 3, \ref{mk-lem:sheafify_tensor_unit_iso_natural}),
  and the \(\mathtt{pullbackValIso}\) reconciliation (Square 4,
  \ref{mk-lem:pullback_val_iso_natural}). Finally, the tensor of the two component morphisms
...
\begin{lemma}[Section-level naturality of the tensor/unit reconciliation \(\mathtt{sheafifyTensorUnitIso}\)]\label{mk-lem:sheafify_tensor_unit_iso_natural}
  \lean{AlgebraicGeometry.Scheme.Modules.sheafifyTensorUnitIso_hom_natural}
  \leanok
  \dcref{custom}

  \uses{mk-lem:pullback_tensor_map, mk-def:sheafify_tensor_unit_iso, mk-lem:sheafify_tensor_unit_iso_hom_eq_prime, mk-lem:restrictscalarsid_map}
  The tensor/unit reconciliation isomorphism \(\mathtt{sheafifyTensorUnitIso}\) entering the
  fourth naturality square of \ref{mk-lem:pullback_tensor_map_natural} is natural in both
...
\begin{proof}
  \leanok
  \uses{mk-lem:pullback_tensor_map}
  ...
  \ref{mk-lem:pullback_tensor_map}), so its components are morphisms of presheaves of modules.
...
\begin{lemma}[Naturality of the \(\mathtt{pullbackValIso}\) comparison]\label{mk-lem:pullback_val_iso_natural}
  \lean{AlgebraicGeometry.Scheme.Modules.pullbackValIso_hom_natural}
  \leanok
  \dcref{custom}

  \uses{mk-def:pullback_val_iso}
  ...
  pasting that proves \ref{mk-lem:pullback_tensor_map_natural}.
\end{lemma}

\begin{lemma}[The comparison on the unit pair $(\mathcal{O},\mathcal{O})$ is an isomorphism (D2$'$)]\label{mk-lem:pullback_tensor_iso_unit}
  \lean{AlgebraicGeometry.Scheme.Modules.pullbackTensorMap_unit_isIso}
  \leanok
  \dcref{custom}

  \uses{mk-lem:pullback_tensor_map, mk-lem:pullback_unit_iso, mk-lem:isiso_pullbacktensormap_of_sheafifydelta}
...
\begin{proof}
  \leanok
  \uses{mk-lem:pullback_tensor_map, mk-lem:isiso_pullbacktensormap_of_sheafifydelta}
  By \ref{mk-lem:isiso_pullbacktensormap_of_sheafifydelta} applied on the unit pair,
  ...
  \(f\) (\ref{mk-lem:pullback_unit_iso}) --- and the \(\delta\)-wrapping reduction above then
  delivers that \(\delta_{(\mathcal{O},\mathcal{O})}\) is an isomorphism.
\end{proof}

\subsection{The unit square (D2$'$): a mate-calculus telescope}

The proof of \ref{mk-lem:pullback_tensor_iso_unit} bottoms out in the single morphism equation
...
\begin{lemma}[Composite-adjunction homEquiv factorisation]\label{mk-lem:comp_homequiv_factor_sheafify_pullback}
...
\begin{lemma}[The sheafify-then-pullback comparison is the left-adjoint-uniqueness isomorphism]\label{mk-lem:sheafification_comp_pullback_eq_leftadjointuniq}
...
\begin{lemma}[leftAdjointUniq transport of the composite unit]\label{mk-lem:leftadjointuniq_app_unit_eta}
  \lean{AlgebraicGeometry.Scheme.Modules.leftAdjointUniqUnitEta}
  \leanok
  \dcref{custom}

  \uses{mk-lem:comp_homequiv_factor_sheafify_pullback, mk-lem:sheafification_comp_pullback_eq_leftadjointuniq}
...
\begin{proof}
  ...
  \(\mathtt{Adjunction.leftAdjointUniq}\,A\,B\)
  (\ref{mk-lem:sheafification_comp_pullback_eq_leftadjointuniq}), the canonical isomorphism between two left
...
\begin{lemma}[leftAdjointUniq transport of the composite unit ($P$-general form)]\label{mk-lem:leftadjointuniq_app_unit_eta_general}
  \lean{AlgebraicGeometry.Scheme.Modules.leftAdjointUniqUnitEta_app}
  \leanok
  \dcref{custom}

  \uses{mk-lem:leftadjointuniq_app_unit_eta, mk-lem:sheafification_comp_pullback_eq_leftadjointuniq}
  This is the \(P\)-general twin of \ref{mk-lem:leftadjointuniq_app_unit_eta}: the latter is the
  ...
  \ref{mk-lem:pullback_tensor_map_basechange}: applying \(A_{\varphi}.\mathrm{homEquiv}\) to the
  comparison component recovers the composite-adjunction unit at \(P\). Its proof is identical
  to that of \ref{mk-lem:leftadjointuniq_app_unit_eta} with \(\mathbf{1}^{\mathrm{p}}\) replaced
  by \(P\): \(\mathtt{sheafCompPb}\,\varphi\) is by definition
  \(\mathtt{Adjunction.leftAdjointUniq}\,A_{\varphi}\,B_{\varphi}\)
  (\ref{mk-lem:sheafification_comp_pullback_eq_leftadjointuniq}), so Mathlib's
  \(\mathtt{Adjunction.homEquiv\_leftAdjointUniq\_hom\_app}\) at \(P\) returns
  \(B_{\varphi}.\mathrm{unit}.\mathrm{app}\,P\).
\end{lemma}

\begin{lemma}[Sheafification kills the \(\mathtt{pullbackComp}\) hom--inv pair (D3$'$ residual brick)]\label{mk-lem:sheafify_pullbackcomp_hom_inv_cancel}
...
[Sq3 --- composition coherence of \(\mathtt{sheafifyTensorUnitIso}\) (D3$'$)]
  \label{mk-lem:sheafify_tensor_unit_iso_comp}
  \lean{}
  \dcref{custom}
  \uses{mk-def:sheafify_tensor_unit_iso, mk-lem:sheafify_tensor_unit_iso_hom_eq_prime, mk-lem:toringcatsheafhom_comp_hom_reconcile}
  With notation as in \ref{mk-lem:pullback_tensor_map_basechange}, write \(F^{\mathrm p}_\bullet =
  ...
  (\ref{mk-lem:toringcatsheafhom_comp_hom_reconcile} licenses the composite ring map) --- with the
...
\begin{proof}
  \uses{mk-def:sheafify_tensor_unit_iso, mk-lem:sheafify_tensor_unit_iso_hom_eq_prime, mk-lem:toringcatsheafhom_comp_hom_reconcile}
  By \ref{mk-lem:sheafify_tensor_unit_iso_hom_eq_prime} each \(\mathtt{sheafifyTensorUnitIso}.\mathrm{hom}\)
...
\begin{lemma}[Sq4 --- per-leg presheaf composition coherence of \(\mathtt{pullbackValIso}\) (D3$'$)]\label{mk-lem:pullback_val_iso_comp}
  \lean{AlgebraicGeometry.Scheme.Modules.pullbackValIso_comp_leg}
  \leanok
  \dcref{custom}

  \uses{mk-def:pullback_val_iso, mk-lem:sheafificationcomppullback_comp, mk-lem:pullback_val_iso_comp_scpb, mk-lem:pullback_val_iso_comp_counit}
  ...
  \(\mathtt{pVI} = \mathtt{pullbackValIso}\) (\ref{mk-def:pullback_val_iso}),
  ...
  (\mathtt{pullbackComp}\,h\,f)_N\bigr)\) step of \ref{mk-lem:pullback_tensor_map_basechange}; it is
...
\begin{proof}
  ...
  (\ref{mk-def:pullback_val_iso}): for every \(g\) and \(V\),
  \[
  ...
  \(\mathtt{sheafCompPb}^{-1}\) factors recombine by \ref{mk-lem:pullback_val_iso_comp_scpb}, the
  inverse of the Sq1 coherence \ref{mk-lem:sheafificationcomppullback_comp}. The pulled-back-counit
  factors recombine by \ref{mk-lem:pullback_val_iso_comp_counit}, the naturality
  ...
  \(\mathtt{sheafCompPb}\), so the counit reassembly of \ref{mk-lem:pullback_val_iso_comp_counit}
  interleaves with the \(\mathtt{sheafCompPb}^{-1}\) reassembly of
  \ref{mk-lem:pullback_val_iso_comp_scpb} through that mediating comparison.
\end{proof}

\begin{lemma}[Sq4a --- the \(\mathtt{sheafCompPb}^{-1}\) reassembly (inverse of Sq1)]\label{mk-lem:pullback_val_iso_comp_scpb}
  \lean{AlgebraicGeometry.Scheme.Modules.sheafificationCompPullback_comp_inv}
  \leanok
  \dcref{custom}

  \uses{mk-lem:sheafificationcomppullback_comp}
...
\begin{proof}
  \leanok
  \uses{mk-lem:sheafificationcomppullback_comp}
  The Sq1 coherence \ref{mk-lem:sheafificationcomppullback_comp} expresses the \(\mathrm{hom}\) leg
...
[Sq4b --- the counit reassembly (counit pseudofunctoriality)]
  \label{mk-lem:pullback_val_iso_comp_counit}
  \lean{}
  \dcref{custom}
  \uses{mk-def:pullback_val_iso}
  Let \(h : Z \to Y\), \(f : Y \to X\) be composable and \(W \in \Scheme.\mathtt{Modules}\,X\);
  write \(\varepsilon\) for the sheafification counit (\ref{mk-def:pullback_val_iso}) and
  ...
  \ref{mk-lem:pullback_val_iso_comp}, moves the composite counit into the \(h\)-image of the
...
\end{proof}
\begin{lemma}[Composition coherence of the pullback tensorator $\delta$ (D3$'$)]\label{mk-lem:pullback_tensor_map_basechange}
  \lean{AlgebraicGeometry.Scheme.Modules.pullbackTensorMap_restrict}
  \leanok
  \dcref{custom}

  \uses{mk-lem:pullback_tensor_map, mk-lem:pullback_tensor_map_natural, mk-lem:presheaf_pullback_oplaxmonoidal, mk-def:tensorobjisoofiso, mk-lem:toringcatsheafhom_comp_hom_reconcile, mk-lem:pullbackcomp_delta, mk-lem:sheafificationcomppullback_comp, mk-def:pullback_val_iso, mk-def:sheafify_tensor_unit_iso, mk-lem:sheafify_pullbackcomp_hom_inv_cancel}
  ...
  \ref{mk-lem:pullback_tensor_map}, \(h^{*}(-) = (\mathtt{pullback}\,h).\mathrm{map}\) for
  ...
  downstream by \ref{mk-lem:pullback_tensor_iso_loctriv} is the \emph{specialisation} of
...
\begin{proof}
  \leanok
  \uses{mk-lem:pullback_tensor_map, mk-lem:pullback_tensor_map_natural, mk-lem:presheaf_pullback_oplaxmonoidal, mk-lem:pullbackObjUnitToUnit_comp, mk-def:tensorobjisoofiso, mk-lem:toringcatsheafhom_comp_hom_reconcile, mk-lem:pullbackcomp_delta, mk-lem:sheafifymap_deltacomp_split, mk-lem:sheafificationcomppullback_comp, mk-def:pullback_val_iso, mk-def:sheafify_tensor_unit_iso, mk-lem:sheafify_pullbackcomp_hom_inv_cancel, mk-lem:sheafify_tensor_unit_iso_hom_eq_prime, mk-lem:pullback_val_iso_comp}
  Unfolding \(\delta_{\mathrm{sheaf}} = \mathtt{pullbackTensorMap}\)
  (\ref{mk-lem:pullback_tensor_map}) on both sides exposes each as the same four-fold
  composite
  \[
  ...
  \ref{mk-lem:presheaf_pullback_oplaxmonoidal}, \(S_3\) is the monoidal-unit
  ...
  the \emph{naturality} paste of \ref{mk-lem:pullback_tensor_map_natural} (D1$'$), this is
  ...
  \ref{mk-lem:pullbackObjUnitToUnit_comp} succeeds because
  ...
  \ref{mk-lem:pullbackObjUnitToUnit_comp}, and it is proved by the \emph{same} mate-calculus
  ...
  \ref{mk-lem:presheaf_pullback_oplaxmonoidal}), \(\delta\) is \emph{itself} an adjunction
  ...
  \ref{mk-lem:pullbackObjUnitToUnit_comp}'s \(\delta\)-mirror.
  ...
  specifically the \(\eta\to\delta\) mirror of \ref{mk-lem:pullbackObjUnitToUnit_comp} is the
  correct route.
  ...
  \ref{mk-lem:pullbackObjUnitToUnit_comp}.
  ...
  sheaf-level composite adjunction of \ref{mk-lem:leftadjointuniq_app_unit_eta_general} and
  ...
  \ref{mk-lem:pullbackObjUnitToUnit_comp} (whose proof is the \(\delta\)-free analog of this one,
  ...
  \begin{enumerate}
    ...
    \item[(c)] \emph{Recover the sub-comparison units.} Apply
      \ref{mk-lem:leftadjointuniq_app_unit_eta_general}
  ...
  This is the sheafification-laden analog of \ref{mk-lem:pullbackObjUnitToUnit_comp}; unlike
  ...
  \ref{mk-lem:sheafify_tensor_unit_iso_comp} (\(\mathtt{sheafifyTensorUnitIso\_comp}\)): via
  \ref{mk-lem:sheafify_tensor_unit_iso_hom_eq_prime} the \(\mathtt{hom}\) leg is a single
  ...
  \(\mathtt{pullbackValIso}\) is isolated as the named brick \ref{mk-lem:pullback_val_iso_comp}
  ...
  \ref{mk-lem:pullback_val_iso_comp} its composition coherence reduces to that of Sq1
  (\ref{mk-lem:sheafificationcomppullback_comp}, now closed) together with the
  ...
  \mathbin{;} a_Z.\mathrm{map}\,\mathtt{tcomp}\), via \ref{mk-lem:pullbackcomp_delta} under
  \(\mathtt{congrArg}\,a_Z.\mathrm{map}\)) by \(\mathtt{erw}\). After that splice the residual is a
  three-brick chain:
  \begin{enumerate}
    \item[(i)] \emph{The hom--inv cancellation} \ref{mk-lem:sheafify_pullbackcomp_hom_inv_cancel}
    ...
    \item[(iii)] \emph{Sq3 \ref{mk-lem:sheafify_tensor_unit_iso_comp} and Sq4
      \ref{mk-lem:pullback_val_iso_comp}}, discharged via the interleaved four-square merge below.
  \end{enumerate}
  ...
  \ref{mk-lem:sheafificationcomppullback_comp}, closed) --- must be made to act instead on the
  ...
  \ref{mk-lem:sheafify_tensor_unit_iso_comp} (the composition coherence of
  \(\mathtt{sheafifyTensorUnitIso}\)) and Sq4 \ref{mk-lem:pullback_val_iso_comp} (the
  ...
  \(\mathtt{pullbackValIso}\,f\) (\ref{mk-def:pullback_val_iso}): one side presents the data
  ...
  exactly as in the merge of \ref{mk-lem:pullback_tensor_map_natural} (D1$'$): there the two
  ...
  (\ref{mk-lem:presheaf_pullback_oplaxmonoidal}); the single-\(\mathtt{tensorHom}\)
  presentation \(\mathtt{sheafifyTensorUnitIso}.\mathrm{hom} = a.\mathrm{map}(\eta\otimes\eta)\)
  of \ref{mk-lem:sheafify_tensor_unit_iso_hom_eq_prime}, which keeps every factor inside one
  monoidal instance; and the \(\mathtt{pullbackValIso}\) factorisation of
  \ref{mk-def:pullback_val_iso} (the inverse of \(\mathtt{sheafificationCompPullback}\) followed
...
The open-immersion comparison lemma K1 (\ref{mk-lem:pullback_tensor_map_isiso_open_immersion})
reduces, by the same two-ingredient model as \ref{mk-lem:tensorobj_restrict_iso}, to the
...
template \ref{mk-lem:presheafunit_comp_map_eta}, and the open-immersion analogues of the D3$'$
composition coherence \ref{mk-lem:pushforwardcomp_lax_mu}; they use the same D3$'$ pure-tensor
...
\begin{lemma}[Presheaf-side mate identity for the $\eta$-bridge]\label{mk-lem:presheafunit_comp_map_eta}
  \lean{AlgebraicGeometry.Scheme.Modules.presheafUnit_comp_map_eta}
  \leanok
  \dcref{custom}

  \uses{mk-lem:presheaf_pushforward_laxmonoidal, mk-lem:presheaf_pullback_oplaxmonoidal}
...
\begin{proof}
  ...
  \(\mathtt{presheafPushforwardLaxMonoidal}\) (\ref{mk-lem:presheaf_pushforward_laxmonoidal}) and
  \(\mathtt{presheafPullbackOplaxMonoidal}\) (\ref{mk-lem:presheaf_pullback_oplaxmonoidal}).
\end{proof}

\begin{lemma}[K1 $\eta$-side collapse: $H_1$ respects the unit comparison]\label{mk-lem:pushforward_eta_appiso_collapse}
  \lean{AlgebraicGeometry.Scheme.Modules.pushforward_eta_appIso_collapse}
  \leanok
  \dcref{custom}

  \uses{mk-lem:tensorobj_restrict_iso, mk-lem:restrictscalars_laxmonoidal, mk-lem:presheaf_pullback_oplaxmonoidal, mk-lem:pushforwardpushforwardadj_unit_app_app_apply, mk-lem:restrictscalars_oplaxmonoidal_eta_app_one}
...
\begin{proof}
  \leanok
  \uses{mk-lem:tensorobj_restrict_iso, mk-lem:restrictscalars_laxmonoidal, mk-lem:pushforwardpushforwardadj_unit_app_app_apply, mk-lem:restrictscalars_oplaxmonoidal_eta_app_one}
  This is the $\eta$/$\varepsilon$-side of ``\(H_1\) is a monoidal natural isomorphism'', the
  open-immersion analogue of the D2$'$ template \ref{mk-lem:presheafunit_comp_map_eta}. It is
  ...
  \ref{mk-lem:tensorobj_restrict_iso}); the mate \(\eta(\mathtt{pullback}\,\varphi')\) is the same
  ...
  \ref{mk-lem:pushforwardpushforwardadj_unit_app_app_apply}, after which both sides reduce to the
  ring-unit identity \(1 \mapsto 1\).
\end{proof}

\begin{lemma}[Presheaf-level value of the pushforward--pushforward adjunction unit]\label{mk-lem:pushforwardpushforwardadj_unit_app_app_apply}
...
\begin{lemma}[Presheaf-level value of the pushforward--pushforward adjunction counit]\label{mk-lem:pushforwardpushforwardadj_counit_app_app_apply}
  \lean{AlgebraicGeometry.Scheme.Modules.pushforwardPushforwardAdj_counit_app_app_apply}
  \leanok
  \dcref{custom}

  The exact dual of \ref{mk-lem:pushforwardpushforwardadj_unit_app_app_apply}. For the same
...
\begin{lemma}[Unit-preservation of the strong-monoidal $\mathtt{restrictScalars}$ oplax unit]\label{mk-lem:restrictscalars_oplaxmonoidal_eta_app_one}
  \lean{AlgebraicGeometry.Scheme.Modules.restrictScalars_oplaxMonoidal_η_app_one}
  \leanok
  \dcref{custom}

  \uses{mk-lem:restrictscalars_laxmonoidal}
...
\begin{lemma}[Sectionwise comparison of the LHS mate tensorator on a pure tensor]\label{mk-lem:pushforward_lax_mu_comparison_lhs_tmul}
  \lean{AlgebraicGeometry.Scheme.Modules.pushforward_lax_mu_comparison_lhs_tmul}
  \leanok
  \dcref{custom}

  \uses{mk-lem:pushforwardpushforwardadj_unit_app_app_apply, mk-lem:pushforwardpushforwardadj_counit_app_app_apply, mk-lem:restrictscalars_mu_app, mk-lem:restrictscalars_mu_app_tmul, mk-lem:forget2_restrictscalars_mu_hom_tmul, mk-lem:restrictscalars_delta_app_tmul, mk-lem:pushforward_lax_mu_comparison_rhs_tmul}
...
\begin{proof}
  \leanok
  \uses{mk-lem:pushforwardpushforwardadj_unit_app_app_apply, mk-lem:pushforwardpushforwardadj_counit_app_app_apply, mk-lem:restrictscalars_mu_app, mk-lem:restrictscalars_mu_app_tmul, mk-lem:forget2_restrictscalars_mu_hom_tmul, mk-lem:restrictscalars_delta_app_tmul, mk-lem:pushforward_lax_mu_comparison_rhs_tmul}
  The value lemma \ref{mk-lem:pushforwardpushforwardadj_unit_app_app_apply} characterises the
  ...
  \ref{mk-lem:pushforwardpushforwardadj_unit_app_app_apply}. Reindexing the
  ...
  which is the identity on pure tensors by \ref{mk-lem:restrictscalars_mu_app},
  \ref{mk-lem:restrictscalars_mu_app_tmul} and \ref{mk-lem:forget2_restrictscalars_mu_hom_tmul}, while its
  ...
  \ref{mk-lem:restrictscalars_delta_app_tmul}. \emph{Step 4 (the counit pair).} Split the
  ...
  leg by \ref{mk-lem:pushforwardpushforwardadj_counit_app_app_apply}. \emph{Closing (triangle
  ...
  \ref{mk-lem:pushforward_lax_mu_comparison_rhs_tmul}.
\end{proof}

\begin{lemma}[Sectionwise value of the RHS composition tensorator on a pure tensor]\label{mk-lem:pushforward_lax_mu_comparison_rhs_tmul}
  \lean{AlgebraicGeometry.Scheme.Modules.pushforward_lax_mu_comparison_rhs_tmul}
  \leanok
  \dcref{custom}

  \uses{mk-lem:pushforward_mu_eq, mk-lem:restrictscalars_mu_app, mk-lem:restrictscalars_mu_app_tmul, mk-lem:forget2_restrictscalars_mu_hom_tmul, mk-lem:pushforward_map_restrictscalars_mu_app_tmul}
...
\begin{proof}
  \leanok
  \uses{mk-lem:pushforward_mu_eq, mk-lem:restrictscalars_mu_app, mk-lem:restrictscalars_mu_app_tmul, mk-lem:forget2_restrictscalars_mu_hom_tmul, mk-lem:pushforward_map_restrictscalars_mu_app_tmul}
  By \ref{mk-lem:pushforward_mu_eq} the pushforward composition tensorator reduces to the
  ...
  \ref{mk-lem:restrictscalars_mu_app} and collapsing on the pure tensor by
  \ref{mk-lem:restrictscalars_mu_app_tmul} and \ref{mk-lem:forget2_restrictscalars_mu_hom_tmul}, with the
  outer \((\mathtt{pushforward}\,\varphi').\mathrm{map}\) leg collapsed by
  \ref{mk-lem:pushforward_map_restrictscalars_mu_app_tmul}, the value is \(m \otimes_t n\). This reuses
  the same helper family as the composition coherence of the single pushforward.
\end{proof}

\begin{lemma}[Bare tensorator comparison of the two lax structures on $\mathtt{pushforward}\,\varphi'$]\label{mk-lem:pushforward_lax_mu_comparison}
  \lean{AlgebraicGeometry.Scheme.Modules.pushforward_lax_mu_comparison}
  \leanok
  \dcref{custom}

  \uses{mk-lem:pushforward_lax_mu_comparison_lhs_tmul, mk-lem:pushforward_lax_mu_comparison_rhs_tmul, mk-lem:pushforward_mu_eq}
  ...
  \(\mathtt{presheafPushforwardLaxMonoidal}\) (\ref{mk-lem:presheaf_pushforward_laxmonoidal}). Their
...
\begin{proof}
  ...
  \(\mathtt{restrictScalars}\,\varphi'\) tensorator via \ref{mk-lem:pushforward_mu_eq} at the
  \emph{morphism level}, before descending to sections --- \ref{mk-lem:pushforward_mu_eq} is an
  ...
  \(m \otimes_t n\). By \ref{mk-lem:pushforward_lax_mu_comparison_lhs_tmul} the left-hand mate evaluates
  to \(m \otimes_t n\), and by \ref{mk-lem:pushforward_lax_mu_comparison_rhs_tmul} the right-hand
...
\begin{lemma}[K1 $\mu$/$\delta$-side collapse: $H_1$ respects the tensorator]\label{mk-lem:pushforward_mu_appiso_collapse}
  \lean{AlgebraicGeometry.Scheme.Modules.pushforward_mu_appIso_collapse}
  \leanok
  \dcref{custom}

  \uses{mk-lem:pushforward_lax_mu_comparison, mk-lem:pushforward_mu_eq, mk-lem:tensorobj_restrict_iso, mk-lem:presheaf_pushforward_laxmonoidal, mk-lem:presheaf_pullback_oplaxmonoidal, mk-lem:restrictscalars_monoidal_of_bijective, mk-lem:functor_monoidal_muiso, mk-lem:deltaconjofmucomparison, mk-lem:adjunction_leftadjointuniq_trans}
  ...
  (the $\mu$/$\delta$-side twin of \ref{mk-lem:presheafunit_comp_map_eta}, and the open-immersion
  analogue of \ref{mk-lem:pushforwardcomp_lax_mu}).
\end{lemma}
\begin{proof}
  \leanok
  \uses{mk-lem:pushforward_lax_mu_comparison, mk-lem:pushforward_mu_eq}
  The genuine geometric content is the bare tensorator comparison
  \ref{mk-lem:pushforward_lax_mu_comparison}; this proof is the formal reduction to it, and uses
  ...
  \ref{mk-lem:pushforward_mu_eq}. The goal is then exactly
  \ref{mk-lem:pushforward_lax_mu_comparison}, which closes it.
\end{proof}

\begin{lemma}[Pullback commutes with $\otimes$ along an open immersion (K1)]\label{mk-lem:pullback_tensor_map_isiso_open_immersion}
  \dcref{custom}

  \uses{mk-lem:pullback_tensor_map, mk-lem:tensorobj_restrict_iso, mk-lem:isiso_pullbacktensormap_of_sheafifydelta}
  ...
  \ref{mk-lem:pullback_tensor_map},
  \[
    f^*(M \otimes_X N)
      \;\xrightarrow{\;\sim\;}\;
    (f^*M) \otimes_Y (f^*N),
...
\begin{proof}
  \uses{mk-lem:pullback_tensor_map, mk-lem:tensorobj_restrict_iso, mk-lem:isiso_pullbacktensormap_of_sheafifydelta, mk-lem:presheafunit_comp_map_eta, mk-lem:pushforward_eta_appiso_collapse, mk-lem:pushforward_mu_appiso_collapse, mk-lem:deltaconjofmucomparison, mk-lem:isiso_oplaxdelta_of_conj}
  \textit{Source: \cite{stacks-project}, Modules on Ringed Spaces, Lemma
  \texttt{lemma-tensor-product-pullback}.}

  Unfold \(\mathtt{pullbackTensorMap}\,f\,M\,N\) by its four-fold factorisation
  (\ref{mk-lem:pullback_tensor_map}, made explicit in
  \ref{mk-lem:isiso_pullbacktensormap_of_sheafifydelta}). The three outer factors
  ...
  closes \ref{mk-lem:tensorobj_restrict_iso}.
  ...
  that closes \ref{mk-lem:tensorobj_restrict_iso}.)
  ...
  isomorphism used in \ref{mk-lem:tensorobj_restrict_iso}. In particular
  \(\mathtt{pushforward}\,\beta\) carries an \emph{invertible} tensorator.

  The two pure-tensor collapses --- the unit compatibility
  \ref{mk-lem:pushforward_eta_appiso_collapse} (with template \ref{mk-lem:presheafunit_comp_map_eta})
  and the tensorator compatibility \ref{mk-lem:pushforward_mu_appiso_collapse} --- confirm that
  \(H_1\) is a monoidal natural isomorphism. The tensorator compatibility in particular supplies
  the hypothesis of the abstract \(\delta\)-conjugation \ref{mk-lem:deltaconjofmucomparison},
  ...
  \(\mu_{\mathrm{Iso}}\,G_\beta\) (\ref{mk-lem:functor_monoidal_muiso}) and is therefore an
  isomorphism; \ref{mk-lem:isiso_oplaxdelta_of_conj} then concludes that
...
\begin{lemma}[Pullback commutes with $\otimes$ on locally-trivial pairs (D4$'$)]\label{mk-lem:pullback_tensor_iso_loctriv}
  \dcref{custom}

  \uses{mk-def:IsLocallyTrivial, mk-lem:pullback_tensor_map, mk-lem:pullback_tensor_map_natural, mk-lem:pullback_tensor_iso_unit, mk-lem:pullback_tensor_map_basechange, mk-lem:pullback_tensor_map_isiso_open_immersion, mk-lem:isiso_of_isiso_restrict, mk-lem:tensorobj_preserves_locally_trivial, mk-lem:IsLocallyTrivial_pullback}
  Let \(f : Y \to X\) be a morphism of schemes and let
  \(M, N \in \Scheme.\mathtt{Modules}\,X\) be \emph{locally trivial}
  (\ref{mk-def:IsLocallyTrivial}). Then the sheaf-level comparison map
  \(\delta_{\mathrm{sheaf}} = \mathtt{pullbackTensorMap}\) of
  \ref{mk-lem:pullback_tensor_map} is an isomorphism:
  \[
    f^*(M \otimes_X N)
      \;\xrightarrow{\;\sim\;}\;
    (f^*M) \otimes_Y (f^*N),
...
\begin{proof}
  ...
  By local triviality of \(M\) and \(N\) (\ref{mk-def:IsLocallyTrivial}) choose a common
  ...
  (\ref{mk-lem:tensorobj_preserves_locally_trivial}). The preimages
  \(\{f^{-1}U_i\}\) form an open cover of \(Y\). It suffices, by the
  restriction-detects-isomorphism criterion \ref{mk-lem:isiso_of_isiso_restrict}, to show
  ...
  \ref{mk-lem:pullback_tensor_map_basechange} expresses the restriction
  ...
  \ref{mk-lem:pullback_tensor_map_isiso_open_immersion} (K1) the comparison along an open
  ...
  \(\delta_{\mathrm{sheaf}}\) in both arguments (\ref{mk-lem:pullback_tensor_map_natural})
  ...
  \ref{mk-lem:pullback_tensor_iso_unit} --- the unit-pair case, where the structure-sheaf
  ...
  f^*(M\otimes_X N)\) are locally trivial by \ref{mk-lem:IsLocallyTrivial_pullback} and
  \ref{mk-lem:tensorobj_preserves_locally_trivial} --- which is what makes the chart-local
  comparison a map between copies of the structure sheaf.) By
  \ref{mk-lem:isiso_of_isiso_restrict}, \(\delta_{\mathrm{sheaf}}(M,N)\) is a global
  isomorphism, natural in \(M, N\) by \ref{mk-lem:pullback_tensor_map_natural}.
\end{proof}

\begin{lemma}[$\delta_{\mathrm{sheaf}}$ is an isomorphism on a structure-sheaf pair (D4$'$ base case)]\label{mk-lem:pullback_tensor_map_isiso_base_unit}
  \lean{AlgebraicGeometry.Scheme.Modules.pullbackTensorMap_isIso_of_base_unit}
  \leanok
  \dcref{custom}

  \uses{mk-lem:pullback_tensor_map_natural, mk-lem:pullback_tensor_iso_unit}
  Let \(f : Y \to X\) be a morphism of schemes and let
  ...
  \(\mathtt{pullbackTensorMap}\,f\,P\,Q\) of \ref{mk-lem:pullback_tensor_map} is an isomorphism.
\end{lemma}
...
\begin{proof}
  ...
  (\ref{mk-lem:pullback_tensor_map_natural}), the chosen isomorphisms
  ...
  \ref{mk-lem:pullback_tensor_iso_unit} (D2$'$). Transport of an isomorphism along
  isomorphisms is again an isomorphism, so \(\mathtt{pullbackTensorMap}\,f\,P\,Q\) is one.
\end{proof}

\begin{lemma}[Per-chart isomorphism obligation for the D4$'$ chart-chase]\label{mk-lem:chart_isiso}
  \lean{AlgebraicGeometry.Scheme.Modules.chart_isIso}
  \leanok
  \dcref{custom}

  \uses{mk-lem:isiso_of_isiso_restrict, mk-lem:pullback_tensor_map_basechange, mk-lem:pullback_tensor_map_isiso_open_immersion, mk-lem:pullback_tensor_map_isiso_base_unit}
  In the situation of \ref{mk-lem:pullback_tensor_iso_loctriv}, fix a member \(f^{-1}U_i\) of
...
\begin{proof}
  \leanok
  \uses{mk-lem:isiso_of_isiso_restrict, mk-lem:pullback_tensor_map_basechange, mk-lem:pullback_tensor_map_isiso_open_immersion, mk-lem:pullback_tensor_map_isiso_base_unit}
  The base-change coherence \ref{mk-lem:pullback_tensor_map_basechange}, applied at the two
  ...
  are isomorphisms by \ref{mk-lem:pullback_tensor_map_isiso_open_immersion} (K1, applied twice)
  ...
  structure-sheaf-pair case \ref{mk-lem:pullback_tensor_map_isiso_base_unit}. (The
  restriction criterion \ref{mk-lem:isiso_of_isiso_restrict} is what makes this chart-local
...
\label{mk-sec:tensorobj_pic_carrier}

The group law of \ref{mk-lem:tensorobj_isoclass_commgroup} is carried on the
...
(\ref{mk-lem:tensorobj_inverse_invertible}): one must \emph{manufacture} the dual
...
the dual-infrastructure build of \ref{mk-sec:tensorobj_dual_infra}.
...
\(\mathtt{IsInvertible}\) of \ref{mk-def:scheme_modules_isinvertible},
\[
...
well-definedness fact (\ref{mk-lem:isinvertible_inverse_welldef}: the witness \(N\) is
...
\begin{lemma}[Associator for $\otimes_X$ on invertible objects (corollary of the unconditional associator)]\label{mk-lem:tensorobj_assoc_iso_invertible}
  \lean{AlgebraicGeometry.Scheme.Modules.tensorObj_assoc_iso_invertible}
  \leanok
  \dcref{custom}

  \uses{mk-def:scheme_modules_tensorobj, mk-def:scheme_modules_isinvertible, mk-lem:tensorobj_assoc_iso}
  Let \(X\) be a scheme and let
  \(M, N, P \in \Scheme.\mathtt{Modules}\,X\) be \(\otimes\)-invertible
  (\ref{mk-def:scheme_modules_isinvertible}). There is an isomorphism
  \[
    (M \otimes_X N) \otimes_X P
      \;\xrightarrow{\ \sim\ }\;
    M \otimes_X (N \otimes_X P)
...
\begin{proof}
  \leanok
  \uses{mk-lem:tensorobj_assoc_iso}
  Immediate specialisation of the \emph{unconditional} associator
  \ref{mk-lem:tensorobj_assoc_iso} to the invertible arguments \(M, N, P\); no
  ...
  obligation \ref{mk-lem:islocallyinjective_whisker_of_W} being closed via the d.2
  stalk--tensor commutation \ref{mk-lem:islocallyinjective_whiskerleft_via_stalk}.)
\end{proof}

\begin{definition}[The invertibility-carrier Picard group \(\Pic X\)]\label{mk-def:pic_carrier}
  \dcref{01CX,custom}

  \uses{mk-def:scheme_modules_tensorobj, mk-def:scheme_modules_isinvertible, mk-def:pic_setoid}
  ...
  \]
  of \ref{mk-def:scheme_modules_isinvertible}, and equip it with the setoid whose
...
\begin{lemma}[\(\otimes\)-invertibility is closed under tensor product]\label{mk-lem:isinvertible_tensor}
  \dcref{01CR,custom}

  \uses{mk-def:scheme_modules_isinvertible, mk-lem:tensorobj_assoc_iso_invertible, mk-lem:tensorobj_comm_iso, mk-lem:tensorobj_unit_iso}
...
\begin{proof}
  ...
  factors with the associator \ref{mk-lem:tensorobj_assoc_iso_invertible} and the
  braiding \ref{mk-lem:tensorobj_comm_iso},
  \[
  ...
  unitor \ref{mk-lem:tensorobj_unit_iso}. Hence \(N \otimes_X N'\) witnesses
...
\begin{lemma}[The tensor inverse is determined up to isomorphism]\label{mk-lem:isinvertible_inverse_welldef}
  \dcref{01CR,custom}

  \uses{mk-def:scheme_modules_isinvertible, mk-lem:tensorobj_unit_iso, mk-lem:tensorobj_assoc_iso_invertible, mk-lem:tensorobj_comm_iso}
...
\begin{proof}
  ...
  \ref{mk-lem:tensorobj_unit_iso}, the two hypotheses, the associator
  \ref{mk-lem:tensorobj_assoc_iso_invertible} and the braiding
  \ref{mk-lem:tensorobj_comm_iso}:
  \[
    N
...
\label{mk-sec:tensorobj_consumer_acg}


\begin{theorem}[Abelian-group instance on the relative Picard quotient via \(\Scheme.\mathtt{Modules}.\mathtt{tensorObj}\)]\label{mk-thm:rel_pic_addcommgroup_via_tensorobj}
  \lean{AlgebraicGeometry.Scheme.PicSharp.addCommGroup_via_tensorObj}
  \leanok
  \dcref{custom}

  \uses{mk-lem:tensorobj_lift_onproduct, mk-lem:pullback_tensor_iso_loctriv, mk-def:scheme_modules_isinvertible, mk-lem:tensorobj_isoclass_commgroup, mk-lem:tensorobj_assoc_iso, mk-lem:tensorobj_unit_iso, mk-lem:tensorobj_comm_iso, mk-thm:relative_pic_quotient_well_defined, mk-lem:rel_pic_sharp_groupoid}
  ...
  \]
  of \ref{mk-thm:relative_pic_quotient_well_defined} carries a canonical
  ...
  supplied by the substrate of \ref{mk-def:scheme_modules_tensorobj}
  restricted to the \(\texttt{LineBundle.OnProduct}\,\pi_C\,\pi_T\) carrier
  via \ref{mk-lem:tensorobj_lift_onproduct}. With this structure the quotient
  map
  \(\Pic(C \times_k T) \twoheadrightarrow \Pic^{\sharp}_{C/k}(T)\) is a
  surjective group homomorphism with kernel exactly \(\pi_T^*\Pic(T)\),
  in agreement with \ref{mk-lem:rel_pic_sharp_groupoid}.
\end{theorem}

\begin{proof}
  \leanok
  \uses{mk-lem:tensorobj_lift_onproduct, mk-lem:pullback_tensor_iso_loctriv, mk-def:scheme_modules_isinvertible, mk-lem:tensorobj_isoclass_commgroup, mk-thm:relative_pic_quotient_well_defined, mk-lem:rel_pic_sharp_groupoid}
  ...
  \(\otimes\) (\ref{mk-lem:scheme_modules_tensorobj_functoriality}), and
  ...
  (\ref{mk-lem:tensorobj_isoclass_commgroup}): multiplication
  ...
  \(\otimes\)-invertible (\ref{mk-def:scheme_modules_isinvertible}), the inverse
  ...
  (\ref{mk-def:scheme_modules_isinvertible}), the scheme-level analogue of
  ...
  By \ref{mk-lem:pullback_tensor_iso_loctriv} applied to the projection
  ...
  By \ref{mk-thm:relative_pic_quotient_well_defined}, the underlying set of
  ...
  structure of \ref{mk-lem:rel_pic_sharp_groupoid}.
\end{proof}

\section{Sheaf internal-hom and tensor-inverse infrastructure}
\label{mk-sec:tensorobj_dual_infra}
...
sheafified like the tensor product \ref{mk-def:scheme_modules_tensorobj}. The remedy is
...
\begin{definition}[The global-ring module on morphisms of presheaves of modules (value module)]\label{mk-def:presheaf_internal_hom_value}
...
\begin{definition}[Slice specialisation of the morphism module]\label{mk-def:presheaf_internal_hom_slice_value}
  \lean{PresheafOfModules.InternalHom.internalHomObjModule}
  \leanok
  \dcref{custom}

  \uses{mk-def:presheaf_internal_hom_value}
  Specialise \ref{mk-def:presheaf_internal_hom_value} to a slice category. For an object
  ...
  \ref{mk-def:presheaf_internal_hom_value} makes the morphism group
  \(M|_U \longrightarrow N|_U\) an \(R(U)\)-module, the
  \(\mathtt{internalHomObjModule}\). This \(R(U)\)-module \((M|_U \to N|_U)\) is exactly
  the value \(\mathcal{H}om(M, N)(U)\) of \ref{mk-def:presheaf_internal_hom}.
\end{definition}

\begin{definition}[Presheaf internal hom of \(\mathcal{O}_X\)-modules]\label{mk-def:presheaf_internal_hom}
  \lean{PresheafOfModules.InternalHom.internalHom}
  \leanok
  \dcref{01CM,custom}

  \uses{mk-def:scheme_modules_tensorobj, mk-def:presheaf_internal_hom_value, mk-def:presheaf_internal_hom_slice_value, mk-lem:presheaf_internal_hom_restriction}
  \textit{Source: \cite{stacks-project}, ``Modules on Ringed Spaces'', \S Internal Hom
  (tag area 01CM).}
  ...
  \ref{mk-def:presheaf_internal_hom_slice_value} of the morphism module
  \ref{mk-def:presheaf_internal_hom_value}.
  ...
  \(R(U)\)-module \((M|_U \to N|_U)\) of \ref{mk-def:presheaf_internal_hom_value} /
  \ref{mk-def:presheaf_internal_hom_slice_value}. \emph{(b) Restriction maps.} For an
  inclusion \(V \subseteq U\) the restriction map
  \(\mathcal{H}om(M, N)(U) \to \mathcal{H}om(M, N)(V)\) is the further-restriction map
  \ref{mk-lem:presheaf_internal_hom_restriction}, \(\varphi \mapsto \varphi|_V\), which
  ...
  tensor product \ref{mk-def:scheme_modules_tensorobj} --- which is covariant in the
...
\begin{lemma}[Restriction map of the presheaf internal hom]\label{mk-lem:presheaf_internal_hom_restriction}
  \lean{PresheafOfModules.InternalHom.restrictionMap}
  \leanok
  \dcref{01CM,custom}

  \uses{mk-def:presheaf_internal_hom_value, mk-def:presheaf_internal_hom_slice_value}
  \textit{Source: \cite{stacks-project}, ``Modules on Ringed Spaces'', \S Internal Hom
  ...
  \ref{mk-def:presheaf_internal_hom} (through \(\mathtt{PresheafOfModules.ofPresheaf}\) /
  \(\mathtt{mk}\)) requires.
\end{lemma}
...
\begin{proof}
  ...
  \ref{mk-def:presheaf_internal_hom_value} acts on \(\varphi\) by post-composition with
...
\begin{definition}[Presheaf dual against the structure presheaf]\label{mk-def:presheaf_dual}
  \lean{PresheafOfModules.dual}
  \leanok
  \dcref{custom}

  \uses{mk-def:presheaf_internal_hom}
  With \(R\) and \(M\) as in \ref{mk-def:presheaf_internal_hom}, the
  ...
  \ref{mk-def:presheaf_internal_hom}.
\end{definition}

\begin{lemma}[Evaluation/contraction of the internal hom]\label{mk-lem:internal_hom_eval}
  \lean{PresheafOfModules.internalHomEval}
  \leanok
  \dcref{01CM,custom}

  \uses{mk-def:presheaf_dual, mk-def:scheme_modules_tensorobj}
  \textit{Source: \cite{stacks-project}, ``Modules on Ringed Spaces'', \S Internal Hom
  (tag area 01CM).}
  ...
  \ref{mk-def:scheme_modules_tensorobj}.
\end{lemma}

\begin{proof}
  \leanok
  \uses{mk-def:presheaf_dual, mk-def:scheme_modules_tensorobj, mk-lem:presheaf_internal_hom_restriction}
  ...
  \ref{mk-lem:presheaf_internal_hom_restriction}. This is the counit of the tensor--hom
  adjunction of \ref{mk-def:presheaf_internal_hom} specialised to the unit presheaf.
\end{proof}

\begin{lemma}[The internal hom is a sheaf; the sheaf-level dual]\label{mk-lem:internal_hom_isSheaf}
  \lean{AlgebraicGeometry.Scheme.Modules.dual}
  \leanok
  \dcref{01CM,custom}

  \uses{mk-def:presheaf_dual, mk-lem:internal_hom_eval}
  \textit{Source: \cite{stacks-project}, ``Modules on Ringed Spaces'', \S Internal Hom
  (tag area 01CM).}
  Let \(X\) be a scheme, \(R = X.\mathtt{ringCatSheaf}\), and let
  \(M, N \in \Scheme.\mathtt{Modules}\,X\) with \(N\) a sheaf of modules. Then the
  presheaf internal hom \(\mathcal{H}om(M, N)\) of \ref{mk-def:presheaf_internal_hom}
  ...
  \]
  and the evaluation \ref{mk-lem:internal_hom_eval} descends to a morphism of sheaves of
...
\begin{proof}
  ...
  \(\mathtt{dual}\,M\), and the evaluation morphism of \ref{mk-lem:internal_hom_eval},
  being a morphism of presheaves into the sheaf \(\mathcal{O}_X\), is a morphism of
  the corresponding sheaves of modules.
\end{proof}

\begin{lemma}[Base change along a ring isomorphism commutes with the linear dual (H2$'$)]\label{mk-lem:restrictscalars_ringiso_dualequiv}
  \lean{restrictScalarsRingIsoDualEquiv}
  \leanok
  \dcref{custom}

  \uses{mk-lem:restrictscalars_ringiso_tensorequiv}
  ...
  analogue of \ref{mk-lem:restrictscalars_ringiso_tensorequiv}: it is to the internal hom
  ...
  \ref{mk-lem:dual_isLocallyTrivial} --- the ring-iso/dual commutation that, composed with
...
\begin{definition}[Precomposition equivalence on dual sections]\label{mk-def:presheaf_dual_precomp_equiv}
  \lean{PresheafOfModules.dualPrecompEquiv}
  \leanok
  \dcref{custom}

  \uses{mk-def:presheaf_dual}
  ...
  (\ref{mk-def:presheaf_dual}) in isomorphisms: precomposing a functional on \(M'\) with
  the comparison isomorphism produces a functional on \(M\), \(R(U)\)-linearly and
  invertibly.
\end{definition}

\begin{definition}[The presheaf dual carries an iso \(M \cong M'\) to an iso \(M'^\vee \cong M^\vee\)]\label{mk-def:presheaf_dual_iso_of_iso}
  \lean{PresheafOfModules.dualIsoOfIso}
  \leanok
  \dcref{custom}

  \uses{mk-def:presheaf_dual, mk-def:presheaf_dual_precomp_equiv}
  ...
  \(\mathtt{dualPrecompEquiv}\,e\,U\) of \ref{mk-def:presheaf_dual_precomp_equiv}.
...
\begin{definition}[The sheaf-level dual carries an iso \(M \cong M'\) to an iso \(\mathtt{dual}\,M' \cong \mathtt{dual}\,M\)]\label{mk-def:scheme_modules_dual_iso_of_iso}
  \lean{AlgebraicGeometry.Scheme.Modules.dualIsoOfIso}
  \leanok
  \dcref{custom}

  \uses{mk-lem:internal_hom_isSheaf, mk-def:presheaf_dual_iso_of_iso}
  ...
  \ref{mk-def:presheaf_dual_iso_of_iso} through the sheaf-level dual of
  \ref{mk-lem:internal_hom_isSheaf}. It is the dual (contravariant) analogue of the
...
\begin{lemma}[The per-section dual transport \(\mathtt{sliceDualTransport}\)]\label{mk-lem:slice_dual_transport}
  \lean{AlgebraicGeometry.Scheme.Modules.sliceDualTransport}
  \leanok
  \dcref{custom}

  \uses{mk-lem:internal_hom_isSheaf, mk-lem:restrictscalars_ringiso_dualequiv, mk-def:dual_unit_ring_swap, mk-lem:isiso_eps_restrictscalars_appiso, mk-def:dual_unit_iso_gen, mk-lem:image_preimage_of_le, mk-def:dual_unit_ring_swap_hom, mk-lem:isiso_eps_restrictscalars_appiso_hom, mk-lem:slice_dual_transport_inv, mk-def:dual_unit_ring_swap_inv, mk-lem:dual_unit_ring_swap_comp_inv, mk-lem:dual_unit_ring_swap_inv_comp, mk-lem:slice_dual_transport_naturality_apply, mk-lem:appiso_hom_naturality_apply}
...
\begin{proof}
  \leanok
  \uses{mk-lem:internal_hom_isSheaf, mk-lem:restrictscalars_ringiso_dualequiv, mk-def:dual_unit_ring_swap, mk-lem:isiso_eps_restrictscalars_appiso, mk-def:dual_unit_iso_gen, mk-lem:image_preimage_of_le, mk-def:dual_unit_ring_swap_hom, mk-lem:isiso_eps_restrictscalars_appiso_hom, mk-lem:slice_dual_transport_inv, mk-def:dual_unit_ring_swap_inv, mk-lem:dual_unit_ring_swap_comp_inv, mk-lem:dual_unit_ring_swap_inv_comp, mk-lem:slice_dual_transport_inv_app_apply, mk-lem:slice_dual_transport_naturality_apply, mk-lem:appiso_hom_naturality_apply}
  ...
  section (\ref{mk-def:dual_unit_iso_gen}).
  ...
  \(\mathtt{image\_preimage\_of\_le}\) (\ref{mk-lem:image_preimage_of_le})),
  ...
  (f.\mathtt{appIso}\,W'').\mathrm{hom})\bigr)\) (\ref{mk-def:dual_unit_ring_swap_hom})
  --- that is, the \emph{inverse} of \(\varepsilon\) in the \(\mathrm{hom}\)-direction (where
  \(\varepsilon\) is invertible by
  \ref{mk-lem:isiso_eps_restrictscalars_appiso_hom}), \emph{not}
  ...
  (\ref{mk-def:presheaf_internal_hom_slice_value}) --- the pointwise module structure in which the
  ...
  \begin{enumerate}
    ...
    \item[(iii)] \emph{Apply the change-of-rings compatibility on the morphism level.} Once the
      ...
      \ref{mk-lem:restrictscalars_ringiso_dualequiv}.
  \end{enumerate}
...
\label{mk-subsec:dual_inverse_helpers}

These are the individual ingredients out of which \ref{mk-lem:slice_dual_transport} and its
...
\begin{lemma}[Leg-(B) unit \(\varepsilon\) is invertible, \(\mathrm{inv}\)-direction]\label{mk-lem:isiso_eps_restrictscalars_appiso}
  \lean{AlgebraicGeometry.Scheme.Modules.isIso_ε_restrictScalars_appIso}
  \leanok
  \dcref{custom}

  \uses{mk-def:restrict_scalars_lax_epsilon}
...
\begin{proof}
  ...
  its restriction-of-scalars unit \(\varepsilon\) (\ref{mk-def:restrict_scalars_lax_epsilon}) is
  invertible by the bijectivity criterion for \(\varepsilon\).
\end{proof}

\begin{lemma}[Leg-(B) unit \(\varepsilon\) is invertible, \(\mathrm{hom}\)-direction]\label{mk-lem:isiso_eps_restrictscalars_appiso_hom}
...
\begin{lemma}[Unit \(\varepsilon\) is invertible, section-relabel direction]\label{mk-lem:isiso_eps_restrictscalars_presheafmap}
...
\begin{proof}
  ...
  restriction-of-scalars unit \(\varepsilon\) (\ref{mk-def:restrict_scalars_lax_epsilon}) is
  invertible by the bijectivity criterion for \(\varepsilon\).
\end{proof}

\begin{definition}[Leg-(B) codomain unit ring-iso swap, \(\mathrm{inv}\)-direction]\label{mk-def:dual_unit_ring_swap}
  \lean{AlgebraicGeometry.Scheme.Modules.dualUnitRingSwap}
  \leanok
  \dcref{custom}

  \uses{mk-lem:isiso_eps_restrictscalars_appiso}
  ...
  \]
  the inverse of the (invertible, \ref{mk-lem:isiso_eps_restrictscalars_appiso}) lax-monoidal
  unit \(\varepsilon\), carrying the restriction-of-scalars of the \(X\)-unit section to the
  \(Y\)-unit section. It is the leg-(B) ingredient of \ref{mk-lem:slice_dual_transport}.
\end{definition}
\begin{proof}
  \leanok
  \uses{mk-lem:isiso_eps_restrictscalars_appiso}
...
\begin{definition}[Leg-(B) unit ring-iso swap, reverse (\(\varepsilon\) itself)]\label{mk-def:dual_unit_ring_swap_inv}
...
\begin{definition}[Reverse leg codomain unit ring-iso swap, \(\mathrm{hom}\)-direction]\label{mk-def:dual_unit_ring_swap_hom}
  \lean{AlgebraicGeometry.Scheme.Modules.dualUnitRingSwapHom}
  \leanok
  \dcref{custom}

  \uses{mk-lem:isiso_eps_restrictscalars_appiso_hom}
  ...
  \]
  the inverse of the (invertible, \ref{mk-lem:isiso_eps_restrictscalars_appiso_hom}) unit
  \(\varepsilon\) in the \(\mathrm{hom}\)-direction. It is the leg-(B) ingredient of the
  reverse transport \ref{mk-lem:slice_dual_transport_inv}.
\end{definition}
\begin{proof}
  \leanok
  \uses{mk-lem:isiso_eps_restrictscalars_appiso_hom}
...
\begin{definition}[Unit-section relabel swap (the fourth leg of the reverse transport)]\label{mk-def:unit_relabel_swap}
  \lean{AlgebraicGeometry.Scheme.Modules.unitRelabelSwap}
  \leanok
  \dcref{custom}

  \uses{mk-lem:isiso_eps_restrictscalars_presheafmap}
  ...
  \]
  the inverse of the (invertible, \ref{mk-lem:isiso_eps_restrictscalars_presheafmap}) unit
  ...
  \ref{mk-lem:slice_dual_transport_inv} --- the unit-section mirror of
  \ref{mk-def:dual_unit_ring_swap_hom}, needed because the source relabel
...
\begin{lemma}[Round-trip of the swap pair, \(\mathrm{inv}\circ\mathrm{swap}\)]\label{mk-lem:dual_unit_ring_swap_comp_inv}
  \lean{AlgebraicGeometry.Scheme.Modules.dualUnitRingSwap_comp_dualUnitRingSwapInv}
  \leanok
  \dcref{custom}

  \uses{mk-def:dual_unit_ring_swap, mk-def:dual_unit_ring_swap_inv}
...
\begin{proof}
  \leanok
  \uses{mk-def:dual_unit_ring_swap, mk-def:dual_unit_ring_swap_inv}
the inverse-isomorphism identity gives the cancellation of
  \(\varepsilon\) with its inverse.
\end{proof}

\begin{lemma}[Round-trip of the swap pair, \(\mathrm{swap}\circ\mathrm{inv}\)]\label{mk-lem:dual_unit_ring_swap_inv_comp}
...
\begin{definition}[Section equivalence for the dual of the unit]\label{mk-def:unit_dual_section_equiv}
  \lean{PresheafOfModules.unitDualSectionEquiv}
  \leanok
  \dcref{custom}

  \uses{mk-def:eval_lin, mk-def:global_smul, mk-def:presheaf_internal_hom_slice_value}
  ...
  (\ref{mk-def:global_smul}).
\end{definition}
\begin{proof}
  \leanok
  \uses{mk-def:eval_lin, mk-def:global_smul, mk-def:presheaf_internal_hom_slice_value}
 The forward map is \(\mathtt{evalLin}\) at \(1\)
  (\ref{mk-def:eval_lin}); the inverse is the global-scalar action \(\mathtt{globalSMul}\)
  (\ref{mk-def:global_smul}). The substantive field is \(\mathtt{left\_inv}\): every
  ...
  (\ref{mk-def:presheaf_internal_hom_slice_value}).
\end{proof}

\begin{definition}[Presheaf dual of the unit is the unit (general base)]\label{mk-def:dual_unit_iso_gen}
  \lean{PresheafOfModules.dualUnitIsoGen}
  \leanok
  \dcref{custom}

  \uses{mk-def:unit_dual_section_equiv, mk-def:presheaf_dual, mk-lem:internal_hom_eval}
  ...
  \ref{mk-def:unit_dual_section_equiv} with the evaluation-at-\(1\) naturality.
\end{definition}
\begin{proof}
  \leanok
  \uses{mk-def:unit_dual_section_equiv, mk-def:presheaf_dual, mk-lem:internal_hom_eval}
 The sectionwise components are the section equivalences
  \ref{mk-def:unit_dual_section_equiv} packaged by \(\mathtt{isoMk}\); the naturality square is
  the evaluation-at-\(1\) naturality of the internal-hom evaluation
  (\ref{mk-lem:internal_hom_eval}) specialised at \(M = \mathbf 1\).
\end{proof}

\begin{definition}[Scheme-level presheaf dual of the unit is the unit]\label{mk-def:presheaf_dual_unit_iso}
  \lean{AlgebraicGeometry.Scheme.Modules.presheafDualUnitIso}
  \leanok
  \dcref{custom}

  \uses{mk-def:dual_unit_iso_gen}
  ...
  \ref{mk-def:dual_unit_iso_gen} at \(R_0 = Y.\mathtt{presheaf}\). It is the presheaf core of
  the third leg \(\mathtt{dual\_unit\_iso}\) (\ref{mk-lem:dual_unit_iso}).
\end{definition}
\begin{proof}
  \leanok
  \uses{mk-def:dual_unit_iso_gen}
...
\begin{lemma}[Reverse slice dual transport (the \(\mathtt{invFun}\) of \ref{mk-lem:slice_dual_transport})]\label{mk-lem:slice_dual_transport_inv}
  \lean{AlgebraicGeometry.Scheme.Modules.sliceDualTransportInv}
  \leanok
  \dcref{custom}

  \uses{mk-def:dual_unit_ring_swap_hom, mk-lem:image_preimage_of_le, mk-lem:isiso_eps_restrictscalars_appiso_hom, mk-lem:restrictscalars_ringiso_dualequiv, mk-def:presheaf_dual_unit_iso, mk-def:unit_relabel_swap}
  \textit{Source: internal categorical construction; no external reference.}
  In the setting of \ref{mk-lem:slice_dual_transport}, with \(f : Y \hookrightarrow X\) an open
  ...
  of the forward equivalence \ref{mk-lem:slice_dual_transport}.
\end{lemma}
\begin{proof}
  \leanok
  \uses{mk-def:dual_unit_ring_swap_hom, mk-lem:image_preimage_of_le, mk-lem:isiso_eps_restrictscalars_appiso_hom, mk-lem:restrictscalars_ringiso_dualequiv, mk-def:presheaf_dual_unit_iso, mk-lem:slice_dual_transport_inv_naturality_apply}
  ...
  \ref{mk-lem:slice_dual_transport} with two changes.
  ...
  (\ref{mk-lem:image_preimage_of_le}); this is the single set-theoretic fact the inverse
  ...
  its codomain swap is \(\mathtt{dualUnitRingSwapHom}\) (\ref{mk-def:dual_unit_ring_swap_hom}),
  the inverse of \(\varepsilon\) in the \(\mathrm{hom}\)-direction
  (\ref{mk-lem:isiso_eps_restrictscalars_appiso_hom}). The fourth leg
  ...
  (steps (i)--(iii) of \ref{mk-lem:slice_dual_transport}): identify the two module
  ...
  identification \ref{mk-def:presheaf_dual_unit_iso}. The whole family is reindexed by the
  inverse of the down-set bijection \(W'' \leftrightarrow f^{-1}(W'')\) supplied by
  \ref{mk-lem:image_preimage_of_le}.
\end{proof}

\begin{lemma}[Pointwise naturality square of the reverse slice transport]\label{mk-lem:slice_dual_transport_inv_naturality_apply}
  \lean{AlgebraicGeometry.Scheme.Modules.sliceDualTransportInv_naturality_apply}
  \leanok
  \dcref{custom}

  \uses{mk-def:dual_unit_ring_swap_hom, mk-lem:image_preimage_of_le}
  \textit{Source: internal categorical construction; no external reference.}
  With notation as in \ref{mk-lem:slice_dual_transport_inv}, the reverse-transport component
...
\begin{lemma}[Clean pointwise form of the reverse-transport component]\label{mk-lem:slice_dual_transport_inv_app_apply}
  \lean{AlgebraicGeometry.Scheme.Modules.sliceDualTransportInv_app_apply}
  \leanok
  \dcref{custom}

  \uses{mk-def:dual_unit_ring_swap_hom}
  \textit{Source: internal categorical construction; no external reference.}
  ...
  round-trip proofs of \ref{mk-lem:slice_dual_transport} to expose the inverse leg in clean form
  before the \(\mathtt{appIso}\) round-trip cancellation.
\end{lemma}
\begin{proof}
  \leanok
  \uses{mk-lem:slice_dual_transport_inv}
...
\begin{lemma}[Clean pointwise form of the forward-transport component]\label{mk-lem:slice_dual_transport_app_apply}
  \lean{AlgebraicGeometry.Scheme.Modules.sliceDualTransport_app_apply}
  \leanok
  \dcref{custom}

  \uses{mk-def:dual_unit_ring_swap, mk-lem:slice_dual_transport}
  \textit{Source: internal categorical construction; no external reference.}
  The \(\mathtt{app}\) component of \(\mathtt{sliceDualTransport}\,f\,M\,V\) --- the forward per-slice
  dual-transport family of \ref{mk-lem:slice_dual_transport} --- evaluated at a dual section
  ...
  codomain unit ring-swap \(\mathtt{dualUnitRingSwap}\,f\,W'\) (\ref{mk-def:dual_unit_ring_swap}, the
  ...
  the forward leg in clean form --- the mirror of \ref{mk-lem:slice_dual_transport_inv_app_apply} ---
  and is the sectionwise tool that closes the pushforward-flank residual of
  \ref{mk-lem:presheafdual_pullback_comparison_restrict}.
\end{lemma}
\begin{proof}
  \leanok
  \uses{mk-lem:slice_dual_transport}
...
\begin{lemma}[Action of the codomain unit ring-swap]\label{mk-lem:dual_unit_ring_swap_apply}
  \lean{AlgebraicGeometry.Scheme.Modules.dualUnitRingSwap_apply}
  \leanok
  \dcref{custom}

  \uses{mk-def:dual_unit_ring_swap}
  \textit{Source: internal categorical construction; no external reference.}
  The underlying map of the codomain unit ring-swap \(\mathtt{dualUnitRingSwap}\,f\,W'\)
  (\ref{mk-def:dual_unit_ring_swap}) is the \(\mathrm{hom}\)-direction of the open-immersion structure-ring
...
\begin{lemma}[Open-immersion structure-ring cocycle]
  \label{mk-lem:scheme_modules_comp_appiso}
...
\begin{lemma}[Sectionwise split of the composite slice dual transport via the \(\mathtt{appIso}\) cocycle]\label{mk-lem:slice_dual_transport_comp}
  \lean{AlgebraicGeometry.Scheme.Modules.sliceDualTransport_comp}
  \leanok
  \dcref{custom}

  \uses{mk-lem:slice_dual_transport, mk-lem:slice_dual_transport_app_apply, mk-def:dual_unit_ring_swap, mk-lem:dual_unit_ring_swap_apply, mk-lem:scheme_modules_comp_appiso}
  \textit{Source: internal categorical construction; dual of the proven tensor base-change \ref{mk-lem:pullback_tensor_map_basechange} (statement anchor: internal-hom base-change, Stacks Modules ch.~17).}
  ...
  law for the forward slice dual transport (\ref{mk-lem:slice_dual_transport}): evaluated sectionwise, the
  ...
  \ref{mk-lem:scheme_modules_comp_appiso}). In words: the reindexed component \(\varphi.\mathtt{app}\) at
  ...
  cocycle \(\mathtt{comp\_appIso}\) (\ref{mk-lem:scheme_modules_comp_appiso}) reindexed by
  \(\mathtt{comp\_image}\).
\end{lemma}
\begin{proof}
  \leanok
  \uses{mk-lem:slice_dual_transport_app_apply, mk-lem:dual_unit_ring_swap_apply, mk-lem:scheme_modules_comp_appiso}
  Evaluate both sides at \(W\) and the section \(z\). The forward apply lemma
  \ref{mk-lem:slice_dual_transport_app_apply} rewrites the left-hand component as a single
  \(\mathtt{dualUnitRingSwap}\,(h\mathbin{;}f)\)-conjugated reindex of \(\varphi\), and its action
  \ref{mk-lem:dual_unit_ring_swap_apply} replaces that swap by the \(\mathrm{hom}\)-direction of the
  composite structure-ring iso \((h\mathbin{;}f).\mathtt{appIso}\,W'\). The cocycle \(\mathtt{comp\_appIso}\)
  (\ref{mk-lem:scheme_modules_comp_appiso}) factors that composite iso as the \(\mathtt{eqToIso}\)-twisted
  product
  ...
  \mathbin{;} h.\mathtt{appIso}\,W'\), and applying \ref{mk-lem:dual_unit_ring_swap_apply} once more to each
  ...
  Working sectionwise in the slice category --- where \ref{mk-lem:slice_dual_transport_app_apply} yields a
  definitional identity at each component --- this composition law separates from the adjunction
  unit/counit layers of \ref{mk-lem:presheafdual_pullback_comparison_restrict}.
\end{proof}

\begin{lemma}[Pointwise \(\mathrm{hom}\)-naturality of the structure ring iso]\label{mk-lem:appiso_hom_naturality_apply}
...
\begin{proof}
 ...
 It is the \(\beta_W\)-naturality of the structure-ring isomorphism,
...
\begin{lemma}[Pointwise naturality square of the forward slice transport]\label{mk-lem:slice_dual_transport_naturality_apply}
  \lean{AlgebraicGeometry.Scheme.Modules.sliceDualTransport_naturality_apply}
  \leanok
  \dcref{custom}

  \uses{mk-def:dual_unit_ring_swap, mk-lem:appiso_hom_naturality_apply}
  \textit{Source: internal categorical construction; no external reference.}
  With notation as in \ref{mk-lem:slice_dual_transport}, the forward-transport component family
  ...
  the \(\mathtt{toFun}\)-side mirror of \ref{mk-lem:slice_dual_transport_inv_naturality_apply},
...
\begin{proof}
  ...
  \(\mathtt{Subsingleton.elim}\)), and the ring-level square \ref{mk-lem:appiso_hom_naturality_apply}
  --- that \((f.\mathtt{appIso}).\mathrm{hom}\) intertwines the \(X\)- and \(Y\)-side
  restriction maps.
\end{proof}

\begin{lemma}[Restriction commutes with the dual along an open immersion (the C-bridge)]\label{mk-lem:dual_restrict_iso}
  \lean{AlgebraicGeometry.Scheme.Modules.dual_restrict_iso}
  \leanok
  \dcref{01CM,custom}

  \uses{mk-lem:internal_hom_isSheaf, mk-lem:restrictscalars_ringiso_dualequiv, mk-lem:slice_dual_transport}
  \textit{Source: \cite{stacks-project}, ``Modules on Ringed Spaces'', \S Internal Hom,
  ...
  (\ref{mk-lem:internal_hom_isSheaf}) and the dual of the restriction. (Only opens
...
\begin{proof}
  ...
  shadow on duals is exactly \ref{mk-lem:restrictscalars_ringiso_dualequiv}, carrying the
  ...
  \(\mathtt{sliceDualTransport}\,f\,M\,V\) (\ref{mk-lem:slice_dual_transport}), built
  ...
  \emph{not} carry (cf.\ \ref{mk-rem:scheme_modules_monoidal_off_path}). The atom must
  therefore be constructed directly.

  Concretely (\ref{mk-lem:slice_dual_transport}), the forward component of
  ...
  because \(\beta_W\) is a ring isomorphism, as detailed in \ref{mk-lem:slice_dual_transport}.
  ...
  \ref{mk-lem:slice_dual_transport} carries the full leg-(A)\(\circ\)(B) content. The outer
  \(\mathtt{isoMk}\) naturality square follows from the per-section family naturality
  \(\mathtt{sliceDualTransport.naturality}\) of \ref{mk-lem:slice_dual_transport}: the
  ...
  \ref{mk-lem:internal_hom_isSheaf} resolves the H1 residual. Precomposing with the stage-H1
  ...
  isomorphism from the already-established \ref{mk-lem:tensorobj_restrict_iso} by uniqueness of
...
\begin{lemma}[Dual of the structure sheaf]\label{mk-lem:dual_unit_iso}
  \lean{AlgebraicGeometry.Scheme.Modules.dual_unit_iso}
  \leanok
  \dcref{custom}

  \uses{mk-lem:internal_hom_eval, mk-lem:internal_hom_isSheaf, mk-lem:tensorobj_unit_iso}
...
\begin{proof}
  \leanok
  \uses{mk-lem:internal_hom_eval, mk-lem:internal_hom_isSheaf, mk-def:presheaf_dual_unit_iso}
  ...
  Sheafifying through the sheaf-level dual of \ref{mk-lem:internal_hom_isSheaf} yields
  \(\mathtt{dual}\,\mathcal{O}_Y \cong \mathcal{O}_Y\). The scheme-level alias of this
  presheaf evaluation-at-\(1\) isomorphism is
  \(\mathtt{presheafDualUnitIso}\) (\ref{mk-def:presheaf_dual_unit_iso}).
\end{proof}

\begin{lemma}[The dual of a line bundle is a line bundle]\label{mk-lem:dual_isLocallyTrivial}
  \lean{AlgebraicGeometry.Scheme.Modules.dual_isLocallyTrivial}
  \leanok
  \dcref{01CR,custom}

  \uses{mk-lem:internal_hom_isSheaf, mk-lem:dual_restrict_iso, mk-lem:dual_unit_iso, mk-def:scheme_modules_dual_iso_of_iso, mk-lem:restrictscalars_ringiso_dualequiv}
  \textit{Source: \cite{stacks-project}, Tag~01CR,
  ...
  \(\mathtt{dual}\,L\) of \ref{mk-lem:internal_hom_isSheaf} is again locally trivial of
...
\begin{proof}
  ...
  \begin{enumerate}
    \item The first isomorphism is the restriction-compatibility isomorphism
      \ref{mk-lem:dual_restrict_iso}: restriction along the open immersion \(f\) commutes
      ...
      (\ref{mk-lem:restrictscalars_ringiso_dualequiv}), as detailed there.
    \item The second isomorphism is the dual applied to the trivialisation
      ...
      \ref{mk-def:scheme_modules_dual_iso_of_iso}): an isomorphism \(L|_U \cong \mathcal{O}_U\)
      induces \(\mathtt{dual}(L|_U) \cong \mathtt{dual}\,\mathcal{O}_U\).
    \item The third isomorphism trivialises the dual of the unit,
      \(\mathtt{dual}\,\mathcal{O}_U \cong \mathcal{O}_U\) (\ref{mk-lem:dual_unit_iso}):
...
\begin{lemma}[Local isomorphisms of \(\mathcal{O}_X\)-modules glue to a global isomorphism]\label{mk-lem:isiso_of_isiso_restrict}
...
\begin{lemma}[Down-set image identity \(\iota_!(\iota^{-1}V) = V\)]\label{mk-lem:image_preimage_of_le}
  ...
  (\ref{mk-lem:slice_dual_transport_inv}).
\end{lemma}
\begin{proof}
  \leanok
...
\begin{definition}[Local section of the hom-sheaf from a compatible family]\label{mk-def:scheme_modules_hom_local_section}
  \lean{AlgebraicGeometry.Scheme.Modules.homLocalSection}
  \leanok
  \dcref{custom}

  \uses{mk-lem:image_preimage_of_le}
...
\begin{definition}[Top section to global hom]\label{mk-def:scheme_modules_top_section_to_hom}
...
\begin{lemma}[Sectionwise value of `topSectionToHom`]\label{mk-lem:scheme_modules_top_section_to_hom_app}
  \lean{AlgebraicGeometry.Scheme.Modules.topSectionToHom_app}
  \leanok
  \dcref{custom}

  \uses{mk-def:scheme_modules_top_section_to_hom}
...
\begin{definition}[Promote an \(\mathcal{O}_X\)-linear presheaf morphism to a module morphism]\label{mk-def:scheme_modules_homMk}
...
\begin{lemma}[Compatible local morphisms glue to a global morphism]\label{mk-lem:sheafofmodules_hom_of_local_compat}
  \lean{AlgebraicGeometry.Scheme.Modules.homOfLocalCompat}
  \leanok
  \dcref{custom}

  \uses{mk-def:scheme_modules_homMk, mk-def:scheme_modules_hom_local_section, mk-def:scheme_modules_top_section_to_hom, mk-lem:scheme_modules_top_section_to_hom_app, mk-lem:image_preimage_of_le}
...
[The glued morphism restricts to its local datum]
  \label{mk-lem:hom_of_local_compat_restrict}
  \lean{}
  \lean{AlgebraicGeometry.Scheme.Modules.homOfLocalCompat_restrictFunctor_map}
  \dcref{custom}

  \uses{mk-lem:sheafofmodules_hom_of_local_compat, mk-lem:scheme_modules_top_section_to_hom_app}
  In the situation of \ref{mk-lem:sheafofmodules_hom_of_local_compat} --- a cover \(\{U_i\}\)
  ...
  \ref{mk-lem:isiso_of_isiso_restrict}: when every \(f_i\) is an isomorphism, this identity
...
\begin{proof}
  ...
  faithful (\ref{mk-lem:scheme_modules_top_section_to_hom_app}), and the section-to-morphism
...
[How the dual infrastructure discharges the \(\otimes\)-inverse]
  \label{mk-rem:dual_discharges_inverse}
  \dcref{custom}
  \uses{mk-lem:tensorobj_inverse_invertible, mk-lem:dual_isLocallyTrivial, mk-lem:sheafofmodules_hom_of_local_compat, mk-lem:hom_of_local_compat_restrict, mk-lem:isiso_of_isiso_restrict, mk-lem:tensorobj_restrict_iso, mk-lem:tensorobj_unit_iso, mk-lem:tensorobjisoofiso_trans, mk-lem:tensorobjisoofiso_refl, mk-lem:dualisoofiso_trans, mk-lem:dualisoofiso_refl, mk-lem:trivialisation_restrict_compat, mk-lem:tensorobj_unit_self_duality_collapse}
  Once the dual package \ref{mk-lem:dual_isLocallyTrivial} and the A-bridge
  \ref{mk-lem:sheafofmodules_hom_of_local_compat} are available,
  \ref{mk-lem:tensorobj_inverse_invertible} is discharged by taking \(L^{-1} := \mathtt{dual}\,L\)
  (\ref{mk-lem:internal_hom_isSheaf}), which is a line
  bundle by \ref{mk-lem:dual_isLocallyTrivial}. The global contraction \(\varepsilon_L
  ...
  restriction-compatibility iso \ref{mk-lem:tensorobj_restrict_iso} carries
  ...
  \ref{mk-lem:tensorobj_unit_iso}, an isomorphism. These local contractions are canonical,
  hence agree on overlaps, and so glue --- by
  \ref{mk-lem:sheafofmodules_hom_of_local_compat} --- to a single global morphism
  ...
  \mathcal{O}_X\) by the B-connector \ref{mk-lem:isiso_of_isiso_restrict}.
  ...
  Two conditions make the gluing of \ref{mk-lem:sheafofmodules_hom_of_local_compat} applicable.
  ...
  \ref{mk-lem:tensorobj_restrict_iso}, \(\mathtt{tensorObjIsoOfIso}\), \(\mathtt{dualIsoOfIso}\),
  and \ref{mk-lem:tensorobj_unit_iso}, so the transition isomorphism \(g_{ij}\) entering
  ...
  \(\mathtt{tensorObjIsoOfIso}\) (\ref{mk-lem:tensorobjisoofiso_trans}, \ref{mk-lem:tensorobjisoofiso_refl})
  and contravariant functoriality of \(\mathtt{dualIsoOfIso}\) (\ref{mk-lem:dualisoofiso_trans},
  \ref{mk-lem:dualisoofiso_refl}). Pushing both legs to a pure tensor \(a \otimes b\), these laws
  ...
  \ref{mk-lem:hom_of_local_compat_restrict}, the restriction of \(\varepsilon_L\) to \(U_x\)
  equals the local isomorphism \(f_x\), so \ref{mk-lem:isiso_of_isiso_restrict} upgrades
  \(\varepsilon_L\) to a global isomorphism.
\end{remark}

The two mechanism steps named in the overlap-compatibility argument of
\ref{mk-rem:dual_discharges_inverse} are isolated below as formal target lemmas, so the
cocycle obligation (residual~A) of \ref{mk-lem:tensorobj_inverse_invertible} has a named
target for each. The iso-transport operators
...
(\ref{mk-lem:tensorobjisoofiso_trans}, \ref{mk-lem:dualisoofiso_trans},
\ref{mk-lem:tensorobj_unit_iso}).

The five restriction-naturality squares constituting \ref{mk-lem:trivialisation_restrict_compat}
...
(\ref{mk-lem:image_preimage_of_le}); write \(\varrho\) for the canonical equality-of-opens
...
(\ref{mk-lem:pullback_tensor_map}) --- transported from the pullback world onto the
restriction world along the comparison \(\mathtt{restrictFunctorIsoPullback}\,f :
\mathtt{restrict}\,f \cong \mathtt{pullback}\,f\) (\ref{mk-def:restrictfunctorisopullback})
and reindexed by \(\varrho\). For an open immersion \(f\) the comparison
\(\mathtt{pullbackTensorMap}\,f\) is invertible
(\ref{mk-lem:pullback_tensor_map_isiso_open_immersion}, a public witness), so every
...
(\ref{mk-lem:tensorobj_restrict_iso_eq_pullback_tensor_map}) identifies
...
(\ref{mk-lem:pushforward_mu_appiso_collapse}) together with uniqueness of left adjoints.
\emph{Bridge B2} (\ref{mk-lem:restrictfunctorisopullback_comp_compat}) is the reindex
...
identifying \(\varrho = \mathtt{restrictCompReindex}\,j\) (\ref{mk-def:restrictcompreindex}) with
...
(\ref{mk-lem:pullback_tensor_map_basechange}) together with naturality
\(\mathtt{pullbackTensorMap\_natural}\) (\ref{mk-lem:pullback_tensor_map_natural}),
...
\(\mathtt{pullbackObjUnitToUnit\_comp}\) (\ref{mk-lem:pullbackObjUnitToUnit_comp}) re-wrapped by the
...
\begin{definition}[Restriction-composite reindexing \(\varrho\)]\label{mk-def:restrictcompreindex}
...
\begin{definition}[Unit-restriction identification \(u_\iota\)]\label{mk-def:unitrestrictiso}
  ...
  structure-sheaf comparison \(\mathtt{pullbackUnitIso}\) (\ref{mk-lem:pullback_unit_iso}). It is the
  unit identification on the chart-scheme side of S4a/S4b and the final leg \(u_\iota^{-1}\) of S4c.
\end{definition}

\begin{lemma}[Bridge B2, general literal composite: \(\mathtt{restrictFunctorIsoPullback}\) across a composite of immersions]\label{mk-lem:restrictfunctorisopullback_comp_general}
  \lean{AlgebraicGeometry.Scheme.Modules.restrictFunctorIsoPullback_comp_general}
  \leanok
  \dcref{custom}

  \uses{mk-def:restrictfunctorisopullback, mk-lem:scheme_modules_restrictfunctorcomp}
  \textit{Source: internal categorical construction; no external reference.}
  ...
  \(\mathtt{restrictFunctorIsoPullback}\) (\ref{mk-def:restrictfunctorisopullback}) is pseudonatural
  ...
  (\ref{mk-lem:scheme_modules_restrictfunctorcomp}) onto the pullback-world composition isomorphism
  ...
  \(\mathtt{restrictCompReindex}\) of \ref{mk-def:restrictcompreindex}), so that \emph{no}
...
\begin{proof}
  ...
  \ref{mk-lem:restrictfunctorisopullback_comp_compat} is its specialisation along
  \(j \mathbin{;} \iota_U = \iota_V\), where the additional congruence legs appear.
\end{proof}

\begin{lemma}[Bridge B2: reindex coherence of \(\mathtt{restrictFunctorIsoPullback}\)]\label{mk-lem:restrictfunctorisopullback_comp_compat}
  \lean{AlgebraicGeometry.Scheme.Modules.restrictFunctorIsoPullback_comp_compat}
  \leanok
  \dcref{custom}

  \uses{mk-def:restrictcompreindex, mk-lem:chart_isiso, mk-lem:image_preimage_of_le, mk-lem:restrictfunctorisopullback_comp_compat_hom}
  ...
  \(\varrho = \mathtt{restrictCompReindex}\,j\) (\ref{mk-def:restrictcompreindex}) corresponds, under
  ...
  \(j \mathbin{;} \iota_U = \iota_V\) (\ref{mk-lem:image_preimage_of_le}).
\end{lemma}
\begin{proof}
  \leanok
  \uses{mk-def:restrictcompreindex, mk-lem:chart_isiso, mk-lem:adjunction_leftadjointuniq_trans, mk-lem:restrictfunctorisopullback_comp_compat_hom, mk-lem:unit_leftadjointuniq_hom_app, mk-lem:conjugate_equiv_restrictfunctorcomp_inv, mk-lem:image_preimage_of_le}
  ...
  identity is already used in \ref{mk-lem:chart_isiso}; the present statement promotes it to an
  ...
  \ref{mk-lem:restrictfunctorisopullback_comp_compat_hom}, evaluated at \(A\). That factorisation is
  ...
  \emph{leg-by-leg} through \(\mathtt{conjugateEquiv\_comp}\) (\ref{mk-lem:conjugateequiv_comp}): every
  ...
  \(\mathtt{restrictFunctorIsoPullback}\,\iota_V\) (\ref{mk-lem:unit_leftadjointuniq_hom_app}),
  together with the transitivity \(\mathtt{leftAdjointUniq\_trans}\)
  (\ref{mk-lem:adjunction_leftadjointuniq_trans}) aligning the two transports of the comparison.
\end{proof}
\begin{lemma}[Bridge B2, hom level: leg-by-leg factorisation of \(\mathtt{restrictFunctorIsoPullback}\)]\label{mk-lem:restrictfunctorisopullback_comp_compat_hom}
  \lean{AlgebraicGeometry.Scheme.Modules.restrictFunctorIsoPullback_comp_compat_hom}
  \leanok
  \dcref{custom}

  \uses{mk-def:restrictcompreindex, mk-lem:conjugateequiv_comp, mk-lem:conjugate_equiv_restrictfunctorcomp_inv,
    mk-lem:conjugateequiv_pullbackcomp_hom, mk-lem:conjugateequiv_restrictfunctorisopullback_whiskerright,
    mk-lem:conjugateequiv_restrictfunctorisopullback_whiskerleft, mk-lem:conjugateequiv_reindexcongr,
    mk-lem:pushforwardcomp_reindex_telescope, mk-lem:unit_leftadjointuniq_hom_app, mk-lem:image_preimage_of_le,
    mk-lem:conjugateequiv_restrictfunctorisopullback_hom}
  ...
  (\ref{mk-lem:restrictfunctorisopullback_comp_compat}) reduces.
\end{lemma}
\begin{proof}
  \leanok
  \uses{mk-def:restrictcompreindex, mk-lem:conjugateequiv_comp, mk-lem:conjugate_equiv_restrictfunctorcomp_inv,
    mk-lem:conjugateequiv_pullbackcomp_hom, mk-lem:conjugateequiv_restrictfunctorisopullback_whiskerright,
    mk-lem:conjugateequiv_restrictfunctorisopullback_whiskerleft, mk-lem:conjugateequiv_reindexcongr,
    mk-lem:pushforwardcomp_reindex_telescope, mk-lem:image_preimage_of_le,
    mk-lem:conjugateequiv_restrictfunctorisopullback_hom}
  ...
  (\ref{mk-lem:conjugateequiv_comp}); no horizontal-composite mate identity is required. Each leg is a
  known pushforward-world map:
  \begin{itemize}
    \item \(c_2 \mapsto (\mathtt{pushforwardComp}\,j\,\iota_U).\mathrm{hom}\) by the keystone
      \ref{mk-lem:conjugate_equiv_restrictfunctorcomp_inv};
    \item \(c_5 \mapsto (\mathtt{pushforwardComp}\,j\,\iota_U).\mathrm{inv}\) by
      \ref{mk-lem:conjugateequiv_pullbackcomp_hom};
    \item \(c_3, c_4 \mapsto\) the pushforward-whiskered units of \(\iota_U\) and \(j\) by
      \ref{mk-lem:conjugateequiv_restrictfunctorisopullback_whiskerright} and
      \ref{mk-lem:conjugateequiv_restrictfunctorisopullback_whiskerleft};
    \item \(c_1, c_6 \mapsto\) the pushforward congruences along
      \(j \mathbin{;} \iota_U = \iota_V\) by \ref{mk-lem:conjugateequiv_reindexcongr}.
  \end{itemize}
  The resulting reversed product of pushforward-world legs telescopes to
  \(\mathbf{1}_{\mathtt{pushforward}\,\iota_V}\) by \ref{mk-lem:pushforwardcomp_reindex_telescope}: the
  two \(\mathtt{pushforwardComp}\) legs are mutually inverse, and the unit and congruence legs cancel
  against the reindexing \(j \mathbin{;} \iota_U = \iota_V\) (\ref{mk-lem:image_preimage_of_le}). Both
...
(\ref{mk-lem:h1inv_app_eq_pullbackval_restrict}).

\begin{lemma}[\(c_5\): conjugate of the pullback-composition hom]\label{mk-lem:conjugateequiv_pullbackcomp_hom}
  \lean{AlgebraicGeometry.Scheme.Modules.conjugateEquiv_pullbackComp_hom}
  \leanok
  \dcref{custom}

  \uses{mk-lem:conjugate_equiv_pullbackcomp_inv}
...
\begin{proof}
  ...
  \((\mathtt{pushforwardComp}\,f\,g).\mathrm{hom}\) (\ref{mk-lem:conjugate_equiv_pullbackcomp_inv}).
...
\begin{lemma}[LHS-collapse: the conjugate of the comparison hom is the identity]\label{mk-lem:conjugateequiv_restrictfunctorisopullback_hom}
  \lean{AlgebraicGeometry.Scheme.Modules.conjugateEquiv_restrictFunctorIsoPullback_hom}
  \leanok
  \dcref{custom}

  \uses{mk-def:restrictfunctorisopullback}
...
\begin{lemma}[\(c_3\): conjugate of the \(\iota_U\)-comparison whiskered by \(\mathtt{restrict}\,j\)]\label{mk-lem:conjugateequiv_restrictfunctorisopullback_whiskerright}
  \lean{AlgebraicGeometry.Scheme.Modules.conjugateEquiv_restrictFunctorIsoPullback_whiskerRight}
  \leanok
  \dcref{custom}

  \uses{mk-def:restrictfunctorisopullback, mk-lem:unit_leftadjointuniq_hom_app, mk-lem:conjugateequiv_comp,
    mk-lem:conjugateequiv_restrictfunctorisopullback_hom}
...
\begin{proof}
  \leanok
  \uses{mk-def:restrictfunctorisopullback, mk-lem:unit_leftadjointuniq_hom_app}
  ...
  triangle \(\mathtt{unit\_leftAdjointUniq\_hom\_app}\) (\ref{mk-lem:unit_leftadjointuniq_hom_app}); the
...
\begin{lemma}[\(c_4\): conjugate of the \(j\)-comparison whiskered into \(\mathtt{pullback}\,\iota_U\)]\label{mk-lem:conjugateequiv_restrictfunctorisopullback_whiskerleft}
...
\begin{proof}
  \leanok
  \uses{mk-def:restrictfunctorisopullback, mk-lem:unit_leftadjointuniq_hom_app}
  As in \ref{mk-lem:conjugateequiv_restrictfunctorisopullback_whiskerright}, the comparison
  ...
  (\ref{mk-lem:unit_leftadjointuniq_hom_app}). The \(\mathtt{whiskerLeft}\) by \(\mathtt{pullback}\,\iota_U\)
  threads this datum through the first factor of \(G_3, G_4\), giving the displayed map.
\end{proof}

\begin{lemma}[\(c_1, c_6\): conjugate of the reindex congruences along \(j \mathbin{;} \iota_U = \iota_V\)]\label{mk-lem:conjugateequiv_reindexcongr}
  ...
  (\ref{mk-lem:image_preimage_of_le}). Under conjugation they map to the corresponding
...
\begin{lemma}[Telescope of the reindexed pushforward legs]
  \label{mk-lem:pushforwardcomp_reindex_telescope}
  \lean{}
  \lean{}
  \dcref{custom}
  \uses{mk-lem:conjugate_equiv_restrictfunctorcomp_inv, mk-lem:conjugateequiv_pullbackcomp_hom, mk-lem:image_preimage_of_le}
...
\begin{proof}
  ...
  (\ref{mk-lem:image_preimage_of_le}), which are mutually inverse transports and cancel. The product
...
\begin{lemma}[Transitivity of the left-adjoint uniqueness isomorphism]
  \label{mk-lem:adjunction_leftadjointuniq_trans}
...
\begin{lemma}[Conjugation distributes over composition]
  \label{mk-lem:conjugateequiv_comp}
...
\begin{lemma}[Conjugate of the pullback-composition inverse]
  \label{mk-lem:conjugate_equiv_pullbackcomp_inv}
...
\begin{lemma}[Unit triangle of the left-adjoint uniqueness comparison]
  \label{mk-lem:unit_leftadjointuniq_hom_app}
...
\begin{lemma}[Hom-transpose of the left-adjoint uniqueness comparison is the second unit]
  \label{mk-lem:homequiv_leftadjointuniq_hom_app}
...
\begin{lemma}[Counit triangle of the left-adjoint uniqueness comparison]
  \label{mk-lem:leftadjointuniq_hom_app_counit}
...
\begin{lemma}[Generic adjunction unit/counit collapse]\label{mk-lem:adj_unit_counit_collapse}
...
\begin{lemma}[Restrict-side conjugate of the restriction-composition isomorphism]\label{mk-lem:conjugate_equiv_restrictfunctorcomp_inv}
  \lean{AlgebraicGeometry.Scheme.Modules.conjugateEquiv_restrictFunctorComp_inv}
  \leanok
  \dcref{custom}

  \uses{mk-def:restrictcompreindex}
  ...
  both Bridge B2 (\ref{mk-lem:restrictfunctorisopullback_comp_compat}) and the Bridge B1 crux
...
\begin{definition}[Restriction--pullback comparison]
  \label{mk-def:restrictfunctorisopullback}
...
(\ref{mk-lem:pullback_tensor_map_isiso_open_immersion}) and by Bridge B1
(\ref{mk-lem:tensorobj_restrict_iso_eq_pullback_tensor_map}), rests on three pieces of machinery: the
...
\begin{lemma}[H2: strong-monoidal restriction of scalars along a sectionwise iso]\label{mk-lem:restrictscalars_monoidal_of_bijective}
  ...
  tensorator \(\mu\) and lax unit \(\varepsilon\) (\ref{mk-lem:restrictscalars_laxmonoidal}) are
...
\begin{lemma}[Strong-monoidal tensorator iso]
  \label{mk-lem:functor_monoidal_muiso}
...
\begin{lemma}[Abstract \(\delta\)-conjugation across uniqueness of left adjoints]\label{mk-lem:deltaconjofmucomparison}
  \lean{AlgebraicGeometry.Scheme.Modules.deltaConjOfMuComparison}
  \leanok
  \dcref{custom}

  \uses{mk-lem:adjunction_leftadjointuniq_trans, mk-lem:mateequiv_vcomp, mk-lem:functor_monoidal_muiso}
...
\begin{proof}
  \leanok
  \uses{mk-lem:adjunction_leftadjointuniq_trans, mk-lem:mateequiv_vcomp}
  ...
  (\ref{mk-lem:mateequiv_vcomp}) and transitivity of the uniqueness comparison
  (\ref{mk-lem:adjunction_leftadjointuniq_trans}) align the two transports. The residue is exactly the
  hypothesis comparing the two lax tensorators of \(G\) on the image of \(F_1\), which closes the
  identity.
\end{proof}

\begin{lemma}[Reading \(\mathtt{IsIso}\,\delta\) off a conjugation]\label{mk-lem:isiso_oplaxdelta_of_conj}
  \lean{AlgebraicGeometry.Scheme.Modules.isIso_oplaxδ_of_conj}
  \leanok
  \dcref{custom}

  \uses{mk-lem:deltaconjofmucomparison, mk-lem:functor_monoidal_muiso}
...
\begin{proof}
  ...
  (\ref{mk-lem:functor_monoidal_muiso}) since \(F_1\) is strong monoidal. A composite of isomorphisms
  is an isomorphism.
\end{proof}

\begin{lemma}[Bridge B1 crux: sheafification intertwines the presheaf and sheaf adjoint-uniqueness comparisons]\label{mk-lem:h1inv_app_eq_pullbackval_restrict}
  \lean{AlgebraicGeometry.Scheme.Modules.H1inv_app_eq_pullbackVal_restrict}
  \leanok
  \dcref{custom}

  \uses{mk-def:restrictfunctorisopullback, mk-def:pullback_val_iso,
    mk-lem:sheafification_comp_pullback_eq_leftadjointuniq, mk-lem:leftadjointuniq_app_unit_eta,
    mk-lem:adjunction_leftadjointuniq_trans, mk-lem:conjugateequiv_comp,
    mk-lem:conjugate_equiv_restrictfunctorcomp_inv, mk-lem:conjugateequiv_pullbackcomp_hom,
    mk-lem:conjugateequiv_restrictfunctorisopullback_whiskerright,
    mk-lem:conjugateequiv_restrictfunctorisopullback_whiskerleft, mk-lem:conjugateequiv_reindexcongr,
    mk-lem:unit_leftadjointuniq_hom_app, mk-lem:sheaf_pullback_unit_forget_eq}
  ...
  (\ref{mk-def:pullback_val_iso}) is the sheaf-level pullback comparison, and the maps are transported
  ...
  \(\mathtt{tensorHom}\) in Bridge B1 (\ref{mk-lem:tensorobj_restrict_iso_eq_pullback_tensor_map}).
\end{lemma}
\begin{proof}
  \leanok
  \uses{mk-def:restrictfunctorisopullback, mk-def:pullback_val_iso,
  ...
  (\(\mathtt{unit\_leftAdjointUniq\_hom\_app}\), \ref{mk-lem:unit_leftadjointuniq_hom_app}); the right image
  is expanded by the unit formula. Rewriting \(\mathtt{pullbackValIso}\)
  (\ref{mk-def:pullback_val_iso}) and \(\mathtt{restrictFunctorIsoPullback}\)
  (\ref{mk-def:restrictfunctorisopullback}) by their \(\mathtt{leftAdjointUniq}\) presentations, and
  identifying the sheafify-then-pullback comparison with the left-adjoint-uniqueness comparison
  (\ref{mk-lem:sheafification_comp_pullback_eq_leftadjointuniq}), turns the obligation into a pure
  \(\mathtt{leftAdjointUniq}\)-coherence: the sheafification unit leg is supplied by the composite-unit
  transport \(\mathtt{leftAdjointUniqUnitEta}\) (\ref{mk-lem:leftadjointuniq_app_unit_eta}), and the
  remaining comparison reduces to the \emph{same} leg-by-leg conjugate telescope as the Bridge B2
  helper (\ref{mk-lem:restrictfunctorisopullback_comp_compat_hom}): the conjugation distributes through
  \(\mathtt{conjugateEquiv\_comp}\) (\ref{mk-lem:conjugateequiv_comp}) and transitivity
  \(\mathtt{leftAdjointUniq\_trans}\) (\ref{mk-lem:adjunction_leftadjointuniq_trans}), the
  \(\mathtt{restrictFunctorComp}\) factor is discharged by the keystone
  (\ref{mk-lem:conjugate_equiv_restrictfunctorcomp_inv}), and the whisker, congruence and
  pullback-composition legs are the shared atomic computations
  (\ref{mk-lem:conjugateequiv_restrictfunctorisopullback_whiskerright},
  \ref{mk-lem:conjugateequiv_restrictfunctorisopullback_whiskerleft},
  \ref{mk-lem:conjugateequiv_reindexcongr}, \ref{mk-lem:conjugateequiv_pullbackcomp_hom}).
  In the construction this comparison is split as a four-part \(\mathtt{forget}\)-transport
  of the sheaf \(\mathtt{unit\_leftAdjointUniq}\) triangle, whose genuine sheafification-boundary
  content is isolated in Part~III (\ref{mk-lem:sheaf_pullback_unit_forget_eq}).
\end{proof}

\begin{lemma}[Bridge B1 crux, Part III: the sheaf pullback unit, forget-transported, factors through sheafification]\label{mk-lem:sheaf_pullback_unit_forget_eq}
  \lean{AlgebraicGeometry.Scheme.Modules.sheafPullbackUnit_forget_eq}
  \leanok
  \dcref{custom}

  \uses{mk-def:pullback_val_iso, mk-lem:sheafification_comp_pullback_eq_leftadjointuniq}
  Let \(f : Y \hookrightarrow X\) be an open immersion presented by the presheaf-ring map
  ...
  the pullback comparison \(\mathtt{pullbackValIso}\) (\ref{mk-def:pullback_val_iso}):
  \[
  ...
  crux \(H_1\) (\ref{mk-lem:h1inv_app_eq_pullbackval_restrict}); the other three parts of that crux are
  definitional \(\mathtt{forget}\)/\(\mathtt{pushforward}\) functoriality identities.
\end{lemma}
\begin{proof}
  \leanok
  \uses{mk-def:pullback_val_iso, mk-lem:sheafification_comp_pullback_eq_leftadjointuniq,
    mk-lem:unit_leftadjointuniq_hom_app}
    The identity the identity is, after computing the opaque sheaf unit via the
  ...
  \begin{enumerate}
    ...
    \item Feed the defining unit triangle of the composite comparison,
      ...
      (\ref{mk-lem:unit_leftadjointuniq_hom_app}), where
      ...
      \(\mathtt{sheafificationCompPullback}\), \ref{mk-lem:sheafification_comp_pullback_eq_leftadjointuniq}).
  ...
  three parts of the crux \(H_1\) (\ref{mk-lem:h1inv_app_eq_pullbackval_restrict}) are definitional
  \(\mathtt{forget}\)/\(\mathtt{pushforward}\) functoriality identities.
\end{proof}

\begin{lemma}[Bridge B1: \(\mathtt{tensorObj\_restrict\_iso}\) is the conjugated base-change comparison]\label{mk-lem:tensorobj_restrict_iso_eq_pullback_tensor_map}
  \lean{AlgebraicGeometry.Scheme.Modules.tensorObj_restrict_iso_eq_pullbackTensorMap}
  \leanok
  \dcref{custom}

  \uses{mk-lem:tensorobj_restrict_iso, mk-lem:pullback_tensor_map, mk-lem:pullback_tensor_map_isiso_open_immersion, mk-lem:isiso_pullbacktensormap_of_sheafifydelta, mk-lem:pushforward_mu_appiso_collapse, mk-def:restrictfunctorisopullback}
  ...
  comparison and \(\mathtt{pullbackTensorMap}\,f\) (\ref{mk-lem:pullback_tensor_map}) is an
  isomorphism for the open immersion \(f\)
  (\ref{mk-lem:pullback_tensor_map_isiso_open_immersion}).
\end{lemma}
\begin{proof}
  \leanok
  \uses{mk-lem:tensorobj_restrict_iso, mk-lem:pullback_tensor_map, mk-lem:pullback_tensor_map_isiso_open_immersion, mk-lem:isiso_pullbacktensormap_of_sheafifydelta, mk-lem:pushforward_mu_appiso_collapse, mk-lem:deltaconjofmucomparison, mk-def:restrictfunctorisopullback, mk-lem:adjunction_leftadjointuniq_trans, mk-lem:h1inv_app_eq_pullbackval_restrict}
  ...
  \(\mathtt{tensorObj\_restrict\_iso}\,f\) (\ref{mk-lem:tensorobj_restrict_iso}) factors as the shared
  ...
  \(\mathtt{pullbackTensorMap}\,f\) (\ref{mk-lem:pullback_tensor_map}) is built from the identical
  prefix followed by the sheafified oplax tensorator
  \(\delta\bigl(\mathtt{PresheafOfModules.pullback}\,\varphi'\bigr)\)
  (\ref{mk-lem:isiso_pullbacktensormap_of_sheafifydelta}).
  ...
  (\ref{mk-lem:pushforward_mu_appiso_collapse}), which expresses the strong tensorator
  \(\delta\,G_\beta\) as the \(H_1\)-conjugate of the oplax
  \(\delta\bigl(\mathtt{pullback}\,\varphi'\bigr)\) --- the very abstract conjugation
  \(\mathtt{deltaConjOfMuComparison}\) (\ref{mk-lem:deltaconjofmucomparison}) specialised to the
  pullback adjunction --- combined with uniqueness of left adjoints
  (\(\mathtt{leftAdjointUniq\_trans}\), \ref{mk-lem:adjunction_leftadjointuniq_trans}, and naturality of
  ...
  invertible (\ref{mk-lem:pullback_tensor_map_isiso_open_immersion}), so
  ...
  (\ref{mk-def:restrictfunctorisopullback}) reassemble the displayed conjugate factorisation
...
\begin{lemma}[S2: tensor-restriction comparison commutes with further restriction]\label{mk-lem:tensorobj_restrict_iso_restrict_compat}
  \lean{AlgebraicGeometry.Scheme.Modules.tensorObj_restrict_iso_restrict_compat}
  \leanok
  \dcref{custom}

  \uses{mk-lem:tensorobj_restrict_iso, mk-lem:image_preimage_of_le}
  \textit{Source: \cite{stacks-project}, Modules on Ringed Spaces, Lemma
  ...
  (\ref{mk-lem:tensorobj_restrict_iso}). The tensor-restriction comparison is \emph{natural under
  ...
  \ref{mk-lem:image_preimage_of_le}. Equivalently, modulo \(\varrho\),
  \((\mathtt{restrict}\,j)\,T(\iota_U) = T(\iota_V)\).
\end{lemma}
\begin{proof}
  \uses{mk-lem:tensorobj_restrict_iso_eq_pullback_tensor_map, mk-lem:pullback_tensor_map_basechange, mk-lem:pullback_tensor_map_natural, mk-def:restrictcompreindex, mk-lem:image_preimage_of_le}
  \dcref{custom}
  This square is \emph{not} a bespoke leg-by-leg chase: it is base-change bookkeeping built on
  Bridge B1 (\ref{mk-lem:tensorobj_restrict_iso_eq_pullback_tensor_map}). Substitute B1 to rewrite
  ...
  (\ref{mk-lem:pullback_tensor_map_basechange}): for the factorisation
  ...
  the two module arguments (\ref{mk-lem:pullback_tensor_map_natural}) carries the per-leg comparisons
  \(\rho_M, \rho_N\) of B1 through the square. Finally the reindexing
  \(\varrho = \mathtt{restrictCompReindex}\,j\) (\ref{mk-def:restrictcompreindex}) threads the
  equality of opens \(j \mathbin{;} \iota_U = \iota_V\) (\ref{mk-lem:image_preimage_of_le}) on both
...
\begin{definition}[Cone B / presheaf dual base-change comparison \(\theta_M\) (Step~4 of \(\mathtt{dual\_restrict\_iso}\))]\label{mk-def:presheafdual_pullback_comparison}
  \lean{AlgebraicGeometry.Scheme.Modules.presheafDualPullbackComparison}
  \leanok
  \dcref{custom}

  \uses{mk-lem:slice_dual_transport, mk-def:presheaf_dual}
  \textit{Source: internal categorical construction; no external reference.}
  ...
  the pushforward \(\mathtt{pushforward}\,\beta\) (Stage~H1 of \ref{mk-lem:dual_restrict_iso}), and the
  second factor assembles the per-slice dual transport \(\mathtt{sliceDualTransport}\)
  (\ref{mk-lem:slice_dual_transport}) sectionwise over the thin poset \(\mathrm{Opens}\,Y\); the
  ...
  downstream consumers (\ref{mk-lem:presheafdual_pullback_comparison_eval_apply},
  \ref{mk-lem:presheafdual_pullback_restrict_natural}) require: the parent
  \(\mathtt{dual\_restrict\_iso}\,f\,M\) (\ref{mk-lem:dual_restrict_iso}) is refactored to consume it as
...
\begin{lemma}[Cone B / L1: the dual base-change comparison is sectionwise an internal-hom evaluation]\label{mk-lem:presheafdual_pullback_comparison_eval_apply}
  \lean{AlgebraicGeometry.Scheme.Modules.presheafDual_pullback_comparison_eval_apply}
  \leanok
  \dcref{custom}

  \uses{mk-def:presheafdual_pullback_comparison, mk-lem:internal_hom_eval, mk-def:presheaf_dual}
  \textit{Source: internal categorical construction; no external reference.}
  This is an alternative eval-based decomposition retained for completeness; it is \emph{not} on the
  critical path, as the immersion-naturality crux \ref{mk-lem:presheafdual_pullback_restrict_natural}
  ...
  comparison \(\theta_M\) (\ref{mk-def:presheafdual_pullback_comparison})
  acts as the internal-hom \emph{evaluation} of \ref{mk-lem:internal_hom_eval}: a pulled-back dual
  ...
  \(\mathtt{presheafDualUnitIso}\) in \ref{mk-lem:presheafdualunitiso_naturality}).
\end{lemma}
\begin{proof}
  \leanok
  \uses{mk-def:presheafdual_pullback_comparison, mk-lem:internal_hom_eval, mk-def:presheaf_dual}
  Unfolding \(\theta_M\) through its construction (\ref{mk-def:presheafdual_pullback_comparison}), the forward
  ...
  \ref{mk-lem:internal_hom_eval} reindexed across \(j.\mathtt{opensFunctor}\). This is a direct
  read-off of the definition, sectionwise; no naturality is required for this step.
\end{proof}

\begin{definition}[Cone B / morphism-level dual precomposition operator \(\mathtt{dualPrecompHom}\)]\label{mk-def:presheaf_dual_precomp_hom}
  \lean{PresheafOfModules.dualPrecompHom}
  \leanok
  \dcref{custom}

  \uses{mk-def:presheaf_dual, mk-def:presheaf_dual_precomp_equiv, mk-def:presheaf_dual_iso_of_iso}
  \textit{Source: internal categorical construction; no external reference.}
  ...
  (\ref{mk-def:presheaf_dual_precomp_equiv}) stripped of any appeal to an inverse: where
  ...
  \(\mathtt{dualIsoOfIso}\,e\) (\ref{mk-def:presheaf_dual_iso_of_iso}). It is the contravariant
  functoriality of the presheaf dual \(M \mapsto M^\vee\) (\ref{mk-def:presheaf_dual}) in arbitrary
  morphisms --- the operator needed to conjugate the base-change comparison \(\theta\) by the
  non-invertible restriction map.
\end{definition}

\begin{lemma}[Cone B / L2: morphism-level dual transport is precomposition by the restriction map]\label{mk-lem:dual_precomp_hom_restrict_apply}
  \lean{PresheafOfModules.dualPrecompHom_restrict_apply}
  \leanok
  \dcref{custom}

  \uses{mk-def:presheaf_dual_precomp_hom, mk-def:presheaf_dual}
  \textit{Source: internal categorical construction; no external reference.}
  With \(j : V \hookrightarrow U\) an open immersion as above, the morphism-level dual transport
  \(\mathtt{dualPrecompHom}\,(\mathtt{restrict}\,j)\) (\ref{mk-def:presheaf_dual_precomp_hom}) is, on
  ...
  \(\mathtt{dualPrecompHom}\) (\ref{mk-def:presheaf_dual_precomp_hom}) is precisely the precomposition
  ...
  for a unit automorphism, underlies \ref{mk-lem:presheaf_dualisoofiso_trans}.
\end{lemma}
\begin{proof}
  \leanok
  \uses{mk-def:presheaf_dual_precomp_hom, mk-def:presheaf_dual}
  Immediate from the definition of \(\mathtt{dualPrecompHom}\)
  (\ref{mk-def:presheaf_dual_precomp_hom}): its sectionwise value at \(\varphi\) is
...
\begin{lemma}[Cone B / L3a: internal-hom evaluation commutes with the immersion restriction map (genuine base-change atom)]\label{mk-lem:presheafdual_eval_restrict_commute_apply}
  \lean{AlgebraicGeometry.Scheme.Modules.presheafDual_eval_restrict_commute_apply}
  \leanok
  \dcref{custom}

  \uses{mk-lem:internal_hom_eval, mk-lem:image_preimage_of_le}
  \textit{Source: internal categorical construction; no external reference.}
  This is an alternative eval-based decomposition retained for completeness; it is \emph{not} on the
  critical path, as the immersion-naturality crux \ref{mk-lem:presheafdual_pullback_restrict_natural}
  closes directly by the built-in naturality of \(\theta\).
  Fix a dual section \(\varphi\) and a section \(s\). Internal-hom evaluation
  (\ref{mk-lem:internal_hom_eval}) commutes with the immersion's restriction map: evaluating then
  ...
  (\ref{mk-lem:presheafdualunitiso_naturality}). This is the genuine internal-hom base-change content ---
  the dual-flank analogue of Bridge~B1 (\ref{mk-lem:tensorobj_restrict_iso_eq_pullback_tensor_map}) ---
  and is the immersion analogue of the evaluate/restrict commutation core of
  \ref{mk-lem:presheafdualunitiso_naturality}. The
  down-set fact \(j \mathbin{;} \iota_U = \iota_V\) (\ref{mk-lem:image_preimage_of_le}) pins the open
...
\begin{proof}
  ...
  \ref{mk-lem:internal_hom_eval}: the restriction \(\varphi|_V\) of a functional inside the dual is its
...
\begin{lemma}[Cone B / L3b: pointwise naturality square of the dual base-change comparison (sectionwise form)]\label{mk-lem:presheafdual_pullback_restrict_natural_apply}
  \lean{AlgebraicGeometry.Scheme.Modules.presheafDual_pullback_restrict_natural_apply}
  \leanok
  \dcref{custom}

  \uses{mk-def:presheafdual_pullback_comparison, mk-lem:image_preimage_of_le}
  \textit{Source: internal categorical construction; no external reference.}
  ...
  \(\theta_M\) (\ref{mk-def:presheafdual_pullback_comparison}) commutes with the immersion's restriction
  map by construction. This is the immersion analogue of the eval-core of
  \ref{mk-lem:presheafdualunitiso_naturality}; the down-set factorisation
  \(j \mathbin{;} \iota_U = \iota_V\) (\ref{mk-lem:image_preimage_of_le}) pins the open along which both
...
\begin{proof}
  ...
  (\ref{mk-def:presheafdual_pullback_comparison}) commutes with the immersion's restriction map by
...
\begin{lemma}[Immersion-naturality of the presheaf dual base-change comparison (Cone B)]\label{mk-lem:presheafdual_pullback_restrict_natural}
  \lean{AlgebraicGeometry.Scheme.Modules.presheafDual_pullback_restrict_natural}
  \leanok
  \dcref{custom}

  \uses{mk-def:presheafdual_pullback_comparison, mk-def:presheaf_dual_precomp_hom, mk-lem:presheafdual_pullback_restrict_natural_apply}
  ...
  \(\theta_M\) (\ref{mk-def:presheafdual_pullback_comparison}) --- is natural in the immersion
  \(j\): conjugating \(\theta\) by the morphism-level transport \(\mathtt{dualPrecompHom}\)
  (\ref{mk-def:presheaf_dual_precomp_hom}) along \(j\) on the source matches its transport along
  \(j\) on the target. This is the immersion analogue of \ref{mk-lem:presheafdualunitiso_naturality},
  with the unit automorphism there replaced by the restriction map \((\mathtt{restrict}\,j)\).
\end{lemma}
\begin{proof}
  \leanok
  \uses{mk-def:presheafdual_pullback_comparison, mk-lem:presheafdual_pullback_restrict_natural_apply}
  ...
  (\ref{mk-def:presheafdual_pullback_comparison}): a morphism of presheaves of modules commutes with all
  ...
  \ref{mk-lem:presheafdual_pullback_restrict_natural_apply} packages this naturality of \(\theta_M\)
  directly.
\end{proof}

\begin{lemma}[Bridge dual-B1: \(\mathtt{dual\_restrict\_iso}\) is the conjugated dual base-change comparison]\label{mk-lem:dual_restrict_iso_eq_comparison}
  \lean{AlgebraicGeometry.Scheme.Modules.dual_restrict_iso_eq_comparison}
  \leanok
  \dcref{custom}

  \uses{mk-lem:dual_restrict_iso, mk-def:presheafdual_pullback_comparison}
  \textit{Source: internal categorical construction; no external reference.}
  ...
  \(\mathtt{dual\_restrict\_iso}\,f\,M\) (\ref{mk-lem:dual_restrict_iso}) factors as the shared transport
  ...
  (\ref{mk-def:presheafdual_pullback_comparison}) is the presheaf dual base-change comparison. This is
  the dual analogue of Bridge B1 (\ref{mk-lem:tensorobj_restrict_iso_eq_pullback_tensor_map}): both sides
  are the \emph{same} three-factor composite by construction, so the identity is definitional.
\end{lemma}
\begin{proof}
  \leanok
  \uses{mk-lem:dual_restrict_iso, mk-def:presheafdual_pullback_comparison}
  Definitional. The parent \(\mathtt{dual\_restrict\_iso}\,f\,M\) (\ref{mk-lem:dual_restrict_iso}) was
  refactored to \emph{consume} the Step-4 comparison \(\theta_M\)
  (\ref{mk-def:presheafdual_pullback_comparison}) as a named residual: by construction it is the composite
...
\begin{lemma}[Module-naturality of the presheaf dual base-change comparison \(\theta\) (Cone B)]\label{mk-lem:presheafdual_pullback_comparison_natural_module}
  \lean{AlgebraicGeometry.Scheme.Modules.presheafDual_pullback_comparison_natural}
  \leanok
  \dcref{custom}

  \uses{mk-def:presheafdual_pullback_comparison, mk-def:presheaf_dual_iso_of_iso, mk-def:presheaf_dual_precomp_hom, mk-lem:slice_dual_transport_app_apply}
  \textit{Source: internal categorical construction; no external reference.}
  ...
  (\ref{mk-def:presheafdual_pullback_comparison}) is natural in the \emph{module} argument \(M\):
  ...
  \]
  where \(\mathtt{dualPrecompHom}\) (\ref{mk-def:presheaf_dual_precomp_hom}) is the contravariant
  morphism-level dual transport (so that, on an isomorphism \(\phi = e\), it specialises to the
  \(\mathrm{hom}\) leg of \(\mathtt{dualIsoOfIso}\,e\), \ref{mk-def:presheaf_dual_iso_of_iso}). This is
  the module-variant naturality of \(\theta\), distinct from the immersion-variant
  \ref{mk-lem:presheafdual_pullback_restrict_natural} (natural in \(j\)).
\end{lemma}
\begin{proof}
  \leanok
  \uses{mk-def:presheafdual_pullback_comparison, mk-def:presheaf_dual_precomp_hom, mk-lem:slice_dual_transport_app_apply}
  The comparison factors (\ref{mk-def:presheafdual_pullback_comparison}) as
  ...
  (\ref{mk-lem:slice_dual_transport_app_apply}) is natural in \(M\): on each slice it reindexes the
...
\begin{lemma}[Module-naturality of \(\mathtt{dual\_restrict\_iso}\) (contravariant analogue of S2's tensor naturality)]\label{mk-lem:dual_restrict_iso_natural}
  \lean{AlgebraicGeometry.Scheme.Modules.dual_restrict_iso_natural}
  \leanok
  \dcref{custom}

  \uses{mk-lem:dual_restrict_iso, mk-lem:dual_restrict_iso_eq_comparison, mk-lem:presheafdual_pullback_comparison_natural_module, mk-def:scheme_modules_dual_iso_of_iso}
  \textit{Source: internal categorical construction; no external reference.}
  ...
  (\ref{mk-lem:dual_restrict_iso}) is natural in the module isomorphism: the \(\mathtt{restrict}\,j\)-image
  of \(\mathtt{dualIsoOfIso}\,e\) (\ref{mk-def:scheme_modules_dual_iso_of_iso}) equals
  ...
  \ref{mk-lem:tensorobj_restrict_iso_natural}; it is the missing ingredient the Seam-1 split
  (\ref{mk-lem:trivialisation_restrict_em_split}) wraps around the dual constituent, letting
  \(\mathtt{restrict}\,j\) commute past the \((\mathtt{dualIsoOfIso}\,e^M)^{-1}\) factor.
\end{lemma}
\begin{proof}
  \leanok
  \uses{mk-lem:dual_restrict_iso_eq_comparison, mk-lem:presheafdual_pullback_comparison_natural_module, mk-def:scheme_modules_dual_iso_of_iso}
  By the dual-B1 factorisation (\ref{mk-lem:dual_restrict_iso_eq_comparison}),
  ...
  \ref{mk-lem:presheafdual_pullback_comparison_natural_module}; sheafification is a functor, so it carries
  ...
  \(\mathtt{dualIsoOfIso}\) (\ref{mk-def:scheme_modules_dual_iso_of_iso}) on the isomorphism \(e\), yields
  the displayed conjugated identity.
\end{proof}

\begin{lemma}[Module-naturality of \(\mathtt{tensorObj\_restrict\_iso}\) (S2's bifunctorial naturality)]\label{mk-lem:tensorobj_restrict_iso_natural}
  \lean{AlgebraicGeometry.Scheme.Modules.tensorObj_restrict_iso_natural}
  \leanok
  \dcref{custom}

  \uses{mk-lem:tensorobj_restrict_iso, mk-lem:tensorobj_restrict_iso_eq_pullback_tensor_map, mk-lem:pullback_tensor_map_natural, mk-def:restrictfunctorisopullback, mk-def:tensorobjisoofiso}
  \textit{Source: internal categorical construction; no external reference.}
  ...
  \(\mathtt{tensorObj\_restrict\_iso}\,f\) (\ref{mk-lem:tensorobj_restrict_iso}) is natural in the two
  module isomorphisms: the \(\mathtt{restrict}\,f\)-image of \(\mathtt{tensorObjIsoOfIso}\,a\,b\)
  (\ref{mk-def:tensorobjisoofiso}) equals \(\mathtt{tensorObjIsoOfIso}\) of the \(\mathtt{restrict}\,f\)-images
  ...
  \]
  This is the covariant ingredient the Seam-1 split (\ref{mk-lem:trivialisation_restrict_em_split})
...
\begin{proof}
  \leanok
  \uses{mk-lem:tensorobj_restrict_iso_eq_pullback_tensor_map, mk-lem:pullback_tensor_map_natural, mk-def:restrictfunctorisopullback}
  ...
  pullback world via Bridge B1 (\ref{mk-lem:tensorobj_restrict_iso_eq_pullback_tensor_map}), reducing the
  square to the naturality of the pullback--tensor comparison \(\delta_{\mathrm{sheaf}}\)
  (\ref{mk-lem:pullback_tensor_map_natural}) sandwiched between two naturality squares of the
  restriction--pullback comparison \(\mathtt{restrictFunctorIsoPullback}\,f\)
  (\ref{mk-def:restrictfunctorisopullback}) --- one for \(\mathtt{restrictFunctorIsoPullback}.\mathrm{hom}\)
...
\begin{lemma}[Bridge dual-(b): composition law of \(\mathtt{sheafificationCompPullback}\) over an immersion factorisation]\label{mk-lem:sheafificationcomppullback_restrict}
  \lean{AlgebraicGeometry.Scheme.Modules.sheafificationCompPullback_restrict}
  \leanok
  \dcref{custom}

  \uses{mk-lem:sheafificationcomppullback_comp, mk-def:restrictcompreindex, mk-lem:toringcatsheafhom_comp_hom_reconcile, mk-lem:image_preimage_of_le}
  \textit{Source: internal categorical construction; no external reference.}
...
\begin{proof}
  ...
  (\ref{mk-lem:sheafificationcomppullback_comp}), which for composable \(h, f\) and \emph{any} presheaf
  ...
  \(\mathtt{toRingCatSheafHom\_comp}\) (\ref{mk-lem:toringcatsheafhom_comp_hom_reconcile}), so Sq1
  specialises to the displayed equation. Finally the reindexing
  \(\varrho = \mathtt{restrictCompReindex}\,j\) (\ref{mk-def:restrictcompreindex}) threads the equality of
  opens \(j \mathbin{;} \iota_U = \iota_V\) (\ref{mk-lem:image_preimage_of_le}) on the restriction flank,
  ...
  \ref{mk-lem:pullback_tensor_map_basechange}, the analogue of the
  \(\mathtt{pullbackValIso}\)-composition \ref{mk-lem:pullback_val_iso_comp}.
\end{proof}

\begin{definition}[Cone B / c.1: object-identification iso \((\mathtt{pushforward}\,\beta).\mathrm{obj}\,M_{\mathrm{val}} \cong (M|_f)_{\mathrm{val}}\)]\label{mk-def:pushforward_obj_val_restrict_iso}
  \lean{AlgebraicGeometry.Scheme.Modules.pushforwardObjValRestrictIso}
  \leanok
  \dcref{custom}

  \uses{mk-def:presheafdual_pullback_comparison, mk-def:restrictfunctorisopullback, mk-lem:sheafification_comp_pullback_eq_leftadjointuniq}
  \textit{Source: internal categorical construction; no external reference.}
  ...
  \ref{mk-def:presheafdual_pullback_comparison}. There is a canonical isomorphism of presheaves of modules
  over \(Y\)
  \[
...
\begin{proof}
  ...
  \ref{mk-def:presheafdual_pullback_comparison} re-presents the abstract presheaf pullback
  ...
  (\ref{mk-def:restrictfunctorisopullback}) followed by the sheafify-then-pullback identification
  (\ref{mk-lem:sheafification_comp_pullback_eq_leftadjointuniq}) relates
...
\begin{lemma}[The presheaf pullback pseudofunctoriality isomorphism \(\mathtt{pullbackComp}\)]
  \label{mk-lem:presheafofmodules_pullbackcomp}
  ...
  \ref{mk-lem:conjugate_equiv_pullbackcomp_inv}. It is leg~1 of the cocycle
  \ref{mk-lem:presheafdual_pullback_comparison_restrict}.
\end{lemma}

\begin{lemma}[Restriction along a composite as a composition of restrictions \(\mathtt{restrictFunctorComp}\)]
  \label{mk-lem:scheme_modules_restrictfunctorcomp}
  ...
  (\ref{mk-def:scheme_modules_dual_iso_of_iso}) of the forgetful image of
  \((\mathtt{restrictFunctorComp}\,h\,f).\mathrm{app}\,M\) --- is the dual-target pseudofunctoriality
  leg~4 of the cocycle \ref{mk-lem:presheafdual_pullback_comparison_restrict}.
\end{lemma}

\begin{lemma}[Mate of a \(\mathtt{leftAdjointUniq}\) comparison is the identity]\label{mk-lem:conjugate_equiv_leftadjointuniq_hom}
...
\begin{lemma}[Mate of the \(\mathtt{leftAdjointCompIso}\) hom is the reconciling inverse]\label{mk-lem:conjugate_equiv_leftadjointcompiso_hom}
  \lean{CategoryTheory.Adjunction.conjugateEquiv_leftAdjointCompIso_hom}
  \leanok
  \dcref{custom}

  \uses{mk-lem:conjugate_equiv_leftadjointuniq_hom}
  \textit{Source: internal categorical construction; no external reference.}
...
\begin{lemma}[Explicit component of the \(\mathtt{leftAdjointCompIso}\) hom]
  \label{mk-lem:leftadjointcompiso_hom_app}
  ...
  \ref{mk-lem:conjugate_equiv_leftadjointcompiso_hom}.
\end{lemma}

\begin{lemma}[Abstract \(H1\) cocycle: \(\mathtt{leftAdjointUniq}\) intertwines two \(\mathtt{leftAdjointCompIso}\)s]\label{mk-lem:leftadjointuniq_leftadjointcompiso_comm}
  \lean{CategoryTheory.Adjunction.leftAdjointUniq_leftAdjointCompIso_comm}
  \leanok
  \dcref{custom}

  \uses{mk-lem:conjugate_equiv_leftadjointuniq_hom, mk-lem:conjugate_equiv_leftadjointcompiso_hom}
  \textit{Source: internal categorical construction; no external reference.}
  ...
  \ref{mk-lem:presheafdual_h1_cocycle}, and the generic engine behind
  \(\mathtt{conjugateEquiv\_restrictFunctorComp\_inv}\)
  (\ref{mk-lem:conjugate_equiv_restrictfunctorcomp_inv}): both reduce a composite comparison to a chain
...
\begin{proof}
  ...
  \ref{mk-lem:conjugate_equiv_leftadjointuniq_hom} the first is \(\mathbf{1}\) and by
  \ref{mk-lem:conjugate_equiv_leftadjointcompiso_hom} the second is \(e.\mathrm{inv}\), so the left mate is
  ...
  \ref{mk-lem:conjugate_equiv_leftadjointuniq_hom}, and the \(PC.\mathrm{hom}\) factor becomes
  \(e.\mathrm{inv}\) by \ref{mk-lem:conjugate_equiv_leftadjointcompiso_hom}. Both mates equal \(e.\mathrm{inv}\),
...
\begin{lemma}[Cone B / c.2(a): cocycle law of the adjunction-uniqueness iso \(H1\) over the immersion factorisation]
  \label{mk-lem:presheafdual_h1_cocycle}
  \lean{AlgebraicGeometry.Scheme.Modules.presheafDualH1Cocycle}
  \leanok
  \dcref{custom}

  \uses{mk-def:presheafdual_pullback_comparison, mk-lem:presheafofmodules_pullbackcomp, mk-lem:scheme_modules_restrictfunctorcomp, mk-lem:conjugate_equiv_pullbackcomp_inv, mk-lem:adjunction_leftadjointuniq_trans, mk-lem:conjugate_equiv_restrictfunctorcomp_inv, mk-lem:leftadjointuniq_leftadjointcompiso_comm}
  \textit{Source: internal categorical construction; no external reference.}
  ...
  (Stage~H1 of \ref{mk-def:presheafdual_pullback_comparison}). Then \(H1\) satisfies a \emph{cocycle}
  ...
  (\ref{mk-lem:presheafofmodules_pullbackcomp}) and on the pushforward (target) flank by the
  restriction-composition iso \(\mathtt{restrictFunctorComp}\,h\,f\)
  (\ref{mk-lem:scheme_modules_restrictfunctorcomp}, equivalently the pushforward composition
  ...
  \ref{mk-lem:presheafdual_pullback_comparison_restrict}.
  This is the dual-flank analogue of \ref{mk-lem:conjugate_equiv_pullbackcomp_inv}
  (\(\mathtt{conjugateEquiv\_pullbackComp\_inv}\)) and of
  \(\mathtt{conjugateEquiv\_restrictFunctorComp\_inv}\)
  (\ref{mk-lem:conjugate_equiv_restrictfunctorcomp_inv}). This is the \(H1\)-flank of the cocycle
  \ref{mk-lem:presheafdual_pullback_comparison_restrict}.
\end{lemma}
\begin{proof}
  \leanok
  ...
  (\ref{mk-lem:adjunction_leftadjointuniq_trans}) once the composite pullback
  ...
  \(\mathtt{pullbackComp}\,\varphi_f\,\varphi_h\) (\ref{mk-lem:presheafofmodules_pullbackcomp}), and the
  ...
  \ref{mk-lem:scheme_modules_restrictfunctorcomp}). Because \(\mathtt{pullbackComp}\) is \emph{defined} as
  the left-adjoint composition isomorphism \(\mathtt{leftAdjointCompIso}\), the reconciliation of these
  two pseudofunctor isos on the conjugate is exactly \ref{mk-lem:conjugate_equiv_pullbackcomp_inv},
  ...
  \ref{mk-lem:leftadjointuniq_leftadjointcompiso_comm} at the three pushforward/pullback adjunction pairs,
...
\begin{lemma}[Generic \(H1\)-cancellation skeleton for the c.2 cocycle]\label{mk-lem:c2_assemble}
  \lean{AlgebraicGeometry.Scheme.Modules.c2_assemble}
  \leanok
  \dcref{custom}

  \uses{mk-lem:presheafdual_h1_cocycle, mk-lem:conjugate_equiv_leftadjointcompiso_hom, mk-lem:leftadjointuniq_leftadjointcompiso_comm}
  \textit{Source: internal categorical construction; no external reference.}
  ...
  (instantiating \ref{mk-lem:presheafdual_h1_cocycle}), the \(\mathtt{pullbackComp}\) cancellation
  ...
  \ref{mk-lem:presheafdual_pullback_comparison_restrict}; the consumer supplies its eight hypotheses at
  the concrete pushforward/pullback morphisms (the cocycle hypothesis being
  \ref{mk-lem:presheafdual_h1_cocycle}, itself \ref{mk-lem:leftadjointuniq_leftadjointcompiso_comm} at the
  three pushforward adjunctions, reconciled by \ref{mk-lem:conjugate_equiv_leftadjointcompiso_hom}).
\end{lemma}
\begin{proof}
  \leanok
  \uses{mk-lem:presheafdual_h1_cocycle, mk-lem:conjugate_equiv_leftadjointcompiso_hom, mk-lem:leftadjointuniq_leftadjointcompiso_comm}
...
\begin{lemma}[Bridge dual-(c): cocycle law of the presheaf dual base-change comparison \(\theta\) over \(\mathtt{pullbackComp}\)]
  \label{mk-lem:presheafdual_pullback_comparison_restrict}
  \lean{AlgebraicGeometry.Scheme.Modules.presheafDualPullbackComparison_restrict}
  \leanok
  \dcref{custom}

  \uses{mk-def:presheafdual_pullback_comparison, mk-lem:presheafdual_h1_cocycle, mk-lem:slice_dual_transport_app_apply, mk-def:pushforward_obj_val_restrict_iso, mk-lem:presheafofmodules_pullbackcomp, mk-lem:scheme_modules_restrictfunctorcomp, mk-def:scheme_modules_dual_iso_of_iso, mk-lem:presheafdual_pullback_restrict_natural, mk-lem:image_preimage_of_le, mk-lem:leftadjointcompiso_hom_app, mk-lem:slice_dual_transport_comp, mk-lem:conjugate_equiv_leftadjointcompiso_hom, mk-lem:c2_assemble, mk-lem:pushforwardpushforwardadj_unit_app_app_apply, mk-lem:pushforwardpushforwardadj_counit_app_app_apply}
  \textit{Source: internal categorical construction; no external reference.}
  ...
  (\ref{mk-def:presheafdual_pullback_comparison}) satisfies a \emph{cocycle} (composition) law over the
  immersion factorisation, in the \emph{exact dual shape} of the tensor composition coherence
  \ref{mk-lem:pullback_tensor_map_basechange}: the \(\iota_V\)-instance \(\theta_{\iota_V, M}\) factors
  ...
  (\ref{mk-def:pushforward_obj_val_restrict_iso}, bridge c.1) is \(\mathtt{Iso.refl}\): the underlying
  ...
  the form consumed by \ref{mk-lem:dual_restrict_iso_restrict_compat}. On the dual (target) flank the
  contravariant transport is realised by \(\mathtt{dualIsoOfIso}\)
  (\ref{mk-def:scheme_modules_dual_iso_of_iso}) of the forgetful image of the restriction-composition iso
  \((\mathtt{restrictFunctorComp}\,j\,\iota_U).\mathrm{app}\,M\)
  (\ref{mk-lem:scheme_modules_restrictfunctorcomp}); this pseudofunctoriality leg appears in place of the
  \(\varrho \otimes \varrho\) of the tensor side. This is the dual analogue of the composition role of
  \ref{mk-lem:pullback_tensor_map_natural} inside \ref{mk-lem:pullback_tensor_map_basechange}.
\end{lemma}
\begin{proof}
  \leanok
  \uses{mk-def:presheafdual_pullback_comparison, mk-lem:presheafdual_h1_cocycle, mk-lem:slice_dual_transport_app_apply, mk-def:pushforward_obj_val_restrict_iso, mk-lem:presheafofmodules_pullbackcomp, mk-lem:scheme_modules_restrictfunctorcomp, mk-def:scheme_modules_dual_iso_of_iso, mk-lem:image_preimage_of_le, mk-lem:presheafdual_pullback_restrict_natural, mk-lem:leftadjointcompiso_hom_app, mk-lem:slice_dual_transport_comp, mk-lem:conjugate_equiv_leftadjointcompiso_hom, mk-lem:c2_assemble, mk-lem:pushforwardpushforwardadj_unit_app_app_apply, mk-lem:pushforwardpushforwardadj_counit_app_app_apply, mk-lem:dual_unit_ring_swap_apply, mk-lem:scheme_modules_comp_appiso}
  The cocycle \emph{factors cleanly}: every \(H1\) factor cancels against the abstract \(H1\) cocycle
  (a) (\ref{mk-lem:presheafdual_h1_cocycle}) together with a single ordinary naturality square, leaving
  ...
  \ref{mk-def:presheafdual_pullback_comparison} as
  ...
  \begin{enumerate}
    ...
    \item \textbf{Substitute cocycle (a).} By the abstract \(H1\) cocycle
      \ref{mk-lem:presheafdual_h1_cocycle} this prefix equals
      ...
      (\(\mathtt{pullbackComp}\), \ref{mk-lem:presheafofmodules_pullbackcomp}).
  ...
  plays the dual role of \ref{mk-lem:pullback_tensor_map_natural} inside the proven tensor analogue
  \ref{mk-lem:pullback_tensor_map_basechange}.
  ...
  \(\mathtt{leftAdjointCompIso\_hom\_app}\) (\ref{mk-lem:leftadjointcompiso_hom_app}), which expands
  ...
  \ref{mk-lem:leftadjointcompiso_hom_app}. (The mate characterisation
  \ref{mk-lem:conjugate_equiv_leftadjointcompiso_hom} is an alternative reading --- it identifies the
  ...
  coherence}. The leading \(FC.\mathrm{hom}\) is handled sectionwise via \ref{mk-lem:leftadjointcompiso_hom_app}: it is the composite of the pushforward adjunction's
  unit and two counits, and \emph{each} of these, evaluated at a section, is --- by the triangle-component
  identities \ref{mk-lem:pushforwardpushforwardadj_unit_app_app_apply} and
  \ref{mk-lem:pushforwardpushforwardadj_counit_app_app_apply} --- precisely a presheaf restriction map
  ...
  closes. The slice-side split \ref{mk-lem:slice_dual_transport_comp} supplies the matching
  ...
  the proven cocycle~(a) \ref{mk-lem:presheafdual_h1_cocycle}, which needs both flanks to be
  \(\mathtt{leftAdjointCompIso}\) of the \emph{same} \(e\).
  The sectionwise content, supplied by \ref{mk-lem:slice_dual_transport_comp}, runs
  through the forward apply lemma \ref{mk-lem:slice_dual_transport_app_apply}: on each open \(W\) of \(V\)
  ...
  \ref{mk-lem:dual_unit_ring_swap_apply} that swap is the \(\mathrm{hom}\)-direction of the composite
  structure-ring iso, and the cocycle \(\mathtt{comp\_appIso}\) (\ref{mk-lem:scheme_modules_comp_appiso})
  ...
  (\ref{mk-lem:presheafdual_pullback_restrict_natural}) together with poset-thinness of
  \(\mathrm{Opens}\) --- the consumer reads everything over the single open \(j\mathbin{;}\iota_U=\iota_V\)
  (\ref{mk-lem:image_preimage_of_le}) --- makes the assembly canonical, and the target-flank reindexing is
  realised by \(\mathtt{dualIsoOfIso}\) (\ref{mk-def:scheme_modules_dual_iso_of_iso}) of the forgetful image
  of \(\varrho\) (\ref{mk-lem:scheme_modules_restrictfunctorcomp}). The middle factor already sits at
  \(M|_{\iota_U}\) because the object identification \(\mathtt{pushforwardObjValRestrictIso}\)
  (\ref{mk-def:pushforward_obj_val_restrict_iso}, c.1) is \(\mathtt{Iso.refl}\): the underlying presheaf
  ...
  instance-agnostic skeleton lemma \(\mathtt{c2\_assemble}\) (\ref{mk-lem:c2_assemble}), stated over an
  ...
  (\ref{mk-lem:presheafdual_h1_cocycle}), the \(\mathtt{pullbackComp}\) cancellation, the single
...
\begin{lemma}[L1: folded composition cocycle for \(\mathtt{dual\_restrict\_iso}\) along a composite]\label{mk-lem:dual_restrict_iso_comp}
  \lean{AlgebraicGeometry.Scheme.Modules.dual_restrict_iso_comp}
  \leanok
  \dcref{custom}

  \uses{mk-lem:dual_restrict_iso, mk-lem:dual_restrict_iso_eq_comparison, mk-lem:restrictfunctorisopullback_comp_compat, mk-lem:sheafificationcomppullback_restrict, mk-lem:presheafdual_pullback_comparison_restrict, mk-lem:presheafdual_pullback_restrict_natural, mk-def:pushforward_obj_val_restrict_iso, mk-def:scheme_modules_dual_iso_of_iso, mk-lem:scheme_modules_restrictfunctorcomp}
  \textit{Source: internal categorical construction; no external reference (dual mirror of the
  tensor-flank composition law \ref{mk-lem:tensorobj_restrict_iso_restrict_compat}).}
  Let \(f : Z \hookrightarrow Y\) and \(g : Y \hookrightarrow X\) be composable open immersions
  ...
  (\ref{mk-lem:dual_restrict_iso}). The dual-restriction comparison along the composite \(f \mathbin{;} g\)
  ...
  \]
  where \(\mathtt{restrictFunctorComp}\,f\,g\) (\ref{mk-lem:scheme_modules_restrictfunctorcomp}) is the
  restriction-composition pseudofunctoriality iso \(A|_{f\mathbin{;}g} \cong (A|_g)|_f\) and
  \(\mathtt{dualIsoOfIso}\) (\ref{mk-def:scheme_modules_dual_iso_of_iso}) is its contravariant dual image.
...
\begin{proof}
  ...
  (\ref{mk-lem:tensorobj_restrict_iso_restrict_compat}), routed through Cone~B (internal-hom base-change);
  ...
  By the dual analogue of Bridge B1 (\ref{mk-lem:dual_restrict_iso_eq_comparison}) each
  ...
  \(\mathtt{restrictFunctorComp}\)-reindex by Bridge B2 (\ref{mk-lem:restrictfunctorisopullback_comp_compat}),
  ...
  (\ref{mk-lem:sheafificationcomppullback_restrict}); the \(\theta\)-cocycle by bridge~(c)
  (\ref{mk-lem:presheafdual_pullback_comparison_restrict}, itself through the immersion-naturality of
  \(\theta\), \ref{mk-lem:presheafdual_pullback_restrict_natural}). Bridge~(c) delivers its middle
  ...
  (\ref{mk-def:pushforward_obj_val_restrict_iso}, bridge c.1), which is \(\mathtt{Iso.refl}\), converts that
  ...
  \(\mathtt{dualIsoOfIso}\) (\ref{mk-def:scheme_modules_dual_iso_of_iso}) of
  \((\mathtt{restrictFunctorComp}\,f\,g).\mathrm{app}\,M\)
  (\ref{mk-lem:scheme_modules_restrictfunctorcomp}). Assembling the leg-for-leg identities yields the
  displayed composite.
\end{proof}

\begin{lemma}[L2: folded \(\mathtt{restrictFunctorCongr}\) coherence for \(\mathtt{dual\_restrict\_iso}\)]\label{mk-lem:dual_restrict_iso_pullbackcongr}
  \lean{AlgebraicGeometry.Scheme.Modules.dual_restrict_iso_pullbackCongr}
  \leanok
  \dcref{custom}

  \uses{mk-lem:dual_restrict_iso, mk-def:scheme_modules_dual_iso_of_iso, mk-lem:dualisoofiso_refl}
  \textit{Source: internal categorical construction; no external reference.}
...
\begin{proof}
  ...
  (\ref{mk-lem:dualisoofiso_refl}, \ref{mk-def:scheme_modules_dual_iso_of_iso}). Both sides reduce to
  \(\mathtt{dual\_restrict\_iso}\,f\,M\), and the equation holds.
\end{proof}

\begin{lemma}[S3: dual-restriction comparison commutes with further restriction (clean core)]\label{mk-lem:dual_restrict_iso_restrict_compat}
  \lean{AlgebraicGeometry.Scheme.Modules.dual_restrict_iso_restrict_compat}
  \leanok
  \dcref{custom}

  \uses{mk-lem:dual_restrict_iso, mk-lem:dual_restrict_iso_comp, mk-lem:dual_restrict_iso_pullbackcongr, mk-lem:dualisoofiso_trans, mk-def:scheme_modules_dual_iso_of_iso, mk-def:restrictcompreindex, mk-lem:image_preimage_of_le}
  ...
  (\mathtt{dual}\,M)|_f \cong \mathtt{dual}\bigl(M|_f\bigr)\) (\ref{mk-lem:dual_restrict_iso}). The
  \emph{bare} dual-restriction comparison is natural under further restriction along \(j\): modulo
  the reindexing \(\varrho = \mathtt{restrictCompReindex}\,j\) (\ref{mk-def:restrictcompreindex}),
  with its contravariant image \(\mathtt{dualIsoOfIso}\,\varrho\)
  (\ref{mk-def:scheme_modules_dual_iso_of_iso}) on the dual flank,
  \[
    D(\iota_V)
  ...
  telescope in the parent (\ref{mk-lem:trivialisation_restrict_compat}).
\end{lemma}
\begin{proof}
  \leanok
  \uses{mk-lem:dual_restrict_iso, mk-lem:dual_restrict_iso_comp, mk-lem:dual_restrict_iso_pullbackcongr, mk-lem:dualisoofiso_trans, mk-def:scheme_modules_dual_iso_of_iso, mk-def:restrictcompreindex, mk-lem:image_preimage_of_le}
  The square is now a short assembly of the folded composition cocycle L1
  (\ref{mk-lem:dual_restrict_iso_comp}) and the folded congruence coherence L2
  (\ref{mk-lem:dual_restrict_iso_pullbackcongr}); the seam-crossing work (dual-B1, B2, bridges (b), (c),
  ...
  Specialise L1 (\ref{mk-lem:dual_restrict_iso_comp}) at \((f, g) = (j, \iota_U)\). The equality of opens
  \(j \mathbin{;} \iota_U = \iota_V\) (\(hjι\), \ref{mk-lem:image_preimage_of_le}) converts the
  \(\iota_V\)-leg \(D(\iota_V)\) on the left of the target into the composite leg \(D(j\mathbin{;}\iota_U)\)
  via L2 (\ref{mk-lem:dual_restrict_iso_pullbackcongr}). Combining the \(\mathtt{restrictFunctorCongr}\,hjι\)
  ...
  (\ref{mk-def:restrictcompreindex}), giving the head \(\varrho_{\mathtt{dual}\,M}\). On the dual flank the
  two contravariant \(\mathtt{dualIsoOfIso}\) transports fuse, by contravariant functoriality
  (\ref{mk-lem:dualisoofiso_trans}, \ref{mk-def:scheme_modules_dual_iso_of_iso}), into the single factor
  ...
  square; they telescope in the parent (\ref{mk-lem:trivialisation_restrict_compat}).
\end{proof}

\begin{definition}
[Oplax-monoidal unit map $\eta$ of the presheaf pullback --- conceptual anchor]
  \label{mk-def:presheaf_pullback_unit_eta}
  \lean{CategoryTheory.Functor.OplaxMonoidal.η}
  \mathlibok
  \dcref{custom}

  \uses{mk-lem:presheaf_pullback_oplaxmonoidal}
  ...
  (\ref{mk-lem:presheaf_pullback_oplaxmonoidal}) along the chart immersion
  ...
  \ref{mk-def:presheaf_pullback_unit_iso}.
\end{definition}

\begin{definition}[Pullback-side unit comparator $p$ --- the H1 realization]\label{mk-def:presheaf_pullback_unit_iso}
  \lean{AlgebraicGeometry.Scheme.Modules.presheafPullbackUnitIso}
  \leanok
  \dcref{custom}

  \uses{mk-def:presheaf_pushforward_unit_iso, mk-lem:unit_leftadjointuniq_hom_app, mk-lem:pushforwardpushforwardadj_unit_app_app_apply, mk-lem:presheaf_pullback_oplaxmonoidal}
  \textit{Source: internal categorical construction; no external reference.}
  ...
  \]
  realized \emph{not} as the bare oplax \(\eta\) (\ref{mk-def:presheaf_pullback_unit_eta}) but through
  ...
  \]
  identifies the two left adjoints (\ref{mk-lem:unit_leftadjointuniq_hom_app},
  \ref{mk-lem:pushforwardpushforwardadj_unit_app_app_apply}). Evaluated at the structure-sheaf unit
  \(\mathbf 1_U\), it carries the pullback flank to the pushforward flank, on which the pushforward
  comparator \(q = \mathtt{presheafPushforwardUnitIso}\,j\) (\ref{mk-def:presheaf_pushforward_unit_iso})
  ...
  \ref{mk-lem:presheafdualunitiso_pullback_natural}.
\end{definition}

\begin{definition}
[Lax-monoidal unit map $\varepsilon$ of the presheaf pushforward --- conceptual anchor]
  \label{mk-def:presheaf_pushforward_unit_eps}
  ...
  \(\mathtt{PresheafOfModules.pushforward}\,\beta\) (\ref{mk-lem:presheaf_pushforward_laxmonoidal}) along
  ...
  (\ref{mk-lem:presheaf_pushforward_beta_monoidal}), and the resulting comparator \(q\) is recorded in
  \ref{mk-def:presheaf_pushforward_unit_iso}.
\end{definition}

\begin{lemma}[Strong monoidality of the open-immersion presheaf pushforward $\mathtt{pushforward}\,\beta$]\label{mk-lem:presheaf_pushforward_beta_monoidal}
  \lean{AlgebraicGeometry.Scheme.Modules.presheafPushforwardBetaMonoidal}
  \leanok
  \dcref{custom}

  \uses{mk-lem:presheaf_pushforward_laxmonoidal, mk-lem:restrictscalars_monoidal_of_bijective}
  \textit{Source: internal categorical construction; no external reference.}
  ...
  (\ref{mk-lem:presheaf_pushforward_laxmonoidal}); in particular its unit comparator
  \(\varepsilon\) and tensorator \(\mu\) are isomorphisms.
\end{lemma}

\begin{definition}[Pushforward-side unit comparator $q$ --- the strong-monoidal $\varepsilon$ inverse]\label{mk-def:presheaf_pushforward_unit_iso}
  \lean{AlgebraicGeometry.Scheme.Modules.presheafPushforwardUnitIso}
  \leanok
  \dcref{custom}

  \uses{mk-lem:presheaf_pushforward_beta_monoidal}
  \textit{Source: internal categorical construction; no external reference.}
  In the open-immersion setting above, the strong monoidality of \(\mathtt{pushforward}\,\beta\)
  (\ref{mk-lem:presheaf_pushforward_beta_monoidal}) makes the unit comparator an isomorphism
  ...
  \ref{mk-def:presheaf_pushforward_unit_eps}).
\end{definition}

\begin{lemma}[Carrier-value of the pushforward comparator: $q$ is the $f.\mathtt{appIso}$-conjugation on carriers]\label{mk-lem:pushforward_beta_unit_eps_app_one}
  \lean{AlgebraicGeometry.Scheme.Modules.pushforwardBetaUnitEpsAppOne}
  \leanok
  \dcref{custom}

  \uses{mk-lem:presheaf_pushforward_beta_monoidal, mk-def:presheaf_pushforward_unit_iso, mk-lem:pushforward_mu_appiso_collapse, mk-lem:restrictscalars_oplaxmonoidal_eta_app_one}
  \textit{Source: internal categorical construction; no external reference.}
  This is the unit-side analogue of the tensorator collapse
  \ref{mk-lem:pushforward_mu_appiso_collapse}. With \(f : Y \to X\) an open immersion,
  ...
  \beta)^{-1}\) (\ref{mk-def:presheaf_pushforward_unit_iso}), the sectionwise carrier action of
  ...
  \ref{mk-lem:presheafdualunitiso_pullback_natural} requires (it instantiates at a non-unit
  carrier).
\end{lemma}
\begin{proof}
  \leanok
  ...
  \ref{mk-lem:presheaf_pushforward_beta_monoidal}. The argument is cleanest run on the \emph{inverse}
  ...
  \ref{mk-lem:restrictscalars_laxmonoidal_eps_app} (the strong-monoidal companion of the oplax
  \(\eta\)-template \ref{mk-lem:restrictscalars_oplaxmonoidal_eta_app_one}; \(\varepsilon = \eta^{-1}\)
  since \(\beta\) is sectionwise bijective). Hence
  \(\bigl(q.\mathrm{inv}\bigr)_V (w) = (f.\mathtt{appIso}\,V).\mathrm{inv}\,(w)\)
  (\ref{mk-lem:presheaf_pushforward_unit_iso_inv_app}). Now both \(\bigl(q.\mathrm{hom}\bigr)_V(y)\) and
  ...
  tensorator collapse \ref{mk-lem:pushforward_mu_appiso_collapse} isolates on the \(\mu\)-side.
\end{proof}

\begin{lemma}[Abstract $\varepsilon$-value of $\mathtt{restrictScalars}$ on carriers]\label{mk-lem:restrictscalars_laxmonoidal_eps_app}
  \lean{AlgebraicGeometry.Scheme.Modules.restrictScalars_laxMonoidal_ε_app}
  \leanok
  \dcref{custom}

  \uses{mk-lem:restrictscalars_oplaxmonoidal_eta_app_one}
  \textit{Source: internal categorical construction; no external reference.}
  ...
  unit-value template \ref{mk-lem:restrictscalars_oplaxmonoidal_eta_app_one}; the abstract framing makes
...
\begin{lemma}[Carrier-value of the pushforward comparator inverse $q.\mathrm{inv} = \varepsilon$]\label{mk-lem:presheaf_pushforward_unit_iso_inv_app}
  \lean{AlgebraicGeometry.Scheme.Modules.presheafPushforwardUnitIso_inv_app}
  \leanok
  \dcref{custom}

  \uses{mk-def:presheaf_pushforward_unit_iso, mk-lem:presheaf_pushforward_beta_monoidal, mk-lem:restrictscalars_laxmonoidal_eps_app}
  \textit{Source: internal categorical construction; no external reference.}
  In the open-immersion setting, with \(q = \mathtt{presheafPushforwardUnitIso}\,f\)
  (\ref{mk-def:presheaf_pushforward_unit_iso}), the inverse \(q.\mathrm{inv} =
  \varepsilon(\mathtt{pushforward}\,\beta)\) acts sectionwise on carriers as the inverse structure-ring
  transport: over a section \(V\),
  \(\bigl(q.\mathrm{inv}\bigr)_V (w) = (f.\mathtt{appIso}\,V).\mathrm{inv}\,(w)\).
  This is the \(\varepsilon\)-side companion of \ref{mk-lem:pushforward_beta_unit_eps_app_one}; it is
  consumed both inside that lemma and directly by the L1\('\) flank
  \ref{mk-lem:presheafdualunitiso_pullback_natural}.
\end{lemma}
\begin{proof}
  \leanok
  \uses{mk-lem:presheaf_pushforward_beta_monoidal, mk-lem:restrictscalars_laxmonoidal_eps_app}
  Split \(\mathtt{pushforward}\,\beta = \mathtt{pushforward}_0^{\mathrm{Comm}} \circ
  \mathtt{restrictScalars}\,\beta\) (\ref{mk-lem:presheaf_pushforward_beta_monoidal}). Under
  ...
  \ref{mk-lem:restrictscalars_laxmonoidal_eps_app} is \((\beta.\mathrm{app}\,V)\)-multiplication, i.e.
  \((f.\mathtt{appIso}\,V).\mathrm{inv}\) on carriers. Hence
  \(\bigl(q.\mathrm{inv}\bigr)_V (w) = (f.\mathtt{appIso}\,V).\mathrm{inv}\,(w)\).
\end{proof}

\begin{lemma}[S4a / L1$'$: $\theta$-at-unit bridge --- the presheaf dual base-change comparison at the structure sheaf is the dual-of-unit iso]\label{mk-lem:presheafdualunitiso_pullback_natural}
  \lean{AlgebraicGeometry.Scheme.Modules.presheafDualUnitIso_pullback_natural}
  \leanok
  \dcref{custom}

  \uses{mk-def:presheafdual_pullback_comparison, mk-lem:presheafdual_pullback_restrict_natural, mk-lem:presheafdualunitiso_naturality, mk-def:presheaf_dual_unit_iso, mk-lem:internal_hom_eval, mk-def:presheaf_pullback_unit_iso, mk-def:presheaf_pushforward_unit_iso, mk-lem:pushforward_beta_unit_eps_app_one, mk-lem:presheaf_pushforward_unit_iso_inv_app, mk-lem:slice_dual_transport_app_apply, mk-lem:unit_leftadjointuniq_hom_app}
  \textit{Source: internal categorical construction; no external reference.}
  ...
  (\ref{mk-def:presheafdual_pullback_comparison}) at the structure-sheaf unit
  ...
  \begin{itemize}
    \item the pullback-side comparator
      ...
      (\ref{mk-def:presheaf_pullback_unit_iso}), the H1-transport \((H_1.\mathrm{app}\,\mathbf 1_U)^{-1}
    ...
    \item the pushforward-side comparator
      ...
      (\ref{mk-def:presheaf_pushforward_unit_iso}), the inverse of the strong-monoidal unit comparator
      of the pushforward.
  \end{itemize}
  Modulo the eval-at-\(1\) dual-of-unit isomorphism \(\mathtt{presheafDualUnitIso}\)
  (\ref{mk-def:presheaf_dual_unit_iso}) on the \(U\)- and \(V\)-flanks, \(\theta_{\mathbf 1}\)
  ...
  unit-specialisation of the immersion-naturality \ref{mk-lem:presheafdual_pullback_restrict_natural}
  and the immersion analogue of the unit-automorphism core
  \ref{mk-lem:presheafdualunitiso_naturality}.
\end{lemma}
\begin{proof}
  \leanok
  \uses{mk-def:presheafdual_pullback_comparison, mk-lem:presheafdual_pullback_restrict_natural, mk-lem:presheafdualunitiso_naturality, mk-lem:internal_hom_eval, mk-def:presheaf_dual_unit_iso, mk-def:presheaf_pullback_unit_iso, mk-def:presheaf_pushforward_unit_iso, mk-lem:pushforward_beta_unit_eps_app_one, mk-lem:slice_dual_transport_app_apply, mk-lem:unit_leftadjointuniq_hom_app}
  By element extensionality on dual sections, exactly as in
  \ref{mk-lem:presheafdual_pullback_restrict_natural}, of which this is the unit specialisation. Both
  ...
  (\ref{mk-def:presheafdual_pullback_comparison}), and \(\mathtt{presheafDualUnitIso}\) is the
  sectionwise evaluation-at-\(1\) of the internal hom of the unit into itself
  (\ref{mk-def:presheaf_dual_unit_iso}). Testing the displayed equality against an arbitrary dual
  section reduces, as in \ref{mk-lem:presheafdual_pullback_restrict_natural}, to the built-in
  naturality of \(\theta\) (its \(\mathtt{.hom}\) naturality), now read through the eval-at-\(1\)
  identification, where the internal-hom evaluation (\ref{mk-lem:internal_hom_eval}) collapses both
  ...
  \beta)^{-1}\) (\ref{mk-def:presheaf_pushforward_unit_iso}) is transported contravariantly by
  \(\mathtt{dualIsoOfIso}\); on the pullback domain the comparator
  \(p = \mathtt{presheafPullbackUnitIso}\,j\) (\ref{mk-def:presheaf_pullback_unit_iso}) closes the
  ...
  (\ref{mk-lem:unit_leftadjointuniq_hom_app}). After this cancellation the entire claim reduces to a
  ...
  \ref{mk-lem:presheafdualunitiso_naturality}. Fix a section \(V\) and a dual section \(\varphi\).
  \begin{itemize}
    \item \emph{Left-hand side.} The verified forward transport formula
      (\ref{mk-lem:slice_dual_transport_app_apply}) evaluates \(\mathtt{sDT}\) at the terminal slice as
      ...
      \ref{mk-def:presheaf_dual_unit_iso}) exposes \(\mathtt{evalLin}\,\mathbf 1_U\,(\mathrm{op}\,fV)\,
    ...
    \item \emph{Right-hand side.} The pushforward of the eval-at-\(1\) contraction
      ...
      \ref{mk-lem:pushforward_beta_unit_eps_app_one}: the carrier action of \(q.\mathrm{hom}\) at \(1\)
  ...
  \ref{mk-lem:presheafdualunitiso_naturality}, now with \(p\) and \(q\) in place of the unit
  automorphism. Hence both sides of ($\dagger$) agree sectionwise, and with the H1 telescope this gives the displayed
  square.
\end{proof}

\begin{lemma}[S4a / L2$'$: unit assembly --- the dual-restriction comparison at the unit intertwines the dual-unit contractions]\label{mk-lem:dual_unit_iso_restrict_assemble}
  \lean{AlgebraicGeometry.Scheme.Modules.dual_unit_iso_restrict_assemble}
  \leanok
  \dcref{custom}

  \uses{mk-lem:presheafdualunitiso_pullback_natural, mk-lem:dual_restrict_iso, mk-lem:dual_restrict_iso_eq_comparison, mk-lem:sheafificationcomppullback_restrict, mk-lem:dual_unit_iso, mk-def:unitrestrictiso, mk-def:scheme_modules_dual_iso_of_iso, mk-lem:image_preimage_of_le, mk-lem:forget_unit_restrict_eq_pushforward_unit, mk-lem:pullback_unit_sheafify_reconcile}
  \textit{Source: internal categorical construction; no external reference.}
  With \(j : V \hookrightarrow U\) as above, write \(\mathbf 1_U, \mathbf 1_V\) for the structure
  sheaves, \(u_j = \mathtt{unitRestrictIso}\,j\) (\ref{mk-def:unitrestrictiso}) for the chart-scheme
  ...
  (\ref{mk-lem:dual_unit_iso}). The \emph{bare} dual-restriction comparison
  \(\mathtt{dual\_restrict\_iso}\,j\,(\mathbf 1_U) : (\mathtt{dual}\,\mathbf 1_U)|_j \cong
  \mathtt{dual}(\mathbf 1_U|_j)\) (\ref{mk-lem:dual_restrict_iso}) intertwines the two contractions
...
\begin{proof}
  ...
  (\ref{mk-lem:dual_restrict_iso_eq_comparison}) into the three-factor composite
  ...
  \mathtt{counit}\) (\ref{mk-lem:dual_unit_iso}), turns the square into a purely syntactic identity of
  ...
  bridge~(b), \ref{mk-lem:sheafificationcomppullback_restrict}, threading the equality of opens
  \(j \mathbin{;} \iota_U = \iota_V\), \ref{mk-lem:image_preimage_of_le}; and the
  sheafification-counit naturality moves the counits past the restriction while contravariant
  functoriality of \(\mathtt{dualIsoOfIso}\), \ref{mk-def:scheme_modules_dual_iso_of_iso}, collects the
  dual transport into \((\mathtt{dualIsoOfIso}\,u_j)^{-1}\)). The whole \(\theta\)-at-unit
  identification supplied by L1$'$ (\ref{mk-lem:presheafdualunitiso_pullback_natural}) is consumed
  inside this fold. The substantive content of the four hypotheses reduces to exactly two residual
  reconciliations:

  \begin{itemize}
    \item the \emph{dual flank} (\ref{mk-lem:forget_unit_restrict_eq_pushforward_unit}): the
    ...
    \item the \emph{pullback (unit-only) flank} (\ref{mk-lem:pullback_unit_sheafify_reconcile}): the
  ...
  hom-transpose infrastructure (\ref{mk-lem:sheafification_comp_pullback_eq_leftadjointuniq},
  \ref{mk-lem:leftadjointuniq_app_unit_eta_general}, \ref{mk-lem:comp_homequiv_factor_sheafify_pullback},
  \ref{mk-lem:unit_leftadjointuniq_hom_app}, \ref{mk-lem:pushforward_beta_unit_eps_app_one}).
\end{proof}

\begin{lemma}[RES1 (dual-flank reconciliation): the forgotten sheaf unit-restriction iso equals the presheaf pushforward-unit comparator]\label{mk-lem:forget_unit_restrict_eq_pushforward_unit}
  \lean{AlgebraicGeometry.Scheme.Modules.forgetUnitRestrict_eq_pushforwardUnit}
  \leanok
  \dcref{custom}

  \uses{mk-def:restrictfunctorisopullback, mk-lem:pushforward_beta_unit_eps_app_one, mk-lem:unit_leftadjointuniq_hom_app, mk-lem:homequiv_leftadjointuniq_hom_app, mk-lem:leftadjointuniq_hom_app_counit, mk-lem:presheafdualunitiso_naturality, mk-def:presheaf_pushforward_unit_iso, mk-lem:pullbackObjUnitToUnit_comp}
  \textit{Source: internal categorical construction; no external reference.}
  ...
  \(\mathtt{restrictFunctorIsoPullback}\,f\) (\ref{mk-def:restrictfunctorisopullback}) at the
  ...
  \(q = \mathtt{presheafPushforwardUnitIso}\,f\) (\ref{mk-def:presheaf_pushforward_unit_iso}, the inverse
  ...
  \]
  This is the leg of the unit assembly (\ref{mk-lem:dual_unit_iso_restrict_assemble}) that closes its
  dual flank.
\end{lemma}
\begin{proof}
  \leanok
  ...
  \ref{mk-lem:pushforward_beta_unit_eps_app_one}.
  ...
  (\mathtt{pullbackPushforwardAdjunction}\,f)\) (\ref{mk-def:restrictfunctorisopullback}); the second is
  the adjunction transpose \((\mathtt{pullbackPushforwardAdjunction}\,f).\mathrm{homEquiv}^{-1}\) of
  the canonical \(\mathtt{unitToPushforwardObjUnit}\) (\ref{mk-lem:pullbackObjUnitToUnit_comp}).
  ...
  \]
  by the uniqueness coherences \ref{mk-lem:homequiv_leftadjointuniq_hom_app},
  \ref{mk-lem:leftadjointuniq_hom_app_counit} and the unit triangle
  \ref{mk-lem:unit_leftadjointuniq_hom_app} rewrites the left composite as the single
  ...
  mirroring the eval-core template \ref{mk-lem:presheafdualunitiso_naturality}. Both sides equal
  \((f.\mathtt{appIso}\,V).\mathrm{hom}\), closing the identity.
\end{proof}

\begin{lemma}[RES2 L1: the unit pullback identification factors through \texttt{sheafificationCompPullback} and the $X$-sheafification counit]\label{mk-lem:pullback_val_iso_unit_factor}
  ...
  \(\mathtt{pullbackValIso}\,f\,M_0\) (\ref{mk-def:pullback_val_iso}) factors, on the nose, as the
...
\begin{proof}
  ...
  (\ref{mk-def:pullback_val_iso}): reading off the homs of the two composed isomorphisms gives the
  displayed identity by reflexivity. This is the unit instance of the template factorisation
  \(\mathtt{hpbv}\) of \ref{mk-lem:sheaf_pullback_unit_forget_eq}.
\end{proof}

\begin{lemma}[RES2 L2: the sheafified presheaf pullback-unit comparator reconciles with the sheaf pullback identification]\label{mk-lem:presheaf_pullback_unit_iso_sheafify_reconcile}
  \lean{AlgebraicGeometry.Scheme.Modules.presheafPullbackUnitIso_sheafify_reconcile}
  \leanok
  \dcref{custom}

  \uses{mk-def:presheaf_pullback_unit_iso, mk-def:presheaf_pushforward_unit_iso, mk-lem:pullback_unit_iso, mk-def:pullback_val_iso, mk-lem:h1inv_app_eq_pullbackval_restrict, mk-lem:forget_unit_restrict_eq_pushforward_unit, mk-lem:adj_unit_counit_collapse}
  \textit{Source: internal categorical construction; no external reference.}
  ...
  \(p = \mathtt{presheafPullbackUnitIso}\,f\) (\ref{mk-def:presheaf_pullback_unit_iso}, the presheaf
  ...
  \ref{mk-def:presheaf_pushforward_unit_iso}). Then sheafifying \(p\) and cancelling the
  $Y$-sheafification counit reproduces the sheaf pullback identification followed by the sheaf
  pullback-unit iso \(\mathtt{pullbackUnitIso}\,f\) (\ref{mk-lem:pullback_unit_iso}):
  \[
...
\begin{proof}
  ...
  (\ref{mk-def:presheaf_pullback_unit_iso}), with \(q = \mathtt{presheafPushforwardUnitIso}\,f\)
  (\ref{mk-def:presheaf_pushforward_unit_iso}). The H1-reconciliation
  \(\mathtt{H1inv\_app\_eq\_pullbackVal\_restrict}\)
  (\ref{mk-lem:h1inv_app_eq_pullbackval_restrict}) writes \(H_1^{-1}\) as the sheafification unit
  \(\eta_Y\), the forgotten sheaf pullback identification \(\mathtt{pullbackValIso}\,f\,\mathbf 1_X\)
  (\ref{mk-def:pullback_val_iso}), and the forgotten inverse \(\mathtt{restrictFunctorIsoPullback}\);
  RES1 \(\mathtt{forgetUnitRestrict\_eq\_pushforwardUnit}\)
  (\ref{mk-lem:forget_unit_restrict_eq_pushforward_unit}) writes \(q\) as the forgotten
  \(\mathtt{restrictFunctorIsoPullback} \mathbin{;} \mathtt{pullbackUnitIso}\,f\)
  (\ref{mk-lem:pullback_unit_iso}). The two \(\mathtt{restrictFunctorIsoPullback}\) legs cancel, leaving
  ...
  (\ref{mk-lem:adj_unit_counit_collapse}) at the sheafification adjunction over \(Y\) and the morphism
  \(g = \mathtt{pullbackValIso}\,f\,\mathbf 1_X \mathbin{;} \mathtt{pullbackUnitIso}\,f\), which returns
  \(g\) --- the asserted right-hand side.
\end{proof}

\begin{lemma}[$(*)$ / RES2 (pullback unit-only reconciliation): the sheaf pullback-unit comparator sheafifies the presheaf pullback-unit comparator]\label{mk-lem:pullback_unit_sheafify_reconcile}
  \lean{AlgebraicGeometry.Scheme.Modules.pullbackUnit_sheafify_reconcile}
  \leanok
  \dcref{custom}

  \uses{mk-lem:sheafification_comp_pullback_eq_leftadjointuniq, mk-def:presheaf_pullback_unit_iso, mk-lem:pullback_unit_iso, mk-lem:pullback_val_iso_unit_factor, mk-lem:presheaf_pullback_unit_iso_sheafify_reconcile}
  \textit{Source: internal categorical construction; no external reference.}
  ...
  \(p = \mathtt{presheafPullbackUnitIso}\,f\) (\ref{mk-def:presheaf_pullback_unit_iso}), then the
...
\begin{proof}
  ...
  (\ref{mk-lem:presheaf_pullback_unit_iso_sheafify_reconcile}), then expand
  \((\mathtt{pullbackValIso}\,f\,M_0).\mathrm{hom}\) by L1
  (\ref{mk-lem:pullback_val_iso_unit_factor}); the leading
...
\begin{lemma}[S4a: dual-unit contraction commutes with further restriction]\label{mk-lem:dual_unit_iso_restrict_compat}
  \lean{AlgebraicGeometry.Scheme.Modules.dual_unit_iso_restrict_compat}
  \leanok
  \dcref{custom}

  \uses{mk-lem:dual_unit_iso, mk-lem:dual_restrict_iso, mk-lem:dual_unit_iso_restrict_assemble, mk-lem:presheafdualunitiso_pullback_natural, mk-def:unitrestrictiso, mk-def:scheme_modules_dual_iso_of_iso}
  ...
  (\ref{mk-lem:dual_unit_iso}) commutes with further restriction along \(j\). Writing
  \(\mathtt{dual\_unit\_iso}_U\), \(\mathtt{dual\_unit\_iso}_V\) for the contraction over \(U\), \(V\)
  and \(u_j = \mathtt{unitRestrictIso}\,j\) (\ref{mk-def:unitrestrictiso}) for the chart-scheme unit
  ...
  (\ref{mk-lem:dual_restrict_iso}) is the dual-restriction comparison along \(j\), and
  \(\mathtt{dualIsoOfIso}\) (\ref{mk-def:scheme_modules_dual_iso_of_iso}) is the contravariant transport
  of \(u_j\) on the dual flank.
\end{lemma}
\begin{proof}
  \leanok
  \uses{mk-lem:dual_unit_iso_restrict_assemble, mk-lem:presheafdualunitiso_pullback_natural, mk-lem:dual_unit_iso, mk-def:scheme_modules_dual_iso_of_iso, mk-def:unitrestrictiso}
  Pure folded iso-algebra from the unit assembly square L2$'$
  (\ref{mk-lem:dual_unit_iso_restrict_assemble}), which records
  \[
    (\mathtt{restrict}\,j)\,\mathtt{dual\_unit\_iso}_U \,\mathbin{;}\, u_j
      = \mathtt{dual\_restrict\_iso}\,j\,(\mathbf 1_U)
        \,\mathbin{;}\, \bigl(\mathtt{dualIsoOfIso}\,u_j\bigr)^{-1}
  ...
  \ref{mk-lem:presheafdualunitiso_pullback_natural}); no unfolding is needed here.
\end{proof}

\begin{lemma}[The unit contraction is the left unitor at the unit (Cone A coverage)]\label{mk-lem:tensorobj_unit_iso_eq_left_unitor}
  \lean{AlgebraicGeometry.Scheme.Modules.tensorObj_unit_iso_eq_left_unitor}
  \leanok
  \dcref{custom}

  \uses{mk-lem:tensorobj_unit_iso}
  The structure-sheaf unit contraction
  \(\mathtt{tensorObj\_unit\_iso} : \mathcal{O}_X \otimes_X \mathcal{O}_X \cong \mathcal{O}_X\)
  coincides with the left unitor \(\mathtt{tensorObj\_left\_unitor}\) (\ref{mk-lem:tensorobj_unit_iso})
...
\begin{lemma}[Naturality of the left unitor (Cone A coverage)]\label{mk-lem:tensorobj_left_unitor_naturality}
  ...
  (\ref{mk-lem:tensorobj_unit_iso}) is natural:
  \[
...
\begin{lemma}[$\eta$ mate-identification: the unit iso is the sheafified presheaf unit comparison (Cone A)]\label{mk-lem:pullback_unit_iso_eq_sheafify_eta}
  \lean{AlgebraicGeometry.Scheme.Modules.pullbackUnitIso_eq_sheafify_eta}
  \leanok
  \dcref{custom}

  \uses{mk-lem:pullback_unit_iso, mk-lem:presheaf_pullback_oplaxmonoidal, mk-lem:pullback_tensor_iso_unit, mk-def:pullback_val_iso}
  Let \(f : Y \to X\) be a morphism of schemes and \(\varphi'\) its associated change-of-rings map.
  The sheaf-level unit comparison \(\mathtt{pullbackUnitIso}\,f\) (\ref{mk-lem:pullback_unit_iso}) is the
  sheafification transport of the presheaf oplax unit comparison
  \(\eta\,(\mathtt{pullback}\,\varphi')\) (\ref{mk-lem:presheaf_pullback_oplaxmonoidal}):
  \[
  ...
  \]
  where \(\mathtt{pullbackValIso}\) (\ref{mk-def:pullback_val_iso}) reconciles the sheaf-level pullback
...
\begin{proof}
  \leanok
  \uses{mk-lem:pullback_unit_iso, mk-lem:pullback_tensor_iso_unit, mk-def:pullback_val_iso}
  This is the unit square already isolated in the proof that the tensor comparison on the unit pair is
  an isomorphism (\ref{mk-lem:pullback_tensor_iso_unit}): there the right-hand composite is shown to
  equal \(\mathtt{pullbackObjUnitToUnit}\,\varphi\), which by definition is
  \(\mathtt{pullbackUnitIso}\,f\) (\ref{mk-lem:pullback_unit_iso}). Reading that square as a defining
  equation for the sheafified \(\eta\) yields the displayed identity.
\end{proof}

\begin{lemma}[$\delta$ mate-identification: the tensor comparison is the sheafified cotensorator (Cone A)]\label{mk-lem:pullback_tensor_map_eq_sheafify_delta}
  \lean{AlgebraicGeometry.Scheme.Modules.pullbackTensorMap_eq_sheafify_delta}
  \leanok
  \dcref{custom}

  \uses{mk-lem:pullback_tensor_map, mk-lem:presheaf_pullback_oplaxmonoidal, mk-def:pullback_val_iso, mk-def:scheme_modules_tensorobj}
  Let \(f : Y \to X\) and \(M, N \in \Scheme.\mathtt{Modules}\,X\). The sheaf-level tensor comparison
  \(\mathtt{pullbackTensorMap}\,f\,M\,N\) (\ref{mk-lem:pullback_tensor_map}) is the sheafification
  transport of the presheaf oplax cotensorator
  \(\delta\,(\mathtt{pullback}\,\varphi')\,M_{\mathrm{val}}\,N_{\mathrm{val}}\)
  (\ref{mk-lem:presheaf_pullback_oplaxmonoidal}):
  \[
    \mathtt{pullbackTensorMap}\,f\,M\,N
  ...
  (\ref{mk-def:pullback_val_iso}, \ref{mk-def:scheme_modules_tensorobj}) on the \(\delta\)-codomain,
  identifying the \(a_Y\)-image of the presheaf tensor with \(f^{*}M \otimes_Y f^{*}N\).
\end{lemma}
\begin{proof}
  \leanok
  \uses{mk-lem:pullback_tensor_map, mk-def:pullback_val_iso}
  This is the definitional unfolding of \(\mathtt{pullbackTensorMap}\) (\ref{mk-lem:pullback_tensor_map}):
...
\begin{lemma}[Cone A sub-lemma 1: \texttt{pullbackValIso} naturality at the sheaf left unitor (RHS reconciliation)]\label{mk-lem:pullback_val_iso_naturality_left_unitor}
  \lean{AlgebraicGeometry.Scheme.Modules.pullbackValIso_naturality_leftUnitor}
  \leanok
  \dcref{custom}

  \uses{mk-def:pullback_val_iso, mk-lem:tensorobj_unit_iso, mk-lem:presheaf_pullback_oplaxmonoidal}
  ...
  (\ref{mk-def:pullback_val_iso}). The reconciliation isomorphism \(\mathtt{pullbackValIso}\,f\) is
  ...
  \ref{mk-lem:pullback_tensor_map_left_unitality} --- as the \(a_Y\)-sheafification of the
...
\begin{proof}
  \leanok
  \uses{mk-def:pullback_val_iso, mk-lem:tensorobj_unit_iso}
  ...
  (\ref{mk-def:pullback_val_iso}); both legs are natural transformations. Hence the displayed equation
  ...
  counit (\ref{mk-lem:tensorobj_unit_iso}) and cancelling that counit against the counit leg inside
...
\begin{lemma}[Localization-monoidal left-unitor collapse]
  \label{mk-lem:localization_monoidal_left_unitor_hom_app}
  ...
  reduces by \ref{mk-lem:localization_lift_nat_trans_app} to the source datum conjugated by the lifting
...
\begin{lemma}[Evaluation of a lifted natural transformation on an image object]
  \label{mk-lem:localization_lift_nat_trans_app}
...
\begin{lemma}[The sheafification of presheaves of modules is a localization]
  \label{mk-lem:presheaf_sheafification_is_localization}
  ...
  \ref{mk-lem:localization_monoidal_left_unitor_hom_app} and
  \ref{mk-lem:localization_lift_nat_trans_app} in particular --- is applicable to \(a\).
\end{lemma}

\begin{lemma}[STU-collapse: the sheaf left unitor at a pullback image object]\label{mk-lem:tensorobj_left_unitor_image_collapse}
  \lean{AlgebraicGeometry.Scheme.Modules.tensorObj_left_unitor_image_collapse}
  \leanok
  \dcref{custom}

  \uses{mk-lem:sheafify_tensor_unit_iso_hom_eq_prime, mk-def:sheafify_tensor_unit_iso, mk-lem:tensorobj_unit_iso}
  ...
  \]
  This is the shared structural residual of \ref{mk-lem:tensorobj_left_unitor_pullback_eq_sheafify} and
  of the central-leg sub-step of
  \ref{mk-lem:pullback_unit_iso_whisker_eq_sheafify_eta_whisker}.
\end{lemma}
\begin{proof}
  \leanok
  \uses{mk-lem:sheafify_tensor_unit_iso_hom_eq_prime, mk-def:sheafify_tensor_unit_iso, mk-lem:tensorobj_unit_iso}
  ...
  (\ref{mk-lem:sheafify_tensor_unit_iso_hom_eq_prime}, \ref{mk-def:sheafify_tensor_unit_iso}):
  ...
  presheaf left unitor (\ref{mk-lem:tensorobj_unit_iso}). All factors are kept inside the one
...
\begin{lemma}[Cone A sub-lemma 2: the pullback-object left unitor is the sheafified presheaf unitor]\label{mk-lem:tensorobj_left_unitor_pullback_eq_sheafify}
  \lean{AlgebraicGeometry.Scheme.Modules.tensorObj_left_unitor_pullback_eq_sheafify}
  \leanok
  \dcref{custom}

  \uses{mk-lem:tensorobj_unit_iso, mk-lem:tensorobj_left_unitor_naturality, mk-lem:tensorobj_left_unitor_image_collapse, mk-def:pullback_val_iso}
  ...
  \ref{mk-lem:pullback_unit_iso_whisker_eq_sheafify_eta_whisker}, whose unit is \(F\,\mathbf 1\) and whose
  ...
  \ref{mk-lem:pullback_unit_iso_whisker_eq_sheafify_eta_whisker}; that lemma performs the
...
\begin{proof}
  ...
  \(\mathtt{pbv}\,M\)-leg (\ref{mk-lem:tensorobj_left_unitor_naturality}) slides that right-factor whisker
  ...
  localization-image collapse \ref{mk-lem:tensorobj_left_unitor_image_collapse}: the sheaf left unitor at
  ...
  image-collapse \ref{mk-lem:tensorobj_left_unitor_image_collapse}, whose own proof is the by-hand
  bifunctoriality + adjunction-triangle reduction recorded there.
\end{proof}

\begin{lemma}[Cone A sub-lemma 3: the whiskered unit comparison is the sheafified \(\eta\)-whisker]\label{mk-lem:pullback_unit_iso_whisker_eq_sheafify_eta_whisker}
  \lean{AlgebraicGeometry.Scheme.Modules.pullbackUnitIso_whisker_eq_sheafify_eta_whisker}
  \leanok
  \dcref{custom}

  \uses{mk-lem:pullback_unit_iso_eq_sheafify_eta, mk-lem:pullback_tensor_map_eq_sheafify_delta, mk-lem:presheaf_pullback_oplaxmonoidal, mk-lem:sheafify_tensor_unit_iso_natural, mk-def:pullback_val_iso}
  ...
  oplax unit (\ref{mk-lem:presheaf_pullback_oplaxmonoidal}), and let
  \begin{align*}
  ...
  \ref{mk-lem:pullback_tensor_map_eq_sheafify_delta} (tensor unit \(F\,\mathbf 1\), left leg
  \(\mathtt{pbv}\,\mathcal{O}_X\)) and the unit-reconciled wrapper of
  \ref{mk-lem:tensorobj_left_unitor_pullback_eq_sheafify} (tensor unit \(\mathbf 1 =
  ...
  wrapper consumed by \ref{mk-lem:tensorobj_left_unitor_pullback_eq_sheafify}, at the cost of the
  \(\eta\)-whisker prefix.
\end{lemma}
\begin{proof}
  \leanok
  \uses{mk-lem:pullback_unit_iso_eq_sheafify_eta, mk-lem:presheaf_pullback_oplaxmonoidal, mk-lem:pullback_tensor_map_eq_sheafify_delta, mk-lem:sheafify_tensor_unit_iso_natural, mk-def:pullback_val_iso}
  ...
  identity. Substituting bridge~1 (\ref{mk-lem:pullback_unit_iso_eq_sheafify_eta}),
  ...
  naturality \ref{mk-lem:sheafify_tensor_unit_iso_natural} of \(\mathtt{sheafifyTensorUnitIso}\):
  \[
  ...
  i.e. converts \(W\) into the unit-reconciled wrapper \(W_{\lambda}\) (\ref{mk-def:pullback_val_iso}).
  ...
  for the unit pair in \ref{mk-lem:pullback_tensor_iso_unit}, now retaining the whisker rather than
  discarding it as an isomorphism.
\end{proof}

\begin{lemma}[Sheaf-level left unitality of the pullback tensor comparison (Cone A core)]\label{mk-lem:pullback_tensor_map_left_unitality}
  \lean{AlgebraicGeometry.Scheme.Modules.pullbackTensorMap_left_unitality}
  \leanok
  \dcref{custom}

  \uses{mk-lem:pullback_val_iso_naturality_left_unitor, mk-lem:tensorobj_left_unitor_pullback_eq_sheafify, mk-lem:pullback_unit_iso_whisker_eq_sheafify_eta_whisker, mk-lem:pullback_tensor_map_eq_sheafify_delta, mk-lem:presheaf_pullback_oplaxmonoidal}
...
\begin{proof}
  ...
  M_{\mathrm{val}}\) (\ref{mk-lem:presheaf_pullback_oplaxmonoidal}; pentagon-free): applying
  ...
  \(\ref{mk-lem:pullback_tensor_map_eq_sheafify_delta}\) (which also pins the shared right wrapper), the
  \(\eta\)-whisker leg to the whiskered \(\mathtt{pullbackUnitIso}\,f\) by sub-lemma~3
  (\ref{mk-lem:pullback_unit_iso_whisker_eq_sheafify_eta_whisker}), the \(\lambda\)-leg to
  \(\mathtt{tensorObj\_left\_unitor}\,(f^{*}M)\) by sub-lemma~2
  (\ref{mk-lem:tensorobj_left_unitor_pullback_eq_sheafify}), and the right-hand side to the
  ...
  covered by sub-lemma~1 (\ref{mk-lem:pullback_val_iso_naturality_left_unitor}). The
  \(\eta\)-whisker leg itself --- inside sub-lemma~3 (\ref{mk-lem:pullback_unit_iso_whisker_eq_sheafify_eta_whisker}),
  via the STU-collapse (\ref{mk-lem:tensorobj_left_unitor_image_collapse}) --- decomposes into the
...
\begin{lemma}[S4b inner seam: pullback-world left-unitor coherence at the chart unit (Cone A)]\label{mk-lem:tensorobj_unit_iso_restrict_compat_inner}
  \lean{AlgebraicGeometry.Scheme.Modules.tensorObj_unit_iso_restrict_compat_inner}
  \leanok
  \dcref{custom}

  \uses{mk-lem:pullback_tensor_map_left_unitality, mk-lem:tensorobj_restrict_iso_eq_pullback_tensor_map, mk-def:restrictfunctorisopullback, mk-def:unitrestrictiso, mk-lem:pullback_unit_iso}
  ...
  chart comparison \(\mathtt{restrictFunctorIsoPullback}\,j\) (\ref{mk-def:restrictfunctorisopullback})
  and promoting the tensor leg by Bridge~B1
  (\ref{mk-lem:tensorobj_restrict_iso_eq_pullback_tensor_map}), the square reduces to the sheaf-level
  left unitality \(\mathtt{pullbackTensorMap\_left\_unitality}\,j\)
  (\ref{mk-lem:pullback_tensor_map_left_unitality}) evaluated at \(M = \mathcal{O}_U\), with the unit leg
  supplied by \(\mathtt{unitRestrictIso}\,j\) (\ref{mk-def:unitrestrictiso}) and
  \(\mathtt{pullbackUnitIso}\,j\) (\ref{mk-lem:pullback_unit_iso}).
\end{lemma}
\begin{proof}
  \leanok
  \uses{mk-lem:pullback_tensor_map_left_unitality, mk-lem:tensorobj_restrict_iso_eq_pullback_tensor_map, mk-def:restrictfunctorisopullback, mk-def:unitrestrictiso}
  ...
  precisely \ref{mk-lem:pullback_tensor_map_left_unitality} at \(M = \mathcal{O}_U\), with the unit
  whiskering supplied by \(\mathtt{unitRestrictIso}\,j\) (\ref{mk-def:unitrestrictiso}); iso-algebra then
...
\begin{lemma}[S4b: unit left-unitor commutes with further restriction]\label{mk-lem:tensorobj_unit_iso_restrict_compat}
  \lean{AlgebraicGeometry.Scheme.Modules.tensorObj_unit_iso_restrict_compat}
  \leanok
  \dcref{custom}

  \uses{mk-lem:tensorobj_unit_iso, mk-lem:image_preimage_of_le}
  ...
  unit, \ref{mk-lem:tensorobj_unit_iso}) commutes with further restriction along \(j\): modulo the
  reindexing \(\varrho\),
  \[
  ...
  sheafification-adjunction counit (\ref{mk-lem:tensorobj_unit_iso}). Bridge B1 does not apply to the
  ...
  \ref{mk-lem:pullback_tensor_map_left_unitality}), established below.
\end{lemma}
\begin{proof}
  \leanok
  \uses{mk-lem:tensorobj_unit_iso, mk-lem:tensorobj_unit_iso_eq_left_unitor, mk-lem:tensorobj_left_unitor_naturality, mk-lem:tensorobj_unit_iso_restrict_compat_inner, mk-lem:presheaf_pullback_oplaxmonoidal, mk-def:unitrestrictiso, mk-def:restrictcompreindex, mk-lem:image_preimage_of_le}
  ...
  the sheafification-adjunction counit. By \ref{mk-lem:tensorobj_unit_iso_eq_left_unitor} the
  contraction is the sheaf-level left unitor \(\mathtt{tensorObj\_left\_unitor}\)
  (\ref{mk-lem:tensorobj_unit_iso}) evaluated at \(M = \mathcal{O}\); restriction along \(j\) commutes
  through the outer \(\mathtt{Functor.mapIso}\)/\(\mathtt{toSheafify}\)/counit legs by functoriality
  and naturality of the unitor (\ref{mk-lem:tensorobj_left_unitor_naturality}), so the square reduces to
  a single \emph{inner} seam.

  \emph{Inner seam (Cone~A).} The inner seam is the pullback-world left-unitor coherence at the chart
  unit, \ref{mk-lem:tensorobj_unit_iso_restrict_compat_inner}. Crucially, this is \emph{not} a missing
  monoidal instance: the presheaf change-of-rings pullback \(\mathtt{pullback}\,\varphi'\) is
  \emph{already} oplax monoidal (\ref{mk-lem:presheaf_pullback_oplaxmonoidal}, via
  ...
  \ref{mk-lem:pullback_tensor_map_left_unitality} at \(M = \mathcal{O}_U\), through the chart comparison
  \(\mathtt{restrictFunctorIsoPullback}\,j\) and the unit conjugator \(\mathtt{unitRestrictIso}\,j\)
  (\ref{mk-def:unitrestrictiso}). The pentagon plays no role.
  ...
  (\ref{mk-def:restrictcompreindex}) threads the equality of opens
  \(j \mathbin{;} \iota_U = \iota_V\) (\ref{mk-lem:image_preimage_of_le}) through both flanks, absorbing
...
\begin{lemma}[S4c: global-unit comparison \(u_\iota\) commutes with further restriction]\label{mk-lem:trivialisation_uiota_restrict_compat}
  \lean{AlgebraicGeometry.Scheme.Modules.trivialisation_uIota_restrict_compat}
  \leanok
  \dcref{custom}

  \uses{mk-lem:pullback_unit_iso, mk-lem:image_preimage_of_le}
  ...
  \]
  (\ref{mk-lem:pullback_unit_iso}), whose inverse \(u_\iota(f)^{-1}\) is the final leg of the
...
\begin{proof}
  \leanok
  \uses{mk-def:unitrestrictiso, mk-lem:restrictfunctorisopullback_comp_compat, mk-lem:pullbackObjUnitToUnit_comp, mk-lem:pullback_unit_iso, mk-lem:image_preimage_of_le}
  This is the cheapest constituent: it is the project's proven \emph{unit composition law}
  \(\mathtt{pullbackObjUnitToUnit\_comp}\) (\ref{mk-lem:pullbackObjUnitToUnit_comp}) re-wrapped by
  ...
  \mathtt{pullbackUnitIso}\,f\) (\ref{mk-def:unitrestrictiso}), and
  \(\mathtt{pullbackUnitIso} = \mathtt{pullbackObjUnitToUnitIso}\) (\ref{mk-lem:pullback_unit_iso}).
  Strip the \(\mathtt{restrictFunctorIsoPullback}\) factor across \(j \mathbin{;} \iota_U = \iota_V\)
  by Bridge B2 (\ref{mk-lem:restrictfunctorisopullback_comp_compat}); what remains is exactly the
  ...
  \ref{mk-lem:pullbackObjUnitToUnit_comp}. Taking inverses and threading the \(\varrho\) reindexing of
  \ref{mk-lem:image_preimage_of_le} gives the stated equation for \(u_\iota^{-1}\).
\end{proof}

\begin{lemma}[RFIP pseudonaturality across an equality of open immersions]\label{mk-lem:restrictfunctorisopullback_congr_hom}
  ...
  (\ref{mk-lem:restrict_iso_unit_of_le_eq_restrict}) to push the leading comparison onto the literal
...
\begin{lemma}[Seam 1 keystone: the unit-of-\(\le\) restriction iso is the chart restriction of \(e^M\)]\label{mk-lem:restrict_iso_unit_of_le_eq_restrict}
  \lean{AlgebraicGeometry.Scheme.Modules.restrictIsoUnitOfLE_eq_restrict}
  \leanok
  \dcref{custom}

  \uses{mk-def:restrictcompreindex, mk-def:unitrestrictiso, mk-lem:restrictfunctorisopullback_comp_general, mk-lem:restrictfunctorisopullback_congr_hom}
  \textit{Source: internal categorical construction; no external reference.}
  Let \(j : V \hookrightarrow U\) be the chart morphism (\(j \mathbin{;} \iota_U = \iota_V\)) and
  ...
  (\ref{mk-def:restrictcompreindex}) and \(\mathtt{unitRestrictIso}\,j\) the unit-restriction
  identification \(u_\iota(j)\) (\ref{mk-def:unitrestrictiso}).
\end{lemma}
\begin{proof}
  \leanok
  \uses{mk-def:restrictcompreindex, mk-def:unitrestrictiso, mk-lem:restrictfunctorisopullback_comp_general, mk-lem:restrictfunctorisopullback_congr_hom}
  ...
  (\ref{mk-lem:restrictfunctorisopullback_comp_general}) turns the right-hand side into the same
...
\begin{lemma}[Telescope seam 1, Step A: \(e^M\)-bifunctoriality split of constituent (2)]\label{mk-lem:trivialisation_em_split_bifunctor}
  \lean{AlgebraicGeometry.Scheme.Modules.trivialisation_restrict_eM_split_bifunctor}
  \leanok
  \dcref{custom}

  \uses{mk-lem:tensorobj_restrict_iso_natural, mk-def:tensorobjisoofiso}
  \textit{Source: internal categorical construction; no external reference.}
  Let \(j : V \hookrightarrow U\) be the chart morphism (\(j \mathbin{;} \iota_U = \iota_V\)) and
...
\begin{proof}
  \leanok
  \uses{mk-lem:tensorobj_restrict_iso_natural}
  Immediate specialisation of the bifunctorial module-naturality of the tensor-restriction comparison
  (\ref{mk-lem:tensorobj_restrict_iso_natural}) at \(f = j\), \(a = e^M\) and \(b = c\): the source objects
...
\begin{lemma}[Telescope seam 1, Step B: the \(e^M\)-leg refined to the unit-of-\(\le\) restriction iso]\label{mk-lem:trivialisation_em_leg}
  \lean{AlgebraicGeometry.Scheme.Modules.trivialisation_restrict_eM_leg}
  \leanok
  \dcref{custom}

  \uses{mk-lem:restrict_iso_unit_of_le_eq_restrict, mk-def:restrictcompreindex, mk-def:unitrestrictiso}
  \textit{Source: internal categorical construction; no external reference.}
  With \(j : V \hookrightarrow U\), \(h_{VU} : V \le U\) and \(e^M : L|_{\iota_U} \cong \mathcal O_U\),
  write \(\varrho_L = \mathtt{restrictCompReindex}\,j\,L\) (\ref{mk-def:restrictcompreindex}) and
  \(u_j = \mathtt{unitRestrictIso}\,j\) (\ref{mk-def:unitrestrictiso}). The \(\mathtt{restrict}\,j\)-image of
...
\begin{proof}
  \leanok
  \uses{mk-lem:restrict_iso_unit_of_le_eq_restrict, mk-def:restrictcompreindex, mk-def:unitrestrictiso}
  The Seam-1 keystone (\ref{mk-lem:restrict_iso_unit_of_le_eq_restrict}) reads
  \(\mathtt{restrictIsoUnitOfLE}\,h_{VU}\,e^M
...
\begin{lemma}[Telescope seam 1, Step C: the dual-chain leg, distributed and identified leg by leg]\label{mk-lem:trivialisation_dual_chain_leg}
  \lean{AlgebraicGeometry.Scheme.Modules.trivialisation_restrict_dual_leg}
  \leanok
  \dcref{custom}

  \uses{mk-lem:dual_restrict_iso_restrict_compat, mk-lem:dual_restrict_iso_natural, mk-lem:dual_unit_iso_restrict_compat, mk-lem:restrict_iso_unit_of_le_eq_restrict, mk-lem:dualisoofiso_trans, mk-def:scheme_modules_dual_iso_of_iso, mk-def:restrictcompreindex, mk-def:unitrestrictiso}
  \textit{Source: internal categorical construction; no external reference.}
...
\begin{proof}
  \leanok
  \uses{mk-lem:dual_restrict_iso_restrict_compat, mk-lem:dual_restrict_iso_natural, mk-lem:dual_unit_iso_restrict_compat, mk-lem:restrict_iso_unit_of_le_eq_restrict, mk-lem:dualisoofiso_trans, mk-def:scheme_modules_dual_iso_of_iso}
  ...
  \begin{itemize}
    \item \emph{Leg 1}, \(\mathtt{dual\_restrict\_iso}\,\iota_U\): S3
      (\ref{mk-lem:dual_restrict_iso_restrict_compat}) reads
    ...
    \item \emph{Leg 2}, \((\mathtt{dualIsoOfIso}\,e^M)^{-1}\): module-naturality of
      \(\mathtt{dual\_restrict\_iso}\) (T2, \ref{mk-lem:dual_restrict_iso_natural}) reads
    ...
    \item \emph{Leg 3}, \(\mathtt{dual\_unit\_iso}_U\): S4a
      (\ref{mk-lem:dual_unit_iso_restrict_compat}) reads
  ...
  is, by contravariant functoriality of \(\mathtt{dualIsoOfIso}\) (\ref{mk-lem:dualisoofiso_trans},
  \ref{mk-def:scheme_modules_dual_iso_of_iso}),
  ...
  (\ref{mk-lem:restrict_iso_unit_of_le_eq_restrict}), \((\ast)\) is the middle leg
...
\begin{lemma}[Telescope seam 1: \(e^M\)-bifunctoriality split of the wrapped dual constituent]\label{mk-lem:trivialisation_restrict_em_split}
  \lean{AlgebraicGeometry.Scheme.Modules.trivialisation_restrict_eM_split}
  \leanok
  \dcref{custom}

  \uses{mk-lem:trivialisation_em_split_bifunctor, mk-lem:trivialisation_em_leg, mk-lem:trivialisation_dual_chain_leg, mk-lem:tensorobjisoofiso_trans, mk-lem:restrict_iso_unit_of_le_eq_restrict, mk-def:scheme_modules_dual_iso_of_iso, mk-def:unitrestrictiso, mk-def:restrictcompreindex}
  ...
  (\ref{mk-lem:tensorobj_restrict_iso_natural}), into:
  \begin{itemize}
    \item the \(e^M\)-leg, whose \(V\)-refinement is the identification
      \(\mathtt{restrictIsoUnitOfLE}\,h_{VU}\,e^M = (\mathtt{restrict}\,j)\,e^M\)
      (\ref{mk-lem:restrict_iso_unit_of_le_eq_restrict}, \ref{mk-def:unitrestrictiso}); and
    \item the dual-chain leg, distributed leg by leg across \(\mathtt{restrict}\,j\): the bare
      dual-restriction comparison covered by S3 (\ref{mk-lem:dual_restrict_iso_restrict_compat}), the
      contravariant \((\mathtt{dualIsoOfIso}\,e^M)^{-1}\) (\ref{mk-def:scheme_modules_dual_iso_of_iso})
      transported by module-naturality of \(\mathtt{dual\_restrict\_iso}\)
      (\ref{mk-lem:dual_restrict_iso_natural}) into \((\mathtt{dualIsoOfIso}\,((\mathtt{restrict}\,j)\,e^M))^{-1}\),
      and \(\mathtt{dual\_unit\_iso}\) covered by S4a (\ref{mk-lem:dual_unit_iso_restrict_compat}).
  \end{itemize}
  Concretely, modulo the reindexing \(\varrho = \mathtt{restrictCompReindex}\,j\)
  (\ref{mk-def:restrictcompreindex}), the \(\mathtt{restrict}\,j\)-image of constituent (2) equals the
...
\begin{proof}
  \leanok
  \uses{mk-lem:trivialisation_em_split_bifunctor, mk-lem:trivialisation_em_leg, mk-lem:trivialisation_dual_chain_leg, mk-lem:tensorobjisoofiso_trans, mk-lem:restrict_iso_unit_of_le_eq_restrict, mk-def:scheme_modules_dual_iso_of_iso, mk-def:unitrestrictiso, mk-def:restrictcompreindex}
  Assemble the three steps of this seam. Step A (\ref{mk-lem:trivialisation_em_split_bifunctor}) splits the
  ...
  (\ref{mk-lem:trivialisation_em_leg}) identifies the first inner argument as
  \(\varrho_L^{-1} \mathbin{;} \mathtt{restrictIsoUnitOfLE}\,h_{VU}\,e^M \mathbin{;} u_j^{-1}\) (keystone,
  \ref{mk-lem:restrict_iso_unit_of_le_eq_restrict}, \ref{mk-def:unitrestrictiso}) and Step C
  (\ref{mk-lem:trivialisation_dual_chain_leg}) the second as
  ...
  bifunctoriality law (\ref{mk-lem:tensorobjisoofiso_trans}), the leading \(\varrho^{-1}\) and trailing
...
\begin{lemma}[Telescope seam 2: generic \(\varrho\)-cancellation assembly of the five squares]\label{mk-lem:trivialisation_telescope_assemble}
  \lean{AlgebraicGeometry.Scheme.Modules.trivialisation_telescope_assemble}
  \leanok
  \dcref{custom}

  \uses{mk-lem:tensorobj_restrict_iso_restrict_compat, mk-lem:dual_restrict_iso_restrict_compat, mk-lem:dual_unit_iso_restrict_compat, mk-lem:tensorobj_unit_iso_restrict_compat, mk-lem:trivialisation_uiota_restrict_compat, mk-def:restrictcompreindex}
  \textit{Source: internal categorical construction; no external reference.}
  ...
  \ref{mk-def:restrictcompreindex}), and the two outer reindexings \(\varrho_0, \varrho_5\), suppose each
...
\begin{lemma}[Telescope seam 3: sectionwise evaluation of the two outer reindexings]\label{mk-lem:trivialisation_restrict_sectionwise}
  \lean{AlgebraicGeometry.Scheme.Modules.trivialisation_restrict_sectionwise}
  \leanok
  \dcref{custom}

  \uses{mk-lem:image_preimage_of_le, mk-lem:trivialisation_telescope_assemble}
  ...
  identities of \ref{mk-lem:image_preimage_of_le}, with images \(\iota_U\,''^U(\iota_U^{-1}(V)) = V\) and
  \(\iota_V\,''^V(\iota_V^{-1}(V)) = V\). Threading the corresponding \(\mathtt{eqToHom}\) transports
  through the two flanks of the telescoped chain (\ref{mk-lem:trivialisation_telescope_assemble}) and
  ...
  \ref{mk-lem:trivialisation_restrict_compat}.
\end{lemma}
\begin{proof}
  \leanok
  \uses{mk-lem:image_preimage_of_le, mk-lem:trivialisation_telescope_assemble}
  The telescoped identity (\ref{mk-lem:trivialisation_telescope_assemble}) leaves only the outer
...
\begin{lemma}[Trivialisation iso-chain compatible with further restriction (cocycle residual A, step 1)]\label{mk-lem:trivialisation_restrict_compat}
  \lean{AlgebraicGeometry.Scheme.Modules.trivialisation_restrict_compat}
  \leanok
  \dcref{custom}

  \uses{mk-lem:tensorobj_restrict_iso_restrict_compat, mk-lem:dual_restrict_iso_restrict_compat, mk-lem:dual_unit_iso_restrict_compat, mk-lem:tensorobj_unit_iso_restrict_compat, mk-lem:trivialisation_uiota_restrict_compat, mk-lem:restrictfunctorisopullback_comp_compat, mk-def:restrictcompreindex, mk-lem:image_preimage_of_le, mk-lem:trivialisation_restrict_em_split, mk-lem:trivialisation_telescope_assemble, mk-lem:trivialisation_restrict_sectionwise}
  ...
  contraction --- the restriction-compatibility iso \ref{mk-lem:tensorobj_restrict_iso}
  carrying \((L \otimes_X L^{-1})|_U\) to \(\mathcal{O}_U \otimes_U \mathcal{O}_U\),
  composed with the unit contraction \ref{mk-lem:tensorobj_unit_iso} --- is \emph{natural in
  ...
  (\ref{mk-lem:tensorobjisoofiso_trans}, \ref{mk-lem:dualisoofiso_trans}) and
  \ref{mk-lem:tensorobj_unit_self_duality_collapse} can act.
\end{lemma}
\begin{proof}
  \leanok
  \uses{mk-lem:trivialisation_restrict_em_split, mk-lem:trivialisation_telescope_assemble, mk-lem:trivialisation_restrict_sectionwise, mk-lem:tensorobjisoofiso_trans, mk-lem:dualisoofiso_trans}
  ...
  its own restriction-naturality square (\ref{mk-lem:tensorobj_restrict_iso_restrict_compat},
  \ref{mk-lem:dual_restrict_iso_restrict_compat}, \ref{mk-lem:dual_unit_iso_restrict_compat},
  \ref{mk-lem:tensorobj_unit_iso_restrict_compat}, \ref{mk-lem:trivialisation_uiota_restrict_compat}). The
  assembly is the three-seam chain of this section. Seam~1
  (\ref{mk-lem:trivialisation_restrict_em_split}) handles the \((\mathtt{dualIsoOfIso}\,e^M)^{-1}\)
  ...
  dual-chain leg covered by S3/S4a. Seam~2 (\ref{mk-lem:trivialisation_telescope_assemble}) is the generic
  ...
  (\ref{mk-lem:trivialisation_restrict_sectionwise}) threads those two outer \(\mathtt{eqToHom}\)s and
  ...
  the iso-level functoriality laws (\ref{mk-lem:tensorobjisoofiso_trans}, \ref{mk-lem:dualisoofiso_trans})
  and \ref{mk-lem:tensorobj_unit_self_duality_collapse} act.
\end{proof}

\begin{lemma}[Mutually-inverse unit endomorphisms cancel against the left unitor]\label{mk-lem:tensorhom_inv_comp_leftunitor}
...
\begin{lemma}[Pairing mutually-inverse unit autos through $\mathtt{tensorObjIsoOfIso}$ contracts (cocycle residual A, step 2b)]\label{mk-lem:tensorobjisoofiso_comp_unit_iso}
  \lean{AlgebraicGeometry.Scheme.Modules.tensorObjIsoOfIso_comp_unit_iso}
  \leanok
  \dcref{custom}

  \uses{mk-lem:tensorhom_inv_comp_leftunitor, mk-def:tensorobjisoofiso, mk-lem:tensorobj_unit_iso}
...
\begin{proof}
  ...
  unit; \ref{mk-lem:tensorhom_inv_comp_leftunitor} collapses the composite to the bare unitor.
...
[Naturality of the presheaf dual-of-unit iso against a unit automorphism (B1 eval-core)]
  \label{mk-lem:presheafdualunitiso_naturality}
  \lean{}
  \lean{AlgebraicGeometry.Scheme.Modules.presheafDualUnitIso_naturality}
  \dcref{custom}

  \uses{mk-def:presheaf_dual_unit_iso, mk-def:dual_unit_iso_gen, mk-lem:internal_hom_eval, mk-lem:presheaf_dualisoofiso_trans}
...
\begin{proof}
  \uses{mk-def:presheaf_dual_unit_iso, mk-def:dual_unit_iso_gen, mk-lem:internal_hom_eval, mk-lem:presheaf_dualisoofiso_trans}
  By construction (\ref{mk-def:dual_unit_iso_gen}, \ref{mk-def:presheaf_dual_unit_iso})
  ...
  underlying map of \(\hat s\) (\ref{mk-lem:presheaf_dualisoofiso_trans}), so the left-hand composite
  ...
  internal-hom evaluation (\ref{mk-lem:internal_hom_eval}) specialised at the unit.
\end{proof}

\begin{lemma}[Dual of a unit automorphism is itself (cocycle residual A, step 2a / B1)]\label{mk-lem:dualunitiso_dualisoofiso}
  \lean{AlgebraicGeometry.Scheme.Modules.dualUnitIso_dualIsoOfIso}
  \leanok
  \dcref{custom}

  \uses{mk-lem:dual_unit_iso, mk-def:scheme_modules_dual_iso_of_iso, mk-lem:presheafdualunitiso_naturality, mk-def:presheaf_dual_unit_iso}
...
\begin{proof}
  ...
  \begin{itemize}
    \item the presheaf eval-core naturality
      \(\mathtt{dualIsoOfIso}\,\hat s \mathbin{;} \mathtt{presheafDualUnitIso}
        = \mathtt{presheafDualUnitIso} \mathbin{;} \hat s\)
      (\ref{mk-lem:presheafdualunitiso_naturality}, with \(\hat s\) the underlying presheaf map of
...
\begin{lemma}[Unit self-duality evaluation collapse (cocycle residual A, step 2)]\label{mk-lem:tensorobj_unit_self_duality_collapse}
  \lean{AlgebraicGeometry.Scheme.Modules.tensorObj_unit_self_duality_collapse}
  \leanok
  \dcref{custom}

  \uses{mk-lem:tensorobjisoofiso_trans, mk-lem:tensorobjisoofiso_refl, mk-lem:dualisoofiso_trans, mk-lem:dualisoofiso_refl, mk-lem:tensorobj_unit_iso, mk-lem:dual_unit_iso, mk-lem:dualunitiso_dualisoofiso, mk-lem:tensorobjisoofiso_comp_unit_iso}
  ...
  (\ref{mk-lem:dual_unit_iso}) --- and then contracting via the unit comparison cancels:
  \[
...
\begin{proof}
  ...
  \ref{mk-lem:dualunitiso_dualisoofiso} (applied to \(t\), read off its inverse) turns the conjugated
  ...
  contracts against the unit comparison by \ref{mk-lem:tensorobjisoofiso_comp_unit_iso}, leaving
...
[Alternative route via object-level descent (fallback)]
  \label{mk-rem:dual_via_stack}
  \dcref{custom}
  Should the slice-and-sheafify route of
  \ref{mk-def:presheaf_internal_hom} and \ref{mk-lem:internal_hom_isSheaf} bottom out, the dual can
  ...
  associator re-route of \ref{mk-lem:tensorobj_assoc_iso} also waits.
\end{remark}

\subsection{Internal-hom and restrict-scalars helper declarations}
\label{mk-subsec:tensorobj_internalhom_helpers}


\begin{definition}[Canonical ring map from a terminal object]\label{mk-def:term_ring_map}
...
\begin{lemma}[Naturality of the terminal ring map]\label{mk-lem:term_ring_map_naturality}
  \lean{PresheafOfModules.InternalHom.termRingMap_naturality}
  \leanok
  \dcref{custom}

  \uses{mk-def:term_ring_map}
...
\begin{definition}[Multiplication by a global scalar as an endomorphism]\label{mk-def:global_smul}
  \lean{PresheafOfModules.InternalHom.globalSMul}
  \leanok
  \dcref{custom}

  \uses{mk-def:term_ring_map, mk-lem:term_ring_map_naturality}
...
\begin{definition}[Evaluation functional of a dual section]\label{mk-def:eval_lin}
...
\begin{definition}[Lax-monoidal unit of restrict-scalars]\label{mk-def:restrict_scalars_lax_epsilon}
...
\begin{definition}[Lax-monoidal tensorator of restrict-scalars]\label{mk-def:restrict_scalars_lax_mu}
...
\begin{definition}[Underlying \(\mathrm{Ab}\)-presheaf of the internal hom]\label{mk-def:internal_hom_presheaf}
  \lean{PresheafOfModules.InternalHom.internalHomPresheaf}
  \leanok
  \dcref{custom}

  \uses{mk-def:presheaf_internal_hom, mk-def:global_smul}
  \textit{Source: \cite{stacks-project}, Modules on Ringed Spaces, \S Internal Hom.}
  ...
  \ref{mk-def:presheaf_internal_hom}; the \(R(U)\)-module refinement is supplied by the
  global scalar action \ref{mk-def:global_smul}.
\end{definition}
\begin{proof}
  \leanok
  \uses{mk-def:presheaf_internal_hom, mk-def:global_smul}
 Functoriality is \(\mathtt{restrictionMap\_id}\)/%
...
\begin{definition}[Open-by-open evaluation pairing of the internal hom]\label{mk-def:internal_hom_eval}
  \lean{PresheafOfModules.internalHomEvalApp}
  \leanok
  \dcref{custom}

  \uses{mk-def:presheaf_dual, mk-def:eval_lin, mk-lem:internal_hom_eval}
  \textit{Source: \cite{stacks-project}, Modules on Ringed Spaces, \S Internal Hom.}
  ...
  pairing \(M \otimes M^\vee \to \mathcal{O}\) of \ref{mk-lem:internal_hom_eval}; it is the
  substrate of the tensor--hom counit used to invert an invertible module.
\end{definition}
\begin{proof}
  \leanok
  \uses{mk-def:eval_lin}
 Bilinearity over \(R(X)\): linearity in \(s\) is linearity of
  the evaluation functional \(\mathtt{evalLin}\) (\ref{mk-def:eval_lin}); linearity in
...
[Extension of scalars is left adjoint to restriction of scalars]
  \label{mk-lem:extend_scalars_adjunction}
  \dcref{05DQ,custom}

  \uses{mk-lem:restrictscalars_laxmonoidal, mk-lem:restrictscalars_ringiso_tensorequiv}
...
[Pullback is left adjoint to pushforward on module sheaves]
  \label{mk-lem:pushforward_adjunction}
  \dcref{0096,custom}
...
\label{mk-subsec:tensorobj_vestigial_helpers}


\begin{lemma}
[A module morphism is a stalkwise isomorphism iff its stalk maps are bijective]
  \label{mk-lem:iso_iff_stalk_bijective}
  \dcref{007T,custom}

  \uses{mk-lem:stalk_linear_map}
  ...
  then the \(R_x\)-linear stalk map \ref{mk-lem:stalk_linear_map} is bijective and hence
  upgrades to an \(R_x\)-linear equivalence \(M_x \xrightarrow{\sim} N_x\)
  (\(\mathtt{stalkLinearEquivOfIsIso}\)).
\end{lemma}
\begin{proof}
  \uses{mk-lem:stalk_linear_map}
 The forgetful functor to \(\mathrm{Ab}\) sends an isomorphism
  to a bijection, and the \(R_x\)-linear structure of the stalk map
  \ref{mk-lem:stalk_linear_map} shares the same underlying additive map; bundling the
...
[Local injectivity is equivalent to injectivity on stalks]
  \label{mk-lem:locally_injective_iff_stalk}
  \dcref{007T,custom}
...
\begin{proof}
  \uses{mk-lem:stalk_linear_map}
 A section killed on a stalk is killed on some neighbourhood,
  ...
  stalkwise-property criterion in the same family as \ref{mk-lem:iso_iff_stalk_bijective},
...
[Tensoring with a flat module preserves the weak-equivalence class]
  ...
  unconditional \ref{mk-lem:islocallyinjective_whiskerleft_via_stalk}.
\end{lemma}
\begin{proof}
  \uses{mk-def:scheme_modules_tensorobj}
 Flatness of \(F\) makes \(F \otimes (-)\) exact, hence it
...
[Sheaf-category equivalence induced by an over-slice dense subsite]
  \label{mk-lem:over_slice_sheaf_equiv}
  \dcref{03CH,custom}
...
\begin{definition}[Substrate \(\otimes\) respects a pair of isomorphisms]\label{mk-def:tensorobjisoofiso}
...
\begin{proof}
  \leanok
  \uses{mk-def:scheme_modules_tensorobj}
  It is obtained by applying \(\mathrm{mapIso}\) to the monoidal
  \(\mathtt{tensorIso}\) of the two forgetful images of the given isomorphisms.
\end{proof}
\begin{lemma}[\(\mathtt{tensorObjIsoOfIso}\) is bifunctorial in composition]\label{mk-lem:tensorobjisoofiso_trans}
  \lean{AlgebraicGeometry.Scheme.Modules.tensorObjIsoOfIso_trans}
  \leanok
  \dcref{custom}

  \uses{mk-def:tensorobjisoofiso}
...
\begin{lemma}[\(\mathtt{tensorObjIsoOfIso}\) preserves identities]\label{mk-lem:tensorobjisoofiso_refl}
...
\end{proof}
\begin{lemma}[Threefold tensor/composition interchange]\label{mk-lem:tensorhom_comp3}
...
\begin{lemma}[Functor image of the threefold interchange]\label{mk-lem:map_tensorhom_comp3}
  \lean{AlgebraicGeometry.Scheme.Modules.map_tensorHom_comp3}
  \leanok
  \dcref{custom}

  \uses{mk-lem:tensorhom_comp3}
  For a functor \(F : C \to D\) out of a monoidal category, the \(F\)-image of the
  threefold tensor/composition interchange (\ref{mk-lem:tensorhom_comp3}) splits as the
  threefold composite of the \(F\)-images of the per-leg tensors.
\end{lemma}
\begin{proof}
  \leanok
  \uses{mk-lem:tensorhom_comp3}
  Apply \ref{mk-lem:tensorhom_comp3} inside \(F\), then functoriality of \(F\) on composition
  (\(F(g \circ f) = F(g) \circ F(f)\)) twice.
\end{proof}

\begin{lemma}[Presheaf dual-transport is contravariantly functorial in composition]\label{mk-lem:presheaf_dualisoofiso_trans}
  \lean{AlgebraicGeometry.Scheme.Modules.presheaf_dualIsoOfIso_trans}
  \leanok
  \dcref{custom}

  \uses{mk-def:presheaf_dual_iso_of_iso}
...
\begin{lemma}[Presheaf dual-transport preserves identities]\label{mk-lem:presheaf_dualisoofiso_refl}
...
\begin{lemma}[Sheaf-level dual-transport is contravariantly functorial in composition]\label{mk-lem:dualisoofiso_trans}
  \lean{AlgebraicGeometry.Scheme.Modules.dualIsoOfIso_trans}
  \leanok
  \dcref{custom}

  \uses{mk-def:scheme_modules_dual_iso_of_iso, mk-lem:presheaf_dualisoofiso_trans}
...
\begin{proof}
  ...
  (\ref{mk-lem:presheaf_dualisoofiso_trans}) transported through the sheaf-equivalence and the
  forgetful functor; functoriality of \(\mathrm{mapIso}\) carries the presheaf identity to the
  sheaf identity.
\end{proof}

\begin{lemma}[Sheaf-level dual-transport preserves identities]\label{mk-lem:dualisoofiso_refl}
  \lean{AlgebraicGeometry.Scheme.Modules.dualIsoOfIso_refl}
  \leanok
  \dcref{custom}

  \uses{mk-def:scheme_modules_dual_iso_of_iso, mk-lem:presheaf_dualisoofiso_refl}
...
\begin{proof}
  \leanok
  \uses{mk-def:scheme_modules_dual_iso_of_iso, mk-lem:presheaf_dualisoofiso_refl}
  Immediate from the presheaf-level statement \ref{mk-lem:presheaf_dualisoofiso_refl} and
  functoriality of \(\mathrm{mapIso}\) on the identity.
\end{proof}

\begin{definition}[\(\mathcal{O}_X \otimes_X \mathcal{O}_X \cong \mathcal{O}_X\)]\label{mk-def:tensorobj_unit_self_iso}
...
\begin{definition}[Middle-four interchange for \(\otimes_X\)]\label{mk-def:tensorobj_middlefour}
...
\begin{proof}
  \leanok
  \uses{mk-lem:tensorobj_assoc_iso, mk-lem:tensorobj_comm_iso, mk-def:tensorobjisoofiso}
  \ref{mk-lem:tensorobj_assoc_iso} and the braiding \ref{mk-lem:tensorobj_comm_iso} through
  \(\mathtt{tensorObjIsoOfIso}\); no coherence pentagon is consumed.
\end{proof}

\begin{lemma}[Unit comparison is an iso from a finality hypothesis]\label{mk-lem:isiso_pbu_of_final}
...
\begin{definition}[Bundled iso form of the unit comparison]\label{mk-def:pullback_obj_unit_to_unit_iso}
  \lean{AlgebraicGeometry.Scheme.Modules.pullbackObjUnitToUnitIso}
  \leanok
  \dcref{custom}

  \uses{mk-lem:isiso_pbu_of_final}
  For \(g : Y \to X\) with \((\mathtt{Opens.map}\,g.\mathtt{base}).\mathtt{Final}\), the
  isomorphism \(g^*\mathcal{O}_X \cong \mathcal{O}_Y\) bundling the unit comparison
  \(\mathtt{pullbackObjUnitToUnit}\,g\) of \ref{mk-lem:isiso_pbu_of_final}. Handing out the
  bundled \(\mathtt{Iso}\) keeps downstream reasoning at the \(\mathtt{Iso}\) level.
\end{definition}
\begin{proof}
  \leanok
  \uses{mk-lem:isiso_pbu_of_final}
  \ref{mk-lem:isiso_pbu_of_final}.
\end{proof}

\begin{definition}[Sheafification--monoidality reconciliation brick]\label{mk-def:sheafify_tensor_unit_iso}
  \lean{AlgebraicGeometry.Scheme.Modules.sheafifyTensorUnitIso}
  \leanok
  \dcref{custom}

  \uses{mk-lem:isiso_sheafification_map_of_W, mk-lem:whisker_of_W}
...
\begin{proof}
  ...
  sheafification via \ref{mk-lem:isiso_sheafification_map_of_W} together with the
  flatness-free whiskering stability \ref{mk-lem:whisker_of_W}.
\end{proof}

\begin{lemma}[\(\mathtt{hom}\) of the reconciliation brick as a whisker composite]\label{mk-lem:sheafify_tensor_unit_iso_hom_eq}
  \lean{AlgebraicGeometry.Scheme.Modules.sheafifyTensorUnitIso_hom_eq}
  \leanok
  \dcref{custom}

  \uses{mk-def:sheafify_tensor_unit_iso}
  The \(\mathtt{hom}\) of \ref{mk-def:sheafify_tensor_unit_iso} equals the sheafification of
...
\begin{lemma}[\(\mathtt{hom}\) of the reconciliation brick as one \(\mathtt{tensorHom}\)]\label{mk-lem:sheafify_tensor_unit_iso_hom_eq_prime}
  \lean{AlgebraicGeometry.Scheme.Modules.sheafifyTensorUnitIso_hom_eq'}
  \leanok
  \dcref{custom}

  \uses{mk-lem:sheafify_tensor_unit_iso_hom_eq, mk-def:sheafify_tensor_unit_iso}
  The \(\mathtt{hom}\) of \ref{mk-def:sheafify_tensor_unit_iso} equals the sheafification of
...
\begin{proof}
  \leanok
  \uses{mk-lem:sheafify_tensor_unit_iso_hom_eq}
  \ref{mk-lem:sheafify_tensor_unit_iso_hom_eq} by \(\mathtt{\leftarrow Functor.map\_comp}\)
  and identify with \(\mathtt{tensorHom\_def}\).
\end{proof}

\begin{definition}[Sheafified presheaf-pullback \(\cong\) abstract pullback on a value]\label{mk-def:pullback_val_iso}
  \lean{AlgebraicGeometry.Scheme.Modules.pullbackValIso}
  \leanok
  \dcref{custom}

  \uses{mk-lem:sheafification_comp_pullback_eq_leftadjointuniq}
...
\begin{lemma}[The sheafified \(\otimes\) of the two \(\mathtt{pullbackValIso}\) is an iso]\label{mk-lem:isiso_sheafify_tensorhom_pullbackvaliso}
  \lean{AlgebraicGeometry.Scheme.Modules.isIso_sheafify_tensorHom_pullbackValIso}
  \leanok
  \dcref{custom}

  \uses{mk-def:pullback_val_iso, mk-def:scheme_modules_tensorobj}
...
\begin{lemma}[Composite ring-map reconciliation]\label{mk-lem:toringcatsheafhom_comp_hom_reconcile}
  ...
  decomposition (Sq2 of \ref{mk-lem:pullback_tensor_map_basechange}).
\end{lemma}
\begin{proof}
  \leanok
...
\begin{lemma}[Sq2b --- monoidality of the presheaf \(\mathtt{pullbackComp}\)]\label{mk-lem:pullbackcomp_delta}
  \lean{AlgebraicGeometry.Scheme.Modules.pullbackComp_δ}
  \leanok
  \dcref{custom}

  \uses{mk-lem:presheaf_pullback_oplaxmonoidal, mk-lem:presheaf_pushforward_laxmonoidal, mk-lem:pushforwardcomp_lax_mu}
  ...
  coherence \ref{mk-lem:pullbackObjUnitToUnit_comp}.
\end{lemma}
\begin{proof}
  \leanok
  \uses{mk-lem:presheaf_pullback_oplaxmonoidal, mk-lem:presheaf_pushforward_laxmonoidal}
  pullback--pushforward adjunction, rewrite the oplax \(\delta\) as the mate of the lax
  \(\mu\) of the pushforward (\ref{mk-lem:presheaf_pushforward_laxmonoidal}), and use the
...
\begin{lemma}[Sheafified split of the composite-pullback tensorator (step (ii) sheafified)]\label{mk-lem:sheafifymap_deltacomp_split}
  \lean{AlgebraicGeometry.Scheme.Modules.sheafifyMap_δcomp_split}
  \leanok
  \dcref{custom}

  \uses{mk-lem:pullbackcomp_delta}
...
\begin{lemma}[Sheaf-level conjugate of \(\mathtt{pullbackComp}.\mathrm{inv}\)]\label{mk-lem:sheaf_unit_comp_pushforward_pullbackcomp_inv}
...
\begin{lemma}[Sq1 --- composition coherence of \(\mathtt{sheafificationCompPullback}\)]\label{mk-lem:sheafificationcomppullback_comp}
  \lean{AlgebraicGeometry.Scheme.Modules.sheafificationCompPullback_comp}
  \leanok
  \dcref{custom}

  \uses{mk-lem:sheafification_comp_pullback_eq_leftadjointuniq, mk-lem:sheaf_unit_comp_pushforward_pullbackcomp_inv, mk-lem:sheafificationcomppullback_comp_tail}
  ...
  This is the \(\mathtt{sheafificationCompPullback}\) twin of \ref{mk-lem:pullbackObjUnitToUnit_comp};
  it is the S1-foundational coherence consumed by \ref{mk-lem:pullback_tensor_map_basechange}.
\end{lemma}
\begin{proof}
  \leanok
  \uses{mk-lem:sheafification_comp_pullback_eq_leftadjointuniq, mk-lem:sheaf_unit_comp_pushforward_pullbackcomp_inv, mk-lem:sheafificationcomppullback_comp_tail}
  \(\mathtt{leftAdjointUniq}\) of composite adjunctions
  (\ref{mk-lem:sheafification_comp_pullback_eq_leftadjointuniq}); transpose the identity under
  ...
  \ref{mk-lem:sheaf_unit_comp_pushforward_pullbackcomp_inv}, and discharge the residual unit
  identity by \ref{mk-lem:sheafificationcomppullback_comp_tail}.
\end{proof}

\begin{lemma}[Sq1 tail --- the R1/R5 unit collapse]
  \label{mk-lem:sheafificationcomppullback_comp_tail}
  \lean{}
  \dcref{custom}
  \uses{mk-lem:sheafification_comp_pullback_eq_leftadjointuniq, mk-lem:leftadjointuniq_app_unit_eta_general, mk-lem:pullbackObjUnitToUnit_comp, mk-lem:restrictscalarsid_map, mk-lem:forget_map_pushforward_map}
  \textit{Source: internal categorical construction; no external reference.}
  After the R0-peel of \ref{mk-lem:sheafificationcomppullback_comp}, the residual obligation is
...
\begin{proof}
  ...
  \ref{mk-lem:pullbackObjUnitToUnit_comp}, one sheafification layer up. There the adjunctions
  ...
  (\ref{mk-lem:sheafification_comp_pullback_eq_leftadjointuniq}). The strategy is to keep the
  ...
  uniqueness-of-left-adjoints brick \ref{mk-lem:leftadjointuniq_app_unit_eta_general}
  ...
  \ref{mk-lem:pullbackObjUnitToUnit_comp} does, to \(\mathtt{Iso.inv\_hom\_id}\) /
...
\label{mk-subsec:tensorobj_changeofrings_helpers}


\begin{lemma}[\(\mathtt{restrictScalars}(\mathbf{1})\) on morphisms]\label{mk-lem:restrictscalarsid_map}
...
\begin{lemma}[Sectionwise value of the \(\mathtt{restrictScalars}\) tensorator \(\mu\)]\label{mk-lem:restrictscalars_mu_app}
  \lean{AlgebraicGeometry.Scheme.Modules.restrictScalars_μ_app}
  \leanok
  \dcref{custom}

  \uses{mk-def:restrict_scalars_lax_mu}
...
\begin{proof}
  \leanok
  \uses{mk-def:restrict_scalars_lax_mu}
  \(\mathtt{restrictScalarsLax}\mu\) (\ref{mk-def:restrict_scalars_lax_mu}) is defined
  sectionwise as the \(\mathtt{ModuleCat}\) tensorator.
\end{proof}

\begin{lemma}[Pure-tensor value of the \(\mathtt{ModuleCat}\) \(\mathtt{restrictScalars}\) tensorator]\label{mk-lem:forget2_restrictscalars_mu_hom_tmul}
...
\begin{proof}
  ...
  The change-of-rings tensorator is the identity on pure tensors, by the
  defining formula for \(\mathtt{restrictScalars}\) over commutative rings.
\end{proof}

\begin{lemma}[Pure-tensor value of the presheaf \(\mathtt{restrictScalars}\) tensorator]\label{mk-lem:restrictscalars_mu_app_tmul}
  \lean{AlgebraicGeometry.Scheme.Modules.restrictScalars_μ_app_tmul}
  \leanok
  \dcref{custom}

  \uses{mk-lem:restrictscalars_mu_app}
...
\begin{proof}
  \leanok
  \uses{mk-lem:restrictscalars_mu_app}
  \ref{mk-lem:restrictscalars_mu_app}, then apply the Mathlib pure-tensor rule
  \(\mathtt{ModuleCat.restrictScalars\_}\mu\mathtt{\_tmul}\).
\end{proof}

\begin{lemma}[Pure-tensor value of the presheaf \(\mathtt{restrictScalars}\) \emph{oplax} tensorator \(\delta\)]\label{mk-lem:restrictscalars_delta_app_tmul}
  \lean{AlgebraicGeometry.Scheme.Modules.restrictScalars_δ_app_tmul}
  \leanok
  \dcref{custom}

  \uses{mk-lem:restrictscalars_mu_app_tmul}
  The \(\delta\)-twin of \ref{mk-lem:restrictscalars_mu_app_tmul}. For a sectionwise-bijective
...
\begin{proof}
  \leanok
  \uses{mk-lem:restrictscalars_mu_app_tmul}
  Since \(\delta\) is inverse to \(\mu\), rewrite the goal through the inverse and discharge the resulting
  \(\mu\)-value on the pure tensor by
  \ref{mk-lem:restrictscalars_mu_app_tmul}.
\end{proof}

\begin{lemma}[Pure-tensor value of the \(\mathtt{pushforward}\)-mapped \(\mathtt{restrictScalars}\) tensorator]\label{mk-lem:pushforward_map_restrictscalars_mu_app_tmul}
  \lean{AlgebraicGeometry.Scheme.Modules.pushforward_map_restrictScalars_μ_app_tmul}
  \leanok
  \dcref{custom}

  \uses{mk-lem:restrictscalars_mu_app_tmul}
  The outer leg of \ref{mk-lem:pushforwardcomp_lax_mu}: applied to a pure tensor,
...
\begin{proof}
  ...
  by \ref{mk-lem:restrictscalars_mu_app_tmul}.
\end{proof}

\begin{lemma}[Reduction of the \(\mathtt{pushforward}\) tensorator to the \(\mathtt{restrictScalars}\) \(\mu\)]\label{mk-lem:pushforward_mu_eq}
  \lean{AlgebraicGeometry.Scheme.Modules.pushforward_μ_eq}
  \leanok
  \dcref{custom}

  \uses{mk-lem:presheaf_pushforward_laxmonoidal, mk-def:restrict_scalars_lax_mu}
...
\begin{proof}
  \leanok
  \uses{mk-lem:presheaf_pushforward_laxmonoidal, mk-def:restrict_scalars_lax_mu}
  (\ref{mk-lem:presheaf_pushforward_laxmonoidal}) is the \(\mathtt{Functor.LaxMonoidal.comp}\)
  of the (identity) \(\mathtt{pushforward}_0\) tensorator with the
  \(\mathtt{restrictScalars}\) tensorator (\ref{mk-def:restrict_scalars_lax_mu}), which is
  definitionally the right-hand side.
\end{proof}

\begin{lemma}[Sq2b residual --- monoidality of the \(\mathtt{pushforward}\) composition cell]\label{mk-lem:pushforwardcomp_lax_mu}
  \lean{AlgebraicGeometry.Scheme.Modules.pushforwardComp_lax_μ}
  \leanok
  \dcref{custom}

  \uses{mk-lem:pushforward_mu_eq, mk-lem:restrictscalars_mu_app, mk-lem:forget2_restrictscalars_mu_hom_tmul, mk-lem:pushforward_map_restrictscalars_mu_app_tmul}
  ...
  the sole genuine residual of the presheaf-level Sq2b lemma \ref{mk-lem:pullbackcomp_delta}.
\end{lemma}
\begin{proof}
  \leanok
  \uses{mk-lem:pushforward_mu_eq, mk-lem:restrictscalars_mu_app, mk-lem:forget2_restrictscalars_mu_hom_tmul, mk-lem:pushforward_map_restrictscalars_mu_app_tmul}
  ...
  \ref{mk-lem:pushforward_mu_eq}, expose the \(\mathtt{ModuleCat}\) \(\mu\) by
  \ref{mk-lem:restrictscalars_mu_app}, and collapse the inner leg by
  \ref{mk-lem:forget2_restrictscalars_mu_hom_tmul} and the outer
  \((\mathtt{pushforward}\,\varphi).\mathrm{map}\) leg by
  \ref{mk-lem:pushforward_map_restrictscalars_mu_app_tmul}; the two collapsed pure tensors are
...
\begin{lemma}[Forgetful image of a sheaf-pushforward map]\label{mk-lem:forget_map_pushforward_map}
...
\label{mk-subsec:tensorobj_pic_quotient_helpers}

This subsection blueprints the three quotient-level helpers underlying the
invertibility-carrier Picard group \ref{mk-def:pic_carrier} and its abelian-group law
\texttt{pic\_commgroup}: the isomorphism setoid, the tensor-induced multiplication, and
the dual-induced inverse on iso-classes.

\begin{definition}[The isomorphism setoid of invertible modules]\label{mk-def:pic_setoid}
  \lean{AlgebraicGeometry.Scheme.Modules.picSetoid}
  \leanok
  \dcref{custom}

  \uses{mk-def:scheme_modules_isinvertible}
...
\begin{definition}[Multiplication on \(\Pic X\) induced by \(\otimes_X\)]\label{mk-def:pic_mul}
  \lean{AlgebraicGeometry.Scheme.Modules.picMul}
  \leanok
  \dcref{custom}

  \uses{mk-def:pic_setoid, mk-def:pic_carrier, mk-def:tensorobjisoofiso, mk-lem:isinvertible_tensor}
...
\begin{proof}
  \leanok
  \uses{mk-def:pic_setoid, mk-def:pic_carrier, mk-def:tensorobjisoofiso, mk-lem:isinvertible_tensor}
The quotient lift is well-defined: the value lands in \(\Pic X\)
  because the tensor of invertibles is invertible (\ref{mk-lem:isinvertible_tensor}), and it
  respects the setoid because \(\mathtt{tensorObjIsoOfIso}\) (\ref{mk-def:tensorobjisoofiso})
  turns a pair of isomorphisms into an isomorphism of tensors.
\end{proof}

\begin{definition}[Inverse on \(\Pic X\) induced by the tensor-inverse]\label{mk-def:pic_inv}
  \lean{AlgebraicGeometry.Scheme.Modules.picInv}
  \leanok
  \dcref{custom}

  \uses{mk-def:pic_setoid, mk-def:pic_carrier, mk-lem:isinvertible_inverse_welldef, mk-lem:tensorobj_comm_iso}
...
\begin{proof}
  \leanok
  \uses{mk-def:pic_setoid, mk-def:pic_carrier, mk-lem:isinvertible_inverse_welldef, mk-lem:tensorobj_comm_iso}
A chosen inverse witness
  (\(\mathtt{Classical.choose}\) of the invertibility) is itself invertible, with the
  braiding (\ref{mk-lem:tensorobj_comm_iso}) supplying the membership isomorphism; the choice
  is independent of the representative because any two tensor-inverses of \(M\) are
  isomorphic (\ref{mk-lem:isinvertible_inverse_welldef}).
\end{proof}

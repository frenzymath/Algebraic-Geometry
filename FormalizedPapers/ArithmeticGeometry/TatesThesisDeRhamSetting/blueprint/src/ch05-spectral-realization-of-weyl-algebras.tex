...
\end{equation}
 ...
 We then need to check that the $\vph_{\omega}$ operators
...
\]
 ...
 Applying $p_{2,*}^{\IndCoh}$, we obtain a canonical
morphism:
%
\[
...
\]
 ...
 By \emph{loc. cit}., this is a morphism
of algebras.
...
\]
 ...
 where the first map is given by the action and the second
...
\]
 ...
 where $\pi_n$ is the ind-proper morphism $\sZ^{\leq n} \to \sY^{\leq n}$
...
\]
 ...
 from \eqref{eq:integration}. The detailed construction articulates
...
\]
 ...
 (with the last term really meaning the
...
\]
 ...
 We then applying $\act_{neg,*}^{\IndCoh}$
...
\]
 ...
 Composing these two maps, we obtain
...
\]
 ...
 as the composition of $\on{can} (= \on{can}_1)$
with $f\cdot-$.
...
\]
 ...
 By adjunction, we obtain the desired
morphism:
%
\[
\xi_f:\sF_n \to \sF_n.
...
\]
 ...
 is $\bZ/2$-equivariant (for the natural action
...
\]
 ...
 given as follows. To a point of the left hand side
defined by
...
\]
 ...
 Indeed, for a point $(D,(\sL,s))$ of the left hand
...
\]
 ...
 The corresponding map $\sF_n \to \sF_n$ is
\eqref{eq:fns}.
...
\]
 ...
 We remark that $p_{13}$ corresponds to the multiplication
on the groupoid.
...
\]
 ...
 yields an isomorphism: % reference?
%
\[
p_{12}^!\sO_{\sH^{\leq n}} =
...
\]
 ...
 Combining the above isomorphims yields \eqref{eq:p13-!}.
...
\]
 ...
 where the last arrow is $(p_2,\act)$. Note that
...
\]
 ...
 where this time the last arrow is
...
\]
 ...
 scheme-theoretically supported on
...
\]
 ...
 is $\omega \boxtimes p_2^*(f)\cdot\omega^{univ}$,
...
\]
 ...
 is:\footnote{For clarity, we again
...
\]
 ...
 for $m$ the multiplication on $\Gr_{\bG_m}^{\leq n}$.
...
\end{align*}
 ...
 where in the last two lines the map
...
\]
 ...
 maps to $\Res(f\omega) \in k$ under the
adjunction morphism:
%
\[
\Gamma^{\IndCoh}(\sD^{\leq n} \times \sD^{\leq n},
...
\]
 ...
 It is easy to see that when we evaluate this
...
\]
 ...
 By Lemma \ref{l:cotr-2}, this section is supported
...
\]
 ...
 This amounts to \eqref{eq:uncertainty}, concluding
the argument.

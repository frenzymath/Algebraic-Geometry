...
\begin{lemma}[Extend over dvr]
\label{curves-lemma-extend-over-dvr}
\source{stacks:0BXY}
\href{https://stacks.math.columbia.edu/tag/0BXY}{\texttt{Stacks tag 0BXY}}
\group{curves}
...
\begin{lemma}[Extend over normal curve]
\label{curves-lemma-extend-over-normal-curve}
\source{stacks:0BXZ}
\href{https://stacks.math.columbia.edu/tag/0BXZ}{\texttt{Stacks tag 0BXZ}}
\group{curves}
...
\begin{proof}
\uses{curves-lemma-extend-over-dvr}

A rational map from $X$ to $Y$ can be extended to a morphism $X \to Y$
by Lemma \ref{curves-lemma-extend-over-dvr}
...
\source{stacks:0CCK}
\href{https://stacks.math.columbia.edu/tag/0CCK}{\texttt{Stacks tag 0CCK}}
\group{curves}
...
\source{stacks:0CCL}
\href{https://stacks.math.columbia.edu/tag/0CCL}{\texttt{Stacks tag 0CCL}}
\group{curves}
...
\begin{lemma}[Extend to completion]
\label{curves-lemma-extend-to-completion}
\source{stacks:0BY0}
\href{https://stacks.math.columbia.edu/tag/0BY0}{\texttt{Stacks tag 0BY0}}
\group{curves}
...
This is clear from Lemma \ref{curves-lemma-extend-over-dvr}
...
\begin{theorem}[Curves rational maps]
\label{curves-theorem-curves-rational-maps}
\source{stacks:0BY1}
\href{https://stacks.math.columbia.edu/tag/0BY1}{\texttt{Stacks tag 0BY1}}
\group{curves}
...
\begin{proof}
\uses{curves-lemma-extend-over-normal-curve, curves-lemma-extend-over-dvr}
...
Lemma \ref{curves-lemma-extend-over-normal-curve}.
Conversely, suppose that we have a map
$k(Y) \to k(X)$ in the category (1). Then we obtain a morphism $U \to Y$
for some nonempty open $U \subset X$. By Lemma \ref{curves-lemma-extend-over-dvr}
...
\begin{definition}[Normal projective model]
\label{curves-definition-normal-projective-model}
\source{stacks:0BY2}
\href{https://stacks.math.columbia.edu/tag/0BY2}{\texttt{Stacks tag 0BY2}}
\group{curves}
...
isomorphism by Theorem \ref{curves-theorem-curves-rational-maps}.
...
but Lemma \ref{curves-lemma-nonsingular-model-smooth}
shows this can only happen in positive characteristic.

\begin{lemma}[Nonsingular model smooth]
\label{curves-lemma-nonsingular-model-smooth}
\source{stacks:0BY3}
\href{https://stacks.math.columbia.edu/tag/0BY3}{\texttt{Stacks tag 0BY3}}
\group{curves}
...
\begin{lemma}[Smooth models]
\label{curves-lemma-smooth-models}
\source{stacks:0BY4}
\href{https://stacks.math.columbia.edu/tag/0BY4}{\texttt{Stacks tag 0BY4}}
\group{curves}
...
$$
\begin{array}{cc}\Omega & K' \longleftarrow \\ K \uparrow & k \longleftarrow\uparrow\end{array}
$$
...
(see Remark \ref{curves-remark-classical-linear-series})
by defining linear series in the following manner.

\begin{definition}[Linear series]
\label{curves-definition-linear-series}
\source{stacks:0CCN}
\href{https://stacks.math.columbia.edu/tag/0CCN}{\texttt{Stacks tag 0CCN}}
\group{curves}
...
\begin{lemma}[Linear series]
\label{curves-lemma-linear-series}
\source{stacks:0CCP}
\href{https://stacks.math.columbia.edu/tag/0CCP}{\texttt{Stacks tag 0CCP}}
\group{curves}
...
\begin{lemma}[Linear series trivial existence]
\label{curves-lemma-linear-series-trivial-existence}
\source{stacks:0CCQ}
\href{https://stacks.math.columbia.edu/tag/0CCQ}{\texttt{Stacks tag 0CCQ}}
\group{curves}
...
\source{stacks:0CCR}
\href{https://stacks.math.columbia.edu/tag/0CCR}{\texttt{Stacks tag 0CCR}}
\group{curves}

\uses{curves-lemma-linear-series}
Let $k$ be a field. Let $X$ be a nonsingular proper curve over $k$.
Let $(\mathcal{L}, V)$ be a $\mathfrak g^1_d$ on $X$. Then the morphism
$\varphi : X \to \mathbf{P}^1_k$ of Lemma \ref{curves-lemma-linear-series}
either
\begin{enumerate}
...
By Lemma \ref{curves-lemma-linear-series} we see that
...
\begin{lemma}[Grd inequalities]
\label{curves-lemma-grd-inequalities}
\source{stacks:0CCS}
\href{https://stacks.math.columbia.edu/tag/0CCS}{\texttt{Stacks tag 0CCS}}
\group{curves}

\uses{curves-definition-genus}
...
the genus of $X$ (Definition \ref{curves-definition-genus}) is $0$.
\end{lemma}
...
\source{stacks:0CCT}
\href{https://stacks.math.columbia.edu/tag/0CCT}{\texttt{Stacks tag 0CCT}}
\group{curves}
...
\begin{lemma}[Duality dim 1]
\label{curves-lemma-duality-dim-1}
\source{stacks:0BS2}
\href{https://stacks.math.columbia.edu/tag/0BS2}{\texttt{Stacks tag 0BS2}}
\group{curves}
...
\begin{lemma}[Duality dim 1 CM]
\label{curves-lemma-duality-dim-1-CM}
\source{stacks:0BS3}
\href{https://stacks.math.columbia.edu/tag/0BS3}{\texttt{Stacks tag 0BS3}}
\group{curves}

\uses{curves-lemma-duality-dim-1}
Let $X$ be a proper scheme over a field $k$ which is Cohen-Macaulay
and equidimensional of dimension $1$. The module $\omega_X$
of Lemma \ref{curves-lemma-duality-dim-1} has the following properties
\begin{enumerate}
...
Recall from the proof of Lemma \ref{curves-lemma-duality-dim-1}
...
Lemma \ref{curves-lemma-duality-dim-1}
...
\begin{remark}[Rework duality locally free]
\label{curves-remark-rework-duality-locally-free}
\source{stacks:0BS4}
\href{https://stacks.math.columbia.edu/tag/0BS4}{\texttt{Stacks tag 0BS4}}
\group{curves}

\uses{curves-lemma-duality-dim-1, curves-lemma-duality-dim-1-CM}
Let $X$ be a proper scheme of dimension $\leq 1$ over a field $k$.
Let $\omega_X^\bullet$ and $\omega_X$ be as in Lemma \ref{curves-lemma-duality-dim-1}.
...
$$
by Lemma \ref{curves-lemma-duality-dim-1-CM}. In particular
...
\source{stacks:0E32}
\href{https://stacks.math.columbia.edu/tag/0E32}{\texttt{Stacks tag 0E32}}
\group{curves}
...
are as in  Lemma \ref{curves-lemma-duality-dim-1} for the morphism
$X_K \to \Spec(K)$.
\end{enumerate}
\end{lemma}
...
are as in Lemma \ref{curves-lemma-duality-dim-1} for the morphism
$f' : X \to \Spec(k')$. In the proof of Lemma \ref{curves-lemma-duality-dim-1}
...
\begin{lemma}[Closed immersion dim 1 CM]
\label{curves-lemma-closed-immersion-dim-1-CM}
\source{stacks:0E33}
\href{https://stacks.math.columbia.edu/tag/0E33}{\texttt{Stacks tag 0E33}}
\group{curves}
...
be as in Lemma \ref{curves-lemma-duality-dim-1}. Then
\begin{enumerate}
...
In the proof of Lemma \ref{curves-lemma-duality-dim-1}
we set
...
\begin{lemma}[Closed subscheme reduced gorenstein]
\label{curves-lemma-closed-subscheme-reduced-gorenstein}
\source{stacks:0E34}
\href{https://stacks.math.columbia.edu/tag/0E34}{\texttt{Stacks tag 0E34}}
\group{curves}
...
\begin{proof}
\uses{curves-lemma-closed-immersion-dim-1-CM}
...
We have
$i_*\omega_Y = \SheafHom_{\mathcal{O}_X}(i_*\mathcal{O}_Y, \omega_X)$
by Lemma \ref{curves-lemma-closed-immersion-dim-1-CM}.
...
\medskip\noindent
We first use Lemmas \ref{curves-lemma-duality-dim-1} and \ref{curves-lemma-duality-dim-1-CM}
...
\source{stacks:0BS5}
\href{https://stacks.math.columbia.edu/tag/0BS5}{\texttt{Stacks tag 0BS5}}
\group{curves}

\uses{curves-lemma-duality-dim-1}
Let $X$ be a proper scheme of dimension $\leq 1$ over a field $k$.
With $\omega_X^\bullet$ and $\omega_X$ as in Lemma \ref{curves-lemma-duality-dim-1}
we have
$$
...
\begin{proof}
\uses{curves-lemma-duality-dim-1-CM}
...
Lemma \ref{curves-lemma-duality-dim-1-CM}.
\end{proof}

We will use Lemma \ref{curves-lemma-euler} to get the desired formula for
...
\begin{proof}
\uses{curves-lemma-duality-dim-1-CM, curves-lemma-euler, curves-remark-rework-duality-locally-free}
...
$$
Combined with Lemma \ref{curves-lemma-euler} this gives
$$
...
Remark \ref{curves-remark-rework-duality-locally-free}.
\end{proof}

\section{Some vanishing results}
\label{curves-section-vanishing}
This section contains some very weak vanishing results.
Please see Section \ref{curves-section-more-vanishing} for
a few more and more interesting results.

\begin{lemma}[Automatic]
\label{curves-lemma-automatic}
\source{stacks:0BY5}
\href{https://stacks.math.columbia.edu/tag/0BY5}{\texttt{Stacks tag 0BY5}}
\group{curves}
...
\begin{definition}[Situation: Cohen Macaulay curve]
\label{curves-situation-Cohen-Macaulay-curve}
\source{stacks:0B5D}
\href{https://stacks.math.columbia.edu/tag/0B5D}{\texttt{Stacks tag 0B5D}}
\group{curves}
...
By Lemma \ref{curves-lemma-automatic} the scheme $X$ is Cohen-Macaulay and
equidimensional of dimension $1$. The dualizing module $\omega_X$
discussed in Lemmas \ref{curves-lemma-duality-dim-1} and
\ref{curves-lemma-duality-dim-1-CM} has nonvanishing $H^1$ because in fact
...
\source{stacks:0B5E}
\href{https://stacks.math.columbia.edu/tag/0B5E}{\texttt{Stacks tag 0B5E}}
\group{curves}

\uses{curves-situation-Cohen-Macaulay-curve}
In Situation \ref{curves-situation-Cohen-Macaulay-curve}. Given an exact sequence
$$
...
(see Section \ref{curves-section-Riemann-Roch})
...
\source{stacks:0B62}
\href{https://stacks.math.columbia.edu/tag/0B62}{\texttt{Stacks tag 0B62}}
\group{curves}

\uses{curves-situation-Cohen-Macaulay-curve}
In Situation \ref{curves-situation-Cohen-Macaulay-curve}. Let
...
\begin{proof}
By duality as discussed in Section \ref{curves-section-Riemann-Roch} we have to
...
\begin{lemma}[Globally generated]
\label{curves-lemma-globally-generated}
\source{stacks:0B5F}
\href{https://stacks.math.columbia.edu/tag/0B5F}{\texttt{Stacks tag 0B5F}}
\group{curves}
...
Hence $H^1(X, \mathcal{F}') = 0$ by Lemma \ref{curves-lemma-vanishing}
...
\begin{lemma}[Globally generated curve]
\label{curves-lemma-globally-generated-curve}
\source{stacks:0B5G}
\href{https://stacks.math.columbia.edu/tag/0B5G}{\texttt{Stacks tag 0B5G}}
\group{curves}

\uses{curves-situation-Cohen-Macaulay-curve}
In Situation \ref{curves-situation-Cohen-Macaulay-curve} assume that
...
\begin{proof}
\uses{curves-lemma-globally-generated}

Consider the submodule $\mathcal{F}'$ generated by the global sections. By
Lemma \ref{curves-lemma-globally-generated} we see that $\mathcal{F}' \to \mathcal{Q}$
...
\begin{lemma}[Tensor omega with globally generated invertible]
\label{curves-lemma-tensor-omega-with-globally-generated-invertible}
\source{stacks:0B5H}
\href{https://stacks.math.columbia.edu/tag/0B5H}{\texttt{Stacks tag 0B5H}}
\group{curves}

\uses{curves-situation-Cohen-Macaulay-curve}
In Situation \ref{curves-situation-Cohen-Macaulay-curve}. Let $\mathcal{L}$
...
\begin{proof}
\uses{curves-lemma-globally-generated, curves-situation-Cohen-Macaulay-curve, curves-lemma-sanity-check-duality}
...
$$
By Lemma \ref{curves-lemma-globally-generated} we see that the module generated
...
of Situation \ref{curves-situation-Cohen-Macaulay-curve}. By flat base change
...
module of $X_{\overline{k}}$ by Lemma \ref{curves-lemma-sanity-check-duality}.
...
\begin{lemma}[Criterion very ample]
\label{curves-lemma-criterion-very-ample}
\source{stacks:0E8V}
\href{https://stacks.math.columbia.edu/tag/0E8V}{\texttt{Stacks tag 0E8V}}
\group{curves}
...
\begin{lemma}[Criterion very ample bis]
\label{curves-lemma-criterion-very-ample-bis}
\source{stacks:0E8W}
\href{https://stacks.math.columbia.edu/tag/0E8W}{\texttt{Stacks tag 0E8W}}
\group{curves}
...
\begin{proof}
\uses{curves-lemma-automatic, curves-lemma-criterion-very-ample}
...
Cohen-Macaulay by Lemma \ref{curves-lemma-automatic} and hence $\mathcal{L}$
has no embedded associated points).

\medskip\noindent
In particular, all of the statements given in the proof of
Lemma \ref{curves-lemma-criterion-very-ample} hold with this $s$.
...
of points of $Z$). Then in the proof of Lemma \ref{curves-lemma-criterion-very-ample}
...
proof of Lemma \ref{curves-lemma-criterion-very-ample} we find that
...
\source{stacks:0BY7}
\href{https://stacks.math.columbia.edu/tag/0BY7}{\texttt{Stacks tag 0BY7}}
\group{curves}
...
(Lemma \ref{curves-lemma-automatic}). If $\omega_X$ denotes the dualizing
module as in Lemmas \ref{curves-lemma-duality-dim-1} and
\ref{curves-lemma-duality-dim-1-CM} we see that
\begin{equation}
\label{equation-genus}
g = \dim_k H^1(X, \mathcal{O}_X) = \dim_k H^0(X, \omega_X)
\end{equation}
by duality, see Remark \ref{curves-remark-rework-duality-locally-free}.
...
\begin{lemma}[Genus base change]
\label{curves-lemma-genus-base-change}
\source{stacks:0BY9}
\href{https://stacks.math.columbia.edu/tag/0BY9}{\texttt{Stacks tag 0BY9}}
\group{curves}
...
\begin{lemma}[Genus gorenstein]
\label{curves-lemma-genus-gorenstein}
\source{stacks:0C19}
\href{https://stacks.math.columbia.edu/tag/0C19}{\texttt{Stacks tag 0C19}}
\group{curves}
...
\begin{proof}
\uses{curves-lemma-rr}

Immediate from Lemma \ref{curves-lemma-rr}.
\end{proof}

\begin{lemma}[Genus smooth]
\label{curves-lemma-genus-smooth}
\source{stacks:0C1A}
\href{https://stacks.math.columbia.edu/tag/0C1A}{\texttt{Stacks tag 0C1A}}
\group{curves}
...
\begin{proof}
\uses{curves-lemma-duality-dim-1, curves-lemma-genus-gorenstein}

By Lemma \ref{curves-lemma-duality-dim-1} we have $\Omega_{X/k} = \omega_X$.
Hence the formulas hold by (\ref{equation-genus}) and
Lemma \ref{curves-lemma-genus-gorenstein}.
\end{proof}
...
$F \in k[T_0, T_1, T_2]$, see Lemma \ref{curves-lemma-equation-plane-curve}.
Since $X$ is irreducible, it follows that $F$ is irreducible, see
Lemma \ref{curves-lemma-plane-curve}. Moreover, looking at the short exact
sequence
$$
...
has genus $(d - 1)(d - 2)/2$, see proof of Lemma \ref{curves-lemma-genus-plane-curve}.
...
\begin{lemma}[Equation plane curve]
\label{curves-lemma-equation-plane-curve}
\source{stacks:0BYB}
\href{https://stacks.math.columbia.edu/tag/0BYB}{\texttt{Stacks tag 0BYB}}
\group{curves}
...
\begin{lemma}[Plane curve]
\label{curves-lemma-plane-curve}
\source{stacks:0BYC}
\href{https://stacks.math.columbia.edu/tag/0BYC}{\texttt{Stacks tag 0BYC}}
\group{curves}

\uses{curves-lemma-equation-plane-curve}
Let $Z \subset \mathbf{P}^2_k$ be as in Lemma \ref{curves-lemma-equation-plane-curve}
...
\begin{lemma}[Genus plane curve]
\label{curves-lemma-genus-plane-curve}
\source{stacks:0BYD}
\href{https://stacks.math.columbia.edu/tag/0BYD}{\texttt{Stacks tag 0BYD}}
\group{curves}
...
\begin{lemma}[Smooth plane curve point over separable]
\label{curves-lemma-smooth-plane-curve-point-over-separable}
\source{stacks:0CCU}
\href{https://stacks.math.columbia.edu/tag/0CCU}{\texttt{Stacks tag 0CCU}}
\group{curves}
...
by Riemann-Roch (Lemma \ref{curves-lemma-rr}), hence $\mathcal{L}$ has a
nonzero section, hence $\mathcal{L} \cong \mathcal{O}_X$ by
Varieties, Lemma \ref{varieties-lemma-check-invertible-sheaf-trivial}.
\end{proof}

\begin{lemma}[Genus zero positive degree]
\label{curves-lemma-genus-zero-positive-degree}
\source{stacks:0C6T}
\href{https://stacks.math.columbia.edu/tag/0C6T}{\texttt{Stacks tag 0C6T}}
\group{curves}
...
\source{stacks:0C6N}
\href{https://stacks.math.columbia.edu/tag/0C6N}{\texttt{Stacks tag 0C6N}}
\group{curves}
...
\begin{proof}
\uses{curves-lemma-duality-dim-1, curves-lemma-genus-gorenstein, curves-lemma-genus-zero-positive-degree, curves-lemma-equation-plane-curve, curves-lemma-genus-plane-curve}

Consider the invertible sheaf $\mathcal{L} = \omega_X^{\otimes -1}$ where
$\omega_X$ is as in Lemma \ref{curves-lemma-duality-dim-1}. Then
$\deg(\omega_X) = -2$ by Lemma \ref{curves-lemma-genus-gorenstein}
and hence $\deg(\mathcal{L}) = 2$. By
Lemma \ref{curves-lemma-genus-zero-positive-degree}
...
be its equation (Lemma \ref{curves-lemma-equation-plane-curve}).
Because the genus of $X$ is $0$ we see that $d$ is $1$ or $2$
(Lemma \ref{curves-lemma-genus-plane-curve}). Observe that
...
\source{stacks:0C6U}
\href{https://stacks.math.columbia.edu/tag/0C6U}{\texttt{Stacks tag 0C6U}}
\group{curves}
...
\begin{proof}
\uses{curves-lemma-duality-dim-1, curves-lemma-genus-gorenstein, curves-lemma-genus-zero-positive-degree}
...
$\omega_X$ as in Lemma \ref{curves-lemma-duality-dim-1} has degree
$\deg(\omega_X) = -2$ by Lemma \ref{curves-lemma-genus-gorenstein}.
...
Thus $X \cong \mathbf{P}^1_k$ by Lemma \ref{curves-lemma-genus-zero-positive-degree}.
\end{proof}

\begin{lemma}[Genus zero singular]
\label{curves-lemma-genus-zero-singular}
\source{stacks:0DJB}
\href{https://stacks.math.columbia.edu/tag/0DJB}{\texttt{Stacks tag 0DJB}}
\group{curves}

Let $X$ be a proper curve over a field $k$ with $H^0(X, \mathcal{O}_X) = k$.
Assume $X$ is singular and has genus $0$. Then there exists a diagram
$$
\begin{array}{ccc}x' \downarrow\longrightarrow & X' \overset{\nu}{\downarrow}\longrightarrow & \Spec(k') \downarrow \\ x \longrightarrow & X \longrightarrow & \Spec(k)\end{array}
$$
where
\begin{enumerate}
...
\begin{proof}
\uses{curves-proposition-projective-line}
...
By Proposition \ref{curves-proposition-projective-line}
...
\begin{example}[Squish on P1]
\label{curves-example-squish-on-P1}
\source{stacks:0DJC}
\href{https://stacks.math.columbia.edu/tag/0DJC}{\texttt{Stacks tag 0DJC}}
\group{curves}

\uses{curves-lemma-genus-zero-singular}
In fact, the situation described in Lemma \ref{curves-lemma-genus-zero-singular}
...
$$
is called the {\it geometric genus} of $X$. By Lemma \ref{curves-lemma-genus-smooth}
...
and the arithmetic genus, see Remark \ref{curves-remark-genus-higher-dimension}.
...
\source{stacks:0BYF}
\href{https://stacks.math.columbia.edu/tag/0BYF}{\texttt{Stacks tag 0BYF}}
\group{curves}

\uses{curves-lemma-smooth-models}
...
Lemma \ref{curves-lemma-smooth-models}.
\end{definition}

If $k$ is perfect, then the nonsingular projective model $Y$ of $X$
is smooth (Lemma \ref{curves-lemma-nonsingular-model-smooth})
...
Lemmas \ref{curves-lemma-smooth-models} and \ref{curves-lemma-genus-base-change}.

\begin{remark}[Genus higher dimension]
\label{curves-remark-genus-higher-dimension}
\source{stacks:0BYG}
\href{https://stacks.math.columbia.edu/tag/0BYG}{\texttt{Stacks tag 0BYG}}
\group{curves}
...
\'etale, see Lemma \ref{curves-lemma-generically-etale}. Let $R \subset X$
...
\deg(f) (2g_Y - 2) + \deg(R)
\end{align*}
The first and last equality by Lemma \ref{curves-lemma-genus-smooth}.
...
(Lemma \ref{curves-lemma-uniformizer-works}).
...
to Lemma \ref{curves-lemma-rhe} and its proof.

\begin{lemma}[A morphism of smooth curves is separable iff it is etale almost everywhere]
\label{curves-lemma-generically-etale}
\source{stacks:0C1C}
\href{https://stacks.math.columbia.edu/tag/0C1C}{\texttt{Stacks tag 0C1C}}
\group{curves}
...
\source{stacks:0C1D}
\href{https://stacks.math.columbia.edu/tag/0C1D}{\texttt{Stacks tag 0C1D}}
\group{curves}

\uses{curves-lemma-generically-etale}
Let $f : X \to Y$ be a morphism of smooth proper curves
over a field $k$ which satisfies the equivalent conditions of
Lemma \ref{curves-lemma-generically-etale}. If
$k = H^0(Y, \mathcal{O}_Y) = H^0(X, \mathcal{O}_X)$
...
\source{stacks:0C1F}
\href{https://stacks.math.columbia.edu/tag/0C1F}{\texttt{Stacks tag 0C1F}}
\group{curves}

\uses{curves-lemma-rh}
Notation and assumptions as in Lemma \ref{curves-lemma-rh}. For a closed point
...
By Lemma \ref{curves-lemma-rh} and the discussion above
(which used
Varieties, Lemma \ref{varieties-lemma-algebraic-scheme-dim-0}
and
...
\begin{lemma}[Dominated by smooth]
\label{curves-lemma-dominated-by-smooth}
\source{stacks:0CCW}
\href{https://stacks.math.columbia.edu/tag/0CCW}{\texttt{Stacks tag 0CCW}}
\group{curves}
...
This follows from Lemma \ref{curves-lemma-flat}.
\end{proof}

\begin{lemma}[Purely inseparable]
\label{curves-lemma-purely-inseparable}
\source{stacks:0CCX}
\href{https://stacks.math.columbia.edu/tag/0CCX}{\texttt{Stacks tag 0CCX}}
\group{curves}
...
\begin{proof}
\uses{curves-theorem-curves-rational-maps}

This follows from Theorem \ref{curves-theorem-curves-rational-maps}
...
\source{stacks:0CCY}
\href{https://stacks.math.columbia.edu/tag/0CCY}{\texttt{Stacks tag 0CCY}}
\group{curves}
...
\begin{proof}
\uses{curves-theorem-curves-rational-maps, curves-lemma-generically-etale}
...
Theorem \ref{curves-theorem-curves-rational-maps}.

\medskip\noindent
Write $K = k(X)$. Consider the map $\text{d} : K \to \Omega_{K/k}$.
It follows from Lemma \ref{curves-lemma-generically-etale}
...
\begin{lemma}[Purely inseparable smooth]
\label{curves-lemma-purely-inseparable-smooth}
\source{stacks:0CCZ}
\href{https://stacks.math.columbia.edu/tag/0CCZ}{\texttt{Stacks tag 0CCZ}}
\group{curves}
...
\begin{proof}
\uses{curves-lemma-inseparable-deg-p-smooth, curves-lemma-purely-inseparable}

The $n$-fold relative Frobenius of $X/k$ is defined in
Varieties, Remark \ref{varieties-remark-n-fold-relative-frobenius}.
The lemma follows by combining Lemmas \ref{curves-lemma-inseparable-deg-p-smooth}
and \ref{curves-lemma-purely-inseparable}.
\end{proof}

\begin{lemma}[Purely inseparable smooth genus]
\label{curves-lemma-purely-inseparable-smooth-genus}
\source{stacks:0CD0}
\href{https://stacks.math.columbia.edu/tag/0CD0}{\texttt{Stacks tag 0CD0}}
\group{curves}
...
\begin{proof}
\uses{curves-lemma-purely-inseparable-smooth, curves-lemma-genus-base-change}

By Lemma \ref{curves-lemma-purely-inseparable-smooth}
...
Lemma \ref{curves-lemma-genus-base-change}.
\end{proof}

\begin{example}[Inseparable]
\label{curves-example-inseparable}
\source{stacks:0CD1}
\href{https://stacks.math.columbia.edu/tag/0CD1}{\texttt{Stacks tag 0CD1}}
\group{curves}

\uses{curves-lemma-genus-plane-curve}
...
$$
as in Section \ref{curves-section-plane-curves}.
...
curve of genus $1$ (Lemma \ref{curves-lemma-genus-plane-curve}).
...
\begin{proposition}[Unwind morphism smooth]
\label{curves-proposition-unwind-morphism-smooth}
\source{stacks:0CD2}
\href{https://stacks.math.columbia.edu/tag/0CD2}{\texttt{Stacks tag 0CD2}}
\group{curves}
...
\begin{proof}
\uses{curves-theorem-curves-rational-maps, curves-lemma-purely-inseparable-smooth}
...
By Theorem \ref{curves-theorem-curves-rational-maps}
this corresponds to a factorization
$X \to Z \to Y$ of $f$ with $Z$ a nonsingular proper curve.
Apply Lemma \ref{curves-lemma-purely-inseparable-smooth}
to the morphism $X \to Z$ to conclude.
\end{proof}

\begin{lemma}[Inseparable linear system]
\label{curves-lemma-inseparable-linear-system}
\source{stacks:0CD3}
\href{https://stacks.math.columbia.edu/tag/0CD3}{\texttt{Stacks tag 0CD3}}
\group{curves}

\uses{curves-lemma-linear-series, curves-lemma-generically-etale}
...
$X \to \mathbf{P}^1_k$ (Lemma \ref{curves-lemma-linear-series})
is generically \'etale (i.e., is as in Lemma \ref{curves-lemma-generically-etale}), or
...
\begin{proof}
\uses{curves-proposition-unwind-morphism-smooth, curves-lemma-extend-over-normal-curve}
...
Proposition \ref{curves-proposition-unwind-morphism-smooth}.
...
\begin{lemma}[Point over separable extension]
\label{curves-lemma-point-over-separable-extension}
\source{stacks:0CD4}
\href{https://stacks.math.columbia.edu/tag/0CD4}{\texttt{Stacks tag 0CD4}}
\group{curves}
...
\begin{proof}
\uses{curves-lemma-genus-smooth, curves-lemma-linear-series-trivial-existence, curves-lemma-g1d, curves-lemma-flat, curves-lemma-finite, curves-lemma-grd-inequalities, curves-lemma-inseparable-linear-system}

Set $\omega = \Omega_{X/k}$. By
Lemma \ref{curves-lemma-genus-smooth} this has degree $2g - 2$
and has $g$ global sections. Thus we have a $\mathfrak g^{g - 1}_{2g - 2}$.
By the trivial Lemma \ref{curves-lemma-linear-series-trivial-existence}
there exists a $\mathfrak g^1_{2g - 2}$
and by Lemma \ref{curves-lemma-g1d} we obtain a morphism
$$
\varphi : X \longrightarrow \mathbf{P}^1_k
$$
of some degree $d \leq 2g - 2$. Since $\varphi$ is flat
(Lemma \ref{curves-lemma-flat}) and finite
(Lemma \ref{curves-lemma-finite})
...
Lemma \ref{curves-lemma-grd-inequalities}.
Applying Lemma \ref{curves-lemma-inseparable-linear-system}
...
\begin{lemma}[Ramification to algebraic closure]
\label{curves-lemma-ramification-to-algebraic-closure}
\source{stacks:0C1G}
\href{https://stacks.math.columbia.edu/tag/0C1G}{\texttt{Stacks tag 0C1G}}
\group{curves}
...
$$
\begin{array}{cc}\mathcal{O}_{X_{\overline{k}}, \overline{x}} & \mathcal{O}_{\mathbf{A}^1_{\overline{k}}, \pi(\overline{x})} \longleftarrow \\ \mathcal{O}_{X, x} \uparrow & \mathcal{O}_{\mathbf{A}^1_k, \pi(x)} \longleftarrow\uparrow\end{array}
$$
...
$$
\begin{array}{cc}Z' \downarrow\underset{i'}{\longrightarrow} & X' \overset{a}{\downarrow} \\ Z \overset{i}{\dashrightarrow} & X\end{array}
$$
...
\begin{lemma}[Complete local ring pushout]
\label{curves-lemma-complete-local-ring-pushout}
\source{stacks:0E36}
\href{https://stacks.math.columbia.edu/tag/0E36}{\texttt{Stacks tag 0E36}}
\group{curves}
...
$$
\begin{array}{cc}\prod\nolimits_{i = 1, \ldots, n} k'_i & \prod\nolimits_{i = 1, \ldots, n} \mathcal{O}_{X', x'_i}^\wedge \longleftarrow \\ k' \uparrow & \mathcal{O}_{X, x}^\wedge \uparrow\longleftarrow\end{array}
$$
...
$$
\begin{array}{cc}\prod\nolimits_{i = 1, \ldots, n} k'_i & A' \longleftarrow \\ k' \uparrow & A \uparrow\longleftarrow\end{array}
$$
...
\begin{lemma}[No in between over k]
\label{curves-lemma-no-in-between-over-k}
\source{stacks:0C1I}
\href{https://stacks.math.columbia.edu/tag/0C1I}{\texttt{Stacks tag 0C1I}}
\group{curves}
...
\source{stacks:0C1J}
\href{https://stacks.math.columbia.edu/tag/0C1J}{\texttt{Stacks tag 0C1J}}
\group{curves}
...
\source{stacks:0C1K}
\href{https://stacks.math.columbia.edu/tag/0C1K}{\texttt{Stacks tag 0C1K}}
\group{curves}
...
\begin{lemma}[Factor almost isomorphism]
\label{curves-lemma-factor-almost-isomorphism}
\source{stacks:0C1L}
\href{https://stacks.math.columbia.edu/tag/0C1L}{\texttt{Stacks tag 0C1L}}
\group{curves}

\uses{curves-example-glue-points, curves-example-squish-tangent-vector}
...
(see Examples \ref{curves-example-glue-points} and
\ref{curves-example-squish-tangent-vector}).
\end{lemma}

\begin{proof}
\uses{curves-lemma-no-in-between-over-k}
...
or $C = k[\epsilon]$ by Lemma \ref{curves-lemma-no-in-between-over-k}.
In this case $f$ is correspondingly the glueing two points or the
squishing of a tangent vector.
\end{proof}

\begin{lemma}[Glue points]
\label{curves-lemma-glue-points}
\source{stacks:0C1M}
\href{https://stacks.math.columbia.edu/tag/0C1M}{\texttt{Stacks tag 0C1M}}
\group{curves}

\uses{curves-example-glue-points}
Let $k$ be an algebraically closed field. If $f : X' \to X$ is the
glueing of two points $a, b$ as in Example \ref{curves-example-glue-points}, then
...
\begin{lemma}[Squish tangent vector]
\label{curves-lemma-squish-tangent-vector}
\source{stacks:0C1N}
\href{https://stacks.math.columbia.edu/tag/0C1N}{\texttt{Stacks tag 0C1N}}
\group{curves}

\uses{curves-example-squish-tangent-vector}
Let $k$ be an algebraically closed field. If $f : X' \to X$ is the
squishing of a tangent vector $\vartheta$ as in
Example \ref{curves-example-squish-tangent-vector}, then
there is an exact sequence
$$
(k, +) \to \Pic(X) \to \Pic(X') \to 0
$$
...
\xrightarrow{\vartheta} x_*k \to 0
$$
of Example \ref{curves-example-squish-tangent-vector} we obtain
$$
...
\begin{lemma}[Multicross algebra]
\label{curves-lemma-multicross-algebra}
\source{stacks:0C1V}
\href{https://stacks.math.columbia.edu/tag/0C1V}{\texttt{Stacks tag 0C1V}}
\group{curves}
...
\begin{definition}[Multicross]
\label{curves-definition-multicross}
\source{stacks:0C1W}
\href{https://stacks.math.columbia.edu/tag/0C1W}{\texttt{Stacks tag 0C1W}}
\group{curves}
...
\begin{lemma}[Multicross]
\label{curves-lemma-multicross}
\source{stacks:0C1X}
\href{https://stacks.math.columbia.edu/tag/0C1X}{\texttt{Stacks tag 0C1X}}
\group{curves}
...
is the glueing of two points as in Example \ref{curves-example-glue-points}.
\end{enumerate}
\end{lemma}

\begin{proof}
\uses{curves-lemma-multicross-algebra, curves-lemma-factor-almost-isomorphism}

The equivalence of (1) and (2) is Lemma \ref{curves-lemma-multicross-algebra}.
...
Then we apply Lemma \ref{curves-lemma-factor-almost-isomorphism} to
...
By Lemma \ref{curves-lemma-multicross-algebra}
this implies that $u$ cannot be a multicross singularity which
is a contradiction.
\end{proof}

\begin{lemma}[Multicross gorenstein is nodal]
\label{curves-lemma-multicross-gorenstein-is-nodal}
\source{stacks:0CDZ}
\href{https://stacks.math.columbia.edu/tag/0CDZ}{\texttt{Stacks tag 0CDZ}}
\group{curves}

\uses{curves-definition-multicross}
Let $k$ be an algebraically closed field. Let $X$ be a reduced algebraic
$1$-dimensional $k$-scheme. Let $x \in X$ be a multicross singularity
(Definition \ref{curves-definition-multicross}).
...
Lemma \ref{curves-lemma-torsion-picard-smooth-projective}.
...
\begin{lemma}[Torsion picard smooth projective]
\label{curves-lemma-torsion-picard-smooth-projective}
\source{stacks:0C1Z}
\href{https://stacks.math.columbia.edu/tag/0C1Z}{\texttt{Stacks tag 0C1Z}}
\group{curves}
...
\begin{lemma}[Torsion picard becomes visible]
\label{curves-lemma-torsion-picard-becomes-visible}
\source{stacks:0CDU}
\href{https://stacks.math.columbia.edu/tag/0CDU}{\texttt{Stacks tag 0CDU}}
\group{curves}
...
\begin{proof}
\uses{curves-lemma-point-over-separable-extension, curves-lemma-torsion-picard-smooth-projective}
...
Lemma \ref{curves-lemma-point-over-separable-extension}.
...
By Lemma \ref{curves-lemma-torsion-picard-smooth-projective} we have
$$
...
\begin{proposition}[Torsion picard reduced proper]
\label{curves-proposition-torsion-picard-reduced-proper}
\source{stacks:0C20}
\href{https://stacks.math.columbia.edu/tag/0C20}{\texttt{Stacks tag 0C20}}
\group{curves}
...
\begin{proof}
\uses{curves-lemma-factor-almost-isomorphism, curves-lemma-glue-points, curves-lemma-squish-tangent-vector, curves-example-glue-points, curves-example-squish-tangent-vector, curves-lemma-torsion-picard-smooth-projective, curves-lemma-multicross}
...
$$
as in Lemma \ref{curves-lemma-factor-almost-isomorphism}.
Let us denote $h^0_i = \dim_k H^0(X_i, \mathcal{O}_{X_i})$
and $h^1_i = \dim_k H^1(X_i, \mathcal{O}_{X_i})$.
By Lemmas \ref{curves-lemma-glue-points} and \ref{curves-lemma-squish-tangent-vector}
...
Examples \ref{curves-example-glue-points} and \ref{curves-example-squish-tangent-vector}.
...
\end{enumerate}
The last equality by Lemma \ref{curves-lemma-torsion-picard-smooth-projective}.
...
by Lemma \ref{curves-lemma-multicross}. Conversely, if all the singularities
are multicross, then Lemma \ref{curves-lemma-multicross} guarantees that
...
\begin{lemma}[Bound geometric genus]
\label{curves-lemma-bound-geometric-genus}
\source{stacks:0CE1}
\href{https://stacks.math.columbia.edu/tag/0CE1}{\texttt{Stacks tag 0CE1}}
\group{curves}
...
\begin{lemma}[Geometric genus normalization]
\label{curves-lemma-geometric-genus-normalization}
\source{stacks:0CE2}
\href{https://stacks.math.columbia.edu/tag/0CE2}{\texttt{Stacks tag 0CE2}}
\group{curves}
...
\begin{proof}
\uses{curves-lemma-bound-geometric-genus}

Part (1) is immediate from Lemma \ref{curves-lemma-bound-geometric-genus}.
...
\begin{lemma}[Genus goes down]
\label{curves-lemma-genus-goes-down}
\source{stacks:0CE3}
\href{https://stacks.math.columbia.edu/tag/0CE3}{\texttt{Stacks tag 0CE3}}
\group{curves}
...
\begin{lemma}[Genus normalization]
\label{curves-lemma-genus-normalization}
\source{stacks:0CE4}
\href{https://stacks.math.columbia.edu/tag/0CE4}{\texttt{Stacks tag 0CE4}}
\group{curves}
...
\begin{proof}
\uses{curves-lemma-genus-goes-down}

If $f : X' \to X$ is proper birational, then it is finite and
an isomorphism over a dense open (see
Varieties, Lemmas \ref{varieties-lemma-finite-in-codim-1} and
\ref{varieties-lemma-modification-normal-iso-over-codimension-1}).
Thus the inequality by Lemma \ref{curves-lemma-genus-goes-down}.
...
\begin{lemma}[Bound geometric genus curve]
\label{curves-lemma-bound-geometric-genus-curve}
\source{stacks:0CE5}
\href{https://stacks.math.columbia.edu/tag/0CE5}{\texttt{Stacks tag 0CE5}}
\group{curves}
...
By Lemma \ref{curves-lemma-genus-goes-down} we have
$$
...
\begin{lemma}[Bound torsion simple]
\label{curves-lemma-bound-torsion-simple}
\source{stacks:0CE6}
\href{https://stacks.math.columbia.edu/tag/0CE6}{\texttt{Stacks tag 0CE6}}
\group{curves}
...
\begin{proof}
\uses{curves-proposition-torsion-picard-reduced-proper}
...
Proposition \ref{curves-proposition-torsion-picard-reduced-proper}.
\end{proof}

\section{Nodal curves}
\label{curves-section-nodal}
We have already defined ordinary double points over algebraically
closed fields, see Definition \ref{curves-definition-multicross}. Namely,
...
Section \ref{curves-section-multicross}.

\begin{definition}[Nodal]
\label{curves-definition-nodal}
\source{stacks:0C47}
\href{https://stacks.math.columbia.edu/tag/0C47}{\texttt{Stacks tag 0C47}}
\group{curves}
...
\begin{lemma}[Reduced quotient regular ring dim 2]
\label{curves-lemma-reduced-quotient-regular-ring-dim-2}
\source{stacks:0C48}
\href{https://stacks.math.columbia.edu/tag/0C48}{\texttt{Stacks tag 0C48}}
\group{curves}
...
\begin{lemma}[Nodal algebraic]
\label{curves-lemma-nodal-algebraic}
\source{stacks:0C49}
\href{https://stacks.math.columbia.edu/tag/0C49}{\texttt{Stacks tag 0C49}}
\group{curves}
...
\begin{proof}
\uses{curves-lemma-reduced-quotient-regular-ring-dim-2}
...
By Lemma \ref{curves-lemma-reduced-quotient-regular-ring-dim-2}
...
\begin{lemma}[2 branches delta 1]
\label{curves-lemma-2-branches-delta-1}
\source{stacks:0C4A}
\href{https://stacks.math.columbia.edu/tag/0C4A}{\texttt{Stacks tag 0C4A}}
\group{curves}

\uses{curves-lemma-nodal-algebraic}
Let $k$ be a field. Let $(A, \mathfrak m, \kappa)$ be a
Nagata local $k$-algebra. The following are equivalent
\begin{enumerate}
\item $k \to A$ is as in Lemma \ref{curves-lemma-nodal-algebraic},
...
\begin{lemma}[Fitting ideal well defined]
\label{curves-lemma-fitting-ideal-well-defined}
\source{stacks:0C4B}
\href{https://stacks.math.columbia.edu/tag/0C4B}{\texttt{Stacks tag 0C4B}}
\group{curves}
...
\begin{lemma}[Fitting ideal]
\label{curves-lemma-fitting-ideal}
\source{stacks:0C4C}
\href{https://stacks.math.columbia.edu/tag/0C4C}{\texttt{Stacks tag 0C4C}}
\group{curves}
...
are equivalent
\begin{enumerate}
\item $k \to A/I$ is as in Lemma \ref{curves-lemma-nodal-algebraic},
...
\begin{proof}
\uses{curves-lemma-fitting-ideal-well-defined, curves-lemma-reduced-quotient-regular-ring-dim-2, curves-lemma-nodal-algebraic}

By Lemma \ref{curves-lemma-fitting-ideal-well-defined}
...
Lemma \ref{curves-lemma-reduced-quotient-regular-ring-dim-2}
...
Lemma \ref{curves-lemma-nodal-algebraic}.
\end{proof}

\begin{lemma}[Nodal]
\label{curves-lemma-nodal}
\source{stacks:0C4D}
\href{https://stacks.math.columbia.edu/tag/0C4D}{\texttt{Stacks tag 0C4D}}
\group{curves}
...
\begin{enumerate}
\item $x$ is a node,
\item $k \to \mathcal{O}_{X, x}$ is as in Lemma \ref{curves-lemma-nodal-algebraic},
...
\begin{proof}
\uses{curves-lemma-multicross-algebra, curves-lemma-fitting-ideal, curves-lemma-2-branches-delta-1}
...
Lemma \ref{curves-lemma-multicross-algebra}.
...
are equivalent by Lemma \ref{curves-lemma-fitting-ideal} applied
...
Lemma \ref{curves-lemma-2-branches-delta-1}.
...
hold for $x \in X$ by
Lemma \ref{curves-lemma-2-branches-delta-1}. Thus
Varieties, Lemma
...
\source{stacks:0CBT}
\href{https://stacks.math.columbia.edu/tag/0CBT}{\texttt{Stacks tag 0CBT}}
\group{curves}

\uses{curves-lemma-nodal-algebraic, curves-lemma-2-branches-delta-1, curves-lemma-fitting-ideal}
Let $k$ be a field. Let $(A, \mathfrak m, \kappa)$ be a
Noetherian local $k$-algebra. Assume that either
$(A, \mathfrak m, \kappa)$ is as in Lemma \ref{curves-lemma-nodal-algebraic}, or
$A$ is Nagata as in Lemma \ref{curves-lemma-2-branches-delta-1}, or
$A$ is complete and as in Lemma \ref{curves-lemma-fitting-ideal}.
Then $A$ defines canonically a degree $2$ separable $\kappa$-algebra
$\kappa'$ as follows
\begin{enumerate}
\item let $q = ax^2 + bxy + cy^2$ be a nondegenerate quadric
as in Lemma \ref{curves-lemma-nodal-algebraic} with coordinates $x, y$ chosen
such that $a \not = 0$ and set
...
Lemma \ref{curves-lemma-2-branches-delta-1}. We omit the equivalence of
...
\source{stacks:0CBU}
\href{https://stacks.math.columbia.edu/tag/0CBU}{\texttt{Stacks tag 0CBU}}
\group{curves}

\uses{curves-remark-quadratic-extension, curves-lemma-nodal-algebraic, curves-lemma-2-branches-delta-1}
Let $k$ be a field. Let $(A, \mathfrak m, \kappa)$ be as in
Remark \ref{curves-remark-quadratic-extension} and let $\kappa'/\kappa$
be the associated separable algebra of degree $2$.
Then the following are equivalent
\begin{enumerate}
\item $\kappa' \cong \kappa \times \kappa$ as $\kappa$-algebra,
\item the form $q$ of Lemma \ref{curves-lemma-nodal-algebraic}
can be chosen to be $xy$,
\item $A$ has two branches,
\item the extension $A'/A$ of Lemma \ref{curves-lemma-2-branches-delta-1}
...
proof of Lemma \ref{curves-lemma-2-branches-delta-1}. If $X$ is a
...
\begin{definition}[Split node]
\label{curves-definition-split-node}
\source{stacks:0CBV}
\href{https://stacks.math.columbia.edu/tag/0CBV}{\texttt{Stacks tag 0CBV}}
\group{curves}

\uses{curves-remark-trivial-quadratic-extension}
Let $k$ be a field. Let $X$ be a $1$-dimensional algebraic $k$-scheme.
Let $x \in X$ be a closed point. We say $x$ is a {\it split node}
if $x$ is a node, $\kappa(x) = k$, and the equivalent assertions of
Remark \ref{curves-remark-trivial-quadratic-extension}
...
\begin{lemma}[Split node]
\label{curves-lemma-split-node}
\source{stacks:0CBW}
\href{https://stacks.math.columbia.edu/tag/0CBW}{\texttt{Stacks tag 0CBW}}
\group{curves}
...
\begin{proof}
\uses{curves-remark-trivial-quadratic-extension, curves-lemma-nodal}

This follows from the discussion in
Remark \ref{curves-remark-trivial-quadratic-extension}
and Lemma \ref{curves-lemma-nodal}.
\end{proof}

\begin{lemma}[Node field extension]
\label{curves-lemma-node-field-extension}
\source{stacks:0C56}
\href{https://stacks.math.columbia.edu/tag/0C56}{\texttt{Stacks tag 0C56}}
\group{curves}
...
\begin{proof}
\uses{curves-lemma-nodal, curves-lemma-nodal-algebraic, curves-lemma-fitting-ideal}
...
which means that $y$ cannot be a node by Lemma \ref{curves-lemma-nodal}.
...
asking these rings to be as in Lemma \ref{curves-lemma-nodal-algebraic}.
Via Lemma \ref{curves-lemma-fitting-ideal} this translates into a statement
...
\begin{lemma}[Node etale local]
\label{curves-lemma-node-etale-local}
\source{stacks:0C57}
\href{https://stacks.math.columbia.edu/tag/0C57}{\texttt{Stacks tag 0C57}}
\group{curves}
...
\begin{proof}
\uses{curves-lemma-node-field-extension}

By Lemma \ref{curves-lemma-node-field-extension}
...
\begin{lemma}[Node over separable extension]
\label{curves-lemma-node-over-separable-extension}
\source{stacks:0CD6}
\href{https://stacks.math.columbia.edu/tag/0CD6}{\texttt{Stacks tag 0CD6}}
\group{curves}
...
\begin{proof}
\uses{curves-lemma-nodal}

Follows immediately from the characterization of nodes in
Lemma \ref{curves-lemma-nodal}.
\end{proof}

\begin{lemma}[Nodal lci]
\label{curves-lemma-nodal-lci}
\source{stacks:0C4E}
\href{https://stacks.math.columbia.edu/tag/0C4E}{\texttt{Stacks tag 0C4E}}
\group{curves}
...
\begin{proof}
\uses{curves-lemma-nodal, curves-lemma-nodal-algebraic}
...
by Lemma \ref{curves-lemma-nodal}. If $x \in X$, $x \not \in Z$, then
...
By Lemma \ref{curves-lemma-nodal} for every point $x \in Z$
...
Finally, Lemma \ref{curves-lemma-nodal} via part (3) of
Lemma \ref{curves-lemma-nodal-algebraic}, shows that $\mathcal{O}_{X, x}$
...
\begin{lemma}[Facts about nodal curves]
\label{curves-lemma-facts-about-nodal-curves}
\source{stacks:0E37}
\href{https://stacks.math.columbia.edu/tag/0E37}{\texttt{Stacks tag 0E37}}
\group{curves}
...
\begin{proof}
\uses{curves-lemma-nodal-lci}

The Gorenstein assertion follows from Lemma \ref{curves-lemma-nodal-lci}
...
\begin{lemma}[Closed subscheme nodal curve]
\label{curves-lemma-closed-subscheme-nodal-curve}
\source{stacks:0E38}
\href{https://stacks.math.columbia.edu/tag/0E38}{\texttt{Stacks tag 0E38}}
\group{curves}
...
\begin{proof}
\uses{curves-lemma-nodal, curves-remark-trivial-quadratic-extension}
...
by Lemma \ref{curves-lemma-nodal},
...
as in Remark \ref{curves-remark-trivial-quadratic-extension}.
The field $\kappa(y)$ is finite separable over $k$ by
Lemma \ref{curves-lemma-nodal} for example.
...
\begin{lemma}[Nodal family]
\label{curves-lemma-nodal-family}
\source{stacks:0C59}
\href{https://stacks.math.columbia.edu/tag/0C59}{\texttt{Stacks tag 0C59}}
\group{curves}
...
Thus the lemma follows from Lemma \ref{curves-lemma-nodal-lci},
...
\source{stacks:0C5A}
\href{https://stacks.math.columbia.edu/tag/0C5A}{\texttt{Stacks tag 0C5A}}
\group{curves}

\uses{curves-lemma-nodal-family}
Let $f : X \to S$ be a morphism of schemes. We say $f$ is
{\it at-worst-nodal of relative dimension $1$} if $f$ satisfies
the equivalent conditions of Lemma \ref{curves-lemma-nodal-family}.
\end{definition}
...
\begin{lemma}[Smooth relative dimension 1]
\label{curves-lemma-smooth-relative-dimension-1}
\source{stacks:0CD7}
\href{https://stacks.math.columbia.edu/tag/0CD7}{\texttt{Stacks tag 0CD7}}
\group{curves}
...
\begin{lemma}[Base change nodal family]
\label{curves-lemma-base-change-nodal-family}
\source{stacks:0C5B}
\href{https://stacks.math.columbia.edu/tag/0C5B}{\texttt{Stacks tag 0C5B}}
\group{curves}
...
\source{stacks:0DSC}
\href{https://stacks.math.columbia.edu/tag/0DSC}{\texttt{Stacks tag 0DSC}}
\group{curves}
...
\begin{lemma}[Nodal family precompose etale]
\label{curves-lemma-nodal-family-precompose-etale}
\source{stacks:0C5C}
\href{https://stacks.math.columbia.edu/tag/0C5C}{\texttt{Stacks tag 0C5C}}
\group{curves}
...
\begin{proof}
\uses{curves-lemma-node-etale-local}
...
This follows from Lemma \ref{curves-lemma-node-etale-local}
...
\begin{lemma}[Nodal family postcompose etale]
\label{curves-lemma-nodal-family-postcompose-etale}
\source{stacks:0CD8}
\href{https://stacks.math.columbia.edu/tag/0CD8}{\texttt{Stacks tag 0CD8}}
\group{curves}
...
\begin{proof}
\uses{curves-lemma-node-over-separable-extension}
...
This follows from Lemma \ref{curves-lemma-node-over-separable-extension}
and the analogous result for smooth points.
\end{proof}

\begin{lemma}[Nodal family etale local source]
\label{curves-lemma-nodal-family-etale-local-source}
\source{stacks:0C5D}
\href{https://stacks.math.columbia.edu/tag/0C5D}{\texttt{Stacks tag 0C5D}}
\group{curves}
...
\begin{proof}
\uses{curves-lemma-nodal-family-precompose-etale, curves-lemma-node-etale-local}

One direction we have seen in Lemma \ref{curves-lemma-nodal-family-precompose-etale}.
...
\begin{proof}
\uses{curves-lemma-base-change-nodal-family, curves-lemma-node-field-extension}

One direction we have seen in Lemma \ref{curves-lemma-base-change-nodal-family}.
For the other direction, observe that being locally of finite
presentation, flat, or to have relative dimension $1$
is fpqc local on the target
(Descent, Lemmas
...
to the case where $S$ is the spectrum of a field. In this case the
result follows from Lemma \ref{curves-lemma-node-field-extension}
(and the fact that being smooth is fpqc local on the target by
Descent, Lemma \ref{descent-lemma-descending-property-smooth}).
\end{proof}

\begin{lemma}[Descend nodal family]
\label{curves-lemma-descend-nodal-family}
\source{stacks:0C5F}
\href{https://stacks.math.columbia.edu/tag/0C5F}{\texttt{Stacks tag 0C5F}}
\group{curves}
...
\begin{lemma}[Formal local structure nodal family]
\label{curves-lemma-formal-local-structure-nodal-family}
\source{stacks:0CBX}
\href{https://stacks.math.columbia.edu/tag/0CBX}{\texttt{Stacks tag 0CBX}}
\group{curves}
...
By assumption (4) and Lemma \ref{curves-lemma-split-node}
...
\begin{lemma}[Genus in nodal family of curves]
\label{curves-lemma-genus-in-nodal-family-of-curves}
\source{stacks:0GKA}
\href{https://stacks.math.columbia.edu/tag/0GKA}{\texttt{Stacks tag 0GKA}}
\group{curves}
...
\section{\'Etale local structure of nodal families}
...
\begin{lemma}[Etale local structure nodal family]
\label{curves-lemma-etale-local-structure-nodal-family}
\source{stacks:0CBY}
\href{https://stacks.math.columbia.edu/tag/0CBY}{\texttt{Stacks tag 0CBY}}
\group{curves}
...
$$
\begin{array}{ccccc}X \downarrow & U \longrightarrow\longleftarrow\searrow &  & W \longrightarrow\swarrow & \Spec(\mathbf{Z}[u, v, a]/(uv - a)) \downarrow \\ S &  & V \longleftarrow\longrightarrow &  & \Spec(\mathbf{Z}[a])\end{array}
$$
...
\begin{proof}
\uses{curves-lemma-descend-nodal-family, curves-lemma-nodal, curves-remark-quadratic-extension, curves-definition-split-node, curves-lemma-formal-local-structure-nodal-family}
...
Lemma \ref{curves-lemma-descend-nodal-family}.
...
$x \in X_s$ is a split node, see Definition \ref{curves-definition-split-node}.
By Lemma \ref{curves-lemma-formal-local-structure-nodal-family} we see that there
...
\begin{proof}
\uses{curves-definition-split-node, curves-lemma-split-node}
[Proof not using Artin approximation]
We only sketch this proof; it was contributed by Mohan Swaminathan
and it mimicks an argument of \cite[Proposition X.2.1]{ACG}. Contrary
to the previous proof, this proof uses the actual equations.

\medskip\noindent
In exactly the same manner as in the first proof we reduce to the
case where $S$ is of finite type over $\mathbf{Z}$ and $x \in X_s$
is a split node, see Definition \ref{curves-definition-split-node}.
By Lemma \ref{curves-lemma-split-node} we see that there exists an
$\kappa(s)$-algebra isomorphism
$$
\mathcal{O}_{X_s, x}^\wedge \cong \kappa(s)[[u, v]]/(uv)
$$
Note that the tangent space of $X_s$ at $x$ has dimension $2$.
Thus by Varieties, Lemma \ref{varieties-lemma-immersion-into-affine}
after Zariski shrinking we may assume there exists a closed immersion
$X \to Y$ of schemes over $S$ with $Y \to S$ smooth of relative dimension $2$.
Since $X$ is syntomic of relative dimension $1$ (by definition), we see
that $X$ is an effective Cartier divisor in $Y$ (follows from
Divisors, Lemma
\ref{divisors-lemma-immersion-lci-into-smooth-regular-immersion}).
Thus after shrinking $Y$ and $X$ we may assume $Y = \Spec(B)$
is affine and that there exists a nonzerodivisor $g \in B$ such
that $X = \Spec(B/gB)$.

\medskip\noindent
Write $S = \Spec(R)$ and denote $\mathfrak p \subset R$ be the prime
corresponding to $s$. Let $\mathfrak q \subset B$ be the prime ideal
corresponding to $x$ (viewed as a point of $Y$). We have maps
$$
...
\kappa(s)[[u, v]]/(uv)
$$
where the second arrow becomes an isomorphism after completion.
After replacing $B$ by a suitable princpal localization,
we may assume that there exist $U, V \in B$ which map to
$u$ and $v$ up to $(u, v)^2$. Then the completion of
$B_\mathfrak q/\mathfrak p B_\mathfrak q$
is $\kappa(s)[[U, V]]$. Furthermore, after
shrinking $Y$ and after multiplying $g$ by a unit we may
assume that $g$ maps to $UV$ plus higher order terms in
$\kappa(s)[[U, V]]$. After shrinking $Y$ we may further assume
\begin{enumerate}
\item $\mathfrak q = \mathfrak pB + (U, V)$
\item $\Omega_{B/R}$ is free on $\text{d}U$ and $\text{d}V$
\end{enumerate}
Write $\text{d}(g) = g_2 \text{d}U + g_1 \text{d}V$ with $g_1, g_2 \in B$.
Note that the ideal $J = (g_1, g_2) \subset B$ is independent of the
choice of the ``coordinates'' $U$ and $V$ (for fixed $g$).
Looking modulo $\mathfrak p B_\mathfrak q + (U, V)^3B_\mathfrak q$
we see that $g_1$ and $g_2$ are congruent to $U$ and $V$ modulo
$\mathfrak p B_\mathfrak q + (U, V)^2B_\mathfrak q$.
Thus we may redo the argument above with $U$ replaced by $g_1$
and $V$ replaced by $g_2$. Then we find that $J = (U, V)$
which implies that $g_1, g_2 \in (U, V) = J$ (of course these
are different from the $g_1$ and $g_2$ with our previous choice
of coordinates).

\medskip\noindent
Note that $\Spec(B/J) \to S = \Spec(R)$ is \'etale at $x$.
Thus after replacing $S$ by an \'etale neighbourhood and shrinking
$Y$ again, we may assume that $R \to B \to B/J$ is an isomorphism.
Denote $r \in R$ the element whose image in $B/J$ is the image of $-g$.
Then we can write
$$
g + r \equiv a U + b V \bmod (U, V)^2
$$
for some $a, b \in R$. Taking derivatives using that
$\text{d}g \in J\Omega_{B/R}$ we find $a = b = 0$.
Thus we can write
$$
g + r = \alpha U^2 + \beta UV + \gamma V^2
$$
for some $\alpha, \beta, \gamma \in B$. Then
$\beta$ maps to $1$ in $\kappa(x)$ and $\alpha$ and $\gamma$
map to $0$ in $\kappa(x)$.
Thus we may assume $\beta$ and $\alpha + \beta + \gamma$
are invertible in $B$. Consider the cover $Y'$ of $Y$ given by
$$
Y' = \Spec(B[t]/(t(1 - t) - \alpha \gamma / \beta^2))
$$
with point $x'$ the unique point over $x$ with $t = 1$.
Note that $Y' \to Y$ is \'etale at $x'$.
...
g = T_1 T_2 - r
$$
Since $T_1$ and $T_2$ are also coordinates in the sense above,
we see that the morphism $Y' \to \mathbf{A}^2_S$ given by $T_1$ and $T_2$
is \'etale in a neighbourhood of $x'$ and that the pullback
$X' \subset Y'$ of $X$ is the inverse image of the closed subscheme
of $\mathbf{A}^2_S = \Spec(R[x_1, x_2])$ given by the equation
$x_1x_2 - r = 0$ (again after possibly shrinking $Y'$).
We leave it to the reader to put everything together to
obtain the desired diagram.
\end{proof}

\section{More vanishing results}
\label{curves-section-more-vanishing}
Continuation of Section \ref{curves-section-vanishing}.

\begin{lemma}[H1 nonzero degree leq 2g 2]
\label{curves-lemma-h1-nonzero-degree-leq-2g-2}
\source{stacks:0E3A}
\href{https://stacks.math.columbia.edu/tag/0E3A}{\texttt{Stacks tag 0E3A}}
\group{curves}

\uses{curves-situation-Cohen-Macaulay-curve}
In Situation \ref{curves-situation-Cohen-Macaulay-curve} assume $X$ is integral and
...
\begin{proof}
\uses{curves-lemma-duality-dim-1-CM, curves-lemma-euler, curves-lemma-rr}
...
\end{align*}
by Lemmas \ref{curves-lemma-euler} and \ref{curves-lemma-rr}, by (\ref{equation-genus}),
...
\begin{lemma}[Degree more than 2g 2]
\label{curves-lemma-degree-more-than-2g-2}
\source{stacks:0E3B}
\href{https://stacks.math.columbia.edu/tag/0E3B}{\texttt{Stacks tag 0E3B}}
\group{curves}
...
\end{reference}
In Situation \ref{curves-situation-Cohen-Macaulay-curve}
...
\begin{proof}
\uses{curves-lemma-h1-nonzero-degree-leq-2g-2, curves-lemma-genus-zero-positive-degree}

The vanishing of $H^1(X, \mathcal{I}\mathcal{L})$ follows from
Lemma \ref{curves-lemma-h1-nonzero-degree-leq-2g-2}.
...
\begin{lemma}[Degree more than 2g]
\label{curves-lemma-degree-more-than-2g}
\source{stacks:0E3C}
\href{https://stacks.math.columbia.edu/tag/0E3C}{\texttt{Stacks tag 0E3C}}
\group{curves}
...
\begin{proof}
\uses{curves-lemma-degree-more-than-2g-2, curves-lemma-duality-dim-1-CM}

Let $Z \subset X$ be the closed subscheme cut out by the
global sections of $\mathcal{L}$. By Lemma \ref{curves-lemma-degree-more-than-2g-2}
...
By Lemma \ref{curves-lemma-duality-dim-1-CM} this gives a
...
\begin{lemma}[Degree more than 2g 1 and Z]
\label{curves-lemma-degree-more-than-2g-1-and-Z}
\source{stacks:0E3D}
\href{https://stacks.math.columbia.edu/tag/0E3D}{\texttt{Stacks tag 0E3D}}
\group{curves}
...
\begin{proof}
\uses{curves-lemma-degree-more-than-2g, curves-lemma-h1-nonzero-degree-leq-2g-2}

Global generation by Lemma \ref{curves-lemma-degree-more-than-2g}.
If $\mathcal{I} \subset \mathcal{O}_X$ is the ideal sheaf
of $Z$, then $H^1(X, \mathcal{I}\mathcal{L}) = 0$ by
Lemma \ref{curves-lemma-h1-nonzero-degree-leq-2g-2}. Hence surjectivity.
\end{proof}

\begin{lemma}[Degree at least 2g+1]
\label{curves-lemma-degree-at-least-2g+1}
\source{stacks:0H2V}
\href{https://stacks.math.columbia.edu/tag/0H2V}{\texttt{Stacks tag 0H2V}}
\group{curves}
...
\begin{proof}
\uses{curves-lemma-degree-more-than-2g, curves-lemma-degree-more-than-2g-1-and-Z}

By Lemma \ref{curves-lemma-degree-more-than-2g}, $\mathcal{L}$
...
integral, by assumption. Lemma \ref{curves-lemma-degree-more-than-2g-1-and-Z}
...
\begin{lemma}[Vanishing on gorenstein]
\label{curves-lemma-vanishing-on-gorenstein}
\source{stacks:0E3E}
\href{https://stacks.math.columbia.edu/tag/0E3E}{\texttt{Stacks tag 0E3E}}
\group{curves}
...
\begin{proof}
\uses{curves-lemma-duality-dim-1-CM, curves-lemma-closed-immersion-dim-1-CM, curves-lemma-euler}

If $H^1(X, \mathcal{I}\mathcal{L})$ is nonzero, then there is a nonzero map
$\varphi : \mathcal{I}\mathcal{L} \to \omega_X$, see
Lemma \ref{curves-lemma-duality-dim-1-CM}. Let $Y \subset X$
...
$$
by Lemma \ref{curves-lemma-euler} and
Varieties, Lemma \ref{varieties-lemma-chi-tensor-finite}.
...
\begin{lemma}[Global generation]
\label{curves-lemma-global-generation}
\source{stacks:0E3F}
\href{https://stacks.math.columbia.edu/tag/0E3F}{\texttt{Stacks tag 0E3F}}
\group{curves}
...
\begin{proof}
\uses{curves-lemma-degree-more-than-2g, curves-lemma-sanity-check-duality, curves-lemma-vanishing-on-gorenstein, curves-lemma-closed-immersion-dim-1-CM}

By induction on the number of irreducible components of $X$.
If $X$ is irreducible, then the lemma holds by
Lemma \ref{curves-lemma-degree-more-than-2g}
...
Cohomology of Schemes, Lemma \ref{coherent-lemma-flat-base-change-cohomology},
Lemma \ref{curves-lemma-sanity-check-duality} and the elementary fact that
...
By Lemma \ref{curves-lemma-vanishing-on-gorenstein} we have
...
$$
\vcenter{
\begin{array}{ccccc}0 \longrightarrow & \mathcal{I}\mathcal{L} \longrightarrow\downarrow & \mathcal{L} \longrightarrow\downarrow & \mathcal{L}/\mathcal{I}\mathcal{L} \longrightarrow\downarrow & 0 \\ 0 \longrightarrow & \mathcal{I}'\mathcal{L}|_Y \longrightarrow & \mathcal{L}|_Y \longrightarrow & \mathcal{L}|_Y/\mathcal{I}'\mathcal{L}|_Y \longrightarrow & 0\end{array}
}
\quad\text{and}\quad
\vcenter{
\begin{array}{cc}\mathcal{I}\mathcal{L} \underset{\varphi}{\longrightarrow}\downarrow & \omega_X \\ \mathcal{I}'\mathcal{L}|_Y \overset{\varphi''}{\longrightarrow} & \omega_Y \uparrow\end{array}
}
$$
...
\source{stacks:0E3H}
\href{https://stacks.math.columbia.edu/tag/0E3H}{\texttt{Stacks tag 0E3H}}
\group{curves}

\uses{curves-lemma-nodal, curves-lemma-closed-subscheme-nodal-curve}
Let $k$ be a field. Let $X$ be a proper scheme over $k$ having dimension $1$
...
(Lemma \ref{curves-lemma-nodal}). There is a canonical morphism
$$
...
(Lemma \ref{curves-lemma-closed-subscheme-nodal-curve})
...
\begin{lemma}[Rational tail negative]
\label{curves-lemma-rational-tail-negative}
\source{stacks:0E63}
\href{https://stacks.math.columbia.edu/tag/0E63}{\texttt{Stacks tag 0E63}}
\group{curves}

\uses{curves-example-rational-tail}
Let $k$ be a field. Let $X$ be a proper scheme over $k$ having dimension $1$
and $H^0(X, \mathcal{O}_X) = k$. Assume the singularities of $X$ are
at-worst-nodal. Let $C \subset X$ be a rational tail
(Example \ref{curves-example-rational-tail}). Then $\deg(\omega_X|_C) < 0$.
\end{lemma}

\begin{proof}
\uses{curves-lemma-closed-subscheme-reduced-gorenstein, curves-lemma-facts-about-nodal-curves, curves-lemma-closed-subscheme-nodal-curve, curves-proposition-projective-line}
...
$$
See Lemmas \ref{curves-lemma-closed-subscheme-reduced-gorenstein},
\ref{curves-lemma-facts-about-nodal-curves}, and
\ref{curves-lemma-closed-subscheme-nodal-curve}.
With $k'$ as in the example we see that $\deg(\omega_C) = -2[k' : k]$
as $C \cong \mathbf{P}^1_{k'}$ by Proposition \ref{curves-proposition-projective-line}
and $\deg(C \cap X') = [k' : k]$.
Hence $\deg(\omega_X|_C) = -[k' : k]$ which is negative.
\end{proof}

\begin{lemma}[Rational tail field extension]
\label{curves-lemma-rational-tail-field-extension}
\source{stacks:0E3I}
\href{https://stacks.math.columbia.edu/tag/0E3I}{\texttt{Stacks tag 0E3I}}
\group{curves}
...
and $H^0(X, \mathcal{O}_X) = k$. Assume the singularities of $X$ are
at-worst-nodal. Let $C \subset X$ be a rational tail
(Example \ref{curves-example-rational-tail}).
...
Proposition \ref{curves-proposition-projective-line}.
...
\begin{lemma}[No rational tail]
\label{curves-lemma-no-rational-tail}
\source{stacks:0E3J}
\href{https://stacks.math.columbia.edu/tag/0E3J}{\texttt{Stacks tag 0E3J}}
\group{curves}
...
\begin{proof}
\uses{curves-lemma-closed-subscheme-reduced-gorenstein, curves-lemma-facts-about-nodal-curves, curves-lemma-closed-subscheme-nodal-curve}
...
Lemma \ref{curves-lemma-closed-subscheme-reduced-gorenstein}.
We will use the results of this lemma and of
Lemmas \ref{curves-lemma-facts-about-nodal-curves} and
\ref{curves-lemma-closed-subscheme-nodal-curve} without further mention.
Then we get a short exact sequence
$$
0 \to \omega_Y \to \omega_X|_Y \to \mathcal{O}_{Y \cap Z} \to 0
$$
See Lemma \ref{curves-lemma-closed-subscheme-reduced-gorenstein}.
We conclude that
$$
...
\begin{lemma}[No rational tail semiample genus geq 2]
\label{curves-lemma-no-rational-tail-semiample-genus-geq-2}
\source{stacks:0E3K}
\href{https://stacks.math.columbia.edu/tag/0E3K}{\texttt{Stacks tag 0E3K}}
\group{curves}
...
(Example \ref{curves-example-rational-tail}). If
\begin{enumerate}
...
\begin{proof}
\uses{curves-lemma-facts-about-nodal-curves, curves-lemma-no-rational-tail, curves-lemma-genus-zero, curves-lemma-genus-gorenstein, curves-lemma-degree-more-than-2g, curves-lemma-global-generation}

By Lemma \ref{curves-lemma-facts-about-nodal-curves} we find that $X$ is
...
with Lemma \ref{curves-lemma-no-rational-tail}. In the irreducible case
we see that $X$ is isomorphic to an irreducible plane conic and
$\omega_X^{\otimes -1}$ is very ample by Lemma \ref{curves-lemma-genus-zero}.
...
by Lemma \ref{curves-lemma-no-rational-tail}, the sum of these numbers is
$0$ by Lemma \ref{curves-lemma-genus-gorenstein}, and we can apply
Varieties, Lemma \ref{varieties-lemma-degree-in-terms-of-components}.
...
Lemma \ref{curves-lemma-degree-more-than-2g}.
Assume $X$ reducible.
By Lemma \ref{curves-lemma-no-rational-tail} the
inequalities of Lemma \ref{curves-lemma-global-generation}
hold for every $Y \subset X$ as in the statement, except for
$Y = X$. Analyzing the proof of Lemma \ref{curves-lemma-global-generation}
...
\begin{lemma}[Contracting rational tails]
\label{curves-lemma-contracting-rational-tails}
\source{stacks:0E3L}
\href{https://stacks.math.columbia.edu/tag/0E3L}{\texttt{Stacks tag 0E3L}}
\group{curves}
...
$$
of contractions of rational tails (Example \ref{curves-example-rational-tail})
...
\begin{proof}
\uses{curves-lemma-no-rational-tail-semiample-genus-geq-2, curves-lemma-closed-subscheme-reduced-gorenstein, curves-lemma-rational-tail-negative, curves-lemma-rational-tail-field-extension}

We proceed by contracting rational tails until there are none
left. Then we see that (1), (2), (3) hold by
Lemma \ref{curves-lemma-no-rational-tail-semiample-genus-geq-2}.
...
contracted by $f$. See Lemma \ref{curves-lemma-closed-subscheme-reduced-gorenstein}.
...
(Lemma \ref{curves-lemma-rational-tail-negative}).

\medskip\noindent
The statement on base field extensions follows from
Lemma \ref{curves-lemma-rational-tail-field-extension}. Some details omitted.
\end{proof}
...
discussed in Section \ref{curves-section-contracting-rational-tails}) of contracting
a scheme to improve positivity properties of its canonical sheaf.
...
\source{stacks:0E3M}
\href{https://stacks.math.columbia.edu/tag/0E3M}{\texttt{Stacks tag 0E3M}}
\group{curves}

\uses{curves-lemma-closed-subscheme-nodal-curve, curves-lemma-nodal, curves-lemma-complete-local-ring-pushout, curves-lemma-2-branches-delta-1}
Let $k$ be a field. Let $X$ be a proper scheme over $k$ having dimension $1$
and $H^0(X, \mathcal{O}_X) = k$. Assume the singularities of $X$ are
at-worst-nodal.
...
or $k'' = k' \times k'$ (Lemma \ref{curves-lemma-nodal}). By
Section \ref{curves-section-pushouts} there exists a pushout
$$
\begin{array}{cc}C \cap X' \longrightarrow\downarrow & X' \overset{a}{\downarrow} \\ \Spec(k') \longrightarrow & Y\end{array}
$$
...
This follows from Lemma \ref{curves-lemma-complete-local-ring-pushout}
...
Thus we see that $y \in Y$ is a node, see Lemmas \ref{curves-lemma-nodal}
and \ref{curves-lemma-2-branches-delta-1} and in particular the discussion
...
$$
\begin{array}{cccc}C \cap X' \longrightarrow\downarrow & X' \overset{a}{\downarrow}\longrightarrow & X \overset{c}{\swarrow} & C \swarrow\longleftarrow \\ \Spec(k') \longrightarrow & Y & \Spec(k') \longleftarrow & \end{array}
$$
...
\begin{lemma}[Rational bridge zero]
\label{curves-lemma-rational-bridge-zero}
\source{stacks:0E64}
\href{https://stacks.math.columbia.edu/tag/0E64}{\texttt{Stacks tag 0E64}}
\group{curves}

\uses{curves-example-rational-bridge}
Let $k$ be a field. Let $X$ be a proper scheme over $k$ having dimension $1$
and $H^0(X, \mathcal{O}_X) = k$. Assume the singularities of $X$ are
at-worst-nodal. Let $C \subset X$ be a rational bridge
(Example \ref{curves-example-rational-bridge}). Then $\deg(\omega_X|_C) = 0$.
\end{lemma}

\begin{proof}
\uses{curves-lemma-closed-subscheme-reduced-gorenstein, curves-lemma-facts-about-nodal-curves, curves-lemma-closed-subscheme-nodal-curve, curves-lemma-rr}
...
(Lemma \ref{curves-lemma-rr})
and $\deg(C \cap X') = [k'' : k] = 2[k' : k]$.
Hence $\deg(\omega_X|_C) = 0$.
\end{proof}

\begin{lemma}[Rational bridge field extension]
\label{curves-lemma-rational-bridge-field-extension}
\source{stacks:0E65}
\href{https://stacks.math.columbia.edu/tag/0E65}{\texttt{Stacks tag 0E65}}
\group{curves}
...
and $H^0(X, \mathcal{O}_X) = k$. Assume the singularities of $X$ are
at-worst-nodal. Let $C \subset X$ be a rational bridge
(Example \ref{curves-example-rational-bridge}).
For any field extension $K/k$ the base change $C_K \subset X_K$
...
\begin{lemma}[Rational bridge canonical]
\label{curves-lemma-rational-bridge-canonical}
\source{stacks:0E3N}
\href{https://stacks.math.columbia.edu/tag/0E3N}{\texttt{Stacks tag 0E3N}}
\group{curves}
...
\begin{proof}
\uses{curves-lemma-duality-dim-1-CM, curves-lemma-closed-subscheme-reduced-gorenstein, curves-lemma-genus-zero-pic}
...
Lemma \ref{curves-lemma-duality-dim-1-CM} or
Duality for Schemes, Lemma
\ref{duality-lemma-dualizing-module-proper-over-A}.
...
Thus $\omega_X|_C$ is trivial by Lemma \ref{curves-lemma-genus-zero-pic}
...
the diagram
$$
\begin{array}{ccc}\mathcal{C}_Y \underset{c^*}{\longrightarrow}\underset{H^1(Y, -)^\vee}{\searrow} &  & \mathcal{C}_X \overset{H^1(X, -)^\vee}{\swarrow} \\  & \textit{Sets} & \end{array}
$$
...
\begin{lemma}[No rational bridge ample genus geq 2]
\label{curves-lemma-no-rational-bridge-ample-genus-geq-2}
\source{stacks:0E3P}
\href{https://stacks.math.columbia.edu/tag/0E3P}{\texttt{Stacks tag 0E3P}}
\group{curves}

\uses{curves-example-rational-tail, curves-example-rational-bridge}
Let $k$ be a field. Let $X$ be a proper scheme over $k$ having dimension $1$
...
\begin{proof}
\uses{curves-lemma-genus-gorenstein, curves-lemma-closed-subscheme-reduced-gorenstein, curves-lemma-facts-about-nodal-curves, curves-lemma-closed-subscheme-nodal-curve}
...
and Lemma \ref{curves-lemma-genus-gorenstein}.
...
\begin{lemma}[Contracting rational bridges]
\label{curves-lemma-contracting-rational-bridges}
\source{stacks:0E3Q}
\href{https://stacks.math.columbia.edu/tag/0E3Q}{\texttt{Stacks tag 0E3Q}}
\group{curves}
...
(Example \ref{curves-example-rational-bridge}) until none are left.
Then $\omega_{X'}$ ample.
...
\begin{proof}
\uses{curves-lemma-no-rational-bridge-ample-genus-geq-2, curves-lemma-rational-bridge-canonical, curves-lemma-sanity-check-duality}

We proceed by contracting rational bridges until there are none
left. Then $\omega_{X'}$ is ample by
Lemma \ref{curves-lemma-no-rational-bridge-ample-genus-geq-2}.

\medskip\noindent
Denote $f : X \to X'$ the composition. By
Lemma \ref{curves-lemma-rational-bridge-canonical} and induction we see that
$f^*\omega_{X'} = \omega_X$.
...
(Lemma \ref{curves-lemma-sanity-check-duality}) and we have
...
Sections \ref{curves-section-contracting-rational-tails} and
...
the morphism of Lemma \ref{curves-lemma-contracting-rational-tails}
with the morphism of Lemma \ref{curves-lemma-contracting-rational-bridges}
we get a morphism
$$
...
Lemma \ref{curves-lemma-characterize-contraction-to-stable}
shows these conditions in fact characterize
this morphism.

\begin{lemma}[Contract gorenstein canonical]
\label{curves-lemma-contract-gorenstein-canonical}
\source{stacks:0E7P}
\href{https://stacks.math.columbia.edu/tag/0E7P}{\texttt{Stacks tag 0E7P}}
\group{curves}
...
\begin{proof}
\uses{curves-lemma-duality-dim-1, curves-lemma-rr}
...
Lemma \ref{curves-lemma-duality-dim-1} and its proof. Then
...
By Riemann-Roch (Lemma \ref{curves-lemma-rr}) we have
...
\begin{lemma}[Characterize contraction to stable]
\label{curves-lemma-characterize-contraction-to-stable}
\source{stacks:0E7Q}
\href{https://stacks.math.columbia.edu/tag/0E7Q}{\texttt{Stacks tag 0E7Q}}
\group{curves}
...
\begin{proof}
\uses{curves-lemma-contracting-rational-tails, curves-lemma-contracting-rational-bridges, curves-example-rational-tail, curves-example-rational-bridge, curves-lemma-automatic, curves-lemma-closed-subscheme-nodal-curve, curves-lemma-rational-tail-negative, curves-lemma-no-rational-tail-semiample-genus-geq-2, curves-lemma-vanishing-twist, curves-lemma-contract-gorenstein-canonical, curves-lemma-rational-bridge-canonical}

Existence: A morphism with all the properties listed exists by
combining Lemmas \ref{curves-lemma-contracting-rational-tails} and
\ref{curves-lemma-contracting-rational-bridges} as discussed in the
...
tail (Example \ref{curves-example-rational-tail}) and contraction of a
rational bridge (Example \ref{curves-example-rational-bridge}).
...
$X_y$ is Cohen-Macaulay (Lemma \ref{curves-lemma-automatic}).
...
$Y$ passing through $y$ by Lemma \ref{curves-lemma-closed-subscheme-nodal-curve}).
Then $X = X_y \cup C$ which means $g = 0$ because it is
equal to the genus of $X_y$ by the discussion in
Example \ref{curves-example-rational-tail}; a contradiction.
...
Section \ref{curves-section-curves-function-fields}). Then $D$ is
...
globally generated by Lemma \ref{curves-lemma-no-rational-tail-semiample-genus-geq-2}.
It follows that $H^1(X, \omega_X^{\otimes 3}) = 0$ by
Lemma \ref{curves-lemma-vanishing-twist}.
By Lemma \ref{curves-lemma-contract-gorenstein-canonical} applied with $m = 3$
we find that $c^*\omega_Y \cong \omega_X$.
We also have that $\omega_X = (X \to X_{n'})^*\omega_{X_{n'}}$ by
Lemma \ref{curves-lemma-rational-bridge-canonical} and induction.
...
\begin{lemma}[Tricanonical]
\label{curves-lemma-tricanonical}
\source{stacks:0E8X}
\href{https://stacks.math.columbia.edu/tag/0E8X}{\texttt{Stacks tag 0E8X}}
\group{curves}
...
\begin{proof}
\uses{curves-lemma-rational-tail-negative, curves-lemma-rational-bridge-zero, curves-lemma-contracting-rational-tails, curves-lemma-vanishing-on-gorenstein, curves-lemma-no-rational-tail, curves-lemma-rr}

Combining Varieties, Lemma
\ref{varieties-lemma-ampleness-in-terms-of-degrees-components} and
Lemmas \ref{curves-lemma-rational-tail-negative} and \ref{curves-lemma-rational-bridge-zero}
we see that $X$ contains no rational tails or bridges.
Then we see that $\omega_X^{\otimes 3}$ is globally generated
by Lemma \ref{curves-lemma-contracting-rational-tails}.
...
To do this we will use Lemma \ref{curves-lemma-vanishing-on-gorenstein}.
...
$$
which is true by Lemma \ref{curves-lemma-no-rational-tail}
...
$Y = X$ we can use Riemann-Roch (Lemma \ref{curves-lemma-rr})
...
\begin{lemma}[Smooth vector fields]
\label{curves-lemma-smooth-vector-fields}
\source{stacks:0E67}
\href{https://stacks.math.columbia.edu/tag/0E67}{\texttt{Stacks tag 0E67}}
\group{curves}
...
\begin{proof}
\uses{curves-lemma-duality-dim-1, curves-lemma-rr, curves-lemma-uniformizer-works}
...
\begin{lemma}[Nodal vector fields]
\label{curves-lemma-nodal-vector-fields}
\source{stacks:0E68}
\href{https://stacks.math.columbia.edu/tag/0E68}{\texttt{Stacks tag 0E68}}
\group{curves}
...
\begin{proof}
\uses{curves-definition-split-node, curves-lemma-split-node, curves-lemma-multicross}
...
Definition \ref{curves-definition-split-node} and
Lemma \ref{curves-lemma-split-node}.)
...
$$
This follows from Lemma \ref{curves-lemma-multicross}.
...
\begin{lemma}[Stable vector fields]
\label{curves-lemma-stable-vector-fields}
\source{stacks:0E69}
\href{https://stacks.math.columbia.edu/tag/0E69}{\texttt{Stacks tag 0E69}}
\group{curves}
...
\begin{proof}
\uses{curves-lemma-nodal-vector-fields, curves-lemma-smooth-vector-fields}

Let $D \in \text{Der}_k(\mathcal{O}_X, \mathcal{O}_X)$.
Let $X^\nu$ be the normalization of $X$.
Let $D' \in \text{Der}_k(\mathcal{O}_{X^\nu}, \mathcal{O}_{X^\nu})$
be the element corresponding to $D$ via Lemma \ref{curves-lemma-nodal-vector-fields}.
Let $C \subset X^\nu$ be an irreducible component.
If the genus of $C$ is $> 1$, then $D'|_{\mathcal{O}_C} = 0$
by Lemma \ref{curves-lemma-smooth-vector-fields} part (1).
...
Lemma \ref{curves-lemma-smooth-vector-fields} part (2).
...
Lemma \ref{curves-lemma-smooth-vector-fields} part (3).
\end{proof}

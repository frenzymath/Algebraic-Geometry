...
\label{sec:cm_equidim}

The transport step of Milne's Lemma 3.3 (\S\ref{sec:milne33_transport}) rests on a
...
\begin{proof}
  ...
  (\ref{thm:depth_ses}) bounds the depth of the middle term by
...
\begin{theorem}[Ischebeck's bound]
  ...
  \uses{def:depth, lem:depth_quotSMulTop, lem:ringKrullDim_quotient_eq_coheight}
  \leanok
  Let \((R, \mathfrak m)\) be a Noetherian local ring, \(M\) a finite nontrivial
...
\begin{proof}
  ...
  By \ref{lem:depth_quotSMulTop}, \(n \le \operatorname{depth}(M/xM)\), so the
...
\begin{theorem}[Cohen--Macaulay unmixedness]
  ...
  \uses{thm:ischebeck, def:cohen_macaulay, lem:ringKrullDim_quotient_eq_coheight}
  \leanok
...
\begin{proof}
  ...
  (\ref{def:cohen_macaulay}); and \(\operatorname{depth} R \le
  \operatorname{coheight}(\mathfrak p)\) by Ischebeck's bound (\ref{thm:ischebeck})
  applied to \(M = R\) (finite, nontrivial, with \(\mathfrak p \in
  \operatorname{Ass} R\)). Equality follows, and the quotient form is
  \ref{lem:ringKrullDim_quotient_eq_coheight}.
\end{proof}
...
\begin{proof}
  ...
  \ref{thm:cm_unmixed}); since \(\operatorname{coheight}(\mathfrak q) \le \dim R\)
...
\begin{theorem}[Equidimensionality at a height-one prime]
  ...
  \uses{thm:ischebeck, thm:cm_ass_minimal, lem:depth_quotSMulTop, def:cohen_macaulay,
    lem:ringKrullDim_quotient_eq_coheight}
  \leanok
...
\begin{proof}
  \leanok
  By \ref{lem:ringKrullDim_quotient_eq_coheight} it suffices to prove
  ...
  \ref{thm:cm_ass_minimal}, minimal. Now \(\mathfrak p\) is not contained in any
  ...
  (\ref{thm:ischebeck}) for the module \(R/(y)\) gives
  \(\operatorname{depth}_R(R/(y)) \le \operatorname{coheight}(\mathfrak p)\), and
  the depth drop (\ref{lem:depth_quotSMulTop}) for the \(R\)-regular \(y\) gives
...
\begin{lemma}[Equal-dimension surjection has trivial kernel]
  ...
  \uses{thm:regular_isDomain, lem:coheight_eq_dim_minimal,
    lem:ringKrullDim_quotient_eq_coheight}
  \leanok
...
\begin{proof}
  ...
  prime. By \ref{lem:ringKrullDim_quotient_eq_coheight},
  ...
  rings). By \ref{lem:coheight_eq_dim_minimal}, \(\ker\varphi\) is a minimal
  prime of \(B\). Since \(B\), being regular local, is a domain
  (\ref{thm:regular_isDomain}), its only minimal prime is \(0\); hence
  \(\ker\varphi = 0\).
\end{proof}
...
\begin{theorem}[Kernels of surjections of regular local rings]
  ...
  \uses{thm:regular_isDomain, lem:regular_quotient, lem:ker_eq_bot_equal_dim,
    lem:exists_mem_ker_notMemSq}
  \leanok
  Let \(\varphi : B \twoheadrightarrow A\) be a surjection of regular local
...
\begin{proof}
  ...
  \(\dim A = \dim B\), then \(\ker\varphi = 0\) by \ref{lem:ker_eq_bot_equal_dim}
  and the empty list works. Otherwise \(\dim A < \dim B\); by
  \ref{lem:exists_mem_ker_notMemSq} choose \(x \in \ker\varphi \setminus
  \mathfrak m_B^2\). The quotient \(B/(x)\) is again regular local, of dimension
  \(\dim B - 1\) (\ref{lem:regular_quotient}), and \(\varphi\) descends to a
...
\begin{proof}
  ...
  (\ref{thm:cm_height_one}), \(\dim R = \dim(R/J) + 1 = \dim C + 1 \le r + 1\).
\end{proof}
...
\begin{theorem}[Transported codimension count]
  ...
  \uses{thm:ker_gen_regular_local, thm:krull_codim_count}
  \leanok
...
\begin{proof}
  \leanok
  By kernel generation (\ref{thm:ker_gen_regular_local}), write
  ...
  (\ref{thm:krull_codim_count}) gives \(\dim B' \le r + 1\); cancelling the
...
(\ref{def:difference_map}) with maximal representative \(\Phi_0\),
...
(substep~4b) built on \S\ref{sec:cm_equidim} and the polar purity of
\S\ref{sec:pole_purity}.
...
\begin{lemma}[Representative agreement of the difference map]
  ...
  \uses{def:difference_map, def:difference_pairing, def:grp_scheme_diff}
  \leanok
...
\begin{proof}
  \leanok
  The pairing representative (\ref{def:difference_pairing}) represents \(\Phi\):
  its class is \((f \circ \mathrm{pr}_1,\ f \circ \mathrm{pr}_2)\), whose
  post-composition with \(\delta_G\) is \(\Phi\) (\ref{def:difference_map}). Two
...
\begin{lemma}[The evaluation formula]
  ...
  \uses{lem:diff_representative_agreement, def:difference_pairing, def:grp_scheme_diff}
  \leanok
  \dcref{custom}
...
\begin{proof}
  ...
  (\ref{lem:diff_representative_agreement}) to \(j\) and \(q\) gives \(j \circ
...
\begin{theorem}[Diagonal triviality]
  ...
  \uses{lem:diff_evaluation_formula, def:grp_scheme_diff}
  \leanok
  \dcref{custom}
...
\begin{proof}
  \leanok
  Apply the evaluation formula (\ref{lem:diff_evaluation_formula}) with
...
\begin{lemma}[The diagonal maps into the domain of the difference map]
  ...
  \uses{thm:difference_map_domain}
  \leanok
...
\begin{theorem}[The generic image specialises to the unit]
  ...
  \uses{thm:diagonal_triviality, lem:diff_diagonal_domain}
  \leanok
  \dcref{custom}
...
\begin{proof}
  ...
  (\ref{lem:diff_diagonal_domain}) with \(\Phi_0(\delta(z)) = e\)
  (\ref{thm:diagonal_triviality}). The specialisation \(P \rightsquigarrow
...
\begin{theorem}[The row argument: substep 2, hard direction]
  ...
  \uses{lem:diff_evaluation_formula, lem:grp_scheme_diff_reconstruct,
    lem:jacobson_closed_points}
  \leanok
  \dcref{custom}
...
\begin{proof}
  ...
  irreducible) and applying Jacobson density (\ref{lem:jacobson_closed_points})
  ...
  (\ref{lem:diff_evaluation_formula}) gives \(\Phi_0(v, u) = f(v)\,f(u)^{-1}\)
  there; the scheme-level reconstruction identity
  (\ref{lem:grp_scheme_diff_reconstruct}), \((f(v)\,f(u)^{-1})\,f(u) = f(v)\),
...
\begin{theorem}[The germ-range criterion: substep 3]
  ...
  \uses{thm:spreading_criterion_stalk}
  \leanok
  \dcref{custom}
...
\begin{proof}
  ...
  check the hypotheses of \ref{thm:spreading_criterion_stalk}: (i) the generic
  ...
  \Cref{thm:spreading_criterion_stalk} gives \(P \in \operatorname{Dom} F\).
\end{proof}
...
\begin{theorem}[Pole existence where the map is undefined]
  ...
  \uses{thm:germ_range_criterion, thm:pole_purity}
  \leanok
  \dcref{custom}
...
\begin{proof}
  \leanok
  By the contrapositive of \ref{thm:germ_range_criterion}, some \(s\) has
  \(\Lambda s \notin \operatorname{im}(\struct{Y,P} \to K(Y))\) --- a pole of
  \(\Lambda s\) at \(P\). By polar purity (\ref{thm:pole_purity}) the pole
...
\begin{theorem}[The transport kernel]
  ...
  \uses{lem:chart_image_point, thm:transport_codim_count, lem:regular_at_generisation}
  \leanok
  \dcref{custom}
...
\begin{proof}
  ...
  (\ref{lem:chart_image_point}); moreover \(z \rightsquigarrow x_0\) (as
  ...
  (\ref{thm:transport_codim_count}) gives \(\dim\struct{X,z} \le 1\). Finally,
  \(h\) non-regular at \(w\) and \(w \rightsquigarrow \tilde z\) force \(h\)
  non-regular at \(\tilde z = g(z)\) (\ref{lem:regular_at_generisation},
  contrapositive).
\end{proof}
...
\begin{theorem}[The codimension-one transport: substep 4b]
  ...
  \uses{thm:row_section, thm:pole_existence, thm:transport_kernel,
    lem:diff_diagonal_domain, thm:diagonal_triviality, thm:germ_range_of_domain,
    thm:generic_image_specializes_unit, lem:generic_value_bridge,
    lem:coheight_strictness, thm:self_product_integral, thm:smooth_stalk_regular}
  \leanok
  \dcref{custom}
...
\begin{proof}
  ...
  stalks of \(Y\) and \(X\) are regular (\ref{thm:smooth_stalk_regular}), \(Y\)
  is integral (\ref{thm:self_product_integral}), and \(\delta\) is a closed
  immersion (\(X\) is separated over \(\bar k\)).

  \emph{Front end.} If \(\delta(x_0) \in \operatorname{Dom}\Phi\) then \(x_0 \in
  \operatorname{Dom} f\) (\ref{thm:row_section}, contrapositive); so
  \(\delta(x_0) \notin \operatorname{Dom}\Phi\). Take an affine open \(V \ni e\)
  at the unit point. The generic image \(\gamma = \Phi(\eta_Y)\) lies in \(V\):
  \(\gamma \rightsquigarrow e\) (\ref{thm:generic_image_specializes_unit}, with
  the generic-value identification \ref{lem:generic_value_bridge}) and \(V\) is
  open about \(e\). By pole existence (\ref{thm:pole_existence}), some section
  \(s\) of \(V\) has generic germ pullback with a pole at a coheight-one
  \(w \rightsquigarrow \delta(x_0)\).

  \emph{Transport.} The transport kernel (\ref{thm:transport_kernel}) moves the
  pole to \(\delta(z)\) for some \(z \rightsquigarrow x_0\) with
  \(\dim\struct{X,z} \le 1\).

  \emph{Back end.} If \(f\) were defined at \(z\), then \(\delta(z) \in
  \operatorname{Dom}\Phi\) (\ref{lem:diff_diagonal_domain}) with \(\Phi(\delta(z))
  = e \in V\) (\ref{thm:diagonal_triviality}), so the germ pullback of \(s\)
  would be regular at \(\delta(z)\) (\ref{thm:germ_range_of_domain}) ---
  ...
  \(\operatorname{coheight}(z) \ge 1\) (\ref{lem:coheight_strictness}).
\end{proof}
...
\begin{theorem}[Milne's Lemma 3.3]
  ...
  \uses{def:milne33_indeterminacy, def:indeterminacy_locus, thm:transport_assembly,
    lem:jacobson_closed_points, lem:coheight_strictness}
  \leanok
  \dcref{custom}
  Let \(\bar k\) be an algebraically closed field, \(X\) a nonsingular
...
\begin{proof}
  ...
  (\ref{lem:jacobson_closed_points}) pick a point \(x_0 \in C\), closed in \(X\),
  avoiding the other components; then \(x_0 \in Z(f)\).

  The codimension-one transport (\ref{thm:transport_assembly}) yields \(z \in
  ...
  \(\operatorname{coheight}(\zeta) \ge 1\) (\ref{lem:coheight_strictness});

...
\begin{proof}
  \leanokChoose a representative \((U, \varphi_U)\) of \(f\); its underlying morphism
...
\end{lemma}
\begin{proof}
  \leanokBoth sides act on a chosen partial-map representative \((V, \varphi_V)\) of \(f\) by
...
\end{lemma}
\begin{proof}
  \leanokBoth sides are everywhere-defined; on the whole of \(W\) the left-hand side is
...
\end{lemma}
\begin{proof}
  \leanokRight-composition with a morphism is functorial on generic-point morphisms:
...
\end{lemma}
\begin{proof}
  \leanokThis is the triangle identity \(\varphi_{\mathrm{left}} \circ g_{\mathrm{hom}} =
...
\end{lemma}
\begin{proof}
  \leanokBy associativity of right-composition, \(\Phi \circ (G \to \operatorname{Spec}\bar k) =
...
\end{lemma}
\begin{proof}
  \leanokA partial-map representative \((V, F_V)\) of \(F\) yields the representative
...
\end{lemma}
\begin{proof}
  \leanokRight-composition with \(\operatorname{diff}_G\) only enlarges the domain of
...
\end{lemma}
\begin{proof}
  \leanokIf \(p(w) \in \mathrm{Dom}(f)\), choose a partial-map representative \((V, f_V)\) of
...
\end{lemma}
\begin{proof}
  \leanokThe maximal representatives of \(a\) and \(b\) are defined on \(\mathrm{Dom}(a)\),
...
\end{lemma}
\begin{proof}
  \leanokPrecomposition by \(\langle a, b\rangle\) is a group homomorphism on \(T\)-points, so
...
\end{lemma}
\begin{proof}
  \leanokThe forgetful functor \((-)^{\mathrm{left}} \colon \mathbf{Sch}/\bar k \to \mathbf{Sch}\)
...
\end{lemma}
\begin{proof}
  \leanokBoth sides are \(\bar k\)-rational maps out of the integral scheme \(X \times_{\bar k}
...
\end{lemma}
\begin{proof}
  \leanokRewriting \(f \circ \mathrm{pr}_1\) by
...
\end{theorem}
\begin{proof}
  \leanokFix an indeterminacy point \(z \in Z(f)\); we must show its coheight is \(\geq 2\).
...
\end{lemma}
\begin{proof}
  \leanokThe fibre \(\mathrm{pr}_1^{-1}\{x\}\) is the base change
...
\end{lemma}
\begin{proof}
  \leanokThe principal ideal \((t)\) is contained in \(\mathfrak p\), so it has a
...
\end{lemma}
\begin{proof}
  \leanokImmediate from functoriality of stalks along the specialisation map: for
...
\end{lemma}
\begin{proof}
  \leanok\uses{mk-lem:regular_stalk_propagates_to_generisation}
...
\end{lemma}
\begin{proof}
  \leanok\uses{mk-def:rationalMap_precomp, mk-lem:rationalMap_precomp_compHom,
